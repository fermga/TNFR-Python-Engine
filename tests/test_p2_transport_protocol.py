...
def _run(
    ...
    *,
    ...
    with localcontext() as context:
        ...
        shape = [
            amplitude * (-2 * rate * Decimal.from_float(time)).exp() for time in times
        ]
        samples = (
    ...
    return NodalMeasurementRun(
        ...
        timestamps=times,
        samples=samples,
        value_unit="fixture_temperature_unit",
        time_unit="s",
...
def _forecast(calibration, observation=None):
    observation = (
        _run("reserved", mean="2", amplitude="-0.5")
        if observation is None
        else observation
    )
    return p.forecast_p2_transport(
        calibration,
        evaluation_run_id=observation.run_id,
...
def test_continuous_capacity_and_each_forecast_channel_are_enclosed(amplitude):
    ...
    for index, sample in enumerate(forecast.tube.samples):
        ...
        represented_contrast = Fraction.from_float(
            reserved.samples[0][index]
...
def test_continuous_fit_does_not_silently_reuse_biased_finite_euler_fit():
    ...
    discrete = calibrate_nodal_prediction(
        ...
        support_provenance="known fixture graph",
        measurement_provenance="fixture mapping",
    )
...
def test_only_declared_calibration_endpoints_constrain_capacity():
    ...
    changed = replace(
        ...
        samples=tuple(
            tuple(
                value if index in (0, len(row) - 1) else value + 10
...
def test_multiple_calibration_runs_intersect_outer_capacity_constraints():
    calibration = _calibrate(
        [_run(), _run("calibration-2", mean="3", amplitude="-1.1")]
    )
    _contains(calibration.capacity, Fraction(2, 5))
...
def test_clock_uncertainty_is_propagated_instead_of_refitted():
    calibration = _calibrate(
        measurement=_measurement(structural_time_per_unit=(0.9, 1.1))
    )
...
def test_sensor_scale_and_offset_map_precedes_the_epi_error_bound():
    ...
    forecast = _forecast(calibration, reserved)
    assert (
        _score(forecast, calibration, reserved).status == "not_falsified_by_enclosures"
    )
...
def test_mean_drift_rejects_even_when_the_contrast_is_unchanged():
    ...
    shifted = replace(
        ...
        samples=tuple(
            tuple(
                value if index == 0 else value + 0.01 for index, value in enumerate(row)
            )
            for row in observed.samples
        ),
    )
    assert (
        _score(forecast, calibration, shifted).status
...
def test_writer_preserves_exact_rational_endpoints_and_does_not_overwrite(tmp_path):
    ...
    for original, encoded in zip(
        forecast.tube.samples, payload["forecast"]["tube"]["samples"]
    ):
...
def test_reserved_initialization_cannot_change_after_forecast():
    ...
    forecast = _forecast(calibration, observed)
    changed = replace(
        observed, samples=tuple((row[0] + 1,) + row[1:] for row in observed.samples)
    )
    with pytest.raises(ValueError):
...
def test_renamed_subwindow_of_calibration_retains_acquisition_overlap():
    ...
    renamed = replace(
        original,
        run_id="renamed",
        timestamps=original.timestamps[1:],
...
def test_contrast_sign_change_and_calibration_mean_shift_are_not_admitted():
    ...
    shifted = replace(
        original,
        samples=tuple(row[:-1] + (row[-1] + 0.01,) for row in original.samples),
    )
    with pytest.raises(ValueError):
        _calibrate([shifted])
...
@pytest.mark.parametrize(
    "kind", ["directed", "loop", "disconnected", "third-node", "zero", "negative"]
)
def test_only_declared_passive_two_node_graph_domain_is_accepted(kind):
    ...
    if kind != "disconnected":
        graph.add_edge(
            "left",
            "right",
            weight=-1 if kind == "negative" else (0 if kind == "zero" else 1),
        )
    if kind == "loop":
...
@pytest.mark.parametrize(
    ...
    [
        {"epi_error": (-1e-10, 0)},
        {"epi_error": (float("nan"), 0)},
        {"epi_error": (True, 0)},
        {"timestamp_error": -0.1},
        {"timestamp_error": float("inf")},
