...
def _alternating_bounds(denominator, count):
    terms = tuple(Fraction(1, (2 * k + 1) * denominator ** (2 * k + 1))
                  for k in range(count + 1))
    partial = sum((term if k % 2 == 0 else -term for k, term in enumerate(terms[:-1])), Fraction(0))
...
def test_reciprocal_arctangent_enclosure_uses_the_signed_first_omitted_term(denominator, count):
    ...
    assert actual[1] - actual[0] == Fraction(1, (2 * count + 1) * denominator ** (2 * count + 1))
...
@pytest.mark.parametrize("first,second,expected_mean", [
    (1.0, 1.0 + 2.0**-52, 1.0),
...
@pytest.mark.parametrize("center_n,first_n,second_n,mean_n,delta_n,negative_zero", [
    (0, 0, 1, 0, 0, False),
    (0, 1, 2, 2, 2, False),
    (2, 1, 2, 2, 0, True),
    (4, 0, 3, 2, -2, False),
...
def test_subnormal_midpoints_round_once_including_negative_underflow(
    ...
    tiny = math.ulp(0.0)
    result = certified_two_neighbor_phase(center_n * tiny, first_n * tiny, second_n * tiny)
    assert result is not None
...
@pytest.mark.parametrize("center,first,second,mean_coefficient,delta_coefficient", [
...
def test_symbolic_pi_handles_both_wrap_directions_and_two_crossing_neighbors(
    ...
    for rational, coefficient, enclosure in (
        ...
        true_bounds = sorted((rational + coefficient * independent_low,
...
@pytest.mark.parametrize("center,first,second", [
    (1.0, 0.75, 1.25),
...
def test_undecided_irrational_rounding_uses_fallback(monkeypatch):
    monkeypatch.setattr(owner, "_pi_bounds", lambda: (Fraction(314, 100), Fraction(315, 100)))
    assert certified_two_neighbor_phase(0.0, 0.25, math.tau - 0.25) is None


@pytest.mark.parametrize("position", [0, 1, 2])
@pytest.mark.parametrize("value", [True, 1, Fraction(1, 2), "0.5", None,
                                   math.nan, math.inf, -math.inf, -0.1, math.tau,
