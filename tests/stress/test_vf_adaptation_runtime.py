...
def test_adapt_vf_by_coherence_large_graph_consistent_across_workers() -> None:
    ...
    serial_sum = sum(
        float(get_attr(serial_graph.nodes[node], ALIAS_VF, 0.0)) for node in serial_graph.nodes
    )
    parallel_sum = sum(
        float(get_attr(parallel_graph.nodes[node], ALIAS_VF, 0.0)) for node in parallel_graph.nodes
    )
    ...
    serial_counts = Counter(
        int(serial_graph.nodes[node].get("stable_count", -1)) for node in serial_graph.nodes
    )
    parallel_counts = Counter(
        int(parallel_graph.nodes[node].get("stable_count", -1)) for node in parallel_graph.nodes
    )
