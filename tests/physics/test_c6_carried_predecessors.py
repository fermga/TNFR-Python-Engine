...
def _state(point=ZERO, band=(.375, .625)):
    exact = tuple(F(LOW) + QUANTUM * index for index in point)
    visible = tuple(map(float, exact))
    return NodalRemainderState(visible, tuple(value - F(x) for value, x in zip(exact, visible, strict=True)), *band)
...
def _derive(reference, target=TARGET, *, state=None, **kwargs):
    return owner.derive_c6_carried_predecessors(
        reference, state=_state() if state is None else state, target=_state(target),
        epi_states=ROWS, timestep=1., max_depth=kwargs.pop('max_depth', 6), **kwargs,
    )
...
def _replay(reference, path):
    for first, second in zip(path, path[1:]):
        ...
        step = advance_nodal_remainder(first, timestep=1., capacity=(1.,) * 6, pressure=pressure)
...
def test_complete_predecessor_frontiers_match_all_4096_physical_states(inverse, complete):
    frontier = {TARGET}
    expected = []
    while frontier:
    ...
    expected.append(set())
    observed = tuple({_point(state) for state in layer.states} for layer in complete.layers)
...
def test_every_predecessor_binds_its_exact_successor_and_retained_carry(reference, complete):
    for depth, layer in enumerate(complete.layers[1:], 1):
        assert layer.depth == depth and len(layer.states) == len(layer.successor_indices)
        previous = complete.layers[depth - 1].states
        assert len(set(layer.states)) == len(layer.states)
        for state, target_index in zip(layer.states, layer.successor_indices, strict=True):
...
def test_pointwise_past_exclusion_never_promotes_to_a_domain_or_actual_origin_theorem(complete):
...
def test_no_predecessor_is_an_exact_one_layer_absence(reference, inverse):
    ...
    assert result.past_exclusion_depth == 1 and result.maximum_compatible_past_depth == 0
    assert result.row_checks == 64
    assert result.witness_path == (result.target,)


def test_incomplete_frontier_is_discarded_even_if_its_last_candidate_is_missing(reference, complete):
    ...
    assert result.past_exclusion_depth is None and result.maximum_compatible_past_depth is None
...
def test_resource_guard_before_first_complete_layer_keeps_only_the_target(reference):
    ...
    assert result.frontier_counts == (1,) and result.layers[0].states == (result.target,)
    assert result.past_exclusion_depth is None


def test_depth_query_does_not_turn_a_nonempty_frontier_into_exclusion(reference, complete):
...
def test_exact_two_cycle_stays_a_finite_depth_query_and_preserves_origin_path(reference):
    origin = _state((0, 2) * 3)
    target = (2, 0) * 3
    ...
    assert 1 in result.origin_reachability_depths and 3 in result.origin_reachability_depths
...
def test_explicit_subcell_domain_removes_predecessors_without_changing_pressure(reference, complete):
...
def test_coordinate_cosets_are_stricter_than_the_global_common_grid(reference):
    ...
    assert target.epi == origin.epi and target.exact_epi[1] - origin.exact_epi[1] == QUANTUM
    with pytest.raises(ValueError, match='coset|lattice|spacing'):
        owner.derive_c6_carried_predecessors(
            reference, state=origin, target=target, epi_states=rows, timestep=1., max_depth=1,
        )


def test_tiny_carry_change_cannot_silently_change_the_exact_target_coset(reference):
    target = _state(TARGET)
    changed = replace(target, remainder=(target.remainder[0] + F(1, 2**3222),) + target.remainder[1:])
    assert changed.epi == target.epi
    with pytest.raises(ValueError, match='coset|lattice|spacing'):
        owner.derive_c6_carried_predecessors(
            reference, state=_state(), target=changed, epi_states=ROWS, timestep=1., max_depth=1,
        )
...
@pytest.mark.parametrize('field,value', (('max_row_checks', 0), ('max_row_checks', True), ('max_cells', 0)))
...
def test_domain_cannot_expand_beyond_its_nearest_even_source_cell(reference):
    ...
    bounds = tuple(tuple(0 if i == j else high[i] - low[j] for j in range(7)) for i in range(7))
...
def test_target_band_must_match_the_source_state_band(reference):
    target = replace(_state(TARGET), epi_lower=.4)
    with pytest.raises(ValueError, match='band'):
        owner.derive_c6_carried_predecessors(
            reference, state=_state(), target=target, epi_states=ROWS, timestep=1., max_depth=1,
        )


def test_reference_caches_are_rebuilt_and_selected_path_uses_fresh_canonical_pressure(reference, complete, monkeypatch):
    altered = replace(reference, sources=(F(99),) * 6, epi_quantum=F(3), gradient_quantum=F(7), rows=())
    calls = []
...
def test_exhausted_past_still_records_a_real_origin_path_at_depth_one(reference, complete):
...
def test_origin_found_only_in_an_unfinished_frontier_is_not_certified(reference, complete):
    origin = complete.layers[1].states[0]
