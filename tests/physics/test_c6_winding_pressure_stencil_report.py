...
INPUTS = tuple(DIRECTORY / name for name in (
    "c6_winding_pressure_levels.json", "c6_winding_pressure_repayment.json",
    "c6_winding_pressure_sign.json", "c6_winding_carry_itinerary.json", "c6_winding_pressure_lattice.json",
))
...
def test_eight_boundary_ceiling_censors_the_sign_question_after_eighteen_steps(report, parents):
    assert report["source"]["retained_B30_report_replayed"] is True
    assert campaign._payload(report["source"]["inherited_state"]) == parents[0]["continuation"]["endpoint"]
    assert report["analysis_limits"] == {
        "boundary_budget": 8, "step_budget": 256, "maximum_boundaries": 8, "maximum_steps": 256,
    }
    ...
    assert report["continuation"]["step_count"] == 18 and report["continuation"]["boundary_count"] == 8
    rows = report["boundaries"]
    assert tuple(row["step_count"] for row in rows) == (1, 5, 1, 4, 1, 3, 2, 1)
    assert tuple(row["cumulative_steps"] for row in rows) == (1, 6, 7, 11, 12, 15, 17, 18)
    assert tuple(_offsets(row["endpoint"]) for row in rows) == (
        (-2, 0, 2, -2, 4, -3), (-2, 0, 0, -2, 4, -3), (-2, 0, 2, -2, 4, -3),
        (-2, 0, 0, -2, 4, -3), (-2, 0, 2, -2, 4, -3), (-2, 0, 2, -2, 4, -4),
        (-2, 0, 0, -2, 4, -4), (-2, 0, 2, -2, 4, -4),
    )


def test_neighbor_five_changes_before_the_selected_center_horizon_and_keeps_pressure_positive(report):
    changed = report["first_stencil_change"]
    assert report["target_node"] == 4 and report["stencil_nodes"] == (3, 4, 5)
    assert (changed["step"], changed["boundary"], changed["changed_nodes"]) == (15, 6, (5,))
    ...
    assert F(changed["pressure_before"]) == before > after == F(changed["pressure_after"]) > 0
    assert tuple(row["changed_stencil_nodes"] for row in report["boundaries"]) == ((), (), (), (), (), (5,), (), ())
    assert tuple(row["node4_gradient_after"] for row in report["boundaries"]) == (-13,) * 5 + (-14,) * 3
    steps = report["continuation"]["steps"]
    assert tuple(F(step["pressure"][4]) for step in steps) == (before,) * 15 + (after,) * 3
...
def test_initial_and_endpoint_stencil_bounds_are_separately_conditioned(report):
    initial, endpoint = report["initial_frozen_stencil"], report["endpoint_frozen_stencil"]
    assert initial["stencil"] == endpoint["stencil"] == (3, 4, 5)
    assert (initial["initial_max_frozen_steps"], initial["initial_first_exit_bound"]) == (20, 21)
    assert (initial["uniform_max_frozen_steps"], initial["uniform_first_exit_bound"]) == (43, 44)
    assert (endpoint["initial_max_frozen_steps"], endpoint["initial_first_exit_bound"]) == (3, 4)
    assert (endpoint["uniform_max_frozen_steps"], endpoint["uniform_first_exit_bound"]) == (49, 50)
    ...
    for owner in (initial, endpoint):
        ...
        assert owner["uniform_max_frozen_steps"] == owner["center_cell_width"] // owner["exact_increment"]
        assert owner["graph_provenance_certified"] is owner["sign_hit_certified"] is owner["positive_band_exit_certified"] is False
    assert report["continuation"]["step_count"] == 18


def test_integer_cut_distances_are_necessary_and_not_a_future_trajectory(report):
    initial, endpoint = report["initial_nonpositive_cut"], report["endpoint_nonpositive_cut"]
    ...
    assert initial["future_feasible_transition_certified"] is endpoint["future_feasible_transition_certified"] is False
    ...
    assert gradient["gradient_index_change"] == tuple(a + b for a, b in zip(
        gradient["nodal_gradient_term"], gradient["remainder_gradient_term"], strict=True,
    ))
    assert gradient["gradient_index_change"][4] == -1 > initial["necessary_gradient_change_upper_bound"]
...
def test_all_six_area_and_carry_identities_keep_the_B27_reference(report, parents):
    ...
    assert incoming == tuple(F(x) + r - F(y) - s for x, r, y, s in zip(
        initial["epi"], initial["remainder"], baseline["epi"], baseline["remainder"], strict=True,
    ))
    ...
    for step, prefix in zip(continuation["steps"], continuation["net_prefixes"], strict=True):
        ...
        assert prefix["identity_residual"] == prefix["display_carry_identity_residual"] == (0,) * 6
        for i in range(6):
        ...
        previous = step["after"]
    assert previous == continuation["endpoint"] and tuple(local) == continuation["local_nodal_area"]
    ...
    assert all(prefix["local_nodal_area"][4] > 0 for prefix in continuation["net_prefixes"])
    assert report["mean_nodal_area_compensated"] is report["total_nodal_area_compensated"] is False


def test_visible_stasis_at_node_four_does_not_reset_carry_or_close_the_itinerary(report):
    ...
    assert endpoint["remainder"][4] - initial["remainder"][4] == continuation["local_nodal_area"][4] > 0
    itinerary = continuation["itinerary"]
    assert itinerary["feasible"] is True and itinerary["zero_initial_carry_feasible"] is False
    assert itinerary["visible_closed"] is itinerary["conditional_carried_cycle"] is False
...
@pytest.mark.parametrize("boundary_budget,step_budget,count,calls,reason,changed", (
...
def test_smaller_budgets_stop_before_an_oversized_boundary(parents, monkeypatch, boundary_budget, step_budget, count, calls, reason, changed):
    ...
    result = campaign.analyze_c6_winding_pressure_stencil(*parents, boundary_budget=boundary_budget, step_budget=step_budget)
    assert result["stop_reason"] == reason
    assert result["continuation"]["step_count"] == sum(observed) == count and len(observed) == calls
    ...
    if result["censor"] is not None:
        assert result["censor"]["horizon"]["first_exit_step"] > result["censor"]["remaining_step_budget"]


@pytest.mark.parametrize("kwargs", ({"step_budget": 0}, {"step_budget": True}, {"step_budget": 257},
                                    {"boundary_budget": 0}, {"boundary_budget": 9}))
def test_invalid_limits_fail_before_parent_replay(parents, monkeypatch, kwargs):
    def forbidden(*args):
        raise AssertionError("invalid resource limits must fail before historical replay")
...
@pytest.mark.parametrize("change", ("claim", "scope", "carry", "area", "baseline", "pressure", "prefix", "cached_bound", "flag", "lineage", "source"))
def test_source_tampering_fails_full_replay_validation(parents, change):
    ...
    with pytest.raises(ValueError):
        campaign.analyze_c6_winding_pressure_stencil(parent, previous, ancestor, earlier, source)


def test_exactly_eight_new_boundaries_are_run_without_a_graph_word(parents, monkeypatch):
...
def test_scope_and_finite_serialization_preserve_the_censored_outcome(report):
    for flag in ("runtime_executed", "live_provenance_certified", "original_tail_reachability_certified",
                 "future_pressure_sign_exit_certified", "future_compensation_cycle_certified", "positive_band_exit_certified"):
...
def _cli(monkeypatch, paths, output):
    monkeypatch.setattr(sys, "argv", ["stencil", "--input", str(paths[0]), "--parent-input", str(paths[1]),
                                      "--previous-input", str(paths[2]), "--earlier-input", str(paths[3]),
                                      "--source-input", str(paths[4]), "--output", str(output)])


def test_cli_retains_all_five_input_hashes_and_distinct_output_source(tmp_path, monkeypatch, report):
    ...
    monkeypatch.setattr(campaign, "analyze_c6_winding_pressure_stencil", lambda *args: deepcopy(report))
    ...
    for key, source in zip(("input_evidence", "parent_input_evidence", "previous_input_evidence", "earlier_input_evidence", "source_input_evidence"), INPUTS, strict=True):
...
def test_cli_rejects_wrong_historical_bytes_before_replay(tmp_path, monkeypatch, changed_index):
    paths = tuple(tmp_path / f"source_{i}.json" for i in range(5))
