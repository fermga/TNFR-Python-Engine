...
def _profile(*, actual=False, weight=0.5):
    return derive_c6_carried_profile(
        derive_c6_pressure_lattice(
            phase=PHASE if actual else (0.0,) * 6,
            epi_weight=CHANNEL_WEIGHT_SECONDARY if actual else weight,
            phase_weight=CHANNEL_WEIGHT_PRIMARY if actual else 0.5,
        )
    )


def _state(epi=(0.5,) * 6):
    return initialize_nodal_remainder(epi, epi_lower=0.375, epi_upper=0.625)
...
@pytest.mark.parametrize("value", (0.0, 1, F(-1)))
...
def test_self_consistent_floor_closes_the_exact_affine_norm_recurrence():
    result = derive_c6_carried_closure(
        _profile(actual=True), state=_state(), timestep=1 / 16
    )
    base, h = result.base_tube, F(1, 16)
    weight = base.contraction.profile.forced_balance.epi_weight
    q, a, b = (
        base.contraction.norm_factor,
        result.rounding_affine_constant,
        result.rounding_affine_slope,
    )
    assert result.effective_norm_factor == q + 3 * h * b < 1
    assert (
        result.energy_floor * (1 - result.effective_norm_factor) ** 2
        == 6 * h**2 * (2 * weight * base.carry_bound + a) ** 2
    )
    assert result.energy_bound == max(result.energy_floor, base.initial_energy)
    assert (
        result.rounding_bound
        == result.product_error_bound + result.assembly_error_bound
    )
...
@pytest.mark.parametrize(
    ...
    (
        ((0.5,) * 6, False),
        ((0.375, 0.625) * 3, False),
        ((0.375, 0.5, 0.625) * 2, False),
        (tuple(0.5 + n * 2.0**-54 for n in (-2, 0, 2, -2, 4, -4)), True),
    ),
)
def test_shared_pressure_steps_satisfy_closed_spatial_rounding_and_mean_bounds(
    epi, actual
):
    ...
    for _ in range(3):
        ...
        step = advance_nodal_remainder(
            state, timestep=1 / 16, capacity=(1.0,) * 6, pressure=reading.pressure
        )
        observed = observe_c6_carried_profile_step(profile, step=step)
        assert (
            sum(value * value for value in observed.error_after) <= result.energy_bound
        )
        assert all(
            abs(value) <= result.product_error_bound
            for value in reading.epi_reduction_error
        )
        assert all(
            abs(value) <= result.assembly_error_bound
            for value in reading.assembly_error
        )
        assert all(
            abs(value) <= result.rounding_bound for value in observed.rounding_defect
        )
        assert (
            result.mean_increment_lower
            <= observed.mean_change
            <= result.mean_increment_upper
        )
        assert all(
            lo <= value <= hi
            for value, lo, hi in zip(
                reading.gradient_indices,
                result.gradient_index_lower,
                result.gradient_index_upper,
                strict=True,
            )
        )
        state = step.after
...
def test_gradient_integer_hulls_are_sharp_for_the_derived_scalar_inequality():
    ...
    for center, lower, upper in zip(
        result.profile_gradient_indices,
        result.gradient_index_lower,
        result.gradient_index_upper,
        strict=True,
    ):
        ...
        def admitted(index):
            ...
            return (
                remaining <= 0 or remaining**2 <= result.laplacian_error_squared_bound
            )
...
def test_actual_endpoint_spatial_floor_leaves_the_signed_mean_unresolved():
    state = _state(tuple(0.5 + n * 2.0**-54 for n in (-2, 0, 2, -2, 4, -4)))
    result = derive_c6_carried_closure(
        _profile(actual=True), state=state, timestep=1 / 16
    )
    ...
    assert (result.mean_increment_lower + result.mean_increment_upper) / 2 == -F(
        1, 6 * 2**113
    )
    assert (
        not result.infinite_mean_control_certified
...
def test_near_noncontracting_boundary_cannot_hide_rounding_feedback():
    with pytest.raises(ValueError, match=r"q \+ 3\*h\*b < 1"):
        derive_c6_carried_closure(
            _profile(weight=1.0), state=_state(), timestep=math.nextafter(1.0, 0.0)
        )
...
def test_profile_caches_are_rebuilt_from_primitive_pressure_source():
    ...
    forged = replace(
        profile,
        forced_balance=replace(
            profile.forced_balance, relative_profile=(F(0),) * 6, mean_drift=F(100)
        ),
    )
    assert derive_c6_carried_closure(
        forged, state=state, timestep=1 / 16
    ) == derive_c6_carried_closure(profile, state=state, timestep=1 / 16)


def test_large_initial_disagreement_is_retained_instead_of_replaced_by_small_floor():
    result = derive_c6_carried_closure(
        _profile(), state=_state((0.375, 0.5, 0.625) * 2), timestep=1 / 16
    )
