...
def test_same_three_cases_two_cycles_and_shared_eligible_results(comparison):
    ...
    assert tuple(case["mode"] for case in comparison["current_cases"]) == (
        "null",
        "k1",
        "k3",
    )
    ...
    for summary in comparison["current"]:
        ...
        assert all(
            row["recorded_method"] == "exact_two_neighbor_midpoint"
...
def test_historical_midpoint_tie_is_wrong_even_before_subtraction(comparison):
    row = next(
        row
        for row in comparison["retained"][2]["il_midpoint_rows"]
    ...
    assert row["midpoint"]["mean"] == math.nextafter(
        float(row["recorded_mean"]), math.inf
    )
        ...
        for case in comparison[period]:
            ...
            for budget in case["flow_mean_budgets"]:
                assert budget["actual_mean_change"] == (
                    ...
                    + budget["integrator_rounding_mean_effect"]
                )
...
def test_shared_serializer_preserves_only_named_sha_infinity(comparison):
    ...
    for case in decoded["current_cases"]:
        ...
        assert any(
            metric.get("time_to_collapse") == {"numeric_kind": "positive_infinity"}
            for metric in metrics
        )


def test_current_cases_keep_preparation_coefficients_word_and_closure(
    parent, comparison
):
    for old, current in zip(parent["cases"], comparison["current_cases"], strict=True):
        ...
        for name in ("epi", "capacity", "conductance", "support_neighbors"):
            assert (
                current["initial_capture"]["snapshot"][name]
        ...
        assert current["final_before_closure"]["state"]["time"] == 0.5
        closure = current["closure_after_measurement"]["after"]["state"]
        assert closure["epi"] == current["final_before_closure"]["state"]["epi"]
        assert all(
            history == ["UM", "IL", "UM", "IL", "SHA"]
            for history in closure["glyph_history"].values()
        )


def test_invalid_retained_source_is_rejected_before_any_new_execution(
    parent, monkeypatch
):
    ...
    def forbidden(*args):
        raise AssertionError(
            "invalid historical input must fail before running a graph"
        )
