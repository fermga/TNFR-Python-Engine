...
def _state(point=ZERO):
    ...
    return NodalRemainderState(
        row, tuple(x - F(y) for x, y in zip(exact, row, strict=True)), 0.375, 0.625
    )
...
def _zone(points, *, origin=ZERO, quantum=QUANTUM):
    ...
    indices = tuple(
        tuple(
            QUANTUM * (x - base) / quantum
            for x, base in zip(point, origin, strict=True)
        )
        + (F(0),)
        for point in points
    )
    ...
    bounds = tuple(
        tuple(max(int(point[i] - point[j]) for point in indices) for j in range(7))
        for i in range(7)
    )
...
def _derive(reference, targets, **kwargs):
    return owner.derive_c6_carried_region_exclusions(
        ...
        epi_states=kwargs.pop("epi_states", ROWS),
        timestep=1.0,
...
def _represented(result, zones):
    ...
    for point in DOMAIN:
        ...
        if all(
            shifted[i] - shifted[j] <= matrix[i][j] for i in range(7) for j in range(7)
        ):
            points.add(point)
    return points
...
def oracle(reference):
    ...
    def hull(points):
        ...
        for row, group in groups.items():
            bounds = tuple(
                tuple(max(p[i] - p[j] for p in group) for j in range(7))
                for i in range(7)
            )
            represented.update(
                point
                for point in cells[row]
                if all(
                    (point + (0,))[i] - (point + (0,))[j] <= bounds[i][j]
                    for i in range(7)
                    for j in range(7)
                )
            )
...
def slab():
    ...
    points = frozenset(
        point for point in DOMAIN if _row(point) == row and point[0] == 1
    )
    assert len(points) == 27
    return points, _zone(points)


def test_an_entire_27_point_slab_is_excluded_by_complete_region_layers(
    reference, oracle, slab
):
    points, zone = slab
...
def test_nonnested_region_sizes_are_not_treated_as_a_descending_sequence(
    reference, oracle
):
...
def test_incomplete_region_layer_retains_the_previous_complete_geometry(
    reference, oracle
):
    ...
    assert (
        ...
        and query.past_exclusion_depth is None
    )


def test_origin_in_a_new_hull_does_not_mean_the_actual_origin_reaches_a_target(
    reference, oracle
):
    origin = (0, 2, 0, 0, 0, 0)
    ...
    for expected_point in (_image(origin), _image(origin)):
        ...
        current = advance_nodal_remainder(
            current, timestep=1.0, capacity=(1.0,) * 6, pressure=pressure
        ).after
...
def test_exact_stationary_region_with_absent_origin_certifies_path_exclusion(reference):
    ...
    assert (
        query.origin_path_within_domain_excluded and query.past_exclusion_depth is None
    )
...
def test_equal_cardinality_two_cycle_is_not_misread_as_a_stationary_region(reference):
    ...
    result = _derive(
        reference, ((_zone((CYCLE_A,)),),), domain_zones=domain, max_intersections=3
    )
    query = result.queries[0]
...
def test_shared_budget_advances_labels_round_robin_without_promoting_a_partial_query(
    reference,
):
    domain = tuple(_zone((point,)) for point in (ZERO, CYCLE_A, CYCLE_B))
    ...
    assert (
        ...
        == sum(query.intersections for query in result.queries)
        == 3
    )
...
def test_excluded_slabs_do_not_certify_the_entire_domain_or_a_live_graph(
    reference, slab
):
...
def test_target_rn_ties_cannot_be_extended_into_the_wrong_visible_cell(reference):
    ...
    bounds = tuple(
        tuple(0 if i == j else upper[i] - lower[j] for j in range(7)) for i in range(7)
    )
...
@pytest.mark.parametrize(
    "keyword,value",
    (("max_intersections", 0), ("max_intersections", True), ("max_cells", 0)),
)
...
def test_reference_derived_caches_are_rebuilt_before_any_region_pressure_is_used(
    reference, slab, monkeypatch
):
    forged = replace(
        reference,
        sources=(F(99),) * 6,
