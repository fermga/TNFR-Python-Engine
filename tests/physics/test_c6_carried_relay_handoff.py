...
def previous():
    ...
    state = NodalRemainderState(epi, tuple(F(a, 2**b) for a, b in terms), 0.375, 0.625)
    ...
    closure = derive_c6_carried_closure(
        derive_c6_carried_profile(lattice), state=state, timestep=0.0625
    )
    return observe_c6_carried_relay_exit(
        derive_c6_carried_relay(closure), step_budget=10
    )
...
def _state_with_exact(state, changes):
    ...
    result = replace(
        state, epi=epi, remainder=tuple(value - F(v) for value, v in zip(exact, epi))
    )
    _validate_nodal_remainder_state(result)
    return result


def test_handoff_preserves_exact_endpoint_and_retains_only_local_dependencies(
    previous, budget
):
    ...
    assert (
        budget.deadline == 198 == previous.relay.exit_deadlines[1] - previous.exit_step
    )
    assert budget.band_covers_deadline
    axis = previous.relay.axes[0]
    assert (budget.decrement, budget.increment, budget.length) == (
        axis.decrement,
        axis.increment,
        axis.length,
    )
...
def test_full_refresh_crosses_free_cells_until_first_held_exit(
    previous, budget, passage
):
    ...
    assert passage.endpoint.epi[1] == math.nextafter(
        previous.endpoint.epi[1], -math.inf
    )
    assert tuple(
        sum(step.before.epi[i] != step.after.epi[i] for step in passage.steps)
        for i in range(6)
    ) == (182, 1, 0, 118, 23, 0)
    ...
    for ordinal, step in enumerate(passage.steps):
        assert step.before == (
            previous.endpoint if ordinal == 0 else passage.steps[ordinal - 1].after
        )
        assert all(
            step.before.epi[node] == previous.endpoint.epi[node]
            for node in budget.held_nodes
        )
        assert (
            step.pressure
            == _observe_rebuilt_c6_pressure_lattice(reference, step.before.epi).pressure
        )
        ...
        assert (
            point.corrected_budget
            == step.after.exact_epi[1]
            - budget.budget_correction * step.after.exact_epi[0]
        )
        assert (
            point.corrected_budget
            == budget.corrected_initial + (ordinal + 1) * budget.budget_drift
        )
...
def test_full_vector_area_retains_mean_drift_across_the_handoff(previous, passage):
    ...
    assert (
        tuple(
            ...
            for start, end in zip(previous.endpoint.exact_epi, passage.exact_endpoint)
        )
        == area
    )
    ...
    assert (
        tuple(
            ...
            for start, end in zip(
                previous.steps[0].before.exact_epi, passage.exact_endpoint
            )
        )
        == total
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (0.625, 0.375),
        (0.5, 0.5),
        (math.nextafter(0.375, math.inf), math.nextafter(0.625, -math.inf)),
    ),
)
@pytest.mark.parametrize("lower", (False, True))
def test_fresh_canonical_local_rows_ignore_arbitrary_free_visible_values(
    previous, budget, free, lower
):
    reference = budget.closure.base_tube.contraction.profile.lattice
    ...
    assert (
        observed.gradient_indices[0] * reference.gradient_quantum
        == (exact[5] + exact[1]) / 2 - exact[0]
    )
    assert (
        observed.gradient_indices[1] * reference.gradient_quantum
        == (exact[0] + exact[2]) / 2 - exact[1]
    )
...
def test_admission_is_not_restricted_to_the_eight_local_cells(previous, budget, free):
    ...
    assert (
        altered.deadline == budget.deadline
        and altered.budget_drift == budget.budget_drift
    )
...
def test_lower_visible_incoming_facet_keeps_the_original_relay_pair(previous, budget):
    ...
    assert (
        bound.upper_visible == budget.upper_visible
        and bound.lower_visible == budget.lower_visible
    )
...
def test_open_and_closed_strip_edges_are_checked_on_the_shared_grid(previous, budget):
    included = _state_with_exact(
        previous.endpoint, {0: budget.facet + budget.increment}
    )
    closed = derive_c6_carried_local_relay_budget(previous.relay, state=included)
    assert closed.initial_offset == budget.increment
    excluded = _state_with_exact(
        previous.endpoint, {0: budget.facet - budget.decrement}
    )
    with pytest.raises(ValueError):
        derive_c6_carried_local_relay_budget(previous.relay, state=excluded)
    interior = _state_with_exact(
        previous.endpoint, {0: budget.facet - budget.decrement + GRID}
    )
...
def test_uncovered_band_returns_formal_candidate_without_invalid_after_state(previous):
    ...
    anchor = derive_c6_carried_relay(
        derive_c6_carried_closure(profile, state=narrow, timestep=0.0625)
    )
    ...
    observed = observe_c6_carried_local_relay_exit(bound)
    assert (
        observed.exit_step == 1
        and observed.band_failure
        and observed.exiting_nodes == ()
    )
    assert (
        observed.endpoint is None and observed.steps == () and len(observed.points) == 2
    )
...
def test_cached_local_coefficients_and_coverage_are_rebuilt(budget, passage):
    forged = replace(
        budget,
        local_pressure_rows=((0.0, 0.0), (0.0, 0.0)),
        budget_drift=F(0),
        budget_correction=F(0),
        corrected_initial=F(0),
        held_nodes=(),
        free_nodes=tuple(range(6)),
        deadline=1,
        band_covers_deadline=False,
        decrement=F(1),
        increment=F(1),
    )
...
def test_positive_budget_and_upper_owned_tie_use_the_other_signed_deadline(previous):
    ...
    bound = derive_c6_carried_local_relay_budget(
        anchor, state=state, relay_node=3, budget_node=2
    )
    ...
    assert all(
        point.corrected_budget
        == bound.corrected_initial + point.ordinal * bound.budget_drift
        for point in observed.points
    )
    assert not any(observed.nodal_balance_residual)


@pytest.mark.parametrize("step_budget", (0, -1, True, 198.0, F(198)))
...
@pytest.mark.parametrize(
    "relay_node,budget_node", ((False, 1), (0, True), (0, 2), (1, 0), (6, 1))
)
def test_node_selection_requires_an_existing_axis_and_adjacent_budget_node(
    previous, relay_node, budget_node
):
    with pytest.raises((TypeError, ValueError)):
        derive_c6_carried_local_relay_budget(
            previous.relay,
            state=previous.endpoint,
...
def test_nonstate_and_changed_declared_band_are_rejected(previous):
    ...
    changed = replace(previous.endpoint, epi_lower=0.25)
    with pytest.raises((TypeError, ValueError)):
        derive_c6_carried_local_relay_budget(previous.relay, state=changed)
