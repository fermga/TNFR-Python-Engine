...
def _state(epi=(LOW,) * 6, carry=(F(0),) * 6, band=(.375, .625)):
    return NodalRemainderState(epi, carry, *band)


def _derive(reference, state=None, rows=ROWS, timestep=1., **kwargs):
    return derive_c6_carried_forward_envelope(
        reference, state=_state() if state is None else state, epi_states=rows,
        timestep=timestep, derive_pair_barriers=False, **kwargs,
    )


def _contains(result, zones, exact):
    """Read public inequalities at an independently specified rational point."""
    indices = tuple((x - origin) / result.grid_quantum
...
def _small_image(point):
    ...
    return tuple(point[i] + visible[(i - 1) % 6] + visible[(i + 1) % 6]
                 - 2 * visible[i] for i in range(6))


def test_missing_cartesian_corner_cannot_be_hidden_by_complete_axis_projections(reference):
...
def test_visible_gap_without_any_omitted_affine_grid_point_is_allowed(reference):
    skipped = float(F(LOW) + 2 * DELTA)
    result = _derive(reference, rows=tuple(product((LOW, skipped), repeat=6)),
                     timestep=2., max_intersections=1)
...
def test_six_distinct_affine_bases_preserve_exact_cell_membership(reference):
    ...
    coordinates = tuple(tuple(origin + index * result.grid_quantum
...
def test_even_and_odd_rounding_ties_are_not_interchangeable(reference):
    ...
    for offset, admitted in ((-DELTA / 2, True), (DELTA / 2, True),
                             (3 * DELTA / 2, False)):
    ...
    step = advance_nodal_remainder(initial, timestep=1., capacity=(1.,) * 6, pressure=pressure)
...
def test_stationary_relational_abstraction_with_fourteen_outgoing_points_is_not_invariant(reference):
    ...
    for bits in product((0, 1), repeat=6):
        ...
        expected = tuple(DELTA / 2 * (bits[(i - 1) % 6] + bits[(i + 1) % 6] - 2 * bits[i])
                         for i in range(6))
        assert tuple(map(F, pressure)) == expected
    result = _derive(reference)
    domain = set(product(range(-1, 3), repeat=6))
    retained = {point for point in domain if _contains(
        result, result.retained_zones, tuple(F(LOW) + DELTA / 2 * x for x in point),
    )}
...
def test_empty_compatible_past_domain_does_not_relabel_itself_as_an_origin_exit_proof(reference):
    row = (LOW, HIGH) * 3
...
def test_proved_invariant_core_does_not_certify_an_origin_that_fails_to_enter(reference, narrow_band):
    values = tuple(float(F(LOW) + i * DELTA) for i in range(3))
    epi = tuple(values[i] for i in (0, 2, 0, 0, 0, 0))
    band = (values[0], values[-1]) if narrow_band else (.375, .625)
    initial = _state(epi, band=band)
    result = _derive(reference, state=initial, rows=tuple(product(values, repeat=6)), timestep=2.)
    ...
    if narrow_band:
        ...
        assert step.after.exact_epi == tuple(F(LOW) + i * DELTA for i in (2, -2, 2, 0, 0, 0))
