...
def report(parent, historical):
    return campaign.analyze_c6_winding_relay_handoff(
        parent, historical_bytes=historical
    )
...
def test_source_carry_and_conditional_lineage_remain_explicit(report, parent):
    ...
    assert (
        campaign._payload(source["retained_B46_endpoint"])
        == parent["B46_conditional_first_exit"]["endpoint"]
    )
...
def test_nonzero_coupled_switch_term_does_not_break_local_budget(report):
    ...
    assert control["node3_node4_mixed_increment"] == tuple(
        F(x, 2**113) for x in (0, 0, 0, -8, -8, 0)
    )
...
def test_complete_handoff_vector_area_and_first_held_boundary(report):
    ...
    assert tuple((F(x) - F(1, 2)) * 2**54 for x in endpoint["epi"]) == (
        -3,
        -1,
        2,
        0,
    ...
    assert all(
        step["nodal_balance_residual"] == (F(0),) * 6 for step in observed["steps"]
    )
    assert report["original_preparation_plus_conditional_mean_area"] == F(1215, 2**114)
    assert report["original_preparation_plus_conditional_nodal_area"] == tuple(
        x - F(1, 2) for x in _exact(endpoint)
    )
    assert any(change)


def test_analysis_does_not_mutate_inputs_and_serializes_exact_evidence(
    parent, historical
):
    original = deepcopy(parent)
    report = campaign.analyze_c6_winding_relay_handoff(
        parent, historical_bytes=historical
    )
...
def test_replay_binds_complete_handoff_and_returns_original_carry(
    retained_handoff, historical
):
    ...
    assert state.exact_epi == _exact(
        retained_handoff["B47_conditional_first_exit"]["endpoint"]
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (("B47_conditional_first_exit", "steps", 0, "pressure", 0), 0.0),
...
def test_handoff_replay_rejects_changed_evidence(
    ...
    monkeypatch.setattr(
        campaign, "analyze_c6_winding_relay_handoff", lambda *_a, **_k: deepcopy(report)
    )
...
def test_handoff_replay_requires_exact_retained_bytes(retained_handoff, historical):
    ...
    with pytest.raises(TypeError, match="exact retained"):
        campaign.replay_c6_relay_handoff_evidence(
            retained_handoff,
            relay_bytes=relay_bytes.decode(),
            historical_bytes=historical,
        )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (("B46_conditional_first_exit", "steps", 0, "pressure", 0), 0.0),
...
def test_handoff_rejects_tampered_numeric_chain_and_provenance(
    parent, historical, path, value
):
...
def test_cli_binds_both_inputs_to_current_result(
    tmp_path, monkeypatch, parent, historical, report
):
    source, base, output = (
        tmp_path / name for name in ("source.json", "base.json", "output.json")
    )
    ...
    assert (
        result["input_evidence"]["historical_sha256"]
...
def test_cli_rejects_mid_analysis_changes(
    tmp_path, monkeypatch, parent, historical, report, which
):
    source, base, output = (
        tmp_path / name for name in ("source.json", "base.json", "output.json")
    )
