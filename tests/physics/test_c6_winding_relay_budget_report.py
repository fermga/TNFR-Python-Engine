...
def test_actual_pre_sha_carry_is_retained_and_conditional_scope_is_explicit(
    report, parent
):
    ...
    assert source["capacity"] == (1.0,) * 6
    assert source["historical_terminal_SHA_capacity"] == (0.9204225284540524,) * 6
    ...
    for flag in (
        ...
        "full_region_invariant",
        "indefinite_trapping_certified",
        "general_correlated_region_excluded",
        "future_runtime_certified",
    ):
        assert report[flag] is False


def test_deadline_precedes_bounded_observation_and_complete_vector_area_is_retained(
    report,
):
    certificate, observed = (
        report["B46_two_relay_certificate"],
        report["B46_conditional_first_exit"],
    )
    ...
    x = tuple(
        F(a) + b for a, b in zip(initial["epi"], initial["remainder"], strict=True)
    )
    y = tuple(
        F(a) + b for a, b in zip(endpoint["epi"], endpoint["remainder"], strict=True)
    )
...
@pytest.mark.parametrize(
    "field,value",
    (("flow_count", 355), ("runtime_provenance_certified_at_capture", False)),
)
...
def test_cli_retains_historical_input_identity_and_current_source(
    ...
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_relay_budget",
        lambda _parent, **_: deepcopy(report),
    )
    monkeypatch.setattr(
        campaign,
...
def test_cli_fails_closed_when_evidence_changes_during_analysis(
