...
def _template(offsets, lower=.375, upper=.625):
    return NodalRemainderState(_epi(offsets), (COMMON_CARRY,) * 6, lower, upper)


def _origin(lower=.375, upper=.625):
    ...
    return NodalRemainderState(
        _epi((-3, 0, 2, 0, 8, -5)), tuple(F(numerator, 2**power) for numerator, power in terms),
        lower, upper,
    )


def _closure(lower=.375, upper=.625):
    reference = derive_c6_pressure_lattice(
        phase=PHASE, epi_weight=CHANNEL_WEIGHT_SECONDARY, phase_weight=CHANNEL_WEIGHT_PRIMARY,
    )
    return derive_c6_carried_closure(derive_c6_carried_profile(reference), state=_origin(lower, upper), timestep=.0625)


@pytest.fixture(scope="module")
def obstruction():
    return derive_c6_carried_mean_cylinder_obstruction(
        _closure(), positive_state=_template(POSITIVE_OFFSETS), negative_state=_template(NEGATIVE_OFFSETS),
    )
...
def test_canonical_templates_bind_both_pressure_signs_at_the_actual_mean(obstruction):
    ...
    assert _mean(obstruction.positive_point.state) == _mean(obstruction.negative_point.state) == mean
...
def _assert_boundary(boundary, lower, upper, expected_direction):
    ...
    assert boundary.exact_candidate == tuple(x + a for x, a in zip(exact, boundary.exact_increment, strict=True))
    assert boundary.mean_after == sum(boundary.exact_candidate, F(0)) / 6
    assert boundary.mean_after - boundary.mean_before == sum(boundary.exact_increment, F(0)) / 6
...
@pytest.mark.parametrize("left,right", ((F(0), F(0)), (-F(1, 3), F(1, 3)), (-F(7, 3), F(11, 3))))
def test_rational_endpoints_need_not_lie_on_the_uniform_mean_grid(obstruction, left, right):
    origin_mean = _mean(_origin())
    lower, upper = origin_mean + left * GRID, origin_mean + right * GRID
    result = observe_c6_carried_mean_cylinder_escape(obstruction, mean_lower=lower, mean_upper=upper)
...
def test_extreme_admitted_window_has_actual_legal_witnesses_on_both_sides(obstruction):
    ...
    _assert_boundary(result.upper_boundary, result.mean_lower, result.mean_upper, "upper")
    _assert_boundary(result.lower_boundary, result.mean_lower, result.mean_upper, "lower")
...
def test_declared_band_clipping_can_supply_a_closed_lower_translation_boundary():
    ...
    result = derive_c6_carried_mean_cylinder_obstruction(
        _closure(lower, upper), positive_state=_template(POSITIVE_OFFSETS, lower, upper),
        negative_state=_template(NEGATIVE_OFFSETS, lower, upper),
    )
    assert result.translation_grid_lower == int(-COMMON_CARRY / GRID)
    escape = observe_c6_carried_mean_cylinder_escape(result, mean_lower=result.mean_lower, mean_upper=result.mean_upper)
...
@pytest.mark.parametrize("positive,negative", (
    (NEGATIVE_OFFSETS, POSITIVE_OFFSETS), (POSITIVE_OFFSETS, POSITIVE_OFFSETS),
    (NEGATIVE_OFFSETS, NEGATIVE_OFFSETS),
))
def test_pressure_signs_cannot_be_interchanged_or_inferred_from_labels(positive, negative):
    with pytest.raises(ValueError):
        derive_c6_carried_mean_cylinder_obstruction(
            _closure(), positive_state=_template(positive), negative_state=_template(negative),
        )


def test_template_mean_must_match_the_actual_origin_without_projection():
    positive = _template(POSITIVE_OFFSETS)
    changed = replace(positive, remainder=(positive.remainder[0] + GRID,) + positive.remainder[1:])
    with pytest.raises(ValueError):
        derive_c6_carried_mean_cylinder_obstruction(
            _closure(), positive_state=changed, negative_state=_template(NEGATIVE_OFFSETS),
        )


def test_forged_derived_obstruction_caches_are_recomputed(obstruction):
    forged = replace(obstruction, grid_quantum=F(1), mean_lower=F(0), mean_upper=F(1),
                     positive_mean_increment=-F(1), negative_mean_increment=F(1),
                     translation_grid_lower=-1, translation_grid_upper=1)
    mean = _mean(_origin())
    expected = observe_c6_carried_mean_cylinder_escape(obstruction, mean_lower=mean, mean_upper=mean)
    observed = observe_c6_carried_mean_cylinder_escape(forged, mean_lower=mean, mean_upper=mean)
    assert observed == expected


def test_primitive_template_tampering_is_not_hidden_by_a_valid_point_cache(obstruction):
    changed = replace(obstruction.positive_point, state=obstruction.negative_point.state)
    forged = replace(obstruction, positive_point=changed)
    mean = _mean(_origin())
    with pytest.raises(ValueError):
        observe_c6_carried_mean_cylinder_escape(forged, mean_lower=mean, mean_upper=mean)


@pytest.mark.parametrize("value", (0, .5, True, "1/2"))
def test_mean_interval_endpoints_must_be_exact_fractions(obstruction, value):
    with pytest.raises((TypeError, ValueError)):
        observe_c6_carried_mean_cylinder_escape(obstruction, mean_lower=value, mean_upper=_mean(_origin()))


@pytest.mark.parametrize("case", ("reversed", "miss_origin", "below_window", "above_window"))
def test_interval_admission_keeps_the_proved_translation_window(obstruction, case):
    ...
    with pytest.raises(ValueError):
        observe_c6_carried_mean_cylinder_escape(obstruction, mean_lower=lower, mean_upper=upper)
