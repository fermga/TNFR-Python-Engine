...
def test_first_hit_is_after_three_complete_boundaries_and_nineteen_steps(report, parents):
    ...
    assert tuple(row["cumulative_steps"] for row in report["boundaries"]) == (12, 15, 19)
    assert _offsets(report["source"]["inherited_state"]) == (-1, 0, 0, -1, 2, -2)
    assert tuple(_offsets(row["endpoint"]) for row in report["boundaries"]) == (
        (-1, 0, 0, -1, 4, -2), (-1, 0, 0, -2, 4, -2), (-1, 0, 2, -2, 4, -3),
    )
    assert tuple(row["horizon"]["first_exit_leaves_cell"] for row in report["boundaries"]) == (
...
def test_sign_flip_comes_from_neighbor_two_while_selected_visible_node_stays_fixed(report):
    rows = report["boundaries"]
    assert tuple(row["source_pressure"]["gradient_indices"][1] for row in rows) == (-1, -1, -1)
    assert tuple(row["refreshed_pressure"]["gradient_indices"][1] for row in rows) == (-1, -1, 1)
    expected_negative = -F(128295757220873, 2**106)
    expected_positive = F(2786216251278205, 2**108)
    assert all(F(row["source_pressure"]["pressure"][1]) == expected_negative for row in rows)
    ...
    assert all(step["after"]["epi"][1] == initial["epi"][1] for step in report["continuation"]["steps"])
    before, after = rows[-1]["initial"], rows[-1]["endpoint"]
    changes = tuple((F(b) - F(a)) * 2**54 for a, b in zip(before["epi"], after["epi"], strict=True))
    assert changes == (0, 0, 2, 0, 0, -1)
    assert _integer_gradient(after)[1] - _integer_gradient(before)[1] == changes[0] + changes[2] - 2 * changes[1] == 2
...
def test_all_steps_preserve_the_inherited_carry_and_actual_held_nodal_product(report):
    ...
    for step in steps:
        ...
        assert step["exact_increment"] == expected and step["nodal_balance_residual"] == (0,) * 6
        area = [a + b for a, b in zip(area, expected, strict=True)]
        after = step["after"]
        for i in range(6):
            assert F(after["epi"][i]) + after["remainder"][i] == F(initial["epi"][i]) + initial["remainder"][i] + area[i]
    ...
    for row in report["boundaries"]:
        ...
        assert all(step["pressure"] == row["source_pressure"]["pressure"] for step in local)
        assert local[-1]["after"] == row["endpoint"]
        cursor += len(local)


def test_exact_gradient_change_is_split_into_nodal_and_carry_terms_at_every_prefix(report):
    initial = report["source"]["inherited_state"]
    ...
    for step, row in zip(report["continuation"]["steps"], rows, strict=True):
        ...
        assert row["gradient_indices_before"] == old and row["gradient_indices_after"] == new
        area, dr = row["nodal_area"], row["remainder_change"]
        independent_area = tuple((area[i - 1] + area[(i + 1) % 6] - 2 * area[i]) * 2**54 for i in range(6))
        independent_carry = tuple((2 * dr[i] - dr[i - 1] - dr[(i + 1) % 6]) * 2**54 for i in range(6))
        assert row["nodal_gradient_term"] == independent_area
        assert row["remainder_gradient_term"] == independent_carry
        assert row["gradient_index_change"] == tuple(b - a for a, b in zip(old, new, strict=True))
        assert row["gradient_index_change"] == tuple(a + b for a, b in zip(independent_area, independent_carry, strict=True))
        assert row["identity_residual"] == (0,) * 6
        assert row["node1_nodal_area"] == F(row["ordinal"], 16) * F(row["node1_source_pressure"]) < 0
...
def test_pressure_sign_mean_change_and_vector_compensation_remain_distinct(report):
    ...
    assert tuple(row["prefix_balances"][-1]["mean_nodal_area"] for row in rows) == (
        -F(1, 2**111), F(3, 2**113), -F(1, 3 * 2**110),
    )
    assert tuple(sum(map(F, row["refreshed_pressure"]["pressure"])) for row in rows) == (
        F(3, 2**108), -F(1, 2**107), -F(5, 2**108),
    )
    continuation = report["continuation"]
    assert continuation["mean_nodal_area"] == continuation["mean_reconstructed_change"] == -F(11, 3 * 2**113)
    assert continuation["total_nodal_area"][1] == -F(2437619387196587, 2**110)
    for prefix in continuation["prefix_balances"]:
        assert prefix["mean_identity_residual"] == 0 and prefix["nodal_identity_residual"] == (0,) * 6
        assert prefix["mean_nodal_area"] == prefix["mean_reconstructed_change"]
    for name in ("node1_nodal_area_compensated", "mean_nodal_area_compensated", "total_nodal_area_compensated"):
...
def test_uniform_sector_bound_is_conditional_and_uses_the_actual_carry(report):
    ...
    initial = report["source"]["inherited_state"]
    assert certificate["outward_distance"] == F(initial["epi"][1]) + initial["remainder"][1] - F(3, 8)
    assert certificate["per_step_margin"] == sector["signed_pressure_margin"] / 16
    assert certificate["max_sector_steps"] == certificate["outward_distance"] // certificate["per_step_margin"] == 1264728314825478005
...
def test_inverse_itinerary_binds_the_actual_carry_and_rejects_a_zero_reset(report):
    ...
    assert itinerary["visible_closed"] is False and itinerary["conditional_carried_cycle"] is False
    ...
    initial = report["source"]["inherited_state"]
    for x, r, cell in zip(initial["epi"], initial["remainder"], itinerary["coordinates"], strict=True):
        exact = F(x) + r
...
@pytest.mark.parametrize("boundaries,steps,expected_calls,expected_steps,reason", (
...
def test_budget_censoring_precedes_any_oversized_shared_replay(parents, monkeypatch, boundaries, steps, expected_calls, expected_steps, reason):
    ...
    result = campaign.analyze_c6_winding_pressure_sign(*parents, boundary_budget=boundaries, step_budget=steps)
...
@pytest.mark.parametrize("kwargs", ({"step_budget": 0}, {"step_budget": True}, {"step_budget": 257},
                                    {"step_budget": 2.0}, {"boundary_budget": 0}, {"boundary_budget": 9},
                                    {"boundary_budget": True}))
def test_invalid_resource_budget_is_rejected_before_parent_replay(parents, monkeypatch, kwargs):
    def forbidden(*args):
...
@pytest.mark.parametrize("change", ("claim", "scope", "flag", "bool_count", "carry", "pressure", "prefix",
                                    "cached_sector", "source_weight", "producer_manifest"))
...
def test_cli_binds_both_distinct_input_hashes_and_retains_historical_sources(tmp_path, monkeypatch, report):
    parent_path, source_path, output = (tmp_path / name for name in ("parent.json", "source.json", "output.json"))
    parent_path.write_bytes(PARENT.read_bytes())
    source_path.write_bytes(SOURCE.read_bytes())
    monkeypatch.setattr(sys, "argv", ["sign-audit", "--input", str(parent_path), "--source-input", str(source_path), "--output", str(output)])
    monkeypatch.setattr(campaign, "analyze_c6_winding_pressure_sign", lambda *args: deepcopy(report))
    ...
    assert result["input_evidence"]["sha256"] == hashlib.sha256(PARENT.read_bytes()).hexdigest()
    assert result["source_input_evidence"]["sha256"] == hashlib.sha256(SOURCE.read_bytes()).hexdigest()
    assert result["source_input_evidence"]["sha256"] == result["input_evidence"]["producer_input_evidence"]["sha256"]
    assert parent_path.read_bytes() == PARENT.read_bytes() and source_path.read_bytes() == SOURCE.read_bytes()
