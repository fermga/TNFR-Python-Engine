...
def _fixture(glyph="EN", *, initial=(0.5, 0.0), factor=None):
    ...
    obs = NonEpiForcingObservation(
        snapshot=snap, phase=(F(0), F(0)), epi_weight=F(1), forcing=(F(0), F(0)),
        phase_gradient=(F(0), F(0)),
        normalized_weights=(("phase", F(0)), ("epi", F(1)), ("vf", F(0)), ("topo", F(0))),
        full_kernel_pressure=tuple(map(F, pressure)),
        kernel_pressure_defect=tuple(F(p)-g for p, g in zip(pressure, snap.epi_gradient, strict=True)),
        stored_pressure_residual=(F(0), F(0)),
    )
    configuration = {"GLYPH_FACTORS": {"EN_mix": factor if glyph == "EN" else 0.25,
                                       "AL_boost": factor if glyph == "AL" else 0.125},
                     "EPI_MIN": -10.0, "EPI_MAX": 10.0}

    def record(values, time=2.25):
        return {"state": {"nodes": [0, 1], "epi": list(values), "capacity": [1.0, 1.0],
                          "phase": [0.0, 0.0], "pressure": list(pressure), "time": time,
                          "edges": [[0, 1, {"weight": 1.0}]]},
                "ordered_neighbors": [[0, [1]], [1, [0]]],
                "node_attributes": [[node, {"EPI": _literal(value), "epi_kind": "AL"}]
    ...
    rows = [{"ordinal": 0, "boundary": "_prepare_dnfr", "outcome": "completed",
             "before": deepcopy(generation), "after": deepcopy(generation)},
    ...
    for node in (0, 1):
        ...
        rows.append({"ordinal": len(rows), "boundary": "apply_glyph", "outcome": "completed",
                     "node": node, "glyph": glyph, "before": before, "after": record(current)})
    reset = current
    after = tuple(x+0.25*p for x, p in zip(reset, pressure, strict=True))
    rows.append({"ordinal": len(rows), "boundary": "integrate", "outcome": "completed",
                 "before": record(reset), "after": record(after, 2.5)})
    payload = {"observation": asdict(obs), "components": tuple((key, (F(0), F(0))) for key in ("phase", "vf", "topo"))}
    step = {"ordinal": 0, "status": "executed", "before": record(initial), "endpoint": record(after, 2.5),
            "native_trace": {"status": "executed", "boundaries": rows, "captures": {
...
def _reset_oracle(step):
    ...
    calls = [row for row in step["native_trace"]["boundaries"] if row["boundary"] == "apply_glyph"]
    for call in calls:
        ...
        x, y = (tuple(map(F, call[side]["state"]["epi"])) for side in ("before", "after"))
        ...
        propagated = tuple(a+b for a, b in zip(_apply(local, propagated), defect, strict=True))
        offset = tuple(a+b for a, b in zip(_apply(local, offset), shift, strict=True))
        composite = _product(local, composite)
        rows.append(local)
        defects.append(defect)
    return {"S": composite, "c": offset, "local_matrices": rows, "local_defects": defects,
            "propagated_individual": individual, "total_defect": propagated}
...
def _assert_reset(audit, step):
    ...
    assert tuple(reset["propagated_local_defects"]) == tuple(expected["propagated_individual"])
    assert reset["total_reset_defect"] == expected["total_defect"]
    for row, local, defect in zip(reset["local_rows"], expected["local_matrices"], expected["local_defects"], strict=True):
    ...
    assert xg == tuple(a+b+c for a, b, c in zip(_apply(reset["S"], x0), reset["c"], reset["total_reset_defect"], strict=True))
    raw = step["native_trace"]["captures"]["pressure_generation"]["payload"]["observation"]
    snapshot = raw["snapshot"]
    weights = {(i, j): F(w) for i, j, w in snapshot["conductance"]}
    nu, e = tuple(map(F, snapshot["capacity"])), F(raw["epi_weight"])
    strengths = tuple(sum(w for (j, _), w in weights.items() if i == j) for i in range(len(x0)))
    a = tuple(tuple(e*nu[i]*(F(i == j)-weights.get((i, j), 0)/strengths[i])
                    for j in range(len(x0))) for i in range(len(x0)))
    b = tuple(v*F(f) for v, f in zip(nu, raw["forcing"], strict=True))
    assert audit["generation"]["A"] == a and audit["generation"]["b"] == b
    ideal = tuple(reset_value+offset+F(1, 4)*(force-rate) for reset_value, offset, force, rate
                  in zip(_apply(reset["S"], x0), reset["c"], b, _apply(a, x0), strict=True))
    assert audit["runtime"]["ideal_endpoint"] == ideal
    runtime_names = ("pressure_term", "integration_remainder", "postintegration_change")
    actual_terms = [reset["total_reset_defect"]]+[audit["runtime"][name] for name in runtime_names]
    assert xf == tuple(value+sum(parts, F(0)) for value, parts in zip(ideal, zip(*actual_terms, strict=True), strict=True))
    assert not any(reset["identity_residual"]) and not any(audit["runtime"]["identity_residual"])
...
def test_detached_reset_matches_independent_ordered_algebra_without_runtime(glyph):
    ...
    saved = deepcopy(step)
    with patch("tnfr.dynamics.step", side_effect=AssertionError("offline audit cannot advance dynamics")):
...
def test_paired_reset_map_mean_change_and_residuals(glyph):
    ...
    expected_map = tuple(tuple(s-F(1, 4)*g for s, g in zip(sr, gr, strict=True))
                         for sr, gr in zip(a["reset"]["S"], a["generation"]["A"], strict=True))
    assert paired["pair_map"] == expected_map
    delta0 = tuple(y-x for x, y in zip(a["vectors"]["x0"], b["vectors"]["x0"], strict=True))
    deltag = tuple(y-x for x, y in zip(a["vectors"]["xg"], b["vectors"]["xg"], strict=True))
    deltaf = tuple(y-x for x, y in zip(a["vectors"]["xf"], b["vectors"]["xf"], strict=True))
    assert paired["mean_change_at_reset"] == sum(deltag, F(0))/2-sum(delta0, F(0))/2
    assert deltaf == tuple(value+sum(parts, F(0)) for value, parts in zip(
        _apply(expected_map, delta0), zip(*paired["residual_terms"].values(), strict=True), strict=True))
    if glyph == "AL":
        assert a["reset"]["S"] == _identity(2) and a["reset"]["c"] == b["reset"]["c"] == (F(1, 8),)*2
        assert paired["mean_change_at_reset"] == 0
    else:
        assert paired["mean_change_at_reset"] == -F(1, 128)


@pytest.mark.parametrize("corruption", ("order", "target", "space", "factor", "pressure", "endpoint", "extra_refresh"))
def test_corrupt_record_boundaries_are_rejected(corruption):
    ...
    if corruption == "order":
        ...
        first["before"]["graph_attributes"]["GLYPH_FACTORS"]["EN_mix"] = ["binary64", float(0.125).hex()]
...
def test_pair_abstains_if_declared_reset_map_differs():
    ...
    result = audit_pair(audit_reset_step(left, reference), audit_reset_step(right, reference), reference.metric_weights)
...
def test_changed_input_bytes_are_rejected_before_payload_admission(tmp_path):
    ...
    with pytest.raises(ValueError, match="digest"):
        load_evidence(path, path, expected_window_sha256=digest, expected_native_sha256=digest)


def test_pinned_retained_four_resets_without_any_new_native_step():
    from benchmarks.thol_retained_reset_audit import (
        WINDOW_PATH, run_study, window,
    )
    ...
    hashes = {path: hashlib.sha256(path.read_bytes()).hexdigest() for path in (WINDOW_PATH, window.NATIVE_PATH)}
    with patch("tnfr.dynamics.step", side_effect=AssertionError("no new native invocation")):
        report = run_study()
    ...
    for branch, source in zip(report["branches"], archived["branches"], strict=True):
        for audit, ordinal in zip(branch["resets"], (2, 4), strict=True):
            step = window.expand_record(source["steps"][ordinal], archived["record_pool"])
            _assert_reset(audit, step)
