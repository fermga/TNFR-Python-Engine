...
def patch_previous(monkeypatch, previous):
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_return_histories",
...
def test_memory_report_keeps_original_origin_and_conditional_scope(inputs, report):
    ...
    assert (
        campaign._payload(report["B57_return_envelope"]["state"])
        == parent["B56_return_envelope"]["state"]
    )
    ...
    assert (
        ...
        and not report["whole_band_exit_certified"]
    )
    assert (
        not report["future_runtime_certified"]
        and not report["asymptotic_convergence_certified"]
    )
    assert report["B57_reconstruction"]["prior_first_exit_slabs_reverified"] == 39
    assert (
        report["excluded_first_exit_slabs"]
        + len(report["B57_remaining_first_exit_facets"])
        == 56
    )
...
def test_memory_report_preserves_every_whole_original_target(inputs, report):
    ...
    assert [item["target"] for item in observations] == parent[
        "B56_remaining_first_exit_facets"
    ]
    for item in observations:
        original = next(
            q["query"]["target_regions"][0]["bounds"]
            for q in b55["B55_union_queries"]
            if q["target"] == item["target"]
        )
    ...
    transient = next(
        item
        for item in observations
        if item["target"] == dict(mask=30, node=3, direction="upper")
    )
    ...
    for memory_index, edge_index in transient["terminal_transient_memory_edges"]:
        ...
        source_row = (
            env["state"]["epi"]
            if memory_index == -1
            else env["return_relation"][memory_index]["target_epi"]
        )
...
def test_memory_queries_keep_full_targets_and_complete_layers(inputs, report):
    ...
    assert [item["target"] for item in queries] == parent[
        "B56_remaining_first_exit_facets"
    ]
    ...
    for item in queries:
        ...
        assert (
            len(query["initial_endpoint_zones"])
            == len(query["retained_endpoint_zones"])
            == count
        )
        assert query["intersections"] <= 30_000
        assert not query["actual_origin_reachability_certified"]
        assert [step["depth"] for step in query["iterations"]] == list(
            range(len(query["iterations"]))
        )
...
def test_memory_rejects_each_altered_ancestor(inputs, index):
    ...
    with pytest.raises(ValueError, match="lineage"):
        campaign.analyze_c6_winding_return_memory(
            json.loads(changed[0]), **arguments(changed)
        )


@pytest.mark.parametrize(
    "mutation", ("claim", "result", "origin", "guard", "target", "word", "count")
)
def test_memory_reconstructs_untrusted_parent_claims(
...
def test_memory_incomplete_initialization_cannot_exclude(monkeypatch, inputs, previous):
    ...
    assert all(
        not item["observation_complete"] for item in result["B57_target_observations"]
    )
    assert all(
        not item["origin_path_before_original_cube_exit_excluded"]
        for item in result["B57_target_observations"]
    )
    assert not result["B57_query_relation_complete"]
    assert all(
        not item["query"]["initialization_complete"]
        for item in result["B57_memory_queries"]
    )
    assert all(
...
def test_memory_cli_binds_all_twelve_inputs_in_order(
...
def test_memory_cli_rejects_midflight_input_changes(
