...
def _profile(*, actual=False):
    lattice = derive_c6_pressure_lattice(
        ...
        epi_weight=CHANNEL_WEIGHT_SECONDARY if actual else 0.5,
        phase_weight=CHANNEL_WEIGHT_PRIMARY if actual else 0.5,
    )
    return derive_c6_carried_profile(lattice)


def _state(epi=(0.5,) * 6, remainder=ZERO, **band):
    state = initialize_nodal_remainder(
        epi,
        epi_lower=band.get("epi_lower", 0.375),
        epi_upper=band.get("epi_upper", 0.625),
    )
...
def _step(profile, state=None, **changes):
    ...
    args = dict(timestep=0.0625, capacity=(1.0,) * 6, pressure=pressure)
    args.update(changes)
...
def _lap(values):
    return tuple(
        value - (values[i - 1] + values[(i + 1) % 6]) / 2
...
def test_actual_closed_phase_profile_matches_independent_exact_poisson_witness():
    ...
    expected = tuple(
        F(value, 4279441930180617987241864442413056) for value in numerators
    )
...
def test_exact_alternating_diffusion_has_hand_derived_fifteen_sixteenths_factor():
    profile = _profile()
    state = _state(tuple(0.5 + (-1) ** i / 16 for i in range(6)))
...
def test_hidden_alternating_carry_is_exactly_cancelled_by_readout_feedback():
    ...
    result = observe_c6_carried_profile_step(profile, step=_step(profile, state))
    assert result.step.pressure == (0.0,) * 6
...
def test_uniform_carry_is_a_mean_coordinate_and_not_a_shape_error():
    ...
    result = observe_c6_carried_profile_step(
        profile,
        step=_step(profile, _state(remainder=(F(1, 2**60),) * 6)),
    )
...
def test_unequal_epi_signed_decomposition_uses_full_vector_and_retained_carry():
    profile = _profile(actual=True)
    state = _state(
        (0.4, 0.45, 0.5, 0.55, 0.6, 0.475), tuple(F((-1) ** i, 2**61) for i in range(6))
    )
    result = observe_c6_carried_profile_step(profile, step=_step(profile, state))
    assert any(result.rounding_defect) and any(result.carry_feedback)
    p, weight, h = (
        tuple(map(F, result.step.pressure)),
        profile.forced_balance.epi_weight,
        F(1, 16),
    )
    expected_rounding = tuple(
        force - source + weight * lap
        for force, source, lap in zip(
            p,
            profile.forced_balance.forcing,
            _lap(tuple(map(F, state.epi))),
            strict=True,
        )
    )
    assert result.rounding_defect == expected_rounding
    assert result.carry_feedback == tuple(
        weight * value for value in _lap(state.remainder)
    )
...
def test_forged_step_caches_are_rejected(field, value):
    ...
    with pytest.raises(ValueError, match="replay"):
        observe_c6_carried_profile_step(
            profile, step=replace(_step(profile), **{field: value})
        )
...
def test_subclass_exact_epi_property_cannot_forge_the_initial_shape():
    ...
    state = AlteredReadout((0.5,) * 6, ZERO, 0.375, 0.625)
    step = _step(profile, state, timestep=0.5)
...
def test_zero_duration_is_not_a_positive_flow_claim():
    ...
    with pytest.raises(ValueError, match="positive timestep"):
        observe_c6_carried_profile_step(profile, step=_step(profile, timestep=0.0))
...
def test_wider_declared_state_band_is_rejected_even_if_point_is_inside_slab():
    ...
    with pytest.raises(ValueError, match="complete carried-state band"):
        observe_c6_carried_profile_step(
            profile, step=_step(profile, _state(epi_lower=0.05, epi_upper=1.0))
        )
...
def test_narrower_state_band_is_accepted():
    ...
    result = observe_c6_carried_profile_step(
        profile,
        step=_step(profile, _state(epi_lower=0.45, epi_upper=0.55)),
    )
    assert result.recurrence_residual == ZERO
