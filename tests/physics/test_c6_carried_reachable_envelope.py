...
def _state(point=ORIGIN):
    return NodalRemainderState(
        tuple(VALUES[i] for i in point), (F(0),) * 6, 0.375, 0.625
    )
...
def _points(result, zones):
    ...
    for zone in zones:
        ...
        assert lower == upper
        exact = tuple(
            a + result.grid_quantum * n for a, n in zip(result.affine_origin, lower)
        )
...
def test_complete_origin_injected_layers_match_an_independent_729_state_oracle(result):
    ...
    assert tuple(item.zone_count for item in result.iterations) == tuple(
        map(len, layers)
    )
    ...
    assert (
        result.domain_confined_paths_covered
        and result.clipped_forward_inclusion_certified
    )


def test_every_saved_pressure_is_the_exact_integer_diffusion_map(result):
    for point, pressure in zip(
        product(range(3), repeat=6), result.pressures, strict=True
    ):
...
def test_stationary_clipped_envelope_can_contain_an_origin_that_actually_exits(
    reference,
):
    result = owner.derive_c6_carried_reachable_envelope(
        reference,
        state=_state(ESCAPING),
        epi_states=ROWS,
        timestep=2.0,
    )
...
def test_partial_first_image_keeps_all_validated_source_cells(reference):
    ...
    assert (
        limited.domain_confined_paths_covered
        and limited.clipped_forward_inclusion_certified
    )


def test_final_equality_requires_a_complete_image_and_never_uses_a_partial_layer(
    reference, result
):
    limited = owner.derive_c6_carried_reachable_envelope(
        reference,
        ...
        epi_states=ROWS,
        timestep=2.0,
        max_intersections=result.intersections - 1,
    )
    ...
    complete = owner.derive_c6_carried_reachable_envelope(
        ...
        epi_states=ROWS,
        timestep=2.0,
        max_intersections=result.intersections,
    )
    assert complete.status == "fixed_point" and complete.iterations == result.iterations


def test_actual_six_coordinate_carry_origin_is_preserved_in_zero_pressure_domain(
    reference,
):
    quantum = F(1, 2**3222)
    state = NodalRemainderState(
        (0.5,) * 6, tuple(i * quantum for i in range(6)), 0.375, 0.625
    )
    result = owner.derive_c6_carried_reachable_envelope(
        reference,
        state=state,
    ...
    assert result.retained_zones == result.domain_zones and result.pressures == (
        (0.0,) * 6,
    )
...
def test_explicit_singleton_domain_keeps_only_the_declared_conditional_path(reference):
    ...
    assert (
        result.clipped_forward_inclusion_certified
        and not result.conditional_boundedness_certified
    )
    assert _image(ORIGIN) != ORIGIN


def test_forged_reference_caches_cannot_change_the_reachable_geometry(
    reference, result
):
    forged = replace(
        reference,
    ...
    assert (
        rebuilt.retained_zones == result.retained_zones
        and rebuilt.iterations == result.iterations
    )


@pytest.mark.parametrize(
    "tamper", ("origin_absent", "nonclosed", "outside_rn", "phase", "carry")
)
def test_invalid_domain_or_primitive_is_rejected(reference, result, tamper):
    args = dict(
        state=_state(), epi_states=ROWS, timestep=2.0, domain_zones=result.domain_zones
    )
    if tamper == "origin_absent":
        args["domain_zones"] = tuple(
            z for z in result.domain_zones if z.epi != args["state"].epi
        )
        ...
        if tamper == "nonclosed":
            ...
            b = owner._dbm_box(
                tuple(-10 for _ in range(6)), tuple(10 for _ in range(6))
            )
        args["domain_zones"] = (
            replace(zone, bounds=tuple(map(tuple, b))),
