...
def test_zero_time_preserves_dependency_and_does_not_inflate_coordinate_boxes():
    ...
    sample = bound_p2_transport(initial, capacity=(1, 2), elapsed_times=[(0, 0)]).samples[0]
...
def test_common_uniform_initial_state_remains_exact_for_every_capacity_and_time():
    ...
    assert all(sample.epi == initial and sample.contrast == (0, 0) for sample in tube.samples)
...
@pytest.mark.parametrize("initial,final,time,reason", [
    ...
    (((1, 1), (-1, -1)), ((F(1, 2), F(1, 2)), (-F(1, 2), -F(1, 2))),
...
@pytest.mark.parametrize("capacity,times,reason", [
...
def test_nonfinite_or_nonreal_endpoints_are_not_silently_coerced(value):
    with pytest.raises((TypeError, ValueError)):
        bound_p2_transport(((value, value), (0, 0)), capacity=(1, 1), elapsed_times=[(0, 0)])
...
def test_outward_binary64_capacity_bridge_preserves_realistic_log_enclosure():
    calibration = bound_p2_capacity(
        ((0.9, 1.1), (-1.1, -0.9)), ((0.4, 0.6), (-0.6, -0.4)),
        elapsed_time=(0.9, 1.1))
    exact = calibration.capacity
    bounded = (F.from_float(fraction_lower_float(exact[0])),
               F.from_float(fraction_upper_float(exact[1])))
    assert 0 < bounded[0] <= exact[0] <= exact[1] <= bounded[1]
    sample = bound_p2_transport(
        ((1, 1), (-1, -1)), capacity=bounded, elapsed_times=[(1, 1)]).samples[0]
    with localcontext() as context:
        context.prec = 90
...
@pytest.mark.parametrize("initial,reference", [
    ((F(3), F(5)), (F(-1), F(1))),
    ((F(-5), F(-3)), (F(-1), F(1))),
    ((F(-2), F(2)), (F(-1), F(1))),
])
def test_fixed_reference_transport_contains_independent_corner_trajectories(initial, reference):
    rate = F(1, 5), F(2, 5)
    ...
    with localcontext() as context:
        context.prec = 90
        for sample in tube.samples:
            for x0, r, nu, time in product(initial, reference, rate, sample.elapsed_time):
...
def test_fixed_reference_time_zero_does_not_inflate_by_reference_uncertainty():
    sample = bound_fixed_reference_transport(
        (3, 4), reference=(-100, 100), capacity=(1, 2),
        elapsed_times=[(0, 0)]).samples[0]
...
def test_fixed_reference_mean_changes_while_reference_stays_fixed():
    first, last = bound_fixed_reference_transport(
        (4, 4), reference=(0, 0), capacity=(1, 1),
        elapsed_times=[(0, 0), (1, 1)]).samples
    assert last.epi[1] < first.epi[0]
    assert last.reference == first.reference == (0, 0)
    assert (last.epi[1] + last.reference[1]) / 2 < 2


@pytest.mark.parametrize("initial,final,reference,time,reason", [
...
def test_fixed_reference_calibration_rejects_domain_failures(
    ...
    with pytest.raises(ValueError, match=reason):
        bound_fixed_reference_capacity(initial, final, reference=reference, elapsed_time=time)


@pytest.mark.parametrize("rate,times,reason", [
    ((0, 1), [(1, 1)], "strictly positive"),
    ((1, 1), [(-1, 0)], "nonnegative"),
    ((1, 1), [(1, 2), (0, 3)], "ordered"),
    ((1, 1), [], "between 1 and 10000"),
...
def test_fixed_reference_transport_reuses_domain_and_resource_guards(rate, times, reason):
    with pytest.raises(ValueError, match=reason):
        bound_fixed_reference_transport((1, 1), reference=(0, 0),
                                        capacity=rate, elapsed_times=times)
