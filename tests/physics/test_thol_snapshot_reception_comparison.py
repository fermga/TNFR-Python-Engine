    ...
    for witness in retained["admission"]["witnesses"].values():
        for side in ("control", "perturbed"):
            ...
            for i, row in enumerate(branch["rows"]):
                row["control"] = {
                    "EN_mix": "1/2",
                    "neighbor_indices": branch["snapshot"]["support_neighbors"][i],
                }
                row["configuration"] = {
                    "EPI_MIN": -4.0,
                    "EPI_MAX": 4.0,
...
def test_snapshot_restores_map_symmetry_but_not_captured_field_symmetry():
    ...
    assert {key: len(group) for key, group in symmetry.operator_group_indices} == {
        "A": 2,
        "J": 2,
        "U": 2,
    }
    ...
    assert (
        len(comparison["row_admission"])
        == result["coefficient_builder_calls"]
        == result["kernel_calls"]
        == 16
    )
    assert (
        result["native_calls"]
        == result["scalar_evolution_kernel_calls"]
...
def test_same_snapshot_J_is_local_rows_and_U_uses_pre_generated_A():
    ...
    assert matrices["U"] == tuple(
        tuple(j - a / 4 for j, a in zip(jr, ar, strict=True))
...
def test_actual_paired_environment_is_retained_instead_of_symmetrized():
    ...
    assert result["snapshot_comparison"]["paired_environment_fixed"] == {
        "cohort": True,
        "localized": True,
    }
