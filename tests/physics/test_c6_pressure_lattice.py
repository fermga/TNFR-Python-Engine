...
def test_closed_phase_has_strict_joint_sign_obstruction_with_scoped_conclusion(
    reference,
):
    assert reference.source.phase == PHASE
    assert (reference.source.epi_lower, reference.source.epi_upper) == (0.375, 0.625)
    assert reference.epi_quantum == DELTA and reference.gradient_quantum == Q
    assert tuple(row.nonnegative_min_index for row in reference.rows) == (
        2,
        0,
        -5,
        11,
    ...
    assert tuple(row.nonpositive_max_index for row in reference.rows) == (
        1,
        -1,
        -6,
        10,
    ...
    assert (
        reference.every_pressure_has_positive and reference.every_pressure_has_negative
    )
...
def test_thresholds_use_exact_cell_parity_and_actual_source_reducer(reference):
    ...
    for row, source in zip(reference.rows, reference.sources, strict=True):
        ...
        minimum = math.ceil(scaled_lower) + int(
            not closed and scaled_lower.denominator == 1
        )
        maximum = math.floor(scaled_upper) - int(
            not closed and scaled_upper.denominator == 1
        )
        ...
        def pressure(index):
            ...
            linear = mean_neighbor_difference(
                center,
                (neighbor, center),
                coefficient=CHANNEL_WEIGHT_SECONDARY,
            )
...
@pytest.mark.parametrize(
    ...
    (
        (0.5,) * 6,
        (0.375, 0.625, 0.375, 0.625, 0.375, 0.625),
        (0.625, 0.375, 0.625, 0.375, 0.625, 0.375),
        (
            0.375,
            math.nextafter(0.375, math.inf),
            0.5,
...
def test_actual_scalar_vector_and_fused_pressure_use_the_exact_compatible_lattice(
    reference, epi
):
    observed = owner.observe_c6_pressure_lattice(reference, epi=epi)
    indices = tuple(int((F(value) - F(0.375)) / DELTA) for value in epi)
    integer_gradient = tuple(
        indices[i - 1] + indices[(i + 1) % 6] - 2 * indices[i] for i in range(6)
    )
    gradient = tuple(
        (F(epi[i - 1]) + F(epi[(i + 1) % 6])) / 2 - F(epi[i]) for i in range(6)
    )
    ...
    assert (
        observed.gradient == gradient == tuple(value * Q for value in integer_gradient)
    )
    expected_epi = tuple(
        float(F(CHANNEL_WEIGHT_SECONDARY) * value) for value in gradient
    )
    expected_pressure = tuple(
        float(F(a) + F(g)) for a, g in zip(reference.sources, expected_epi, strict=True)
    )
    assert observed.epi_contributions == expected_epi
    assert observed.pressure == expected_pressure
    assert any(value > 0 for value in observed.pressure) and any(
        value < 0 for value in observed.pressure
    )
    reduction = tuple(
        F(g) - F(CHANNEL_WEIGHT_SECONDARY) * q
        for g, q in zip(expected_epi, gradient, strict=True)
    )
    assembly = tuple(
        F(p) - F(a) - F(g)
        for p, a, g in zip(
            expected_pressure, reference.sources, expected_epi, strict=True
        )
    )
    ...
    assert observed.mean_pressure == (
        observed.mean_phase_contribution
        + observed.mean_epi_reduction_error
        + observed.mean_assembly_error
    )
    assert (
        observed.scalar_reducer_agreement
        and observed.vector_reducer_agreement
        and observed.fused_pressure_agreement
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (0.5, (math.nextafter(0.5, 0.0), 0.5)),
        (0.375, (0.375, math.nextafter(0.375, math.inf))),
    ),
)
def test_both_production_reducers_preserve_exact_half_lattice_across_binades(
    center, neighbors
):
    gradient = sum(map(F, neighbors)) / 2 - F(center)
    assert (gradient / Q).denominator == 1
    scalar = mean_neighbor_difference(center, neighbors, coefficient=1.0)
...
def test_slab_boundary_attains_largest_proven_integer_gradient(reference):
    observed = owner.observe_c6_pressure_lattice(reference, epi=(0.375, 0.625) * 3)
    assert observed.gradient_indices == (2**53, -(2**53)) * 3
    assert observed.epi_indices == (0, 2**52) * 3


def test_actual_zero_mean_pressure_is_not_zero_pressure_or_a_trapping_certificate(
    reference,
):
    observed = owner.observe_c6_pressure_lattice(
        reference, epi=(math.nextafter(0.5, 0.0), 0.5, 0.5, 0.5, 0.5, 0.5)
    )
    assert observed.mean_pressure == 0
    assert any(observed.pressure)
    assert (
        observed.mean_epi_reduction_error + observed.mean_assembly_error
        == -observed.mean_phase_contribution
    )
...
def test_cartesian_corner_obstruction_uses_exact_carried_coordinates_not_visible_cell_exit(
    reference, sign
):
    radius = F(1, 2**60)
    state = NodalRemainderState((0.5,) * 6, (sign * radius,) * 6, 0.05, 1.0)
    ...
    step = advance_nodal_remainder(
        state, timestep=0.0625, capacity=(1.0,) * 6, pressure=pressure
    )
    assert any(
        sign * (after - before) > 0
        for before, after in zip(corner, step.after.exact_epi, strict=True)
    )
    ...
    assert all(F(0.05) <= value <= 1 for value in step.after.exact_epi)


def test_zero_phase_control_permits_the_uniform_singleton_product():
    reference = _derive(phase=(0.0,) * 6)
    assert reference.nonnegative_min_sum == reference.nonpositive_max_sum == 0
    assert (
        not reference.every_pressure_has_positive
        and not reference.every_pressure_has_negative
    )
    assert not reference.no_cartesian_trap
    observed = owner.observe_c6_pressure_lattice(reference, epi=(0.5,) * 6)
    assert observed.pressure == (0.0,) * 6
    state = NodalRemainderState((0.5,) * 6, (F(0),) * 6, 0.05, 1.0)
    assert (
        advance_nodal_remainder(
            state, timestep=0.0625, capacity=(1.0,) * 6, pressure=observed.pressure
        ).after
        == state
    )
...
@pytest.mark.parametrize(
    "lower,upper", ((0.5, 1.0), (0.0625, 0.125), (0.5, 0.5), (0.4375, 0.5625))
)
...
@pytest.mark.parametrize(
    "changes,error",
    (
        ({"epi_upper": math.nextafter(0.625, math.inf)}, ValueError),
        ({"epi_lower": 0.05, "epi_upper": 1.0}, ValueError),
        ({"epi_lower": 0.04}, ValueError),
        ({"epi_upper": 0.25}, ValueError),
        ({"epi_lower": F(3, 8)}, TypeError),
...
@pytest.mark.parametrize(
    ...
    (
        ([0.5] * 6, TypeError),
        ((0.5,) * 5, ValueError),
        ((True, 0.5, 0.5, 0.5, 0.5, 0.5), TypeError),
        ((math.nan, 0.5, 0.5, 0.5, 0.5, 0.5), ValueError),
        ((math.nextafter(0.375, 0.0), 0.5, 0.5, 0.5, 0.5, 0.5), ValueError),
        ((math.nextafter(0.625, math.inf), 0.5, 0.5, 0.5, 0.5, 0.5), ValueError),
    ),
)
...
def test_observer_rebuilds_all_outer_and_nested_derived_caches(reference):
    corrupted_source = replace(
        reference.source, rows=(), gradient_quantum=F(1), no_zero_pressure=False
    )
    forged = replace(
        reference,
        ...
        sources=(0.0,) * 6,
        epi_quantum=F(1),
        gradient_quantum=F(1),
        rows=(),
        nonnegative_min_sum=-1,
        nonpositive_max_sum=1,
        every_pressure_has_positive=False,
        every_pressure_has_negative=False,
        no_cartesian_trap=False,
    )
    observed = owner.observe_c6_pressure_lattice(forged, epi=(0.5,) * 6)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"epi_weight": 0.5}, TypeError),
        ({"phase_weight": F(0)}, ValueError),
        ({"epi_lower": 0.05, "epi_upper": 1.0}, ValueError),
    ),
)
def test_observer_revalidates_primitive_fields_of_forged_sources(
    reference, changes, error
):
    forged = replace(reference, source=replace(reference.source, **changes))
    with pytest.raises(error):
        owner.observe_c6_pressure_lattice(forged, epi=(0.5,) * 6)


def test_shared_reducer_disagreement_cannot_be_published_as_a_binding(
    reference, monkeypatch
):
    monkeypatch.setattr(owner, "mean_neighbor_difference", lambda *args, **kwargs: 1.0)
    with pytest.raises(RuntimeError, match="reducer"):
        owner.observe_c6_pressure_lattice(reference, epi=(0.5,) * 6)
