...
def _memory(branches=False):
    ...
    specs = ((0, ((0, 0),), (4, 0)), (1, tuple(product(range(4, 9), range(-1, 2))), (0, 0)))
    if branches:
    ...
    zones = (C6CarriedForwardZone(rows[0], _hull(((0, 0),))), C6CarriedForwardZone(rows[1], box),
...
def _derive(memory, depth=1, groups=None, **changes):
    kwargs = dict(exclusion_query_max_intersections=10_000, max_partition_pieces=1000,
    ...
    return owner._derive_return_safe_partition(
        memory, groups or (_target(memory),), excluded_predecessor_depth=depth, **kwargs,
    )
        ...
        for target in targets:
            ...
            for terminal in envelope.intermediate_transitions:
                if terminal.source_epi == row and terminal.target_epi == target.epi:
                    bad[i] |= {p for p in points & _members(terminal.source_guard)
                               if (p[0]+terminal.shift[0], p[1]+terminal.shift[1]) in _members(target.bounds)}
        ...
        for i, points in enumerate(zones):
            for j, edge in enumerate(edges):
                if edges[i].target_epi == edge.source_epi:
                    following[i] |= {p for p in points & _members(memory.source_guards[j])
...
def test_complete_predecessor_cuts_match_independent_finite_path_enumeration(memory, depth):
    ...
    for index, old in enumerate(memory.retained_endpoint_zones):
        seeds = tuple(zone for (i, _ordinal), zone in zip(partition.vertices, partition.seed_zones, strict=True) if i == index)
    ...
    assert not partition.conditional_boundedness_certified and not partition.future_runtime_certified
    if depth:
        assert partition.predecessor_complete and partition.excluded_predecessor_depth == depth


def test_every_generated_piece_replays_its_exact_guarded_path_to_the_original_exclusion(memory):
    partition = _derive(memory, 3)
    edges = memory.return_envelope.return_relation
    extra = [piece for piece in partition.bad_pieces if hasattr(piece, "successor_memory_indices")]
    ...
    for piece in extra:
        ...
        for start in _members(piece.bounds):
            ...
            for successor in piece.successor_memory_indices:
                ...
                index = successor
            assert index == original.memory_index and current in _members(original.bounds)
            terminal = memory.return_envelope.intermediate_transitions[original.terminal_transition_index]
...
def test_disconnected_exact_predecessors_do_not_authorize_their_joined_gap():
    ...
    assert (7, -1) in expected[1] and (7, 1) in expected[1] and (7, 0) not in expected[1]
    actual_bad = set().union(*(_members(piece.bounds) for piece in partition.bad_pieces if piece.memory_index == 1))
    seeds = set().union(*(_members(zone) for (i, _ordinal), zone in zip(partition.vertices, partition.seed_zones, strict=True) if i == 1))
...
def test_default_and_explicit_zero_preserve_b59_type_payload_and_work(memory):
    ...
    assert type(default) is owner.C6CarriedReturnSafePartition and type(explicit) is type(default)
    assert default == explicit and asdict(default) == asdict(explicit)
    assert not hasattr(default, "predecessor_complete")


@pytest.mark.parametrize("row,point", ((0, (0, 0)), (1, (4, 0)), (2, (0, 7)), (2, (4, 5))))
def test_origin_and_terminal_only_visits_remain_admitted_after_three_predecessor_layers(memory, row, point):
    partition = _derive(memory, 3)
    target = _target(memory, row, point)
    query = owner._derive_return_safe_partition_region_queries(partition, (target,), 10_000).queries[0]
...
def test_interrupted_generation_never_publishes_a_partial_safe_partition(memory, changes):
    partition = _derive(memory, 2, **changes)
    assert not partition.initialization_complete and not partition.relation_complete
    assert partition.vertices == partition.seed_zones == partition.root_zones == partition.partition_arcs == ()
    assert not partition.conditional_boundedness_certified
    query = owner._derive_return_safe_partition_region_queries(partition, (_target(memory),), 10_000).queries[0]
...
def test_invalid_depth_rejected_before_canonical_reconstruction(monkeypatch, value):
    def forbidden(*_a, **_k):
        raise AssertionError("invalid depth must be rejected before rebuilding the canonical source")
    ...
    with pytest.raises((TypeError, ValueError), match="depth"):
        owner.derive_c6_carried_return_safe_partition(
            ...
            excluded_target_region_groups=(), excluded_predecessor_depth=value,
        )


def test_completed_predecessor_layers_cover_exact_futures_without_joining(memory):
    partition = _derive(memory, 3)
    original = [piece for piece in partition.bad_pieces if not hasattr(piece, "successor_memory_indices")]
...
def test_interruption_inside_second_layer_keeps_only_first_complete_evidence_and_abstains(monkeypatch, memory):
    starts = []
    ...
    guard = starts[0] + complete.predecessor_relation_intersections + first.work + second.work - 1
    ...
    assert interrupted.bad_pieces == interrupted.vertices == interrupted.seed_zones == interrupted.partition_arcs == ()
    assert interrupted.memory_envelope == memory
    query = owner._derive_return_safe_partition_region_queries(interrupted, (_target(memory),), 10_000).queries[0]
    assert not query.origin_path_within_domain_excluded
