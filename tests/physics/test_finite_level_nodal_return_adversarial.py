...
def test_exact_common_scaling_preserves_the_return_restriction(exponent):
    ...
    levels = tuple(value * scale for value in (-2.0, 3.0, 8.0))
    result = derive_finite_level_nodal_return(pressure_levels=levels)
    assert all(
        F(value) == integer * F(scale)
        for value, integer in zip(levels, (-2, 3, 8), strict=True)
    )
...
@pytest.mark.parametrize(
    ...
    [
        (0.5, 2, 5),
        (0.25, 4, 1),
        (math.ldexp(1.0, -100), 2**100, 1),
        (math.ulp(0.0), 2**1074, 1),
    ],
)
def test_finer_denominator_superset_can_retain_or_weaken_the_modulus(
    ...
    original = derive_finite_level_nodal_return(pressure_levels=(-2.0, 3.0))
    extended = derive_finite_level_nodal_return(pressure_levels=(-2.0, 3.0, extra))
    ...
    assert tuple(
        F(z, extended.common_denominator) for z in extended.integer_levels
    ) == tuple(F(value) for value in (-2.0, 3.0, extra))
    # Refinement changes the integer encoding, not any old pressure or witness.
    assert 3 * F(-2.0) + 2 * F(3.0) == 0


def test_superset_can_remove_only_part_of_a_nonprime_length_restriction():
    pair = derive_finite_level_nodal_return(pressure_levels=(-4.0, 14.0))
    triple = derive_finite_level_nodal_return(pressure_levels=(-4.0, 14.0, 26.0))
...
def test_extreme_denominator_and_magnitude_do_not_use_float_period_arithmetic():
    tiny, largest = math.ulp(0.0), sys.float_info.max
...
def test_near_cancelling_binary64_levels_have_large_exact_restriction():
    ...
    result = derive_finite_level_nodal_return(pressure_levels=(-1.0, above_one))
    assert result.integer_levels == (-(2**52), 2**52 + 1)
    assert result.necessary_length_multiple == 2**53 + 1
    assert F(-1.0) + F(above_one) == F(1, 2**52)
    assert (2**52 + 1) * F(-1.0) + 2**52 * F(above_one) == 0
