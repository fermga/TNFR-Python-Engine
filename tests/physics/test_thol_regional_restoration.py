...
def _fixture():
    ...
    graph.graph.update(
        _t=1.5,
        _node_sample=nodes,
        hierarchy=dict(zip(parents, ([c] for c in children))),
        history={"C_steps": deque((0.75, 0.8), maxlen=20)},
        RANDOM_SEED=3,
    )
    ...
    prefix = {
        "birth": {
            "parent_children": tuple(zip(parents, children)),
    ...
    prior = {
        "branch": "control",
        "status": "executed",
        "endpoint": study.full._state(graph),
        "prefix": prefix,
        "lineage": lineage,
    }
    index = {node: i for i, node in enumerate(nodes)}
    edges = tuple(
        sorted((index[u], index[v], F(1)) for u in nodes for v in graph.neighbors(u))
    )
    ...
    snapshot = _from_data(
        ...
        edges,
        support,
        tuple(F(graph.nodes[n]["EPI"]) for n in nodes),
        (F(0.95),) * 16,
        (F(0),) * 16,
    )
    original = derive_forced_support_balance(
        snapshot, epi_weight=F(1), forcing=(F(0),) * 16
    )
...
def test_shared_copy_restores_neighbor_order_and_detaches_aliases_and_histories():
    ...
    assert (
        tuple(control.neighbors(7))
        == tuple(perturbed.neighbors(7))
        == (6, 0, "child_7")
    )
    ...
    for clone in (control, perturbed):
        assert (
            clone.graph["history_alias"] is clone.nodes["child_0"]["epi_time_history"]
        )
...
@pytest.mark.parametrize(
    "mutation",
    ("time", "lineage", "node_order", "pressure_hook", "selector", "integrator"),
)
...
def _portable_runtime(
    monkeypatch, *, refused=None, allowed=True, jump=0.0625, no_contrast=False
):
    ...
    monkeypatch.setattr(
        ...
        lambda g, targets: {
            ...
            "targets": targets,
            "rows": (),
            "fixture": "not native admission evidence",
        },
    )
    ...
    def event(g, targets):
        ...
        return {
            "before": before,
            "after": study.full._state(g),
            "fixture": "synthetic localized write",
        }
    ...
    def analyze(*args):
        ...
        return {
            "fixture": "accounting tested independently; no numerical assertion here"
        }
    ...
    monkeypatch.setattr(
        study.native,
        "_tetrad",
        lambda g: tetrads.append(g) or {"fixture": "detached readout seam"},
    )
...
def test_fixed_six_step_budget_localized_energy_and_lossless_packing(monkeypatch):
    control, perturbed, prior, original, calls, analyses, events, tetrads = (
        _portable_runtime(monkeypatch)
    )
    ...
    assert [len([x for x in calls if x[0] is g]) for g in (control, perturbed)] == [
        6,
        6,
    ]
    assert all(
        [row[2] for row in calls if row[0] is g] == [1.5, 1.75, 2.0, 2.25, 2.5, 2.75]
    ...
    assert (
        len(a) == len(b) == 6
        and model is original
        and children == prior["lineage"]["children"]
    )
    ...
    for branch, raw in zip(result["branches"], (a, b), strict=True):
        ...
        assert [
            study.window.expand_record(ref, pool.nodes) for ref in branch["steps"]
        ] == study.full._payload(raw)


@pytest.mark.parametrize(
    "options,status,event_count",
    (
...
def test_initial_gates_stop_before_native_calls(
    monkeypatch, options, status, event_count
):
    control, perturbed, prior, _, calls, analyses, events, tetrads = _portable_runtime(
        monkeypatch, **options
    )
    result = study.run_branches(
        control, perturbed, prior, pool=study.window.RecordPool()
    )
...
def test_native_refusal_stops_paired_study_and_analyzes_common_prefix(monkeypatch):
    control, perturbed, prior, _, calls, analyses, _, _ = _portable_runtime(
        monkeypatch, refused=(1, 2)
    )
    result = study.run_branches(
        control, perturbed, prior, pool=study.window.RecordPool()
    )
...
def test_fixed_domain_analysis_refusal_retains_all_attempted_records(monkeypatch):
    ...
    assert (
        len(result["branches"][0]["steps"]) == len(result["branches"][1]["steps"]) == 6
    )
...
def test_one_source_replay_and_complete_report_admission_precede_fork(
    ...
    path.write_text(
        json.dumps(study.full._payload({"branches": (prior,)})), encoding="utf8"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda branch: chronology.append(("replay", branch)) or (graph, prior),
    )
    ...
    monkeypatch.setattr(
        study,
        "run_branches",
        lambda *a, **k: chronology.append(("continue",))
        or {"status": "fixture", "attempted_native_calls": 0},
    )
    ...
    packed = study.window.expand_record(
        result["replayed_control_report"], result["record_pool"]
    )
    ...
    assert (
        packed["fixture_glyph_scalar"] == "IL"
        and type(packed["fixture_glyph_scalar"]) is str
    )
...
def test_source_pack_failure_prevents_all_new_branch_execution(
    ...
    monkeypatch.setattr(
        study.full, "replay_response_branch", lambda branch: (graph, prior)
    )
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError(
            "branch execution must follow both successful source packs"
        )
...
def test_cli_never_overwrites_source(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sys, "argv", ["study", "--input", str(path), "--output", str(path)]
    )
...
def _cli_fixture(monkeypatch, tmp_path):
    ...
    calls = []
    monkeypatch.setattr(
        sys, "argv", ["study", "--input", str(source), "--output", str(output)]
    )
    monkeypatch.setattr(
        study, "current_git_source_provenance", lambda *args: ("a" * 40, False, None)
    )
    ...
    def synthetic_run(*args, **kwargs):
        ...
        return {
            ...
            "record_pool": {},
            "exact_fixture_value": F(1, 7),
            "fixture_glyph": Glyph.IL,
...
def test_complete_cli_prevalidates_real_manifest_and_saves_complete_result(
    monkeypatch, tmp_path
):
...
def test_cli_metadata_failure_runs_no_study(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *args: ("not-a-git-sha", False, None),
    )
    with pytest.raises(ValueError):
        study.main()
    assert (
        ...
        and not output.with_suffix(".completed.json").exists()
    )
