...
def _observe(before=None, after=None, *, dt=F(1, 2), region=("a", "b"), forcing=(0, 0, 0)):
    before = _path() if before is None else before
    after = replace(before, epi=(F(1, 2), F(5, 2), F(3))) if after is None else after
    return observe_regional_support_euler(before, after, region, dt=dt, epi_weight=1, forcing=forcing)
...
def test_hand_path_exact_euler_keeps_boundary_growth_and_quadratic_term():
    ...
    assert result.variance_defect_linear_term == result.variance_defect_quadratic_term == 0
...
def test_uniform_endpoint_shift_changes_mean_only_relative_to_euler_reference():
    ...
    shifted = _observe(before, replace(before, epi=tuple(x+7 for x in exact.expected_epi)))
    ...
    assert shifted.variance_defect_linear_term == shifted.variance_defect_quadratic_term == 0
...
def test_after_pressure_is_not_used_to_replace_the_held_initial_rate():
    ordinary = _observe()
    updated = _observe(after=replace(ordinary.after, stored_pressure=(F(100), F(-20), F(7))))
    for field in ("expected_epi", "state_defect", "mass_drift_term", "variance_drift_term",
                  "variance_quadratic_term", "mass_change", "variance_change"):
...
def test_zero_duration_can_account_for_a_detached_event_without_claiming_integration():
    ...
    assert result.mass_drift_term == result.variance_drift_term == result.variance_quadratic_term == 0
...
def test_nonuniform_metric_direct_endpoint_quadratic_matches_complete_decomposition():
    before = _path(epi=(F(-2, 3), F(7, 5), F(11, 7)), capacity=(2, 3, 5),
    ...
    direct = coefficient*((after.epi[0]-after.epi[1])**2-(before.epi[0]-before.epi[1])**2)
    assert result.variance_change == direct
    assert direct == (result.variance_drift_term+result.variance_quadratic_term
                      + result.variance_defect_linear_term+result.variance_defect_quadratic_term)
    assert result.mass_change == h0*(after.epi[0]-before.epi[0])+h1*(after.epi[1]-before.epi[1])
    assert result.mass_identity_residual == result.variance_identity_residual == 0
...
def test_forged_caches_are_ignored_but_bad_endpoint_primitives_fail():
    ordinary = _observe()
    cached = {"rate": (F(99),)*3, "epi_gradient": (F(99),)*3,
              "dirichlet_energy": F(99), "energy_rate": F(99)}
    assert _observe(replace(ordinary.before, **cached), replace(ordinary.after, **cached)) == ordinary
