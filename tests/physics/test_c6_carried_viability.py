...
def _state(row, carry=F(0)):
    return NodalRemainderState(row, (carry,) * 6, 0.375, 0.625)
...
def _advance(reference, state):
    ...
    return advance_nodal_remainder(
        state, timestep=1.0, capacity=(1.0,) * 6, pressure=pressure
    )


def _swap_rows():
    first = (0.4375, 0.5625) * 3
    return first, first[::-1]


def _transient_candidate(reference):
    rows = _swap_rows() + ((0.4375, 0.4375, 0.4375, 0.4375, 0.4375, 0.5625),)
...
def test_zero_pressure_whole_rn_cell_is_an_exact_conditional_fixed_point(reference):
    ...
    result = derive_c6_carried_viability(
        reference, state=state, epi_states=(row,), timestep=1.0
    )
...
def test_nonzero_canonical_two_cycle_proves_invariance_without_claiming_convergence(
    reference,
):
    rows = _swap_rows()
    state = _state(rows[0], GRID)
    result = derive_c6_carried_viability(
        reference, state=state, epi_states=rows, timestep=1.0
    )
    ...
    assert first.pressure != (0.0,) * 6 and second.pressure != first.pressure
...
def test_exact_descending_partition_can_remove_transients_then_certify_a_cycle(
    reference,
):
...
def test_two_descents_exclude_origin_with_a_verified_two_step_witness(reference):
    origin = _state((0.5, 0.5, 0.5, 0.5, 0.5, 0.625))
...
def test_one_step_leaving_the_declared_band_is_also_an_exact_exclusion(reference):
    rows = _swap_rows()
    result = derive_c6_carried_viability(
        reference, state=_state(rows[0]), epi_states=rows, timestep=4.0
    )
    assert result.status == "origin_excluded" and result.exclusion_step_bound == 1
    pressure = result.pressures[0]
    candidate = tuple(
        value + 4 * F(p) for value, p in zip(result.state.exact_epi, pressure)
    )
    assert any(not F(0.375) <= value <= F(0.625) for value in candidate)


def test_limit_during_second_descent_exposes_only_the_first_complete_partition(
    reference,
):
    ...
    limited = derive_c6_carried_viability(
        ...
        epi_states=complete.epi_states,
        timestep=1.0,
        initial_boxes=complete.initial_boxes,
        max_work_items=complete.work_items - 1,
    )
    ...
    assert (
        ...
        and not limited.origin_exit_certified
    )
...
def test_limit_before_any_complete_descent_retains_the_validated_candidate(reference):
    ...
    assert (
        limited.iterations[0].point_count == 2 and limited.iterations[0].origin_retained
    )


def test_complete_small_affine_domain_matches_independent_pointwise_preimages(
    reference,
):
    first = (math.nextafter(0.5, -math.inf), 0.5) * 3
    rows, state = (first, first[::-1]), _state(first)
    result = derive_c6_carried_viability(
        reference, state=state, epi_states=rows, timestep=0.25, max_boxes=100
    )
    ...
    def indices(box):
        lower = tuple(
            int((a - x) / g)
            for a, x, g in zip(
                box.lower, result.affine_origin, result.coordinate_spacings
            )
        )
        upper = tuple(
            int((b - x) / g)
            for b, x, g in zip(
                box.upper, result.affine_origin, result.coordinate_spacings
            )
        )
    ...
    for row in rows:
        ...
        translations[row] = tuple(
            int(F(0.25) * F(p) / g)
            for p, g in zip(pressure, result.coordinate_spacings)
        )
    image = {
        point: tuple(a + d for a, d in zip(point, translations[row]))
        for point, row in source.items()
    }
    ...
    assert tuple(record.point_count for record in result.iterations) == (
        18522,
        3458,
        254,
    )
    assert {
        point for box in result.retained_boxes for point in indices(box)
    } == second_kernel
...
def test_piece_limit_discards_a_complete_but_unadmitted_partial_partition(reference):
    ...
    assert (
        result.iterations[0].box_count == 2
        and result.iterations[0].point_count == 18522
    )
    assert (
        not result.origin_exit_certified and not result.conditional_invariance_certified
    )


def test_even_and_odd_rn_ties_are_read_from_the_shared_grid_owner(reference):
    for visible in (0.5, math.nextafter(0.5, -math.inf)):
        row = (visible,) * 6
        result = derive_c6_carried_viability(
            reference, state=_state(row), epi_states=(row,), timestep=1.0
        )
        ...
        previous, following = math.nextafter(visible, -math.inf), math.nextafter(
            visible, math.inf
        )
        lower_tie, upper_tie = (F(previous) + F(visible)) / 2, (
            F(visible) + F(following)
        ) / 2
        if visible == 0.5:
...
def test_candidate_cells_are_clipped_to_the_actual_state_band(reference):
    row = (0.5,) * 6
    state = replace(_state(row), epi_lower=0.5)
    result = derive_c6_carried_viability(
        reference, state=state, epi_states=(row,), timestep=1.0
    )
    assert (
        result.status == "fixed_point"
        and result.retained_boxes[0].lower == (F(0.5),) * 6
    )
...
def test_adjacent_same_cell_boxes_coalesce_without_filling_a_gap(reference):
    row = (0.5,) * 6
    base = _box(row)
    first = replace(base, lower=(F(0.5) - GRID,) + base.lower[1:])
    second = replace(
        base,
        lower=(F(0.5) + GRID,) + base.lower[1:],
        upper=(F(0.5) + GRID,) + base.upper[1:],
    )
    result = derive_c6_carried_viability(
        reference,
        state=_state(row),
...
def test_forged_derived_reference_fields_are_rebuilt(reference):
    ...
    result = derive_c6_carried_viability(
        forged, state=_state(row), epi_states=(row,), timestep=1.0
    )
    assert result.reference == reference and result.pressures == ((0.0,) * 6,)
    assert result.status == "fixed_point"
...
def test_initial_size_limit_is_checked_before_search(reference):
    ...
    with pytest.raises(ValueError, match="max_boxes"):
        derive_c6_carried_viability(
            reference, state=_state(rows[0]), epi_states=rows, timestep=1.0, max_boxes=1
        )
...
def test_duplicate_rows_and_overlapping_boxes_are_rejected(reference):
    ...
    with pytest.raises(ValueError, match="distinct"):
        derive_c6_carried_viability(
            reference, state=_state(row), epi_states=(row, row), timestep=1.0
        )
    with pytest.raises(ValueError, match="disjoint"):
        derive_c6_carried_viability(
            ...
            epi_states=(row,),
            timestep=1.0,
            initial_boxes=(_box(row), _box(row)),
        )
...
def test_initial_box_must_contain_the_unmodified_origin(reference):
    ...
    with pytest.raises(ValueError, match="unchanged supplied origin"):
        derive_c6_carried_viability(
            ...
            epi_states=rows,
            timestep=1.0,
            initial_boxes=(_box(rows[1]),),
        )


def test_custom_bounds_cannot_include_an_odd_rounding_tie(reference):
    visible = math.nextafter(0.5, -math.inf)
    row = (visible,) * 6
    midpoint = (F(visible) + F(0.5)) / 2
...
def test_noncanonical_state_or_nonpositive_timestep_is_rejected(reference):
    ...
    with pytest.raises(TypeError, match="exact NodalRemainderState"):
        derive_c6_carried_viability(
            reference, state=None, epi_states=(row,), timestep=1.0
        )
    with pytest.raises(ValueError, match="positive timestep"):
        derive_c6_carried_viability(
            reference, state=_state(row), epi_states=(row,), timestep=0.0
        )
    state = initialize_nodal_remainder(row, epi_lower=0.25, epi_upper=0.75)
