...
def _derive(initial, *, timestep=1., capacity=1., pressure=1., max_steps=1):
    return derive_nodal_area_crossings(
        initial_area=(initial,), timestep=timestep, capacity=(capacity,),
        pressure=(pressure,), max_steps=max_steps,
    )
...
def test_binary64_cancellation_cannot_create_a_false_exact_return(direction, offset):
    ...
    assert float(initial) - direction == 0.
...
def test_underflowing_exact_baseline_is_nonzero_and_can_be_repaid(direction):
    ...
    assert float(initial) == 0.
    observed = _derive(initial, timestep=MIN_SUBNORMAL, capacity=MIN_SUBNORMAL,
    ...
    assert coordinate.area_before_crossing == initial and coordinate.area_at_crossing == 0
...
def test_non_dyadic_supplied_area_is_algebraic_input_not_provenance():
    observed = _derive(-F(1, 3), pressure=.25, max_steps=2)
    coordinate, = observed.coordinates
...
def test_astronomical_crossing_index_is_solved_without_iterating_the_prefix():
    root = 2**3222
    before = _derive(-F(1), timestep=MIN_SUBNORMAL, capacity=MIN_SUBNORMAL,
                     pressure=MIN_SUBNORMAL, max_steps=root - 1)
    coordinate, = before.coordinates
    assert coordinate.continuous_zero_step == root
    assert coordinate.integer_zero_step == coordinate.first_zero_or_opposite_step == root
    ...
    at = _derive(-F(1), timestep=MIN_SUBNORMAL, capacity=MIN_SUBNORMAL,
                 pressure=MIN_SUBNORMAL, max_steps=root)
...
def test_roots_that_round_to_the_same_float_do_not_establish_a_joint_return():
    ...
    result = derive_nodal_area_crossings(
        initial_area=(-F(1), -F(1) - separation), timestep=1.,
        capacity=(1., 1.), pressure=(1., 1.), max_steps=2,
    )
    roots = tuple(coordinate.continuous_zero_step for coordinate in result.coordinates)
    assert roots == (F(1), F(1) + separation)
    assert tuple(map(float, roots)) == (1., 1.)
...
def test_initial_vector_zero_is_distinct_from_any_later_common_return():
    result = derive_nodal_area_crossings(
        initial_area=(F(0), F(0)), timestep=1.,
        capacity=(1., 1.), pressure=(MIN_SUBNORMAL, -MIN_SUBNORMAL), max_steps=2**1074,
    )
...
def test_a_tiny_nonzero_stationary_coordinate_blocks_every_joint_zero():
    result = derive_nodal_area_crossings(
        initial_area=(-F(1), GRID), timestep=1.,
        capacity=(1., 0.), pressure=(1., 1.), max_steps=1,
    )
