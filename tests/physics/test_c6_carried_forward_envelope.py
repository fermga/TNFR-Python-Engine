...
def _state(indices=ORIGIN_INDICES):
    return NodalRemainderState(tuple(VALUES[i] for i in indices), (F(0),) * 6, .375, .625)


def _step(reference, state, timestep):
    return advance_nodal_remainder(
        state, timestep=timestep, capacity=(1.,) * 6,
...
def _singleton_indices(result, zones):
    ...
    for zone in zones:
        ...
        assert low == high
        exact = tuple(a + result.grid_quantum * index
...
def triple(reference):
    return owner.derive_c6_carried_forward_envelope(
        reference, state=_state(), epi_states=TRIPLE_ROWS,
        timestep=2., derive_pair_barriers=False,
    )


def test_zero_pressure_core_certifies_stationarity_with_unchanged_nonzero_carry(reference):
    row = (.5,) * 6
    state = NodalRemainderState(row, tuple(i * GRID for i in range(6)), .375, .625)
    result = owner.derive_c6_carried_forward_envelope(
        reference, state=state, epi_states=(row,), timestep=1., derive_pair_barriers=False,
    )
    ...
    assert result.entry_state == state and result.entry_steps == () and result.entry_failure is None
...
def test_positive_nonstationary_core_preserves_the_actual_carried_step(reference):
    ...
    result = owner.derive_c6_carried_forward_envelope(
        reference, state=state, epi_states=rows, timestep=.5, derive_pair_barriers=False,
    )
    assert result.status == "invariant_core" and result.conditional_boundedness_certified
    ...
    first = _step(reference, state, .5)
    assert first.after != state and first.pressure != (0.,) * 6
    ...
    assert _step(reference, first.after, .5).after == first.after
    assert not result.asymptotic_convergence_certified


def test_every_canonical_pressure_matches_the_independent_729_state_integer_map(reference, triple):
    assert triple.grid_quantum == DELTA
    for point, row, pressure in zip(product(range(3), repeat=6), TRIPLE_ROWS, triple.pressures, strict=True):
        target = _integer_image(point)
        expected = tuple(DELTA * (after - before) for before, after in zip(point, target, strict=True))
        ...
        state = _state(point)
        step = advance_nodal_remainder(state, timestep=2., capacity=(1.,) * 6, pressure=fresh)
        assert step.after.exact_epi == tuple(F(VALUES[0]) + DELTA * value for value in target)
...
def test_exact_forward_layers_match_the_complete_independent_finite_oracle(triple):
    ...
    assert tuple(item.zone_count for item in triple.iterations) == tuple(map(len, layers))
    assert tuple(item.origin_retained for item in triple.iterations) == (True, False, False, False)
    assert tuple(item.outgoing_facets for item in triple.iterations) == (1296, 108, 12, 0)
    ...
    assert triple.conditional_invariance_certified and triple.conditional_boundedness_certified
...
def test_origin_outside_core_is_bound_by_a_fresh_proof_derived_entry(reference, triple):
    ...
    for retained in triple.entry_steps:
        fresh = _step(reference, current, 2.)
    ...
    assert triple.entry_steps[0].pressure != (0.,) * 6
    assert all(step.pressure == (0.,) * 6 for step in triple.entry_steps[1:])
...
def test_incomplete_image_discards_partial_targets_and_retains_the_last_complete_layer(reference, triple):
    limited = owner.derive_c6_carried_forward_envelope(
        reference, state=_state(), epi_states=TRIPLE_ROWS, timestep=2., derive_pair_barriers=False,
    ...
    assert not limited.conditional_invariance_certified and not limited.conditional_boundedness_certified
    assert not limited.origin_exit_certified


def test_exactly_completed_intersection_budget_still_permits_the_inclusion_proof(reference, triple):
...
def test_reference_caches_are_rebuilt_and_entry_pressure_is_refreshed(reference, triple, monkeypatch):
    forged = replace(reference, sources=(F(99),) * 6, epi_quantum=F(3),
                     gradient_quantum=F(7), rows=(), every_pressure_has_positive=True)
    calls = []
    ...
    result = owner.derive_c6_carried_forward_envelope(
        forged, state=_state(), epi_states=TRIPLE_ROWS, timestep=2., derive_pair_barriers=False,
    )
    assert result.reference == reference and result.pressures == triple.pressures
    assert result.retained_zones == triple.retained_zones and result.entry_steps == triple.entry_steps
    assert len(calls) == len(TRIPLE_ROWS) + len(result.entry_steps)
    assert all(ref == reference for ref, _row in calls)
    assert tuple(row for _ref, row in calls[-3:]) == tuple(step.before.epi for step in result.entry_steps)


def test_derived_pair_intervals_are_universally_protected_on_the_declared_source(reference):
    result = owner.derive_c6_carried_forward_envelope(
        reference, state=_state(), epi_states=TRIPLE_ROWS, timestep=2., derive_pair_barriers=True,
    )
    ...
    for barrier in result.pair_barriers:
        ...
        for point in product(range(3), repeat=6):
            ...
            if barrier.lower <= value <= barrier.upper:
                target = _integer_image(point)
                after = tuple(x - a for x, a in zip(target, ORIGIN_INDICES, strict=True))
                assert barrier.lower <= after[barrier.first] - after[barrier.second] <= barrier.upper
    assert _contains(result, result.initial_zones, result.state)


@pytest.mark.parametrize("argument,value,error", (
    ("max_intersections", 0, ValueError), ("max_intersections", True, ValueError),
    ("max_cells", 0, ValueError), ("max_cells", 1., ValueError),
    ("derive_pair_barriers", 1, TypeError), ("derive_pair_barriers", None, TypeError),
))
def test_resource_and_policy_parameters_have_strict_primitive_types(reference, argument, value, error):
    with pytest.raises(error):
        owner.derive_c6_carried_forward_envelope(
            reference, state=_state(), epi_states=TRIPLE_ROWS, timestep=2., **{argument: value},
        )
