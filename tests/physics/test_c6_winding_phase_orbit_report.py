...
BASE_HEX = (
    "0x0.0p+0", "0x1.0c152382d7365p+0", "0x1.0c152382d7365p+1",
...
def test_bounded_discovery_finds_first_repeat_and_preserves_every_transient(report):
    ...
    for before, after, step in zip(states[:-1], states[1:], orbit["steps"], strict=True):
        assert before == step["phase_before"] and after == step["phase_after_coherence"]
        assert tuple(value.hex() for value in step["phase_after_coupling"][1:]) == BASE_HEX[1:]
...
def test_first_two_phase_transitions_bind_the_historical_actual_operators(parent, report):
    assert len(report["source"]["first_two_retained_bindings"]) == 2
    for recorded, step, binding in zip(
        parent["current_cases"][0]["cycles"], report["phase_orbit"]["steps"][:2],
        report["source"]["first_two_retained_bindings"], strict=True,
    ):
        assert all(binding[name] for name in ("before_matches", "UM_matches", "IL_matches"))
        assert tuple(map(F, step["phase_after_coupling"])) == tuple(map(F, recorded["um"]["raw_capture"]["phase"]))
        assert tuple(map(F, step["phase_after_coherence"])) == tuple(map(F, recorded["il"]["raw_capture"]["phase"]))
...
def test_all_transition_boundaries_retain_unit_cycle_phase_support(report):
    for step in report["phase_orbit"]["steps"]:
        ...
        for edges, nonedges in zip(step["edge_margins"], step["nonedge_margins"], strict=True):
            assert len(edges) == 6 and len(nonedges) == 9
            assert min(edges) > .52 and min(nonedges) > .52
        assert step["um_phase_factor"].hex() == "0x1.ee7eea04ddca0p-3"
        assert step["il_phase_factor"] == .3
...
def test_closed_phase_source_has_nonzero_exact_bias_and_no_zero_pressure_point(report):
    ...
    assert obstruction["phase"] == report["phase_orbit"]["steps"][-1]["phase_after_coherence"]
    assert obstruction["no_zero_pressure"] is True
    assert tuple((row["first_grid_index"], row["last_grid_index"]) for row in obstruction["rows"]) == (
    ...
    assert sum(row["phase_response"]["delta_rational"] for row in obstruction["rows"]) == 0
    assert sum(row["phase_response"]["delta_pi_coefficient"] for row in obstruction["rows"]) == 0
    budget = report["periodic_phase_source"]
    assert budget["phase_contributions"] == (source,)
    assert budget["period"] == 1 and budget["block_duration"] == .25
...
def test_only_selected_source_is_validated_and_no_live_word_or_flow_runs(parent, monkeypatch):
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError("B25 must not execute a new graph campaign or operator word")
...
def test_phase_closure_never_promotes_the_unobserved_epi_and_mean_conditions(report):
    ...
    for flag in ("runtime_executed", "operator_word_admission_certified", "live_provenance_certified",
    ...
    assert report["source_budget_alignment"] == "Each quarter-duration block follows the cycle step's Coherence output"
    json.dumps(campaign._payload(report), allow_nan=False)


@pytest.mark.parametrize("change", ("seed", "horizon", "order", "initial_phase", "stage_phase", "support"))
