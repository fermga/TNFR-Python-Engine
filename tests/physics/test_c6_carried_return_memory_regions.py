...
def _members(bounds):
    if bounds is None:
        return set()
    return {value for value in range(-1, 12) if all(
...
def memory():
    ...
    terminals = tuple(owner.C6CarriedReturnTransition(
        rows[0], None, rows[2], _hull((value,)), None, _hull((value + 1,)), (1, 0, 0, 0, 0, 0),
    ) for value in (2, 10))
    ...
    return owner._derive_return_memory_envelope(env, max_memory_work=1000, max_memory_arcs=100)


def _targets(memory, *entries):
    return tuple(C6CarriedForwardZone(memory.return_envelope.epi_states[index], _hull(values))
...
def _oracle(memory, targets):
    ...
    graph.extend((sentinel, j, 0, edges[j].shift[0]) for j, root in enumerate(memory.root_endpoint_zones)
                 if root is not None)
    current = []
        ...
        for target in targets:
            ...
            for edge in envelope.intermediate_transitions:
                if edge.source_epi == row and edge.target_epi == target.epi:
                    selected.update(
                        v for v in zone & _members(edge.source_guard) if v + edge.shift[0] in target_values
                    )
...
def test_complete_layers_match_independent_enumeration_including_terminal_pieces(memory, entries):
    ...
    assert query.initial_endpoint_zones == expected[0] and query.retained_endpoint_zones == expected[-1]
...
def test_return_to_origin_rn_mode_is_distinct_from_original_zero_sentinel(memory):
    ...
    assert not reached.iterations[0].origin_present and reached.iterations[-1].origin_present
    assert unreachable.status == "stationary_complete_layer" and unreachable.completed_depth == 1
...
def test_each_query_budget_retains_only_complete_layers(memory):
    ...
    for maximum in range(1, complete.queries[0].intersections + 1):
        ...
        query = result.queries[0]
        assert query.intersections <= maximum and result.query_relation_intersections <= maximum
        ...
        if not query.initialization_complete:
            assert not query.origin_path_within_domain_excluded and query.completed_depth is None
...
def test_partial_target_initialization_discards_every_piece(memory):
    ...
    for maximum in range(complete.query_relation_intersections, complete.queries[0].intersections):
        result = _query(memory, targets, maximum)
        query = result.queries[0]
        if result.query_relation_complete and not query.initialization_complete:
            ...
            assert query.initial_endpoint_zones == query.retained_endpoint_zones == query.iterations == ()
            assert not query.origin_path_within_domain_excluded and query.intersections == maximum
...
def test_each_whole_target_gets_its_own_budget(memory):
    ...
    result = owner._derive_return_memory_region_queries(memory, (targets, targets), maximum)
    assert result.queries[0] == result.queries[1]
    assert all(q.status == "resource_limit" and q.intersections == maximum for q in result.queries)


def test_incomplete_pair_graph_cannot_supply_any_query_exclusion(memory):
    for incomplete in (replace(memory, pair_relation_complete=False, pair_arcs=()),
                       replace(memory, initialization_complete=False, pair_relation_complete=False,
                               retained_endpoint_zones=(), pair_arcs=())):
        result = _query(incomplete, _targets(memory, (0, (10,))))
        assert not result.query_relation_complete and result.query_relation_intersections == 0
        query = result.queries[0]
        assert query.status == "memory_query_construction_resource_limit"
        assert not query.initialization_complete and not query.origin_path_within_domain_excluded
...
@pytest.mark.parametrize("entries", (
    ((0, (0,)),), ((0, (2,)),), ((2, (3,)),),
    ((0, (2,)), (2, (11,))), ((0, (2,)), (1, (1,)), (2, (3, 11))),
))
def test_shared_initialization_helper_preserves_exact_arrays_and_atomic_budget(memory, entries):
    targets = _targets(memory, *entries)
    ...
    initial, complete, work = owner._return_memory_query_initialization(memory, targets, required)
    assert complete and initial == expected and work == required
    for maximum in range(1, required):
        initial, complete, work = owner._return_memory_query_initialization(memory, targets, maximum)
        assert initial == () and not complete and work == maximum


def test_shared_helpers_keep_initial_cover_and_graph_completeness_as_separate_premises(memory):
    targets = _targets(memory, (0, (2,)))
    ...
    assert owner._return_memory_query_initialization(incomplete_graph, targets, 1000) == (
        owner._return_memory_query_initialization(memory, targets, 1000)
    )
    incomplete_cover = replace(incomplete_graph, initialization_complete=False, retained_endpoint_zones=())
    assert owner._return_memory_query_initialization(incomplete_cover, targets, 1000) == ((), False, 0)
...
def _canonical(reference, origin=ORIGIN, points=(ORIGIN, OTHER, *CHAIN), targets=(CHAIN[-1],), **changes):
...
def test_canonical_full_targets_exclude_by_empty_or_exact_stationary_layers(reference):
    ...
    assert tuple(q.status for q in result.queries) == (
        "empty_complete_layer", "stationary_complete_layer", "origin_not_excluded",
    )
    assert tuple(q.completed_depth for q in result.queries) == (0, 1, 0)
    assert tuple(q.origin_path_within_domain_excluded for q in result.queries) == (True, True, False)
...
def test_nonstationary_memory_cover_allows_complete_backward_exclusion(reference):
    ...
    coarse = _canonical(reference, max_intersections=envelope.construction_intersections)
    memory = coarse.memory_envelope
    result = _canonical(reference, max_intersections=envelope.construction_intersections,
...
def test_terminal_transient_without_any_completed_origin_return_is_still_detected(reference):
    result = _canonical(reference, origin=CHAIN[0], points=(ORIGIN, CHAIN[0], CHAIN[1]), targets=(CHAIN[1],))
    memory, query = result.memory_envelope, result.queries[0]
    assert all(edge.source_epi != _row(CHAIN[0]) for edge in memory.return_envelope.return_relation)
    pressure = memory.return_envelope.pressures[memory.return_envelope.epi_states.index(_row(CHAIN[0]))]
    step = advance_nodal_remainder(_state(CHAIN[0]), timestep=2., capacity=(1.,) * 6, pressure=pressure)
...
@pytest.mark.parametrize("changes", ({"query_max_intersections": True}, {"query_max_intersections": 0},
