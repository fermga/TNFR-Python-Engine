...
def _box(epi=(0.5,), pressure=(0.0,), **kwargs):
    return owner.derive_binary64_quarter_pressure_box(
        epi=epi, pressure=pressure, **kwargs
    )
...
def _trace(epi, pressure):
    ...
    for _ in range(4):
        values = tuple(
            x + 0.0625 * (1.0 * p + 0.0) for x, p in zip(values, pressure, strict=True)
        )
        rows.append(values)
    return tuple(rows)
...
def test_half_stasis_recovers_the_exact_asymmetric_pressure_band():
    ...
    assert _trace((0.5,), (cell.first_pressure,)) == ((0.5,),) * 4
    assert _trace((0.5,), (cell.last_pressure,)) == ((0.5,),) * 4
...
def test_subnormal_scaling_preimage_uses_increment_parity(
    ...
    assert (cell.pressure_lower, cell.pressure_upper) == (
        lower * F(TINY),
        upper * F(TINY),
    )
    ...
    for candidate in (
        math.nextafter(cell.first_pressure, -math.inf),
...
@pytest.mark.parametrize("value", (0.0, TINY, 2 * TINY, 2**-1022, 0.25, 0.5, 0.75, 1.0))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ((0.375,), (-0.03125,)),
        ((0.99,), (0.001,)),
        ((math.nextafter(0.5, math.inf),), (2.0**-50,)),
        ((0.5, 0.5, 0.75), (2.0**-50, -(2.0**-50), 0.01)),
    ),
)
def test_cartesian_extrema_preserve_every_step_and_adjacent_outsiders_do_not(
    epi, pressure
):
    box = _box(epi, pressure)
    expected = tuple(step.after for step in box.flow.substeps)
    for values in (
        tuple(cell.first_pressure for cell in box.coordinates),
        tuple(cell.last_pressure for cell in box.coordinates),
        pressure,
    ):
...
def test_zero_signs_are_numeric_trace_equivalent():
    ...
    assert _trace((0.5,), (0.0,)) == _trace((0.5,), (-0.0,))
...
def test_scope_is_a_trace_box_not_a_graph_or_future_certificate():
    box = _box()
    assert box.flow.epi == (0.5,) and len(box.flow.substeps) == 4
...
@pytest.mark.parametrize(
    "fractional,sign", ((F(1, 8), 0), (F(3, 8), -1), (F(5, 8), 1), (F(7, 8), 0))
)
def test_opposite_pressures_at_half_follow_the_two_lattice_bias_law(
    integer, fractional, sign
):
    spacing = F(1, 2**53)
    ...
    result = owner.observe_binary64_unit_quarter_flow(
        epi=(0.5, 0.5), pressure=(pressure, -pressure)
    )
    assert result.mean_pressure == 0
    for ordinal, step in enumerate(result.substeps, 1):
        expected = (
            F(1, 2) + ordinal * spacing * n,
            F(1, 2) - ordinal * spacing * m / 2,
        )
