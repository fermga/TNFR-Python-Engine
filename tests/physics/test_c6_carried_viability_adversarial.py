...
from tnfr.physics.c6_carried_viability import C6CarriedViabilityBox, derive_c6_carried_viability
...
def _state(epi, first=DELTA / 4, fifth=DELTA / 4):
    return NodalRemainderState(epi, (F(0), first, F(0), F(0), F(0), fifth), .375, .625)


def _step(reference, state):
    return advance_nodal_remainder(
        state, timestep=1., capacity=(1.,) * 6,
...
def _box(epi):
    return C6CarriedViabilityBox(
        epi, tuple(F(x) - (DELTA / 4 if i in (1, 5) else 0) for i, x in enumerate(epi)),
...
def _enumerate_boxes(result):
    ...
    for box in result.retained_boxes:
        ...
        for lower, upper, origin, spacing in zip(
                box.lower, box.upper, result.affine_origin, result.coordinate_spacings, strict=True):
            first = ((lower - origin) / spacing).__ceil__()
            last = ((upper - origin) / spacing).__floor__()
            coordinates.append(tuple(origin + spacing * index for index in range(first, last + 1)))
        for exact in product(*coordinates):
            assert exact not in points, "a returned partition double-counts one exact state"
            points.add(exact)
            state = NodalRemainderState(box.epi, tuple(x - F(y) for x, y in zip(exact, box.epi)), .375, .625)
            assert state.exact_epi == exact
    return points


def _eight_states():
    return tuple(_state(row, a, b) for row in (FIRST, SECOND)
                 for a, b in product((-DELTA / 4, DELTA / 4), repeat=2))


def test_nonzero_carry_fixed_point_matches_independent_eight_state_permutation(reference):
    ...
    assert all(_step(reference, _step(reference, state).after).after == state for state in states)
    result = derive_c6_carried_viability(
        reference, state=_state(FIRST), epi_states=(FIRST, SECOND, THIRD), timestep=1.,
...
def test_visible_two_step_return_with_wrong_carry_is_excluded_at_exact_depth_two(reference):
    ...
    result = derive_c6_carried_viability(
        reference, state=initial, epi_states=(initial.epi, middle.epi), timestep=1.,
...
def test_outgoing_odd_target_tie_cannot_be_treated_as_a_closed_cell(reference):
    initial = NodalRemainderState(FIRST, (DELTA / 2,) + (F(0),) * 5, .375, .625)
    ...
    result = derive_c6_carried_viability(
        reference, state=initial, epi_states=(FIRST, SECOND), timestep=1.,
...
def test_exact_band_failure_is_rejected_even_when_rounding_would_hide_it(reference):
    initial = NodalRemainderState(
        FIRST, (F(0), -DELTA / 4, F(0), F(0), F(0), F(0)), LOW, HIGH,
    )
...
def test_incomplete_second_descent_retains_last_complete_partition_without_fixed_point(reference):
    inputs = dict(
        state=_state(FIRST), epi_states=(FIRST, SECOND, THIRD), timestep=1.,
    ...
    interrupted = derive_c6_carried_viability(reference, **inputs, max_work_items=complete.work_items - 1)
    assert interrupted.status == "resource_limit"
    assert tuple(record.point_count for record in interrupted.iterations) == (9, 8)
    assert _enumerate_boxes(interrupted) == {state.exact_epi for state in _eight_states()}
    assert not interrupted.conditional_invariance_certified
    assert not interrupted.origin_exit_certified and interrupted.exclusion_step_bound is None


def test_forged_reference_caches_cannot_change_the_independent_finite_map(reference):
    forged = replace(
        reference, sources=(99.,) * 6, rows=(), epi_quantum=F(7), gradient_quantum=F(11),
        every_pressure_has_positive=True, every_pressure_has_negative=True, no_cartesian_trap=True,
    )
    result = derive_c6_carried_viability(
        forged, state=_state(FIRST), epi_states=(FIRST, SECOND, THIRD), timestep=1.,
        initial_boxes=(_box(FIRST), _box(SECOND)),
    )
