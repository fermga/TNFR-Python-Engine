...
def _case(study, mode, rotation=0):
    return next(case for case in study["cases"]
...
def _rename_state(source, mapping):
    ...
    for field in (
        "glyph_history", "physical_epi_history", "node_attributes",
        "ordered_neighbors",
    ):
        if field in source:
            result[field] = {rename(node): values for node, values in source[field].items()}
    ...
    result["edges"] = tuple((rename(u), rename(v), attributes)
                            for u, v, attributes in source["edges"])
...
@pytest.mark.parametrize("options", (
    {"preparation": "all"}, {"preparation": True}, {"preparation": None},
    {"rotation": True}, {"rotation": -1}, {"rotation": 8},
    {"rotation": 1.0}, {"rotation": "3"},
))
...
def test_declared_study_has_exactly_the_six_preparation_rotation_controls(study):
    assert tuple((case["preparation"], case["rotation"]) for case in study["cases"]) == (
        ("none", 0), ("none", 3), ("single_parent", 0),
        ("single_parent", 3), ("all_nodes", 0), ("all_nodes", 3),
    )
...
def test_each_prefix_is_actual_public_execution_before_the_same_physical_partition(
    ...
    assert all(not history for history in prep["initial"]["physical_epi_history"].values())
    ...
    assert branch["policy"]["external_target"] == (rotation if len(targets) == 1 else None)
...
@pytest.mark.parametrize("mode", ("none", "single_parent", "all_nodes"))
def test_rotation_transports_all_node_attributes_neighbors_histories_and_marks(study, mode):
    original = _case(study, mode)["explicit"]
    moved = _case(study, mode, 3)["explicit"]
    mapping = {node: (node + 3) % 8 for node in range(8)}
    assert _rename_state(original["source_projection"], mapping) == moved["source_projection"]
    expected = deepcopy(original["eligibility"])
    expected["eligible_nodes"] = tuple((node + 3) % 8 for node in expected["eligible_nodes"])
    expected["candidates"] = tuple({**row, "node": (row["node"] + 3) % 8}
    ...
    for checkpoint in ("initial", "after_prefix", "before_birth"):
        ...
        for history in ("glyph_history", "physical_epi_history"):
            assert {(node + 3) % 8: value for node, value in left[history].items()} == right[history]


@pytest.mark.parametrize("mode", ("none", "single_parent", "all_nodes"))
def test_each_finite_continuation_transports_birth_lineage_or_runtime_state_exactly(study, mode):
    ...
    assert _rename_state(original["explicit"]["after"], birth_mapping) == moved["explicit"]["after"]
    for left, right in zip(original["runtime"], moved["runtime"], strict=True):
        assert left["selector"] == right["selector"]
        assert {mapping[node]: glyph for node, glyph in left["actual_selector_proposals"].items()} == right["actual_selector_proposals"]
        for checkpoint in ("dispatch_boundary", "endpoint"):
            assert _rename_state(left[checkpoint], mapping) == right[checkpoint]
    control = next(row for row in study["rotation_controls"] if row["preparation"] == mode)
    assert control["source_projection_equal"] and control["eligibility_equal"]
    assert control["explicit_dispatch_state_equal"]
    assert all(row["proposals_equal"] and row["dispatch_state_equal"]
               and row["endpoint_state_equal"] for row in control["runtime"])
...
def test_full_eligibility_separates_observed_acceleration_from_actual_grammar_marks(
    ...
    assert tuple(row["node"] for row in observation["candidates"]) == branch["before"]["nodes"]
    for row in observation["candidates"]:
        assert row["grammar_allowed"] == (row["node"] in expected)
        assert row["eligible"] == row["grammar_allowed"]
        assert row["proposal_valid"] and row["birth_proposed"] and row["threshold_crossed"]
        assert row["application_preconditions_passed"] and not row["optional_gate_enabled"]
        assert row["depth_limit_reached"] is False
        acceleration = row["acceleration"]
        assert acceleration["available"] and acceleration["source"] == "epi_time_history"
        assert acceleration["time_basis"] == "physical_time"
        assert acceleration["current_endpoint_matches_state"] is True
        samples = tuple(tuple(Fraction.from_float(value) for value in pair)
...
@pytest.mark.parametrize("mode", ("none", "single_parent", "all_nodes"))
@pytest.mark.parametrize("rotation", (0, 3))
def test_explicit_policy_dispatches_every_eligible_parent_without_a_tie_break(study, mode, rotation):
    branch = _case(study, mode, rotation)["explicit"]
    ...
    assert set(branch["after"]["nodes"]) == set(branch["before"]["nodes"]) | set(children)
...
@pytest.mark.parametrize("mode", ("none", "single_parent", "all_nodes"))
@pytest.mark.parametrize("rotation", (0, 3))
def test_both_unchanged_selectors_replay_source_but_do_not_dispatch_thol(study, mode, rotation):
    case = _case(study, mode, rotation)
    explicit = case["explicit"]
    assert tuple(row["selector"] for row in case["runtime"]) == ("default", "parametric")
...
def test_real_prefix_phase_effect_is_recorded_instead_of_assuming_unchanged_state(study):
    preparations = {
    ...
    for mode in ("single_parent", "all_nodes"):
        ...
        assert actual["before_birth"]["physical_epi_history"] == none["before_birth"]["physical_epi_history"]
...
def test_study_serializes_finite_evidence_without_claiming_spontaneous_selection(study):
    ...
    assert "Initial checkerboard EPI, winding phase, support and capacity are supplied" in limitations
    assert "No UM, post-birth physical flow, restoration or sustained pattern" in limitations
