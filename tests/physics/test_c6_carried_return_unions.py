...
from tnfr.physics.c6_carried_viability import C6CarriedForwardZone, _dbm_join, _Work
...
def state(point=ORIGIN):
    return NodalRemainderState(row(point), (F(0),) * 6, 0.375, 0.625)
...
def derive(reference, targets=(CHAIN[-1],), origin=ORIGIN, **changes):
    args = dict(
        state=state(origin),
        epi_states=tuple(map(row, POINTS)),
        timestep=2.0,
...
def test_chain_oracle_matches_shared_nodal_integrator(reference):
    ...
    for before, after in zip(CHAIN, CHAIN[1:]):
        ...
        step = advance_nodal_remainder(
            state(before), timestep=2.0, capacity=(1.0,) * 6, pressure=pressure
        )
...
def test_empty_and_stationary_complete_union_certificates(result):
    for query in result.queries[:2]:
        ...
        assert (
            query.initialization_complete and query.origin_path_within_domain_excluded
        )
    stationary = result.queries[2]
    assert (
        stationary.status == "stationary_complete_layer"
        and stationary.completed_depth == 1
    )
    assert (
        stationary.initial_base_zones == stationary.retained_zones == (target(FIXED),)
    )
    ...
    assert (
        ...
        and not result.conditional_invariance_certified
    )
    assert (
        not result.asymptotic_convergence_certified
...
def test_origin_collision_after_two_guarded_returns_is_not_promoted(reference):
    ...
    assert tuple(layer.origin_present for layer in query.iterations) == (
        False,
        False,
        True,
    )
...
def test_transient_first_image_without_any_later_return_is_not_lost(reference):
    ...
    query = queried.queries[0]
    assert not any(
        edge.source_epi == row(CHAIN[0])
        for edge in queried.return_envelope.return_relation
    )
...
def test_two_step_return_keeps_the_actual_intermediate_gate(reference):
    envelope = derive(reference, origin=CHAIN[0]).return_envelope
    edges = [
        edge for edge in envelope.return_relation if edge.source_epi == row(CHAIN[0])
    ]
    assert len(edges) == 1
    edge = edges[0]
...
def test_mixed_targets_keep_transient_pullbacks_and_original_quantification(
    reference, result
):
    supplied = (target(CHAIN[-1]), target(CHAIN[1]))
    queried = derive(
        reference,
        target_region_groups=(supplied,),
...
def test_partial_relation_and_initialization_do_not_publish_absence(reference, result):
    ...
    assert (
        not partial.initialization_complete
        and not partial.origin_path_within_domain_excluded
    )
    partial = derive(
        reference,
        targets=(CHAIN[1],),
    ...
    assert (
        partial.initial_base_zones
        == partial.retained_zones
        == partial.transient_target_preimages
        == ()
    )
...
def test_initial_subsumption_stop_discards_only_the_unfinished_query(reference, result):
    ...
    queried = derive(
        reference,
        target_region_groups=(supplied, supplied),
        query_max_subsumptions=1,
        max_intersections=result.return_envelope.construction_intersections,
    )
    ...
    assert (
        not second.initialization_complete
        and not second.origin_path_within_domain_excluded
    )
    assert (
        second.initial_base_zones
        == second.retained_zones
        == second.transient_target_preimages
        == ()
    )
    assert second.iterations == () and second.subsumptions == 0


def test_shared_intersection_budget_advances_independent_queries_round_robin(
    reference, result
):
    queried = derive(
        reference,
...
def test_zone_cap_inside_a_backward_layer_discards_already_inserted_pieces(reference):
    alternative = tuple(
        a + b for a, b in zip(CHAIN[2], (1, 1, 0, -1, -1, 0), strict=True)
    )
...
def test_exact_union_preserves_a_hole_that_a_hull_turns_into_a_spurious_past():
    ...
    assert set().union(*(members(z.bounds) for z in union.public())) == {
        (-2, 0),
        (-1, 0),
        (1, 0),
        (2, 0),
...
def test_subsumption_is_exact_under_an_independent_finite_set_oracle():
    key, union, expected = (
        row(ORIGIN),
        owner._ReturnUnion((row(ORIGIN),), 25, _Work(10000)),
        set(),
    )
    boxes = (
        box((-2, -2), (-1, 1)),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"query_max_zones": 0},
        {"query_max_zones": 1.5},
        {"query_max_subsumptions": 0},
        {"query_max_subsumptions": True},
...
def test_unclosed_and_noninteger_targets_are_rejected(reference):
    ...
    with pytest.raises(ValueError, match="closed difference bounds"):
        derive(
            reference,
            target_region_groups=(
                (replace(item, bounds=tuple(map(tuple, unclosed))),),
            ),
        )
...
def test_forged_reference_caches_cannot_change_the_union_map(reference, result):
    forged = replace(
        reference, sources=(99.0,) * 6, rows=(), epi_quantum=F(3), gradient_quantum=F(5)
    )
