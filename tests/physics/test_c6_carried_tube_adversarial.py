...
def _profile(weight=.5):
    return derive_c6_carried_profile(derive_c6_pressure_lattice(
        phase=(0.,) * 6, epi_weight=weight, phase_weight=.5,
...
def _state(**changes):
    ...
    args.update(changes)
    return initialize_nodal_remainder((.5,) * 6, **args)


@pytest.mark.parametrize("weight,mode,eigenvalue", [
    (math.nextafter(.8, -math.inf), (2, 1, -1, -2, -1, 1), F(1, 2)),
    (.8, (1, -1, 1, -1, 1, -1), F(2)),
])
def test_represented_values_on_opposite_sides_of_the_sharp_mode_switch(weight, mode, eigenvalue):
    ref = derive_c6_carried_contraction(_profile(weight), timestep=1.)
    source = tuple(map(F, mode))
    image = tuple(value - F(weight) * (value - (source[i-1] + source[(i+1) % 6])/2)
                  for i, value in enumerate(source))
    factor = 1 - F(weight) * eigenvalue
    assert image == tuple(factor * value for value in source)
    assert ref.norm_factor == abs(factor)
    assert sum(value**2 for value in image) == ref.energy_factor * sum(value**2 for value in source)
    assert ref.energy_factor == ref.norm_factor**2 < ref.norm_factor
    assert all(sum(row) == 1 for row in ref.transition)  # The uniform mode never contracts.
    assert F(math.nextafter(.8, -math.inf)) < F(4, 5) < F(.8)
...
def test_carry_feedback_can_sustain_nonzero_centered_error_under_zero_visible_pressure():
    ...
    state = NodalRemainderState((.5625,) * 6, residuals, .375, .625)
    tube = derive_c6_carried_tube(profile, state=state, timestep=.25)
    step = advance_nodal_remainder(state, timestep=.25, capacity=(1.,) * 6, pressure=(0.,) * 6)
...
@pytest.mark.parametrize("weight", [.3, math.nextafter(1., 0.), math.ulp(0.)])
def test_uniform_rounding_envelopes_cover_actual_extreme_slab_reducers(weight):
    profile = _profile(weight)
    state = initialize_nodal_remainder((.375, .625, .375, .625, .375, .625), epi_lower=.375, epi_upper=.625)
    tube = derive_c6_carried_tube(profile, state=state, timestep=.5)
...
def test_narrower_state_band_controls_admission_even_with_the_same_wider_pressure_slab():
    profile = _profile()
    wide = derive_c6_carried_band_horizon(derive_c6_carried_tube(profile, state=_state(), timestep=.25))
    narrow_state = _state(epi_upper=.5)
    narrow = derive_c6_carried_band_horizon(derive_c6_carried_tube(profile, state=narrow_state, timestep=.25))
    ...
    step = advance_nodal_remainder(narrow_state, timestep=.25, capacity=(1.,) * 6, pressure=(0.,) * 6)
...
def test_band_horizon_is_the_exact_maximal_integer_for_its_sufficient_inequality():
    tube = derive_c6_carried_tube(_profile(), state=_state(), timestep=.25)
    ...
    assert horizon.next_step_margin == next_reserve and horizon.next_step_passes is False
...
def test_forged_zero_mean_rate_or_energy_does_not_create_an_infinite_certificate():
    ...
    forged = replace(tube, mean_increment_bound=F(0), energy_bound=F(0), initial_mean=F(1, 2))
...
def test_forged_spectral_and_profile_caches_are_not_authority_for_the_horizon():
    tube = derive_c6_carried_tube(_profile(), state=_state(), timestep=.25)
    bad_balance = replace(tube.contraction.profile.forced_balance, relative_profile=(F(100),) * 6)
    bad_profile = replace(tube.contraction.profile, forced_balance=bad_balance)
    bad_contraction = replace(tube.contraction, profile=bad_profile, norm_factor=F(0), energy_factor=F(0))
    forged = replace(tube, contraction=bad_contraction)
    assert derive_c6_carried_band_horizon(forged) == derive_c6_carried_band_horizon(tube)
