...
def _mv(matrix, vector):
    return tuple(sum((a*b for a, b in zip(row, vector, strict=True)), F(0)) for row in matrix)
...
def _fixture(n=4, defects=False):
    ...
    edges = tuple(sorted((i, j, F(1)) for pair in undirected for i, j in (pair, pair[::-1])))
    support = tuple(tuple(j for i, j, _ in edges if i == k) for k in range(n))
    nu, zero = (F(1),)*n, (F(0),)*n
    a = tuple(tuple(F(i == j)-(F(1, len(support[i])) if j in support[i] else F(0))
    ...
    def receipt(initial, perturbed):
        ...
        kernel = tuple(F(1, 1024) if defects and perturbed and i == half else F(0) for i in range(n))
        stored = tuple(F(-1, 2048) if defects and perturbed and i == half+1 else F(0) for i in range(n))
        ...
        observation = NonEpiForcingObservation(
            snapshot=snap, phase=zero, epi_weight=F(1), forcing=zero, phase_gradient=zero,
            normalized_weights=(("phase", F(0)), ("epi", F(1)), ("vf", F(0)), ("topo", F(0))),
            full_kernel_pressure=fresh, kernel_pressure_defect=kernel, stored_pressure_residual=stored)
        current, matrix, offset, propagated, rows = initial, _identity(n), zero, [], []
        for i in range(n):
            row = tuple(F(3, 4) if j == i else (F(1, 4*len(support[i])) if j in support[i] else F(0))
                        for j in range(n))
            local = list(_identity(n))
            local[i] = row
            local = tuple(local)
            ideal = _mv(local, current)
            defect = tuple(F(1, 4096) if defects and perturbed and i == half and j == i else F(0) for j in range(n))
            after = _add(ideal, defect)
            rows.append({"node": i, "control": {"EN_mix": F(1, 4), "neighbor_indices": support[i],
                         "neighbor_mean": sum((current[j] for j in support[i]), F(0))/len(support[i])},
                         "configuration": {"GLYPH_FACTORS": {"EN_mix": 0.25}}, "row": row, "offset": F(0),
                         "before_epi": current, "after_epi": after,
                         "kernel_evaluation_defect": defect[i], "clipping_defect": F(0), "local_defect": defect})
        ...
        integration = tuple(F(-1, 8192) if defects and perturbed and i == half+1 else F(0) for i in range(n))
        post = tuple(F(1, 16384) if defects and perturbed and i == half else F(0) for i in range(n))
        xi = _add(_add(current, tuple(F(1, 4)*p for p in pressure)), integration)
        xf = _add(xi, post)
        ideal_endpoint = _sub(_mv(matrix, initial), tuple(F(1, 4)*v for v in _mv(a, initial)))
        report = {"glyph": "EN", "nodes": tuple(range(n)), "start_time": F(9, 4), "end_time": F(5, 2),
                  "vectors": {"x0": initial, "xg": current, "xi": xi, "xf": xf},
                  "reset": {"S": matrix, "c": offset, "local_rows": rows, "propagated_local_defects": propagated,
                            "total_reset_defect": total, "identity_residual": zero},
                  "generation": {"A": a, "b": zero, "observation": asdict(observation)},
                  "runtime": {"ideal_endpoint": ideal_endpoint,
                              "pressure_term": tuple(F(1, 4)*v for v in _add(kernel, stored)),
                              "integration_remainder": integration, "postintegration_change": post,
                              "identity_residual": zero, "pre_generated_pressure_retained": True}}
...
def _energy(values, metric):
    ...
    return mean, sum((h*x*x/2 for h, x in zip(metric, centered, strict=True)), F(0))
...
def test_exact_per_write_and_integration_telescope(defects):
    ...
    for row in result["local_EN_writes"]:
        ...
        assert row["paired_increment"] == _sub(row["perturbed_increment"], row["control_increment"])
        assert all(value == 0 for i, value in enumerate(row["paired_increment"]) if i != row["node"])
        ...
        assert change == row["budget"]["variance_defect_linear_term"]+row["budget"]["variance_defect_quadratic_term"]
        total += change
    assert total == result["reset_budget"]["variance_change"]
    total += result["integration_budget"]["variance_change"]+result["postintegration_budget"]["variance_change"]
    assert total == result["states"]["xf"]["centered_H_energy"]-result["states"]["x0"]["centered_H_energy"]
    assert result["telescope"]["variance_residual"] == result["telescope"]["mass_residual"] == 0
    for key in ("reset_budget", "integration_budget", "postintegration_budget"):
        assert result[key]["mass_identity_residual"] == result[key]["variance_identity_residual"] == 0
...
def test_held_generation_pressure_is_not_refreshed_at_reset_endpoint():
    ...
    expected = tuple(sum((part["pressure"][i] for part in result["pressure_work"].values()), F(0)) for i in range(4))
    assert expected == g["balance"]["stored_pressure_defect"]
    assert sum((part["variance_work"] for part in result["pressure_work"].values()), F(0)) == F(1, 4)*g["balance"]["variance_defect_rate"]
    # Actual xi uses pressure generated at delta0; a refreshed endpoint differs.
    refreshed = _sub(result["delta"]["xg"], tuple(F(1, 4)*x for x in _mv(a, result["delta"]["xg"])))
...
def test_complete_four_mode_gram_reconstruction_and_mean_transfer():
    ...
    for row in modes["gram_terms"]:
        inner = sum((h*a*b for h, a, b in zip(metric, vectors[row["left"]], vectors[row["right"]], strict=True)), F(0))
        assert inner == row["inner_product"]
        assert row["energy_contribution"] == (inner/2 if row["left"] == row["right"] else inner)
    assert sum((row["energy_contribution"] for row in modes["gram_terms"]), F(0)) == result["states"]["xf"]["centered_H_energy"]
...
@pytest.mark.parametrize("mutation", ("row_order", "row_value", "staged_epi", "local_defect", "propagated",
                                      "matrix", "offset", "generation_epi", "kernel_defect", "pressure_retained",
                                      "integration_remainder", "time", "configuration", "neighbor_order"))
...
def _saved_reports():
    ...
    binding = {key: {"sha256": "a"*64, "historical_manifest": {"claim_id": key}}
               for key in ("window", "native")}
    first = {"historical_inputs": deepcopy(binding), "original_reference": audit._payload(asdict(original)),
             "branches": [{"branch": name, "resets": [row]} for name, row in zip(audit.BRANCHES, (left, right))]}
    metric = tuple(original.metric_weights[i] for i in children)
    endpoints = []
    for time, key in ((F(9, 4), "x0"), (F(5, 2), "xf")):
        x, y = (tuple(F(row["vectors"][key][i]) for i in children) for row in (left, right))
        mean, energy = _energy(_sub(y, x), metric)
        delta = _sub(y, x)
        paired = {"epi_difference": delta, "weighted_mean_offset": mean,
                  "centered_epi_difference": tuple(v-mean for v in delta), "centered_H_energy": energy,
                  "full_H_energy": sum((h*v*v/2 for h, v in zip(metric, delta, strict=True)), F(0)),
                  "full_epi_equal": not any(delta), "shape_equal_modulo_uniform_offset": not any(v-mean for v in delta)}
        endpoints.append({"time": time, "control_epi": x, "perturbed_epi": y, "paired": paired})
    second = {"historical_inputs": binding, "nodes": original.source.nodes, "full_metric_weights": original.metric_weights,
              "actual_lineage": {"pairs": tuple((i, i+8) for i in range(8)), "children": children},
              "regions": [{"label": "actual_children", "nodes": children, "full_node_indices": children,
                           "metric_weights": metric, "endpoints": endpoints}]}
    return first, audit._payload(second)


def test_saved_reports_bind_lineage_metric_and_both_endpoints_without_kernels(monkeypatch):
    ...
    assert result["native_calls"] == result["kernel_calls"] == result["forcing_capture_calls"] == 0
    assert len(result["audit"]["local_EN_writes"]) == 16
    assert len(result["audit"]["children"]) == 8


@pytest.mark.parametrize("mutation", ("hash", "manifest", "lineage", "metric", "endpoint", "paired", "branches"))
def test_cross_report_bindings_reject_substitution(mutation):
    first, second = _saved_reports()
    if mutation == "hash":
        second["historical_inputs"]["native"]["sha256"] = "b"*64
    elif mutation == "manifest":
        second["historical_inputs"]["window"]["historical_manifest"]["claim_id"] = "other"
...
def _manifest(claim):
    manifest = audit.CoreExperimentManifest(
        ...
        versions={"python": "synthetic"}, graph_construction="Synthetic detached fixture",
        capacity_specification="Declared positive", solver="Fixture arithmetic", timestep=None, seed=None,
        result_status=audit.ClaimStatus.DERIVED, operator_sequence=(), telemetry=("fixture",),
        controls=("not native execution",), artifacts=("fixture.json",))
...
def test_loader_authenticates_bytes_and_cli_protects_both_inputs(tmp_path, monkeypatch):
    ...
    result = audit.run_study(*paths, expected_reset_sha256=hashes[0], expected_recovery_sha256=hashes[1])
    assert result["historical_inputs"]["recovery"]["sha256"] == hashes[1]
    with pytest.raises(ValueError, match="digest"):
        audit.run_study(*paths, expected_reset_sha256="0"*64, expected_recovery_sha256=hashes[1])
    for path in paths:
        monkeypatch.setattr(sys, "argv", [
            "audit", "--reset-input", str(paths[0]), "--recovery-input", str(paths[1]), "--output", str(path)])
