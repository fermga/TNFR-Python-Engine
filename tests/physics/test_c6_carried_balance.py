...
from tnfr.physics.c6_carried_balance import (
    derive_c6_carried_pressure_balance, observe_c6_carried_pressure_point,
)
...
def _state(offsets, carry=CARRY):
    return NodalRemainderState(_epi(offsets), (carry,) * 6, .375, .625)
...
def closure():
    ...
    return derive_c6_carried_closure(profile, state=state, timestep=.0625)


def test_positive_exact_pressure_balance_refutes_only_the_class_wide_separator(closure):
    original = closure.base_tube.state
    result = derive_c6_carried_pressure_balance(closure, states=tuple(map(_state, OFFSETS)))
    ...
    for i in range(6):
        assert sum(weight * F(point.observation.pressure[i])
                   for weight, point in zip(result.weights, result.points)) == 0
    ...
    assert not result.bounded_trajectory_certified and not result.future_runtime_certified
    assert all(not point.reachable_from_initial_state_certified for point in result.points)
    with pytest.raises(FrozenInstanceError):
        result.weights = (F(0),) * 7


@pytest.mark.parametrize("offsets,scaled_sum", (
    ((-4, 0, 2, -1, 8, -7), -48),
    ((-4, -1, 2, -1, 8, -6), 64),
    ((-3, -1, 2, -1, 6, -5), 0),
))
def test_all_three_mean_signs_occur_at_admitted_near_profile_points(closure, offsets, scaled_sum):
...
@pytest.mark.parametrize("rows", ([], (), ((.5,) * 6,) * 6, ((.5,) * 6,) * 8))
...
@pytest.mark.parametrize("epi", (
    [.5] * 6, (.5,) * 5, (.5,) * 5 + (1,),
    (.5,) * 5 + (float('nan'),), (.5,) * 5 + (float('inf'),),
))
def test_malformed_static_coordinates_do_not_enter_the_certificate(closure, epi):
    with pytest.raises((TypeError, ValueError)):
        observe_c6_carried_pressure_point(closure, state=NodalRemainderState(epi, (F(0),) * 6, .375, .625))
...
def test_energy_cache_is_rebuilt_before_admitting_a_point(closure):
    ...
    with pytest.raises(ValueError, match="outside the rebuilt centered"):
        observe_c6_carried_pressure_point(forged, state=initialize_nodal_remainder(
            (.375, .625) * 3, epi_lower=.375, epi_upper=.625,
        ))
...
def test_declared_band_is_enforced_for_hypothetical_points(closure):
    with pytest.raises(ValueError, match="positive band"):
        observe_c6_carried_pressure_point(
            closure, state=NodalRemainderState((.25,) * 6, (F(0),) * 6, .375, .625),
        )
...
def test_different_declared_band_cannot_broaden_the_point_class(closure):
    state = replace(_state(OFFSETS[0]), epi_lower=.25, epi_upper=.75)
