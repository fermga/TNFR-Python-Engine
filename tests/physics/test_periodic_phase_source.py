...
def _budget(rows=None, duration=0.25):
    return derive_periodic_phase_source_budget(
        phase_contributions=(
            rows if rows is not None else ((0.125,) * 6, (-0.25,) * 6, (0.375,) * 6)
        ),
...
def _observe(reference=None, **changes):
    arguments = {
        "block_count": 1,
        "initial_mean": F(1, 2),
        "actual_nonphase_area": F(0),
    }
    arguments.update(changes)
    return observe_periodic_phase_source_compensation(
        reference or _budget(), **arguments
    )
...
def test_row_means_use_all_six_exact_represented_coordinates():
    source = ((0.5, -0.25, 0.125, 0.0, 0.0, 0.0), (0.1, 0.2, 0.3, -0.1, -0.2, -0.3))
    result = _budget(source, 0.1)
    assert result.mean_sources == (F(1, 16), F(0))
    assert result.mean_source == F(1, 32)
    assert result.phase_area_per_period == F(0.1) / 16
...
def test_arbitrary_prefix_equals_independent_finite_sum(count):
    ...
    expected = sum((F((0.125, -0.25, 0.375)[i % 3]) / 4 for i in range(count)), F(0))
...
def test_zero_average_has_bounded_phase_prefix_but_does_not_certify_nonphase_budget():
    budget = _budget(((0.125,) * 6, (-0.125,) * 6))
...
def test_required_compensation_interval_has_correct_sign_and_inclusive_boundaries():
    ...
    assert initial.required_compensation_lower == F(0.05) - F(1, 2) - initial.phase_area
    assert initial.required_compensation_upper == F(1, 2) - initial.phase_area
    for compensation, endpoint in (
        (initial.required_compensation_lower, F(0.05)),
    ...
    for compensation in (
        initial.required_compensation_lower - F(1, 2**100),
        initial.required_compensation_upper + F(1, 2**100),
    ):
        assert not _observe(
            block_count=1001, actual_nonphase_area=compensation
        ).mean_in_band
...
def test_forged_derived_caches_are_ignored_and_source_fields_rebuilt():
    ...
    forged = replace(
        ...
        mean_source=F(0),
        centered_prefixes=(),
        phase_area_per_period=F(0),
        prefix_amplitude_bound=F(0),
    )
    ...
    with pytest.raises(ValueError):
        _observe(replace(reference, block_duration=0.0))
...
@pytest.mark.parametrize(
    "changes,error",
    (
        ({"block_count": -1}, ValueError),
        ({"block_count": True}, ValueError),
        ({"block_count": 1.0}, ValueError),
        ({"initial_mean": 0.5}, TypeError),
        ({"initial_mean": F(0)}, ValueError),
        ({"initial_mean": F(2)}, ValueError),
        ({"actual_nonphase_area": 0.0}, TypeError),
        ({"epi_lower": 0.0}, ValueError),
        ({"epi_lower": 1}, TypeError),
        ({"epi_upper": 1.1}, ValueError),
        ({"epi_lower": 0.6, "epi_upper": 0.5}, ValueError),
    ),
)
