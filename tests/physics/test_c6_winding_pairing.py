...
def _step(reference=None, *, phase=PHASE, closed=CLOSED, epi=EPI):
    return observe_c6_winding_pairing(
        ...
        phase_before_pi=phase, closed_mean_errors_pi=closed, epi=epi,
    )
...
def test_projection_receiver_coherence_and_laplacian_identities_independently():
    ...
    adjacency = tuple(tuple(F((i - j) % 6 in (1, 5)) for j in range(6)) for i in range(6))
    identity = tuple(tuple(F(i == j) for j in range(6)) for i in range(6))
    receiver = tuple(tuple((identity[i][j] + adjacency[i][j]) / 3 for j in range(6)) for i in range(6))
    laplacian = tuple(tuple(identity[i][j] - adjacency[i][j] / 2 for j in range(6)) for i in range(6))
    alpha = source.coherence_phase_factor
    coherence = tuple(tuple((1 - alpha) * identity[i][j] + alpha * adjacency[i][j] / 2
                            for j in range(6)) for i in range(6))
    projection = tuple(tuple(F(j in (i, i + 3)) - F(1, 3) for j in range(6)) for i in range(3))
    ...
    assert _multiply(projection, laplacian) == tuple(tuple(3 * x / 2 for x in row) for row in projection)
    assert result.receiver_projection_residual == result.coherence_projection_residual == (ZERO,) * 3
...
def test_full_finite_map_pressure_and_two_coordinate_quotient_are_exact():
    ...
    coupled = tuple((1 - t) * PHASE[i] + t * (CLOSED[i - 1] + CLOSED[i] + CLOSED[(i + 1) % 6]) / 3
                    for i in range(6))
    after = tuple((1 - alpha) * coupled[i] + alpha * (coupled[i - 1] + coupled[(i + 1) % 6]) / 2
                  for i in range(6))
    phase_pressure = tuple((after[i - 1] + after[(i + 1) % 6]) / 2 - after[i] for i in range(6))
    pressure = tuple(source.epi_weight * ((EPI[i - 1] + EPI[(i + 1) % 6]) / 2 - EPI[i])
                     + source.phase_weight * phase_pressure[i] for i in range(6))
    endpoint = tuple(EPI[i] + source.timestep * source.capacity * pressure[i] for i in range(6))
    ...
    for i in range(3):
        ...
        predicted = tuple(sum(a * b for a, b in zip(row, original, strict=True))
                          for row in result.reference.quotient_matrix)
        assert predicted == (result.phase_pair_after[i], result.epi_pair_after[i])
    assert result.phase_pair_identity_residual == result.epi_pair_identity_residual == ZERO_PAIRS
    assert result.phase_pressure_pair_identity_residual == result.total_pressure_pair_identity_residual == ZERO_PAIRS
...
def test_centered_pairing_survives_supplied_common_mean_drift():
    ...
    assert result.phase_mean_after_coupling_pi == result.phase_mean_after_coherence_pi == expected_mean
...
def test_common_rotation_changes_only_phase_means_in_the_readout():
    shift = F(123, 7)
    initial = _step()
    shifted = _step(phase=tuple(value + shift for value in PHASE), closed=tuple(value + shift for value in CLOSED))
    assert shifted.phase_mean_after_coherence_pi == initial.phase_mean_after_coherence_pi + shift
    assert shifted.phase_after_coherence_pi == tuple(value + shift for value in initial.phase_after_coherence_pi)
    for field in ("phase_pair_before", "phase_pair_after", "phase_pressure", "modeled_pressure", "epi_after"):
...
def test_interval_only_closed_means_need_not_contract_rho_or_preserve_the_next_reserve():
    source = _reference(coupling_phase_factor=1, coherence_phase_factor=0,
                        phase_weight=1, epi_lower=F(1, 20), epi_upper=1)
    a = F(1, 120)
    phase = (a, a, 0, 0, 0, a)
    ...
    assert result.phase_contraction_slack < 0 and not result.meets_nonlinear_phase_contraction
    ...
    with pytest.raises(ValueError, match="reserve"):
        _step(source, phase=result.phase_after_coherence_pi, closed=result.phase_after_coherence_pi, epi=result.epi_after)
...
@pytest.mark.parametrize("t,alpha,expected", (
    (F(1, 4), 0, F(3, 4)), (F(1, 4), F(2, 3), 0),
    (F(1, 4), 1, F(-3, 8)), (1, F(3, 10), 0),
))
def test_signed_phase_pair_factor_including_annihilation_and_reversal(t, alpha, expected):
    ...
    assert result.phase_pair_after == tuple(expected * value for value in result.phase_pair_before)
...
def test_zero_phase_weight_removes_only_the_off_diagonal_epi_forcing():
    ...
    assert result.epi_pair_after == tuple(result.reference.epi_pair_factor * value for value in result.epi_pair_before)
...
def test_public_outer_and_nested_reference_caches_are_rebuilt():
    ...
    forged_source = replace(source, phase_reference=phase, nodal_euler_matrix=(), epi_phase_budget_weight=0)
    assert derive_c6_winding_pairing(forged_source) == correct.reference
    forged = replace(correct.reference, reference=forged_source, projection_matrix=(), receiver_average_matrix=(),
                     coherence_matrix=(), quotient_matrix=(), phase_pair_factor=99, epi_pair_factor=99,
                     quotient_spectral_radius=99, strict_quotient_decay=False)
...
def test_a_forged_zero_budget_cannot_admit_an_unfunded_initial_field():
    source = _reference()
    forged = replace(derive_c6_winding_pairing(source), reference=replace(source, epi_phase_budget_weight=0))
    with pytest.raises(ValueError, match="reserve"):
...
@pytest.mark.parametrize("values", (
    "000000", {i: 0 for i in range(6)}, set(range(6)), (0,) * 5, (0,) * 7,
    (0, 0, 0, 0, 0, True), (0, 0, 0, 0, 0, float("nan")), (0, 0, 0, 0, 0, complex(1, 1)),
))
