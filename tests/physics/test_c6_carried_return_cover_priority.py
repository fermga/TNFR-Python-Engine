...
def _members(bounds):
    if bounds is None:
        return set()
    return {(x, y) for x, y in product(range(-1, 10), range(-2, 9))
...
def memory():
    ...
    edges = tuple(owner.C6CarriedReturnTransition(
        rows[source], None, rows[1], _hull(points), None,
        _hull((x + shift[0], y + shift[1]) for x, y in points), (*shift, 0, 0, 0, 0),
    ...
    ) for source, before, after in ((0, (0, 0), (0, 7)), (1, (5, 0), (5, 5)), (1, (8, 0), (8, 5))))
    zones = (C6CarriedForwardZone(rows[0], zero), C6CarriedForwardZone(rows[1], wide),
    ...
    envelope = owner.C6CarriedReturnEnvelope(
        ...
        (rows[2],), zones, zones, edges, terminals, len(edges) + len(terminals),
        True, True, (), "fixed_point", 0, 0, 1, 3,
    )
    return owner._derive_return_memory_envelope(envelope, max_memory_work=10_000, max_memory_arcs=1000)


def _target(memory, row=2, point=(8, 5)):
    return (C6CarriedForwardZone(memory.return_envelope.epi_states[row], _hull((point,))),)


def _derive(memory, **changes):
    arguments = dict(exclusion_query_max_intersections=10_000, max_partition_pieces=1000,
                     max_partition_construction_work=100_000, max_partition_arcs=5000,
                     max_partition_work=100_000, max_cover_work=100_000)
...
def _reachable(partition):
    current = {(i, point) for i, root in enumerate(partition.root_zones) for point in _members(root)}
    graph = _exact_graph(partition)
    while True:
        following = current | {(b, after) for a, b, before, after in graph if (a, before) in current}
...
def _reachable_cover(partition):
    ...
    return tuple(_hull(point for index, point in reached if index == i)
                 if any(index == i for index, _ in reached) else None
...
def test_private_finite_fixture_has_a_real_successor_beyond_the_root(memory):
    ...
    assert {point for root in cover.partition.root_zones for point in _members(root)} == {(4, 0)}
    actual = {(a, b, point, (point[0] + shift[0], point[1] + shift[1]))
              for a, b, guard, shift in cover.partition.partition_arcs for point in _members(guard)}
...
def test_default_fifo_preserves_previous_partition_payload_and_has_no_classification_charge(memory, cover):
    previous = owner._derive_return_safe_partition(
        memory, (_target(memory),), exclusion_query_max_intersections=10_000, max_partition_pieces=1000,
        max_partition_construction_work=100_000, max_partition_arcs=5000, max_partition_work=100_000,
    )
    ...
    for name in ("conditional_invariance_certified", "conditional_boundedness_certified",
                 "actual_origin_reachability_certified", "future_runtime_certified", "asymptotic_convergence_certified"):
        assert getattr(cover, name) is False


def test_null_hints_and_missing_memory_hints_charge_every_seed_without_cutting(memory, cover):
    supplied = ((0, None),)
...
def test_advisory_priorities_preserve_all_finite_origin_paths_under_bounded_work(memory, cover, maximum):
    wide = memory.retained_endpoint_zones[2]
    result = _derive(memory, priority_memory_regions=((2, wide),), max_partition_work=maximum)
    ...
    assert all(point in _members(partition.retained_zones[index]) for index, point in _reachable(partition))
    assert result.schedule.classification_work <= maximum
    if result.schedule.classification_complete:
        flags = tuple(bool(_members(seed) & _members(wide)) if index == 2 else False
                      for (index, _), seed in zip(partition.vertices, partition.seed_zones, strict=True))
...
def _algebra_partition(partition):
    ...
    return replace(partition, vertices=((0, 0), (1, 0)), seed_zones=(wide, wide),
                   root_zones=(zero, None), retained_zones=(wide, wide), partition_arcs=arcs)


@pytest.mark.parametrize("flags", ((False, True), (True, False)))
def test_exact_shared_worklist_matches_finite_reachability_for_every_small_budget(cover, flags):
    ...
    for maximum in range(2, 14):
        result = owner._return_partition_descent(
            ...
            maximum, priority_flags=flags, initial_work=2,
        )
        assert all(point in _members(result.retained_zones[index]) for index, point in reachable)
        assert result.work <= maximum
        assert set(result.pending) == set(result.priority_pending) | set(result.ordinary_pending)
...
def test_cover_gate_requires_complete_image_not_merely_roots_and_safe_seeds(cover):
    ...
    assert all(_members(root) <= _members(zone) <= _members(seed)
               for root, zone, seed in zip(partition.root_zones, candidate, partition.seed_zones, strict=True))
    expected = _image_oracle(partition, candidate)
    assert any(_members(image) - _members(zone) for image, zone in zip(expected, candidate, strict=True))
...
def test_gate_accepts_a_strict_image_subset_and_same_index_cover_intersection(cover):
    ...
    checks = [owner._check_return_safe_cover(partition, value, 10_000) for value in (left, right)]
    assert all(check.certified and check.image_zones != check.candidate_zones for check in checks)
    intersection = tuple(_hull(_members(a) & _members(b)) for a, b in zip(left, right, strict=True))
    ...
    assert not owner._check_return_safe_cover(partition, wrong_indices, 10_000).certified


def test_exact_reachable_candidate_is_verified_without_replacing_the_discovery_partition(memory, cover):
    candidate = _reachable_cover(cover.partition)
    result = _derive(memory, candidate_retained_zones=candidate)
    assert result.schedule.policy == "candidate_only" and result.schedule.status == "not_run"
...
def test_failed_cover_inclusion_never_promotes_the_candidate(cover, kind):
    ...
    if kind == "root":
        index = next(i for i, root in enumerate(partition.root_zones) if root is not None)
    ...
    assert forged.retained_zones == () and not forged.domain_confined_origin_histories_covered


def test_cover_budget_abstains_in_the_middle_of_the_image_and_final_inclusion_check(cover):
    partition = cover.partition
    ...
    for maximum in (1, 4 * len(partition.vertices) + 1, complete.work - 1):
        stopped = owner._check_return_safe_cover(partition, partition.seed_zones, maximum)
...
def _malformed_bounds(kind):
    ...
    rows[0][1] = {"bool": False, "float": 0., "infinite": float("inf"), "unclosed": 1}[kind]
    return tuple(map(tuple, rows))


@pytest.mark.parametrize("kind", ("bool", "float", "infinite", "unclosed"))
def test_candidate_and_hint_dbms_require_closed_exact_integer_geometry(memory, cover, kind):
...
@pytest.mark.parametrize("regions", ([], "unbounded", ((True, None),), ((1., None),), ((-1, None),),
                                     ((0,),), ([0, None],), ((0, ((0,),)),)))
def test_malformed_priority_hints_reject_before_source_reconstruction(monkeypatch, regions):
    def forbidden(*_args, **_kwargs):
        raise AssertionError("invalid hints must be rejected before canonical source reconstruction")

    monkeypatch.setattr(owner, "derive_c6_carried_return_memory_envelope", forbidden)
    with pytest.raises((TypeError, ValueError)):
        owner.derive_c6_carried_return_safe_cover(
            None, state=None, epi_states=(), timestep=1., transient_epi_states=(),
            excluded_target_region_groups=(), priority_memory_regions=regions,
        )
...
def test_non_tuple_hint_stream_is_never_materialized():
    ...
    with pytest.raises(ValueError, match="tuple"):
        owner.derive_c6_carried_return_safe_cover(
            None, state=None, epi_states=(), timestep=1., transient_epi_states=(),
            excluded_target_region_groups=(), priority_memory_regions=Unbounded(),
        )


@pytest.mark.parametrize("changes", ({"max_priority_regions": True}, {"max_priority_regions": 0},
                                     {"max_cover_work": False}, {"max_cover_work": 1.0},
                                     {"max_priority_regions": 1, "priority_memory_regions": ((0, None), (0, None))}))
def test_new_resource_limits_reject_noninteger_boolean_or_overfull_inputs(memory, changes):
    with pytest.raises(ValueError):
        _derive(memory, **changes)


def test_out_of_range_priority_memory_and_candidate_hint_mix_are_rejected(memory, cover):
    with pytest.raises(ValueError, match="canonical return"):
        _derive(memory, priority_memory_regions=((len(memory.return_envelope.return_relation), None),))
    with pytest.raises(ValueError, match="mutually exclusive"):
        _derive(memory, priority_memory_regions=((0, None),), candidate_retained_zones=cover.partition.seed_zones)


@pytest.mark.parametrize("row,point", ((0, (0, 0)), (1, (6, 0)), (2, (5, 5)), (2, (0, 7))))
def test_complete_queries_preserve_direct_terminal_and_origin_sentinel_visits(memory, cover, row, point):
...
def test_failed_gate_and_partial_query_budget_cannot_authorize_exclusion(memory, cover):
    ...
    for checked, maximum in ((failed, 10_000), (cover, 1)):
        result = owner._derive_return_safe_cover_region_queries(checked, (supplied,), maximum)
...
def canonical():
    ...
    state = NodalRemainderState(row, (F(0),) * 6, .375, .625)
    excluded = (C6CarriedForwardZone(row, _hull(((1, 0),))),)
    args = dict(state=state, epi_states=(row,), timestep=.0625, transient_epi_states=(),
                excluded_target_region_groups=(excluded,), max_partition_work=10_000)
...
def test_public_boundary_rebuilds_forged_source_caches(canonical):
    ...
    forged = replace(reference, sources=(17.,) * 6, rows=(), epi_quantum=F(7))
...
@pytest.mark.parametrize("field", ("partition", "partition_arcs", "seed_zones", "root_zones", "cover"))
def test_public_query_rejects_external_geometry_and_forged_certificate_dataclasses(canonical, field):
    reference, args, result = canonical
    forged = replace(result, partition=replace(result.partition, partition_arcs=(),
                                               retained_zones=(None,) * len(result.partition.vertices)))
    supplied = forged if field == "cover" else forged.partition if field == "partition" else ()
    ...
    with pytest.raises(TypeError, match="unexpected keyword"):
        owner.derive_c6_carried_return_safe_cover_region_exclusions(
            reference, **args, target_region_groups=(target,), **{field: supplied},
        )


def test_public_candidate_is_rechecked_against_rebuilt_root_and_query_observations(canonical):
    reference, args, result = canonical
    ...
    original = owner.derive_c6_carried_return_safe_cover_region_exclusions(
        reference, **args, candidate_retained_zones=result.retained_zones, target_region_groups=(targets,),
    )
    ...
    forged = owner.derive_c6_carried_return_safe_cover_region_exclusions(
        reference, **args, candidate_retained_zones=(None,) * len(result.partition.vertices), target_region_groups=(targets,),
    )
    assert not forged.cover.check.certified and not forged.queries[0].origin_path_within_domain_excluded


@pytest.mark.parametrize("options", ({"priority_flags": [False, True]}, {"priority_flags": (0, 1)},
                                     {"priority_flags": (False,)}, {"initial_work": True},
                                     {"initial_work": -1}, {"initial_work": 11}, {"initial_work": 1.0}))
def test_shared_scheduler_rejects_malformed_flags_and_classification_work(cover, options):
    partition = _algebra_partition(cover.partition)
    with pytest.raises(ValueError):
        owner._return_partition_descent(partition.seed_zones, partition.root_zones,
                                        partition.partition_arcs, 10, **options)


def test_complete_relation_does_not_authorize_partial_target_initialization(memory, cover):
    group = _target(memory, 0, (0, 0)) + _target(memory, 1, (6, 0)) + _target(memory, 2, (5, 5))
...
def test_partial_backward_layer_retains_only_the_last_complete_image(memory, cover):
    ...
    assert a.iterations == b.iterations and a.retained_endpoint_zones == b.retained_endpoint_zones
    assert not a.origin_path_within_domain_excluded and not b.origin_path_within_domain_excluded
...
def test_incomplete_canonical_relation_never_creates_a_cover_or_query_certificate(memory):
    result = _derive(memory, max_partition_arcs=1)
    assert not result.partition.relation_complete and result.partition.partition_arcs == ()
    assert result.check.status == "source_construction_incomplete"
    assert not result.domain_confined_origin_histories_covered and result.retained_zones == ()
    query = owner._derive_return_safe_cover_region_queries(result, (_target(memory),), 10_000).queries[0]
    assert not query.origin_path_within_domain_excluded
