...
def report(evidence):
    ...
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
            campaign,
            "analyze_c6_winding_memory_cuts",
            lambda *_a, **_k: deepcopy(parent),
        )
        return campaign.analyze_c6_winding_safe_partition(
            json.loads(inputs[0]),
            **arguments,
            priority_bytes=raw,
            priority_target=(28, 3, "upper"),
            excluded_predecessor_depth=1,
            max_partition_work=2000,
...
def test_cover_report_keeps_scope_and_every_original_target(report, evidence):
    ...
    assert report["B63_priority_evidence"] == dict(
        ...
        target=dict(mask=28, node=3, direction="upper"),
        advisory_only=True,
...
def test_missing_sentinel_and_oversized_hints_rejected(evidence):
    ...
    query = next(
        row["query"]
        for row in changed["B59_partition_queries"]
        if row["target"] == dict(mask=28, node=3, direction="upper")
    )
    query["retained_endpoint_zones"].pop()
    with pytest.raises(ValueError, match="sentinel"):
        campaign._c6_priority_hints(
            json.dumps(changed).encode(), (28, 3, "upper"), inputs, 5000
        )
...
def test_target_without_hint_rejected_before_reconstruction(evidence, monkeypatch):
    ...
    def forbidden(*_args, **_kwargs):
        pytest.fail(
            "invalid advisory arguments must fail before ancestor reconstruction"
        )
