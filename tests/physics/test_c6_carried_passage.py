...
def _tube(*, epi=None, phase=None, weight=.5, phase_weight=.5, lower=.375, upper=.625, timestep=.5):
    lattice = derive_c6_pressure_lattice(
        phase=(0.,) * 6 if phase is None else phase, epi_weight=weight, phase_weight=phase_weight,
    )
    if epi is None:
        epi = (math.nextafter(.5, -math.inf),) + (.5,) * 5
    state = initialize_nodal_remainder(epi, epi_lower=lower, epi_upper=upper)
    return derive_c6_carried_tube(derive_c6_carried_profile(lattice), state=state, timestep=timestep)
...
def test_guaranteed_passage_is_observed_before_the_derived_deadline():
    ...
    for index in range(result.latest_pressure_index + 1):
        reading = observe_c6_pressure_lattice(result.tube.contraction.profile.lattice, epi=state.epi)
        assert reading.mean_pressure <= result.mean_pressure_upper
        assert all(abs(value) <= limit for value, limit in
                   zip(reading.epi_reduction_error, result.product_error_bounds, strict=True))
        assert all(abs(value) <= limit for value, limit in
                   zip(reading.assembly_error, result.assembly_error_bounds, strict=True))
        if reading.pressure[0] <= 0:
            break
        state = advance_nodal_remainder(
            state, timestep=result.tube.contraction.timestep, capacity=(1.,) * 6, pressure=reading.pressure,
        ).after
    else:
...
def test_contradiction_horizon_uses_strict_endpoint_inequality_and_is_minimal():
    ...
    assert result.minimum_positive_index == result.positive_sector.bounding_gradient_index
    assert result.minimum_positive_pressure == result.positive_sector.signed_pressure_margin


@pytest.mark.parametrize("node", (True, 1., -1, 6, "0"))
...
def test_forged_tube_caches_cannot_create_a_stronger_deadline():
    ...
    forged = replace(original, energy_bound=F(0), initial_error=(F(0),) * 6,
                     carry_bound=F(0), mean_increment_bound=F(0))
...
def test_initial_nonpositive_pressure_is_a_zero_step_observation(node):
    result = derive_c6_carried_positive_pressure_passage(_tube(epi=(.5,) * 6), node=node)
    assert result.initial_pressure == 0
    assert result.sign_hit_certified and result.latest_pressure_index == result.contradiction_steps == 0
    assert result.abstention_reason is None


def test_narrow_band_cannot_promote_a_conditional_contradiction_to_a_sign_hit():
    upper = math.nextafter(.5, math.inf)
    tube = _tube(epi=(upper,) + (.5,) * 5, lower=.5, upper=upper)
    ...
    assert result.latest_pressure_index is None and "band horizon" in result.abstention_reason


def test_large_source_profile_can_prevent_a_strict_centered_increment_bound():
    tube = _tube(phase=(0., .25, 0., 0., 0., 0.), weight=1e-300,
                 epi=(.5, .5, .5, .625, .375, .5), timestep=1 / 16)
...
def test_empty_positive_sector_does_not_invent_an_extreme_gradient():
    tiny = math.ulp(0.)
    result = derive_c6_carried_positive_pressure_passage(_tube(epi=(.5,) * 6, weight=tiny), node=0)
...
def test_winding_source_obtains_finite_passage_without_an_observed_error_fit():
    ...
    tube = _tube(epi=epi, phase=phase, weight=float(F(3299399280161697, 2**54)),
                 phase_weight=float(F(854047988748571, 2**50)), timestep=1 / 16)
    result = derive_c6_carried_positive_pressure_passage(tube, node=4)
