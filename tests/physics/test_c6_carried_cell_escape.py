...
def _epi(offsets):
    return tuple(0.5 + n * 2.0**-54 for n in offsets)


def _closure(*, phase=PHASE, timestep=1 / 16, lower=0.375, upper=0.625, epi=None):
    profile = derive_c6_carried_profile(
    ...
    state = initialize_nodal_remainder(
        _epi(OFFSETS) if epi is None else epi, epi_lower=lower, epi_upper=upper
    )
...
def test_uniform_negative_step_bound_is_derived_from_pressure_interval_endpoints():
    ...
    assert (
        result.minimum_cell_grid_width
        == lattice.epi_quantum / 2 - 2 * result.grid_quantum
    )
    ...
    for lo, hi, mlo, mhi, source in zip(
        result.pressure_lower,
        result.pressure_upper,
        result.closure.gradient_index_lower,
        result.closure.gradient_index_upper,
        lattice.sources,
        strict=True,
    ):
        assert lo == F(
            source + float(lattice.source.epi_weight * lattice.gradient_quantum * mlo)
        )
        assert hi == F(
            source + float(lattice.source.epi_weight * lattice.gradient_quantum * mhi)
        )
        assert lo <= hi
    assert result.finite_complete_cell_union_invariance_excluded
    assert not result.correlated_carry_region_excluded
    assert (
        not result.saved_trajectory_escape_certified
...
def test_greatest_admissible_carries_produce_one_sided_canonical_escape(offsets):
    ...
    assert (
        result.family_invariance_excluded
        and not result.saved_trajectory_escape_certified
    )
    ...
    assert all(
        y >= x for x, y in zip(result.state.epi, result.endpoint.after.epi, strict=True)
    )
    for value, visible, pressure, changed in zip(
        exact,
        result.state.epi,
        result.pressure,
        result.endpoint.visible_increment,
        strict=True,
    ):
        cell = _rounding_cell(visible, value)
        assert cell.contains_exact_input
        assert value == cell.upper - (
            0 if cell.even_significand else bound.grid_quantum
        )
...
def test_family_witness_selects_the_maximum_visible_sum_and_escapes_the_entire_union():
    ...
    result = observe_c6_carried_complete_cell_escape(
        derive_c6_carried_complete_cell_obstruction(_closure()),
        epi_states=family,
    )
...
def test_physical_upper_boundary_is_reported_as_band_failure_without_a_fake_endpoint():
    ...
    assert (
        result.band_failure
        and result.endpoint is None
        and result.endpoint_visible_sum is None
    )
    assert any(value > F(0.625) for value in result.exact_candidate)
    assert all(value >= F(0.375) for value in result.exact_candidate)
...
def test_public_closure_and_obstruction_caches_are_fully_rederived():
    ...
    forged = replace(
        closure, gradient_index_lower=(0,) * 6, gradient_index_upper=(0,) * 6
    )
    assert derive_c6_carried_complete_cell_obstruction(forged) == expected
    forged_bound = replace(
        expected,
        grid_quantum=F(1),
        minimum_cell_grid_width=F(0),
        pressure_lower=(F(0),) * 6,
        pressure_upper=(F(0),) * 6,
    )
    assert observe_c6_carried_complete_cell_escape(
        forged_bound, epi_states=(_epi(OFFSETS),)
    ) == (
...
def test_a_source_without_positive_pressure_everywhere_cannot_use_the_obstruction():
    with pytest.raises(ValueError, match="all-nonpositive"):
        derive_c6_carried_complete_cell_obstruction(_closure(phase=(0.0,) * 6))
...
def test_a_large_nodal_step_abstains_from_the_complete_cell_argument():
    with pytest.raises(ValueError, match="negative nodal increments"):
        derive_c6_carried_complete_cell_obstruction(_closure(timestep=1.0))


def test_a_singleton_band_has_no_uniform_full_cell_width():
    with pytest.raises(ValueError, match="at least one"):
        derive_c6_carried_complete_cell_obstruction(
            _closure(lower=0.5, upper=0.5, epi=(0.5,) * 6)
        )


def test_narrower_band_membership_is_enforced_before_selecting_a_witness():
    bound = derive_c6_carried_complete_cell_obstruction(
        _closure(lower=0.49, upper=0.51)
    )
    with pytest.raises(ValueError, match="closure's declared band"):
        observe_c6_carried_complete_cell_escape(bound, epi_states=((0.625,) * 6,))
