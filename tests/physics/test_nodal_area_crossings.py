...
from tnfr.physics.nodal_remainder_pressure import derive_nodal_area_crossings, derive_two_level_nodal_return


def _cross(**changes):
    args = dict(initial_area=(F(-3),), timestep=.5, capacity=(1.,), pressure=(2.,), max_steps=4)
    args.update(changes)
...
def test_exact_integer_repayment_retains_before_and_after_areas():
    ...
    assert not result.pressure_provenance_certified and not result.band_provenance_certified


@pytest.mark.parametrize("sign", [-1, 1])
def test_fractional_root_crosses_without_exact_integer_zero_in_both_directions(sign):
    result = _cross(initial_area=(sign * F(3, 2),), pressure=(-sign * 2.,))
    coordinate = result.coordinates[0]
...
def test_actual_b29_positive_level_requires_four_steps_but_declared_prefix_has_two():
    ...
    result = _cross(initial_area=(debt,), timestep=.0625, pressure=(pressure,), max_steps=2)
...
def test_initial_zero_is_not_a_later_repayment():
    ...
    assert result.positive_joint_zero_step is None and not result.joint_zero_for_all_steps
...
def test_stationary_zero_has_all_zero_steps_and_first_positive_candidate_one(maximum):
    result = _cross(initial_area=(F(0), F(0)), timestep=0., capacity=(1., 2.),
                    pressure=(-3., 5.), max_steps=maximum)
...
def test_joint_vector_roots_must_agree_in_every_coordinate():
    mismatch = _cross(initial_area=(F(-2), F(-3)), capacity=(1., 1.), pressure=(2., 2.))
...
def test_constant_zero_coordinate_does_not_constrain_a_common_root():
    result = _cross(initial_area=(F(-1, 2), F(1, 4), F(0)), timestep=.25,
                    capacity=(2., 1., 0.), pressure=(.5, -.5, 100.), max_steps=2)
...
def test_constant_nonzero_coordinate_prevents_joint_zero():
    result = _cross(initial_area=(F(-2), F(1)), capacity=(1., 0.), pressure=(2., 4.))
...
def test_zero_mean_endpoint_is_not_vector_return():
    result = _cross(initial_area=(F(-3), F(3)), capacity=(1., 1.), pressure=(2., -2.), max_steps=2)
...
@pytest.mark.parametrize("changes,error", [
    ({"initial_area": [F(-1)]}, TypeError), ({"initial_area": ()}, ValueError),
    ({"initial_area": (-1,)}, TypeError), ({"initial_area": (-1.,)}, TypeError),
    ({"timestep": -1.}, ValueError), ({"timestep": True}, TypeError),
    ({"timestep": math.inf}, ValueError), ({"timestep": math.nan}, ValueError),
    ({"capacity": [1.]}, TypeError), ({"capacity": (-1.,)}, ValueError),
    ({"capacity": (1,)}, TypeError), ({"capacity": (math.inf,)}, ValueError),
    ({"capacity": (1., 2.)}, ValueError), ({"pressure": (math.nan,)}, ValueError),
    ({"pressure": (F(1),)}, TypeError), ({"pressure": ()}, ValueError),
    ({"max_steps": -1}, ValueError), ({"max_steps": True}, TypeError),
    ({"max_steps": 2.}, TypeError),
])
...
def test_actual_two_level_primitive_counts_exclude_short_exact_returns():
    ...
    assert (result.minimum_negative_steps * F(result.negative_pressure)
            + result.minimum_positive_steps * F(result.positive_pressure)) == 0
    assert not result.pressure_provenance_certified and not result.periodic_execution_certified


def test_two_level_gcd_removes_a_common_numerator_factor():
    result = derive_two_level_nodal_return(negative_pressure=-6., positive_pressure=9.)
    assert (result.common_denominator, result.negative_integer, result.positive_integer) == (1, 6, 9)
    assert result.gcd == 3
    assert (result.minimum_negative_steps, result.minimum_positive_steps, result.minimum_total_steps) == (3, 2, 5)


def test_two_level_different_denominators_and_subnormal_pressures():
    result = derive_two_level_nodal_return(negative_pressure=-.375, positive_pressure=.25)
    assert (result.common_denominator, result.negative_integer, result.positive_integer) == (8, 3, 2)
    assert (result.minimum_negative_steps, result.minimum_positive_steps) == (2, 3)
    tiny = math.ulp(0.)
    result = derive_two_level_nodal_return(negative_pressure=-2 * tiny, positive_pressure=3 * tiny)
...
def test_an_additional_pressure_level_invalidates_the_two_level_length_restriction():
    result = derive_two_level_nodal_return(negative_pressure=-.25, positive_pressure=.5)
    assert result.minimum_total_steps == 3
    assert F(-.25) + F(.25) == 0  # A new level permits two steps, outside this certificate.


@pytest.mark.parametrize("negative,positive,error", [
    (0., 1., ValueError), (-0., 1., ValueError), (1., 1., ValueError),
    (-1., 0., ValueError), (-1., -1., ValueError),
    (-1, 1., TypeError), (-1., 1, TypeError), (False, 1., TypeError),
    (-math.inf, 1., ValueError), (-1., math.nan, ValueError),
])
def test_two_level_invalid_inputs_are_rejected(negative, positive, error):
    with pytest.raises(error):
        derive_two_level_nodal_return(negative_pressure=negative, positive_pressure=positive)
