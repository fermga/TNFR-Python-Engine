...
def test_saved_endpoint_and_static_pressure_rows_keep_separate_provenance(report, parents):
    source = report["source"]
    assert source["source_report_replayed"] is True
    assert campaign._payload(source["saved_B40_state"]) == parents[0]["B40_continuation"]["endpoint"]
    rows = tuple(tuple(point["state"]["epi"]) for point in parents[0]["B38_static_pressure_balance"]["points"])
...
def test_complete_cell_obstruction_uses_exact_uniform_negative_step_bound(report):
    ...
    assert section["maximum_negative_increment"] / delta == F(5757725321284133, 36028797018963968)
    assert 0 < section["maximum_negative_increment"] < section["minimum_cell_grid_width"]
    assert section["finite_complete_cell_union_invariance_excluded"] is True
    for flag in ("correlated_carry_region_excluded", "saved_trajectory_escape_certified", "future_runtime_certified"):
        assert section[flag] is False
    assert "closure" not in section


def test_hypothetical_witness_changes_carry_without_claiming_a_saved_continuation(report):
    ...
    before = tuple(F(x) + r for x, r in zip(witness["state"]["epi"], witness["state"]["remainder"], strict=True))
    assert witness["exact_increment"] == tuple(F(value) / 16 for value in witness["pressure"])
    assert witness["exact_candidate"] == tuple(x + a for x, a in zip(before, witness["exact_increment"], strict=True))
    ...
    assert witness["endpoint_visible_sum"] - witness["source_visible_sum"] == F(1, 2**52)
    assert "obstruction" not in witness


def test_seven_cell_deadline_binds_exactly_the_original_seven_pressures(report, parents):
    ...
    edges = tuple((i, j) for i, row in enumerate(section["adjacency"]) for j, edge in enumerate(row) if edge)
    assert edges == ((0, 0), (1, 1), (2, 2), (3, 3), (4, 4), (5, 4), (5, 5), (6, 6))
    assert tuple(item["first_exit_step"] for item in section["maximal_residence"]) == (77, 51, 39, 51, 40, 30, 50)
    ...
    assert section["finite_family_escape_certified"] is section["carried_cycle_in_family_excluded"] is True
    assert "reference" not in section
    for flag in ("whole_band_exit_certified", "future_runtime_certified", "reachable_from_saved_state_certified"):
        assert section[flag] is False


def test_static_campaign_never_enters_the_runtime_branch_or_mutates_inputs(parents, monkeypatch):
    def forbidden():
        raise AssertionError("the static cell study must not invoke a graph preparation")
...
def test_runtime_branch_receives_no_saved_state_and_does_not_promote_its_reachability(parents, monkeypatch):
    calls = []
    ...
    result = campaign.analyze_c6_winding_temporal_compatibility(*parents, include_runtime=True)
    assert calls == [()]
...
def test_scope_flags_and_exact_payload_remain_json_serializable(report):
    for flag in ("infinite_band_invariance_certified", "full_runtime_stability_certified", "B40_origin_reachability_certified"):
        assert report[flag] is False
    assert report["B43_original_phase_tail_runtime"] is None
    encoded = campaign._payload(report)
    json.dumps(encoded, allow_nan=False)
    assert encoded["B42_finite_cell_graph"]["maximum_steps_until_family_exit_or_band_failure"] == 77


@pytest.mark.parametrize("change", (
    "claim", "scope", "carry", "phase", "balance_weight", "flag", "ancestor_pressure", "lineage",
))
def test_tampered_parent_or_ancestor_cannot_certify_a_new_temporal_result(parents, change):
    ...
    if change == "claim":
        ...
        values[2]["continuation"]["steps"][0]["pressure"][4] = 0.
    else:
...
def _cli(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["temporal", "--history-dir", str(tmp_path), "--output", str(output), "--skip-runtime"])
    return paths, output


def test_cli_binds_eight_historical_hashes_and_respects_static_execution(tmp_path, monkeypatch, report):
...
def test_cli_checks_all_ancestral_byte_links_before_analysis(tmp_path, monkeypatch, changed_index):
    paths, output = _cli(tmp_path, monkeypatch)
    ...
    monkeypatch.setattr(campaign, "analyze_c6_winding_temporal_compatibility", forbidden)
...
def test_cli_aborts_on_source_or_input_changes_during_analysis(tmp_path, monkeypatch, report, change):
    paths, output = _cli(tmp_path, monkeypatch)
...
def test_cli_cannot_overwrite_the_saved_parent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["temporal", "--history-dir", str(tmp_path), "--output", str(paths[0]), "--skip-runtime"])
