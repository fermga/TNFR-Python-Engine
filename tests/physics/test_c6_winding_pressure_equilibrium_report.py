...
def test_only_selected_retained_case_is_validated_without_graph_execution(
    parent, monkeypatch
):
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError(
            "the static report must not execute a graph or operator event"
        )
...
def test_source_is_actual_post_il_phase_and_default_pressure(parent, report):
    ...
    assert tuple(map(F, source["phase"])) != tuple(
        map(F, parent["current_cases"][0]["initial_capture"]["phase"])
    )
    assert source["epi"] == (0.5,) * 6 and source["capacity"] == (1.0,) * 6
    assert source["event_prefix"] == ("coupling", "coherence")
    assert source["shared_cpu_pressure_matches_capture"] is True
    assert tuple(map(F, source["pressure"])) == tuple(
        map(F, retained["snapshot"]["stored_pressure"])
    )
    assert source["pressure_sum"] == F(25, 2**112)
    assert source["pressure_mean"] == F(25, 6 * 2**112)
    assert (
        source["exact_midpoint_sum_rational"]
        == source["exact_midpoint_sum_pi_coefficient"]
        == 0
    )
...
def test_all_six_exact_inverse_cells_exclude_the_entire_epi_gradient_lattice(report):
    ...
    for row, (first, last) in zip(obstruction["rows"], expected, strict=True):
        ...
        assert (
            row["inverse_gradient_lower"] * obstruction["epi_weight"] == cell["lower"]
        )
        assert (
            row["inverse_gradient_upper"] * obstruction["epi_weight"] == cell["upper"]
        )
...
def test_held_source_carry_exits_its_first_cell_on_step_six_without_leaving_band(
    report,
):
    ...
    assert horizon["unchanged_endpoint"] == tuple(
        x + 5 * a for x, a in zip(exact, increment, strict=True)
    )
    assert horizon["first_exit_exact"] == tuple(
        x + 6 * a for x, a in zip(exact, increment, strict=True)
    )
    assert all(
        cell["lower"] <= y <= cell["upper"]
        for cell, y in zip(
            horizon["source_cells"],
            horizon["unchanged_endpoint"],
            strict=True,
        )
    )
    outside = tuple(
        not (cell["lower"] <= y <= cell["upper"])
        for cell, y in zip(
            horizon["source_cells"],
            horizon["first_exit_exact"],
            strict=True,
        )
    )
    ...
    assert float(horizon["first_exit_exact"][5]) == math.nextafter(0.5, -math.inf)


def test_scope_and_serialization_do_not_promote_fixed_phase_to_the_event_schedule(
    report,
):
    for name in (
        "runtime_executed",
        "live_provenance_certified",
        "B23_phase_identity_certified",
        "positive_band_exit_certified",
        "signed_mean_convergence_decided",
...
def test_changed_shared_cpu_source_is_not_accepted_as_historical_pressure(
    parent, monkeypatch
):
