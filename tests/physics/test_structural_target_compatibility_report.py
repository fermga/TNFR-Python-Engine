...
def parent():
    ...
    for name, capacity in (("U", (1.0, 1.0)), ("P", (1.25, 1.0)), ("F", (1.125, 1.0))):
        ...
        cases.append({
            "case": name, "status": "measured", "original_target": asdict(target),
            "postevent_reference": asdict(reference), "initial": {"state": actual},
            "events": [] if name == "U" else [{
                "after_forcing_capture": asdict(capture), "after_refresh": {"state": actual},
            }],
            "segments": [{"forcing_capture": asdict(capture), "after_refresh": {"time": 9.75}}],
            "conditional_fixed_model_limit": {"pattern": asdict(limit)},
        })
    return _payload({"cases": cases})


def test_offline_report_preserves_literal_node_ids_and_exact_channel_allocation(parent, monkeypatch):
    ...
    assert sum(value for _, value in delta["channel_contributions"]) == (
        f["observation"]["compatibility_energy"] - p["observation"]["compatibility_energy"]
    )
...
def test_stale_reference_source_cannot_replace_actual_endpoint(parent, branch, source_field):
...
def test_recorded_limit_is_checked_against_rebuilt_reference(parent):
    parent["cases"][1]["conditional_fixed_model_limit"]["pattern"]["error_variance"] = "0"
...
def test_main_cannot_overwrite_input_evidence(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["target", "--input", str(source), "--output", str(source)])
    with pytest.raises(ValueError, match="overwrite"):
        analysis.main()
    assert source.read_text(encoding="utf-8") == "retained"


def test_analysis_owns_its_source_scope_independently_of_producer_metadata(parent, tmp_path, monkeypatch):
    parent.update(source_scope=["historical/producer.py"], manifest={
        "claim_id": "synthetic-schema-input", "git_sha": "1" * 40,
        "versions": {"python": "synthetic"}, "graph_construction": "prepared fixture",
        "capacity_specification": "fixture", "solver": "no integration",
        "result_status": "measured", "source_dirty": False,
        "telemetry": ["synthetic channels"], "controls": ["synthetic P2"],
    ...
    monkeypatch.setattr(sys, "argv", ["target", "--input", str(source), "--output", str(output)])
    analysis.main()
    result = json.loads(output.read_text(encoding="utf-8"))
    assert queried == [analysis.SOURCE_SCOPE, analysis.SOURCE_SCOPE]
    assert "src/tnfr" in result["source_scope"]
    assert result["input_evidence"]["producer_source_scope"] == ["historical/producer.py"]
