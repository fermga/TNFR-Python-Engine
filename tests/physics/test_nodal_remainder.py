...
def _observe(
    ...
    *,
    timesteps=(0.0625,),
    capacities=((1.0, 1.0),),
    pressures=((2.0**-50, -(2.0**-50)),),
):
    return observe_nodal_remainder_sequence(
        initial=initial or initialize_nodal_remainder((0.5, 0.5)),
        timesteps=timesteps,
...
def test_balanced_area_can_have_visible_mean_bias_but_exact_reconstructed_mean():
    ...
    assert result.endpoint.epi == (0.5, 0.5 - 2.0**-54)
...
def test_variable_supplied_inputs_have_independent_exact_prefix_balance():
    count = 67
    initial = initialize_nodal_remainder((0.25, 0.5, 0.75))
    timesteps = tuple(0.03125 if i % 2 else 0.0625 for i in range(count))
    capacities = tuple((1.0, 0.5, 2.0) for _ in range(count))
    pressures = tuple(
        ((-1.0) ** i * 2.0**-48, 2.0**-50, -(2.0**-52)) for i in range(count)
    )
    result = _observe(
        initial, timesteps=timesteps, capacities=capacities, pressures=pressures
    )
    area = [F(0)] * 3
    for k, prefix in enumerate(result.prefixes):
        ...
        assert (
            prefix.cumulative_nodal_area == prefix.reconstructed_change == tuple(area)
        )
...
def test_nonzero_initial_carry_has_signed_endpoint_telescope():
    ...
    result = _observe(first, timesteps=(0.0,))
...
def test_carry_reset_is_an_exact_state_jump_not_another_valid_continuation():
    ...
    assert tuple(
        y - x for x, y in zip(first.exact_epi, reset.exact_epi, strict=True)
    ) == tuple(-r for r in first.remainder)
    carried = _observe(first).endpoint
    discarded = _observe(reset).endpoint
    assert (
        tuple(
            x - y for x, y in zip(carried.exact_epi, discarded.exact_epi, strict=True)
        )
        == first.remainder
    )
...
def test_heterogeneous_capacity_requires_balanced_nodal_area():
    result = _observe(capacities=((2.0, 1.0),), pressures=((0.125, -0.125),))
    prefix = result.prefixes[0]
...
def test_subnormal_exact_area_is_retained_with_independent_cells():
    tiny = math.ulp(0.0)
    result = _observe(
        initialize_nodal_remainder((tiny,), epi_lower=tiny),
        timesteps=(tiny,),
        capacities=((tiny,),),
        pressures=((tiny,),),
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"timesteps": (0.0625, 0.0625)}, ValueError),
        ({"pressures": [[1.0, 1.0]]}, TypeError),
...
def test_failing_later_step_leaves_initial_encoding_unchanged():
    ...
    with pytest.raises(ValueError, match="band"):
        _observe(
            initial,
            timesteps=(0.0625, 1.0),
            capacities=((1.0, 1.0),) * 2,
            pressures=((2.0**-50, -(2.0**-50)), (2.0, 2.0)),
        )
...
def test_odd_tie_rounds_to_even_cell_and_preserves_the_tie_remainder():
    source = math.nextafter(0.5, 1.0)
    initial = initialize_nodal_remainder((source,))
    result = _observe(
        initial, timesteps=(1.0,), capacities=((1.0,),), pressures=((2.0**-54,),)
    )
    ...
    direct = advance_nodal_remainder(
        initial, timestep=1.0, capacity=(1.0,), pressure=(2.0**-54,)
    )
