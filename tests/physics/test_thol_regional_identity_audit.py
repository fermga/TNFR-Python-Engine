...
from tests.physics.test_thol_regional_balance_audit import _write_fixture, prior as _prior_fixture
...
def _capture(state, template, *, phase_increment=F(0)):
    ...
    snapshot = _from_data(
        state["nodes"], [(i, j, F(w)) for i, j, w in original["conductance"]],
        original["support_neighbors"],
        state["epi"], state["capacity"], state["pressure"],
    )
...
def _record(state, prior, *, glyph_count=0):
    ...
    for _, data in attributes:
        data.update(glyph_history=_literal(["IL"]*glyph_count), _operator_step=glyph_count,
                    _grammar_u2_debt=0, _grammar_prior_coherence=True, stable_count=0)
    return {"state": deepcopy(state), "node_attributes": attributes,
            "ordered_neighbors": [[node, [state["nodes"][j] for j in row]] for node, row in zip(
                state["nodes"], prior["original_reference"]["source"]["support_neighbors"], strict=True)],
            "graph_attributes": {"GLYPH_FACTORS": _literal({"IL_dnfr_factor": .5}),
                                 "_dnfr_weights": {key: _literal(float(F(value))) for key, value in
                                                   prior["prefix"]["coupling"]["refreshed_forcing"]
                                                   ["observation"]["normalized_weights"]},
                                 "history": {"since_AL": {str(n): 1 for n in state["nodes"]},
...
def native_fixture(prior):
    ...
    generated["pressure"] = [float(F(value)) for value in
                             _capture(start, template)["payload"]["observation"]["full_kernel_pressure"]]
    records = []

    def row(name, before, after, **extra):
        records.append({"ordinal": len(records), "boundary": name, "outcome": "completed",
                        "before": _record(before, prior), "after": _record(after, prior), **extra})
        return records[-1]
    ...
    for i, node in enumerate(start["nodes"]):
        ...
        after["pressure"][i] = .5*current["pressure"][i]
    ...
    integrated["epi"] = [float(F(x)+F(1, 4)*F(nu)*F(p)) for x, nu, p in zip(
        consumed["epi"], consumed["capacity"], consumed["pressure"], strict=True)]
    integration = row("integrate", consumed, integrated,
                      effective_arguments={"dt": _literal(.25), "method": "euler", "t": None, "n_jobs": None},
    ...
    branch = {"branch": "control", "status": "executed", "before": _record(start, prior),
              "endpoint": _record(endpoint, prior), "before_capture": _capture(start, template),
              "endpoint_capture": _capture(endpoint, template, phase_increment=F(1, 16)),
              "native_trace": {"status": "executed", "failure": None, "boundaries": records,
                               "actual_glyph_calls": calls, "native_calls": 1,
                               "selector_requests": [{"node": n, "glyph": "IL"} for n in start["nodes"]],
                               "captures": {"integrator_entry": _capture(consumed, template)},
                               "clamp_interval": {"after_integrate_ordinal": integration["ordinal"],
                                                  "before_phase_ordinal": phase["ordinal"]}}}
    return prior, _payload(branch)


def _one(branch, name):
    return next(row for row in branch["native_trace"]["boundaries"] if row["boundary"] == name)
...
def _coordinates(branch):
    ...
    degree = tuple(sum((w for (j, _), w in weights.items() if j == i), F(0)) for i in range(16))
    nu = tuple(map(F, source["capacity"]))
    metric = tuple(d/n for d, n in zip(degree, nu, strict=True))
    x, xf, pressure = (tuple(map(F, values)) for values in
                       (source["epi"], target["epi"], source["pressure"]))
...
def test_nine_exact_finite_budgets_and_regional_source_terms(native_fixture):
    ...
    assert result["actual_lineage"]["pairs"] == tuple(map(tuple, prior["lineage"]["parent_children"]))
    assert [item["label"] for item in result["regions"]] == [f"ancestry_pair_{i}" for i in range(8)]+["actual_children"]
    for item in result["regions"]:
        ...
        def variance(values):
            return sum((metric[i]*(values[i]-mean(values))**2 for i in ids), F(0))/2
        ...
        quadratic = sum((metric[i]*(rate[i]-mean(rate))**2 for i in ids), F(0))/32
        linear_defect = sum((metric[i]*(expected[i]-mean(expected))*(defect[i]-mean(defect))
                             for i in ids), F(0))
        ...
        assert finite["variance_change"] == drift+quadratic+linear_defect+variance(defect)
        assert finite["mass_identity_residual"] == finite["variance_identity_residual"] == 0
        cut = -F(1, 2)*sum((w*(x[i]-x[j]) for (i, j), w in weights.items()
    ...
    assert result["regions"][-1]["finite_budget"]["balance"]["internal_dissipation"] == 0


def test_intentional_il_write_source_change_and_normalization_are_distinct(native_fixture):
    prior, branch = native_fixture
    result = audit.audit_branch(prior, branch)
    split = result["pressure_split"]
    generated = tuple(map(F, _one(branch, "_prepare_dnfr")["after"]["state"]["pressure"]))
    ...
    assert result["IL_factor_certified"] is False  # Attribution is not an IL kernel certificate.
    ...
    assert result["source_channel_delta_endpoint_minus_entry"] == {
        "phase": (F(1, 128),)*16, "vf": (F(0),)*16, "topo": (F(0),)*16}
...
@pytest.mark.parametrize("change", (
    "prior_endpoint", "interval", "dt", "integration_clock", "boundary_order", "duplicate_ordinal",
    "IL_order", "IL_summary", "IL_nonlocal_pressure", "preflow_epi", "postflow_epi", "support",
    "capacity", "parent_pointer", "node_order", "generation_phase", "generation_neighbors",
    "generation_config", "vectorized_config", "intermediate_phase", "normalized_weights",
    "clamp_bracket", "clamp_nonphase", "endpoint_pressure",
    "endpoint_phase", "snapshot_cache", "reference_cache", "forcing_components",
))
def test_inconsistent_native_receipt_is_rejected(native_fixture, change):
    prior, branch = native_fixture
    trace = branch["native_trace"]
    generation, integration, phase = (_one(branch, name) for name in
                                      ("_prepare_dnfr", "integrate", "coordinate_global_local_phase"))
    calls = [row for row in trace["boundaries"] if row["boundary"] == "apply_glyph"]
    if change == "prior_endpoint":
        prior["endpoint"]["epi"][0] += .125
    elif change == "interval":
        branch["endpoint"]["state"]["time"] = 2.0
    elif change == "dt":
        integration["effective_arguments"]["dt"] = _literal(.5)
        ...
        calls[0]["after"]["state"]["pressure"][1] += .125
    elif change == "preflow_epi":
        calls[0]["after"]["state"]["epi"][0] += .125
    elif change == "postflow_epi":
        phase["after"]["state"]["epi"][0] += .125
    elif change == "support":
        calls[0]["after"]["state"]["edges"][0][2]["weight"] = .75
    elif change == "capacity":
        calls[0]["after"]["state"]["capacity"][0] = .75
        ...
        generation["after"]["state"]["phase"][0] = .25
...
def _forbid_runtime(monkeypatch):
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError("retained regional audit must not execute a graph or phase kernel")
...
def test_portable_cli_digest_provenance_and_original_bytes(native_fixture, tmp_path, monkeypatch):
    prior, branch = native_fixture
    ...
    monkeypatch.setattr(sys, "argv", [
        "audit", "--input", str(path), "--expected-sha256", digest, "--output", str(output)])
    audit.main()
    ...
    canonical = json.dumps(branch, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()
...
def test_pinned_interval_if_available_without_runtime_or_kernel(monkeypatch):
    ...
    for region in result["regions"]:
        ...
        assert budget["mass_identity_residual"] == budget["variance_identity_residual"] == 0
