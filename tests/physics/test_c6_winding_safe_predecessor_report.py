...
def inputs():
    ...
    names = (
        ...
        "artifacts/research/c6_winding_memory_cuts.json",
    )
...
def patch_previous(monkeypatch, previous):
    monkeypatch.setattr(
        campaign, "analyze_c6_winding_memory_cuts", lambda *_a, **_k: deepcopy(previous)
    )
...
def report(inputs, previous):
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        return campaign.analyze_c6_winding_safe_partition(
            json.loads(inputs[0]),
            **arguments(inputs),
            excluded_predecessor_depth=1,
            max_partition_work=120_000,
            query_max_intersections=1,
        )


def test_depth_one_report_retains_original_source_and_only_verified_proposals(
    inputs, report
):
    ...
    assert (
        campaign._payload(report["B60_return_envelope"]["state"])
        == baseline["B57_return_envelope"]["state"]
    )
    assert (
        campaign._payload(report["B60_return_envelope"]["domain_zones"])
        == baseline["B57_return_envelope"]["domain_zones"]
    )
    assert report["B60_exclusion_proposals"] == tuple(
        baseline["B57_additional_first_exit_facets"]
    )
    assert (
        report["B60_reconstruction"]["prior_cuts_sha256"]
        == hashlib.sha256(inputs[13]).hexdigest()
    )
    partition = report["B60_partition"]
    assert (
        partition["excluded_predecessor_depth"] == 1
        and partition["predecessor_complete"]
    )
    ...
    assert partition["excluded_target_queries"][0]["status"] in (
        "empty_complete_layer",
        "stationary_complete_layer",
    )
    assert (
        report["primary_proof_origin_remains_B47"]
        and not report["contract"]["physical_dynamics_changed"]
    )
    assert (
...
def test_depth_one_keeps_all_sixteen_original_targets_without_promoting_incomplete_queries(
    inputs, report
):
    ...
    for item in queries:
        original = next(
            row["query"]["target_regions"]
            for row in baseline["B57_memory_queries"]
    ...
    assert (
        report["excluded_first_exit_slabs"] == 40
        and report["additional_first_exit_slabs_excluded"] == 0
    )
    assert (
        report["B60_original_cube_geometry"] == ancestor["B58_original_cube_geometry"]
    )
...
def test_depth_one_reconstructs_parent_evidence_instead_of_trusting_it(
...
def test_invalid_campaign_depth_is_rejected_before_reading_or_replaying_ancestors(
    depth,
):
    with pytest.raises((TypeError, ValueError), match="depth"):
        campaign.analyze_c6_winding_safe_partition(
            {}, **arguments((b"not json",) * 14), excluded_predecessor_depth=depth
        )
...
def test_cli_default_b59_and_depth_one_b60_have_separate_paths_claims_and_full_lineage(
    ...
    expected = (
        ...
        / "artifacts/research"
        / (
            "c6_winding_safe_predecessors.json"
            if depth
            else "c6_winding_safe_partition.json"
        )
    )
    received = []
    ...
    def analyze(parent, **kwargs):
        ...
        if kwargs.get("excluded_predecessor_depth", 0) == 0:
            result = {
                key.replace("B60_", "B59_", 1) if key.startswith("B60_") else key: value
    ...
    expected_claim = (
        "O3.a-C6-carried-safe-predecessors"
        if depth
        else "O3.a-C6-carried-safe-partition"
    )
...
def test_cli_positive_depth_rejects_unrelated_methods_before_input_reads(
...
def test_depth_one_cli_cannot_overwrite_primary_or_latest_evidence(
...
def test_depth_one_cli_rejects_midflight_primary_latest_or_source_changes(
