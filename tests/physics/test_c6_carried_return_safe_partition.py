...
from tnfr.physics.c6_carried_viability import C6CarriedForwardZone, _dbm_close, _Work
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ((2, 2),),
        ((3, 3),),
        tuple(product(range(-2, 3), repeat=2)),
        tuple(product(range(-3, 4), repeat=2)),
        tuple((x, y) for x, y in product(range(-2, 3), repeat=2) if 0 <= x - y <= 1),
    ),
)
def test_exact_integer_difference_matches_independent_grid_and_relative_basis(
    bad_points,
):
    ...
    assert all(
        piece and _dbm_close(zone) == zone
        for zone, piece in zip(pieces, observed, strict=True)
    )
    ...
    if _members(bounds) & _members(bad):
        inside = {
            (x, y)
            for x, y in _members(bounds)
            if all(
                (x, y, 0, 0, 0, 0, 0)[i] - (x, y, 0, 0, 0, 0, 0)[j] <= bound
...
def test_sequential_subtractions_keep_exact_union_and_integer_boundary():
    ...
    for bad in forbidden:
        pieces = tuple(
            piece
            for zone in pieces
            for piece in owner._return_safe_difference(zone, bad, work)[0]
        )
...
def memory():
    ...
    edges = (
        owner.C6CarriedReturnTransition(
            rows[0], None, rows[1], zero, None, _hull(((4, 0),)), (4, 0, 0, 0, 0, 0)
        ),
        owner.C6CarriedReturnTransition(
            rows[1], None, rows[1], box, None, box, (0,) * 6
        ),
    )
    terminal_specs = (
        (0, (0, 0), (0, 7)),
...
def _target(memory, row, *points):
    return (
        C6CarriedForwardZone(memory.return_envelope.epi_states[row], _hull(points)),
    )
...
def _derive(memory, groups=None, **changes):
    ...
    kwargs = dict(
        ...
        max_partition_pieces=100,
        max_partition_construction_work=100_000,
        max_partition_arcs=1000,
...
def test_verified_center_cut_preserves_every_other_endpoint_and_origin_image(
    memory, partition
):
    assert partition.initialization_complete and partition.relation_complete
    assert all(
        query.origin_path_within_domain_excluded
        for query in partition.excluded_target_queries
    )
    for index, zone in enumerate(memory.retained_endpoint_zones):
        seeds = tuple(
            piece
            for (old_index, _ordinal), piece in zip(
    ...
    assert all(
        _members(after) <= _members(seed)
        for after, seed in zip(
            partition.retained_zones, partition.seed_zones, strict=True
        )
    )
...
def test_two_disconnected_terminal_preimages_do_not_authorize_cutting_their_join(
    memory,
):
    ...
    memory = replace(
        ...
        return_envelope=replace(
            memory.return_envelope,
            intermediate_transitions=memory.return_envelope.intermediate_transitions[
                :-1
            ],
        ),
    )
    ...
    seeds = tuple(
        zone
        for (index, _ordinal), zone in zip(
            partition.vertices, partition.seed_zones, strict=True
        )
        if index == 1
    )
...
@pytest.mark.parametrize(
    "targets", ((0, ((0, 0),)), (1, ((4, 0),)), (2, ((0, 7),)), (2, ((4, 5),)))
)
def test_origin_sentinel_and_terminal_only_visits_survive_safe_partition(
    memory, partition, targets
):
    row, points = targets
    supplied = _target(memory, row, *points)
    result = owner._derive_return_safe_partition_region_queries(
        partition, (supplied,), 10_000
    )
...
def test_cut_terminal_target_remains_whole_while_its_safe_preimage_is_empty(
    memory, partition
):
...
def test_unverified_positive_targets_cannot_authorize_partition(memory, change):
    partition = (
        _derive(memory, (_target(memory, 0, (0, 0)),))
        if change == "origin"
    ...
    result = owner._derive_return_safe_partition_region_queries(
        partition, (_target(memory, 2, (4, 5)),), 10_000
    )
...
def _enumerated_graph(partition):
    ...
    for b, (j, _ordinal) in enumerate(partition.vertices):
        edge = edges[j]
        if edge.source_epi == memory.return_envelope.state.epi and (0, 0) in _members(
            memory.source_guards[j]
        ):
            image = edge.shift[:2]
            if image in zones[b]:
...
def test_partition_graph_is_complete_against_independent_point_enumeration(partition):
    ...
    assert complete
    actual = {
        (a, b, p, (p[0] + shift[0], p[1] + shift[1]))
        for a, b, guard, shift in observed
        for p in _members(guard)
    }
        ...
        for target in targets:
            ...
            for edge in envelope.intermediate_transitions:
                if edge.source_epi == row and edge.target_epi == target.epi:
                    selected |= {
                        point
                        for point in points & _members(edge.source_guard)
                        if (point[0] + edge.shift[0], point[1] + edge.shift[1])
                        in members
                    }
...
@pytest.mark.parametrize(
    "row,point",
    ((0, (0, 0)), (1, (4, 0)), (1, (6, -1)), (2, (0, 7)), (2, (4, 5)), (2, (5, 5))),
)
def test_complete_query_layers_match_independent_finite_state_oracle(
    memory, partition, row, point
):
    targets = _target(memory, row, point)
    layers, status = _query_oracle(partition, targets)
    query = owner._derive_return_safe_partition_region_queries(
        partition, (targets,), 10_000
    ).queries[0]
    ...
    assert tuple(step.zone_count for step in query.iterations) == tuple(
        sum(zone is not None for zone in layer) for layer in layers
    )
...
def test_public_owner_rebuilds_canonical_diffusion_and_rechecks_proposed_exclusion():
    ...
    def target(point):
        vector = tuple(
            value - start for value, start in zip(point, origin, strict=True)
        ) + (0,)
        return (
            C6CarriedForwardZone(
                row(point), tuple(tuple(a - b for b in vector) for a in vector)
            ),
        )
    ...
    state = NodalRemainderState(row(origin), (F(0),) * 6, 0.375, 0.625)
    result = owner.derive_c6_carried_return_safe_partition_region_exclusions(
        reference,
        state=state,
        epi_states=tuple(map(row, (origin, fixed, *chain))),
        timestep=2.0,
        transient_epi_states=(row(chain[1]),),
        excluded_target_region_groups=(target(fixed),),
    ...
    assert (
        result.partition.initialization_complete and result.partition.relation_complete
    )
    assert result.partition.excluded_target_queries[
        0
    ...
    assert all(
        not query.actual_origin_reachability_certified for query in result.queries
    )
