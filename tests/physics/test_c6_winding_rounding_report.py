...
@pytest.mark.parametrize("value", [True, None, [], "1/3", "NaN", "Infinity", "1e400", f"1/{2**1075}"])
def test_nonrepresented_scalar_inputs_are_rejected_before_replay(value):
    with pytest.raises((ValueError, TypeError)):
        campaign._represented_scalar(value)


@pytest.mark.parametrize("value,expected", [("1/2", .5), (0, 0.), (.05, .05), (str(F(.05)), .05)])
...
def test_readonly_analysis_uses_shared_primitive_without_running_a_word(parent, monkeypatch):
...
def test_every_captured_endpoint_matches_scalar_and_numpy_substeps(parent, report):
    ...
    for retained, analyzed in zip(parent["cases"], report["cases"], strict=True):
        ...
        for cycle, result in zip(retained["cycles"], analyzed["cycles"], strict=True):
            ...
            assert max(map(abs, observed["endpoint_defect"])) <= observed["local_endpoint_error_bound"]
        ...
        for cycle in case["cycles"]:
            for step in cycle["observation"]["substeps"]:
                for value, total, error, cell in zip(
                    step["after"], step["unrounded_sum"], step["addition_error"], step["result_cells"], strict=True,
                ):
                    predecessor = math.nextafter(value, -math.inf)
                    successor = math.nextafter(value, math.inf)
                    lower, upper = (F(value) + F(predecessor)) / 2, (F(value) + F(successor)) / 2
...
def test_pressure_plus_rounding_exactly_explains_all_six_mean_changes(parent, report):
    ...
    for retained, analyzed in zip(parent["cases"], report["cases"], strict=True):
        for cycle, result in zip(retained["cycles"], analyzed["cycles"], strict=True):
            before, after = cycle["flow"]["before"], cycle["flow"]["raw_after_integrator"]
            pressure_effect = sum(map(F, before["pressure"]), F(0)) / 24
            actual = sum((F(a) - F(b) for a, b in zip(after["epi"], before["epi"], strict=True)), F(0)) / 6
    ...
    assert actual_means == [F(0), F(0), F(1, 3 * 2**52), F(0), F(1, 2**53), -F(1, 3 * 2**53)]
...
def test_both_null_flows_have_half_epi_stasis_while_operator_phases_move(report):
    for cycle in report["cases"][0]["cycles"]:
        ...
        assert observation["endpoint"] == (.5,) * 6
...
def test_disagreement_between_independently_bound_endpoints_fails_closed(parent, monkeypatch, binding):
    if binding == "scalar_endpoint":
        observe = campaign.observe_binary64_unit_quarter_flow

        def changed(*args, **kwargs):
            result = observe(*args, **kwargs)
            return replace(result, endpoint=(.75,) * 6)

        monkeypatch.setattr(campaign, "observe_binary64_unit_quarter_flow", changed)
    elif binding == "numpy_endpoint":
        monkeypatch.setattr(campaign, "_numpy_replay", lambda *args: ((.75,) * 6,) * 4)
    else:
        analyze = campaign.analyze_c6_defect_report

        def changed(data):
            result = analyze(data)
            result["cases"][0]["cycles"][0]["pressure_budget"]["integrator_epi_effect"] = (F(1),) * 6
            return result
...
def test_cli_preserves_historical_manifest_under_a_separate_consumer_digest(source_bytes, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["audit", "--input", str(path), "--output", str(output)])
    campaign.main()
...
def test_publication_rejects_mutation_during_the_offline_audit(source_bytes, tmp_path, monkeypatch, changed):
    path, output = tmp_path / "input.json", tmp_path / "output.json"
    path.write_bytes(source_bytes)
    provenance = iter([("a" * 40, False, None), (("b" if changed == "consumer_source" else "a") * 40, False, None)])
    monkeypatch.setattr(campaign, "current_git_source_provenance", lambda *args: next(provenance))
    if changed == "input":
