...
def test_remote_cell_exit_does_not_end_the_selected_stencil_or_its_pressure():
    epi = (0.5, 0.5, 0.5, 0.5 + 2**-52, 0.5, 0.5 + 2**-52)
    # Node zero is just below its upper tie, while the selected center has no carry.
    initial = NodalRemainderState(
        epi, (F(1, 2**54) - F(1, 2**60),) + (F(0),) * 5, 0.375, 0.625
    )
    reference = _reference()
    observed = observe_c6_frozen_pressure_stencil(
        reference, state=initial, node=4, timestep=1 / 16
    )
    ...
    step = advance_nodal_remainder(
        initial, timestep=1 / 16, capacity=(1.0,) * 6, pressure=pressure
    )
    assert step.after.epi[0] != initial.epi[0]
    assert tuple(step.after.epi[i] for i in observed.stencil) == observed.stencil_epi
    refreshed = observe_c6_frozen_pressure_stencil(
        reference, state=step.after, node=4, timestep=1 / 16
    )
...
def test_a_changed_stencil_can_preserve_its_gradient_and_pressure_exactly():
    ...
    after = (0.5, 0.5, 0.5, float(F(1, 2) - delta), 0.5, float(F(1, 2) - 4 * delta))
    reference = _reference()
    observations = tuple(
        observe_c6_frozen_pressure_stencil(
            reference,
            state=initialize_nodal_remainder(epi, epi_lower=0.375, epi_upper=0.625),
            node=4,
            timestep=1 / 16,
        )
    ...
    assert not any(
        item.sign_hit_certified or item.graph_provenance_certified
...
def test_stationary_center_has_no_local_deadline_despite_a_finite_global_deadline():
    epi = (0.5, 0.5, 0.5, 0.5 - 2**-53, 0.5, 0.5 + 2**-53)
    state = initialize_nodal_remainder(epi, epi_lower=0.375, epi_upper=0.625)
    observed = observe_c6_frozen_pressure_stencil(
        _reference(), state=state, node=4, timestep=1 / 16
    )
    horizon = derive_nodal_remainder_cell_horizon(
        state=state,
        timestep=1 / 16,
    ...
    assert observed.pressure == 0.0 and observed.gradient_index == 0
    ...
    assert (
        observed.initial_max_frozen_steps is observed.initial_first_exit_bound is None
    )
    assert (
        observed.uniform_max_frozen_steps is observed.uniform_first_exit_bound is None
    )
...
def test_exact_band_exit_can_end_the_bound_before_any_center_display_change():
    reference = derive_c6_pressure_lattice(
        phase=(0.25, 0.0, 0.25, 0.0, 0.0, 0.0),
        epi_weight=0.5,
        phase_weight=0.5,
        epi_lower=0.375,
        epi_upper=0.625,
    )
    state = initialize_nodal_remainder((0.5,) * 6, epi_lower=0.375, epi_upper=0.5)
    timestep = 2.0**-60
    observed = observe_c6_frozen_pressure_stencil(
        reference, state=state, node=1, timestep=timestep
    )
    horizon = derive_nodal_remainder_cell_horizon(
        state=state,
        timestep=timestep,
    ...
    assert (
        observed.uniform_max_frozen_steps > 0
    )  # Other admissible carries begin below the band edge.
