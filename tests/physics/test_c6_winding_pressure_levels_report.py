...
def test_first_new_level_stops_four_boundaries_and_eleven_steps(report, parents):
    ...
    assert tuple(row["source_pressure"]["gradient_indices"][1] for row in rows) == (-1, 1, -1, 1)
    assert tuple(row["refreshed_pressure"]["gradient_indices"][1] for row in rows) == (1, -1, 1, -2)
    assert tuple(_offsets(row["endpoint"]) for row in rows) == (
        (-1, 0, 2, -2, 4, -3), (-1, 0, 0, -2, 4, -3),
        (-1, 0, 2, -2, 4, -3), (-2, 0, 0, -2, 4, -3),
    )
...
def test_only_the_two_old_levels_act_and_the_third_is_captured_without_evolution(report):
    ...
    assert report["continuation"]["refreshed_pressure"]["pressure"][1] == levels["captured_novel_level"]
...
def test_visible_revisits_keep_different_carries_and_different_next_exit_times(report):
    ...
    assert second["endpoint"] == third["initial"] and third["endpoint"] == fourth["initial"]


def test_all_six_local_and_B27_area_budgets_match_actual_reconstructed_states(report, parents):
    ...
    for step, row in zip(continuation["steps"], continuation["net_prefixes"], strict=True):
        ...
        assert row["identity_residual"] == row["display_carry_identity_residual"] == (0,) * 6
        for i in range(6):
            ...
            assert row["B27_visible_change"][i] + row["B27_remainder_change"][i] == net[i]
        previous = step["after"]
    ...
    assert tuple(value * 2**112 for value in continuation["local_nodal_area"]) == (
        -53012427250770350, 24049580203736861, 9296044417434,
        -16515588489643380, 146860452144865272, -101391312652605888,
    )
    assert tuple(value * 2**112 for value in continuation["B27_nodal_area"]) == (
        -122873677022523069, 19871535157506923, 124954297600791358,
        -187220008878510488, 562505776726237660, -397237923583502456,
    )
...
def test_actual_overshoot_is_not_an_exact_zero_or_mean_or_vector_return(report):
    ...
    for flag in ("node1_nodal_area_compensated", "mean_nodal_area_compensated", "total_nodal_area_compensated"):
        assert report[flag] is False


def test_new_level_preserves_the_necessary_return_modulus_without_execution_claim(report):
    ...
    assert new["integer_levels"] == (-1026366057766984, 5572432502556410, -4325765337928681)
    for result in (old, new):
        integers = result["integer_levels"]
        assert result["difference_gcd"] == reduce(gcd, (abs(z - integers[0]) for z in integers), 0)
        ...
        assert result["pressure_provenance_certified"] is result["periodic_execution_certified"] is False
...
def test_three_deduplicated_states_have_a_conditional_44_step_escape_bound(report):
    ...
    assert tuple(F(row[4]) for row in observation["pressure_vectors"]) == (pressure,) * 3
    assert observation["minimum_pressure_projection"] == pressure
    assert observation["width"] == F(1, 2**53)
    assert observation["max_confined_steps"] == observation["width"] // (pressure / 16) == 43
    assert observation["escape_step_bound"] == 44
    assert certificate["conditional_class_escape_certified"] is True
    assert certificate["pressure_provenance_certified"] is certificate["positive_band_exit_certified"] is False
    assert report["continuation"]["step_count"] == 11


@pytest.mark.parametrize("remote", ((.375, .375, .375), (.625, .625, .625), (.375, .5, .625)))
def test_generated_node_four_pressure_depends_only_on_its_fixed_local_epi_stencil(parents, report, remote):
    source = parents[3]["lattice_reference"]["source"]
    reference = campaign.derive_c6_pressure_lattice(
        phase=report["source"]["phase"], epi_weight=campaign._represented_scalar(source["epi_weight"]),
        phase_weight=campaign._represented_scalar(source["phase_weight"]), epi_lower=.375, epi_upper=.625,
    )
...
def test_gradient_and_inverse_itinerary_distinguish_inherited_carry_from_zero_reset(report):
    continuation = report["continuation"]
...
@pytest.mark.parametrize("boundaries,budget,completed,calls,stop", (
    (8, 1, 1, 1, "step_budget_exhausted"), (1, 256, 1, 1, "boundary_budget_exhausted"),
    (8, 5, 1, 1, "next_boundary_exceeds_step_budget"), (8, 6, 6, 2, "step_budget_exhausted"),
    (8, 10, 7, 3, "next_boundary_exceeds_step_budget"),
))
def test_resource_censoring_does_not_run_an_oversized_next_boundary(parents, monkeypatch, boundaries, budget, completed, calls, stop):
    ...
    result = campaign.analyze_c6_winding_pressure_levels(*parents, boundary_budget=boundaries, step_budget=budget)
...
@pytest.mark.parametrize("kwargs", ({"step_budget": 0}, {"step_budget": True}, {"step_budget": 257},
                                    {"boundary_budget": 9}, {"boundary_budget": 0}))
...
@pytest.mark.parametrize("change", ("claim", "scope", "carry", "area", "baseline", "pressure", "cached_period",
                                    "flag", "lineage", "source"))
...
def _cli(monkeypatch, paths, output):
    monkeypatch.setattr(sys, "argv", ["levels", "--input", str(paths[0]), "--parent-input", str(paths[1]),
                                      "--previous-input", str(paths[2]), "--source-input", str(paths[3]), "--output", str(output)])


def test_cli_retains_four_input_hashes_and_distinct_output_source_provenance(tmp_path, monkeypatch, report):
    ...
    monkeypatch.setattr(campaign, "analyze_c6_winding_pressure_levels", lambda *args: deepcopy(report))
    ...
    for key, source in zip(("input_evidence", "parent_input_evidence", "previous_input_evidence", "source_input_evidence"), INPUTS, strict=True):
...
def test_cli_rejects_wrong_historical_bytes_before_analysis(tmp_path, monkeypatch, changed_index):
    paths = tuple(tmp_path / f"source_{i}.json" for i in range(4))
