...
def _stages(retained, analyzed):
    yield retained["initial_capture"], retained["initial_joint_readout"]["phase_pi_represented"], analyzed["initial"]
    for cycle, result in zip(retained["cycles"], analyzed["cycles"], strict=True):
        for glyph in ("um", "il"):
            phase = tuple(F(value) / F(math.pi) for value in cycle[glyph]["phase_after"]["represented_lift"])
            for state, suffix in (("raw_capture", "raw"), ("after_capture", "refresh")):
                yield cycle[glyph][state], phase, result["stages"][f"{glyph}_{suffix}"]
        yield cycle["after_capture"], cycle["after_joint_readout"]["phase_pi_represented"], result["stages"]["flow_refresh"]
...
def test_all_33_stage_projections_match_primary_records(parent, report):
    ...
    for retained, analyzed in zip(parent["cases"], report["cases"], strict=True):
        for capture, phases, stage in _stages(retained, analyzed):
            ...
            assert stage["phase_lift_radians"] == tuple(value * F(math.pi) for value in z)
...
def test_all_pressure_pairs_split_exactly_into_five_recorded_terms(parent, report):
    ...
    for retained, analyzed in zip(parent["cases"], report["cases"], strict=True):
        for capture, phases, stage in _stages(retained, analyzed):
            ...
            sigma = tuple(gradient[i] + z[i] - (z[(i - 1) % 6] + z[(i + 1) % 6]) / 2 for i in range(6))
            we = F(capture["epi_weight"])
            wp = F(dict(capture["normalized_weights"])["phase"])
            terms = {
                "epi_pressure": tuple(-F(3, 2) * we * value for value in _centered_pairs(x)),
                "ideal_phase_pressure": tuple(-F(3, 2) * wp * value for value in _centered_pairs(z)),
                "phase_realization": tuple(wp * value for value in _pairs(sigma)),
                "pressure_assembly": _pairs(tuple(map(F, capture["kernel_pressure_defect"]))),
                "stored_pressure_residual": _pairs(tuple(map(F, capture["stored_pressure_residual"]))),
            }
            pressure_pairs = _pairs(tuple(map(F, capture["snapshot"]["stored_pressure"])))
            ...
            assert pressure_pairs == tuple(sum((term[i] for term in terms.values()), F(0)) for i in range(3))
...
def test_first_um_loses_centered_pairing_not_only_common_rotation(report):
    expected_radians = (
        (F(2001599834386887, 2**107), -F(2001599834386887, 2**108), -F(2001599834386887, 2**108)),
...
def test_quotient_residuals_remove_mean_and_compose_without_dropping_rounding(parent, report):
    for retained, case in zip(parent["cases"], report["cases"], strict=True):
        ...
        for cycle in case["cycles"]:
            ...
            assert q["um_residual"] == tuple(a - fu * b for a, b in zip(q["after_um"], previous, strict=True))
            assert q["il_residual"] == tuple(a - fi * b for a, b in zip(q["after_il"], q["after_um"], strict=True))
            assert q["combined_residual"] == tuple(a - fu * fi * b for a, b in zip(q["after_il"], previous, strict=True))
            assert q["combined_residual"] == tuple(fi * a + b for a, b in zip(q["um_residual"], q["il_residual"], strict=True))
...
def test_retained_k3_pressure_pairing_is_lost_specifically_at_il_refresh(report):
    ...
    assert cycle["stages"]["il_refresh"]["phase_gradient_pairs"] == (-F(5215, 2**65), 0, 0)
    assert cycle["stages"]["il_raw"]["phase_centered_pairs_pi"] == cycle["stages"]["il_refresh"]["phase_centered_pairs_pi"]
...
def test_inherited_flow_bindings_keep_the_mean_and_binade_obstructions(report):
    for case in report["cases"]:
        for cycle in case["cycles"]:
            ...
            assert binding["starts_straddling_half_binade"] is (case["mode"] != "null" and cycle["ordinal"] == 2)


def test_corrupted_stage_is_rejected_by_inherited_validation_before_projection(parent, monkeypatch):
...
def test_cli_separates_historical_input_from_own_source_provenance(source_bytes, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["pairing", "--input", str(source), "--output", str(output)])
    campaign.main()
...
def test_cli_refuses_unsafe_or_changed_publication(source_bytes, tmp_path, monkeypatch, change):
    source, output = tmp_path / "input.json", tmp_path / "output.json"
    source.write_bytes(source_bytes)
    if change == "overwrite":
        output = source
    versions = iter([("c" * 40, False, None), (("d" if change == "source" else "c") * 40, False, None)])
    monkeypatch.setattr(campaign, "current_git_source_provenance", lambda *args: next(versions))
    if change == "input":
