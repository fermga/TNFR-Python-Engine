...
def observe(**changes):
    arguments = dict(
        epi_states=((0.5,),),
        pressure_vectors=((2.0**-56,),),
        functional=(Fraction(1),),
        timestep=1.0,
    )
...
def test_class_bound_covers_all_carry_not_only_zero(sign):
    result = observe(
        pressure_vectors=((sign * 2.0**-56,),), functional=(Fraction(sign),)
    )
    ...
    for _ in range(6):
        state = advance_nodal_remainder(
            ...
            timestep=1.0,
            capacity=(1.0,),
            pressure=(sign * 2.0**-56,),
        ).after
        assert state.epi == (0.5,)
    state = advance_nodal_remainder(
        state,
        timestep=1.0,
        capacity=(1.0,),
...
def test_zero_gap_is_not_strict_separation():
    result = observe(epi_states=((0.5,), (0.6,)), pressure_vectors=((0.1,), (0.0,)))
    assert result.minimum_pressure_projection == 0
    assert not result.conditional_class_escape_certified
...
def test_cell_envelopes_respect_global_band_and_functional_signs():
    result = observe(
        epi_states=((0.05, 1.0),),
        pressure_vectors=((1.0, -1.0),),
        functional=(Fraction(2), Fraction(-3)),
    )
    assert result.class_lower == 2 * Fraction(0.05) - 3
