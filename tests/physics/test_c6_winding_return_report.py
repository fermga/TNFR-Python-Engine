...
def previous_modes(return_inputs):
    ...
    return campaign.analyze_c6_winding_mode_excursions(
        json.loads(return_inputs[0]), **kwargs
    )
...
def return_report(return_inputs, previous_modes):
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(
            ...
            lambda *_a, **_k: deepcopy(previous_modes),
        )
        return campaign.analyze_c6_winding_return_regions(
            json.loads(return_inputs[0]), **arguments(return_inputs)
        )
...
def test_full_return_report_excludes_only_two_additional_complete_slabs(return_report):
    ...
    assert result["B54_additional_first_exit_facets"] == (
        dict(mask=4, node=0, direction="lower"),
        dict(mask=12, node=0, direction="lower"),
    )
    ...
    positive = [
        r["query"]
        for r in result["B54_region_queries"]
        if r["query"]["origin_path_within_domain_excluded"]
    ]
    assert len(positive) == 2
    assert all(
        q["status"] == "empty_complete_layer" and q["completed_depth"] == 7
        for q in positive
    )
...
def test_return_audit_rejects_any_changed_historical_input(return_inputs, index):
    ...
    with pytest.raises(ValueError, match="lineage"):
        campaign.analyze_c6_winding_return_regions(
            json.loads(changed[0]), **arguments(changed)
        )


@pytest.mark.parametrize("mutation", ("result", "origin", "domain", "claim"))
def test_return_audit_rebuilds_stored_mode_claims(
    monkeypatch, return_inputs, previous_modes, mutation
):
    ...
    if mutation == "result":
        ...
        old["B53_post_exclusion_reachable_envelope"]["retained_zones"][0]["bounds"][0][
            6
        ] += 1
    else:
...
def test_return_resource_guard_does_not_promote_exclusion(
    monkeypatch, return_inputs, previous_modes, limit
):
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_mode_excursions",
    ...
    result = campaign.analyze_c6_winding_return_regions(
        json.loads(return_inputs[0]),
        **arguments(return_inputs),
        **{limit: 1},
    )
...
def test_return_cli_binds_eight_inputs(
    tmp_path, monkeypatch, return_inputs, return_report
):
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: deepcopy(return_report),
    )
    monkeypatch.setattr(
        campaign,
    ...
    assert [r["sha256"] for r in result["input_evidence"]] == [
        hashlib.sha256(raw).hexdigest() for raw in return_inputs
    ]
    assert (
        result["excluded_first_exit_slabs"] == 38
...
def test_return_cli_rejects_changed_input_or_source(
    tmp_path, monkeypatch, return_inputs, return_report, which
):
    paths = tuple(tmp_path / f"input{i}.json" for i in range(8))
    for path, raw in zip(paths, return_inputs, strict=True):
        path.write_bytes(raw)
