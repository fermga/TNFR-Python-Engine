...
def _case(origin_shift=F(0)):
    ...
    origin = NodalRemainderState(
        _epi((-3, 0, 2, 0, 8, -5)), tuple(value + origin_shift for value in carry), .375, .625,
    )
    template_carry = (COMMON_CARRY + origin_shift,) * 6
    positive = NodalRemainderState(_epi(POSITIVE_OFFSETS), template_carry, .375, .625)
    negative = NodalRemainderState(_epi(NEGATIVE_OFFSETS), template_carry, .375, .625)
    lattice = derive_c6_pressure_lattice(
        phase=PHASE, epi_weight=CHANNEL_WEIGHT_SECONDARY, phase_weight=CHANNEL_WEIGHT_PRIMARY,
    )
    closure = derive_c6_carried_closure(derive_c6_carried_profile(lattice), state=origin, timestep=.0625)
...
def obstruction(case):
    ...
    return derive_c6_carried_affine_mean_obstruction(closure, positive_state=positive, negative_state=negative)
...
def test_actual_affine_spacing_and_energy_bounds_are_exact(obstruction, case):
    ...
    assert obstruction.coordinate_spacings == tuple(value * M for value in (1, 1, 2, 4, 8, 4))
    ...
    assert obstruction.positive_energy_bound == 2 * base.positive_point.energy + 924 * M**2
    assert obstruction.negative_energy_bound == 2 * base.negative_point.energy + 924 * M**2
    assert max(obstruction.positive_energy_bound, obstruction.negative_energy_bound) < closure.energy_bound
    assert obstruction.mean_lower == base.mean_lower + 8 * M
    assert obstruction.mean_upper == base.mean_upper - 19 * M
    assert obstruction.mean_lower < _mean(closure.base_tube.state) < obstruction.mean_upper
...
def test_each_gcd_is_verified_by_canonical_pressure_and_independent_bezout(obstruction):
    ...
    base = F(1, 2) - 64 * DELTA
    values = []
    for node, (lo, hi, spacing) in enumerate(zip(
            closure.gradient_index_lower, closure.gradient_index_upper, obstruction.coordinate_spacings)):
        entries = {}
    ...
    for entries, spacing, (first, second, a, b) in zip(values, obstruction.coordinate_spacings, witnesses):
...
def _assert_boundary(bound, point, boundary, lower, upper, direction):
    ...
    errors = tuple(value - boundary.mean_translation for value in boundary.carry_adjustment)
    ...
    for index, (adjustment, spacing) in enumerate(zip(errors, bound.coordinate_spacings)):
        if index != bound.pivot:
            assert -spacing < adjustment <= 0
    for x, residue, spacing in zip(exact, bound.coordinate_residues, bound.coordinate_spacings):
    ...
    assert boundary.exact_increment == tuple(F(p) / 16 for p in point.observation.pressure)
    assert boundary.exact_candidate == tuple(x + a for x, a in zip(exact, boundary.exact_increment))
    for x, residue, spacing in zip(boundary.exact_candidate, bound.coordinate_residues, bound.coordinate_spacings):
        assert ((x - residue) / spacing).denominator == 1
    if direction == "upper":
        assert 0 <= upper - boundary.mean_before < bound.mean_quantum
        assert boundary.mean_after > upper and boundary.energy <= bound.positive_energy_bound
    else:
        assert 0 <= boundary.mean_before - lower < bound.mean_quantum
        assert boundary.mean_after < lower and boundary.energy <= bound.negative_energy_bound
    if not boundary.band_failure:
        assert boundary.step.before == boundary.state
        assert _validate_nodal_remainder_state(boundary.step.after) == boundary.exact_candidate
...
def test_all_48_lift_residue_classes_for_both_pressure_signs(obstruction):
    ...
    for index in range(48):
        ...
        result = observe_c6_carried_affine_mean_escape(obstruction, mean_lower=lower, mean_upper=upper)
        _assert_boundary(obstruction, base.positive_point, result.upper_boundary, lower, upper, "upper")
        _assert_boundary(obstruction, base.negative_point, result.lower_boundary, lower, upper, "lower")
        assert result.upper_boundary.mean_grid_index == index
        assert result.lower_boundary.mean_grid_index == -index
        adjustments.append(tuple(value - result.upper_boundary.mean_translation
                                 for value in result.upper_boundary.carry_adjustment))
    assert len(set(adjustments)) == 48
    period = observe_c6_carried_affine_mean_escape(obstruction, mean_lower=mean, mean_upper=mean + 48 * quantum)
    assert tuple(value - period.upper_boundary.mean_translation
                 for value in period.upper_boundary.carry_adjustment) == adjustments[0]
...
def test_nondyadic_interval_endpoints_keep_strict_crossings(obstruction):
    ...
    _assert_boundary(obstruction, base.positive_point, result.upper_boundary, lower, upper, "upper")
    _assert_boundary(obstruction, base.negative_point, result.lower_boundary, lower, upper, "lower")
    assert result.upper_boundary.mean_grid_index == 3 and result.lower_boundary.mean_grid_index == -2
...
def test_shrunk_window_endpoints_remain_legal_after_nonuniform_lifting(obstruction):
    result = observe_c6_carried_affine_mean_escape(
        obstruction, mean_lower=obstruction.mean_lower, mean_upper=obstruction.mean_upper,
    )
    base = obstruction.base_obstruction
    _assert_boundary(obstruction, base.positive_point, result.upper_boundary, result.mean_lower, result.mean_upper, "upper")
    _assert_boundary(obstruction, base.negative_point, result.lower_boundary, result.mean_lower, result.mean_upper, "lower")


def test_nonzero_origin_residue_is_carried_through_both_witnesses():
    closure, positive, negative = _case(M / 2)
    bound = derive_c6_carried_affine_mean_obstruction(closure, positive_state=positive, negative_state=negative)
    assert bound.coordinate_residues == (M / 2,) * 6
    mean = _mean(closure.base_tube.state)
    result = observe_c6_carried_affine_mean_escape(bound, mean_lower=mean, mean_upper=mean)
    _assert_boundary(bound, bound.base_obstruction.positive_point, result.upper_boundary, mean, mean, "upper")
    _assert_boundary(bound, bound.base_obstruction.negative_point, result.lower_boundary, mean, mean, "lower")
...
def test_nonuniform_template_carries_are_outside_the_declared_lift_contract(case):
    ...
    def skew(state):
        return replace(state, remainder=(state.remainder[0] - M / 4,
                                         state.remainder[1] + M / 4) + state.remainder[2:])

    with pytest.raises(ValueError, match="uniform carry"):
        derive_c6_carried_affine_mean_obstruction(
            closure, positive_state=skew(positive), negative_state=skew(negative),
        )
...
def test_gradient_budget_is_a_complete_enumeration_limit_not_partial_evidence(case):
    ...
    with pytest.raises(ValueError):
        derive_c6_carried_affine_mean_obstruction(
            closure, positive_state=positive, negative_state=negative, max_gradient_values=335,
        )
    exact = derive_c6_carried_affine_mean_obstruction(
        closure, positive_state=positive, negative_state=negative, max_gradient_values=336,
    )
    assert exact.gradient_value_count == exact.max_gradient_values == 336


@pytest.mark.parametrize("budget", (True, 0, -1, 4096., "4096"))
def test_gradient_budget_type_and_positivity_are_checked(case, budget):
    closure, positive, negative = case
    with pytest.raises((TypeError, ValueError)):
        derive_c6_carried_affine_mean_obstruction(
            closure, positive_state=positive, negative_state=negative, max_gradient_values=budget,
        )


def test_forged_derived_fields_do_not_change_the_observed_certificate(obstruction):
    forged = replace(obstruction, coordinate_spacings=(F(1),) * 6, coordinate_residues=(F(1),) * 6,
                     global_spacing=F(1), mean_quantum=F(1), gradient_value_count=0,
                     carry_lower_slack=F(0), carry_upper_slack=F(0), lift_error_squared_bound=F(0),
                     positive_energy_bound=F(0), negative_energy_bound=F(0), mean_lower=F(0), mean_upper=F(1))
    mean = _mean(obstruction.base_obstruction.closure.base_tube.state)
    expected = observe_c6_carried_affine_mean_escape(obstruction, mean_lower=mean, mean_upper=mean)
    observed = observe_c6_carried_affine_mean_escape(forged, mean_lower=mean, mean_upper=mean)
    assert observed == expected


def test_changed_primitive_origin_mean_is_not_silently_projected(case):
    closure, positive, negative = case
    changed = replace(positive, remainder=(positive.remainder[0] + M,) + positive.remainder[1:])
    with pytest.raises(ValueError):
        derive_c6_carried_affine_mean_obstruction(closure, positive_state=changed, negative_state=negative)


@pytest.mark.parametrize("value", (.5, 0, True, "1/2"))
def test_mean_interval_requires_exact_fraction_endpoints(obstruction, value):
    with pytest.raises((TypeError, ValueError)):
        observe_c6_carried_affine_mean_escape(
            obstruction, mean_lower=value, mean_upper=_mean(obstruction.base_obstruction.closure.base_tube.state),
        )


def test_intervals_cannot_exceed_shrunk_window_or_miss_origin(obstruction):
    mean, quantum = _mean(obstruction.base_obstruction.closure.base_tube.state), obstruction.mean_quantum
    for lower, upper in ((obstruction.mean_lower - quantum, mean), (mean, obstruction.mean_upper + quantum),
                         (mean + quantum, mean + 2 * quantum), (mean + quantum, mean)):
        with pytest.raises(ValueError):
            observe_c6_carried_affine_mean_escape(obstruction, mean_lower=lower, mean_upper=upper)
