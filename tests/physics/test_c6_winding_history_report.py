...
def report(inputs, previous):
    with pytest.MonkeyPatch.context() as patch:
        ...
        return campaign.analyze_c6_winding_return_histories(
            json.loads(inputs[0]), **arguments(inputs)
        )
...
def test_history_report_distinguishes_counts_from_jointly_admissible_words(report):
    ...
    assert (
        not counts["joint_guard_satisfaction_certified"]
        and not counts["actual_origin_reachability_certified"]
    )
    assert len(report["B56_target_count_witnesses"]) == 17
    assert all(
        item["count_witness"]["minimum_edge_count"] > 0
        for item in report["B56_target_count_witnesses"]
    )
...
def test_history_rejects_altered_lineage(inputs, index):
    ...
    with pytest.raises(ValueError, match="lineage"):
        campaign.analyze_c6_winding_return_histories(
...
def test_history_reconstructs_parent_claims(monkeypatch, inputs, previous, mutation):
    ...
    if mutation == "claim":
        ...
        parent["B55_union_queries"][0]["query"]["target_regions"][0]["bounds"][0][
            6
        ] += 1
...
def test_history_checks_untrusted_integer_and_word_proposals(
    ...
    if mutation == "relation":
        ...
        proposal["coordinate_generators"][0].append(
            proposal["coordinate_generators"][0][0]
        )
    elif mutation == "endpoint":
        proposal["remaining_target_proposals"][0]["endpoint_grid_coordinates"][0] = (
            10**100
        )
        ...
        next(
            p
            for p in proposal["remaining_target_proposals"]
            if p["terminal_transient_edge_index"] is not None
        )["terminal_transient_edge_index"] = -1
    else:
...
@pytest.mark.slow
def test_word_resource_stop_cannot_promote_a_repeat_bound(
    ...
    result = campaign.analyze_c6_winding_return_histories(
        json.loads(inputs[0]), **arguments(inputs), max_word_work_items=1
    )
    assert all(
        item["proof"]["status"] == "word_resource_limit"
        for item in result["B56_word_budgets"]
    )
    assert all(
        item["proof"]["maximum_repetitions"] is None
        for item in result["B56_word_budgets"]
    )
    assert (
...
@pytest.mark.slow
def test_history_cli_binds_eleven_input_artifacts(
...
def test_history_cli_detects_midflight_input_changes(
