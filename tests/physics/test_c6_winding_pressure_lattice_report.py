...
INPUT = (
    ...
    / "artifacts/research/c6_winding_phase_orbit.json"
)
...
def test_complete_phase_orbit_is_replayed_and_the_static_stencil_is_fixed(report):
    ...
    assert points[0]["name"] == "half" and points[0]["observation"]["epi"] == (0.5,) * 6
    for node in range(6):
        for offset, name, direction in ((1, "down", -math.inf), (2, "up", math.inf)):
            ...
            expected = tuple(
                math.nextafter(0.5, direction) if i == node else 0.5 for i in range(6)
            )
...
def test_actual_pressure_sums_and_integer_cycle_compatibility(report):
    ...
    for point, expected_sum in zip(report["stencil"], expected_sums, strict=True):
        ...
        assert item["mean_pressure"] == (
            item["mean_phase_contribution"]
            + item["mean_epi_reduction_error"]
            + item["mean_assembly_error"]
        )
        assert item["mean_identity_residual"] == 0
        assert item["assembly_error"] == tuple(
            F(p) - a - F(g)
            for p, a, g in zip(
                item["pressure"], source, item["epi_contributions"], strict=True
            )
        )
        assert all(
            item[name]
            for name in (
                "scalar_reducer_agreement",
                "vector_reducer_agreement",
                "fused_pressure_agreement",
            )
        )
...
def test_lattice_sign_constraints_exclude_cartesian_corners_not_correlated_sets(report):
    ...
    assert (reference["nonnegative_min_sum"], reference["nonpositive_max_sum"]) == (
        2,
        -4,
    )
    assert tuple(
        (row["nonnegative_min_index"], row["nonpositive_max_index"])
    ...
    assert (
        reference["every_pressure_has_positive"]
        and reference["every_pressure_has_negative"]
    )
...
def test_canonical_compensation_is_exact_until_the_first_cell_exit_then_is_lost(report):
    ...
    assert (
        balanced["mean_assembly_error"]
        == -balanced["mean_phase_contribution"]
        == F(1, 6 * 2**109)
    )
    ...
    for prefix in boundary["shared_prefix_balances"]:
        assert (
            prefix["mean_nodal_area"]
            == prefix["mean_reconstructed_change"]
            == prefix["mean_identity_residual"]
            == 0
        )
...
def test_exact_separating_functional_bounds_stencil_residence_without_changing_pressure(
    report,
):
    ...
    projections = tuple(
        sum(
            (
                a * F(p)
                for a, p in zip(
                    functional, point["observation"]["pressure"], strict=True
                )
            ),
            F(0),
        )
...
def test_actual_pressure_evaluated_once_per_point_with_no_new_word_or_phase_search(
    parent, monkeypatch
):
    ...
    def forbidden(*args, **kwargs):
        raise AssertionError(
            "the local audit must not discover another orbit or execute a graph word"
        )
...
def test_parent_phase_or_source_corruption_fails_closed(parent, change):
    ...
    if change == "claim":
        ...
        changed["phase_orbit"]["phase_states"][0][0] = 0.125
    elif change == "transition":
        changed["phase_orbit"]["phase_states"][17][0] = 0.125
    elif change == "margin":
        changed["phase_orbit"]["steps"][1]["edge_margins"][0][0] = 0.125
    elif change == "method":
