...
def _source():
    return initialize_nodal_remainder((0.5,))
...
def test_first_exit_matches_independent_shared_kernel_steps_and_all_prefixes():
    ...
    first = advance_nodal_remainder(
        _source(), timestep=0.0625, capacity=(1.0,), pressure=(2.0**-50,)
    )
    second = advance_nodal_remainder(
        first.after, timestep=0.0625, capacity=(1.0,), pressure=(2.0**-50,)
    )
    assert horizon.max_unchanged_steps == 1 and horizon.first_exit_step == 2
    assert sequence.steps == (first, second)
    assert sequence.steps[0].after.epi == (0.5,)
    assert sequence.endpoint.epi == (math.nextafter(0.5, math.inf),)
...
def test_nonzero_inherited_carry_changes_the_exit_without_resetting_it():
    state = NodalRemainderState((0.5,), (F(1, 2**54),), 0.05, 1.0)
    ...
    assert observed.sequence.endpoint.epi == (math.nextafter(0.5, math.inf),)
    with pytest.raises(ValueError, match="step_budget"):
        _observe(state=_source(), step_budget=1)
    assert state.exact_epi == (F(0.5) + F(1, 2**54),)


def test_one_step_exit_with_zero_unchanged_prefix_is_supported():
    result = _observe(pressure=(0.25,), step_budget=1)
    ...
    assert result.sequence.endpoint.epi == (0.515625,)
...
def test_exact_budget_is_consumed_by_one_shared_sequence_call(monkeypatch):
    ...
    assert calls[0]["timesteps"] == (0.0625, 0.0625)
    assert calls[0]["capacities"] == ((1.0,), (1.0,))
    assert calls[0]["pressures"] == ((2.0**-50,),) * 2
...
def test_budget_rejection_precedes_even_an_astronomical_input_allocation(monkeypatch):
    ...
    with pytest.raises(ValueError, match="step_budget"):
        _observe(
            timestep=minimum, capacity=(minimum,), pressure=(minimum,), step_budget=256
        )
    assert not calls


@pytest.mark.parametrize(
    "changes", ({"timestep": 0.0}, {"capacity": (0.0,)}, {"pressure": (0.0,)})
)
...
def test_hidden_exact_band_exit_is_rejected_before_replay(monkeypatch):
    calls = _forbid_replay(monkeypatch)
    state = initialize_nodal_remainder((1.0,))
...
def test_budget_must_be_an_actual_positive_integer_before_any_replay(
    budget, error, monkeypatch
):
    calls = _forbid_replay(monkeypatch)
...
@pytest.mark.parametrize(
    "fault",
    ("before_visible", "intermediate_visible", "last_before_exact", "final_exact"),
)
def test_horizon_binding_rejects_an_inconsistent_returned_prefix(fault, monkeypatch):
    ...
    changed = initialize_nodal_remainder((0.6,))
    ...
    monkeypatch.setattr(
        owner, "observe_nodal_remainder_sequence", lambda **kwargs: forged
    )
