...
def cases():
    return {index: campaign.run_binary64_capacity_case(index) for index in campaign.INDICES}
...
def test_one_finite_admitted_word_and_flow_per_declared_preparation(cases):
    ...
    for index, record in cases.items():
        ...
        assert record["initial_capture"]["snapshot"]["capacity"] == (1 + F(index, 2**52), 1)
...
def test_actual_default_kernel_endpoints_match_independent_lattice_boundaries(cases):
    ...
    for index, record in cases.items():
        ...
        assert observation["termination_horizon_from_before"] == (0 if index <= 6 else 512)
        assert record["event"]["resolved_factors"]["UM_vf_sync"] == gamma
        assert record["post_um_capture"]["snapshot"]["capacity"] == (1 + F(expected[index], 2**52), 1)
...
def test_actual_family_and_executor_inputs_are_retained(cases):
    for record in cases.values():
        ...
        assert record["post_um_capture"]["snapshot"]["capacity"] == record["final_capture"]["snapshot"]["capacity"]
...
def test_uniform_represented_epi_coexists_with_nonzero_frozen_profile(cases, index):
    ...
    for modeled, pressure, execution in zip(
        record["exact_model_epi_change"], record["pressure_realization_epi_effect"],
        record["execution_epi_effect"], strict=True,
    ):
        assert modeled + pressure + execution == 0


def test_zero_gap_control_and_upper_endpoint_do_not_get_classified_as_the_same_stall(cases):
...
def test_exact_pressure_execution_budget_closes_at_every_actual_endpoint(cases):
    for record in cases.values():
        ...
        for actual, modeled, pressure, execution in zip(
            record["represented_epi_change"], record["exact_model_epi_change"],
            record["pressure_realization_epi_effect"], record["execution_epi_effect"], strict=True,
        ):
            assert actual == modeled + pressure + execution
        k = dict(record["post_um_capture"]["normalized_weights"])["vf"] / record["post_um_capture"]["epi_weight"]
        gap = record["post_um_capture"]["snapshot"]["capacity"][0] - 1
        assert record["current_conditional_profile_mismatch"]["error_variance"] == k**2 * gap**2 / 4
...
def test_closure_is_recorded_after_measurement_without_full_state_stasis(cases):
    for record in cases.values():
        ...
        assert closure["after_refresh"]["state"]["epi"] == record["final_before_closure"]["state"]["epi"]
...
def test_remesh_and_capacity_use_the_same_platform_probe():
    assert remesh._runtime_uses_required_binary64_rounding_model is _binary64.uses_ieee_binary64_rounding
