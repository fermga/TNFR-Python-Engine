...
def _return(point):
    ...
    return (
        _image(first)
        if first in TRANSIENT and _image(first) in DOMAIN
        else (None if first in TRANSIENT else first)
    )
...
def _point(result, guard):
    low, high = tuple(-guard[6][i] for i in range(6)), tuple(
        guard[i][6] for i in range(6)
    )
    assert low == high
    exact = tuple(
        a + result.grid_quantum * k
        for a, k in zip(result.affine_origin, low, strict=True)
    )
...
def test_exact_return_relation_matches_an_independent_729_state_map(result):
    actual = {
        (_point(result, edge.source_guard), _point(result, edge.target_guard))
...
def test_guards_retain_every_complete_intermediate_constraint_and_translation(result):
    two_step = 0
    for edge in result.return_relation:
        source, target = _point(result, edge.source_guard), _point(
            result, edge.target_guard
        )
        ...
        if edge.intermediate_epi is not None:
            ...
            assert (
                middle in TRANSIENT
                and middle == _image(source)
                and target == _image(middle)
            )
...
def test_complete_return_layers_match_the_finite_oracle_and_cover_the_origin_path(
    result,
):
    ...
    assert (
        tuple(
            (layer.base_zone_count, layer.transient_zone_count)
            for layer in result.iterations
        )
        == counts
    )
    assert tuple(layer.ordinal for layer in result.iterations) == tuple(
        range(len(layers))
    )
    ...
    while point in DOMAIN and point not in visited:
        ...
        point = _image(point)
    assert (
        result.status == "fixed_point" and result.domain_confined_origin_paths_covered
    )
    assert result.clipped_return_inclusion_certified


def test_nonreturning_transient_images_are_kept_even_when_the_next_step_exits(
    reference,
):
    ...
    assert any(
        _point(result, e.source_guard) == NO_RETURN_ORIGIN
        for e in result.intermediate_transitions
    )
    assert all(
        _point(result, e.source_guard) != NO_RETURN_ORIGIN
        for e in result.return_relation
    )
    pressure = result.pressures[ROWS.index(result.state.epi)]
    step = advance_nodal_remainder(
        result.state, timestep=2.0, capacity=(1.0,) * 6, pressure=pressure
    )
...
def test_interrupted_ordinary_construction_retains_the_entire_validated_domain(
    reference,
):
    limited = _derive(reference, max_intersections=1)
    assert (
        limited.status == "construction_resource_limit"
        and not limited.relation_complete
    )
    assert (
        not limited.transient_isolation_certified
        and not limited.clipped_return_inclusion_certified
    )
    ...
    assert (
        limited.closure_intersections == 0
        and limited.domain_confined_origin_paths_covered
    )


def test_interrupted_two_step_construction_never_publishes_an_incomplete_relation(
    reference,
):
    limited = _derive(reference, max_intersections=len(ROWS) ** 2)
    assert (
        limited.status == "construction_resource_limit"
        and limited.transient_isolation_certified
    )
    assert not limited.relation_complete and limited.return_relation == ()
    assert (
        limited.retained_zones == limited.domain_zones and len(limited.iterations) == 1
    )
    assert (
        limited.intersections == limited.max_intersections
        and limited.closure_intersections == 0
    )


def test_partial_intermediate_reconstruction_discards_the_entire_layer(
    reference, result
):
    first_layer_work = len(result.return_relation) + len(
        result.intermediate_transitions
    )
    limited = _derive(
        reference,
        max_intersections=result.construction_intersections + first_layer_work - 1,
    )
...
def test_fixed_point_requires_a_complete_layer_including_all_intermediates(
    reference, result
):
    limited = _derive(reference, max_intersections=result.intersections - 1)
    assert (
        limited.status == "resource_limit"
        and len(limited.iterations) == len(result.iterations) - 1
    )
...
def test_stationary_transient_row_is_rejected_once_its_self_edge_is_discovered(
    reference,
):
...
def test_empty_transient_selection_reduces_to_an_ordinary_conditional_envelope(
    reference,
):
    state = NodalRemainderState(
        (0.5,) * 6, tuple(F(i, 2**3222) for i in range(6)), 0.375, 0.625
    )
    result = _derive(
        reference,
        state=state,
    ...
    assert (
        result.status == "fixed_point" and result.return_relation[0].shift == (0,) * 6
    )
    assert result.grid_quantum == F(1, 2**3222) and len(set(result.affine_origin)) == 6
    assert (
        result.retained_zones == result.domain_zones
        and result.intermediate_transitions == ()
    )


def test_forged_reference_caches_are_rebuilt_from_the_canonical_source(
    reference, result
):
    forged = replace(
        reference,
        ...
        rows=(),
        epi_quantum=F(7),
        gradient_quantum=F(19),
    )
    rebuilt = _derive(forged)
    assert rebuilt.reference == reference and rebuilt.pressures == result.pressures
    assert (
        rebuilt.return_relation == result.return_relation
        and rebuilt.retained_zones == result.retained_zones
    )
...
@pytest.mark.parametrize(
    "tamper", ("missing_origin", "unclosed", "outside_rn", "carry", "phase")
)
def test_invalid_domain_or_nodal_primitive_is_rejected(reference, result, tamper):
    ...
    if tamper == "missing_origin":
        args["domain_zones"] = tuple(
            z for z in result.domain_zones if z.epi != result.state.epi
        )
        ...
        args["domain_zones"] = (
            C6CarriedForwardZone(zone.epi, tuple(map(tuple, bounds))),
        ) + result.domain_zones[1:]
    elif tamper == "carry":
