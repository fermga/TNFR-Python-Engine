...
def arguments(raws):
    return dict(zip(("parent_bytes", "relay_bytes", "historical_bytes", "envelope_bytes", "region_bytes",
                     "excursion_bytes", "candidates_bytes", "mode_bytes", "return_bytes"), raws, strict=True))
...
def previous_returns(union_inputs):
    ...
    return campaign.analyze_c6_winding_return_regions(json.loads(union_inputs[0]), **kwargs)


@pytest.fixture(scope="module")
def union_report(union_inputs, previous_returns):
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(campaign, "analyze_c6_winding_return_regions", lambda *_a, **_k: deepcopy(previous_returns))
        return campaign.analyze_c6_winding_return_unions(json.loads(union_inputs[0]), **arguments(union_inputs))


def test_exact_union_closes_mask_zero_without_shrinking_original_target(union_report, previous_returns):
    result = union_report
    ...
    assert result["B55_additional_first_exit_facets"] == (dict(mask=0, node=0, direction="lower"),)
    original = {tuple(item["target"].values()): item["query"]["target_regions"]
                for item in previous_returns["B54_region_queries"]}
    for item in result["B55_union_queries"]:
        assert item["query"]["target_regions"] == original[tuple(item["target"].values())]
    positives = [r["query"] for r in result["B55_union_queries"] if r["query"]["origin_path_within_domain_excluded"]]
    assert len(positives) == 1
    positive = positives[0]
    assert positive["status"] == "empty_complete_layer" and positive["completed_depth"] == 4
...
def test_union_audit_rejects_any_changed_historical_input(union_inputs, index):
    ...
    with pytest.raises(ValueError, match="lineage"):
        campaign.analyze_c6_winding_return_unions(json.loads(changed[0]), **arguments(changed))


@pytest.mark.parametrize("mutation", ("result", "origin", "guard", "target", "claim"))
def test_union_audit_rebuilds_stored_return_claims(monkeypatch, union_inputs, previous_returns, mutation):
    ...
    monkeypatch.setattr(campaign, "analyze_c6_winding_return_regions", lambda *_a, **_k: deepcopy(previous_returns))
    with pytest.raises(ValueError, match="lineage|reconstruction"):
        campaign.analyze_c6_winding_return_unions(json.loads(changed[0]), **arguments(changed))


@pytest.mark.parametrize("limit", ("max_intersections", "query_max_intersections", "query_max_subsumptions", "query_max_zones"))
def test_union_resource_guards_do_not_promote_exclusion(monkeypatch, union_inputs, previous_returns, limit):
    monkeypatch.setattr(campaign, "analyze_c6_winding_return_regions", lambda *_a, **_k: deepcopy(previous_returns))
    result = campaign.analyze_c6_winding_return_unions(
        json.loads(union_inputs[0]), **arguments(union_inputs), **{limit: 1},
    )
...
def cli_args(monkeypatch, paths, output):
    ...
    argv = ["c6_winding_invariant_region.py", "--method", "unions", "--output", str(output)]
...
def test_union_cli_binds_nine_inputs(tmp_path, monkeypatch, union_inputs, union_report):
    ...
    monkeypatch.setattr(campaign, "analyze_c6_winding_return_unions", lambda *_a, **_k: deepcopy(union_report))
    ...
    assert [r["sha256"] for r in result["input_evidence"]] == [hashlib.sha256(raw).hexdigest() for raw in union_inputs]
...
def test_union_cli_rejects_changed_input_or_source(tmp_path, monkeypatch, union_inputs, union_report, which):
    paths = tuple(tmp_path / f"input{i}.json" for i in range(9))
    for path, raw in zip(paths, union_inputs, strict=True):
        path.write_bytes(raw)
    output = tmp_path / "result.json"
