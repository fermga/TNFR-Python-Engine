...
from tests.physics.test_thol_native_runtime_response import execution as native_execution  # noqa: F401
...
def test_pool_roundtrip_is_lossless_for_shared_records_and_reference_like_user_data():
    ...
    repeated = {"epi": (F(1, 3), F(-7, 11)), "time": 1.75,
                "inactive_stage": {"outcome": "completed", "before": 0, "after": 0}}
    source = {"a": repeated, "b": repeated, "$record": "caller-data",
              "nested": [{"$record": "0"*64}], "false": False, "one": 1,
              "signed_zero": ("binary64", (-0.0).hex()), "refused": {"outcome": "raised", "error": "gate"}}
    pool = RecordPool()
...
@pytest.mark.parametrize("reference", ({"$record": True}, {"$record": 0}, {"$record": "0"*64},
                                       {"$record": "bad"}, {"other": "0"*64}))
...
def execution(native_execution, tmp_path_factory):  # noqa: F811 - reused portable fixture
    ...
    with patch.object(benchmark.native, "run_study", return_value=old["study"]), patch(
        "sys.argv", ["native", "--prior-input", str(old["path"]), "--expected-prior-sha256",
                     old["digest"], "--output", str(native_path)],
    ):
    ...
    def native_call(graph, **kwargs):
        ...
        name = next(name for name, (candidate, _) in old["sources"].items() if candidate is graph)
        calls.append((name, graph.graph["_t"]))
        return actual_step(graph, **kwargs)

    with patch.object(benchmark, "replay_response_branch", side_effect=lambda name: old["sources"][name]), patch.object(
        benchmark.native, "run_native_branch", retained_first,
    ), patch.object(benchmark.native.dynamics, "step", native_call), patch.object(benchmark.RecordPool, "pack", record_pack):
        study = benchmark.run_study(native_path, old["path"], expected_native_sha256=native_digest,
                                    expected_full_sha256=old["digest"])
    expanded = [[benchmark.expand_record(ref, study["record_pool"]) for ref in row["steps"]]
                for row in study["branches"]]
    return {"study": study, "expanded": expanded, "packed": packed, "calls": calls,
            "native": old, "native_path": native_path, "native_digest": native_digest,
            "native_payload": json.loads(native_path.read_bytes())}


def test_exactly_five_additional_calls_preserve_causal_adjacency_and_stop_at_three(execution):
    study = execution["study"]
    expected_times = [1.75, 2.0, 2.25, 2.5, 2.75]
    assert execution["calls"] == [(name, t) for name in ("control", "child_emission") for t in expected_times]
    assert study["protocol"]["maximum_steps_per_branch"] == 5
    assert study["protocol"]["retries"] == 0 and not study["protocol"]["parameter_tuning"]
    for index, (branch, steps) in enumerate(zip(study["branches"], execution["expanded"], strict=True)):
...
def test_actual_compaction_roundtrips_every_raw_record_and_original_reference(execution):
    from benchmarks.thol_native_policy_window import expand_record
    ...
    assert reference == _payload(execution["native"]["study"]["replayed_prior_reports"][0]["original_reference"])
    for retained, first in zip(study["first_step_replays"], execution["native"]["study"]["branches"], strict=True):
        assert expand_record(retained, study["record_pool"]) == _payload(first)


def test_each_actual_adaptation_gate_retains_counter_inputs_inactive_stages_and_history(execution):
    for steps in execution["expanded"]:
        for step in steps:
            trace = step["native_trace"]["boundaries"]
            row = next(item for item in trace if item["boundary"] == "adapt_vf_after_structural_stability")
            result = step["adaptation"]
            assert result["available"] and row["outcome"] == "completed"
            cfg = result["configuration"]
            assert cfg["VF_ADAPT_TAU"] == 5
            before, after = row["before"], row["after"]
            old_attrs, new_attrs = dict(before["node_attributes"]), dict(after["node_attributes"])
            for i, observed in enumerate(result["nodes"]):
                node = observed["node"]
                si, pressure = before["stored_Si"][i], before["state"]["pressure"][i]
                qualifies = si >= cfg["si_hi"] and abs(pressure) <= cfg["EPS_DNFR_STABLE"]
                ...
                assert observed["after_count"] == new_attrs[node]["stable_count"] == expected
            ...
            for name in ("compute_Si", "_apply_glyphs", "integrate", "coordinate_global_local_phase",
                         "_advance_math_engine", "_maybe_remesh", "_run_validators", "_run_after_callbacks"):
                assert names.count(name) == 1
            assert set(step["endpoint_tetrad"]["fields"]) == {"phi_s", "grad_phi", "curv_phi", "xi_c"}
...
def _direct_rate(observation, epi):
    ...
    return tuple(F(nu)*(F(observation["epi_weight"])*value/d+F(force))
                 for nu, value, d, force in zip(snapshot["capacity"], drift, degree, observation["forcing"], strict=True))


def test_original_profile_compatibility_is_independent_of_each_evolving_current_profile(execution):
    original = execution["native"]["study"]["replayed_prior_reports"][0]["original_reference"]
    h, z = original["metric_weights"], original["relative_profile"]
    for steps in execution["expanded"]:
        for step in steps:
            for prefix in ("before", "endpoint"):
                observed = step["old_target_"+prefix]
                assert observed["pattern"]["available"] and observed["compatibility_available"]
                target = observed["target"]
                assert target["target_reference"] == _payload(original)
                epi = tuple(map(F, step[prefix]["state"]["epi"]))
                mean = _metric_readout(epi, h)[0]
                error = tuple(x-mean-profile for x, profile in zip(epi, z, strict=True))
                assert tuple(map(F, target["pattern"]["relative_error"])) == error
                assert F(target["pattern"]["error_variance"]) == sum(w*v*v for w, v in zip(h, error, strict=True))/2
                ...
                assert F(target["compatibility_energy"]) == sum(w*v*v for w, v in zip(h, residual, strict=True))/2


def test_common_il_formula_retains_generated_and_consumed_pressure_as_different_boundaries(execution):
    from benchmarks.thol_native_policy_window import expand_record

    study = execution["study"]
    paired = expand_record(study["paired_steps"], study["record_pool"])
    h0 = execution["native"]["study"]["replayed_prior_reports"][0]["original_reference"]["metric_weights"]
    assert len(paired) == 5
    assert [row["common_IL_gate"]["available"] for row in paired] == [True, True, False, True, False]
    for left, right, result in zip(*execution["expanded"], paired, strict=True):
        ...
        for key, boundary in (("before", "before"), ("endpoint", "endpoint")):
            delta = tuple(F(b)-F(a) for a, b in zip(left[boundary]["state"]["epi"], right[boundary]["state"]["epi"], strict=True))
            ...
            assert F(result[key]["centered_H_energy"]) == energy and F(result[key]["full_H_energy"]) == full
        ...
        pressure = tuple(F(1, 4)*v*a*(F(y)-F(x)) for v, x, y in zip(nu, p["epsilon_pressure"], q["epsilon_pressure"], strict=True))
        il = tuple(F(1, 4)*v*(F(y)-F(x)) for v, x, y in zip(nu, p["epsilon_IL"], q["epsilon_IL"], strict=True))
        assert tuple(map(F, gate["pressure_residual_term"])) == pressure
        assert tuple(map(F, gate["IL_residual_term"])) == il
        total = tuple(sum(values) for values in zip(ideal, pressure, il,
                      map(F, gate["integration_residual_difference"]), map(F, gate["postintegration_difference"]), strict=True))
        assert total == tuple(map(F, result["endpoint"]["epi_difference"]))
        assert not any(map(F, gate["identity_residual"]))


def test_actual_policy_sequence_and_binary64_il_products_are_recorded_not_assumed(execution):
    for steps in execution["expanded"]:
        policies = []
        for step in steps:
            rows = [row for row in step["native_trace"]["boundaries"] if row["boundary"] == "apply_glyph"]
            assert len(rows) == 16 and all(row["outcome"] == "completed" for row in rows)
            policy = {row["glyph"] for row in rows}
            assert len(policy) == 1
            policies.append(next(iter(policy)))
            if policy == {"IL"}:
                for index, row in enumerate(rows):
                    exact = F(row["resolved_IL_retention"])*F(row["before"]["state"]["pressure"][index])
                    assert F(row["after"]["state"]["pressure"][index]) == F(float(exact))
            else:
...
@pytest.mark.parametrize("mutation", ("refusal", "start_time", "end_time", "node_order", "old_start", "old_end"))
def test_paired_readout_abstains_without_common_completed_times_and_original_space(execution, mutation):
    ...
    if mutation == "refusal":
        ...
        right["before" if mutation == "start_time" else "endpoint"]["state"]["time"] += .25
        ...
        right["old_target_before" if mutation == "old_start" else "old_target_endpoint"]["pattern"]["available"] = False
    metric = execution["native"]["study"]["replayed_prior_reports"][0]["original_reference"]["metric_weights"]
...
def test_native_replay_allows_only_consistent_child_utc_in_every_enumerated_record(execution):
    from benchmarks.thol_native_policy_window import _admit_native_replay

    first = execution["native"]["study"]["branches"][1]
    old = _payload(first)
    children = execution["native"]["study"]["replayed_prior_reports"][1]["lineage"]["children"]
    for record in _records(old):
        attrs = dict(record["node_attributes"])[children[0]]
        attrs["emission_timestamp"] = attrs["_emission_origin"] = "2031-01-01T00:00:00+00:00"
...
@pytest.mark.parametrize("mutation", ("utc_alias", "physical_time", "activation", "history", "nonchild_utc", "held_prediction"))
def test_native_replay_rejects_scientific_or_unlisted_metadata_changes(execution, mutation):
    from benchmarks.thol_native_policy_window import _admit_native_replay
...
def test_refusal_stops_window_after_one_attempt_and_keeps_partial_record(monkeypatch):
    ...
    prior = {"branch": "control", "prefix": {"coupling": {"refreshed_forcing": capture["payload"]}}}
    first = {"status": "executed", "endpoint": benchmark.native._record(graph)}
    calls = []
    ...
    def refusal(graph, **kwargs):
        ...
        return {"status": "refused", "boundaries": [], "captures": {}, "native_calls": 1,
                "failure": {"type": "OperatorPreconditionError", "message": "bounded control"}}

    monkeypatch.setattr(benchmark.native, "_trace_step", refusal)
    monkeypatch.setattr(benchmark.native, "_capture", lambda graph: {"available": False, "reason": "synthetic"})
    pool = benchmark.RecordPool()
    result = benchmark.continue_window(graph, prior, first, pool=pool)
    assert len(calls) == result["attempted_steps"] == 1 and result["completed_steps"] == 0
...
@pytest.mark.parametrize("mutation", ("claim", "ancestry_digest", "ancestry_manifest", "order", "refusal"))
def test_loader_rejects_incorrect_claim_ancestry_or_incomplete_branches_with_fresh_digest(execution, tmp_path, mutation):
    ...
    with pytest.raises(ValueError):
        load_evidence(path, execution["native"]["path"], expected_native_sha256=digest,
                      expected_full_sha256=execution["native"]["digest"])


def test_cli_keeps_lossless_evidence_and_rejects_overwriting_either_input(execution, tmp_path, monkeypatch):
    import benchmarks.thol_native_policy_window as benchmark

    output = tmp_path / "window.json"
    monkeypatch.setattr(benchmark, "run_study", lambda *args, **kwargs: execution["study"])
    args = ["window", "--native-input", str(execution["native_path"]),
            "--full-response-input", str(execution["native"]["path"])]
