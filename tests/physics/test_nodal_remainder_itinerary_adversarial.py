...
def test_real_open_interval_can_have_no_legal_remainder_grid_point():
    ...
    invalid = NodalRemainderState((ODD,), (midpoint - F(ODD),), .05, 1.)
...
def test_closed_singleton_at_the_full_denominator_limit_is_feasible():
    width = 3 * 2.**-55
    result = _derive(
        (.5, .5, .5), (MIN_SUBNORMAL, 1.),
        (MIN_SUBNORMAL, 1.), (-MIN_SUBNORMAL, width),
    )
    exact_initial = F(.5) - F(1, 2**55) + GRID
    cell, = result.coordinates
    ...
    assert result.witness_sequence.steps[0].after.exact_epi == (F(.5) - F(1, 2**55),)
    assert result.witness_sequence.endpoint.exact_epi == (F(.5) + F(1, 2**54),)
...
def test_closed_tiny_area_cycle_can_require_nonzero_carry_without_source_provenance():
    ...
    second = owner.observe_nodal_remainder_sequence(
        initial=result.witness_sequence.endpoint, timesteps=result.timesteps,
        capacities=result.capacities, pressures=result.pressures,
    )
    assert second.endpoint == result.witness_initial


@pytest.mark.parametrize("value,pressure,lower,upper,expected", (
    (1., MIN_SUBNORMAL, .05, 1., F(1) - GRID),
    (.5, -MIN_SUBNORMAL, .5, 1., F(.5) + GRID),
))
def test_exact_band_constraints_override_invisible_rounding_changes(value, pressure, lower, upper, expected):
    result = _derive(
        (value, value), (MIN_SUBNORMAL,), (MIN_SUBNORMAL,), (pressure,),
    ...
    with pytest.raises(ValueError, match="exact nodal update"):
        advance_nodal_remainder(
            initial, timestep=MIN_SUBNORMAL, capacity=(MIN_SUBNORMAL,), pressure=(pressure,),
        )
...
def test_intermediate_exact_band_failure_cannot_be_hidden_by_zero_final_area():
    result = _derive(
        (1., 1., 1.), (MIN_SUBNORMAL,) * 2,
        (MIN_SUBNORMAL,) * 2, (MIN_SUBNORMAL, -MIN_SUBNORMAL),
        epi_lower=1., epi_upper=1.,
    )
...
def test_one_infeasible_coordinate_blocks_a_joint_witness():
    result = owner.derive_nodal_remainder_itinerary(
        epi_states=((.5, ODD), (.5, EVEN), (.5, NEXT_ODD)),
        timesteps=(1., MIN_SUBNORMAL), capacities=((1., 1.), (MIN_SUBNORMAL, MIN_SUBNORMAL)),
        pressures=((0., ULP), (0., MIN_SUBNORMAL)),
    )
...
def test_detached_result_flags_do_not_authenticate_forged_arithmetic_inputs():
    ...
    fresh = owner.derive_nodal_remainder_itinerary(
        epi_states=forged.epi_states, timesteps=forged.timesteps,
        capacities=forged.capacities, pressures=forged.pressures,
        epi_lower=forged.epi_lower, epi_upper=forged.epi_upper,
    )
...
@pytest.mark.parametrize("changes", (
    {"epi_states": TupleSubclass(((.5,), (.5,)))},
    {"epi_states": (TupleSubclass((.5,)), (.5,))},
    {"epi_states": ((FloatSubclass(.5),), (.5,))},
    {"timesteps": (FloatSubclass(1.),)},
    {"capacities": ((FloatSubclass(1.),),)},
    {"pressures": ((FloatSubclass(0.),),)},
    {"epi_lower": FloatSubclass(.05)},
))
def test_user_numeric_or_container_subclasses_cannot_bypass_strict_primitive_validation(changes):
    inputs = dict(epi_states=((.5,), (.5,)), timesteps=(1.,), capacities=((1.,),), pressures=((0.,),))
