...
def test_fixed_inherited_cases_share_an_explicit_pressure_refresh_mesh(report):
    assert tuple((case["mode"], float(case["epsilon"])) for case in report["cases"]) == owner.CASES
...
def test_every_live_flow_obeys_an_independent_exact_reconstructed_balance(report):
    for case in report["cases"]:
        ...
        for flow, prefix in zip(carried["flows"], carried["prefix_budget"]["prefixes"], strict=True):
            step = flow["step"]
            for i in range(6):
                assert step["pressure"][i] == flow["before_snapshot"]["delta_nfr"][i]
                area[i] += F(step["timestep"]) * F(step["capacity"][i]) * F(step["pressure"][i])
                expected = F(1, 2) + area[i]
                assert F(step["after"]["epi"][i]) + step["after"]["remainder"][i] == expected
...
def test_mean_difference_is_split_between_actual_source_and_executor(report):
    for case in report["cases"]:
        carried, ordinary, comparison = case["carried"], case["ordinary"], case["comparison"]
        assert comparison["visible_mean_gap"] == carried["visible_mean_change"] - ordinary["visible_mean_change"]
        assert comparison["source_mean_gap"] == carried["mean_area"] - ordinary["mean_area"]
        assert comparison["executor_mean_gap"] == carried["executor_mean_defect"] - ordinary["executor_mean_defect"]
        assert comparison["visible_mean_gap"] == comparison["source_mean_gap"] + comparison["executor_mean_gap"]
        assert comparison["identity_residual"] == 0
        assert carried["executor_mean_defect"] == -sum(carried["final_state"]["remainder"]) / 6
...
def test_source_feedback_changes_are_retained_instead_of_forced_equal(report):
    assert any(case["comparison"]["any_pressure_difference"] for case in report["cases"])
    for case in report["cases"]:
        rows = case["comparison"]["pressure_differences"]
        assert case["comparison"]["any_pressure_difference"] == any(any(row) for row in rows)
...
def test_report_keeps_finite_runtime_scope_and_serializes_exact_budgets(report):
    ...
    for original, encoded in zip(report["cases"], decoded["cases"], strict=True):
        assert F(encoded["carried"]["executor_mean_defect"]) == original["carried"]["executor_mean_defect"]


@pytest.mark.parametrize("mode,epsilon", (("k2", 2.**-12), ("null", 0), ("k1", -2.**-12)))
...
def test_cli_refuses_source_changes_before_writing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(owner, "current_git_source_provenance", lambda *args: next(provenance))
