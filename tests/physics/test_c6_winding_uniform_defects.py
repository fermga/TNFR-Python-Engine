...
from tnfr.physics.coupling_winding import (
    bound_c6_winding_uniform_defects, derive_c6_winding_joint_domain,
)
...
def _reference(**overrides):
    values = dict(
        coupling_phase_factor=F(1, 2), coherence_phase_factor=F(3, 10),
        capacity=1, epi_weight=1, phase_weight=F(1, 4), timestep=F(1, 2),
        epi_lower=F(0.05), epi_upper=1,
    )
...
def _bound(reference=None, **overrides):
    values = dict(
        initial_epi=UNIFORM, initial_phase_oscillation=0,
        phase_defect_bound=0, centered_epi_defect_bound=0, mean_prefix_bound=0,
    )
    values.update(overrides)
...
def _euler(values, s):
    return tuple((1 - s) * values[i] + s * (values[(i - 1) % 6] + values[(i + 1) % 6]) / 2
                 for i in range(6))
...
def test_one_step_range_functional_contracts_even_when_one_step_range_does_not():
    ...
    assert _functional(following, s) == bound.range_functional_factor * bound.initial_range_functional
...
def test_uniform_error_tube_uses_range_and_mean_budgets_without_square_roots():
    bound = _bound(phase_defect_bound=F(1, 15000), centered_epi_defect_bound=F(1, 10000))
    ...
    assert bound.phase_class_preserved and bound.epi_band_preserved and bound.joint_domain_preserved
...
def test_declared_class_bounds_two_adversarial_exact_updates_including_signed_mean():
    ...
    bound = _bound(phase_defect_bound=epsilon, centered_epi_defect_bound=error, mean_prefix_bound=mean_limit)
    ...
    for mean_error in (mean_limit, -mean_limit):
        assert _osc(phase) == reference.nonlinear_oscillation_factor * _osc(phase) + epsilon
        centered = (error / 2, -error / 2) * 3
        defect = tuple(value + mean_error for value in centered)
        pressure = tuple((phase[(i - 1) % 6] + phase[(i + 1) % 6]) / 2 - phase[i] for i in range(6))
        diffused = _euler(values, s)
        following = tuple(diffused[i] + b * pressure[i] + defect[i] for i in range(6))
        assert _functional(following, s) <= (
            bound.range_functional_factor * _functional(values, s) + 2 * bound.forcing_oscillation_bound
        )
        assert _functional(following, s) <= bound.range_functional_bound
        assert abs(sum(following) / 6 - F(1, 2)) <= mean_limit
        assert bound.epi_envelope_lower <= min(following) <= max(following) <= bound.epi_envelope_upper
        values = following
...
def test_bounded_centered_error_can_maintain_nonuniform_epi_despite_strict_diffusion():
    ...
    bound = _bound(reference, initial_epi=initial, centered_epi_defect_bound=_osc(defect))
    ...
    for value, admitted in ((before, True), (after, False)):
        ...
        for node in graph:
            graph.nodes[node].update({
                ...
                ALIAS_EPI[0]: float(value), ALIAS_VF[0]: 1.0,
            })
        if admitted:
...
def test_noncontracting_diffusion_rejects_a_positive_uniform_forcing_allowance(s):
    with pytest.raises(ValueError):
        _bound(_reference(timestep=s, phase_weight=0), centered_epi_defect_bound=F(1, 10**6))
...
@pytest.mark.parametrize("field,value", (
    ...
    ("nodal_euler_matrix", tuple(tuple(F(i == j) for j in range(6)) for i in range(6))),
))
def test_uniform_bound_rebuilds_forged_reference_caches_before_derivation(field, value):
    ...
    options = dict(phase_defect_bound=F(1, 15000), centered_epi_defect_bound=F(1, 10000))
