...
def _transition(size=6, *, uniform=False, visible=False):
    initial = initialize_nodal_remainder((.5,) * size, epi_lower=.375, epi_upper=.625)
    pressure = ((2.**-55,) * size if uniform else
                (2.**(-53 if visible else -57), -2.**(-54 if visible else -57)) + (0.,) * (size - 2))
    step = advance_nodal_remainder(initial, timestep=1., capacity=(1.,) * size, pressure=pressure)
    return initial, step


def _observe(initial, step, **overrides):
    arguments = dict(initial=initial, endpoint=step.after, nodal_area=step.exact_increment, epi_quantum=QUANTUM)
...
def test_invisible_nodal_area_is_exactly_cancelled_by_carry_in_the_gradient(size):
    ...
    assert result.gradient_indices_before == result.gradient_indices_after == (0,) * size
    assert result.nodal_area == result.remainder_change == step.exact_increment
    assert any(result.nodal_gradient_term)
    assert result.nodal_gradient_term == tuple(-value for value in result.remainder_gradient_term)
    assert result.identity_residual == (0,) * size
    assert not result.pressure_provenance_certified and not result.runtime_provenance_certified
...
def test_visible_index_change_uses_both_neighbors_with_the_correct_sign(size):
    ...
    result = _observe(initial, step)
    offsets = tuple((F(value) - F(.5)) / QUANTUM for value in step.after.epi)
    expected = tuple(offsets[i - 1] + offsets[(i + 1) % size] - 2 * offsets[i] for i in range(size))
...
@pytest.mark.parametrize("quantum", (F(0), F(-1), 2.**-54, True))
