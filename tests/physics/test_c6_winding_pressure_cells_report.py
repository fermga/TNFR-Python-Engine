...
def test_detached_audit_uses_shared_validation_once_per_case_and_never_runs_graphs(parent, monkeypatch):
...
def test_all_six_boxes_and_numeric_witnesses_bind_complete_recorded_traces(parent, report):
    for source, result in zip(parent["current_cases"], report["cases"], strict=True):
        for cycle, observed in zip(source["cycles"], result["cycles"], strict=True):
            ...
            assert tuple(map(F, box["flow"]["epi"])) == tuple(map(F, cycle["flow"]["before"]["epi"]))
            assert tuple(map(F, box["flow"]["endpoint"])) == tuple(map(F, cycle["flow"]["raw_after_integrator"]["epi"]))
            ...
            for key in ("zero_sum_witness", "opposite_pair_witness"):
                ...
                for value, cell in zip(witness["pressure"], box["coordinates"], strict=True):
...
def test_k1_one_ulp_zero_sum_corrections_preserve_the_same_mean_defect(report):
    ...
    changes = tuple(cycle["zero_sum_witness"]["mean_budget"]["actual_mean_change"] for cycle in k1["cycles"])
    assert changes == (F(1, 3 * 2**52), 0)


def test_k1_opposite_completion_has_two_positive_binade_biases_and_one_zero(parent, report):
    first = report["cases"][1]["cycles"][0]
    source = tuple(map(float, map(F, parent["current_cases"][1]["cycles"][0]["flow"]["before"]["pressure"])))
    ...
    assert tuple(fractional_parts) == (F(8875, 262144), F(153711, 262144), F(314293, 524288))
    ...
    for step in report["cases"][1]["cycles"][1]["opposite_pair_witness"]["substep_pair_sums"]:
...
def test_signed_accounting_keeps_every_cycle_and_zero_net_drift_separate(report):
    ...
    for case in report["cases"]:
        ...
        for cycle in case["cycles"]:
            for budget in (cycle["mean_budget"], cycle["zero_sum_witness"]["mean_budget"],
                           cycle["opposite_pair_witness"]["mean_budget"]):
                assert budget["actual_mean_change"] == (
                    budget["held_pressure_mean_effect"] + budget["scaling_mean_effect"] + budget["addition_mean_effect"]
                )
                assert budget["identity_residual"] == 0
            total += cycle["mean_budget"]["actual_mean_change"]
        sums.append(total)
    assert sums == [0, F(1, 3 * 2**52), 0]
    assert all(cycle["mean_budget"]["actual_mean_change"] != 0 for cycle in report["cases"][2]["cycles"])


def test_scope_never_promotes_pressure_feasibility_to_production_or_future(report):
    for flag in ("runtime_executed", "live_provenance_certified", "future_bounds_verified", "canonical_pressure_witnesses_generated"):
...
def test_infeasible_pair_boxes_and_absent_single_coordinate_witness_are_distinct():
    box = campaign.derive_binary64_quarter_pressure_box(epi=(.5,) * 6, pressure=(.125,) * 6)
