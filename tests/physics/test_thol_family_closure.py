...
def _graph():
    ...
    for node, epi, capacity in zip(graph, (1.75, -0.5, 0.25, 1.25), (1.0, 1.0, 0.5, 0.5), strict=True):
        graph.nodes[node].update(EPI=epi, nu_f=capacity, theta=0.0, delta_nfr=0.0,
                                 glyph_history=[])
    for parent, child in ((0, 2), (1, 3)):
        graph.nodes[parent]["sub_nodes"] = [child]
        graph.nodes[child]["parent_node"] = parent
    graph.graph.update(
        hierarchy={0: [2], 1: [3]}, _node_sample=(0, 1, 2, 3), _t=1.0,
...
def _target(original, current, readout):
    ...
    return asdict(observe_forced_support_target(
        original, current, SupportTransportSnapshot(**readout["observation"]["snapshot"]),
...
def retained_fixture(tmp_path_factory):
    ...
    git_sha, dirty, source_hash = current_git_source_provenance(Path(__file__).resolve().parents[2], scope)

    def manifest(claim, destination):
        record = CoreExperimentManifest(
            claim_id=claim, git_sha=git_sha, source_dirty=dirty, dirty_source_hash=source_hash,
            versions={"python": platform.python_version()}, result_status=ClaimStatus.MEASURED,
            ...
            solver="No solver or trajectory; exact readouts only", seed=0, timestep=0.25,
            operator_sequence=(), telemetry=("Exact affine model and supplied family relation",),
            controls=("Retained unchanged model",), artifacts=(str(destination),),
        )
    ...
    digest = hashlib.sha256(json.dumps(
        _payload(asdict(original)), sort_keys=True,
        separators=(",", ":"), allow_nan=False,
    ).encode()).hexdigest()
    state = _state(graph)
    prefix = {
        "birth": {"parent_children": ((0, 2), (1, 3)),
                  "before": {"nodes": (0, 1)}, "after": deepcopy(state)},
        "coupling": {"refreshed_forcing": readout},
    }
    common = {
        "prefix": prefix, "original_reference": asdict(original),
        "original_reference_sha256": digest, "original_reference_frozen_time": 0.5,
        "original_reference_frozen_before_baseline_flow": True,
        "initial_target": target, "baseline_target": target,
        "baseline_flow": {"after_forcing": readout}, "baseline_steps": (),
        "baseline_capture_checks": {"fixture_geometry_unchanged": True},
        "common_source": _common_source(graph), "before_optional_event": state,
    }
    ...
    for name in ("no_event", "all_node_um", "original_parents", "born_children"):
        ...
        records[name] = {
            **deepcopy(common), "branch": name, "status": "executed",
            "after_optional_event": _state(changed),
            "event": None if name == "no_event" else {
        ...
        if name in ("original_parents", "born_children"):
            records[name]["lineage"] = {
                "parent_children": ((0, 2), (1, 3)), "parents": (0, 1), "children": (2, 3),
                "current_nodes": (0, 1, 2, 3), "live_parentage_verified": True,
                "disjoint_and_exhaustive": True,
                "selected_targets": (0, 1) if name == "original_parents" else (2, 3),
            }
    control = {
        "manifest": manifest("O1.b-distributed-fixed-relative-target-response", control_path),
        "source_scope": scope, "common_causal_baseline_reproduced": True,
    ...
    lineage = {
        "manifest": manifest("O1.b-lineage-scoped-UM-fixed-target-response", lineage_path),
        "source_scope": scope,
        "retained_controls": {"path": str(control_path), "sha256": control_hash,
                              "byte_count": control_path.stat().st_size,
                              "historical_manifest": control["manifest"],
                              "historical_source_scope": scope, "historical_producer_preserved": True},
        "branches": tuple(records[name] for name in ("original_parents", "born_children")),
        "fixture_scope": control["fixture_scope"],
    }
    lineage_hash = _write(lineage_path, lineage)
    return {
        "control_path": control_path, "lineage_path": lineage_path,
        "control_hash": control_hash, "lineage_hash": lineage_hash,
        "control": _json(control), "lineage": _json(lineage), "references": references,
    }
...
def test_four_retained_models_preserve_supplied_family_and_producer_identity(study, retained_fixture):
    ...
    assert study["partition"]["blocks"] == study["partition"]["parent_children"] == ((0, 2), (1, 3))
    assert study["partition"]["family_count"] == 2
    assert all(row["all_fields_equal"] for row in study["common_source_comparison"])
    assert not study["new_trajectories_executed"] and not study["partition_search_performed"]
    for name in ("control", "lineage"):
        binding = study["retained_controls" if name == "control" else "retained_lineage"]
        assert binding["sha256"] == retained_fixture[name + "_hash"]
        assert binding["historical_manifest"] == retained_fixture[name]["manifest"]
        assert "Synthetic detached" in binding["historical_manifest"]["graph_construction"]
...
def test_numerical_norm_labels_and_values_mean_spectral_not_infinity_or_frobenius():
    ...
    for i in graph:
        graph.nodes[i].update(EPI=i, nu_f=1 if i % 2 == 0 else 2,
                              delta_nfr=0, theta=0)
    reference = derive_forced_support_balance(
        observe_support_transport(graph), epi_weight=F(2, 5),
    ...
    r = np.asarray([[h[i] / sum(h[j] for j in block) if i in block else 0.0
                     for i in graph] for block in blocks])
    a = float(reference.epi_weight) * nu[:, None] * (np.eye(6) - w / d[:, None])
    q = np.eye(6) - p @ r
    raq, qap = (r @ a) @ q, (q @ a) @ p
    k0 = raq @ (a @ p)
    assert np.linalg.matrix_rank(raq) == 2
    assert np.linalg.norm(raq, 2) != pytest.approx(np.linalg.norm(raq, np.inf), rel=1e-5)
    assert np.linalg.norm(raq, 2) != pytest.approx(np.linalg.norm(raq, "fro"), rel=1e-5)
    for prefix, matrix in (("projection_defect", raq), ("lift_defect", qap), ("zero_lag_kernel", k0)):
        assert prefix + "_inf_norm" not in numeric
        assert numeric[prefix + "_spectral_norm"] == pytest.approx(
            np.linalg.norm(matrix, 2), rel=1e-14, abs=1e-16,
        )
    assert "spectral" in numeric["norm_definition"].lower()
    for name, matrix, field in (
        ("R", r, "projection"), ("A", a, "micro_generator"),
        ("RAQ", raq, "hidden_to_macro"), ("QAP", qap, "macro_to_hidden"),
        ("K0", k0, "instantaneous_kernel"),
    ):
        expected = np.linalg.norm(matrix - np.asarray(getattr(exact, field), dtype=float), 2)
        assert numeric["exact_cast_discrepancies"][name] == pytest.approx(expected, rel=1e-14, abs=1e-30)
    abar = (r @ a) @ p
    for name, mapping, source, target in (
        ("projection_intertwining", r, a, abar), ("lift_intertwining", p, abar, a),
    ):
        ...
        assert numeric[name]["relative_residual"] == pytest.approx(residual/scale, rel=1e-14)


def test_observer_reset_is_exact_reweighting_of_one_unchanged_microstate(study, retained_fixture):
    ...
    for row in study["models"]:
        ...
        assert reset["projected_epi_change"] == reset["projection_reweighting"] == change
        assert change == _apply(_subtract(g1["R"], g0["R"]), x)
        mean1 = sum(h*v for h, v in zip(g1["H"], x, strict=True))/sum(g1["H"])
        u1 = tuple(v-mean1-z for v, z in zip(x, reference.relative_profile, strict=True))
        ...
        profile_shift = tuple(b-a for a, b in zip(
            _apply(g0["R"], original.relative_profile), _apply(g1["R"], reference.relative_profile), strict=True,
        ))
        assert reset["projected_profile_shift"] == profile_shift
        assert reset["projected_relative_error_change"] == tuple(
            raw-(mean1-mean0)-profile for raw, profile in zip(change, profile_shift, strict=True)
        )
        assert reset["identity_residual"] == (0, 0)
        assert row["old_target_readout"]["pattern"] == study["models"][0]["old_target_readout"]["pattern"]
        saw_nonzero_reweighting |= any(change)
    assert saw_nonzero_reweighting


@pytest.mark.parametrize("which", ("control", "lineage"))
def test_input_content_hash_is_required_before_any_retained_evidence_is_admitted(retained_fixture, which):
    from benchmarks.thol_family_closure import load_evidence

    hashes = {"expected_control_sha256": retained_fixture["control_hash"],
              "expected_lineage_sha256": retained_fixture["lineage_hash"]}
    hashes["expected_" + which + "_sha256"] = "0" * 64
    with pytest.raises(ValueError, match="digest"):
        load_evidence(retained_fixture["control_path"], retained_fixture["lineage_path"], **hashes)
...
def test_digest_format_cannot_be_auto_admitted(value):
    ...
    with pytest.raises(ValueError, match="SHA256"):
        load_evidence("missing_control.json", "missing_lineage.json", expected_control_sha256=value)


@pytest.mark.parametrize("mutation", (
    "producer", "branch_order", "refused", "control_binding", "parent_pointer", "family",
    "cached_target", "cached_snapshot", "forcing", "event_epi", "event_time", "original_hash",
    "event_capacity", "event_pressure", "event_phase",
))
def test_hash_bound_payload_still_requires_lineage_and_exact_model_consistency(
    retained_fixture, tmp_path, mutation,
):
    from benchmarks.thol_family_closure import run_study
    ...
    if mutation == "producer":
        ...
        lineage["retained_controls"]["historical_manifest"]["solver"] = "wrong provenance"
        ...
        branch["event"]["coupling"]["refreshed_forcing"]["observation"]["snapshot"]["rate"][0] = "99"
...
def test_cli_serialization_retains_input_producers_and_no_new_trajectory_claim(
    ...
    monkeypatch.setattr("sys.argv", [
        "thol_family_closure", "--controls", str(retained_fixture["control_path"]),
        "--lineage", str(retained_fixture["lineage_path"]),
        "--expected-control-sha256", retained_fixture["control_hash"],
        "--expected-lineage-sha256", retained_fixture["lineage_hash"], "--output", str(output),
    ])
    benchmark.main()
    assert len(called) == 1
    payload = json.loads(output.read_bytes())
    CoreExperimentManifest(**payload["manifest"]).validate_for_admission()
    assert payload["retained_controls"]["historical_manifest"] == retained_fixture["control"]["manifest"]
    assert payload["retained_lineage"]["historical_manifest"] == retained_fixture["lineage"]["manifest"]
