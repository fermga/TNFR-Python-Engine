...
def _trace(saved):
    ...
    def record(epi, time=F(9, 4)):
        return {
            "state": {
                "nodes": list(nodes),
                "epi": [float(F(x)) for x in epi],
                "capacity": list(map(float, snap.capacity)),
                "pressure": list(map(float, snap.stored_pressure)),
                "phase": list(map(float, obs.phase)),
                "time": float(time),
                "edges": [
                    (nodes[i], nodes[j], {"weight": float(w)})
                    for i, j, w in snap.conductance
                    if i < j
                ],
            },
            "ordered_neighbors": [
                (node, [nodes[j] for j in snap.support_neighbors[i]])
    ...
    boundaries.append(
        {
            ...
            "effective_arguments": {
                "dt": ["binary64", "0x1.0000000000000p-2"],
                "method": "euler",
            },
            "before": record(vectors["xg"]),
            "after": record(vectors["xi"], F(5, 2)),
        }
    )
    entry_snap = _from_data(
        nodes,
        snap.conductance,
        snap.support_neighbors,
        tuple(F(x) for x in vectors["xg"]),
        snap.capacity,
        snap.stored_pressure,
    )
    fresh = tuple(
        obs.epi_weight * g + f
        for g, f in zip(entry_snap.epi_gradient, obs.forcing, strict=True)
    )
    entry_obs = replace(
        obs,
        snapshot=entry_snap,
        ...
        stored_pressure_residual=tuple(
            p - v for p, v in zip(snap.stored_pressure, fresh, strict=True)
        ),
    )
    return {
        "status": "executed",
        "before": before,
        "endpoint": record(vectors["xf"], F(5, 2)),
        "native_trace": {
            "status": "executed",
            "boundaries": boundaries,
            "captures": {
                "pressure_generation": capture(obs),
                "integrator_entry": capture(entry_obs),
            },
        },
    }
...
def test_common_declared_rows_and_complete_realized_remainders(defects, monkeypatch):
    ...
    assert all(
        "kernel_evaluation_defect" not in row and "clipping_defect" not in row
    ...
    assert pair["delta"]["xf"] == tuple(
        b - a
        for a, b in zip(template["vectors"]["xf"], actual["vectors"]["xf"], strict=True)
    )
...
def _file(path, claim):
    manifest = CoreExperimentManifest(
        ...
        graph_construction="synthetic",
        capacity_specification="unit",
        solver="saved arithmetic",
        timestep=0.25,
        seed=17,
...
def test_authentication_precedes_arithmetic_admission(tmp_path, monkeypatch):
    ...
    seen = []
    monkeypatch.setattr(
        admission, "admit_reports", lambda *args: seen.append(args) or {"fixture": True}
    )
    value = admission.load_comparable_witnesses(
        old, new, expected_reset_sha256=a, expected_restoration_sha256=b
    )
    assert len(seen) == 1 and value["historical_inputs"]["restoration"]["sha256"] == b
    old.write_text(old.read_text() + " ")
    with pytest.raises(ValueError, match="digest"):
        admission.load_comparable_witnesses(
            old, new, expected_reset_sha256=a, expected_restoration_sha256=b
        )
    assert len(seen) == 1
...
def test_wrong_historical_claim_refused(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        admission, "admit_reports", lambda *args: pytest.fail("unadmitted claim")
    )
