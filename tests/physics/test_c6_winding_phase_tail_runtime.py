...
from benchmarks.c6_winding_phase_tail_runtime import (
    TAIL_ENTRY_CYCLE, _declared_schedule, run_c6_phase_tail_runtime_bridge,
)
...
@pytest.mark.parametrize("cycles", (True, 0, -1, 90, 2., "89"))
...
def test_full_phase_tail_budget_is_one_complete_word_with_only_terminal_silence():
    ...
    for cycle, partition in enumerate(partitions, 1):
        ...
        assert tuple(segment.duration for segment in partition.segments) == (.0625,) * 4
        assert partition.parent_interval.exact_duration == F(1, 4)
    assert schedule.intervals[-1].duration == 0.
...
def test_two_cycle_runtime_prefix_retains_actual_phase_and_unreset_carry(prefix):
    ...
    assert prefix["source"]["initial_state"]["epi"] == (.5,) * 6
    assert prefix["source"]["initial_state"]["remainder"] == (F(0),) * 6
    assert not prefix["source"]["carry_imported_or_reset"]
    assert prefix["tail_entry_time"] == .25 and prefix["endpoint_time"] == .5
...
def test_unit_capacity_claim_stops_before_the_actual_terminal_sha(prefix):
    ...
    assert terminal["capacity_before"] == (1.,) * 6
    assert all(value < 1. for value in terminal["capacity_after"])
    assert terminal["state_before"] == terminal["state_after"] == prefix["endpoint_before_SHA"]
    assert terminal["phase_before"] == terminal["phase_after"] == prefix["endpoint_phase"]
    assert not prefix["historical_detached_endpoint_reachability_certified"]
    assert not prefix["future_runtime_certified"] and not prefix["indefinite_trapping_certified"]
...
def test_tail_entry_profile_uses_the_actual_reconstructed_preflow_state(prefix):
    ...
    state, profile, bound = prefix["tail_entry_after_last_IL"], prefix["tail_entry_profile"], prefix["tail_entry_closure"]
    exact = tuple(F(value) + carry for value, carry in zip(state["epi"], state["remainder"], strict=True))
    mean = sum(exact) / 6
    errors = tuple(value - mean - z for value, z in zip(exact, profile["relative_profile"], strict=True))
    assert sum(value * value for value in errors) <= bound["energy_bound"]
    assert tuple(profile["epi_weight"] * value for value in laplacian_action(profile["relative_profile"])) == tuple(
        value - profile["mean_drift"] for value in profile["forcing"])
