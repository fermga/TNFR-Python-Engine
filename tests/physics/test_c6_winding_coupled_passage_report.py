...
def test_source_is_the_unchanged_carried_endpoint_and_not_a_balance_point(report, parents):
    ...
    assert all(point["state"] != initial for point in report["B38_static_pressure_balance"]["points"])
    assert report["new_conditional_numerical_steps"] == 118


def test_self_consistent_closure_is_stricter_and_keeps_the_cut_possible(report, parents):
    ...
    assert closure["rounding_bound"] == closure["product_error_bound"] + closure["assembly_error_bound"]
    assert closure["gradient_index_lower"] == (-26, -28, -33, -17, -49, -13)
    assert closure["gradient_index_upper"] == (29, 27, 22, 38, 6, 42)
    assert closure["gradient_index_lower"][4] <= -22 <= closure["gradient_index_upper"][4]
    assert closure["mean_increment_lower"] < 0 < closure["mean_increment_upper"]


def test_static_positive_convex_balance_has_the_actual_initial_mean_without_temporal_promotion(report):
    ...
    pressure_sum = tuple(sum((weight * F(point["observation"]["pressure"][i])
                             for point, weight in zip(points, weights, strict=True)), F(0)) for i in range(6))
    assert pressure_sum == balance["pressure_balance"] == (0,) * 6
    pressure_means = tuple(_mean(tuple(map(F, point["observation"]["pressure"]))) for point in points)
    assert min(pressure_means) < 0 < max(pressure_means)
    assert balance["class_wide_strict_linear_drift_excluded"] is True
    for key in ("temporal_compensation_certified", "bounded_trajectory_certified", "future_runtime_certified"):
...
def test_finite_passage_bound_precedes_and_covers_the_observed_first_hit(report):
    passage, continuation = report["B39_finite_pressure_passage"], report["B40_continuation"]
    ...
    assert passage["centered_increment_lower"] == (passage["minimum_positive_pressure"] - passage["mean_pressure_upper"]) / 16
    assert passage["contradiction_steps"] == 30255
    assert passage["latest_pressure_index"] == 30254
    assert passage["band_horizon"]["maximum_steps"] == 196713720348826219 > passage["contradiction_steps"]
    assert continuation["step_count"] == 118 < passage["latest_pressure_index"]
    assert passage["abstention_reason"] is None
    for key in ("graph_provenance_certified", "future_runtime_certified", "infinite_trapping_certified"):
        assert passage[key] is False


def test_every_transition_has_fresh_canonical_pressure_and_the_stop_is_the_first_cut(report, parents):
    source = parents[-1]["lattice_reference"]["source"]
    lattice = derive_c6_pressure_lattice(
        phase=tuple(report["source"]["phase"]), epi_weight=float(F(source["epi_weight"])),
    ...
    for step in continuation["steps"]:
        ...
        assert step["capacity"] == (1.,) * 6 and step["timestep"] == 1 / 16
        change = _difference(_exact(step["after"]), _exact(step["before"]))
        assert change == tuple(F(value) / 16 for value in reading.pressure)
        assert step["exact_increment"] == change and step["nodal_balance_residual"] == (0,) * 6
        previous = step["after"]
...
def test_boundaries_partition_the_complete_118_step_first_hit_without_a_reset(report):
    ...
    for ordinal, boundary in enumerate(boundaries, 1):
        ...
        assert selected[0]["before"] == previous and selected[-1]["after"] == boundary["endpoint"]
        assert all(step["pressure"] == boundary["source_pressure"]["pressure"] for step in selected)
    ...
    assert continuation["all_centered_energies_within_closure"] is continuation["within_proven_pressure_index"] is True
...
def test_signed_prefix_repayment_is_an_overshoot_not_a_complete_vector_return(report):
    ...
    assert section["B27_incoming_mean_area"] == _mean(_difference(initial, baseline)) == -F(275, 3 * 2**114)
    prefixes = tuple(_mean(_difference(_exact(step["after"]), baseline)) for step in section["steps"])
    ...
    assert all(any(_difference(_exact(step["after"]), baseline)) for step in section["steps"])
    assert section["complete_return_observed"] is False
    assert section["mean_source_area"] == -F(118, 6 * 2**113)
    assert section["local_mean_area"] == section["mean_source_area"] + section["mean_rounding_area"]
    assert section["mean_carry_feedback_area"] == section["mean_identity_residual"] == 0
    assert sum(F(step["pressure"][i]) / 16 for step in section["steps"] for i in range(6)) / 6 == _mean(area)


@pytest.mark.parametrize("change", ("claim", "scope", "endpoint_carry", "phase", "bound", "flag", "ancestor_pressure", "lineage"))
...
def test_campaign_preserves_inputs_and_never_executes_a_graph_operator_word(parents, monkeypatch):
    from tnfr.operators import event_runtime, nodal_remainder_runtime

    def forbidden(*args, **kwargs):
        raise AssertionError("a detached first-passage campaign must not execute graph operators")
    ...
    assert result["new_graph_trajectories"] == 0 and result["new_conditional_numerical_steps"] == 118


def test_report_retains_the_scope_and_json_is_finite(report):
    for key in ("runtime_executed", "live_provenance_certified", "original_tail_reachability_certified",
                "infinite_band_invariance_certified", "full_runtime_stability_certified", "static_balance_is_temporal_itinerary"):
...
def _cli(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["passage", "--history-dir", str(tmp_path), "--output", str(output)])
    return paths, output


def test_cli_binds_all_seven_original_byte_hashes_and_its_own_source_manifest(tmp_path, monkeypatch, report):
    paths, output = _cli(tmp_path, monkeypatch)
    before = tuple(path.read_bytes() for path in paths)
    monkeypatch.setattr(campaign, "analyze_c6_winding_coupled_passage", lambda *args: deepcopy(report))
    provenance = ("a" * 40, True, "sha256:" + "b" * 64)
    monkeypatch.setattr(campaign, "current_git_source_provenance", lambda *args: provenance)
    campaign.main()
...
def test_cli_checks_each_historical_byte_link_before_analysis(tmp_path, monkeypatch, changed_index):
    paths, output = _cli(tmp_path, monkeypatch)
...
def test_cli_rejects_source_or_input_change_during_the_campaign(tmp_path, monkeypatch, report, change):
    paths, output = _cli(tmp_path, monkeypatch)
    original = ("a" * 40, True, "sha256:" + "b" * 64)
    later = ("a" * 40, True, "sha256:" + "c" * 64) if change == "source" else original
    values = iter((original, later))
    monkeypatch.setattr(campaign, "current_git_source_provenance", lambda *args: next(values))
...
def test_cli_cannot_overwrite_any_historical_input(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["passage", "--history-dir", str(tmp_path), "--output", str(paths[3])])
