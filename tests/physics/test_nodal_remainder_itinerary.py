...
def _word(**changes):
    args = dict(
        epi_states=((.5,), (.5,)), timesteps=(.0625,),
        capacities=((1.,),), pressures=((0.,),),
    )
    args.update(changes)
...
def test_upper_tie_is_feasible_for_even_half_but_carry_does_not_close():
    ...
    assert observed.witness_sequence.endpoint.epi == (.5,)
...
def test_two_feasible_edges_cannot_be_joined_by_resetting_carry():
    first = _word(pressures=((2.**-50,),))
    second = _word(pressures=((2.**-50,),))
    assert first.feasible and second.feasible
    combined = _word(epi_states=((.5,),) * 3, timesteps=(.0625,) * 2,
                     capacities=((1.,),) * 2, pressures=((2.**-50,),) * 2)
...
def test_nonzero_initial_carry_can_realize_a_word_that_zero_carry_cannot():
    successor = math.nextafter(.5, math.inf)
    observed = _word(epi_states=((.5,), (successor,)), pressures=((2.**-50,),))
    cell = observed.coordinates[0]
    assert cell.lower == F(1, 2) and not cell.lower_closed
    assert observed.feasible and not observed.zero_initial_carry_feasible
    assert observed.witness_initial.remainder == (F(1, 2**NODAL_REMAINDER_DENOMINATOR_BITS),)
...
def test_legitimate_nonstationary_visible_loop_closes_exact_carry():
    successor = math.nextafter(.5, math.inf)
    observed = _word(
        epi_states=((.5,), (successor,), (.5,)),
        timesteps=(1., 1.), capacities=((1.,), (1.,)),
        pressures=((2.**-53,), (-2.**-53,)),
    )
    assert observed.cumulative_nodal_area == ((0,), (F(1, 2**53),), (0,))
    assert observed.feasible and observed.visible_closed and observed.conditional_carried_cycle
    assert observed.witness_sequence.endpoint == observed.witness_initial
    assert tuple(step.after.epi for step in observed.witness_sequence.steps) == ((successor,), (.5,))


def test_zero_mean_nonzero_coordinate_area_is_not_a_carried_cycle():
    observed = _word(epi_states=((.75, .75),) * 2, timesteps=(1.,),
                     capacities=((1., 1.),), pressures=((2.**-55, -2.**-55),))
...
def test_heterogeneous_capacity_enters_each_exact_nodal_area():
    observed = _word(epi_states=((.5, .5), (.625, .4375)), timesteps=(.25,),
                     capacities=((2., 1.),), pressures=((.25, -.25),))
    assert observed.total_nodal_area == (F(1, 8), -F(1, 16))
    assert observed.zero_initial_carry_feasible
    assert observed.witness_sequence.endpoint.epi == (.625, .4375)


@pytest.mark.parametrize("h,capacity", [(0., (1.,)), (1., (0.,))])
...
def test_singleton_band_and_zero_area_give_one_exact_initial_state():
    observed = _word(epi_lower=.5, epi_upper=.5)
    cell = observed.coordinates[0]
...
def test_opposite_large_internal_areas_do_not_hide_an_infeasible_prefix():
    observed = _word(epi_states=((.5,),) * 3, timesteps=(1., 1.),
                     capacities=((1.,),) * 2, pressures=((1.,), (-1.,)))
    assert observed.total_nodal_area == (0,)
...
@pytest.mark.parametrize("changes,error", [
    ({"epi_states": [[.5], [.5]]}, TypeError),
    ({"epi_states": ((.5,), [.5])}, TypeError),
    ({"epi_states": ((.5,), (1,))}, TypeError),
    ({"epi_states": ((.5,), (True,))}, TypeError),
    ({"epi_states": ((.5,), (math.nan,))}, ValueError),
    ({"epi_states": ((.5,), (math.inf,))}, ValueError),
    ({"epi_states": ()}, ValueError),
    ({"epi_states": ((.5,),)}, ValueError),
    ({"epi_states": ((.5,), (.5, .5))}, ValueError),
    ({"epi_states": ((.5,), ())}, ValueError),
    ({"epi_states": ((.5,), (.01,))}, ValueError),
    ({"timesteps": []}, TypeError),
    ({"timesteps": ()}, ValueError),
    ({"timesteps": (1,)}, TypeError),
    ({"timesteps": (False,)}, TypeError),
    ({"timesteps": (-.25,)}, ValueError),
    ({"timesteps": (math.inf,)}, ValueError),
    ({"capacities": [[1.]]}, TypeError),
    ({"capacities": ((-1.,),)}, ValueError),
    ({"capacities": ((F(1),),)}, TypeError),
    ({"capacities": ((1., 1.),)}, ValueError),
    ({"capacities": ()}, ValueError),
    ({"pressures": ((math.nan,),)}, ValueError),
    ({"pressures": ((0,),)}, TypeError),
    ({"pressures": ()}, ValueError),
    ({"epi_lower": 0.}, ValueError),
    ({"epi_upper": 1.1}, ValueError),
    ({"epi_lower": .75, "epi_upper": .25}, ValueError),
