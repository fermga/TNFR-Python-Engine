...
def _fine_oracle(reference, blocks, steps, epi):
    ...
    for step in steps:
        matrix = tuple(tuple(unit-step*value for unit, value in zip(row_i, row_a, strict=True))
                       for row_i, row_a in zip(identity, a, strict=True))
        matrices.append(matrix)
        states.append(tuple(value+step*force for value, force in zip(
            _apply(matrix, states[-1]), b, strict=True,
        )))
        elapsed.append(elapsed[-1]+step)
    rates = tuple(tuple(force-value for force, value in zip(b, _apply(a, state), strict=True))
                  for state in states)
...
def _assert_forecast(prediction, reference, blocks, steps, epi):
    geometry, states, rates, matrices, times = _fine_oracle(reference, blocks, steps, epi)
    ...
    assert prediction.convex_step_admissible == tuple(step <= reference.max_convex_step for step in steps)
    assert len(prediction.frames) == len(steps)+1
    for ordinal, (frame, state, rate, time) in enumerate(zip(prediction.frames, states, rates, times, strict=True)):
        ...
        assert frame.projected_epi == _apply(geometry["R"], state) == _apply(d, frame.reduced_state)
...
def test_full_coordinate_forecast_matches_independent_fine_affine_euler(steps):
    ...
    with patch("tnfr.physics.epi_memory.matrix_exponential", side_effect=AssertionError("no exponential")):
        prediction = predict_forced_support_realization_euler(reference, BLOCKS, steps, epi=epi)
...
def test_zero_duration_is_identity_with_unchanged_rates_and_no_hidden_time():
    ...
    reference = _full_reference()
    prediction = predict_forced_support_realization_euler(reference, BLOCKS, (F(0), F(0)))
...
def test_forcing_and_stored_pressure_remain_distinct_and_no_mean_is_removed():
    ...
    unforced = predict_forced_support_realization_euler(replace(reference, forcing=(F(0),)*4), BLOCKS, steps)
    ...
    assert mean(prediction.frames[-1].epi)-mean(reference.source.epi) == sum(steps)*reference.mean_drift
    assert mean(unforced.frames[-1].epi) == mean(reference.source.epi)


@pytest.mark.parametrize("size,blocks", ((4, ((0, 3), (1, 2))), (5, ((0, 4), (1, 2, 3)))))
def test_partial_observation_cannot_authorize_a_full_microstate_prediction(size, blocks):
    from tnfr.physics.epi_memory import predict_forced_support_realization_euler
...
@pytest.mark.parametrize("steps", (
    (), (F(-1, 4),), (True,), (float("nan"),), (float("inf"),), ("1/4",),
    (1j,), {F(1, 4), F(1, 2)}, {"first": F(1, 4)}, "0.25", F(1, 4),
))
...
def test_step_guard_requires_a_positive_builtin_integer(maximum):
    ...
    with pytest.raises((TypeError, ValueError)):
        predict_forced_support_realization_euler(_full_reference(), BLOCKS, (F(1, 4),), max_steps=maximum)
...
def test_infinite_step_iterable_is_bounded_and_no_partial_forecast_is_returned():
    ...
    with pytest.raises(ValueError):
        predict_forced_support_realization_euler(_full_reference(), BLOCKS, steps(), max_steps=2)
...
def test_exact_step_limit_and_rank_budget_are_respected():
    ...
    reference = _full_reference()
    prediction = predict_forced_support_realization_euler(reference, BLOCKS, (F(1, 4),)*2, max_steps=2)
    assert prediction.max_steps == len(prediction.steps) == 2
    with pytest.raises(ValueError):
        predict_forced_support_realization_euler(reference, BLOCKS, (F(1, 4),)*3, max_steps=2)
    with pytest.raises(ValueError, match="incomplete"):
        predict_forced_support_realization_euler(reference, BLOCKS, (F(1, 4),), max_rank_calls=1)
...
def test_reference_caches_are_rebuilt_and_invalid_coefficients_fail_closed():
    ...
    reference = _full_reference()
    corrupted = replace(reference, metric_weights=(F(99),)*4, strengths=(F(99),)*4,
                        relative_profile=(F(99),)*4, mean_drift=F(99))
    expected = predict_forced_support_realization_euler(reference, BLOCKS, (F(1, 4),))
    assert predict_forced_support_realization_euler(corrupted, BLOCKS, (F(1, 4),)) == expected
    with pytest.raises(TypeError):
        predict_forced_support_realization_euler(asdict(reference), BLOCKS, (F(1, 4),))
    with pytest.raises(ValueError):
        predict_forced_support_realization_euler(replace(reference, epi_weight=F(0)), BLOCKS, (F(1, 4),))
