...
def boundary():
    ...
    phases = tuple(((i % 8)*math.pi/4+math.pi) % (2*math.pi)-math.pi for i in range(16))
    state = {"nodes": nodes, "edges": tuple((u, v, {"weight": 1.0}) for u, v in graph.edges),
             "epi": (0.25,)*16, "capacity": (1.0,)*16, "pressure": (0.1,)*16, "phase": phases}
    before = {"state": state, "ordered_neighbors": tuple((n, tuple(graph.neighbors(n))) for n in nodes)}
    gains = (.05, .1)
    output = audit._evaluate(before, gains, dict(zip(nodes, nodes, strict=True)))
    after = {"state": {**state, "phase": output["phase"]},
             "graph_attributes": {key: ("binary64", value.hex())
                                  for key, value in zip(("PHASE_K_GLOBAL", "PHASE_K_LOCAL"), gains, strict=True)}}
    return {"boundary": "coordinate_global_local_phase", "outcome": "completed", "before": before, "after": after}, pairs


def test_exact_taylor_zero_parity_and_independent_high_precision_values():
    assert audit.trig_bounds(0) == ((Fraction(1), Fraction(1)), (Fraction(0), Fraction(0)))
    with localcontext() as context:
        ...
        for value in (Fraction(1), Fraction(3), Fraction(7)):
            ...
            cosine = sum((-1)**k*x**(2*k)/Decimal(math.factorial(2*k)) for k in range(150))
            sine = sum((-1)**k*x**(2*k+1)/Decimal(math.factorial(2*k+1)) for k in range(150))
            for point, (low, high) in zip((cosine, sine), bounds, strict=True):
                assert Decimal(low.numerator)/Decimal(low.denominator) < point < Decimal(high.numerator)/Decimal(high.denominator)


@pytest.mark.parametrize("value", (True, "1", float("nan"), float("inf"), 9, complex(1, 0)))
...
def test_relative_pattern_separates_common_rotation_from_nonuniform_change():
    base = [0.0, .25, .5]
    common = audit.compare_phases(base, [.5, .75, 1.0])
    assert common["anchor_common_shift"] == pytest.approx(.5)
    assert common["max_relative_residual"] < 1e-15  # Numerical test, not a certified gauge tolerance.
    changed = audit.compare_phases(base, [.5, .875, 1.0])
    assert changed["relative_pattern_residual"][1] == pytest.approx(.125)
    assert changed["max_relative_residual"] > .1


def test_detached_audit_preserves_inputs_and_calls_no_native_step(boundary, monkeypatch):
    ...
    assert result["rotation"] == 1 and result["live_graph_writes"] == result["native_steps"] == 0
    ...
    moved, reordered = result["transported_numpy"]["initial"], result["canonical_order_numpy"]["initial"]
    assert moved["nodes"] != reordered["nodes"]
    assert dict(zip(moved["nodes"], moved["phase"], strict=True)) == dict(zip(reordered["nodes"], reordered["phase"], strict=True))
    ...
    for name in ("transported_numpy", "canonical_order_numpy"):
        ...
        for field in ("phi_s", "grad_phi", "curv_phi"):
            for node in row["before"]["state"]["nodes"]:
                assert observed["aligned_tetrad_fields"][field][node] == observed["tetrad"]["fields"][field][mapping[node]]
    assert result["ideal_resultant"]["exact_zero"] and "Exact phases" in result["ideal_resultant"]["hypothesis"]
    assert set(result["base_numpy"]["tetrad"]["fields"]) == {"phi_s", "grad_phi", "curv_phi", "xi_c"}


@pytest.mark.parametrize("mutation", ("endpoint", "outcome", "lineage", "neighbors", "order"))
def test_declared_boundary_and_transport_admission_rejects_malformed_inputs(boundary, mutation):
    ...
    if mutation == "endpoint":
        ...
        row["before"]["ordered_neighbors"] = tuple((n, ()) for n, _ in row["before"]["ordered_neighbors"])
    else:
        nodes = row["before"]["state"]["nodes"]
        with pytest.raises(ValueError):
            audit._detached_graph(row["before"], dict(zip(nodes, nodes, strict=True)), node_order=(nodes[0],)*16)
        return
    with pytest.raises(ValueError):
...
def _write_input(path, boundary):
    ...
    manifest = CoreExperimentManifest(
        claim_id="O1.b-generated-native-runtime-response", git_sha="a"*40, source_dirty=False,
        versions={"python": "test"}, graph_construction="Explicit portable synthetic boundary fixture",
        capacity_specification="positive", solver="synthetic fixture", timestep=.25, seed=17,
        result_status=ClaimStatus.MEASURED, operator_sequence=(), telemetry=("fixture",), controls=("synthetic",), artifacts=(str(path),))
    payload = {"manifest": manifest.to_dict(),
               "branches": [{"branch": name, "status": "executed", "native_trace": {"boundaries": [row]}}
...
def test_portable_pinned_input_and_cli_manifest_no_archive_dependency(boundary, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sys.argv", ["audit", "--input", str(source), "--expected-sha256", digest, "--output", str(output)])
    audit.main()
    ...
    monkeypatch.setattr("sys.argv", ["audit", "--input", str(source), "--output", str(source)])
    with pytest.raises(ValueError, match="must not overwrite"):
        audit.main()
    assert source.read_bytes() == before


@pytest.mark.skipif(not audit.INPUT.exists(), reason="Optional historical integration; portable controls do not need ignored evidence")
def test_pinned_actual_boundary_conditioning_is_not_a_claim_of_physical_symmetry_breaking():
    ...
    assert result["canonical_order_vs_numpy"]["max_relative_residual"] > .3
