...
@pytest.mark.parametrize("h", (0.0, -1.0, 1.0, 2.0))
def test_noncontracting_step_cannot_receive_a_strict_certificate(h):
    with pytest.raises(ValueError, match="strict C6 spatial contraction"):
        derive_c6_carried_contraction(_profile(1.0), timestep=h)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (0.5 - 2.0**-54, 0.5, 0.5 + 2.0**-53, 0.5, 0.5 - 2.0**-53, 0.5 + 2.0**-52),
    ),
)
def test_generated_steps_obey_every_uniform_defect_and_energy_bound(epi):
    ...
    for _ in range(2):
        ...
        assert all(
            abs(value) <= tube.product_error_bound
        ...
        assert all(
            abs(value) <= tube.rounding_bound for value in observed.rounding_defect
        )
        assert all(
            abs(value) <= tube.forcing_component_bound
            for value in observed.forcing_defect
        )
        assert (
            sum(value**2 for value in observed.centered_forcing_defect)
        ...
        affine_bound = q * before + F(
            1, 16
        ) ** 2 * tube.centered_forcing_norm_squared_bound / (1 - q)
...
def test_valid_initial_state_does_not_imply_the_whole_tube_fits_its_band():
    ...
    state = initialize_nodal_remainder(
        (0.375, 0.625) * 3, epi_lower=0.375, epi_upper=0.625
    )
    tube = derive_c6_carried_tube(profile, state=state, timestep=1 / 16)
...
def test_mean_budget_does_not_charge_the_zero_mean_carry_feedback():
    profile = _profile()
    state = initialize_nodal_remainder((0.5,) * 6, epi_lower=0.375, epi_upper=0.625)
    tube = derive_c6_carried_tube(profile, state=state, timestep=1 / 16)
...
def test_reference_slab_cannot_silently_admit_a_larger_state_band():
    state = initialize_nodal_remainder((0.5,) * 6, epi_lower=0.25, epi_upper=0.75)
