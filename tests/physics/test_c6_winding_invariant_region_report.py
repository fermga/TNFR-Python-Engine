...
def inputs():
    paths = tuple(
        ARTIFACTS / name
        for name in (campaign.INPUT_NAME, campaign.RELAY_NAME, campaign.HISTORICAL_NAME)
    )
...
def test_forward_envelope_reuses_actual_lineage_without_advancing_it(
    forward_report, inputs
):
    result = forward_report
    candidate = result["B49_profile_candidate"]
    parent = json.loads(inputs[0])
    assert (
        candidate["actual_endpoint_mask"] == 57 and len(candidate["visible_rows"]) == 64
    )
    assert (
        campaign._payload(result["source"]["retained_B47_endpoint"])
        == parent["B47_conditional_first_exit"]["endpoint"]
    )
...
def test_actual_forward_layers_tighten_but_leave_outgoing_facets(forward_report):
    ...
    for flag in (
        "indefinite_trapping_certified",
        "whole_band_exit_certified",
        "future_runtime_certified",
        "all_correlated_candidates_excluded",
    ):
...
def test_forward_envelope_preserves_input_and_discards_partial_layer(inputs):
    ...
    assert (
        len(proof["iterations"]) == 1
        and proof["initial_zones"] == proof["retained_zones"]
    )
    assert proof["status"] == "resource_limit"
    json.dumps(campaign._payload(result), allow_nan=False)


def test_forward_cli_uses_distinct_claim_and_preserves_lineage(
    tmp_path, monkeypatch, inputs, forward_report
):
    ...
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_forward_envelope",
    ...
    assert tuple(item["sha256"] for item in result["input_evidence"]) == tuple(
        hashlib.sha256(raw).hexdigest() for raw in inputs
    )
...
def test_candidate_is_derived_from_profile_and_keeps_actual_endpoint(report, inputs):
    ...
    assert tuple(candidate["visible_rows"][57]) == tuple(
        parent["B47_conditional_first_exit"]["endpoint"]["epi"]
    )
    for target, pair in zip(candidate["exact_targets"], candidate["coordinate_pairs"]):
        assert F(pair[0]) < target < F(pair[1])
    assert (
        campaign._payload(report["source"]["retained_B47_endpoint"])
        == parent["B47_conditional_first_exit"]["endpoint"]
    )
...
def test_actual_pressure_has_exact_negative_quadratic_direction(report):
    ...
    value = sum(
        (
            vector[i] * metric[i] * matrix[i][j] * vector[j]
            for i in range(6)
            for j in range(6)
        ),
        F(0),
    ...
    assert (
        value
        == control["negative_quadratic_value"]
        == -F(4124249100202121, 274949940013475)
    )
    assert control["mixed_bounds"] == (
        (0, 1),
        (0, 0),
        (-8, 0),
        (-8, 0),
    ...
    assert any(
        control["mixed_remainders"][mask][i] for mask in range(64) for i in range(6)
    )
...
def test_positive_dual_identity_binds_actual_nonlinear_rows(report):
    ...
    oriented = sum(
        (
            weight
            * (1 if mask >> node & 1 else -1)
            * control["exact_nodal_areas"][mask][node]
            / unit
            / scale
            for weight, (mask, node) in zip(weights, control["positive_dual_support"])
        ),
        F(0),
    )
...
def test_complete_descents_preserve_state_but_do_not_prove_future(report):
    ...
    assert proof["coordinate_spacings"] == tuple(
        F(value, 2**113) for value in (1, 1, 4, 8, 8, 8)
    )
    assert proof["exclusion_step_bound"] is None
    for flag in (
        "conditional_invariance_certified",
        "conditional_boundedness_certified",
        "origin_exit_certified",
        "future_runtime_certified",
        "asymptotic_convergence_certified",
    ):
        assert proof[flag] is False
    assert (
        report["new_conditional_trajectory_steps"]
        == report["new_live_graph_steps"]
        == 0
    )
    assert (
        not report["indefinite_trapping_certified"]
        and not report["candidate_exit_certified"]
    )
    assert not report["all_correlated_candidates_excluded"]
    assert (
        not report["whole_band_exit_certified"]
        and not report["future_runtime_certified"]
    )
...
def test_cli_retains_all_three_input_hashes_and_current_source(
    tmp_path, monkeypatch, inputs, report
):
    paths = tuple(tmp_path / name for name in ("b47.json", "b46.json", "b43.json"))
    for path, raw in zip(paths, inputs):
        path.write_bytes(raw)
    ...
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_invariant_region",
        lambda *_a, **_k: deepcopy(report),
    )
    monkeypatch.setattr(
        campaign,
        "current_git_source_provenance",
...
def test_cli_rejects_changed_source_or_any_retained_input(
    tmp_path, monkeypatch, inputs, report, which
):
    paths = tuple(tmp_path / f"{i}.json" for i in range(3))
    for path, raw in zip(paths, inputs):
        path.write_bytes(raw)
...
def predecessor_inputs(inputs):
    paths = tuple(
        ARTIFACTS / name
        for name in (
            "c6_winding_forward_envelope.json",
            "c6_winding_forward_envelope.validation.json",
        )
    )
    if not all(path.is_file() for path in paths):
        pytest.skip(
            "the retained B49 envelope and selected exact witnesses are unavailable"
        )
...
def test_predecessor_report_preserves_lineage_and_rebinds_selected_outward_points(
    predecessor_report, predecessor_inputs
):
    ...
    assert (
        binding["envelope_sha256"] == hashlib.sha256(predecessor_inputs[3]).hexdigest()
    )
    assert (
        binding["witness_sha256"] == hashlib.sha256(predecessor_inputs[4]).hexdigest()
    )
    ...
    assert tuple(item["mask"] for item in records) == (
        8,
        12,
        16,
        20,
    ...
    original_sum = sum(
        F(x) + F(r) for x, r in zip(origin["epi"], origin["remainder"], strict=True)
    )
    for record in records:
        ...
        assert (
            sum(
                F(x) + F(r)
                for x, r in zip(target["epi"], target["remainder"], strict=True)
            )
            == original_sum
        )
...
def test_default_predecessor_outcomes_remain_point_and_domain_specific(
    predecessor_report,
):
    ...
    assert {
        item["mask"]: item["domains"]["retained_R259"]["past_exclusion_depth"]
        for item in records
        if item["domains"]["retained_R259"]["status"] == "past_excluded"
    } == {
        8: 12,
        12: 9,
        16: 3,
    ...
    assert {
        item["mask"]: item["domains"]["original_cube"]["past_exclusion_depth"]
        for item in records
        if item["domains"]["original_cube"]["status"] == "past_excluded"
    } == {
        20: 42,
        49: 91,
        51: 120,
    ...
    assert {
        item["mask"]: item["domains"]["original_cube"]["completed_depth"]
        for item in records
        if item["domains"]["original_cube"]["status"] == "resource_limit"
    } == {
        8: 11,
        12: 9,
        16: 32,
    ...
    remaining = next(
        item["domains"]["retained_R259"] for item in records if item["mask"] == 28
    )
    assert (
        remaining["status"] == "resource_limit" and remaining["completed_depth"] == 21
    )
    for record in records:
        for proof in record["domains"].values():
            ...
            assert proof["frontier_counts"] == tuple(
                len(layer["states"]) for layer in proof["layers"]
            )
            ...
            assert (
                not proof["conditional_boundedness_certified"]
                and not proof["future_runtime_certified"]
            )
            if proof["status"] == "past_excluded":
                assert proof["frontier_counts"][-1] == 0
                assert proof["past_exclusion_depth"] == proof["completed_depth"]
                assert (
                    proof["maximum_compatible_past_depth"]
                    == proof["completed_depth"] - 1
                )
        ...
        assert record["selected_point_cannot_cause_first_cube_exit"] == (
            record["mask"] != 28
        )
    for flag in (
        "selected_points_are_actual_reached_states",
        "all_outgoing_facets_excluded",
        "indefinite_trapping_certified",
        "whole_band_exit_certified",
        "future_runtime_certified",
    ):
    ...
    assert (
        predecessor_report["new_conditional_trajectory_steps"]
        == predecessor_report["new_live_graph_steps"]
        == 0
    )
...
def test_predecessors_reject_altered_retained_B49_data(
    predecessor_inputs, cached_forward, mutation
):
    ...
    if mutation == "carry":
        ...
        data["B49_relational_forward_envelope"]["retained_zones"][0]["bounds"][0][
            6
        ] += 1
    else:
        data["manifest"]["claim_id"] = "O3.a-C6-carried-invariant-region"
    altered = predecessor_inputs[:3] + (
        json.dumps(data).encode(),
        predecessor_inputs[4],
    )
    with pytest.raises(ValueError, match="retained B49|canonical reconstruction"):
        _analyze_predecessors(altered, max_depth=0)


def test_predecessors_reject_a_witness_file_for_different_B49_bytes(
    predecessor_inputs, cached_forward
):
...
def test_predecessor_cli_cannot_overwrite_any_of_its_five_inputs(
...
def test_region_report_binds_full_lineage_and_complete_RN_geometry(
    region_report, region_inputs
):
    ...
    assert (
        binding["complete_envelope_rebuilt"]
        and binding["clipped_forward_inclusion_verified"]
    )
    assert binding["envelope_sha256"] == hashlib.sha256(region_inputs[3]).hexdigest()
    state = result["source"]["retained_B47_endpoint"]
    assert (
        campaign._payload(state)
        == json.loads(region_inputs[0])["B47_conditional_first_exit"]["endpoint"]
    )
    assert result["source"]["historical_nodal_steps_replayed"] == 356
    assert result["source"]["previous_conditional_steps_replayed"] == 204
    assert not result["source"]["live_execution_seal_recreated"]
    ...
    for cell in geometry["complete_RN_cells"]:
        ...
        low = tuple(
            (
                (F(coordinate.first_grid_index, 2**3222) - initial)
                / geometry["grid_quantum"]
            ).__ceil__()
            for coordinate, initial in zip(coordinates, original, strict=True)
        )
        high = tuple(
            (
                (F(coordinate.last_grid_index, 2**3222) - initial)
                / geometry["grid_quantum"]
            ).__floor__()
            for coordinate, initial in zip(coordinates, original, strict=True)
        )
        ...
        minima.append(low)
        maxima.append(high)
    assert geometry["lower_grid"] == tuple(
        min(row[i] for row in minima) for i in range(6)
    )
    assert geometry["upper_grid"] == tuple(
        max(row[i] for row in maxima) for i in range(6)
    )


def test_region_report_covers_every_unsafe_slab_of_the_retained_domain(
    region_report, forward_report
):
    ...
    expected = {}
    for mask, (row, pressure) in enumerate(
        zip(source["epi_states"], source["pressures"], strict=True)
    ):
        added = tuple(
            F(source["timestep"]) * F(value) / geometry["grid_quantum"]
        ...
        for node in range(6):
            if -bounds[6][node] + added[node] < geometry["lower_grid"][node]:
                expected[(mask, node, "lower")] = int(
                    geometry["lower_grid"][node] - added[node] - 1
                )
            if bounds[node][6] + added[node] > geometry["upper_grid"][node]:
                expected[(mask, node, "upper")] = -int(
                    geometry["upper_grid"][node] - added[node] + 1
                )
    ...
    for facet, query in zip(facets, proof["queries"], strict=True):
        ...
        first, second = (
            (facet["node"], 6) if facet["direction"] == "lower" else (6, facet["node"])
        )
        bounds[first][second] = min(bounds[first][second], expected[identity])
        assert target["bounds"] == _dbm_close(bounds)


def test_region_report_excludes_24_whole_slabs_without_promoting_the_remaining_32(
    region_report,
):
    proof = region_report["B51_region_predecessors"]
    ...
    for facet, query in zip(facets, queries, strict=True):
        ...
        if query["status"] == "empty_complete_layer":
            ...
            assert (
                query["past_exclusion_depth"]
                == expected_depths[(facet["mask"], facet["node"])]
            )
            assert (
                query["iterations"][-1]["zone_count"] == 0
                and query["retained_zones"] == ()
            )
        else:
            assert query["completed_depth"] in (33, 34)
            assert query["past_exclusion_depth"] is None and query["retained_zones"]
    assert {
        (item["node"], item["direction"]): (item["total"], item["excluded"])
    ...
    assert (
        region_report["new_conditional_trajectory_steps"]
        == region_report["new_live_graph_steps"]
        == 0
    )
    json.dumps(campaign._payload(region_report), allow_nan=False)


def test_region_report_resource_interruption_preserves_complete_initial_regions(
    region_inputs, cached_forward
):
    ...
    assert (
        not result["all_first_exit_slabs_excluded"]
        and not result["indefinite_trapping_certified"]
    )


@pytest.mark.parametrize("mutation", ("carry", "zone", "claim"))
def test_regions_reject_altered_B49_evidence_before_excluding_whole_slabs(
    region_inputs, cached_forward, mutation
):
...
def test_regions_cli_uses_four_inputs_and_a_distinct_regional_claim(
    ...
    assert (
        captured["envelope_bytes"] == region_inputs[3]
        and captured["max_intersections"] == 250_000
    )
...
def test_regions_cli_cannot_overwrite_any_of_its_four_inputs(
    tmp_path, monkeypatch, index
):
    paths = tuple(tmp_path / f"region_input_{i}.json" for i in range(4))
    for path in paths:
...
def excursion_inputs(region_inputs, region_report):
    ...
    public["input_evidence"] = [
        dict(path=f"input_{i}.json", sha256=hashlib.sha256(raw).hexdigest())
...
def test_excursion_adds_eight_whole_slabs_from_the_same_primary_origin(
    excursion_report, excursion_inputs
):
    ...
    assert (
        result["B52_reconstruction"]["prior_region_sha256"]
        == hashlib.sha256(excursion_inputs[-1]).hexdigest()
    )
    assert (
        result["excluded_first_exit_slabs"] == 32
        and result["additional_first_exit_slabs_excluded"] == 8
    )
    assert (
        result["new_conditional_trajectory_steps"] == 56
        and result["new_live_graph_steps"] == 0
    )
    assert result["primary_proof_origin_remains_B47"] is True
    assert (
        campaign._payload(result["source"]["retained_B47_endpoint"])
        == json.loads(excursion_inputs[0])["B47_conditional_first_exit"]["endpoint"]
    )
    assert result["B52_excursion_exclusion"]["origin_path_within_domain_excluded"]
    assert {
        (g["node"], g["direction"]): g["excluded"] for g in result["B52_exit_groups"]
    } == {
        (0, "lower"): 0,
        (1, "upper"): 16,
        (2, "lower"): 8,
...
def test_excursion_coordinate_is_the_primitive_nodal_poisson_contrast():
    ...
    image = tuple(
        F(value) - F(weights[(i - 1) % 6] + weights[(i + 1) % 6], 2)
        for i, value in enumerate(weights)
    )
    assert image == (F(-3, 2), F(0), F(3, 2), F(0), F(0), F(0))


def test_excursion_short_resource_guard_does_not_promote_the_unfinished_prefix(
    excursion_inputs, cached_forward
):
    result = _analyze_excursion(excursion_inputs, max_prefix_steps=1)
    assert result["new_conditional_trajectory_steps"] == 1
    assert (
        result["excluded_first_exit_slabs"] == 24
        and result["additional_first_exit_slabs_excluded"] == 0
    )
    assert not result["B52_excursion_exclusion"]["origin_path_within_domain_excluded"]


@pytest.mark.parametrize(
    "mutation", ("geometry", "carry", "target", "positive", "input_hash", "claim")
)
def test_excursion_rejects_tampered_regional_evidence(
    excursion_inputs, cached_forward, mutation
):
    ...
    if mutation == "geometry":
        ...
        positive = next(
            q
            for q in prior["B51_region_predecessors"]["queries"]
            if q["origin_path_within_domain_excluded"]
        )
    ...
    with pytest.raises(ValueError, match="regional|prior regional"):
        _analyze_excursion(
            excursion_inputs[:-1] + (json.dumps(prior).encode(),), max_prefix_steps=1
        )
...
def test_excursion_cli_binds_five_inputs_and_the_derived_prefix(
    ...
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_excursion_exclusion",
        lambda *_a, **_k: deepcopy(excursion_report),
    )
    monkeypatch.setattr(
        campaign,
        "current_git_source_provenance",
    ...
    assert tuple(x["sha256"] for x in result["input_evidence"]) == tuple(
        hashlib.sha256(raw).hexdigest() for raw in excursion_inputs
    )
...
def mode_inputs(excursion_inputs, excursion_report):
    ...
    prior["input_evidence"] = [
        dict(path=f"mode_input_{i}.json", sha256=hashlib.sha256(raw).hexdigest())
        for i, raw in enumerate(excursion_inputs)
    ]
    candidates = (
        campaign.ROOT / "benchmarks/c6_winding_mode_excursion_candidates.json"
    ).read_bytes()
...
def mode_context(inputs, forward_report):
    with pytest.MonkeyPatch.context() as patch:
        ...
        return campaign._rebuild_c6_outgoing_regions(
            json.loads(inputs[0]),
            relay_bytes=inputs[1],
            historical_bytes=inputs[2],
            envelope_bytes=(
                ARTIFACTS / "c6_winding_forward_envelope.json"
            ).read_bytes(),
        )
...
def cached_mode_context(monkeypatch, mode_context, mode_closure_cache):
    ...
    monkeypatch.setattr(
        campaign, "_rebuild_c6_outgoing_regions", lambda *_a, **_k: mode_context
    )
    monkeypatch.setattr(
        c6_carried_viability, "derive_c6_carried_reachable_envelope", mode_closure_cache
    )
...
def test_modes_reverify_all_priors_and_exclude_four_whole_regions(
    mode_report, mode_inputs
):
    report = mode_report
    ...
    assert (
        audit["prior_regional_verification"]["prior_query_verification_intersections"]
        == 668
    )
    ...
    assert (
        report["excluded_first_exit_slabs"] == 36
        and report["additional_first_exit_slabs_excluded"] == 4
    )
    assert (
        report["new_conditional_trajectory_steps"]
        == report["new_live_graph_steps"]
    ...
    assert (
        campaign._payload(report["source"]["retained_B47_endpoint"])
        == json.loads(mode_inputs[0])["B47_conditional_first_exit"]["endpoint"]
    )
    assert {
        (r["node"], r["direction"]): r["excluded"] for r in report["B53_exit_groups"]
    } == {
        (0, "lower"): 0,
        (1, "upper"): 16,
        (2, "lower"): 8,
...
def test_modes_use_complete_origin_injected_hulls_and_exact_per_target_bounds(
    mode_report,
):
    ...
    assert (
        before["iterations"][-1]["ordinal"] == 265 and before["intersections"] == 232802
    )
    ...
    for envelope in (before, after):
        ...
        assert (
            envelope["domain_confined_paths_covered"]
            and envelope["clipped_forward_inclusion_certified"]
        )
...
def test_modes_reject_parent_objects_not_matching_the_retained_bytes(mode_inputs):
    ...
    with pytest.raises(ValueError, match="unchanged retained B47"):
        campaign.analyze_c6_winding_mode_excursions(
            parent,
            parent_bytes=mode_inputs[0],
            relay_bytes=mode_inputs[1],
            historical_bytes=mode_inputs[2],
            envelope_bytes=mode_inputs[3],
            region_bytes=mode_inputs[4],
            excursion_bytes=mode_inputs[5],
            candidates_bytes=mode_inputs[6],
        )


def test_mode_integer_normalization_uses_one_positive_scale_for_all_coordinates(
    mode_inputs, mode_context
):
    fresh, _profile, _state, _zones, _geometry, facets, _targets = mode_context
    candidates = campaign._c6_mode_candidates(
        mode_inputs[-1],
        facets,
        fresh["B49_relational_forward_envelope"]["epi_states"],
    )
    for original, (_index, _active, weights, offsets, scale) in zip(
        json.loads(mode_inputs[-1])["candidates"], candidates, strict=True
    ):
        assert scale > 0
        assert weights + offsets == tuple(
            F(value) * scale for value in original["weights"] + original["offsets"]
        )


@pytest.mark.parametrize("mutation", ("carry", "positive_count", "target", "claim"))
def test_modes_reconstruct_b52_instead_of_trusting_its_claims(
    mode_inputs, cached_mode_context, mutation
):
...
def test_modes_changed_candidate_coordinates_cannot_inherit_success(
    mode_inputs, cached_mode_context
):
...
def test_modes_resource_guard_retains_only_complete_outer_domains(
    mode_inputs, cached_mode_context
):
    result = _analyze_modes(mode_inputs, max_intersections=1, max_prefix_steps=1)
    for name in (
        "B53_origin_injected_reachable_envelope",
        "B53_post_exclusion_reachable_envelope",
    ):
        proof = result[name]
...
def test_modes_cli_binds_seven_inputs_without_promoting_clipped_closure(
    tmp_path, monkeypatch, mode_inputs, mode_report
):
    ...
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_mode_excursions",
        lambda *_a, **_k: deepcopy(mode_report),
    )
    monkeypatch.setattr(
        campaign,
        "current_git_source_provenance",
    ...
    assert tuple(x["sha256"] for x in result["input_evidence"]) == tuple(
        hashlib.sha256(raw).hexdigest() for raw in mode_inputs
    )
    assert (
        result["new_conditional_trajectory_steps"] == 0
        and not result["indefinite_trapping_certified"]
    )
...
def test_modes_cli_rejects_changed_source_or_any_input(
    tmp_path, monkeypatch, mode_inputs, mode_report, which
):
    paths = tuple(tmp_path / f"mode_{i}.json" for i in range(7))
    for path, raw in zip(paths, mode_inputs, strict=True):
        path.write_bytes(raw)
