...
def _admitted():
    ...
    pair = study.admission._pair(
        reader._admit_reset(left, reference), reader._admit_reset(right, reference)
    )
...
def test_real_manifest_and_complete_dataclass_report_are_serializable(
    tmp_path, monkeypatch
):
    admitted = _admitted()
    retained = tmp_path / "retained.json"
    retained.write_text("{}", encoding="utf-8")
    admitted["historical_inputs"] = {
        "synthetic": {
            "path": str(retained),
    ...
    monkeypatch.setattr(
        study.admission, "load_comparable_witnesses", lambda *a, **k: admitted
    )
    monkeypatch.setattr(
    ...
    assert result["admission"]["witnesses"]["synthetic"]["control"]["observation"][
        "snapshot"
    ]
    assert (
        result["native_calls"]
...
def test_input_overwrite_refused_before_reading(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv", ["criterion", "--reset-input", str(path), "--output", str(path)]
    )
