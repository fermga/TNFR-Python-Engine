...
def cases():
    records = tuple(response.run_structural_response_case(name) for name in response.CASES)
    comparison = response.compare_structural_response(records)
    print("RESPONSE_METRICS", {
        "errors": {item["case"]: float(item["final_original_pattern"]["error_variance"])
                   for item in records},
        "conditional_limits": {
            item["case"]: float(item["conditional_fixed_model_limit"]["pattern"]["error_variance"])
            for item in records
        },
        "event_changes": {
            item["case"]: [(event["requested_glyph"], float(event["original_pattern_change"]))
                           for event in item["events"]] for item in records
        },
...
def _target(record):
    ...
    return derive_forced_support_balance(
        ...
        epi_weight=target["epi_weight"], forcing=target["forcing"],
    )
...
def test_all_branches_retain_the_actual_positive_capacity_pre_sha_checkpoint(cases):
    ...
    for record in cases.values():
        ...
        for field in ("metric_weights", "relative_profile", "epi_weight", "forcing"):
            assert record["original_target"][field] == checkpoint["original_reference"][field]
...
def test_actual_default_val_writes_epi_and_capacity_and_has_a_fixed_target_budget(cases):
    ...
    assert event["resolved_factors"]["VAL_scale"] == canonical_glyph_factor_defaults()["VAL_scale"]
    ...
    assert all(raw["epi"][i] == before["epi"][i] for i in range(len(before["nodes"])) if i != index)
...
def test_all_actual_events_use_admitted_words_and_full_signed_budgets(cases, name):
    ...
    for event in record["events"]:
        ...
        assert event["raw_after_event"]["state"]["glyph_history"][event["target"]][-1] == event["requested_glyph"]
        budget = event["full_event_budget"]
        assert budget["before"]["snapshot"] == event["before_forcing_capture"]["snapshot"]
        ...
        for label in ("variance", "dirichlet"):
            jump, reset = budget[f"{label}_jump_budget"], budget[f"{label}_reset_budget"]
            assert jump["identity_residual"] == reset["identity_residual"] == 0
            assert jump["energy_change"] == jump["cross_term"] + jump["quadratic_term"]
            assert reset["energy_change"] == sum(reset[key] for key in (
                "metric_term", "reference_cross_term", "reference_quadratic_term",
            ))
            assert budget[f"{label}_change"] == jump["energy_change"] + reset["energy_change"]
            assert budget[f"{label}_identity_residual"] == 0


@pytest.mark.parametrize("name", response.CASES)
def test_flow_is_bound_to_actual_endpoints_with_fixed_refreshed_coefficients(cases, name):
    record = cases[name]
    previous = record["events"][-1]["after_refresh"]["state"] if record["events"] else record["initial"]["state"]
...
def test_variance_telescope_never_substitutes_dirichlet_energy(cases, name):
    ...
    b_flow_change = sum(segment["regime_step_budget"]["relative_energy_budget"]["energy_change"]
                        for segment in record["segments"])
...
def test_final_and_conditional_limit_readouts_keep_the_original_target(cases, name):
    ...
    expected = observe_forced_support_pattern(target, nodes=final["nodes"], epi=final["epi"])
    ...
    model_limit = observe_forced_support_pattern(
        target, nodes=current["source"]["nodes"], epi=current["relative_profile"],
    )
    ...
    assert "not an asymptotic assertion" in record["conditional_fixed_model_limit"]["scope"]
...
def test_damage_benefit_gap_use_matched_fixed_target_endpoints(cases):
    ...
    u, p, f = (cases[name]["final_original_pattern"]["error_variance"] for name in response.CASES)
    ...
    assert comparison["damage"] == comparison["corrective_benefit"] + comparison["remaining_gap"]
...
@pytest.mark.parametrize("field", ("node_attributes", "configured_controls", "random_provenance"))
...
def test_live_candidate_refusal_does_not_execute_or_refresh(monkeypatch):
    ...
    monkeypatch.setattr(response, "validate_candidate", lambda *args, **kwargs: Refusal())
    ...
    following, event = response._event(
        nx.Graph(), "child", Expansion(), SimpleNamespace(index=1), object(), capture,
        refresh=True,
    )
...
def test_obstructed_branch_never_gets_a_recovery_classification():
    records = [{"case": name, "status": "controlled_obstruction"} for name in response.CASES]
    assert response.compare_structural_response(records)["status"] == "controlled_obstruction"
    with pytest.raises(ValueError, match="ordered U/P/F"):
        response.compare_structural_response(records[::-1])


@pytest.mark.parametrize("rp,rf,classification,fraction", (
...
def test_pure_comparison_classification_requires_positive_damage(
    ...
    records = [{
        "case": name, "status": "measured", "initial": {}, "original_target": {},
        "pre_feedback_materialized": {}, "events": [], "physical_elapsed_time": 3.0,
