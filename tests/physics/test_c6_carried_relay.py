...
from tnfr.dynamics._euler_kernel import (
    NodalRemainderState, _validate_nodal_remainder_state,
)
...
from tnfr.physics.c6_carried_relay import (
    _formula, derive_c6_carried_relay, observe_c6_carried_relay_exit,
)
...
def _origin():
    ...
    return NodalRemainderState(epi, tuple(F(a, 2**b) for a, b in terms), .375, .625)
...
def _closure(profile, state=None, timestep=.0625):
    return derive_c6_carried_closure(profile, state=_origin() if state is None else state, timestep=timestep)
...
def test_four_pressure_rows_have_exact_independent_opposite_responses(relay, profile):
    expected_offsets = ((-3, 0, 2, 0, 8, -5), (-4, 0, 2, 0, 8, -5),
                        (-3, 0, 2, -1, 8, -5), (-4, 0, 2, -1, 8, -5))
    assert relay.visible_rows == tuple(tuple(float(F(1, 2) + value * DELTA) for value in row)
                                       for row in expected_offsets)
    independent = tuple(_observe_rebuilt_c6_pressure_lattice(profile.lattice, row).pressure
                        for row in relay.visible_rows)
    assert relay.pressure_rows == independent
    exact = tuple(tuple(map(F, row)) for row in independent)
    assert all(exact[3][i] - exact[1][i] - exact[2][i] + exact[0][i] == 0 for i in range(6))
...
def test_actual_coefficients_keep_pressure_rounding_instead_of_ideal_half(relay):
    ...
    assert (first.decrement / M, first.increment / M) == (3039824576180305, 3558973984143090)
    assert (second.decrement / M, second.increment / M) == (1803052714421816, 4795745845901584)
    assert first.correction == (F(1), F(-3299399280161697, 6598798560323395), F(0), F(0), F(0),
                                F(-3299399280161696, 6598798560323395))
    assert second.correction == (F(0), F(0), F(-412424910020212, 824849820040425), F(1),
                                 F(-137474970006737, 274949940013475), F(0))
    ...
    assert tuple((value > 0) - (value < 0) for value in relay.drift) == (0, -1, 1, 0, -1, -1)


def test_complementary_ties_and_full_strip_outer_extents(relay):
    assert tuple((axis.lower_closed, axis.upper_closed) for axis in relay.axes) == ((False, True), (True, False))
    for axis in relay.axes:
        ...
        assert lower.lower < axis.facet - axis.decrement < axis.facet + axis.increment < upper.upper
        assert F(.375) <= axis.facet - axis.decrement < axis.facet + axis.increment <= F(.625)
...
def test_first_passive_exit_has_canonical_prefix_and_exact_nodal_telescope(relay, passage, profile):
    assert passage.exit_step == 7 and passage.exiting_nodes == (4,)
    assert tuple(relay.visible_rows.index(step.before.epi) for step in passage.steps) == (0, 1, 2, 0, 1, 0, 3)
    ...
    for ordinal, step in enumerate(passage.steps):
        ...
        assert step.pressure == _observe_rebuilt_c6_pressure_lattice(profile.lattice, step.before.epi).pressure
        ...
        if ordinal < passage.exit_step - 1:
            assert all(step.after.epi[node] == _origin().epi[node] for node in relay.nonrelay_nodes)
    ...
    assert tuple(value / M for value in area) == (
        -1482376352291950, -17082760244853979, 22807680901284836,
        576228119694088, -4451855519549880, -366916904283112,
    )
    assert passage.mean_area == F(1, 2**114) and sum(area, F(0)) / 6 == passage.mean_area
    assert tuple(b - a for a, b in zip(_origin().exact_epi, passage.exact_endpoint)) == area
...
def test_large_scalar_periods_are_exact_modular_identities_not_runtime_extensions(relay):
    periods = []
    for axis in relay.axes:
    ...
    for ordinal in (1, 7, *periods, math.lcm(*periods)):
        ...
        for index, axis in enumerate(relay.axes):
            ...
            assert (value - axis.initial_offset + ordinal * axis.decrement) == point.lower_visits[index] * axis.length
        assert not any(point.drift_identity_residual)
...
def test_exact_central_facet_arrival_obeys_both_nearest_even_rules(relay, profile, node):
    ...
    assert after.epi[node] == (axis.upper_visible if axis.lower_closed else axis.lower_visible)
...
def test_closed_upper_strip_endpoint_is_admitted_and_open_one_rejected(relay, profile):
    ...
    included = derive_c6_carried_relay(_closure(profile, replace(source, remainder=tuple(carry))))
    ...
    with pytest.raises(ValueError, match="outside a derived relay strip"):
        derive_c6_carried_relay(_closure(profile, replace(source, remainder=tuple(carry))))
    carry[second.node] -= GRID
    admitted = derive_c6_carried_relay(_closure(profile, replace(source, remainder=tuple(carry))))
...
def test_reversing_opposite_axis_order_preserves_the_physical_passage(relay, passage):
    ...
    assert reverse.drift == relay.drift and reverse.exit_deadlines == relay.exit_deadlines
    assert observed.steps == passage.steps and observed.exact_endpoint == passage.exact_endpoint
    assert tuple(point.lower_visits for point in observed.points) == tuple(point.lower_visits[::-1] for point in passage.points)
...
def test_band_failure_retains_exact_candidate_without_an_invalid_state(profile):
    ...
    assert observed.endpoint is None and len(observed.steps) == 1 and len(observed.points) == 3
...
def test_public_cache_tampering_is_rebuilt_before_the_deadline_or_pressure_is_used(relay, passage):
    forged = replace(relay, pressure_rows=((0.,) * 6,) * 4, drift=(F(0),) * 6,
                     base_increment=(F(0),) * 6, deadline=1, exit_deadlines=(None,) * 6,
                     axes=tuple(replace(axis, correction=(F(1),) * 6, decrement=F(1)) for axis in relay.axes))
...
def test_budget_must_cover_the_proved_deadline_even_when_observed_exit_is_earlier(relay, monkeypatch):
...
@pytest.mark.parametrize("budget", (0, -1, True, 10., F(10)))
...
@pytest.mark.parametrize("nodes,reason", (
    ((1, 4), "outside a derived relay strip"), ((2, 5), "strictly inward"),
))
...
def test_scope_and_original_carry_remain_unchanged(relay, passage):
    ...
    assert relay.initial_membership_verified and relay.conditional_product_relay_inclusion
