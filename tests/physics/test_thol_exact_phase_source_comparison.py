...
def study_input(evidence):
    ...
    boundary = next(r for r in branch["native_trace"]["boundaries"] if r["boundary"] == "coordinate_global_local_phase")
    nodes = branch["endpoint"]["state"]["nodes"]
    gains = (.125, .0625)
    # One synthetic fixture reference, not a historical producer or native step.
    boundary["before"]["state"]["phase"] = [float(k % 8)*math.pi/4 for k in range(16)]
    graph = phase_owner._detached_graph(boundary["before"], dict(zip(nodes, nodes, strict=True)))
    audit.coordinate_global_local_phase(graph, *gains, n_jobs=1, global_reduction="legacy")
    ...
    for record in (boundary["before"], boundary["after"], branch["endpoint"]):
        record["graph_attributes"].update(PHASE_K_GLOBAL=_literal(gains[0]), PHASE_K_LOCAL=_literal(gains[1]))
    observation, branch["endpoint_capture"] = fixtures._actual_capture(branch["endpoint"])
    identity = _payload(identity_owner.audit_branch(native["replayed_prior_reports"][0], branch))
    ...
    orders = (("base_numpy", dict(zip(nodes, nodes, strict=True)), nodes),
              ("transported_numpy", mapping, None), ("canonical_order_numpy", mapping, nodes))
    for name, relabel, order in orders:
        old[name]["initial"] = phase_owner._projection(phase_owner._detached_graph(boundary["before"], relabel, node_order=order))
...
def _inputs(evidence):
    ...
    admitted = source_owner.admit_evidence(native, phase, identity, native_sha256=digest)
    return admitted, phase["audit"]["source_boundary"], phase["audit"]["effective_gains"]


def test_three_coordinations_two_captures_fixed_inputs_and_nine_exact_rates(study_input, monkeypatch):
    ...
    calls, captures = [], []
    original, capture = audit.coordinate_global_local_phase, source_owner.capture_non_epi_forcing
    ...
    assert calls == [("legacy", nodes), (audit.VERSION, nodes), (audit.VERSION, tuple(reversed(nodes)))]
    ...
    assert result["comparison_context"]["archived_alternative_phase"] == admitted["phases"]["alternative"]
    assert tuple(map(F, result["coordinator_cases"][1]["aligned_output"])) == captures[1].phase
    assert captures[1].phase != admitted["phases"]["alternative"]
    assert study_input == saved
    first, second = captures
    snap = first.snapshot
    degree = tuple(sum((w for i, _, w in snap.conductance if i == k), F(0)) for k in range(len(nodes)))
    ...
    for row in result["source_comparison"]["regions"]:
        ...
        assert row["model_weighted_total_rate_difference"] == sum((degree[i]*delta[i] for i in ids), F(0))
        assert row["model_variance_rate_difference"] == sum((degree[i]*(snap.epi[i]-mean)*delta[i] for i in ids), F(0))
...
def test_boundary_and_gain_binding_fail_before_calls(study_input, monkeypatch, change):
    admitted, boundary, gains = _inputs(study_input)
    if change == "boundary":
        boundary["before"]["state"]["phase"][0] += .25
    else:
        gains[0] += .25
    monkeypatch.setattr(audit, "coordinate_global_local_phase", lambda *a, **k: pytest.fail("admission before call"))
    with pytest.raises(ValueError):
        audit.compare_versions(admitted, boundary, gains)


def test_legacy_mismatch_stops_after_one_call_without_pressure(study_input, monkeypatch):
    admitted, boundary, gains = _inputs(study_input)
    admitted["phases"]["baseline"] = tuple(F(9) for _ in admitted["phases"]["baseline"])
    monkeypatch.setattr(source_owner, "evaluate_admitted", lambda *a: pytest.fail("no captures after mismatch"))
...
def test_enumeration_input_obstruction_stops_before_pressure(study_input, monkeypatch, field):
    admitted, boundary, gains = _inputs(study_input)
    ...
    monkeypatch.setattr(source_owner, "evaluate_admitted", lambda *a: pytest.fail("no captures after obstruction"))
    with pytest.raises(audit.ComparisonAdmissionError, match=field):
        audit.compare_versions(admitted, boundary, gains)


def test_portable_cli_preserves_inputs_and_serializes_evidence(study_input, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(audit, "current_git_source_provenance", lambda *a: ("a"*40, False, None))
...
def test_cli_refuses_all_input_overwrites(study_input, tmp_path, monkeypatch, which):
    ...
    monkeypatch.setattr(audit, "run_study", lambda *a, **k: pytest.fail("no work before overwrite rejection"))
