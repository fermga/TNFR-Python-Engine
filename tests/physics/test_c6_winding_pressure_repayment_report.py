...
def test_one_analytic_boundary_ends_the_positive_sector_after_two_steps(report, parents):
    ...
    assert report["analysis_limits"] == {"boundary_budget": 1, "step_budget": 256, "maximum_steps": 256}
    assert (report["horizon"]["max_unchanged_steps"], report["horizon"]["first_exit_step"]) == (1, 2)
    assert report["horizon"]["first_exit_leaves_cell"] == (False, False, True, False, False, False)
    ...
    assert report["positive_sector"] == {
        "completed_steps": 2, "duration_exact_steps": 2, "duration_lower_bound_steps": 2,
        "termination_observed": True, "future_duration_open": False,
    }
    ...
    assert F(report["continuation"]["refreshed_pressure"]["pressure"][1]) == -F(128295757220873, 2**106)
    assert report["post_exit_steps_integrated"] == report["hypothetical_crossing_steps_integrated"] == 0


def test_complete_incoming_vector_matches_the_reconstructed_B27_to_B28_change(report, parents):
...
def test_two_actual_steps_are_canonical_and_keep_all_six_area_identities(report):
    ...
    for step, row in zip(continuation["steps"], continuation["net_prefixes"], strict=True):
        ...
        assert step["exact_increment"] == tuple(F(value) / 16 for value in step["pressure"])
        ...
        assert row["node1_first_side_restored"] is row["joint_zero"] is row["mean_zero"] is False
        previous = step["after"]
...
def test_held_pressure_needs_four_steps_but_only_two_are_valid_and_executed(report):
    ...
    assert node["crossing_in_prefix"] is node["exact_zero_in_prefix"] is node["crossing_is_exact_zero"] is False
    ...
    assert crossing["initially_joint_zero"] is crossing["joint_zero_in_prefix"] is crossing["joint_zero_for_all_steps"] is False
    assert crossing["pressure_provenance_certified"] is crossing["band_provenance_certified"] is False
...
def test_two_captured_pressure_levels_have_no_short_exact_scalar_return(report):
    ...
    assert result["gcd"] == gcd(result["negative_integer"], result["positive_integer"]) == 1
    ...
    assert result["minimum_negative_steps"] * F(result["negative_pressure"]) + result["minimum_positive_steps"] * F(result["positive_pressure"]) == 0
    assert result["negative_pressure"] == report["continuation"]["refreshed_pressure"]["pressure"][1]
...
def test_displayed_return_is_distinct_from_reconstructed_nodal_return(report):
    continuation = report["continuation"]
    assert continuation["displayed_matches_B27_reference"] == (True, True, True, False, False, False)
...
def test_gradient_and_inverse_itinerary_preserve_actual_carry(report):
    ...
    assert gradient["gradient_indices_before"][1] == 1 and gradient["gradient_indices_after"][1] == -1
    assert gradient["gradient_index_change"] == (0, -2, 4, -2, 0, 0)
    assert gradient["identity_residual"] == (0,) * 6
    assert gradient["gradient_index_change"] == tuple(a + b for a, b in zip(gradient["nodal_gradient_term"], gradient["remainder_gradient_term"], strict=True))
    ...
    assert itinerary["pressure_provenance_certified"] is itinerary["runtime_provenance_certified"] is False
...
def test_budget_one_censors_before_any_new_cell_replay(parents, monkeypatch):
    ...
    assert result["continuation"]["B27_nodal_area"] == result["source"]["incoming_B27_nodal_area"]
...
@pytest.mark.parametrize("change", ("claim", "scope", "baseline", "incoming_area", "carry", "pressure", "prefix",
                                    "cached_crossing", "false_provenance", "lineage", "source_weight"))
...
def test_cli_retains_three_source_hashes_and_separate_producer_manifest(tmp_path, monkeypatch, report):
    paths = tuple(tmp_path / f"input_{i}.json" for i in range(3))
    for path, source in zip(paths, INPUTS, strict=True):
        path.write_bytes(source.read_bytes())
    output = tmp_path / "result.json"
    monkeypatch.setattr(sys, "argv", ["repayment", "--input", str(paths[0]), "--parent-input", str(paths[1]),
                                      "--source-input", str(paths[2]), "--output", str(output)])
    monkeypatch.setattr(campaign, "analyze_c6_winding_pressure_repayment", lambda *args: deepcopy(report))
    ...
    for key, source in zip(("input_evidence", "parent_input_evidence", "source_input_evidence"), INPUTS, strict=True):
...
def test_cli_checks_parent_and_source_byte_lineage_before_analysis(tmp_path, monkeypatch, wrong_index):
    paths = tuple(tmp_path / f"input_{i}.json" for i in range(3))
