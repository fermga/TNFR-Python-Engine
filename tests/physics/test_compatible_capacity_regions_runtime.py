...
def cases():
    return {name: campaign.run_compatible_capacity_case(name) for name in campaign.CASES}


def _graph(snapshot):
    graph = nx.Graph()
    graph.add_nodes_from(snapshot["nodes"])
    graph.add_edges_from((i, j) for i, row in enumerate(snapshot["support_neighbors"]) for j in row)
    return graph
...
def test_full_pressure_support_is_connected_in_both_preparations(cases):
    ...
    assert cases["split_phase"]["initial_capture"]["phase"] == (F(0),) * 3 + (F(math.pi),) * 3
    assert cases["aligned_phase"]["initial_capture"]["phase"] == (0,) * 6


def test_split_u3_components_have_zero_capacity_energy_despite_the_pressure_bridge(cases):
    ...
    assert balance["component_means_before"] == balance["component_means_after"] == (1, 2)
...
def test_actual_all_target_stage_keeps_prediction_distinct_from_execution(cases, name):
    ...
    assert all(item["allowed"] and item["candidate"] == "UM" for item in event["admissions"])
    ...
    assert event["resolved_factors"]["UM_vf_sync"] == canonical_glyph_factor_defaults()["UM_vf_sync"]
    for node in range(6):
...
def test_split_primary_preservation_does_not_erase_the_raw_pressure_write(cases):
    ...
    initial, raw, refreshed = (
        record["initial_capture"], record["event"]["raw_capture"], record["post_um_capture"],
    )
    for field in ("nodes", "epi", "capacity", "conductance", "support_neighbors"):
        assert initial["snapshot"][field] == raw["snapshot"][field] == refreshed["snapshot"][field]
    ...
    for proposal in record["event"]["prediction"]["target_proposals"]:
        ...
        assert all(item["theta_before"] == item["theta_proposed"] for item in proposal["phase_proposals"])
...
def test_split_forced_profile_has_independently_derived_metric_and_center(cases):
    ...
    assert prediction["conditional_exact_limit"] == tuple(F(1, 2) + value for value in profile)
...
def test_aligned_capacity_update_uses_old_neighbors_not_new_k6_edges(cases):
    ...
    expected = (1, 1, F(1.0 + gamma * (4.0 / 3.0 - 1.0)),
                F(2.0 + gamma * (5.0 / 3.0 - 2.0)), 2, 2)
    ...
    assert record["event"]["capacity_exact_model_defect"] == tuple(a - b for a, b in zip(actual, ideal))
...
def test_split_capacity_preservation_is_not_nodal_stationarity(cases):
    record = cases["split_phase"]
    before, after = record["post_um_capture"]["snapshot"], record["final_capture"]["snapshot"]
    ...
    assert record["final_fixed_target_error"]["error_variance"] < record["initial_fixed_target_error"]["error_variance"]
