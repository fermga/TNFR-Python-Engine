...
def _fixture(*, zero_control=False, uniform_error=False):
    ...
    conductance = tuple(sorted((a, b, F(data["weight"])) for i, j, data in edges for a, b in ((i, j), (j, i))))
    support = tuple(tuple(sorted({j for i, j, _w in conductance if i == node})) for node in nodes)
    ...
    metric = tuple(sum(w for i, _j, w in conductance if i == node)/capacity[node] for node in nodes)
    original = SimpleNamespace(source=source, metric_weights=metric)

    def state(values, time):
        return {"nodes": list(nodes), "epi": list(values), "capacity": list(capacity), "time": time,
                "edges": deepcopy(edges), "hierarchy": deepcopy(hierarchy), "children": deepcopy(children)}

    birth_after = state(initial, F(1, 2))
    birth = {"parent_children": pairs, "before": {"nodes": tuple(range(8))}, "after": birth_after,
             "children": [{"parent": p, "child": c, "node_data": {"parent_node": p}} for p, c in pairs]}
    prior = {"prefix": {"birth": birth},
             "lineage": {"parent_children": pairs, "parents": tuple(range(8)), "children": tuple(range(8, 16)),
                         "nodes": nodes},
             "common_source": {"state": birth_after,
                               "node_attributes": [(i, {"sub_nodes": [i+8]} if i < 8 else {"parent_node": i-8})
                                                   for i in nodes]}}
    ...
    for branch_index, name in enumerate(audit.reset.window.BRANCHES):
        ...
        for k, time in enumerate(audit.TIMES):
            control = (F(3),)*16 if zero_control else tuple((1-F(k, 16))*x+F(k, 32) for x in initial)
            errors = (F(1, 4),)*16 if uniform_error else tuple(
                F(i+1, 32)*(1-F(k, 8)) if i >= 8 else F(0) for i in nodes)
            values = tuple(x+branch_index*d for x, d in zip(control, errors, strict=True))
            records.append({"state": state(values, time)})
        firsts.append(pool.pack({"branch": name, "status": "executed", "endpoint": records[0]}))
        steps = [pool.pack({"ordinal": k, "status": "executed", "before": records[k], "endpoint": records[k+1]})
                 for k in range(5)]
        branches.append({"branch": name, "steps": steps, "status": "completed", "completed_steps": 5,
                         "final_record": pool.pack(records[-1])})
        priors.append({"branch": name, **deepcopy(prior)})
    retained = {"first_step_replays": firsts, "branches": branches, "record_pool": pool.nodes}
...
def _mutate_endpoint(retained, branch, index, change):
    ...
    for b, item in enumerate(retained["branches"]):
        first = audit.reset.window.expand_record(retained["first_step_replays"][b], retained["record_pool"])
        steps = [audit.reset.window.expand_record(ref, retained["record_pool"]) for ref in item["steps"]]
...
def _oracle(values, weights):
    ...
    return mean, centered, sum(w*x*x for w, x in zip(weights, centered, strict=True))/2
...
def test_all_fixed_regions_times_and_independent_weighted_arithmetic():
    ...
    for region in result["regions"]:
        ...
        for row in region["endpoints"]:
            ...
            dm, dz, error = _oracle(tuple(b-a for a, b in zip(x, y, strict=True)), weights)
            cm, cz, variance = _oracle(x, weights)
            drift_mean, drift, drift_energy = _oracle(tuple(b-a for a, b in zip(first, x, strict=True)), weights)
            assert (row["paired"]["weighted_mean_offset"], row["paired"]["centered_epi_difference"],
                    row["paired"]["centered_H_energy"]) == (dm, dz, error)
            assert (row["control_mean"], row["control_centered_epi"], row["control_variance"]) == (cm, cz, variance)
            assert (row["control_mean_drift_from_start"], row["control_centered_drift_from_start"],
                    row["control_centered_drift_energy"]) == (drift_mean, drift, drift_energy)
    ...
    assert result["native_calls"] == result["forcing_capture_calls"] == result["coordination_calls"] == 0
...
@pytest.mark.parametrize("uniform,case", [(False, "zero_control_with_positive_error"),
...
def test_zero_control_contrast_keeps_ratio_unavailable(uniform, case):
    ...
    for region in result["regions"]:
        for row in region["endpoints"]:
            assert row["relative_error"] == {"available": False, "value": None, "zero_case": case}
...
@pytest.mark.parametrize("mutation,match", [
    ...
    (lambda record: record["state"]["edges"][0][2].__setitem__("weight", 7.0), "conductance"),
    (lambda record: record["state"]["edges"].pop(), "conductance"),
    (lambda record: record["state"]["nodes"].reverse(), "nodes"),
    (lambda record: record["state"].__setitem__("time", "9/4"), "physical endpoints"),
    (lambda record: record["state"]["epi"].__setitem__(0, float("nan")), "nonfinite|finite"),
...
def test_wrong_cached_metric_and_parentage_are_rejected():
    ...
    native["replayed_prior_reports"][1]["prefix"]["birth"]["children"][0]["node_data"]["parent_node"] = 7
...
def test_inputs_unchanged_and_no_runtime_or_capture_is_called(monkeypatch):
    ...
    def forbidden(*_args, **_kwargs):
        raise AssertionError("offline regional audit must not execute a runtime or capture")
...
def test_run_study_uses_pinned_loader_and_rechecks_bytes(tmp_path, monkeypatch):
    ...
    args = {"expected_window_sha256": bindings["window"]["sha256"],
