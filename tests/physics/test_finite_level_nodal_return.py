...
from tnfr.physics.nodal_remainder_pressure import (
    derive_finite_level_nodal_return, derive_two_level_nodal_return,
)
...
def test_actual_third_level_retains_the_two_level_modulus():
    ...
    assert result.integer_levels == (-1026366057766984, 5572432502556410, -4325765337928681)
...
@pytest.mark.parametrize("negative,positive", [
    (-2., 3.), (-6., 9.), (-.375, .25), (-.125, 1.5),
    (-2 * math.ulp(0.), 3 * math.ulp(0.)),
])
def test_two_level_payload_and_primitive_length_agree(negative, positive):
    old = derive_two_level_nodal_return(negative_pressure=negative, positive_pressure=positive)
    ...
    assert old.minimum_negative_steps * F(negative) + old.minimum_positive_steps * F(positive) == 0
...
def test_three_level_congruence_is_not_a_sufficient_short_return_condition():
    ...
    assert 7 * F(-4) + 2 * F(14) == 0  # A formal nine-entry multiset, not a runtime itinerary.


@pytest.mark.parametrize("levels", [(-2., 3., 8.), (-.5, .75, 2.), (-4., 14., 26.)])
def test_permutation_and_duplicate_levels_do_not_change_the_modulus(levels):
    ...
    for order in permutations(levels):
        ...
        assert result.arithmetic_zero_area_possible == expected.arithmetic_zero_area_possible


@pytest.mark.parametrize("levels", [(0.,), (0., -0., 0.)])
...
@pytest.mark.parametrize("level", [.5, -.75, math.ulp(0.)])
...
def test_unequal_one_sign_levels_are_impossible_even_with_nontrivial_congruence(sign):
    result = derive_finite_level_nodal_return(pressure_levels=tuple(float(sign * value) for value in (2, 5, 8)))
...
def test_selectable_zero_allows_every_length_without_balancing_nonzero_levels():
    result = derive_finite_level_nodal_return(pressure_levels=(6., 0., 15.))
...
def test_opposite_sign_possibility_is_not_the_average_of_listed_levels():
    result = derive_finite_level_nodal_return(pressure_levels=(-1., 100., 100.))
...
def test_subnormal_three_level_integerization_does_not_underflow():
    tiny = math.ulp(0.)
    result = derive_finite_level_nodal_return(pressure_levels=(-2 * tiny, 3 * tiny, 8 * tiny))
...
def test_every_short_formal_zero_word_obeys_the_necessary_congruence():
    ...
    result = derive_finite_level_nodal_return(pressure_levels=(-2., 3., 8.))
...
@pytest.mark.parametrize("levels,error", [
    ([], TypeError), ([-1., 1.], TypeError), ({-1., 1.}, TypeError),
    ((), ValueError), ((-1, 1.), TypeError), ((False, 1.), TypeError),
    ((F(-1), 1.), TypeError), ((FloatSubclass(-1), 1.), TypeError),
    ((-1., math.nan), ValueError), ((-math.inf, 1.), ValueError),
