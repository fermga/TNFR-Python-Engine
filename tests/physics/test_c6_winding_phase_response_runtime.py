...
def _finite_phase_response(direction, t, alpha):
    ...
    for i, value in enumerate(direction):
        ...
        real = (math.cos(value) + (math.cos(left) + math.cos(right)) / 2
                + root_three_half * (math.sin(left) - math.sin(right)))
        imag = (math.sin(value) + (math.sin(left) + math.sin(right)) / 2
                + root_three_half * (math.cos(right) - math.cos(left)))
        local_means.append(math.atan2(imag, real))
    coupled = tuple((1 - t) * value + t * (
        local_means[(i - 1) % 6] + local_means[i] + local_means[(i + 1) % 6]
    ) / 3 for i, value in enumerate(direction))
    return coupled, tuple((1 - alpha) * value + alpha * (
        coupled[(i - 1) % 6] + coupled[(i + 1) % 6]
    ) / 2 for i, value in enumerate(coupled))


def test_five_declared_inputs_keep_canonical_defaults_and_one_null(cases):
    assert campaign.CASES == (("null", 0.0), ("k1", 2.0**-12), ("k1", -2.0**-12),
                              ("k3", 2.0**-12), ("k3", -2.0**-12))
    ...
    for (mode, epsilon), record in cases.items():
        ...
        assert record["declared_tangent"] == tuple(F(epsilon) * v for v in campaign.DIRECTIONS[mode])
        assert capture["phase"] == tuple(F((base + epsilon * float(v)) % math.tau)
                                         for base, v in zip(campaign.BASE_PHASE, campaign.DIRECTIONS[mode]))
        assert record["word"]["names"] == ("coupling", "coherence", "silence")
        assert record["word"]["string_validator_passed"] and record["word"]["instance_validator_passed"]
        ...
        assert controls.get("UM_SYNC_VF", True) and controls.get("UM_STABILIZE_DNFR", True)
        assert controls.get("UM_CANDIDATE_COUNT", 0) == 0
        assert record["um"]["resolved_factors"]["UM_theta_push"] == defaults["UM_theta_push"]
        assert record["um"]["resolved_factors"]["UM_vf_sync"] == defaults["UM_vf_sync"]
        assert record["il"]["resolved_factors"]["IL_dnfr_factor"] == defaults["IL_dnfr_factor"]
        assert all(p["phase"]["coefficient"] == DEFAULT_PHASE_LOCKING_COEFFICIENT
...
def test_all_measured_cycles_keep_oriented_winding_and_nonedge_exclusion(cases, case):
    ...
    for capture, retained in boundaries:
        ...
        assert certificate.u3_admissible and certificate.quantization_residual <= 8 * math.ulp(1.0)
        ...
        assert retained["certificate"]["minimum_u3_margin"] == certificate.minimum_u3_margin
        assert retained["certificate"]["minimum_branch_margin"] == certificate.minimum_branch_margin
        reverse = certify_phase_winding(graph, tuple(reversed(range(6))), phase_gate=gate)
        assert reverse.is_defined and reverse.winding == -1
        assert reverse.minimum_branch_margin == pytest.approx(certificate.minimum_branch_margin)
        nonedges = tuple((i, j) for i in range(6) for j in range(i + 1, 6) if not graph.has_edge(i, j))
        margins = tuple(abs(angle_diff(float(capture["phase"][i]), float(capture["phase"][j]))) - gate
                        for i, j in nonedges)
        ...
        assert support["balance"]["is_fixed"] and support["balance"]["energy_before"] == 0


@pytest.mark.parametrize("case", campaign.CASES)
def test_shared_observed_stages_bind_actual_glyphs_and_c6_readout(cases, case):
    record = cases[case]
    assert inspect.signature(antipodal._observed_stage).parameters["phase_readout"].default is antipodal._phase_readout
    for name, glyph, history in (("um", "UM", ("UM",)), ("il", "IL", ("UM", "IL"))):
        ...
        assert event["stage_result"]["glyph"] == glyph and event["stage_result"]["nodes_processed"] == 6
        ...
        assert "centered_energy" in event["phase_after"] and "bridge" not in event["phase_after"]
    ...
    assert all(p["link_candidates"] == () and len(p["phase_proposals"]) == 3
               for p in prediction["target_proposals"])
    for node in range(6):
        contributors = [p for target in prediction["target_proposals"] for p in target["phase_proposals"]
                        if p["node"] == node]
...
def test_recorded_il_readiness_is_checked_again_without_executing(cases):
    for record in cases.values():
        event = record["il"]
        graph = _recorded_graph(event["before_capture"], event["before"]["configured_controls"])
        ...
        for item in readiness:
            assert item["passed"]
            assert validate_coherence_strict(graph, item["node"], emit_warnings=False) is None
            assert item["warnings"] == coherence_precondition_warnings(graph, item["node"])


@pytest.mark.parametrize("case", NONZERO)
def test_finite_mode_response_and_centered_gain_have_independent_predictions(cases, case):
    record = cases[case]
    mode, epsilon = case
    ...
    multiplier = ((1 - exact_t / 2) * (1 - exact_alpha / 2) if mode == "k1"
                  else (1 - exact_t) * (1 - 2 * exact_alpha))
    tangent = record["tangent_observation"]
    assert tangent["after_coherence"] == tuple(multiplier * value for value in record["declared_tangent"])
    gain = record["observed_centered_energy_gain"]
    assert 0 < gain < 1
    assert gain == record["phase_after"]["centered_energy"] / record["phase_before"]["centered_energy"]
    ...
    if mode == "k3":
        ...
        expected = tuple(float(multiplier * value) for value in record["declared_tangent"])
        assert il_lift == pytest.approx(expected, rel=0, abs=ANGLE_TOLERANCE)
        assert max(abs(float(v)) for v in record["observed_minus_linear"]) < ANGLE_TOLERANCE
...
def test_neutral_rotation_null_residue_and_linear_defects_are_not_erased(cases):
    for record in cases.values():
        for phase in (record["phase_before"], record["phase_after"]):
            ...
            assert sum(value**2 for value in lift) / 2 == phase["centered_energy"] + 3 * mean**2
        assert record["preparation_lift_residual"] == tuple(
            a - b for a, b in zip(record["phase_before"]["represented_lift"], record["declared_tangent"])
        )
    ...
    assert all(abs(float(v)) < ANGLE_TOLERANCE for v in null["realized_phase_pressure_pi_scaled"])
...
def test_midpoint_phase_pressure_is_bound_to_actual_endpoint_and_forcing(cases, case):
    ...
    midpoint = tuple((lift[(i - 1) % 6] + lift[(i + 1) % 6]) / 2 - lift[i] for i in range(6))
    ...
    assert record["midpoint_pressure_residual"] == tuple(a - b for a, b in zip(scaled, midpoint))
    assert max(abs(float(v)) for v in record["midpoint_pressure_residual"]) < ANGLE_TOLERANCE
    assert record["linear_pressure_residual"] == tuple(
        a - b for a, b in zip(scaled, record["tangent_observation"]["phase_pressure_pi_scaled"])
    )
...
@pytest.mark.parametrize("case", campaign.CASES)
def test_one_nodal_interval_holds_phase_and_binds_positive_capacity_evolution(cases, case):
    record = cases[case]
...
def test_terminal_closure_preserves_winding_without_claiming_autonomous_preparation(cases):
    for record in cases.values():
        ...
        assert all(a["allowed"] and a["candidate"] == "SHA" for a in closure["admissions"])
        ...
        graph = _recorded_graph(record["final_capture"], closure["after"]["configured_controls"])
        ...
        assert "binary64 asymptotic convergence or future complete-word admission" in record["scope"]
...
def test_strict_json_keeps_exact_residuals_and_named_sha_infinity(cases):
    ...
    for original, row in zip(cases.values(), encoded["cases"], strict=True):
        assert row["midpoint_pressure_residual"] == [str(v) for v in original["midpoint_pressure_residual"]]
        assert row["observed_minus_linear"] == [str(v) for v in original["observed_minus_linear"]]
        assert row["final_winding"]["certificate"]["winding"] == 1
        assert all(metric["time_to_collapse"] == {"numeric_kind": "positive_infinity"}
                   for metric in row["closure_after_measurement"]["actual_operator_metrics"])


@pytest.mark.parametrize("case", (("null", 0), ("null", 2.0**-12), ("k2", 2.0**-12), ("k1", math.nan)))
