...
def test_nonzero_carry_changes_exact_pressure_without_changing_visible_epi():
    result = _observe()
    a = F(1, 2**56)
    assert result.state.epi == (0.75, 0.75)
...
def test_regular_c6_common_capacity_cancels_both_arithmetic_readout_means():
    ...
    pattern = (1, 0, -1, 2, 0, -2)
    matrix = tuple(
        tuple(F(int((j - i) % 6 in (1, 5))) for j in range(6)) for i in range(6)
    )
    state = _state((0.75,) * 6, tuple(float(a * value) for value in pattern))
    result = _observe(
        state=state,
        conductance=matrix,
        capacity=(2.0,) * 6,
        stored_pressure=(0.125, -0.125, 0.25, 0.0, -0.25, 0.5),
    )
    expected = tuple(
        a * (F(value) - F(pattern[i - 1] + pattern[(i + 1) % 6], 2)) / 2
...
def test_irregular_p3_has_nonzero_arithmetic_but_zero_degree_weighted_shift():
    a = F(1, 2**56)
    result = _observe(
        state=_state((0.75,) * 3, (float(a), 0.0, 0.0)),
        conductance=P3,
        capacity=(1.0,) * 3,
        stored_pressure=(0.0,) * 3,
    )
    assert result.readout_shift == (a / 2, -a / 4, 0)
    assert (
        result.mean_pressure_readout_shift == result.mean_nodal_readout_shift == a / 12
    )
    assert result.reversible_weights == (1, 2, 1)
    assert (
        result.degree_weighted_pressure_readout_shift
        == result.reversible_mean_nodal_readout_shift
        == 0
    )
...
def test_heterogeneous_capacity_changes_reversible_weights_and_arithmetic_mean():
    a = F(1, 2**56)
    result = _observe(capacity=(2.0, 1.0))
    ...
    supplied = _observe(capacity=(2.0, 1.0), stored_pressure=(0.125, -0.125))
    assert sum(supplied.stored_pressure) == 0
    assert sum(
        nu * p for nu, p in zip(supplied.capacity, supplied.stored_pressure)
    ) / 2 == F(1, 16)
...
def test_nonuniform_visible_field_keeps_other_pressure_channels_in_residual():
    a = F(1, 2**58)
    result = _observe(
        state=_state((0.25, 0.5, 0.75), (float(a), 0.0, 0.0)),
        conductance=P3,
        capacity=(2.0, 1.0, 3.0),
        stored_pressure=(0.25, 0.125, -0.25),
    )
    ...
    assert result.stored_minus_reconstructed_reference == (
        F(1, 8) + a / 2,
        F(1, 8) - a / 4,
        -F(1, 8),
    )
...
@pytest.mark.parametrize("capacity", ((0.0, 1.0), (1.0, 0.0), (0.0, 0.0)))
...
def test_zero_epi_weight_has_zero_shift_without_discarding_stored_pressure():
    result = _observe(epi_weight=0.0, stored_pressure=(0.125, -0.5))
    ...
    assert (
        result.stored_minus_visible_reference
        == result.stored_minus_reconstructed_reference
        == (F(1, 8), -F(1, 2))
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"conductance": ((0.0, 1.0), (1.0, 0.0))}, TypeError),
        ({"conductance": ((F(0), F(1)), (F(2), F(0)))}, ValueError),
        ({"conductance": ((F(0), -F(1)), (-F(1), F(0)))}, ValueError),
        ({"conductance": ((F(0), F(0)), (F(0), F(1)))}, ValueError),
        ({"capacity": [1.0, 1.0]}, TypeError),
        ({"capacity": (1.0,)}, ValueError),
        ({"capacity": (True, 1.0)}, TypeError),
        ({"capacity": (F(1), 1.0)}, TypeError),
        ({"capacity": (-1.0, 1.0)}, ValueError),
        ({"capacity": (math.inf, 1.0)}, ValueError),
        ({"stored_pressure": ()}, ValueError),
        ({"stored_pressure": (0.0,)}, ValueError),
        ({"stored_pressure": (1, 0.0)}, TypeError),
        ({"stored_pressure": (math.nan, 0.0)}, ValueError),
        ({"epi_weight": True}, TypeError),
        ({"epi_weight": 1}, TypeError),
        ({"epi_weight": -0.5}, ValueError),
...
@pytest.mark.parametrize(
    "remainder", ((F(1, 3), F(0)), (F(1, 2**3223), F(0)), (F(1, 2**52), F(0)))
)
...
def test_overridden_reconstruction_property_cannot_replace_shared_validation():
    ...
    original = _state()
    state = WrongReconstruction(
        original.epi, original.remainder, original.epi_lower, original.epi_upper
    )
    assert (
        _observe(state=state).epi_pressure_reconstructed
        == _observe().epi_pressure_reconstructed
    )
