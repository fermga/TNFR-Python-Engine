...
def test_safe_partition_preserves_canonical_origin_and_rechecks_the_proposed_cut(
    inputs, report
):
    parent, baseline = json.loads(inputs[13]), json.loads(inputs[12])
    assert report["source"] == parent["source"]
    assert (
        campaign._payload(report["B59_return_envelope"]["state"])
        == baseline["B57_return_envelope"]["state"]
    )
    assert (
        campaign._payload(report["B59_return_envelope"]["domain_zones"])
        == baseline["B57_return_envelope"]["domain_zones"]
    )
    assert report["B59_exclusion_proposals"] == tuple(
        baseline["B57_additional_first_exit_facets"]
    )
    assert report["B59_exclusion_proposals"] == (
        dict(mask=30, node=3, direction="upper"),
    )
    ...
    assert (
        report["B59_reconstruction"]["prior_cuts_sha256"]
...
def test_safe_partition_queries_keep_all_full_original_residual_targets(inputs, report):
    ...
    for item in queries:
        expected = next(
            query["query"]["target_regions"]
            for query in baseline["B57_memory_queries"]
...
def test_incomplete_residual_query_graph_cannot_add_any_exclusion(report):
    ...
    assert all(
        not item["query"]["initialization_complete"]
        for item in report["B59_partition_queries"]
    )
    assert all(
        not item["query"]["origin_path_within_domain_excluded"]
        for item in report["B59_partition_queries"]
    )


def test_unverified_cut_proposal_has_explicit_no_partition_fallback(
    monkeypatch, inputs, previous
):
...
@pytest.mark.parametrize(
    "mutation",
    ("claim", "scope", "origin", "positive_count", "guard", "target", "equivalence"),
)
def test_safe_partition_reconstructs_untrusted_parent_claims(
...
def test_safe_partition_cli_binds_all_fourteen_inputs(
...
def test_safe_partition_cli_rejects_midflight_input_changes(
