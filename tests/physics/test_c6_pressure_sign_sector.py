...
def _reference(**changes):
    arguments = dict(phase=(0.0,) * 6, epi_weight=0.5, phase_weight=0.5)
    arguments.update(changes)
    return derive_c6_pressure_lattice(**arguments)


def _state(epi=None, **changes):
    arguments = dict(epi_lower=0.375, epi_upper=0.625)
    arguments.update(changes)
    return initialize_nodal_remainder(
        epi or (math.nextafter(0.5, -math.inf),) + (0.5,) * 5, **arguments
    )
...
def test_synchronized_source_has_two_strict_sectors_and_a_zero_index(sign):
    ...
    assert (sector.minimum_gradient_index, sector.maximum_gradient_index) == (
        -(2**53),
        2**53,
    )
    ...
    assert sector.pressure_bound == sign * 2.0**-56
    ...
    assert zero.initial_observation.pressure[1] == 0.0
    assert (
        zero.per_step_margin is zero.max_sector_steps is zero.first_exit_bound is None
    )
...
def test_terminal_node1_cut_separates_negative_and_positive_without_zero():
    ...
    for index in range(-4, 5):
        pressure = reference.sources[1] + float(
            reference.source.epi_weight * reference.gradient_quantum * index
        )
...
def test_exact_carried_distance_enters_the_signed_drift_budget(sign):
    reference = _reference()
    neighbor = math.nextafter(0.5, -math.inf if sign < 0 else math.inf)
    initial = _state((neighbor,) + (0.5,) * 5)
    carry = (F(0), F(1, 2**60), F(0), F(0), F(0), F(0))
    state = NodalRemainderState(
        initial.epi, carry, initial.epi_lower, initial.epi_upper
    )
    result = observe_c6_pressure_sector_exit(
        reference, state=state, node=1, sign=sign, timestep=0.0625
    )
...
def test_first_step_can_fail_the_band_instead_of_hitting_the_opposite_sector(sign):
    phase = ((0.25,) + (0.0,) * 5) if sign < 0 else ((0.0,) + (0.25,) * 5)
    reference = _reference(phase=phase)
    state = _state((0.375 if sign < 0 else 0.625,) * 6)
    observed = observe_c6_pressure_sector_exit(
        reference, state=state, node=0, sign=sign, timestep=0.0625
    )
...
def test_subnormal_epi_weight_clips_unreachable_sign_cut_to_actual_gradient_range():
    tiny = math.ulp(0.0)
    reference = _reference(
        phase=(0.0,) + (0.25,) * 5, epi_weight=tiny, phase_weight=1.0
    )
    ...
    assert (
        negative.bounding_gradient_index
        is negative.pressure_bound
        is negative.signed_pressure_margin
        is None
    )
    result = observe_c6_pressure_sector_exit(
        reference, state=_state(), node=0, sign=-1, timestep=0.0625
    )
...
def test_underflow_zero_plateau_makes_both_strict_sectors_empty(sign):
    reference = _reference(epi_weight=math.ulp(0.0))
...
def test_one_point_zero_pressure_band_has_no_strict_sector(sign):
    reference = _reference(epi_lower=0.5, epi_upper=0.5)
...
def test_one_point_nonzero_phase_band_has_a_nonempty_signed_sector():
    reference = _reference(phase=(0.0,) + (0.25,) * 5, epi_lower=0.5, epi_upper=0.5)
    sector = derive_c6_pressure_sign_sector(reference, node=0, sign=1)
    assert not sector.empty
    assert (
        sector.sector_min_index
        == sector.sector_max_index
        == sector.bounding_gradient_index
        == 0
    )
    state = _state((0.5,) * 6, epi_lower=0.5, epi_upper=0.5)
    result = observe_c6_pressure_sector_exit(
        reference, state=state, node=0, sign=1, timestep=0.0625
    )
...
def test_public_outer_and_source_caches_are_rebuilt():
    reference = _terminal_reference()
    forged_row = replace(
        reference.rows[1], nonnegative_min_index=1000, nonpositive_max_index=-1000
    )
    ...
    result = observe_c6_pressure_sector_exit(
        forged, state=_state(), node=1, sign=-1, timestep=0.0625
    )
...
def test_result_dataclasses_are_frozen():
    result = observe_c6_pressure_sector_exit(
        _reference(), state=_state(), node=1, sign=-1, timestep=0.0625
    )
...
def test_invalid_timestep_is_rejected(h, error):
    with pytest.raises(error):
        observe_c6_pressure_sector_exit(
            _reference(), state=_state(), node=1, sign=-1, timestep=h
        )


def test_state_band_must_be_inside_the_certified_slab():
    state = initialize_nodal_remainder((0.5,) * 6)
    with pytest.raises(ValueError, match="contained"):
        observe_c6_pressure_sector_exit(
            _reference(), state=state, node=1, sign=-1, timestep=0.0625
        )
...
def test_state_dimension_and_forged_carry_are_rejected():
    with pytest.raises(ValueError, match="six ordered"):
        observe_c6_pressure_sector_exit(
            _reference(), state=_state((0.5,)), node=1, sign=-1, timestep=0.0625
        )
    state = _state()
    forged = replace(state, remainder=(F(1, 3),) + (F(0),) * 5)
    with pytest.raises(ValueError, match="dyadic"):
        observe_c6_pressure_sector_exit(
            _reference(), state=forged, node=1, sign=-1, timestep=0.0625
        )
