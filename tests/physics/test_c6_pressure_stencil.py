...
from tnfr.physics.c6_pressure_lattice import derive_c6_pressure_lattice, observe_c6_frozen_pressure_stencil
from tnfr.physics.nodal_remainder import derive_nodal_remainder_cell_horizon


def _reference(**changes):
    args = dict(phase=(0.,) * 6, epi_weight=.5, phase_weight=.5)
    args.update(changes)
    return derive_c6_pressure_lattice(**args)


def _state(epi=None, **changes):
    args = dict(epi_lower=.375, epi_upper=.625)
    args.update(changes)
    if epi is None:
        epi = (math.nextafter(.5, -math.inf),) + (.5,) * 5
    return initialize_nodal_remainder(epi, **args)


def _observe(reference=None, state=None, **changes):
    args = dict(state=_state() if state is None else state, node=1, timestep=.0625)
    args.update(changes)
    return observe_c6_frozen_pressure_stencil(_reference() if reference is None else reference, **args)
...
def test_negative_center_bound_is_not_the_global_first_cell_exit():
    ...
    assert result.gradient_index == -1 and result.pressure == -2.**-56
    assert result.exact_increment == -F(1, 2**60)
    assert result.center_cell_width == F(3, 2**55)
    assert (result.initial_max_frozen_steps, result.initial_first_exit_bound) == (32, 33)
    assert (result.uniform_max_frozen_steps, result.uniform_first_exit_bound) == (96, 97)
    whole = derive_nodal_remainder_cell_horizon(
        state=result.state, timestep=.0625, capacity=(1.,) * 6,
...
def test_positive_center_uses_the_wider_upper_half_cell_at_point_five():
    ...
    assert result.gradient_index == 2 and result.pressure == 2.**-55
    assert result.exact_increment == F(1, 2**59)
    assert (result.initial_max_frozen_steps, result.initial_first_exit_bound) == (32, 33)
    assert (result.uniform_max_frozen_steps, result.uniform_first_exit_bound) == (48, 49)


def test_remote_epi_and_carry_do_not_change_the_selected_row_or_center_bounds():
    initial = _observe()
    remote = _state(initial.state.epi[:3] + (.4, .6, .45))
...
def test_center_carry_changes_its_conditional_limit_but_not_pressure_or_uniform_bound():
    ...
    result = _observe(state=state)
    assert result.pressure == -2.**-56
    assert result.center_cell.exact_input == F(1, 2) - F(1, 2**57)
    assert (result.initial_max_frozen_steps, result.initial_first_exit_bound) == (24, 25)
    assert result.uniform_max_frozen_steps == 96


def test_odd_center_tie_is_open_while_the_uniform_enclosure_is_conservative():
    odd = math.nextafter(.5, math.inf)
    result = _observe(state=_state((.5, odd, .5, .5, .5, .5)))
    assert not result.center_cell.even_significand
    assert result.pressure == -2.**-54 and result.exact_increment == -F(1, 2**58)
    assert (result.initial_max_frozen_steps, result.initial_first_exit_bound) == (15, 16)
    assert (result.uniform_max_frozen_steps, result.uniform_first_exit_bound) == (32, 33)


@pytest.mark.parametrize("sign", [-1, 1])
def test_width_is_band_clipped_and_band_failure_need_not_change_display(sign):
    phases = (0., .25, 0., 0., 0., 0.) if sign < 0 else (.25, 0., .25, 0., 0., 0.)
    reference = _reference(phase=phases)
    lower, upper = (.5, .625) if sign < 0 else (.375, .5)
    result = _observe(reference, _state((.5,) * 6, epi_lower=lower, epi_upper=upper))
...
def test_zero_pressure_supplies_no_finite_bound_or_invariance_claim():
    result = _observe(state=_state((.5,) * 6))
...
def test_one_point_band_has_zero_clipped_width_even_with_a_nonzero_source():
    reference = _reference(phase=(0., .25, 0., 0., 0., 0.), epi_lower=.5, epi_upper=.5)
...
def test_a_changed_stencil_can_keep_the_identical_pressure():
    ...
    right = _observe(state=_state((center + delta, center, center - delta, center, center, center)))
...
def test_ordered_stencil_wraps_at_the_cycle_end():
    ...
    assert result.stencil_epi == (result.state.epi[5], result.state.epi[0], result.state.epi[1])
...
def test_inherited_node_four_static_stencil_has_distinct_initial_and_uniform_limits():
    ...
    reference = _reference(phase=phase, epi_weight=float(F(3299399280161697, 2**54)),
    ...
    state = NodalRemainderState(epi, carry, .375, .625)
    ...
    assert result.center_cell.upper - result.center_cell.exact_input == F(67908905300392561, 2**110)
    assert (result.initial_max_frozen_steps, result.initial_first_exit_bound) == (20, 21)
    assert (result.uniform_max_frozen_steps, result.uniform_first_exit_bound) == (43, 44)


def test_reference_caches_are_rebuilt_and_results_frozen():
    reference = _reference()
    forged = replace(reference, sources=(1.,) * 6, gradient_quantum=F(1), rows=())
...
@pytest.mark.parametrize("node,error", [(True, TypeError), (1., TypeError), (-1, ValueError), (6, ValueError)])
def test_invalid_node_is_rejected(node, error):
    with pytest.raises(error):
        _observe(node=node)


@pytest.mark.parametrize("timestep,error", [
    (0., ValueError), (-1., ValueError), (math.inf, ValueError),
    (math.nan, ValueError), (1, TypeError), (False, TypeError),
])
...
def test_state_validation_rejects_wrong_dimension_band_and_noncanonical_carry():
    with pytest.raises(ValueError, match="six ordered"):
        _observe(state=_state((.5,)))
    with pytest.raises(ValueError, match="contained"):
        _observe(state=initialize_nodal_remainder((.5,) * 6))
    state = _state()
