...
@pytest.fixture(scope="module")
def graph(source):
    return derive_c6_carried_cell_graph(source, epi_states=ROWS, timestep=0.0625)
...
def test_seven_static_compensators_cannot_form_a_closed_temporal_class(graph):
    ...
    assert tuple(item.max_unchanged_steps for item in graph.maximal_residence) == (
        76,
        50,
        38,
        50,
    ...
    assert (
        graph.finite_family_escape_certified and graph.carried_cycle_in_family_excluded
    )
...
def test_directional_carry_maximizes_residence_over_all_cell_corners(graph):
    ...
    for i, row in enumerate(ROWS):
        ...
        for side in product((0, 1), repeat=6):
            indices = tuple(
                cell.last_grid_index if bit else cell.first_grid_index
                for cell, bit in zip(domain.coordinates, side, strict=True)
            )
            state = NodalRemainderState(
                row,
                tuple(
                    F(index, scale) - F(value)
                    for index, value in zip(indices, row, strict=True)
                ),
                0.375,
                0.625,
            )
            ...
            assert (
                horizon.max_unchanged_steps
                <= graph.maximal_residence[i].max_unchanged_steps
            )
...
def test_nonself_cycle_is_explicitly_undecided_without_a_carry_cycle_claim():
    source = derive_c6_pressure_lattice(
        phase=(0.0,) * 6, epi_weight=0.5, phase_weight=1.0
    )
    graph = derive_c6_carried_cell_graph(
        source, epi_states=((0.4, 0.6) * 3, (0.6, 0.4) * 3), timestep=2.0
    )
...
def test_stationary_pressure_abstains_from_finite_escape_even_on_a_dag():
    ...
    graph = derive_c6_carried_cell_graph(
        source, epi_states=((0.5,) * 6,), timestep=0.0625
    )
...
def test_pressure_and_sign_caches_cannot_forge_a_stationary_row(source, graph):
    ...
    rebuilt = derive_c6_carried_cell_graph(forged, epi_states=ROWS, timestep=0.0625)
    assert rebuilt == graph


def test_supplied_cell_order_only_relabels_the_certificate(source, graph):
    reverse = derive_c6_carried_cell_graph(
        source, epi_states=ROWS[::-1], timestep=0.0625
    )
...
def test_missing_or_duplicate_cell_families_are_rejected(source, states):
    with pytest.raises((TypeError, ValueError)):
        derive_c6_carried_cell_graph(source, epi_states=states, timestep=0.0625)
...
def test_malformed_visible_rows_are_rejected(source, row):
    with pytest.raises((TypeError, ValueError)):
        derive_c6_carried_cell_graph(source, epi_states=(row,), timestep=0.0625)


@pytest.mark.parametrize("step", (0.0, -1.0, True, 1, float("nan"), float("inf")))
...
def test_primitive_source_changes_are_recomputed_instead_of_trusting_derived_fields(
    source, graph
):
    changed = replace(source, source=replace(source.source, phase=(0.0,) * 6))
    result = derive_c6_carried_cell_graph(changed, epi_states=ROWS, timestep=0.0625)
    assert result.pressures != graph.pressures
    assert result.reference.source.phase == (0.0,) * 6


def test_bad_reference_is_not_admitted():
    with pytest.raises(TypeError, match="C6PressureLatticeReference"):
        derive_c6_carried_cell_graph(object(), epi_states=(ROWS[0],), timestep=0.0625)
