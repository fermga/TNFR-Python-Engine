...
def inputs():
    ...
    names = (
        ...
        "artifacts/research/c6_winding_return_unions.json",
        "benchmarks/c6_winding_return_count_candidates.json",
        "artifacts/research/c6_winding_return_histories.json",
...
def patch_previous(monkeypatch, previous):
    monkeypatch.setattr(
        campaign,
        "analyze_c6_winding_return_memory",
        lambda *_a, **_k: deepcopy(previous),
    )
...
def test_cut_report_preserves_nodal_source_origin_and_scope(inputs, report):
    ...
    assert (
        campaign._payload(report["B58_return_envelope"]["state"])
        == previous["B57_return_envelope"]["state"]
    )
    ...
    assert (
        report["B58_reconstruction"]["prior_memory_sha256"]
        == hashlib.sha256(inputs[12]).hexdigest()
    )
...
def test_cut_report_keeps_all_sixteen_full_original_targets(inputs, report):
    ...
    expected = [
        item["query"]["target_regions"]
        for item in original["B55_union_queries"]
    ...
    expected_cuts = (
        json.loads(inputs[8])["B54_additional_first_exit_facets"]
        + original["B55_additional_first_exit_facets"]
        + previous["B57_additional_first_exit_facets"]
    )
...
def test_cut_report_cannot_promote_matching_counts_to_equivalence(report):
    ...
    conditions = (
        comparison["canonical_source_equal"],
        comparison["return_record_identities_equal"],
        comparison["retained_endpoint_arrays_equal"],
        comparison["origin_images_equal"],
        comparison["complete_query_graph_equal"],
        all(item["equal"] for item in comparison["target_initializations"]),
    )
    assert comparison["all_target_operators_and_initial_conditions_identical"] == all(
        conditions
    )
...
def test_cut_audit_rejects_each_altered_ancestor(inputs, index):
    ...
    with pytest.raises(ValueError, match="lineage"):
        campaign.analyze_c6_winding_memory_cuts(
...
def test_cut_audit_reconstructs_untrusted_parent_claims(
    monkeypatch, inputs, previous, mutation
):
    ...
    if mutation == "claim":
        ...
        query = next(
            item["query"]
            for item in parent["B57_memory_queries"]
        ...
        parent["B57_memory_queries"][0]["query"]["target_regions"][0]["bounds"][0][
            6
        ] += 1
    elif mutation == "endpoint":
        zone = next(
            zone
            for zone in parent["B57_memory_envelope"]["retained_endpoint_zones"]
            if zone is not None
        )
        zone[0][6] += 1
    else:
...
def _hull(values):
    ...
    return tuple(
        tuple(max(point[i] - point[j] for point in points) for j in range(7))
        for i in range(7)
    )
...
def small_memory():
    ...
    edges = tuple(
        owner.C6CarriedReturnTransition(
            ...
            _hull(values),
            None,
            _hull(value + shift for value in values),
            (shift, 0, 0, 0, 0, 0),
        )
    ...
    terminals = tuple(
        owner.C6CarriedReturnTransition(
            ...
            _hull(values),
            None,
            _hull(value + 1 for value in values),
            (1, 0, 0, 0, 0, 0),
        )
    ...
    return owner._derive_return_memory_envelope(
        envelope, max_memory_work=1000, max_memory_arcs=100
    )
    ...
    return (
        (
            C6CarriedForwardZone(
                memory.return_envelope.epi_states[2], _hull((3, 11, 12, 13))
            ),
        ),
    )
...
def test_complete_operator_comparison_admits_only_exact_geometry(small_memory):
    result = campaign._compare_c6_memory_query_geometry(
        small_memory, small_memory, _small_groups(small_memory), 1000
    )
    ...
    assert (
        not result["actual_origin_reachability_certified"]
        and not result["conditional_boundedness_certified"]
    )


@pytest.mark.parametrize("mutation", ("guard", "endpoint", "initial"))
def test_equal_counts_do_not_hide_changed_guard_endpoint_or_initial_dbm(
    small_memory, mutation
):
    if mutation == "guard":
        changed = replace(
            small_memory,
            source_guards=(*small_memory.source_guards[:3], _hull((10, 11))),
        )
    elif mutation == "endpoint":
        changed = replace(
            small_memory,
            retained_endpoint_zones=(
                *small_memory.retained_endpoint_zones[:3],
                _hull((10, 11)),
            ),
        )
    else:
        ...
        terminal = replace(
            envelope.intermediate_transitions[-1], source_guard=_hull((10, 11))
        )
        changed = replace(
            small_memory,
            return_envelope=replace(
                envelope,
                intermediate_transitions=(
                    *envelope.intermediate_transitions[:-1],
                    terminal,
                ),
            ),
        )
    groups = _small_groups(small_memory)
    before_initial, before_complete, _ = owner._return_memory_query_initialization(
        small_memory, groups[0], 1000
    )
    after_initial, after_complete, _ = owner._return_memory_query_initialization(
        changed, groups[0], 1000
    )
    assert before_complete and after_complete
    assert sum(zone is not None for zone in before_initial) == sum(
        zone is not None for zone in after_initial
    )
    result = campaign._compare_c6_memory_query_geometry(
        small_memory, changed, groups, 1000
    )
    ...
    if mutation == "guard":
        ...
        assert (
            result["retained_endpoint_arrays_equal"]
            and result["complete_query_graph_equal"]
        )
...
def test_tiny_comparison_budget_never_promotes_two_partial_results(small_memory):
    result = campaign._compare_c6_memory_query_geometry(
        small_memory, small_memory, _small_groups(small_memory), 1
    )
    ...
    assert not result["all_target_operators_and_initial_conditions_identical"]
    assert all(
        not item["complete"] and not item["equal"]
        for item in result["target_initializations"]
    )
...
def test_cut_cli_binds_all_thirteen_inputs_in_order(
    ...
    assert [item["sha256"] for item in result["input_evidence"]] == [
...
def test_cut_cli_rejects_midflight_input_changes(
