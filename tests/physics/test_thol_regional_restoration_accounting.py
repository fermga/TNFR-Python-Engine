...
def _capture(epi, pressure, phase, forcing, kernel=ZERO):
    ...
    model = tuple(
        EPI_WEIGHT * g + f for g, f in zip(snapshot.epi_gradient, forcing, strict=True)
    )
...
def _record(epi, pressure, phase, time):
    return {
        "state": {
            "nodes": list(NODES),
            "epi": list(map(float, epi)),
            "capacity": list(map(float, CAPACITY)),
...
def _fixture(
    *,
    ...
    reference_snapshot = _from_data(
        NODES, EDGES, SUPPORT, control_initial, CAPACITY, ZERO
    )
    original = derive_forced_support_balance(
        reference_snapshot, epi_weight=EPI_WEIGHT, forcing=ZERO
    )
    branches = []
    for branch in (0, 1):
        records = []
        ...
        for ordinal in range(6):
            ...
            generated = _add(
                tuple(EPI_WEIGHT * g for g in geometric.epi_gradient),
                _add(source, _add(kernel, stored)),
            )
            phase0 = tuple(F(value) for value in previous["state"]["phase"])
            gen_record = _record(current, generated, phase0, time)
            reset_epi = (
                _add(current, _vector(2, F(1, 64))) if lags and branch else current
            )
            reset_record = _record(reset_epi, generated, phase0, time)
            entry_pressure = (
                _add(generated, _vector(3, F(1, 128))) if lags and branch else generated
            )
            entry_source = (
                _add(source, _vector(3, F(1, 256))) if lags and branch else source
            )
            entry_phase = (
                _add(phase0, _vector(3, F(1, 8))) if lags and branch else phase0
            )
            entry = _record(reset_epi, entry_pressure, entry_phase, time)
            scale = 1 + F(ordinal + 1, 8) * (control_scale - 1)
            next_control = tuple(
                (F(1, 2) + scale * (value - F(1, 2))) if i in CHILDREN else value
                for i, value in enumerate(control_initial)
            )
            amplitude = initial_jump * (1 + F(ordinal + 1, 8) * (error_scale - 1))
            final_epi = (
                _add(next_control, _vector(2, amplitude)) if branch else next_control
            )
            post_jump = _vector(2, F(1, 2048)) if lags and branch else ZERO
            integrated = _record(
                _sub(final_epi, post_jump), entry_pressure, entry_phase, time + DT
            )
            ...
            trace = {
                ...
                "boundaries": rows,
                "captures": {
                    "pressure_generation": _capture(
                        current, generated, phase0, source, kernel
                    ),
                    "integrator_entry": _capture(
                        reset_epi, entry_pressure, entry_phase, entry_source
                    ),
                },
            }
            records.append(
...
def test_complete_exact_endpoint_and_stage_energy_accounting(lags):
    ...
    assert tuple(row["time"] for row in result["endpoints"]) == tuple(
        F(6 + k, 4) for k in range(7)
    )
    ...
    for step in result["steps"]:
        ...
        for stage in step["budgets"].values():
            ...
            assert (
                stage["variance_identity_residual"]
                == stage["mass_identity_residual"]
                == 0
            )
            increment += expected
        ...
        assert (
            sum(row["budget"]["variance_change"] for row in step["local_glyphs"])
            + sum(row["variance_change"] for row in step["intervening_epi_budgets"])
    ...
    assert (
        accumulated
        == result["endpoints"][-1]["paired"]["centered_H_energy"]
        - result["endpoints"][0]["paired"]["centered_H_energy"]
    )
...
def test_all_five_pressure_terms_have_independent_sign_and_work_checks():
    ...
    negative_laplacian = tuple(
        (jump[(i - 1) % 4] + jump[(i + 1) % 4]) / 2 - jump[i] for i in NODES
    )
    assert parts["held_epi_lag"]["pressure"] == tuple(
        -EPI_WEIGHT * x for x in negative_laplacian
    )
    ...
    centered = _center(
        tuple(integration["before"]["epi"][i] for i in CHILDREN), weights
    )
    for part in parts.values():
        expected_mass = DT * sum(2 * part["pressure"][i] for i in CHILDREN)
        expected_shape = DT * sum(
            2 * z * part["pressure"][i] for i, z in zip(CHILDREN, centered, strict=True)
        )
        assert part["weighted_total_work"] == expected_mass
        assert part["variance_work"] == expected_shape
    assert (
        sum(part["variance_work"] for part in parts.values())
        == DT * integration["balance"]["variance_defect_rate"]
    )
    assert first["source_channels"]["phase"]["pressure"] == _add(
        _vector(2, F(1, 32)), _vector(3, F(1, 256))
    )
    assert (
        first["source_channels"]["vf"]["pressure"]
        == first["source_channels"]["topo"]["pressure"]
        == ZERO
    )
...
def test_boundary_and_local_pressure_work_remain_separate_exact_terms():
    ...
    for step in result["steps"]:
        ...
        expected_input = EPI_WEIGHT * (
            (delta[2] - mean) * (delta[1] - mean)
            + (delta[3] - mean) * (delta[0] - mean)
        )
        ...
        assert (
            expected_self + expected_input
            == integration["balance"]["variance_boundary_rate"]
        )
        ...
        for field in ("variance_work", "weighted_total_work"):
            ...
            assert (
                assigned + step["unassigned_pressure_write"][field]
                == step["pressure_parts"]["pressure_operator_write"][field]
            )
    assert not result["decision"]["autonomous_maintenance_certified"]
...
def _score(error, contrast, drift=0):
    return {
        "paired": {"centered_H_energy": F(error)},
        "control_variance": F(contrast),
        "control_centered_drift_energy": F(drift),
    }


@pytest.mark.parametrize(
    "scores,complete,outcome,failed",
    [
        ([_score(4, 2), _score(1, 2)], True, "supported_in_scope", None),
        ([_score(4, 2), _score(1, 2)], False, "inconclusive", "complete_fixed_horizon"),
        (
            [_score(0, 2), _score(0, 2)],
            True,
            "inconclusive",
            "initial_nonuniform_damage",
        ),
        (
            [_score(4, 2), _score(1, 0), _score(1, 2)],
            True,
            "inconclusive",
            "control_contrast_positive_everywhere",
        ),
        (
            [_score(4, 2), _score(5, 4)],
            True,
            "rejected_in_scope",
            "raw_error_decreased",
        ),
        (
            [_score(4, 2), _score(3, 1)],
            True,
            "rejected_in_scope",
            "relative_error_decreased",
        ),
        (
            [_score(4, 2), _score(1, 1)],
            True,
            "rejected_in_scope",
            "control_contrast_not_attenuated",
        ),
    ],
)
...
def test_singular_control_or_absent_damage_is_inconclusive(zero_control, zero_error):
    result = audit.analyze_response(
        *_fixture(zero_control=zero_control, zero_error=zero_error)
    )
    assert result["decision"]["outcome"] == "inconclusive"
    if zero_control:
        assert all(
            not row["relative_error"]["available"] for row in result["endpoints"]
        )
...
def test_incomplete_or_forged_accounting_inputs_reject(fault):
    ...
    if fault == "missing_capture":
        arguments[1][0]["native_trace"]["captures"]["integrator_entry"][
            "available"
        ] = False
    elif fault == "wrong_capture":
        arguments[1][0]["native_trace"]["captures"]["integrator_entry"]["payload"][
            "observation"
        ]["kernel_pressure_defect"] = _vector(2, F(1))
    elif fault == "metric":
