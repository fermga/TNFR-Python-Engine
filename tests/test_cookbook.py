...
class TestTNFRCookbook:
    ...
    def test_list_recipes_combined_filters(self, cookbook):
        ...
        recipes = cookbook.list_recipes(domain="educational", min_health=0.80, max_length=10)
    ...
    def test_search_recipes_use_case(self, cookbook):
        ...
        for r in results:
            found = "team" in r.name.lower() or any("team" in uc.lower() for uc in r.use_cases)
            assert found
    ...
    def test_recommend_recipe_basic(self, cookbook):
        ...
        recipe = cookbook.recommend_recipe(
            context="Need to help team collaborate effectively", constraints={"min_health": 0.75}
        )
    ...
    def test_recommend_recipe_domain_constraint(self, cookbook):
        ...
        recipe = cookbook.recommend_recipe(
            context="Need to facilitate learning", constraints={"domain": "educational"}
        )
    ...
    def test_recipe_has_required_fields(self, cookbook):
        ...
        for recipe in all_recipes:
            ...
            assert (
                recipe.health_metrics is not None
            ), f"Recipe '{recipe.name}' missing health metrics"
    ...
    def test_recipe_sequences_valid(self, cookbook):
        ...
        for recipe in all_recipes:
            for op in recipe.sequence:
                assert (
                    op in CANONICAL_OPERATOR_NAMES
    ...
    def test_recipe_uniqueness(self, cookbook):
        ...
        for domain in cookbook.get_all_domains():
            ...
            assert len(names) == len(set(names)), f"Duplicate recipe names in {domain}: {names}"
...
class TestCookbookIntegration:
    ...
    def test_recipe_health_consistency(self):
        ...
        assert abs(recipe.health_metrics.overall_health - recalculated_health.overall_health) < 0.01
