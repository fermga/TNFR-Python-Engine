...
def run():
    ...
    return NodalMeasurementRun(
        run_id="synthetic-trace", acquisition_id="one-synthetic-preparation",
        channel_ids=("Voltage",), timestamps=times,
...
def prefix_of(run):
    return replace(run, timestamps=run.timestamps[:25],
...
def test_zero_reference_decay_predicts_same_run_without_factor_two(run):
    ...
    result = BENCH.score_continuation(
        run, forecast, expected_forecast_hash=retained_hash,
    )
    ...
    for field in ("sensor_uncertainty", "clock_uncertainty",
...
def test_changed_suffix_is_scored_without_refitting_or_physical_rejection(
    ...
    result = BENCH.score_continuation(
        changed, forecast, expected_forecast_hash=retained_hash,
    )
...
def test_issued_hash_rejects_prediction_and_scope_tampering(run, field, value):
    ...
    with pytest.raises(ValueError, match="issued continuation hash"):
        BENCH.score_continuation(run, changed,
...
@pytest.mark.parametrize("change", [
    ...
    lambda r: replace(r, timestamps=r.timestamps[:-1], samples=(r.samples[0][:-1],)),
])
def test_scoring_binds_prefix_schedule_units_and_acquisition(run, change):
    ...
    with pytest.raises(ValueError, match="observed trace"):
        BENCH.score_continuation(change(run), forecast,
...
def test_json_roundtrip_preserves_issued_identity(run):
    ...
    assert BENCH.score_continuation(
        run, decoded, expected_forecast_hash=retained_hash,
    )["nodal"]["max_absolute_error_volts"] < 1e-13


@pytest.mark.parametrize("change", [
    lambda p: replace(p, timestamps=p.timestamps[:-1], samples=(p.samples[0][:-1],)),
...
def test_nominal_model_uses_only_predeclared_endpoint_calibration(run):
    prefix = prefix_of(run)
    altered_values = ((prefix.samples[0][0],)
    ...
    assert changed["nominal_capacity_enclosure"] == original["nominal_capacity_enclosure"]
...
def test_affine_control_does_not_fit_an_asymptote_into_nodal_model():
    ...
    shifted = NodalMeasurementRun(
        run_id="synthetic-offset", acquisition_id="synthetic-offset-preparation",
        channel_ids=("Voltage",), timestamps=tuple(i * 0.02 for i in range(50)),
        samples=(values,), value_unit="V", time_unit="s",
    )
    forecast = issue(shifted)
    assert forecast["nominal_reference"] == 0
    assert forecast["affine_ar1"] == pytest.approx(values[24:])
    result = BENCH.score_continuation(
        shifted, forecast, expected_forecast_hash=_digest(forecast),
    )
...
def test_optional_ar1_failure_does_not_erase_nodal_forecast(run, monkeypatch):
    ...
    assert forecast["predicted_voltage"] == pytest.approx(run.samples[0][24:])
    assert forecast["affine_ar1"] is None
    assert any(word in forecast["affine_ar1_status"].lower()
...
def test_synthetic_study_issues_before_scoring_and_admits_real_evidence(
    ...
    monkeypatch.setattr(BENCH, "load_volts", lambda path: (
        run, {"source_sha256": source_hash, "rdata_version": "1.1.0"},
    ))
    ...
    def require_issued_artifact(observed, forecast, *, expected_forecast_hash):
        ...
        return original_score(observed, forecast,
                              expected_forecast_hash=expected_forecast_hash)

    monkeypatch.setattr(BENCH, "score_continuation", require_issued_artifact)
    result = BENCH.execute_study(
        raw_path, spec_path=spec_path, protocol_path=protocol_path,
        expected_spec_hash=spec_hash, output=output,
    )
    ...
    assert evidence["numerical_context"] == {
        "sensor_error_bound": None, "clock_error_bound": None,
    }
