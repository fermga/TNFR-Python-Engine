...
class TestVALTHOLSequence:
    ...
    def test_val_thol_sequence_basic(self):
        ...
        assert epi_2 >= epi_0 * 0.9, (
            f"Net EPI should be maintained or grow: " f"{epi_0:.4f} -> {epi_2:.4f}"
        )
    ...
    def test_val_thol_phase_coherence(self):
        ...
        assert phase_shift < np.pi, f"Phase shift should be moderate: Δθ={phase_shift:.3f} < π"
...
class TestVALSequencesCombined:
    ...
    def test_oz_val_il_complete_cycle(self):
        ...
        run_sequence(G, node, [Emission(), Dissonance(), Expansion(), Coherence()])

        epi_final = G.nodes[node][epi_key]

        # Net growth should occur with hook
        assert epi_final > epi_initial, (
            f"Complete cycle should produce net growth: " f"{epi_initial:.4f} -> {epi_final:.4f}"
        )
    ...
    def test_val_thol_il_hierarchical_consolidation(self):
        ...
        run_sequence(G, node, [Expansion(), SelfOrganization(), Coherence()])
        ...
        assert epi_final >= epi_initial * 0.9, (
            f"Hierarchical structure should be preserved: " f"{epi_initial:.4f} -> {epi_final:.4f}"
        )

        # Stabilization should reduce ΔNFR
        assert dnfr_final < 0.2, (
            f"Final ΔNFR should be moderate after stabilization: " f"ΔNFR={dnfr_final:.4f}"
        )
