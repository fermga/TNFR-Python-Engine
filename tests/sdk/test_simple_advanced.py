...
class TestFractalResonantNode:
    ...
    def test_classify_annular(self):
        ...
        assert (
            classify_nodal_topology(nx.cycle_graph(10))["topology"] == "annular"
        )
        assert (
            classify_nodal_topology(nx.complete_graph(6))["topology"] == "annular"
        )
    ...
    def test_micro_and_macro_nfr_equilibrium_agree(self):
        ...
        scan = net.nodal_scan()
        micro = sum(
            1 for r in scan.nodes.values() if r.equilibrium
        ) / len(scan.nodes)
