...
def synthetic_frame():
    ...
    return pd.DataFrame({"Voltage": [3.0 - i / 100 for i in range(50)],
...
def synthetic_rdata(frame=None, *, name="Volts", version=2):
    ...
    parsed = rdata.conversion.convert_python_to_r_data(
        {name: frame}, format_version=version, file_type="rda",
    )
...
def test_custom_r_class_rejected_before_fallback_conversion():
    rdata = pytest.importorskip("rdata")
    parsed = rdata.conversion.convert_python_to_r_data(
        {"Volts": synthetic_frame()}, format_version=2, file_type="rda",
    )
...
@pytest.mark.parametrize("make_raw,match", [
...
@pytest.mark.parametrize("size,match", [(767, "SHA-256"), (766, "size"), (1025, "compressed")])
def test_identity_failure_precedes_any_decode(tmp_path, monkeypatch, size, match):
    path = tmp_path / "synthetic.rda"
    path.write_bytes(b"x" * size)
    monkeypatch.setattr(BENCH, "_bounded_expand", lambda _: pytest.fail("must not decompress"))
    monkeypatch.setattr(BENCH, "_rdata_module", lambda: pytest.fail("must not import reader"))
    with pytest.raises(ValueError, match=match):
        BENCH.load_volts(path)


def test_public_metadata_and_digest_for_identity_substituted_synthetic_fixture(tmp_path, monkeypatch):
    expanded = synthetic_rdata()
