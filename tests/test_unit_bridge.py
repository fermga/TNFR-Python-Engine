...
@pytest.mark.parametrize(
    "value", [0, -1, np.nan, np.inf, -np.inf, 10**1000, Fraction(1, 10**1000)]
)
...
@pytest.mark.parametrize(
    "value", [True, np.bool_(False), np.nan, np.inf, -np.inf, Fraction(1, 10**1000)]
)
...
@pytest.mark.parametrize(
    ...
    [
        (hz_str_to_hz, 1e308, 1e308),
        (hz_str_to_hz, 5e-324, 0.5),
        (hz_to_hz_str, 1e308, 5e-324),
        (hz_to_hz_str, 5e-324, 2),
    ],
)
def test_conversion_does_not_silently_overflow_or_erase_activity(
    function, value, factor
):
