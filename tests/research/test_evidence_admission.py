...
from tnfr.research import (
    ClaimStatus, CoreExperimentManifest, EvidenceAdmissionError, EvidenceSidecar,
    ExperimentManifest, NumericalCertificate, certify_within_tolerance,
)
from tnfr.research import evidence_sidecar as owner


def certificate(**changes):
    values = dict(quantity="residual", value=0.25, tolerance=0.5, passed=True,
                  backward_error=0.001, condition_number=2.0, precision="float64")
...
def sidecar(tmp_path, **changes):
    ...
    manifest = CoreExperimentManifest(
        claim_id="O6.fixture", git_sha="deadbeef", versions={"python": "3.13"},
        graph_construction="declared fixture", capacity_specification="positive",
        solver="none: metadata fixture", result_status=ClaimStatus.MEASURED,
        telemetry=("response",), controls=("changed bytes",),
        artifacts=("response.json",), source_dirty=False,
    )
    values = dict(
        manifest=manifest, artifact="response.json", model="finite fixture", norm="L2",
        distance_convention="declared graph", clock="instrument_seconds",
        finite_horizon=2.0, tail_status="unobserved",
        provenance={name: False for name in (
            "uses_future_samples", "uses_outcome_derived_wiring", "fits_on_evaluation_data",
            "uses_evaluation_labels", "uses_postselection",
        )},
        certificate=certificate(), claim_statement="finite residual", claim_status="measured",
        scope="metadata and byte integrity only", assumptions=("fixed map",), outcome="fixture",
        source_imports=("tnfr.research.evidence_sidecar",),
        graph_context={"nodes": [0, 1]}, state_context={"epi": [0.0, 1.0]},
        numerical_context={"precision": "float64"},
        observation_context={"calibration": {"scale": [1.0, 2.0]}},
        cost_context={"elapsed": 0.0},
        artifact_hashes={"response.json": "sha256:" + hashlib.sha256(artifact.read_bytes()).hexdigest()},
    )
    values.update(changes)
    return EvidenceSidecar(**values)


@pytest.mark.parametrize("value,tolerance,passed", [
    (1, 0, True), (1, 1, True), (-1, 1, True), (0.25, 0.5, False),
])
def test_inconsistent_strict_verdict_cannot_be_admitted(value, tolerance, passed):
    with pytest.raises(ValueError, match="inconsistent"):
        certificate(value=value, tolerance=tolerance, passed=passed).validate_for_admission()


@pytest.mark.parametrize("value,tolerance,passed", [
    (1, 1, False), (-1, 1, False), (0, 0, False), (0.25, 0.5, True),
])
...
@pytest.mark.parametrize("field,value", [
    ("passed", 1), ("passed", "false"), ("value", True), ("value", "0.25"),
    ("value", float("nan")), ("tolerance", float("inf")), ("tolerance", -1),
    ("backward_error", True), ("condition_number", float("inf")),
    ("quantity", " "), ("precision", []),
])
...
@pytest.mark.parametrize("horizon", [float("nan"), float("inf"), -float("inf"), -1, True, "2"])
...
def test_every_manifest_artifact_needs_a_verified_digest(tmp_path):
    evidence = sidecar(tmp_path)
    evidence = replace(evidence, manifest=replace(
        evidence.manifest, artifacts=("response.json", "calibration.json"),
    ))
    with pytest.raises(EvidenceAdmissionError, match="every declared artifact"):
        evidence.validate_metadata()


@pytest.mark.parametrize("name", ["../outside.json", "/outside.json", "C:/outside.json", "a/../b", "a//b"])
...
def test_context_and_returned_payload_are_detached_and_deeply_frozen(tmp_path):
    ...
    assert evidence.to_dict()["observation_context"] == {"calibration": {"scale": [1.0, 2.0]}}
...
def test_legacy_manifest_is_cloned_and_its_versions_cannot_mutate_evidence(tmp_path):
    ...
    manifest = ExperimentManifest(
        claim_id="NT.fixture", git_sha="deadbeef", versions={"python": "3.13"},
        artifacts=("response.json",), controls=("fixture",),
    )
    evidence = replace(evidence, manifest=manifest, dirty_source_hash="sha256:" + "a" * 64,
                       provenance={"uses_known_factors": False, "uses_target_labels": False,
...
def test_export_preserves_payload_schema_and_cannot_overwrite_its_own_evidence(tmp_path):
    evidence = sidecar(tmp_path)
