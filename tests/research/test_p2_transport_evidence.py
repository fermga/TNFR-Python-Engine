...
def _continuous_run(run_id, acquisition_id, times, *, mean, amplitude):
    ...
    return NodalMeasurementRun(
        run_id=run_id, acquisition_id=acquisition_id,
        channel_ids=("left", "right"), timestamps=times, samples=samples,
...
def test_continuous_p2_bundle_binds_issued_exact_forecast_and_result(tmp_path):
    ...
    calibration = calibrate_p2_transport(
        [_continuous_run("calibration", "preparation-A", times,
                         mean="1", amplitude="0.7")],
        graph=nx.Graph([("left", "right")]), measurement=measurement,
    ...
    forecast = forecast_p2_transport(
        calibration, evaluation_run_id="reserved",
    ...
    for sample, saved in zip(forecast.tube.samples, encoded["forecast"]["tube"]["samples"]):
        for interval, saved_interval in zip(sample.epi, saved["epi"]):
            assert tuple(
                Fraction(int(item["numerator_hex"], 16),
    ...
    observed = _continuous_run("reserved", "preparation-B", times,
                               mean="2", amplitude="-0.5")
    comparison = score_p2_transport(
        forecast, calibration, observed, expected_forecast_hash=issued_hash,
    )
    ...
    sidecar = EvidenceSidecar(
        ...
        claim_status="measured", scope="software integration only",
        assumptions=("joint synthetic error bounds", "same latent time per pair",
        ...
        numerical_context={"epi_error": "1/10000000000",
    ...
    saved_sidecar = sidecar.write_admitted(tmp_path / "evidence.json", root_dir=tmp_path)
