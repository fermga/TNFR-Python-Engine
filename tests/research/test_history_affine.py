...
def raw_fixture():
    ...
    return dict(
        normalization=4,
        coefficient_vertices=[2, 7, 19],
        guards=[
            dict(guard_id=i, bounds=interval(a, b))
            for i, (a, b) in enumerate(((-2, -1), (-2, 3), (-3, -2)))
        ],
        ...
        provenance=dict(
            scope="algebraic fixture, no TNFR trajectory", finite_metadata=0.5
        ),
    )
...
def candidate(spec):
    return dict(
        ...
        coefficients=[
            dict(vertex=2, gradient=[0] * 6, offset="3"),
            dict(vertex=7, gradient=[4, 0, 0, 0, 0, 0], offset="0"),
            dict(vertex=19, gradient=[4, 0, 0, 0, 0, 0], offset="1"),
        ],
        drift_proofs=[
            dict(arc_index=9, multipliers=[]),
            dict(arc_index=3, multipliers=[[0, 6, "1"]]),
        ],
...
@pytest.mark.parametrize(
    "value",
    ["1e999999999999", "0e100000000", "1e-100000000", "9" * 10000, "1/" + "1" * 10000],
)
...
def test_resource_limits_are_explicit_and_do_not_accept_a_prefix():
    ...
    for kwargs in (
        dict(max_vertices=2),
        dict(max_guards=2),
        dict(max_obligations=2),
        dict(max_integer_bits=2),
    ):
        with pytest.raises(ValueError):
...
def test_shared_rows_and_forms_match_direct_signed_shifted_potentials():
    ...
    records = [
        dict(
            vertex=v,
            gradient=[
                str(Fraction((v + 3) * (i + 1) % 7 - 3, i + 1)) for i in range(6)
            ],
            offset=str(Fraction(v, 11)),
        )
    ...
    for obligation in spec.obligations:
        ...
        for x in range(-bounds[6][0], bounds[0][6] + 1):
            ...
            if obligation.kind == "drift":
                direct += potential(
                    obligation.target_vertex,
                    tuple(k + s for k, s in zip(point, obligation.shift)),
                )
    ...
    assert minimum == 1 and sum(
        value * blocks.flat[column] for column, value in row
    ) == potential(7, spec.root_point)
    assert (
        h.affine_form(spec, blocks, spec.obligations[2])[0] == (0,) * 6
    )  # Self-loop gradients cancel.
    ...
    with pytest.raises(ValueError, match="parsed"):
        h.affine_form(
            spec, replace(blocks, flat=(Fraction(0),) * 21), spec.obligations[0]
        )
    with pytest.raises(ValueError, match="parsed"):
...
def test_complete_candidate_keeps_nonzero_root_shift_and_target_scope():
    ...
    assert (
        result["checked_drift_obligations"],
        result["checked_target_obligations"],
    ) == (2, 1)
    assert (
        not result["whole_C6_first_exit_label_excluded"]
        and not result["original_target_coverage_verified"]
    )
    blocks = h.parse_coefficients(spec, candidate(spec)["coefficients"])
    assert [
        h.exact_minimum(spec, item, blocks)["minimum"] for item in spec.obligations
    ] == ["0", "0", "1"]
...
@pytest.mark.parametrize(
    "kind", ["flat", "records", "flat_fraction", "record_fraction"]
)
...
def test_complete_failure_collection_does_not_skip_later_obligations():
    ...
    assert (
        one["checked_drift_obligations"] == 2 and one["checked_target_obligations"] == 1
    )
...
def test_exact_extrema_equal_independent_finite_integer_oracle():
    ...
    for first, second in product((-2, 0, 3), (-3, 0, 2)):
        ...
        expected = min(
            constant + sum(c * k for c, k in zip(coefficients, point))
            for point in points
        )
...
def test_owner_receipt_is_independently_checked(monkeypatch, kind):
    ...
    def corrupt(*args):
        ...
        fields = dict(
            value=receipt.value, point=receipt.point, dual_flow=receipt.dual_flow
        )
        if kind == "primal":
            ...
            fields["dual_flow"] = tuple(
                (i, j, True) for i, j, _a in fields["dual_flow"]
            )
