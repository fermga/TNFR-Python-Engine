...
from tnfr.operators.preconditions import OperatorPreconditionError, validate_self_organization
from tnfr.operators.preconditions.self_organization import validate_self_organization_strict
...
def _graph():
    ...
    graph.add_node(0, **{
        ALIAS_EPI[0]: 0.6, ALIAS_DNFR[0]: 0.2, ALIAS_VF[0]: 1.0,
        "theta": 0.0, "epi_history": [0.0, 0.1, 0.6],
        "glyph_history": ["OZ"],
    })
    graph.add_node(1, **{
        ALIAS_EPI[0]: 0.4, ALIAS_DNFR[0]: 0.1, ALIAS_VF[0]: 1.0, "theta": 0.0,
    })
    graph.add_edge(0, 1)
    return graph
...
@pytest.mark.parametrize("history", [
    [0.0, 0.1, 0.6], [0.0, 0.3, 0.6], [0.0, 0.9, 0.6],
])
def test_gate_accepts_both_acceleration_signs_and_closed_window_without_writes(gate, history):
    graph = _graph()
...
@pytest.mark.parametrize("representation", [
    0.6, ensure_bepi(0.6), serialize_bepi(ensure_bepi(0.6)),
])
...
@pytest.mark.parametrize("key,value", [
    ("THOL_MIN_EPI", float("nan")), ("THOL_MIN_EPI", -0.1),
    ("THOL_MIN_VF", True), ("THOL_MIN_VF", float("inf")),
    ("THOL_MIN_DEGREE", 1.5), ("THOL_MIN_DEGREE", True),
    ("THOL_ALLOW_ISOLATED", 1), ("THOL_METABOLIC_ENABLED", "false"),
    ("THOL_MIN_HISTORY_LENGTH", 2), ("THOL_MIN_HISTORY_LENGTH", 3.0),
    ("THOL_MIN_HISTORY_LENGTH", True), ("THOL_MIN_HISTORY_LENGTH", 4),
    ("BIFURCATION_THRESHOLD_TAU", float("nan")),
    ("THOL_BIFURCATION_THRESHOLD", -0.1),
])
def test_invalid_configuration_has_identical_rejection_and_no_execution_writes(key, value):
    reasons = []
    for gate in GATES:
        graph = _graph()
...
@pytest.mark.parametrize("key,value", [
    (ALIAS_EPI[0], -0.6), (ALIAS_EPI[0], True),
    (ALIAS_DNFR[0], 0.0), (ALIAS_DNFR[0], -0.1),
    (ALIAS_DNFR[0], float("nan")), (ALIAS_VF[0], float("inf")),
    (ALIAS_VF[0], -1.0), (ALIAS_VF[0], True),
...
def test_physical_history_rejection_never_falls_back_to_legacy_or_cached_curvature(history):
    for gate in GATES:
        graph = _graph()
...
def test_validator_facade_uses_the_same_read_only_gate():
    graph = _graph()
    before = _state(graph)
    assert TNFRValidator().validate_operator_preconditions(graph, 0, "self_organization")
    assert _state(graph) == before
