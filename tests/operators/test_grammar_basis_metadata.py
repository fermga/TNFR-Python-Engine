...
def test_compositional_bases_cover_rules_without_changing_historical_schema():
    ...
    assert tuple(field.name for field in fields(gc.GrammarRule)) == (
        "rule_id", "name", "physics", "operator_sets", "invariant", "pdf_reference",
    )
    kinds = set()
    for rule in gc.GRAMMAR_RULES:
        ...
        for basis in bases:
            ...
            assert basis.as_dict()["scope"] == "declarative_basis_not_execution_evidence"
...
LEGACY_ROLES = {
    "AL": ["generator"], "EN": [], "IL": ["stabilizer"],
    "OZ": ["closure", "destabilizer"], "UM": ["coupling/resonance"],
    "RA": ["coupling/resonance"], "SHA": ["closure"],
    "VAL": ["destabilizer"], "NUL": [],
    "THOL": ["stabilizer", "transformer"],
    "ZHIR": ["destabilizer", "transformer"],
    "NAV": ["generator", "closure"], "REMESH": ["generator", "closure"],
}
...
def test_changing_declarative_basis_cannot_make_an_invalid_word_admissible(monkeypatch):
    ...
    claimed = tuple(replace(item, kind=gc.GrammarBasisKind.IDENTITY,
                            statement="Caller labels this an identity.")
                    for item in gc.grammar_basis("U1a"))
    monkeypatch.setattr(gc, "GRAMMAR_BASES",
                        MappingProxyType({**gc.GRAMMAR_BASES, "U1a": claimed}))
...
def test_u3_u6_retain_full_tetrad_scope_without_claiming_state_closure():
    for rule_id in ("U3", "U6"):
        tetrad, = (item for item in gc.grammar_basis(rule_id)
        ...
        assert "separate state-closure evidence for dynamical prediction" in tetrad.hypotheses
