...
@pytest.mark.parametrize("path", PATHS)
def test_overflowing_pair_difference_can_have_representable_weighted_pressure(
    path, monkeypatch
):
    ...
    np.testing.assert_allclose(
        _run(graph, path, monkeypatch), [-1.58e308, 0, 0], rtol=5e-16
    )


@pytest.mark.parametrize("path", PATHS)
def test_channel_coefficient_is_applied_before_rejecting_unrepresentable_component(
    path, monkeypatch
):
...
@pytest.mark.parametrize("path", PATHS)
def test_disabled_linear_channels_and_zero_edges_do_not_evaluate_extreme_pairs(
    path, monkeypatch
):
...
def test_weighted_loops_parallel_neighbors_and_aliases_match_exact_rational_means(
    kind, monkeypatch
):
    ...
    for row in range(len(graph)):
        ...
        expected.append(
            float(
                sum(w * (value - epi[row]) for w, value in zip(weights, epi))
                / denominator
            )
            if denominator
            else 0.0
...
def test_fused_jit_dispatch_uses_the_same_linear_reducer(use_jit):
    ...
    expected = [
        sum(values[v] - values[u] for v in graph.neighbors(u))
        / len(list(graph.neighbors(u)))
        for u in graph
    ]
    result = compute_fused_gradients_symmetric(
        edge_src=source,
        edge_dst=target,
        phase=np.zeros(len(graph)),
        epi=values,
        vf=np.ones(len(graph)),
        weights={"w_epi": 1.0},
        use_jit=use_jit,
...
def test_nodal_integration_applies_capacity_once_and_zero_capacity_freezes(
    ...
    np.testing.assert_array_equal(
        [get_attr(graph.nodes[n], ALIAS_DEPI) for n in graph], frequency * pressure
    )
    np.testing.assert_array_equal(
        [get_attr(graph.nodes[n], ALIAS_EPI) for n in graph],
...
def test_example_linear_hooks_retain_small_differences():
    ...
    np.testing.assert_array_equal(
        [get_attr(graph.nodes[n], ALIAS_DNFR) for n in graph], [-2, 3, -4]
    )
...
def test_underflowing_probability_keeps_representable_pressure(path, monkeypatch):
    ...
    np.testing.assert_allclose(
        _run(graph, path, monkeypatch), [1e-92, 0, 0], rtol=5e-16, atol=0
    )


@pytest.mark.parametrize("coefficient", [1e-20, 1e200])
def test_exceptional_products_round_only_after_weight_and_channel_application(
    coefficient,
):
    ...
    expected = float(
        Fraction.from_float(weights[0])
        * Fraction.from_float(neighbors[0])
        * Fraction.from_float(coefficient)
        / sum(Fraction.from_float(w) for w in weights)
    )
    scalar = mean_neighbor_difference(
        center, neighbors, weights, coefficient=coefficient
    )
    array = edge_mean_differences(
        [center, *neighbors], [0, 0], [1, 2], weights, coefficient=coefficient
    )
...
@pytest.mark.parametrize(
    "neighbors, weights, expected",
    [
        ([1e16, 1.0, -1e16], [1.0, 1.0, 1.0], 1.0 / 3.0),
        ([7e16, 1.0, -3e16], [3.0, 1.0, 7.0], 1.0 / 11.0),
    ],
)
def test_mixed_sign_cancellation_retains_weighted_residual(
    path, neighbors, weights, expected, monkeypatch
):
    graph = _graph([0.0, *neighbors], kind=nx.DiGraph)
    graph.remove_edges_from(list(graph.edges()))
    graph.add_weighted_edges_from(
        (0, target, weight) for target, weight in enumerate(weights, 1)
    )
    np.testing.assert_array_equal(
        _run(graph, path, monkeypatch), [expected, 0.0, 0.0, 0.0]
    )
...
@pytest.mark.parametrize("path", PATHS)
def test_overflow_of_assembled_channels_rejects_before_any_pressure_write(
    path, monkeypatch
):
...
def test_exact_cancellation_matches_fraction_oracle_for_seeded_weighted_rows():
    ...
    for _ in range(40):
        ...
        exact = float(
            sum(
                Fraction.from_float(w)
                * (Fraction.from_float(v) - Fraction.from_float(center))
        ...
        array = edge_mean_differences(
            [center, *neighbors], [0, 0, 0], [1, 2, 3], weights, coefficient=coefficient
        )
