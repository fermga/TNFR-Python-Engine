...
class TestDeltaNFRPhysics:
    ...
    def test_delta_nfr_magnitude_bounds(self) -> None:
        ...
        small_pressure = 0.5772156649015329 / 10  # ≈0.058
        medium_pressure = 0.5772156649015329 / 2  # ≈0.289
        large_pressure = 0.5772156649015329  # ≈0.577
        extreme_pressure = 2 * 0.5772156649015329  # ≈1.154
        ...
        for pressure in pressures:
            ...
            # Stability expectation: ΔNFR < ≈0.577 for smooth evolution
            if pressure <= 0.5772156649015329:
                # Within canonical stability bound
                assert rate <= 0.5772156649015329
            else:
                # May require stabilizers (tested in grammar tests)
                assert rate > 0.5772156649015329  # Beyond stability threshold
    ...
    def test_delta_nfr_coupling_influence(self) -> None:
        ...
        for node in G.nodes():
            ...
            if neighbors:
                ...
                avg_neighbor_pressure = sum(neighbor_pressures) / len(
                    neighbor_pressures
                )
...
class TestDeltaNFRCanonicalBounds:
    ...
    def test_gamma_stability_bound(self) -> None:
        ...
        # Test values around the ≈0.577 stability bound
        test_values = [
            0.5772156649015329 / 10,  # Well below bound
            0.5772156649015329 / 2,  # Half bound
            0.5772156649015329 * 0.9,  # Just below bound
            0.5772156649015329,  # At bound
            0.5772156649015329 * 1.1,  # Just above bound
            0.5772156649015329 * 2,  # Well above bound
        ]
    ...
    def test_phi_structural_confinement(self) -> None:
        """Test ΔNFR integration with structural-potential bounds."""
        G = nx.complete_graph(5)  # Pentagon (5-node complete graph)

        # Initialize with diverse pressure scaling
        for i, node in enumerate(G.nodes()):
            G.nodes[node]["EPI"] = f"phi_node_{node}"
            G.nodes[node]["νf"] = 1.0
            # Scale pressure with diverse multiplicative powers
            G.nodes[node]["ΔNFR"] = 0.5772156649015329 / (1.618033988749895**i)
    ...
    def test_exponential_e_decay_relationship(self) -> None:
        ...
        for i in range(len(delta_values) - 1):
            ...
            expected_ratio = math.exp(-1 / 2.718281828459045)  # ≈0.692
            if delta_values[i] > 0:
