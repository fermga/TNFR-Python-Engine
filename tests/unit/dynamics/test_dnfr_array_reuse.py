...
def test_ephemeral_subgraph_isolated_cache():
    ...
    assert (
        parent_manager_id != subgraph_manager_id
    ...
    assert parent_cache is parent_cache_after, "Parent cache should remain intact"
...
def test_centralized_sparse_dense_decision_consistency(prefer_sparse):
    ...
    assert decision1 == decision2, "prefer_sparse decision should be consistent"
    assert path_decision1 == path_decision2, "dnfr_path_decision should be consistent"
...
def test_dense_override_flag_respected():
    ...
    assert (
        data.get("dense_override") is True
    ), "dense_override should be True when dnfr_force_dense is set"
    assert data.get("dnfr_path_decision") in [
        "dense_forced",
        "dense_auto",
