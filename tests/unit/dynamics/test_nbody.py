...
class TestNBodyConservationLaws:
    ...
    def test_momentum_conservation(self):
        ...
        velocities = np.array([
            [1.0, 0.5, 0.0],
            [-0.5, -0.25, 0.0]  # Ensures total momentum = 0
        ])
...
class TestNBodyTNFRInvariants:
    ...
    def test_dnfr_semantics(self):
        ...
        velocities = np.array([
            [0.5, 0.0, 0.0],   # Moving right
            [-0.5, 0.0, 0.0]   # Moving left
        ])
        
        system.set_state(positions, velocities)
        
        # Compute ΔNFR
        dnfr = compute_gravitational_dnfr(
            system.positions, system.masses, system.G
        )
