...
def test_cli_arguments_parser_functions():
    ...
    for func_name in public_functions:
        assert hasattr(arguments, func_name), \
...
def test_callback_utils_exports():
    ...
    # Verify __all__ exists and is a tuple
    assert hasattr(callback_utils, '__all__'), \
    ...
    for symbol in expected_symbols:
        assert symbol in callback_utils.__all__, \
            f"callback_utils.__all__ should include '{symbol}'"
        assert hasattr(callback_utils, symbol), \
...
def test_utils_callbacks_exports():
    ...
    for symbol in core_symbols:
        assert symbol in callbacks.__all__, \
...
def test_utils_cache_exports():
    ...
    for symbol in expected_in_all:
        assert symbol in cache.__all__, \
            f"cache.__all__ should include '{symbol}'"
        assert hasattr(cache, symbol), \
            f"cache should export '{symbol}'"
    ...
    for symbol in private_symbols:
        assert hasattr(cache, symbol), \
            f"cache should define {symbol} for internal use"
...
def test_utils_main_exports():
    ...
    for symbol in callback_symbols:
        assert symbol in utils.__all__, \
            f"utils.__all__ should include callback symbol '{symbol}'"
        assert hasattr(utils, symbol), \
            f"utils should export callback symbol '{symbol}'"

    # Check SecurityError
    assert 'SecurityError' in utils.__all__, \
        "utils.__all__ should include 'SecurityError'"
    assert hasattr(utils, 'SecurityError'), \
        "utils should export 'SecurityError'"
...
def test_validation_exports():
    ...
    for symbol in expected_symbols:
        assert symbol in validation.__all__, \
            f"validation.__all__ should include '{symbol}'"
        assert hasattr(validation, symbol), \
            f"validation should export '{symbol}'"
...
def test_validation_rules_exports():
    ...
    for func_name in public_functions:
        assert func_name in rules.__all__, \
            f"rules.__all__ should include '{func_name}'"
        assert hasattr(rules, func_name), \
            f"rules should export {func_name}"
    ...
    for func_name in private_functions:
        ...
        assert hasattr(rules, func_name), \
            f"rules should define {func_name}"
...
def test_cross_module_import_consistency():
    ...
    assert CM3.__name__ == 'CallbackManager', \
        "callback_utils.CallbackManager should have correct name"
    assert CM3.__module__ == 'tnfr.utils.callbacks', \
