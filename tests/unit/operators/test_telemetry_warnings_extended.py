...
def test_warn_phase_gradient_pass_and_warn():
    ...
    # Create a NEW graph to avoid cache invalidation issues in tests
    G2 = nx.path_graph(4)
    for n in G2.nodes():
        G2.nodes[n]["phase"] = 0.0
    # center node pi apart from neighbors
    center = 1
    G2.nodes[center]["phase"] = math.pi
    safe2, stats2, msg2, flagged2 = warn_phase_gradient_telemetry(G2)
...
def test_warn_phase_curvature_pass_and_warn():
    ...
    for n in G2.nodes():
        G2.nodes[n]["phase"] = 0.0
    G2.nodes[1]["phase"] = math.pi
    safe2, stats2, msg2, hotspots2 = warn_phase_curvature_telemetry(G2, multiscale_check=False)
    assert safe2 is False
    assert "WARN" in msg2
