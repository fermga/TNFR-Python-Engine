...
class TestHierarchyPathTracking:
    ...
    def test_nested_bifurcation_builds_full_path(self):
        ...
        assert sub_epi_2["hierarchy_path"] == expected_path, \
...
class TestMaxDepthValidation:
    ...
    def test_no_warning_below_max_depth(self, caplog):
        ...
        assert "maximum" not in caplog.text.lower(), \
            "Should not warn when below max depth"
    ...
    def test_warning_recorded_in_node(self):
        ...
        # Check warning flag
        assert G.nodes[sub_node].get("_thol_max_depth_warning", False), \
