...
- All tests respect TNFR grammar constraints (U1-U5)
...
class TestSHAIdentityProperty:
    ...
    def test_sha_identity_with_emission(self):
        ...
        assert validate_identity_property(
            G, node, Emission()
    ...
    def test_sha_identity_preserves_epi_not_vf(self):
        ...
        assert abs(epi_sha - epi_oz) < 0.05, f"EPI diverged: SHA={epi_sha}, OZ={epi_oz}"

        # νf should be minimal with SHA (characteristic effect)
        assert vf_sha < 0.15, f"SHA did not reduce νf sufficiently: {vf_sha}"
    ...
    def test_sha_identity_in_complex_sequence(self):
        ...
        G_sha = G.copy()
        run_sequence(G_sha, node, [Emission(), Reception(), Coherence(), Resonance(), Silence()])
        epi_sha = float(get_attr(G_sha.nodes[node], ALIAS_EPI, 0.0))

        # Path with Dissonance terminator
        G_oz = G.copy()
        run_sequence(G_oz, node, [Emission(), Reception(), Coherence(), Resonance(), Dissonance()])
        epi_oz = float(get_attr(G_oz.nodes[node], ALIAS_EPI, 0.0))

        # Both terminators should preserve structural result from the sequence
        tolerance = 0.05  # Relaxed for complex sequence
        assert (
            abs(epi_sha - epi_oz) < tolerance
...
class TestSHAIdempotence:
    ...
    def test_sha_characteristic_effect_on_vf(self):
        ...
        G3 = G.copy()
        run_sequence(G3, node, [Emission(), Reception(), Coherence(), Silence()])
        ...
        assert max_diff < 0.05, f"SHA behavior inconsistent: νf values {vf_1}, {vf_2}, {vf_3}"
...
class TestSHANULCommutativity:
    ...
    def test_sha_nul_commutativity_basic(self):
        ...
        assert validate_commutativity_nul(G, node), "SHA and NUL failed commutativity test"
    ...
    def test_sha_nul_commutativity_after_activation(self):
        ...
        assert validate_commutativity_nul(
            G_test, node
    ...
    def test_sha_nul_order_independence(self):
        ...
        tolerance = 0.02
        assert (
            abs(epi_sha_nul - epi_nul_sha) < tolerance
        ), f"EPI differs by order: SHA→NUL={epi_sha_nul}, NUL→SHA={epi_nul_sha}"
        assert (
            abs(vf_sha_nul - vf_nul_sha) < tolerance
...
class TestAlgebraicPropertiesIntegration:
    ...
    def test_combined_properties_realistic_scenario(self):
        ...
        G_stored = G.copy()
        run_sequence(G_stored, node, [Emission(), Reception(), Coherence(), Silence()])
        ...
        assert (
            abs(epi_learned - epi_stored) < 0.05
        ...
        assert vf_stored < 0.15, f"SHA failed to achieve characteristic low νf: {vf_stored}"
    ...
    def test_algebraic_closure(self):
        ...
        assert final_epi > 0.3, f"Structure degraded to near-zero: {final_epi}"
