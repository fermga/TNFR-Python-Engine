...
def test_density_calculation_correct():
    ...
    assert abs(metrics["density_before"] - expected_density_before) < 1e-9, (
        f"density_before should be {expected_density_before}, "
        f"got {metrics['density_before']}"
    )
    
    # Verify density_after is positive and increased
    assert metrics["density_after"] > 0, "density_after should be positive"
    assert metrics["density_after"] > metrics["density_before"], (
...
def test_densification_ratio_correct():
    ...
    assert abs(metrics["densification_ratio"] - expected_ratio) < 1e-9, (
        f"densification_ratio should be {expected_ratio}, "
...
def test_critical_density_warning_default_threshold():
    ...
    if metrics["density_after"] > 5.0:
        assert metrics["is_critical_density"] is True, (
            "is_critical_density should be True when density exceeds threshold"
        )
    else:
        assert metrics["is_critical_density"] is False, (
...
def test_density_with_zero_dnfr():
    ...
    assert metrics["densification_ratio"] == float('inf'), (
...
def test_density_with_negative_dnfr():
    ...
    assert metrics["density_before"] > 0, (
        "density_before should be positive (uses |ΔNFR|)"
    )
    assert metrics["density_after"] > 0, (
        "density_after should be positive (uses |ΔNFR|)"
    )
...
def test_density_with_very_small_epi():
    ...
    assert metrics["density_before"] > 1e7, (
...
def test_density_metrics_integration_with_existing():
    ...
    expected_keys = {
        # Basic metrics
        "operator", "glyph", "vf_decrease", "vf_final", 
        "delta_epi", "epi_final", "dnfr_final", "contraction_factor",
        # Densification metrics
        "densification_factor", "dnfr_densified", "dnfr_before", "dnfr_increase",
        # NEW density metrics
        "density_before", "density_after", "densification_ratio", "is_critical_density"
    }
...
def test_density_metrics_observability():
    ...
    assert all(k in metrics for k in [
        "density_before", "density_after", "densification_ratio"
...
def test_density_metrics_support_research_workflows():
    ...
    for i in range(len(densities) - 1):
        assert densities[i+1]["density_before"] > densities[i]["density_before"], (
    ...
    for ratio in ratios:
        assert abs(ratio - avg_ratio) < 0.5, (
