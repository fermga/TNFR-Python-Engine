...
def test_nul_increases_dnfr_density():
    ...
    assert density_ratio >= 1.2, (
        f"Density increase must be at least 20%. "
        f"Got ratio: {density_ratio:.4f}"
    )
...
def test_nul_reduces_structural_frequency():
    ...
    assert vf_after < vf_before, (
        f"NUL must reduce νf. "
        f"Before: {vf_before:.4f}, After: {vf_after:.4f}"
    )
    
    # Verify at least 10% reduction (canonical requirement)
    vf_ratio = vf_after / vf_before
    assert vf_ratio <= 0.9, (
        f"νf reduction must be at least 10%. "
        f"Got ratio: {vf_ratio:.4f}"
    )
...
def test_nul_reduces_epi_surface():
    ...
    assert epi_after <= epi_before, (
        f"NUL must reduce EPI. "
        f"Before: {epi_before:.4f}, After: {epi_after:.4f}"
    )
...
def test_nul_rejects_low_vf():
    ...
    assert "frequency" in error_msg.lower() or "νf" in error_msg, (
...
def test_nul_rejects_low_epi():
    ...
    assert "EPI" in error_msg, (
        f"Error should mention EPI. Got: {error_msg}"
    )
...
def test_nul_rejects_critical_density():
    ...
    error_msg = str(exc_info.value)
    assert "density" in error_msg.lower() or "critical" in error_msg.lower(), (
...
def test_nul_collects_density_metrics():
    ...
    has_metrics = (
        "operator_metrics" in G.graph or
        "nul_densification_log" in G.graph or
        "metrics_log" in G.graph
    )
...
def test_expansion_contraction_coherence_cycle():
    ...
    assert epi_change < 0.5 * epi_initial, (
        f"EPI should be similar after cycle. "
        f"Initial: {epi_initial:.4f}, Final: {epi_final:.4f}"
    )
    
    # ΔNFR should be reduced (more stable)
    assert dnfr_final < 0.3, (
        f"ΔNFR should be low after coherence. "
        f"Got: {dnfr_final:.4f}"
    )
...
def test_multiple_nul_raises_warning():
    ...
    assert density > 2.0, (
        f"Initial density should be significant for this test: {density:.2f}"
    )
...
def test_nul_respects_nodal_equation():
    ...
    try:
        ...
        assert "nodal equation" not in error_msg, (
            f"Nodal equation validation failed: {e}"
        )
...
def test_nul_density_increase_physical_consistency():
    ...
    # Verify density increased
    assert density_after > density_before, (
        "Density must increase during contraction"
    )
    ...
    assert 1.3 <= actual_ratio <= 2.0, (
        f"Densification ratio should be in [1.3, 2.0], "
