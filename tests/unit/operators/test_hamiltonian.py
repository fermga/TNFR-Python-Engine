...
class TestInternalHamiltonian:
    ...
    def test_coupling_symmetric(self, simple_graph):
        ...
        assert np.allclose(ham.H_coupling, ham.H_coupling.T), \
    ...
    def test_coherence_potential_negative(self, simple_graph):
        ...
        assert ham.H_coh.dtype in [np.complex64, np.complex128, complex], \
    ...
    def test_spectrum_real_eigenvalues(self, simple_graph):
        ...
        assert np.allclose(V_dag_V, np.eye(ham.N)), \
    ...
    def test_time_evolution_unitary(self, simple_graph):
        ...
        for t in test_times:
            ...
            assert np.allclose(product, identity), \
                f"U({t}) must be unitary: U†U = I"
            
            # Also check UU† = I
            product2 = U_t @ U_dag
            assert np.allclose(product2, identity), \
                f"U({t}) must be unitary: UU† = I"
    ...
    def test_delta_nfr_operator_anti_hermitian(self, simple_graph):
        ...
        assert np.allclose(delta_op, -delta_dagger), \
            "ΔNFR operator must be anti-Hermitian"
    ...
    def test_node_delta_nfr_computation(self, simple_graph):
        ...
        assert all(np.isfinite(v) for v in delta_nfr_values), \
            "All ΔNFR values should be finite"
    ...
    def test_hamiltonian_with_custom_parameters(self):
        ...
        assert np.allclose(ham.H_int, ham.H_int.conj().T), \
            "Total Hamiltonian must remain Hermitian"
    ...
    def test_single_node_graph(self):
        ...
        assert np.allclose(ham.H_coupling, 0), \
            "Single node has no coupling"
        
        # H_freq should equal nu_f
        assert np.isclose(ham.H_freq[0, 0].real, 1.0), \
            "Frequency should match nu_f"
    ...
    def test_hamiltonian_components_sum(self, simple_graph):
        ...
        assert np.allclose(ham.H_int, H_sum), \
            "H_int must equal sum of components"
    ...
    def test_cache_manager_integration(self, simple_graph):
        ...
        assert ham._cache_manager is cache_manager, \
            "Should use provided cache manager"
...
class TestStandaloneBuilders:
    ...
    def test_build_H_coupling(self, test_graph):
        ...
        for i in range(3):
            for j in range(3):
                if i != j:
                    assert np.isclose(H_coupling[i, j], 0.3), \
                        "Off-diagonal should be J_0"
                else:
                    assert np.isclose(H_coupling[i, j], 0.0), \
...
class TestHamiltonianPhysicalProperties:
    ...
    def test_energy_conservation(self, symmetric_graph):
        ...
        assert np.isclose(E_0, E_t, rtol=1e-10), \
            "Energy must be conserved under unitary evolution"
    ...
    def test_eigenstate_time_evolution(self, symmetric_graph):
        ...
        assert np.isclose(overlap, 1.0, rtol=1e-10), \
    ...
    def test_trace_preservation(self, symmetric_graph):
        ...
        assert np.abs(trace_H.imag) < 1e-10, \
            "Trace of Hermitian operator must be real"
        
        # Trace of H_freq should equal sum of frequencies
        sum_frequencies = sum(
            ham.G.nodes[node].get('nu_f', 0.0) for node in ham.nodes
        )
        trace_H_freq = np.trace(ham.H_freq).real
        
        assert np.isclose(trace_H_freq, sum_frequencies), \
