...
def test_coherence_multiple_applications_converge_to_zero():
    ...
    for i, reduction in enumerate(reductions):
        ...
        assert reduction["after"] == pytest.approx(
            expected_after, abs=1e-4
...
def test_coherence_telemetry_logged():
    ...
    assert reduction_event["after"] == pytest.approx(reduction_event["before"] * 0.7, abs=1e-6)
...
def test_coherence_telemetry_accumulates():
    ...
    # Apply Coherence 3 times in 3 separate canonical sequences
    sequences = [
...
def test_coherence_nodal_equation_compliance():
    ...
    final_dnfr = G.nodes[node][DNFR_PRIMARY]
    assert final_dnfr < initial_dnfr, "ΔNFR should decrease with multiple IL applications"
...
def test_coherence_different_reduction_factors_logged(monkeypatch: pytest.MonkeyPatch):
    ...
    for event in non_zero_reductions:
        assert event["reduction_factor"] == pytest.approx(
            0.3, abs=0.05
