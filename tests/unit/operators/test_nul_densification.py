...
def test_nul_increases_dnfr():
    ...
    assert dnfr_after > dnfr_before, (
        f"NUL must increase ΔNFR (densification). "
        f"Before: {dnfr_before}, After: {dnfr_after}"
    )
...
def test_nul_densification_telemetry():
    ...
    assert "nul_densification_log" in G.graph, (
        "Densification telemetry must be recorded in graph"
    )
...
def test_nul_densification_with_negative_dnfr():
    ...
    assert 1.3 <= magnitude_ratio <= 1.5, (
...
def test_nul_vf_reduction_preserved():
    ...
    assert vf_after < vf_before, (
        f"NUL must reduce νf. Before: {vf_before}, After: {vf_after}"
    )
...
def test_nul_epi_reduction_preserved():
    ...
    epi_before_value = float(epi_before) if not isinstance(epi_before, (dict, complex)) else abs(epi_before)
    
    assert epi_after_value < epi_before_value, (
...
def test_nul_densification_metrics():
    ...
    assert "densification_factor" in metrics or "dnfr_increase" in metrics, (
...
def test_nul_nodal_equation_compliance():
    ...
    assert abs(product_after) < 10.0, (
        f"Product νf·ΔNFR diverged: {product_after}"
    )
    
    # Product should be positive if both were positive (directional consistency)
    if vf_before > 0 and dnfr_before > 0:
        assert product_after > 0, (
            "Product sign should be preserved for positive inputs"
        )
...
def test_nul_custom_densification_factor():
    ...
    assert abs(actual_factor - custom_factor) < 0.01, (
        f"Should use custom densification factor {custom_factor}, "
        f"got {actual_factor:.3f}"
    )
