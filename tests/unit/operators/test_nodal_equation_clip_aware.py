...
class TestClipAwareMode:
    ...
    def test_clip_aware_mode_passes_when_clip_intervenes(self):
        ...
        result = validate_nodal_equation(
            G, node, epi_before, epi_actual, dt,
            operator_name="test",
            clip_aware=True,
            tolerance=1e-9
        )
...
class TestToleranceLevels:
    ...
    def test_tolerance_from_graph_config(self):
        ...
        G, node = _create_test_nfr(
            epi=0.5, vf=1.0, dnfr=0.1,
            NODAL_EQUATION_TOLERANCE=1e-6
        )
...
class TestCustomBoundaries:
    ...
    def test_custom_boundaries_upper(self):
        ...
        G, node = _create_test_nfr(
            epi=0.45, vf=1.0, dnfr=0.2,
            EPI_MAX=0.5  # Custom upper boundary
        )
    ...
    def test_custom_boundaries_lower(self):
        ...
        G, node = _create_test_nfr(
            epi=-0.45, vf=1.0, dnfr=-0.2,
            EPI_MIN=-0.5  # Custom lower boundary
        )
...
class TestEdgeCases:
    ...
    def test_zero_dt(self):
        ...
        result = validate_nodal_equation(
            G, node, epi_before, epi_after, dt,
            operator_name="test",
            clip_aware=True
        )
