    ...
    def test_harmonic_period_vs_frequency(self):
        ...
        for nu_f in nu_f_values:
            ...
            assert err_rel < 0.001, (
                f"Period error {err_rel:.6f} exceeds 0.1% for νf={nu_f}"
            )
        ...
        for nu_f, res in results.items():
            print(f"{nu_f:6.1f} | {res['m']:6.2f} | {res['T_num']:10.3f} | "
...
class TestExperiment3KeplerTwoBody:
    ...
    def test_kepler_circular_orbit_period(self):
        ...
        print(f"Simulation time:    {history['time'][-1]:.6f} ({history['time'][-1]/T_expected:.2f} periods)")
