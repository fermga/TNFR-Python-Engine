...
def _graph(phases=(0.0, 0.25, 0.75, 1.25), *, order=None, adaptive=False):
    ...
    history = {
        "phase_state": deque(["stable"], maxlen=4),
        "phase_R": deque([0.75], maxlen=4),
        "phase_disr": deque([0.0], maxlen=4),
        "phase_kG": [0.0625],
        "phase_kL": [0.125],
    }
    graph.graph.update(
        history=history,
        PHASE_K_GLOBAL=0.0625,
        PHASE_K_LOCAL=0.125,
        PHASE_HISTORY_MAXLEN=4,
        PHASE_ADAPT={"enabled": adaptive},
        shared=shared,
    )
    for node in nodes:
        graph.nodes[node].update(
            ...
            nu_f=1.0,
            delta_nfr=0.125,
            glyph_history=deque(["IL"], maxlen=8),
            shared=shared,
        )
    if nodes:
...
def _freeze(value, seen=None):
    ...
    if isinstance(value, Mapping):
        return (
            "mapping",
            tuple((_freeze(k, seen), _freeze(v, seen)) for k, v in value.items()),
        )
...
def _primitive_expectations(graph):
    ...
    components = tuple(
        (float(cache.cos[node]), float(cache.sin[node])) for node in nodes
    )
    sums = tuple(
        sum((F.from_float(pair[i]) for pair in components), F(0)) for i in (0, 1)
    )
    scale = max(map(abs, sums))
    angle = (
        None
        if scale == 0
        else math.atan2(float(sums[1] / scale), float(sums[0] / scale))
    )
    local = tuple(
        (
            neighbor_phase_mean_list(
                tuple(graph.neighbors(node)), cache.cos, cache.sin, fallback=phase
            )
...
def _assert_aliases(graph, shared, history, cache, manager):
    ...
    assert (
        list(history["phase_state"]) == ["stable"]
        and history["phase_state"].maxlen == 4
    )
...
def test_explicit_gains_exact_global_sum_and_unchanged_local_law(monkeypatch, scalar):
    graph = _graph((0.125, -0.25, 0.75, 1.25))
    ...
    report = coordination.coordinate_global_local_phase(
        graph, global_force=0.0625, local_force=0.125, global_reduction=POLICY, n_jobs=1
    )
    ...
    assert report.requested_global_force == report.effective_global_force == 0.0625
    assert report.requested_local_force == report.effective_local_force == 0.125
    expected = tuple(
        th + 0.0625 * angle_diff(target, th) + 0.125 * angle_diff(th_l, th)
        for th, th_l in zip(phases, local, strict=True)
    )
    assert report.raw_proposals == expected
    assert report.realized_phases == tuple(
        get_theta_attr(graph.nodes[node]) for node in nodes
    )
    assert report.neighbor_order == tuple(
    ...
    for node in nodes:
        ...
        ) == (0.5 + node / 32, 1.0, 0.125)


def test_default_and_explicit_legacy_are_identical_and_return_none():
    implicit, explicit = _graph(), _graph()
    assert coordination.coordinate_global_local_phase(implicit, 0.0625, 0.125) is None
    assert (
        coordination.coordinate_global_local_phase(
            explicit, 0.0625, 0.125, global_reduction="legacy"
        )
        is None
    )
    assert tuple(get_theta_attr(implicit.nodes[n]) for n in implicit) == tuple(
        get_theta_attr(explicit.nodes[n]) for n in explicit
    )
...
def test_adaptive_gain_policy_and_local_targets_match_legacy():
    ...
    expected = tuple(
        th
        + report.effective_global_force * angle_diff(target, th)
...
def test_inactive_global_term_preserves_local_only_legacy_proposals():
    ...
    coordination.coordinate_global_local_phase(
        legacy, global_force=0.0, local_force=0.125
    )
    report = coordination.coordinate_global_local_phase(
        exact, global_force=0.0, local_force=0.125, global_reduction=POLICY
    )
    assert report.global_target is None and not report.global_term_active
    assert report.resultant.angle is not None
    assert report.realized_phases == tuple(
        get_theta_attr(legacy.nodes[n]) for n in legacy
    )
    assert exact.graph["history"] == legacy.graph["history"]
...
def test_portable_retained_phase_vector_is_order_invariant_for_fixed_components():
    ...
    for order in orders:
        ...
        report = coordination.coordinate_global_local_phase(
            graph, global_force=0.0625, local_force=0.125, global_reduction=POLICY
        )
        assert (report.resultant.real_sum, report.resultant.imag_sum) == sums
    ...
    for other in reports[1:]:
        ...
        assert dict(zip(other.nodes, other.local_targets, strict=True)) == dict(
            zip(reports[0].nodes, reports[0].local_targets, strict=True)
        )
        assert dict(zip(other.nodes, other.raw_proposals, strict=True)) == dict(
            zip(reports[0].nodes, reports[0].raw_proposals, strict=True)
        )
...
@pytest.mark.parametrize("scalar", (False, True))
def test_exact_joint_zero_with_inactive_global_gain_uses_no_direction(
    monkeypatch, scalar
):
    ...
    if scalar:
        monkeypatch.setattr(coordination, "np", None)
    report = coordination.coordinate_global_local_phase(
        graph, global_force=0.0, local_force=0.125, global_reduction=POLICY, n_jobs=1
    )
    assert report.resultant.joint_zero and report.resultant.angle is None
    assert report.global_target is None and not report.global_term_active
    assert report.raw_proposals == tuple(
        th + 0.125 * angle_diff(local_target, th)
...
def test_empty_graph_has_distinct_evidence_not_a_zero_resultant_error():
    report = coordination.coordinate_global_local_phase(
        _graph(()), global_force=0.0625, local_force=0.125, global_reduction=POLICY
    )
    assert report.status == "empty_graph" and report.resultant is None
    assert (
        report.nodes
        == report.primitive_phases
        == report.local_targets
        == report.raw_proposals
        == report.realized_phases
        == ()
    )
...
def test_real_transaction_precedes_caller_gain_conversion():
    ...
    with pytest.raises(ValueError):
        coordination.coordinate_global_local_phase(
            graph, global_force=MutatingGain(0.0625), global_reduction=POLICY
        )
...
def test_late_commit_failure_restores_real_caches_metadata_and_topology(
...
def test_scalar_process_worker_uses_same_exact_global_and_local_proposals(monkeypatch):
    ...
    monkeypatch.setattr(coordination, "np", None)
    first = coordination.coordinate_global_local_phase(
        sequential, 0.0625, 0.125, n_jobs=1, global_reduction=POLICY
    )
    second = coordination.coordinate_global_local_phase(
        parallel, 0.0625, 0.125, n_jobs=2, global_reduction=POLICY
    )
...
@pytest.mark.parametrize(
    "source,value", (("initial", math.inf), ("up", math.inf), ("kG_max", math.nan))
)
...
def test_failed_worker_count_conversion_restores_previous_gain_and_history_writes():
    ...
    with pytest.raises(ValueError, match="jobs conversion"):
        coordination.coordinate_global_local_phase(
            graph, n_jobs=MutatingJobs(), global_reduction=POLICY
        )
...
def test_final_evidence_failure_rolls_back_all_successful_phase_commits(monkeypatch):
    ...
    def fail_evidence(**values):
        seen.append(values)
        assert (
            tuple(get_theta_attr(graph.nodes[node]) for node in graph)
            == values["realized_phases"]
        )
...
def test_unknown_policy_does_not_invoke_arbitrary_equality():
    ...
    with pytest.raises(ValueError, match="global_reduction"):
        coordination.coordinate_global_local_phase(
            graph, global_reduction=MutatingPolicy()
        )
    assert _state(graph) == before
