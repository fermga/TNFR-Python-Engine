...
def experiment_1_moments_are_coefficients():
    ...
    print(f"  anchor: ||L*EPI - current_divergence|| = "
          f"{np.linalg.norm(L @ epi - divJ):.1e} "
          f"(L = D - A IS the Kirchhoff operator)")
    print()
    eigs = np.linalg.eigvalsh(L)
    moments = spectral_moments(L)
    print(f"  {'k':>3} {'Tr(L^k)=sum lambda^k':>21} {'Tr(L^k)=tr(L^k)':>17} "
          f"{'|diff|':>8}")
    for k in range(5):
        m_spec = float(np.sum(eigs ** k))
        print(f"  {k:>3} {m_spec:>21.4f} {moments[k]:>17.4f} "
    ...
    for t in [0.001, 0.005, 0.02]:
        ...
        Z_series = sum(((-t) ** k) / math.factorial(k) * moments[k]
                       for k in range(5))
        print(f"    t={t:>6.3f}:  Z_exact={Z_exact:.6f}  series={Z_series:.6f}"
...
def experiment_2_hearing_the_geometry():
    ...
    print(f"    Tr(L^1) = {moments[1]:>7.0f}   <->  2m         = {2 * m} "
          f"(edges m = {m}, boundary)")
    print(f"    Tr(L^2) = {moments[2]:>7.0f}   <->  2m + sum d^2 = "
          f"{2 * m + sum_d2:.0f}")
    print()
    ...
    print(f"    Tr(A^3) = {trA3:>7.0f}   <->  6 * #triangles = {6 * tri} "
...
def experiment_3_cannot_hear_the_shape():
    ...
    for Ga, Gb in itertools.combinations(graphs, 2):
        ...
        if np.allclose(la, lb, atol=1e-9) and not nx.is_isomorphic(Ga, Gb):
            ...
            print(f"  cospectral pair on 6 nodes (both {Ga.number_of_edges()} "
                  f"edges):")
            print(f"    shared L-spectrum = {np.round(la, 4)}")
            print(f"    {'Tr(L^k):':>12} " + "  ".join(
                f"k={k}:{np.trace(np.linalg.matrix_power(La, k)):.0f}"
                for k in range(4)))
            print(f"    {'(graph B):':>12} " + "  ".join(
                f"k={k}:{np.trace(np.linalg.matrix_power(Lb, k)):.0f}"
                for k in range(4)))
            print(f"    BUT triangles differ: {ta} vs {tb}; "
