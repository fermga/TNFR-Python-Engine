...
from tnfr.operators.definitions import (
    Emission, Coherence, Dissonance, Mutation, Contraction,
    SelfOrganization, Resonance, Silence, Transition, Coupling,
)
from tnfr.operators.grammar_validate import validate_grammar

INST = {
    "AL": Emission(), "IL": Coherence(), "OZ": Dissonance(),
    "ZHIR": Mutation(), "NUL": Contraction(), "THOL": SelfOrganization(),
    "RA": Resonance(), "SHA": Silence(), "NAV": Transition(), "UM": Coupling(),
}
...
def experiment_1_real_bifurcation():
    ...
    print(f"    ZHIR     {dz_th:.4f}     {dz_ep:.4f}    PHASE (reorganize, theta->theta')")
    print(f"    NUL      {dn_th:.4f}     {dn_ep:.4f}    STRUCTURE (collapse, dim EPI down)")
...
def experiment_2_alternation_regular():
    ...
    for k in (1, 2, 3):
        ...
        print(f"    k={k} binary choices: 2^{k}={2**k} expansions, "
...
def experiment_3_regexp_algebra():
    ...
    print(f"  Fractal {fractal}  valid={valid(fractal)} "
