...
def run_uncertainty_demo(results_dir):
    ...
    for i, sigma_t in enumerate(sigmas_t):
        ...
        print(
            f"Packet {i+1}: Sigma_t = {std_t:.3f}, Sigma_f = {std_f:.3f}, Product = {product:.4f}"
        )
...
def run_interference_demo(results_dir):
    ...
    SLIT_1_Y = range(
        SLIT_Y_CENTER - SLIT_SEP // 2 - SLIT_WIDTH, SLIT_Y_CENTER - SLIT_SEP // 2
    )
    SLIT_2_Y = range(
        SLIT_Y_CENTER + SLIT_SEP // 2, SLIT_Y_CENTER + SLIT_SEP // 2 + SLIT_WIDTH
    )
