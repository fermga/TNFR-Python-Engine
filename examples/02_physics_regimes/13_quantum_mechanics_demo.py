...
def run_quantum_simulation():
    ...
    for step in range(STEPS):
        ...
        delta_E = (LEARNING_RATE * dissonance * direction) + np.random.normal(
            0, NOISE_LEVEL, NUM_NODES
        )
    ...
    theoretical_levels = QuantumMechanicsMapper.calculate_theoretical_levels(
        BOX_LENGTH, n_max + 1
    )
    ...
    for i, n in enumerate(range(1, 4)):
        ...
        plt.plot(
            x,
            psi + i * 2.5,
            label=f"n={n} (E={theoretical_levels[i]:.1f})",
            linewidth=2,
        )
