...
def demo_charge_drift() -> None:
    ...
    print()
    print(f"  {'Topology':<18}  {'Q(t_0)':>10}  {'Q(t_f)':>10}  "
    ...
    for name, topo, n in topologies:
        ...
        print(f"  {name:<18}  {q_0:10.4f}  {q_f:10.4f}  "
...
def demo_conservation_quality() -> None:
    ...
    print()
    print(f"  {'Step':>6}  {'Quality':>10}  {'RMS_res':>10}  "
          f"{'Drift':>10}  {'GVI':>10}")
    print("  " + "-" * 50)
...
def demo_sector_decomposition() -> None:
    ...
    print(f"    Potential sector  (Phi_s, J_DELTA_NFR): {V_pot:10.4f}  "
          f"({V_pot/E_total*100:.1f}%)")
    print(f"    Geometric sector  (K_phi, J_phi):       {V_geo:10.4f}  "
          f"({V_geo/E_total*100:.1f}%)")
    print(f"    Gradient sector   (|grad_phi|):         {V_grad:10.4f}  "
    ...
    r_potential = np.corrcoef(ps, jd)[0, 1] if np.std(ps) > 1e-10 and np.std(jd) > 1e-10 else 0.0
    r_geometric = np.corrcoef(kp, jp)[0, 1] if np.std(kp) > 1e-10 and np.std(jp) > 1e-10 else 0.0
...
def demo_cross_topology() -> None:
    ...
    print(f"  {'Config':<18}  {'Noether Q':>10}  {'Energy E':>10}  "
    ...
    for name, topo, n in configs:
        ...
        print(f"  {name:<18}  {noether_q:10.4f}  {energy:10.4f}  "
...
def demo_scaling_analysis() -> None:
    ...
    print()
    print(f"  {'N':>6}  {'Quality':>10}  {'RMS_res':>10}  "
    ...
    for n in sizes:
        ...
        print(f"  {n:6d}  {balance.conservation_quality:10.4f}  "
...
def demo_lyapunov_stability() -> None:
    ...
    print()
    print(f"  {'Step':>6}  {'Energy E':>12}  {'dE/dt':>12}  {'E >= 0':>8}  {'dE/dt <= 0':>12}")
    print("  " + "-" * 54)
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 5 == 0 or step <= 3:
            print(f"  {step:6d}  {E_curr:12.4f}  {dE_dt:12.6f}  "
