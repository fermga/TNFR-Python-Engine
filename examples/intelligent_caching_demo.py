...
def example_decorator_caching():
    ...
    print(
        f"  Call counts: expensive={call_count['expensive']}, cheap={call_count['cheap']}"
    )
...
def example_graph_change_tracking():
    ...
    print(
        f"  Adjacency cached: {cache.get('adjacency', CacheLevel.GRAPH_STRUCTURE) is not None}"
    )
    ...
    # Cache node property
    cache.set("node_metric_n1", 0.75, CacheLevel.DERIVED_METRICS, {"node_epi_n1"})

    print("\nCache node metric:")
    print(
        f"  Node metric cached: {cache.get('node_metric_n1', CacheLevel.DERIVED_METRICS)}"
    )
...
def example_persistent_cache():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = cache.get_persistent(
            "large_graph_coherence", CacheLevel.DERIVED_METRICS
        )
...
def example_cache_eviction():
    ...
    # High-cost entry
    cache.set(
        "expensive_1", "x" * 1000, CacheLevel.TEMPORARY, set(), computation_cost=100.0
    )
    ...
    print(
        f"  Cheap entry access count: {cache._caches[CacheLevel.TEMPORARY]['cheap_1'].access_count}"
    )
    print(
        f"  Expensive entry access count: {cache._caches[CacheLevel.TEMPORARY]['expensive_1'].access_count}"
    )
    ...
    print(
        f"  Memory used: {stats['memory_used_mb']:.2f} / {stats['memory_limit_mb']:.2f} MB"
    )
    ...
    print(
        f"  Expensive entry: {cache.get('expensive_1', CacheLevel.TEMPORARY) is not None}"
    )
