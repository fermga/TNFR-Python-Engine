"""
TNFR Triatomic Molecule Atlas (Telemetry-only)

Composes simple triatomic molecules by merging element-like radial graphs and adding
...
Output: examples/output/triatomic_atlas.html
"""
...
def analyze_triatomic(Z1: int, Z2: int, Z3: int, *, seed: int = 123, central: str = "B") -> Dict[str, Any]:
    G = build_triatomic_molecule_graph(Z1, Z2, Z3, seed=seed, bond_links=1, central=central)
    ...
    formula = f"{_symbol(Z1)}{_symbol(Z2)}{_symbol(Z3)}"
    row = {
        "Z1": Z1,
        "Z2": Z2,
        "Z3": Z3,
        "formula": formula,
        "central": str(G.graph.get("central_atom", central.upper())),
        "geometry": str(G.graph.get("geometry_hint", "unknown")),
        "angle_deg": float(G.graph.get("angle_est_deg", 120.0)),
        "xi_c": xi_c,
...
def render_html(rows: List[Dict[str, Any]], *, out_path: str) -> None:
    ...
    html = [
        "<html>",
        "<head><meta charset='utf-8'><title>TNFR Triatomic Molecule Atlas</title>",
        ...
        "</head>",
        "<body>",
        "<h1>TNFR Triatomic Molecule Atlas</h1>",
        ...
        "<p>We compose three element-like networks and add coupling edges to emulate triatomic bonding (UM/RA at topology level). Read-only telemetry summarizes stability and ΔΦ_s checks.</p>",
        "<table><thead><tr><th>Formula</th><th>Central</th><th>Geometry</th><th>Angle°</th><th>Signature</th><th>ξ_C</th><th>mean|∇φ|</th><th>mean|K_φ|</th><th>mean path len</th><th>local frac</th><th>ΔΦ_s ok</th><th>ΔΦ_s</th></tr></thead><tbody>",
    ]
    for r in rows:
        html.append(
            f"<tr><td>{r['formula']}</td><td>{r.get('central', '')}</td><td>{r.get('geometry', '')}</td><td>{r.get('angle_deg', 0.0):.1f}</td>"
            f"<td>{r.get('signature', '')}</td><td>{r['xi_c']:.2f}</td>"
            ...
            f.write(
                ",".join(
                    [
                        ...
                        str(r.get("central", "")),
                        str(r.get("geometry", "")),
                        f"{float(r.get('angle_deg', 0.0)):.1f}",
                        str(r.get("signature", "")),
...
def main() -> str:
    rows = [analyze_triatomic(a, b, c) for (a, b, c) in TRIPLES]
    base = os.path.join(os.path.dirname(__file__), "output")
    out_html = os.path.join(base, "triatomic_atlas.html")
    out_jsonl = os.path.join(base, "triatomic_atlas.jsonl")
    out_csv = os.path.join(base, "triatomic_atlas.csv")
...
if __name__ == "__main__":
    path = main()
    print(f"Wrote triatomic atlas to: {path}")
