...
from benchmarks.c6_winding_mean_cylinder import INPUT_NAME, replay_c6_phase_tail_evidence  # noqa: E402
...
def _relay_budget_record(parent, relay, result, step_budget):
    ...
    return {
        ...
        "source": {
            "retained_B43_endpoint": asdict(state), "phase": profile.lattice.source.phase,
            "historical_nodal_steps_replayed": 356, "serialized_numerical_chain_verified": True,
            "live_execution_seal_recreated": False, "capacity": (1.,) * 6,
            "timestep": SEGMENT_DURATION, "origin_mean": closure.base_tube.initial_mean,
            "origin_energy": closure.base_tube.initial_energy, "energy_bound": closure.energy_bound,
            "historical_terminal_SHA_capacity": tuple(
                parent["B43_original_phase_tail_runtime"]["terminal_silence"]["capacity_after"],
            ),
        },
...
def replay_c6_relay_budget_evidence(parent, *, historical_bytes):
    ...
    if (manifest.claim_id != "O3.a-C6-carried-two-relay-budget"
    ...
    if hashlib.sha256(historical_bytes).hexdigest() != evidence["sha256"]:
        raise ValueError("the retained B43 input hash differs from B46's declared source")
    historical = json.loads(historical_bytes)
    if (_canonical(evidence["producer_manifest"]) != _canonical(historical["manifest"])
            or _canonical(evidence["producer_source_scope"]) != _canonical(historical["source_scope"])):
    ...
    payload = {key: value for key, value in parent.items() if key not in ("manifest", "source_scope", "input_evidence")}
    if _canonical(payload) != _canonical(expected):
        raise ValueError("the retained B46 result differs from its complete numerical replay")
    if result.endpoint is None:
        raise ValueError("a band-failed B46 branch has no admitted endpoint for handoff")
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_relay_budget.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-two-relay-budget", git_sha=sha,
        ...
        telemetry=("canonical pressure rows", "relay strip inclusion", "exact corrected-coordinate drift",
                   "analytic family-exit bound", "exact nodal area and mean"),
        controls=("unaltered B43 endpoint carry", "both nearest-even tie orientations",
                  "no new graph-owned word or live seal", "family exit differs from whole-band escape"),
