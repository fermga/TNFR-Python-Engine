...
from tnfr.physics.c6_phase_orbit import derive_c6_coupling_coherence_phase_orbit  # noqa: E402
...
from tnfr.physics.nodal_remainder import (  # noqa: E402
    derive_nodal_remainder_cell_horizon, observe_nodal_remainder_cell_exit,
)
from tnfr.physics.nodal_remainder_pressure import (  # noqa: E402
    derive_periodic_phase_source_budget, observe_finite_nodal_pressure_drift,
)
...
LOCAL_LOWER, LOCAL_UPPER = .375, .625
STEP = 1 / 16
...
def _validated_phase_source(parent):
    ...
    if manifest.claim_id != "O3.a-C6-default-phase-orbit-closure" or tuple(parent["source_scope"]) != SOURCE_SCOPE:
        raise ValueError("the input must be the retained B25 phase-orbit report")
    if (parent["status"] != "exact_phase_cycle" or parent["conditional_phase_periodic"] is not True
            or type(parent["discovery_ceiling"]) is not int or parent["discovery_ceiling"] != 256
            or type(parent["discovery_transitions"]) is not int or parent["discovery_transitions"] != 90
            or parent["phase_proposal_kernels_executed"] is not True
            or type(parent["new_graph_trajectories"]) is not int or parent["new_graph_trajectories"] != 0
            or any(parent[name] is not False for name in (
                "runtime_executed", "operator_word_admission_certified", "live_provenance_certified",
                "future_runtime_certified", "EPI_band_invariance_certified", "actual_mean_compensation_verified",
            ))):
        raise ValueError("the parent must retain its declared phase-only discovery and unproved runtime conditions")
    data = parent["phase_orbit"]
    states = tuple(_represented_vector(row) for row in data["phase_states"])
    if (len(states) != 91 or type(data["preperiod"]) is not int or data["preperiod"] != 89
            or type(data["period"]) is not int or data["period"] != 1):
    ...
    if (signatures[0] != tuple(value.hex() for value in BASE_PHASE)
            or len(set(signatures[:-1])) != 90):
        raise ValueError("the retained path must start at the null winding and preserve its first-repeat boundary")
    source = parent["source"]
    if (source["mode"] != "null" or _represented_scalar(source["epsilon"]) != 0.0
            or tuple(value.hex() for value in _represented_vector(source["phase"])) != signatures[0]):
        raise ValueError("the retained source differs from the declared null initial phase")
    orbit = derive_c6_coupling_coherence_phase_orbit(phase_states=states, cycle_start=89)
    if _payload(asdict(orbit)) != data:
        raise ValueError("the complete retained phase orbit differs from its shared-owner replay")
    weights = {name: _represented_scalar(value) for name, value in source["normalized_weights"]}
    if weights != normalize_weights(DEFAULTS["DNFR_WEIGHTS"], ("phase", "epi", "vf", "topo")):
        raise ValueError("the phase source must retain the canonical default coefficients")
    reference = derive_c6_pressure_lattice(
        phase=orbit.steps[-1].phase_after_coherence, epi_weight=weights["epi"], phase_weight=weights["phase"],
        epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
    )
    periodic = derive_periodic_phase_source_budget(phase_contributions=(reference.sources,), block_duration=.25)
    if _payload(asdict(periodic)) != parent["periodic_phase_source"]:
        raise ValueError("the retained periodic source differs from the regenerated canonical pressure contribution")
    ...
    for node in range(6):
        for name, direction in (("down", -math.inf), ("up", math.inf)):
            yield f"node_{node}_{name}", tuple(
                math.nextafter(.5, direction) if index == node else .5 for index in range(6)
            )


def _balanced_boundary(reference, observation):
    initial = initialize_nodal_remainder(observation.epi, epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER)
    horizon = derive_nodal_remainder_cell_horizon(
        state=initial, timestep=STEP, capacity=(1.0,) * 6, pressure=observation.pressure,
    )
    if horizon.first_exit_step is None or any(horizon.first_exit_leaves_band):
        raise ValueError("the balanced witness must have a finite first cell exit inside the declared slab")
    count = horizon.first_exit_step
    replay = observe_nodal_remainder_cell_exit(
        state=initial, timestep=STEP, capacity=(1.0,) * 6,
        pressure=observation.pressure, step_budget=count,
    ).sequence
    step = replay.steps[-1]
    before_unchanged = tuple(item.before.epi == initial.epi for item in replay.steps)
    intermediate_unchanged = tuple(item.after.epi == initial.epi for item in replay.steps[:-1])
    refreshed = observe_c6_pressure_lattice(reference, epi=step.after.epi)
    mean_exact_change = sum(step.after.exact_epi, Fraction(0)) / 6 - sum(initial.exact_epi, Fraction(0)) / 6
    if observation.mean_pressure != 0 or horizon.mean_increment != 0 or mean_exact_change != 0:
        raise RuntimeError("the balanced finite source lost its exact reconstructed-mean identity")
    return {
        "initial": asdict(initial), "horizon": asdict(horizon), "last_unchanged_state": asdict(step.before),
        "first_exit_step": asdict(step), "refreshed_pressure": _record(refreshed),
        ...
        "shared_prefix_balances": tuple({
            "ordinal": prefix.ordinal, "mean_nodal_area": prefix.mean_nodal_area,
        ...
        "visible_mean_change_through_exit": sum(map(Fraction, step.after.epi), Fraction(0)) / 6
...
def _finite_class_drift(observations, balanced):
    ...
    for item in observations:
        ...
        if deficit <= 0:
            return None
        ratios.append(abs(sum((a * value for a, value in zip(b, p, strict=True)), Fraction(0))) / deficit)
    bound = max(ratios)
    epsilon = 1 / (1 + 2 * bound)
    ...
    drift = observe_finite_nodal_pressure_drift(
        ...
        functional=functional, timestep=STEP, epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
    )
    return {
        "ratio_bound": bound, "separator_epsilon": epsilon, "observation": asdict(drift),
...
def analyze_c6_winding_pressure_lattice(parent):
    ...
    observations = tuple(observe_c6_pressure_lattice(reference, epi=epi) for _, epi in points)
    balanced = tuple(index for index, item in enumerate(observations) if item.mean_pressure == 0)
    boundary = _balanced_boundary(reference, observations[balanced[0]]) if len(balanced) == 1 else None
    drift = _finite_class_drift(observations, observations[balanced[0]]) if len(balanced) == 1 else None
    return {
        "phase_source": {
            "initial": orbit.phase_states[0], "terminal": orbit.phase_states[-1],
            "preperiod": orbit.preperiod, "period": orbit.period,
            "complete_retained_orbit_replayed": True, "new_phase_search_executed": False,
        },
        "lattice_reference": asdict(reference),
        "stencil": tuple({"name": name, "observation": _record(item)}
                         for (name, _), item in zip(points, observations, strict=True)),
        "balanced_stencil_indices": balanced, "balanced_boundary": boundary, "finite_class_drift": drift,
        "runtime_executed": False, "new_graph_trajectories": 0,
        "static_pressure_points": len(points) + (boundary is not None),
        "live_provenance_certified": False, "tail_epi_state_reachability_certified": False,
        "future_mean_compensation_verified": False, "correlated_invariant_set_excluded": False,
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "artifacts/research/c6_winding_phase_orbit.json")
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_lattice.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-local-pressure-lattice-compensation", git_sha=sha,
        ...
        telemetry=("generated pressure compensation", "Cartesian corner sign obstruction",
                   "finite stencil separating functional", "first-exit fresh pressure"),
        controls=("uniform half-EPI and twelve immediate float neighbors", "no reachable-tail assumption",
    ...
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or args.input.read_bytes() != source:
        raise RuntimeError("analysis source or historical input changed during the local lattice audit")
    report.update(
