...
from benchmarks.c6_winding_pressure_lattice import (  # noqa: E402
    LOCAL_LOWER, LOCAL_UPPER, STEP, _balanced_boundary, _record, _stencil,
)
...
from tnfr.physics.nodal_remainder_pressure import observe_finite_nodal_pressure_drift  # noqa: E402
...
def _state(data):
    state = NodalRemainderState(
        _represented_vector(data["epi"]), tuple(_number(value) for value in data["remainder"]),
        _represented_scalar(data["epi_lower"]), _represented_scalar(data["epi_upper"]),
    )
    state.exact_epi
    return state
...
def _validated_source(parent):
    ...
    if manifest.claim_id != "O3.a-C6-local-pressure-lattice-compensation" or tuple(parent["source_scope"]) != SOURCE_SCOPE:
        raise ValueError("input must be the retained B26 local pressure-lattice report")
    if (type(parent["new_graph_trajectories"]) is not int or parent["new_graph_trajectories"] != 0
            or type(parent["static_pressure_points"]) is not int or parent["static_pressure_points"] != 14
            or any(parent[name] is not False for name in (
                "runtime_executed", "live_provenance_certified", "tail_epi_state_reachability_certified",
                "future_mean_compensation_verified", "correlated_invariant_set_excluded", "positive_band_exit_certified",
            ))):
        raise ValueError("the parent must retain its static and conditional carried-prefix scope")
    phase = parent["phase_source"]
    terminal = _represented_vector(phase["terminal"])
    if (type(phase["preperiod"]) is not int or phase["preperiod"] != 89
            or type(phase["period"]) is not int or phase["period"] != 1
        ...
        raise ValueError("the parent phase declaration differs from the inherited null orbit")
    closed = observe_c6_coupling_coherence_phase_step(phase=terminal)
    if tuple(value.hex() for value in closed.phase_after_coherence) != tuple(value.hex() for value in terminal):
        raise ValueError("the inherited terminal phase is not closed under the actual proposal owners")
    source = parent["lattice_reference"]["source"]
    defaults = normalize_weights(DEFAULTS["DNFR_WEIGHTS"], ("phase", "epi", "vf", "topo"))
    if (_represented_scalar(source["epi_weight"]) != defaults["epi"]
        ...
        raise ValueError("the inherited pressure source differs from its phase or default coefficients")
    reference = derive_c6_pressure_lattice(
        phase=terminal, epi_weight=defaults["epi"], phase_weight=defaults["phase"],
        epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
    )
    if _payload(asdict(reference)) != parent["lattice_reference"]:
        raise ValueError("the inherited local lattice differs from its shared-owner reconstruction")
    points = tuple(_stencil())
    indices = parent["balanced_stencil_indices"]
    if (tuple(indices) != (1,) or any(type(index) is not int for index in indices)
            or len(parent["stencil"]) != len(points)
            or any(record["name"] != name or _represented_vector(record["observation"]["epi"]) != epi
                   for record, (name, epi) in zip(parent["stencil"], points, strict=True))):
        raise ValueError("the retained stencil or unique balanced-state declaration changed")
    balanced = observe_c6_pressure_lattice(reference, epi=points[1][1])
    if balanced.mean_pressure != 0 or _payload(_record(balanced)) != parent["stencil"][1]["observation"]:
        raise ValueError("the retained balanced pressure differs from actual canonical generation")
    ...
    if _payload(verified_boundary) != parent["balanced_boundary"]:
        raise ValueError("the inherited carried boundary differs from its complete shared-prefix replay")
...
def _itinerary_record(itinerary):
    ...
    for name in ("witness_sequence", "epi_states", "timesteps", "capacities", "pressures"):
        data.pop(name, None)
    ...
    data["witness_prefix_balances"] = (
        _prefix_summary(itinerary.witness_sequence) if itinerary.witness_sequence is not None else None
    )
    return data
...
def _four_state_drift(balanced, initial, first_pressure, boundaries):
    states = (balanced.epi, initial.epi) + tuple(tuple(item["endpoint"]["epi"]) for item in boundaries)
    ...
    for node in range(6):
        ...
        observed = observe_finite_nodal_pressure_drift(
            epi_states=states, pressure_vectors=pressures, functional=functional,
            timestep=STEP, epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
        )
        return {
            "status": "axis_separator", "node": node, "sign": sign,
...
def analyze_c6_winding_carry_itinerary(parent):
    ...
    for ordinal in range(1, BOUNDARY_COUNT + 1):
        ...
        horizon = derive_nodal_remainder_cell_horizon(
            state=state, timestep=STEP, capacity=(1.0,) * 6, pressure=pressure,
        )
        count = horizon.first_exit_step
        if count is None or any(horizon.first_exit_leaves_band):
            raise ValueError("the declared next boundary must exist inside the local EPI slab")
        replay = observe_nodal_remainder_cell_exit(
            state=state, timestep=STEP, capacity=(1.0,) * 6,
            pressure=pressure, step_budget=count,
        ).sequence
        refreshed = observe_c6_pressure_lattice(reference, epi=replay.endpoint.epi)
        boundaries.append({
            "ordinal": ordinal, "initial": asdict(state), "horizon": asdict(horizon),
            "source_pressure": current_pressure, "endpoint": asdict(replay.endpoint),
            "refreshed_pressure": _record(refreshed), "prefix_balances": _prefix_summary(replay),
    ...
    actual = observe_nodal_remainder_sequence(
        initial=initial, timesteps=times, capacities=capacities, pressures=tuple(pressures),
    )
    if actual.steps != tuple(steps) or actual.endpoint != state:
        raise RuntimeError("whole continuation replay differs from the two boundary prefixes")
    itinerary = derive_nodal_remainder_itinerary(
        epi_states=visible, timesteps=times, capacities=capacities, pressures=tuple(pressures),
        epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
    )
    ...
    for length in SELF_LOOP_LENGTHS:
        control = derive_nodal_remainder_itinerary(
            epi_states=(balanced.epi,) * (length + 1), timesteps=(STEP,) * length,
            capacities=((1.0,) * 6,) * length, pressures=(balanced.pressure,) * length,
            epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
        )
        loops.append({
            "length": length, "epi": balanced.epi, "pressure": balanced.pressure,
            "pressure_mean": balanced.mean_pressure, "itinerary": _itinerary_record(control),
            "scope": "A proposed visible self-loop with existential carry, not a reached augmented-state cycle",
        })
    return {
        "source": {
            "location": "balanced_boundary.first_exit_step.after", "inherited_state": asdict(initial),
            "retained_B26_prefix_replayed": True, "phase": reference.source.phase,
            "pressure_generated_by_shared_cpu_kernel": True,
        },
        "boundary_budget": BOUNDARY_COUNT, "boundaries": tuple(boundaries),
        "continuation": {
            "step_count": len(steps), "steps": tuple(map(asdict, steps)),
            "prefix_balances": _prefix_summary(actual), "endpoint": asdict(actual.endpoint),
        ...
        "four_state_drift": _four_state_drift(balanced, initial, first_pressure, boundaries),
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_lattice.json")
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_carry_itinerary.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-cell-itinerary", git_sha=sha,
        ...
        telemetry=("actual-carry continuation", "generated pressure mean sign changes",
                   "exact inverse itinerary cells", "twelve/thirteen-step visible self-loop controls",
                   "four-state signed-coordinate drift"),
        controls=("exactly two boundary transitions", "no carry reset", "no reachable-tail assumption"),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or args.input.read_bytes() != source:
        raise RuntimeError("analysis source or retained input changed during the finite itinerary audit")
    report.update(
