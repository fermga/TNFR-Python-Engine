...
def _extract_records(raw: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in raw:
        ...
        rec = {
            "topology": topo,
            "phi_s_mean": rd.get("phi_s_mean", row.get("phi_s_mean", math.nan)),
...
def main():  # CLI entry
    parser = argparse.ArgumentParser(
        description=("Compute per-topology tetrad correlations with coherence metrics")
    )
    parser.add_argument(
        "--inputs",
        nargs="+",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("benchmarks/results/tetrad_universality_summary.jsonl"),
        help=(
