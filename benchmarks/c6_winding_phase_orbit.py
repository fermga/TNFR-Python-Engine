...
from tnfr.physics.c6_phase_orbit import (  # noqa: E402
    derive_c6_coupling_coherence_phase_orbit, observe_c6_coupling_coherence_phase_step,
)
from tnfr.physics.nodal_remainder_pressure import derive_periodic_phase_source_budget  # noqa: E402
...
def _retained_prefix_bindings(case, steps):
    ...
    for retained, step in zip(case["cycles"], steps[:2], strict=True):
        ...
        checks = {
            "before_matches": matches(step.phase_before, retained["before_capture"]["phase"]),
            "UM_matches": matches(step.phase_after_coupling, retained["um"]["raw_capture"]["phase"]),
            "IL_matches": matches(step.phase_after_coherence, retained["il"]["raw_capture"]["phase"]),
        }
        if not all(checks.values()):
            raise ValueError("phase proposal prefix differs from the two retained B20 UM/IL transitions")
...
def analyze_c6_winding_phase_orbit(parent):
    ...
    if initial != BASE_PHASE:
        raise ValueError("the discovery source must be the inherited null winding preparation")
    weights = {name: _represented_scalar(value) for name, value in case["initial_capture"]["normalized_weights"]}
    defaults = normalize_weights(DEFAULTS["DNFR_WEIGHTS"], ("phase", "epi", "vf", "topo"))
    if weights != defaults:
        raise ValueError("the inherited phase-source coefficients must retain the canonical defaults")
    states, steps, cycle_start = _discover(initial)
    prefix = _retained_prefix_bindings(case, steps)
    report = {
        "source": {
            "mode": "null", "epsilon": Fraction(0),
            "location": "current_cases[0].initial_capture.phase", "phase": initial,
            "first_two_retained_bindings": prefix,
            "normalized_weights": tuple((name, Fraction(value)) for name, value in weights.items()),
        },
        "discovery_ceiling": DISCOVERY_CEILING, "discovery_transitions": len(steps),
        "status": "exact_phase_cycle" if cycle_start is not None else "no_repeat_within_ceiling",
        "runtime_executed": False, "phase_proposal_kernels_executed": True,
        "new_graph_trajectories": 0, "operator_word_admission_certified": False,
        "live_provenance_certified": False, "future_runtime_certified": False,
        "EPI_band_invariance_certified": False, "actual_mean_compensation_verified": False,
        "scope": (
    ...
    if cycle_start is None:
        report["finite_phase_path"] = {"phase_states": states, "steps": tuple(map(asdict, steps))}
        return report
    orbit = derive_c6_coupling_coherence_phase_orbit(phase_states=states, cycle_start=cycle_start)
    if orbit.steps != steps:
        raise RuntimeError("independent supplied-orbit replay differs from bounded discovery")
    ...
    cycle_phases = tuple(step.phase_after_coherence for step in orbit.steps[cycle_start:])
    obstructions = tuple(derive_binary64_c6_pressure_equilibrium_obstruction(
        phase=phase, epi_weight=weights["epi"], phase_weight=weights["phase"],
    ) for phase in cycle_phases)
    contributions = tuple(tuple(row.phase_contribution for row in result.rows) for result in obstructions)
    source_budget = derive_periodic_phase_source_budget(
        phase_contributions=contributions, block_duration=BLOCK_DURATION,
    )
    report.update(
        phase_orbit=asdict(orbit), conditional_phase_periodic=orbit.conditional_phase_periodic,
        cycle_pressure_obstructions=tuple(map(asdict, obstructions)),
        periodic_phase_source=asdict(source_budget),
        every_cycle_phase_excludes_zero_pressure=all(item.no_zero_pressure for item in obstructions),
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_phase_orbit.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-default-phase-orbit-closure", git_sha=sha,
        ...
        telemetry=("complete transient and periodic phase states", "UM midpoint and all U3 margins",
                   "terminal pressure-equilibrium obstruction", "periodic phase-source mean budget"),
        controls=("one inherited null preparation", "fixed discovery ceiling 256",
                  "first two retained phase transitions", "no full-word admission or nonphase-area claim"),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or args.input.read_bytes() != source:
        raise RuntimeError("analysis source or historical input changed during the phase-only audit")
    report.update(
