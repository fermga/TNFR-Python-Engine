...
def _compute_delta(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ["hits", "misses", "evictions", "entries"]:
        delta["edge_cache"][key] = after["edge_cache"].get(key, 0) - before["edge_cache"].get(key, 0)
    
    # TNFR cache delta
    for key in ["hits", "misses", "evictions"]:
        delta["tnfr_cache"][key] = after["tnfr_cache"].get(key, 0) - before["tnfr_cache"].get(key, 0)
    
    # Combined totals
    delta["total_hits"] = delta["edge_cache"]["hits"] + delta["tnfr_cache"]["hits"]
    delta["total_misses"] = delta["edge_cache"]["misses"] + delta["tnfr_cache"]["misses"]
...
def profile_comprehensive(
    *,
    ...
    for path_name, compute_fn in hot_paths.items():
        ...
        total_edge_hits = sum(m["cache_delta"]["edge_cache"]["hits"] for m in step_metrics)
        total_edge_misses = sum(m["cache_delta"]["edge_cache"]["misses"] for m in step_metrics)
        total_tnfr_hits = sum(m["cache_delta"]["tnfr_cache"]["hits"] for m in step_metrics)
        total_tnfr_misses = sum(m["cache_delta"]["tnfr_cache"]["misses"] for m in step_metrics)
        ...
        results["hot_paths"][path_name] = {
            "edge_cache": {
                ...
                "hit_rate": total_edge_hits / (total_edge_hits + total_edge_misses) if (total_edge_hits + total_edge_misses) > 0 else 0.0,
            },
            "tnfr_cache": {
                "hits": total_tnfr_hits,
                "misses": total_tnfr_misses,
                "hit_rate": total_tnfr_hits / (total_tnfr_hits + total_tnfr_misses) if (total_tnfr_hits + total_tnfr_misses) > 0 else 0.0,
            },
    ...
    total_hits = results["summary"]["total_edge_hits"] + results["summary"]["total_tnfr_hits"]
    total_misses = results["summary"]["total_edge_misses"] + results["summary"]["total_tnfr_misses"]
    ...
    print("=" * 70)
    print(f"Configuration: {node_count} nodes, {G.number_of_edges()} edges, {steps} steps")
    ...
    print(f"  Buffer Reuse Rate:        {results['summary']['buffer_reuse_rate']*100:.1f}%")
    print(f"\nFinal Cache State:")
    print(f"  Edge Cache Entries:       {results['final_state']['edge_cache']['entries']}")
    print(f"  Total Cached Arrays:      {results['final_state']['total_cached_arrays']}")
    print(f"  DNFR Prep Cache:          {'Present' if results['final_state']['dnfr_prep']['exists'] else 'Not initialized'}")
    print(f"\nPer Hot Path:")
