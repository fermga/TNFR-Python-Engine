...
def _support_readout(graph):
    ...
    partial = tuple(
        ...
        for epi, vf, topo in zip(
            snapshot.epi_gradient, snapshot.capacity_gradient,
            snapshot.topology_gradient, strict=True,
        )
    )
    return snapshot, {
        "snapshot": asdict(snapshot), "normalized_channel_weights": weights,
        "exact_nonphase_pressure": partial,
        "exact_stored_minus_nonphase_pressure": tuple(
            stored - modeled for stored, modeled in zip(
                snapshot.stored_pressure, partial, strict=True,
            )
        ),
        "scope": (
    ...
    return {
        ...
        "checkerboard_reference": {
            ...
            "exact_endpoint_residual": tuple(
                Fraction.from_float(actual) - ideal for actual, ideal in zip(
                    before["epi"], exact_endpoint, strict=True,
                )
            ),
            "scope": (
...
def _couple_parent(graph, *, refresh_sample):
    ...
    proposal = propose_coupling_stage(
        graph, (0,), factors, resolved_seed=seed, node_offsets=offsets,
    )
    ...
    return {
        "target": 0, "actual_admission": admission,
        ...
        "before": before, "before_support": before_support,
        ...
        "actual_new_edge_phase_separations": tuple(
            abs(angle_diff(phases[u], phases[v]))
            for u, v, _ in actual_edges
        ),
        "raw_after_coupling": raw, "after_refresh": refreshed,
        "after_support": after_support, "support_reset": asdict(reset),
    }
...
def _refreshed_postbirth_flow(graph):
    ...
    for dt in STEPS:
        ...
        before_snapshot, before_support = _support_readout(graph)
        schedule = build_operator_event_schedule(
            (), start_time=before["time"], flow_durations=(dt,),
        )
        execution = execute_operator_event_schedule(
        ...
        budget = observe_support_transport_euler(
            before_snapshot, after_snapshot, dt=dt,
        )
        segments.append({
            "before": before, "before_support": before_support,
            "raw_after_integrator": raw, "after_refresh": after,
            "after_support": after_support, "duration": dt,
...
def prepare_birth_transport_support(case="attached"):
    ...
    return graph, {
        ...
        "whole_word_validation": {
            "initialized_context": context, "both_validators_passed": True,
            "scope": "Static complete-word validation; live gates checked per call",
        },
        "preparation": preparation, "coupling": coupling,
    }
...
def run_birth_transport_case(case):
    ...
    return {
        **preparation_record, "postbirth_flow": flow,
        "measured_endpoint_before_closure": measured_endpoint,
        "closure": {
            "admission": closure_admission, "after": _state(graph),
...
def main():
    ...
    manifest = CoreExperimentManifest(
        ...
        telemetry=(
            "physical acceleration", "candidate sample and actual UM edge",
            "transport pressure channels", "support and Euler energy budgets",
        ),
