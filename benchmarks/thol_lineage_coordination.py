...
from benchmarks.thol_distributed_target import (  # noqa: E402
    LINEAGE_BRANCHES, run_distributed_target_branch,
)
...
COMMON_FIELDS = (
    "prefix", "original_reference", "original_reference_sha256",
    "original_reference_frozen_time", "original_reference_frozen_before_baseline_flow",
    "initial_target", "baseline_flow", "baseline_steps", "baseline_capture_checks",
    "baseline_target", "common_source", "before_optional_event",
)
...
def load_control_evidence(path=CONTROL_PATH, *, expected_sha256=CONTROL_SHA256):
    ...
    if manifest.claim_id != "O1.b-distributed-fixed-relative-target-response":
        raise ValueError("controls must retain the original distributed-target producer")
    branches = tuple(report["branches"])
    if tuple(row["branch"] for row in branches) != ("no_event", "all_node_um"):
        raise ValueError("controls must preserve the complete ordered no-event/all-node branches")
    ...
    if not report.get("common_causal_baseline_reproduced"):
        raise ValueError("retained controls do not establish their common causal baseline")
    return branches, {
        "path": str(path), "sha256": actual, "byte_count": len(raw),
    ...
    for control in controls:
        for field in COMMON_FIELDS:
            if compared[field] != control[field]:
                raise ValueError(f"fresh branch differs from retained control field {field}")
    return {"all_fields_equal": True, "fields": COMMON_FIELDS,
...
def _proposal_and_effects(branch, all_node_control):
    ...
    parents, children = map(set, (branch["lineage"]["parents"], branch["lineage"]["children"]))
    proposal = event["kernel_proposal"]
    old_rows = {row["node"]: row for row in
                all_node_control["event"]["coupling"]["kernel_proposal"]["target_proposals"]}
    rows = proposal["target_proposals"]
    if any(_json_payload(row) != old_rows[row["node"]] for row in rows):
        raise ValueError("selected target-local proposals differ from the all-node control")
    recipients = {node: [] for node in before["nodes"]}
    for row in rows:
        for phase in row["phase_proposals"]:
            recipients[phase["node"]].append(phase["source"])
    phase_changes = tuple({
        "node": node, "before": left, "after": right,
        "exact_represented_difference": Fraction.from_float(right) - Fraction.from_float(left),
        "circular_difference": angle_diff(right, left),
    } for node, left, right in zip(before["nodes"], before["phase"], after["phase"], strict=True))
    capacity_changes = tuple({
        "node": node, "before": left, "after": right,
        "exact_represented_difference": Fraction.from_float(right) - Fraction.from_float(left),
    } for node, left, right in zip(before["nodes"], before["capacity"], after["capacity"], strict=True))
    ...
    return {
        ...
        "untargeted_phase_write_nodes": tuple(update["node"] for update in proposal["node_updates"]
                                              if update["theta_after"] is not None
                                              and update["node"] not in proposal["targets"]),
        "phase_changes": phase_changes, "capacity_changes": capacity_changes,
        "new_edge_classes": tuple({"left": left, "right": right, "weight": data["weight"],
...
def _outcome(branch):
    ...
    return {
        "branch": branch["branch"], "status": "executed",
        ...
        "current_profile_mismatch_variance": Fraction(endpoint["limiting_pattern"]["error_variance"]),
        "current_model_mean_drift": Fraction(endpoint["reference"]["mean_drift"]),
        "original_metric_mean_change": Fraction(branch["comparison"]["original_metric_mean_change"]),
    }


def run_study(control_path=CONTROL_PATH, *, expected_control_sha256=CONTROL_SHA256):
    controls, binding = load_control_evidence(control_path, expected_sha256=expected_control_sha256)
    branches = []
    for name in LINEAGE_BRANCHES:
        branch = run_distributed_target_branch(name)
        comparison = compare_common_source(branch, controls)
        branches.append({**branch, "retained_common_source_comparison": comparison,
    ...
    return {
        "branches": tuple(branches), "retained_controls": binding,
...
def main():
    ...
    manifest = CoreExperimentManifest(
        claim_id="O1.b-lineage-scoped-UM-fixed-target-response", git_sha=sha,
        ...
        telemetry=("complete actual lineage", "live admission/refusal", "overlapping UM proposals",
    ...
    report = {"manifest": manifest.to_dict(), "source_scope": scope,
              **run_study(args.controls, expected_control_sha256=args.expected_control_sha256),
