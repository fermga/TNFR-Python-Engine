...
from benchmarks.thol_birth_transport import (  # noqa: E402
    CASES, STEPS, _support_readout, prepare_birth_selection_source,
)
...
from tnfr.physics.support_transport import (  # noqa: E402
    _from_data, observe_support_transport_euler, observe_support_transport_reset,
)
...
def _require_stage(stage, operator, targets):
    if (stage.schedule != TWO_PHASE_JACOBI or stage.glyph != operator.glyph.value
            or stage.operator != operator.name or stage.nodes_processed != len(targets)):
...
def _weighted_source_budget(coupling, parents, children):
    ...
    if nodes != parents + children:
        raise ValueError("capacity-source witness requires the complete parent/child order")
    ...
    if (not children or len(child_capacities) != 1
            or any(capacities[node] != 1 for node in parents)):
        raise ValueError("capacity-source witness requires unit parents and uniform children")
    child_capacity = next(iter(child_capacities))
    if not 0 < child_capacity < 1:
        raise ValueError("capacity-source witness requires positive subunit child capacity")
    ...
    for node in nodes:
        ...
        if len(old) != 2 or any(weights[node, neighbor] != 1 for neighbor in old):
            raise ValueError("every parent must retain two unit-conductance parent neighbors")
        incidence.append({"parent": node, "child_count": len(new),
                          "child_strength": sum((weights[node, n] for n in new), Fraction(0))})
    strengths = tuple(sum((w for (source, _), w in weights.items() if source == node),
                          Fraction(0)) for node in nodes)
    metric = tuple(d / nu for d, nu in zip(strengths, snapshot["capacity"], strict=True))
    mass = sum(metric, Fraction(0))

    def weighted(values):
        return sum((d * value for d, value in zip(strengths, values, strict=True)), Fraction(0))

    components = dict(captured["components"])
    epi = tuple(observation["epi_weight"] * value for value in snapshot["epi_gradient"])
    sources = {"epi": weighted(epi), **{name: weighted(values)
                                        for name, values in components.items()}}
    w_vf = dict(observation["normalized_weights"])["vf"]
    closed = 2 * w_vf * (1 - child_capacity) * sum((
    ...
    residual = represented_rate - sum(sources.values(), Fraction(0)) / mass - (
    ...
    return {
        "available": True, "strengths": strengths, "metric_weights": metric,
        "parent_child_incidence": tuple(incidence), "child_capacity": child_capacity,
        "capacity_weighted_source": sources["vf"], "capacity_closed_form_source": closed,
...
def _support_from_flow_state(reference, state):
    ...
    for i, j, weight in reference.conductance:
        dense[i][j] = weight
    if (state.nodes != reference.nodes
            or state.conductance != tuple(tuple(row) for row in dense)
            or state.exact_nu_f != reference.capacity):
        raise ValueError("flow state must retain the captured node order, support and capacity")
    for raw, exact in ((state.epi, state.exact_epi),
    ...
    return _from_data(
        reference.nodes, reference.conductance, reference.support_neighbors,
        state.exact_epi, state.exact_nu_f, state.exact_delta_nfr,
    )
...
def _couple_parents(graph, parents, *, case, target_policy=None):
    ...
    for update in proposal.node_updates:
        ...
        for key, expected in (("phase", update.theta_after),
                              ("capacity", update.vf_after),
                              ("pressure", update.dnfr_after)):
            if expected is not None and raw[key][index] != expected:
                raise RuntimeError("actual shared UM update differs from its kernel proposal")
    old_edges = {frozenset((u, v)) for u, v, _ in before["edges"]}
    new_edges = tuple((u, v, deepcopy(data)) for u, v, data in graph.edges(data=True)
                      if frozenset((u, v)) not in old_edges)
    proposed_edges = {frozenset((edge.left, edge.right)): edge.weight
                      for edge in proposal.edges}
    actual_edges = {frozenset((u, v)): data["weight"] for u, v, data in new_edges}
    if actual_edges != proposed_edges:
        raise RuntimeError("actual shared UM new edges differ from the complete proposal")
    ...
    return after_snapshot, {
        ...
        "target_policy": (target_policy if target_policy is not None else
                          "All actual birth parents once; newborns are not UM targets"),
        "admissions": tuple({"node": parent, "candidate": admission.candidate,
                             "allowed": admission.allowed}
                            for parent, admission in zip(parents, checked, strict=True)),
        "stage_result": asdict(stage), "kernel_proposal": asdict(proposal),
        ...
        "node_sample_before": sample_before, "node_sample_used": sample_used,
        "sampling_refresh_executed": refresh_sample,
        "before": before, "after_raw": raw, "after_refreshed": refreshed,
        "before_support": before_support, "raw_support": raw_support,
        "after_support": after_support, "new_edges": new_edges,
        "raw_forcing": raw_forcing, "refreshed_forcing": refreshed_forcing,
        "support_inventory": _support_inventory(after_snapshot),
        "new_edge_phase_separations": tuple(abs(angle_diff(phases[u], phases[v]))
...
def _physical_flow(graph, reference):
    ...
    schedule = build_operator_event_schedule(
        (), start_time=before["time"], flow_durations=(sum(STEPS),),
    )
    ...
    for boundary in evidence.boundary_observations:
        ...
        boundaries.append({
            "index": boundary.boundary_index, "time": boundary.time,
            "exact_time": boundary.exact_time, "offset": boundary.offset,
            ...
            "before": asdict(boundary.before), "after": asdict(boundary.after),
            "before_support": asdict(left), "after_support": asdict(right),
            "refresh_reset_budget": asdict(observe_support_transport_reset(left, right)),
        })
    segments = []
    for index, flow in enumerate(evidence.segment_flow_evidence):
        ...
        segments.append({
            "index": index, "interval": asdict(flow.interval),
            "before": asdict(certificate.left), "held_after": asdict(certificate.right),
            "before_support": asdict(left), "after_support": asdict(right),
            "exact_euler_budget": asdict(budget), "method": flow.resolved_method,
    ...
    if actual_after != _support_from_flow_state(reference, evidence.boundary_observations[-1].after):
        raise RuntimeError("live final read-out differs from the sealed terminal boundary")
    energy_change = actual_after.dirichlet_energy - reference.dirichlet_energy
    accounted = sum((row["exact_euler_budget"]["energy_change"] for row in segments), Fraction(0))
    if energy_change != accounted:
        raise RuntimeError("finite support-energy telescope failed")
    return {
        "before": before, "after": after, "after_support": actual_support,
        "after_forcing": _forcing_readout(graph),
        "partition": asdict(partition), "boundaries": tuple(boundaries),
        ...
        "exact_energy_change": energy_change, "exact_segment_energy_change_sum": accounted,
        "energy_telescope_residual": energy_change - accounted,
        "solver_accuracy_certified": False, "mesh_convergence_certified": False,
...
def prepare_distributed_transport_support(case="attached"):
    ...
    if dispatch.stage_result is None or parents != tuple(before_birth["nodes"]):
        raise RuntimeError("the unchanged all-node preparation did not birth at every original node")
    after_birth = deepcopy(_state(graph))
    if (any(graph.degree(child) != 0 for child in children)
            or after_birth["epi"][:len(parents)] != before_birth["epi"]
            or tuple(after_birth["nodes"]) != parents + children):
        raise RuntimeError("birth did not preserve old EPI and append isolated children")
    birth = {
        "before": before_birth, "after": after_birth,
        "eligibility": _eligibility_record(dispatch.eligibility),
        "stage_result": asdict(dispatch.stage_result), "parent_children": births,
        "policy": dispatch.policy, "old_epi_preserved": True,
        "children_initially_isolated": True,
        "children": tuple({"parent": parent, "child": child, "degree": graph.degree(child),
    ...
    return graph, {
        "case": case, "preparation": preparation, "birth": birth,
        "coupling": coupling, "weighted_source_budget": source_budget,
    }
...
def run_distributed_transport_case(case):
    ...
    for parent, child in births:
        ...
        for node in (parent, child):
            ...
            rows.append({
                "node": node, "epi_after_birth": initial, "epi_endpoint": final,
                ...
                "refreshed_pressure_after_um": coupling["after_refreshed"]["pressure"][index],
                "endpoint_pressure": endpoint["pressure"][index],
            })
        responses.append({"parent": parent, "child": child,
                          "parent_response": rows[0], "child_response": rows[1]})
    return {
        **prefix, "physical_flow": flow,
        "endpoint": endpoint, "responses": tuple(responses),
        "scope": (
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=(
        ROOT / "artifacts/research/thol_distributed_transport.json"))
    args = parser.parse_args()
    scope = (
        "src/tnfr", "benchmarks/thol_distributed_transport.py",
    ...
    manifest = CoreExperimentManifest(
        ...
        telemetry=("complete eligibility and birth inventory", "UM proposal and actual support",
                   "raw and refreshed pressure", "native flow and exact support-energy budgets"),
