...
from benchmarks.thol_lineage_coordination import (  # noqa: E402
    COMMON_FIELDS, CONTROL_PATH, CONTROL_SHA256, compare_common_source,
...
from tnfr.physics.forcing_realization import (  # noqa: E402
    NonEpiForcingObservation, decompose_non_epi_forcing,
)
from tnfr.physics.structural_morphism import (  # noqa: E402
    _finite_difference, _finite_norm, _finite_product,
    _intertwining_diagnostics, _relative_tolerance,
)
from tnfr.physics.support_transport import _from_data  # noqa: E402
...
def _vector(values):
    if not isinstance(values, (list, tuple)):
        raise ValueError("serialized exact vector must be an ordered array")
    if any(type(value) not in (int, str) for value in values):
        raise ValueError("serialized exact coordinates must be integer/rational text")
    return tuple(Fraction(value) for value in values)


def _digest(value):
    return hashlib.sha256(json.dumps(_json(value), sort_keys=True,
...
def _snapshot(raw):
    """Rebuild every cached coordinate rather than trusting a hydrated record."""
    result = _from_data(
        tuple(raw["nodes"]),
        tuple((i, j, Fraction(weight)) for i, j, weight in raw["conductance"]),
        tuple(tuple(row) for row in raw["support_neighbors"]),
        _vector(raw["epi"]), _vector(raw["capacity"]), _vector(raw["stored_pressure"]),
    )
    _equal(asdict(result), raw, "support snapshot")
    return result


def _reference(raw):
    """Re-derive the profile and compare all saved reference fields."""
    result = derive_forced_support_balance(
        _snapshot(raw["source"]), epi_weight=Fraction(raw["epi_weight"]),
        forcing=_vector(raw["forcing"]),
    )
    _equal(asdict(result), raw, "forced reference")
    return result


def _capture(raw):
    """Check supplied forcing decomposition without rerunning a phase kernel."""
    row = raw["observation"]
    snapshot = _snapshot(row["snapshot"])
    vectors = ("phase", "forcing", "phase_gradient", "full_kernel_pressure",
               "kernel_pressure_defect", "stored_pressure_residual")
    values = {key: _vector(row[key]) for key in vectors}
    ...
    observation = NonEpiForcingObservation(
        snapshot=snapshot, epi_weight=Fraction(row["epi_weight"]),
        normalized_weights=tuple((name, Fraction(value))
                                 for name, value in row["normalized_weights"]), **values,
    )
    _equal(asdict(observation), row, "complete forcing capture")
    components = decompose_non_epi_forcing(observation)
    _equal(components, raw["components"], "forcing components")
    model = tuple(observation.epi_weight * gradient + force for gradient, force
                  in zip(snapshot.epi_gradient, observation.forcing, strict=True))
    _equal(tuple(p - m for p, m in zip(observation.full_kernel_pressure, model)),
           row["kernel_pressure_defect"], "kernel pressure defect")
    stored_residual = tuple(p - fresh for p, fresh in zip(
        snapshot.stored_pressure, observation.full_kernel_pressure))
    _equal(stored_residual,
           row["stored_pressure_residual"], "stored pressure residual")
...
def _admit_target(raw, original, current, capture):
    ...
    if (observation.epi_weight != current.epi_weight
    ...
    value = observe_forced_support_target(
        original, current, snapshot, forcing_components=components,
    )
...
def _match_live_state(state, capture, snapshot):
    """Bind the detached coefficients to the producer's retained live readout."""
    _equal(snapshot.nodes, state["nodes"], "live snapshot node order")
    for field, key in (("epi", "epi"), ("capacity", "capacity"),
                       ("stored_pressure", "pressure")):
        values = state[key]
        if any(type(value) not in (int, float) for value in values):
            raise ValueError("live numeric state must retain real JSON numbers")
        _equal(getattr(snapshot, field), tuple(Fraction(value) for value in values),
               f"live snapshot {field}")
    _equal(_vector(capture["observation"]["phase"]),
           tuple(Fraction(value) for value in state["phase"]), "live captured phase")
...
def _lineage(branch):
    ...
    if (len(set(parents + children)) != len(nodes) or parents + children != nodes
    ...
    if tuple(node for node, _ in attributes) != nodes:
        raise ValueError("retained node attributes must preserve the complete node order")
    attrs = dict(attributes)
    _equal(branch["common_source"]["state"], branch["before_optional_event"],
           "common live source state")
    for state in (birth["after"], branch["before_optional_event"],
    ...
    for parent, child in pairs:
        if (attrs[parent].get("sub_nodes") != [child]
    ...
    for key, expected in (("parent_children", pairs), ("parents", parents),
                          ("children", children), ("current_nodes", nodes)):
        _equal(retained[key], expected, f"lineage {key}")
    if (retained["live_parentage_verified"] is not True
    ...
    return {"nodes": nodes, "parent_children": pairs, "blocks": pairs,
            "family_count": len(pairs), "scope": (
...
def load_evidence(control_path=CONTROL_PATH, lineage_path=LINEAGE_PATH, *,
    ...
    for key in ("sha256", "byte_count", "historical_manifest", "historical_source_scope"):
    ...
    return controls, branches, partitions[0], {
        "retained_controls": control_binding,
        "retained_lineage": {"path": str(lineage_path), "sha256": actual,
                             "byte_count": len(raw), "historical_manifest": report["manifest"],
...
def _numerical_crosscheck(reference, blocks, exact):
    ...
    def diagnostic(mapping, source, target):
        ...
        return {"residual": residual, "residual_scale": scale,
                "relative_residual": residual / scale, "relative_tolerance": tolerance,
    ...
    return {
        ...
        "exact_cast_discrepancies": {
            name: norm(_finite_difference(value, np.array(getattr(exact, field), dtype=float),
                                          name), name)
            for name, value, field in (("R", r, "projection"), ("A", a, "micro_generator"),
                                       ("RAQ", raq, "hidden_to_macro"),
                                       ("QAP", qap, "macro_to_hidden"),
                                       ("K0", k0, "instantaneous_kernel"))
        },
        "projection_intertwining": projection, "lift_intertwining": lift,
        "closure_within_relative_tolerance": bool(
            projection["within_relative_tolerance"] and lift["within_relative_tolerance"]),
        "scope": (
...
def _observer_reset(original, current, before, after, epi, before_state, after_state):
    ...
    reweight = tuple(dot(tuple(b - a for a, b in zip(left, right)), epi)
    ...
    profile_before = tuple(dot(row, original.relative_profile) for row in before.projection)
    profile_after = tuple(dot(row, current.relative_profile) for row in after.projection)
    centered_before = tuple(dot(row, before_state.relative_error) for row in before.projection)
    centered_after = tuple(dot(row, after_state.relative_error) for row in after.projection)
    mean_change = after_state.mean - before_state.mean
    centered_change = tuple(b - a for a, b in zip(centered_before, centered_after))
    decomposed = tuple(raw - mean_change - (zb - za)
    ...
    return {
        "epi_unchanged": True, "projected_epi_before": projected_before,
        "projected_epi_after": projected_after, "projected_epi_change": change,
        "projection_reweighting": reweight,
        "current_mean_before": before_state.mean, "current_mean_after": after_state.mean,
        ...
        "projected_profile_shift": tuple(b - a for a, b in zip(profile_before, profile_after)),
...
def run_study(control_path=CONTROL_PATH, lineage_path=LINEAGE_PATH, *,
              expected_control_sha256=CONTROL_SHA256,
              expected_lineage_sha256=LINEAGE_SHA256):
    ...
    initial, _ = _admit_target(base["initial_target"], original, original,
    ...
    snapshot, target = _admit_target(base["baseline_target"], original, original,
    ...
    for name, branch in zip(MODEL_ORDER, (base, *lineage, controls[1]), strict=True):
        if name == "original":
            ...
            _equal(branch["original_reference"], base["original_reference"], "original reference")
            _equal(branch["after_optional_event"]["epi"], actual_state["epi"], "unchanged event EPI")
            _equal(branch["after_optional_event"]["nodes"], actual_state["nodes"], "event node order")
            ...
            current, readout = _admit_target(
                branch["post_event_target"], original, reference,
            ...
            _match_live_state(branch["after_optional_event"],
                              branch["event"]["coupling"]["refreshed_forcing"], current)
        ...
        rows.append({
            "model": name, "observation_time": 1.0,
            "reference_sha256": _digest(asdict(reference)),
            "actual_snapshot": asdict(current), "exact_observation": asdict(exact),
            "numerical_crosscheck": _numerical_crosscheck(reference, blocks, exact),
            "current_state": asdict(state),
            "old_target_readout": {"pattern": asdict(readout.pattern),
            ...
            "observer_reset": _observer_reset(original, reference, first, exact,
                                              current.epi, before_state, state),
        })
    for path, key in ((control_path, "retained_controls"), (lineage_path, "retained_lineage")):
    ...
    return {
        **bindings, "partition": partition, "models": tuple(rows),
        "common_fields_checked": COMMON_FIELDS, "new_trajectories_executed": False,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=(
        ROOT / "artifacts/research/thol_family_closure.json"))
    ...
    scope = ("src/tnfr", "benchmarks/thol_family_closure.py",
             "benchmarks/thol_lineage_coordination.py", "benchmarks/thol_distributed_target.py",
             "benchmarks/thol_distributed_transport.py", "benchmarks/thol_birth_transport.py",
             "benchmarks/thol_pressure_feedback.py", "benchmarks/thol_eligibility_dispatch.py",
             "benchmarks/capacity_localization.py", "benchmarks/structural_target_compatibility.py",
             "benchmarks/thol_preparation_policy.py", "benchmarks/selection_birth_closure.py")
    provenance = current_git_source_provenance(ROOT, scope)
    sha, dirty, digest = provenance
    manifest = CoreExperimentManifest(
        claim_id="O1.b-generated-family-affine-state-sufficiency", git_sha=sha,
        ...
        timestep=None, seed=None, result_status=ClaimStatus.MEASURED,
        operator_sequence=(), telemetry=("exact RAQ/QAP", "hidden-state witness", "K(0)",
                                         "observer reweighting", "binary64 diagnostic"),
        controls=("complete retained fixed-target and lineage evidence",),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    result = run_study(args.controls, args.lineage,
