...
from benchmarks.thol_regional_balance_audit import _ancestry, _bind_source_edges  # noqa: E402
...
def _ratio(error, variance):
    ...
    return {"available": variance > 0, "value": error/variance if variance > 0 else None,
            "zero_case": None if variance > 0 else (
                "zero_control_and_zero_error" if error == 0 else "zero_control_with_positive_error")}
...
def _endpoints(retained, ordinal):
    ...
    if (branch["branch"] != reset.window.BRANCHES[ordinal] or first["branch"] != branch["branch"]
            or first["status"] != "executed" or branch["status"] != "completed"
            or branch["completed_steps"] != 5 or len(branch["steps"]) != 5):
    ...
    _equal(records[-1], reset.window.expand_record(branch["final_record"], pool), "final window endpoint")
    if tuple(F(row["state"]["time"]) for row in records) != TIMES:
        raise ValueError("the six fixed physical endpoints are required")
    return tuple(records), tuple(_record_ref(path, record) for path, record in zip(paths, records, strict=True))
...
def audit_window(retained, original, native_report):
    ...
    if len(retained["branches"]) != 2 or len(native_report["replayed_prior_reports"]) != 2:
        raise ValueError("exactly the original paired branches are required")
    source = _rebuild(original.source)
    nodes = source.nodes
    strengths = tuple(sum((w for left, _right, w in source.conductance if left == i), F(0))
    ...
    for ordinal in range(2):
        ...
        for record in records:
            state = record["state"]
            if tuple(state["nodes"]) != nodes or reset._v(state["capacity"]) != source.capacity:
                raise ValueError("regional endpoints require unchanged ordered nodes and capacity")
            ...
            if pairs is not None and current_pairs != pairs:
                raise ValueError("regional ancestry changed between endpoints or branches")
            pairs = current_pairs
        branches.append(records)
        references.append(refs)
    regions = tuple([(f"ancestry_pair_{i}", pair) for i, pair in enumerate(pairs)]
    ...
    for label, region in regions:
        ...
        for time, control, perturbed in zip(TIMES, *branches, strict=True):
            ...
            rows.append({"time": time, "control_epi": x, "perturbed_epi": y, "paired": paired,
                         "control_mean": shape["weighted_mean_offset"], "control_variance": variance,
                         "control_centered_epi": shape["centered_epi_difference"],
                         "control_centered_drift_from_start": drift["centered_epi_difference"],
        ...
        ratios_available = first["relative_error"]["available"] and last["relative_error"]["available"]
        result.append({"label": label, "nodes": region, "full_node_indices": indices, "metric_weights": weights,
                       "endpoints": rows,
                       "endpoint_summary": {"centered_error_change": e1-e0,
                                            ...
                                            "control_variance_positive_at_every_endpoint": all(row["control_variance"] > 0 for row in rows),
                                            "control_centered_form_unchanged_at_every_endpoint": all(
                                                row["control_centered_drift_energy"] == 0 for row in rows),
                                            "relative_error_change": last["relative_error"]["value"]-first["relative_error"]["value"]
                                            if ratios_available else None,
                                            "relative_error_decreased": last["relative_error"]["value"] < first["relative_error"]["value"]
                                            if ratios_available else None}})
    return {"protocol": PROTOCOL, "nodes": nodes, "full_metric_weights": metric,
            "actual_lineage": {"pairs": pairs, "children": children}, "retained_endpoints": references,
            "regions": result, "regional_endpoint_count": 54,
            "native_calls": 0, "coordination_calls": 0, "forcing_capture_calls": 0,
...
def run_study(window_path=reset.WINDOW_PATH, native_path=reset.window.NATIVE_PATH, *,
              expected_window_sha256=reset.WINDOW_SHA256, expected_native_sha256=reset.window.NATIVE_SHA256):
    ...
    native, _ = reset._load(native_path, expected_native_sha256, "O1.b-generated-native-runtime-response")
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/thol_regional_recovery_audit_2026_09_18.json")
    ...
    scope = ("src/tnfr", "benchmarks/thol_regional_recovery_audit.py", "benchmarks/thol_retained_reset_audit.py",
             "benchmarks/thol_native_policy_window.py", "benchmarks/thol_native_runtime_response.py",
             "benchmarks/thol_full_state_response.py", "benchmarks/thol_regional_balance_audit.py",
             "benchmarks/thol_regional_identity_audit.py", "benchmarks/thol_family_closure.py",
             "benchmarks/thol_pressure_feedback.py")
    provenance = current_git_source_provenance(ROOT, scope)
    result = run_study(args.window_input, args.native_input, expected_window_sha256=args.expected_window_sha256,
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-retained-regional-paired-response", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        solver="Exact detached regional comparisons; no solver or kernel executed", timestep=None, seed=None,
        result_status=ClaimStatus.DERIVED, operator_sequence=(),
        telemetry=("regional paired error", "control contrast and centered drift", "separate mean offset"),
        controls=("six predeclared times", "all nine lineage regions", "zero denominator explicit", "no new trajectories"),
