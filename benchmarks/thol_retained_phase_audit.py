...
from tnfr.constants.aliases import ALIAS_EPI, ALIAS_VF, ALIAS_THETA, ALIAS_DNFR  # noqa: E402
...
def resultant_enclosure(phases):
    ...
    real, imag = (tuple(sum(row[channel][j] for row in values)/len(values) for j in (0, 1))
                  for channel in (0, 1))

    def square(interval):
        low, high = interval
        return (Fraction(0) if low <= 0 <= high else min(low*low, high*high), max(low*low, high*high))

    r2, i2 = square(real), square(imag)
    squared = tuple(a+b for a, b in zip(r2, i2, strict=True))
    c, s = (float(sum(v)/2) for v in (real, imag))
    return {"real": real, "imag": imag, "squared_norm": squared,
            "nonzero_certified": squared[0] > 0,
            "midpoint_display": {"real": c, "imag": s, "norm": math.hypot(c, s), "angle": math.atan2(s, c)},
            "scope": "Exact represented-input enclosure; midpoint angle is not a certified angle interval"}


def _float_literal(value):
    if not isinstance(value, (tuple, list)) or len(value) != 2 or value[0] != "binary64":
...
def load_evidence(path=INPUT, *, expected_sha256=EXPECTED_SHA256):
    ...
    if tuple(branch["branch"] for branch in report["branches"]) != ("control", "child_emission"):
        raise ValueError("wrong retained branch order")
    return report, {"path": str(path), "sha256": expected_sha256, "historical_manifest": report["manifest"]}
...
def _rotation(nodes, parent_children):
    ...
    if parents != tuple(range(8)) or len(set(children)) != 8 or set(parents) & set(children):
...
def _detached_graph(record, mapping, *, node_order=None):
    ...
    graph = nx.Graph()
    order = tuple(mapping[n] for n in nodes) if node_order is None else tuple(node_order)
    ...
    for i, node in enumerate(nodes):
        data = {alias[0]: state[key][i] for key, alias in (
            ("epi", ALIAS_EPI), ("capacity", ALIAS_VF), ("phase", ALIAS_THETA), ("pressure", ALIAS_DNFR))}
        if any(type(v) not in (int, float) or not math.isfinite(v) for v in data.values()):
            raise ValueError("finite scalar channels required")
        graph.add_node(mapping[node], **data)
    graph.add_edges_from((mapping[u], mapping[v], deepcopy(data)) for u, v, data in state["edges"])
...
def _projection(graph):
    return {"nodes": tuple(graph), "edges": tuple((u, v, dict(d)) for u, v, d in graph.edges(data=True)),
...
"""
    ...
    return {"node_aligned_shifts": shifts, "anchor_common_shift": shifts[0],
            "relative_pattern_residual": relative, "max_relative_residual": max(map(abs, relative)),
...
def _evaluate(record, gains, mapping, *, scalar=False, canonical_order=False):
    graph = _detached_graph(record, mapping, node_order=record["state"]["nodes"] if canonical_order else None)
    ...
    phases = tuple(get_theta_attr(graph.nodes[mapping[n]]) for n in record["state"]["nodes"])
    tetrad = _tetrad(graph)
    aligned = {key: {n: value[mapping[n]] for n in record["state"]["nodes"]} if isinstance(value, dict) else value
               for key, value in tetrad["fields"].items()}
    return {"initial": initial, "phase": phases, "tetrad": tetrad, "aligned_tetrad_fields": aligned,
            "floating_global_resultant": {"real": c, "imag": s, "norm": math.hypot(c, s), "angle": direction},
...
def _compare_fields(base, other):
    ...
    for key, left in base["aligned_tetrad_fields"].items():
        ...
        pairs = [(left[n], right[n]) for n in left] if isinstance(left, dict) else [(left, right)]
        differences = tuple(Fraction(_float_literal(b))-Fraction(_float_literal(a)) for a, b in pairs)
        result[key] = {"represented_equal": left == right,
...
def audit_boundary(row, parent_children):
    ...
    saved = deepcopy(row)
    if row["boundary"] != "coordinate_global_local_phase" or row["outcome"] != "completed":
    ...
    gains = tuple(_float_literal(after["graph_attributes"][key]) for key in ("PHASE_K_GLOBAL", "PHASE_K_LOCAL"))
    ...
    if base["phase"] != tuple(after["state"]["phase"]):
        raise ValueError("held NumPy replay differs from the actual archived coordination output")
    inverse = {v: k for k, v in mapping.items()}
    transported = _transport_state(moved["initial"], inverse) == _transport_state(base["initial"], identity)
    if not transported or moved["phase"] != base["phase"]:
        raise ValueError("transported-label control changed the aligned phase calculation")
    for key in ("phase", "ordered_neighbors"):
        ...
        if by_node(moved["initial"]) != by_node(reordered["initial"]):
            raise ValueError("canonical enumeration changed the named mathematical phase input")
    if row != saved:
        raise RuntimeError("retained input mutated during detached audit")
    comparison = compare_phases(base["phase"], scalar["phase"])
    return {"source_boundary": row, "effective_gains": gains, "rotation": ROTATION,
            "node_mapping": tuple(mapping.items()), "transported_source_equal": transported,
            "numpy_replay_matches_archive": True, "base_numpy": base, "transported_numpy": moved,
            ...
            "scalar_arithmetic": scalar, "scalar_vs_numpy": comparison,
            "tetrad_scalar_equals_numpy": {key: scalar["aligned_tetrad_fields"][key] == base["aligned_tetrad_fields"][key]
                                           for key in ("phi_s", "grad_phi", "curv_phi", "xi_c")},
            "tetrad_canonical_order_equals_numpy": {key: reordered["aligned_tetrad_fields"][key] == base["aligned_tetrad_fields"][key]
                                                    for key in ("phi_s", "grad_phi", "curv_phi", "xi_c")},
            ...
            "ideal_resultant": {"exact_zero": True,
            ...
            "live_graph_writes": 0, "native_steps": 0,
...
def run_study(path=INPUT, *, expected_sha256=EXPECTED_SHA256):
    ...
    for branch in report["branches"]:
        candidates = [row for row in branch["native_trace"]["boundaries"] if row["boundary"] == "coordinate_global_local_phase"]
        if branch["status"] != "executed" or len(candidates) != 1:
            raise ValueError("one completed coordination boundary per retained branch required")
    ...
    return {"input_evidence": binding, "selected_branch": "control", "both_input_phase_vectors_equal": True,
            "audit": result, "experimental_status": "No empirical correspondence tested"}
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/thol_retained_phase_audit_2026_09_18.json")
    args = parser.parse_args()
    if args.output.resolve() == args.input.resolve():
        raise ValueError("output must not overwrite retained input")
    scope = ("src/tnfr", "benchmarks/thol_retained_phase_audit.py", "benchmarks/thol_native_runtime_response.py",
             "benchmarks/thol_preparation_policy.py", "benchmarks/thol_pressure_feedback.py")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-retained-phase-conditioning", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        solver="Exact rational Taylor enclosure and detached held-gain coordination", timestep=None, seed=None,
        result_status=ClaimStatus.DERIVED, operator_sequence=(), telemetry=("relative phase pattern", "tetrad", "exact resultant enclosure"),
        controls=("transported node and neighbor order", "scalar/numpy arithmetic", "no trajectory extension"), artifacts=(str(args.output),))
    manifest.validate_for_admission()
    result = {"manifest": manifest.to_dict(), "source_scope": scope, **result}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    safe_write(args.output, lambda stream: stream.write(json.dumps(_payload(result), indent=2, allow_nan=False)+"\n"))
