...
from benchmarks.c6_winding_defect_budget import (  # noqa: E402
    _number, _vector, analyze_c6_defect_report,
)
...
from tnfr.physics.binary64_nodal_flow import observe_binary64_unit_quarter_flow  # noqa: E402
...
def _numpy_replay(epi, pressure):
    if np is None:
        raise ValueError("the retained NumPy backend must be available for primitive comparison")
...
def _analyze_case(retained, audited):
    ...
    for cycle, defect in zip(retained["cycles"], audited["cycles"], strict=True):
        ...
        observation = observe_binary64_unit_quarter_flow(
            epi=epi, pressure=pressure, epi_lower=lower, epi_upper=upper,
        )
        numpy_states = _numpy_replay(epi, pressure)
        bindings = {
            "scalar_endpoint_matches_capture": observation.endpoint == endpoint,
            "exact_endpoint_matches_capture": observation.exact_endpoint == tuple(map(Fraction, endpoint)),
            "numpy_endpoint_matches_capture": numpy_states[-1] == endpoint,
            "all_numpy_substeps_match_scalar": numpy_states == tuple(step.after for step in observation.substeps),
            "integrator_defect_matches_B17": observation.endpoint_defect == defect["pressure_budget"]["integrator_epi_effect"],
        }
        if not all(bindings.values()):
            raise ValueError("retained endpoint or B17 defect differs from shared Euler replay")
        mean_pressure_effect = sum(map(Fraction, pressure), Fraction(0)) / 24
        mean_actual_change = sum((Fraction(a) - Fraction(b) for a, b in zip(endpoint, epi, strict=True)), Fraction(0)) / 6
        ...
        if (residual or mean_rounding_effect != observation.mean_endpoint_defect
                or mean_pressure_effect != observation.mean_pressure / 4
                or mean_actual_change != observation.mean_after - observation.mean_before):
            raise RuntimeError("captured mean change lost its pressure and rounding identity")
        is_half = all(value == .5 for value in epi)
        null_stasis = {
            "source_epi_is_uniform_half": is_half,
            "all_pressures_in_half_epi_band": all(observation.half_epi_pressure_band_membership),
            ...
            "phase_stays_fixed_across_operator_cycle": (
                _vector(cycle["before_capture"]["phase"]) == _vector(cycle["post_il_capture"]["phase"])
            ),
            "future_pressure_band_verified": False,
            "full_state_fixed_point_certified": False,
        }
        cycles.append({
            "ordinal": cycle["ordinal"], "observation": asdict(observation),
            "numpy_replay": numpy_states, "endpoint_bindings": bindings,
            "mean_budget": {
                "held_pressure_mean_effect": mean_pressure_effect,
                "integrator_rounding_mean_effect": mean_rounding_effect,
                "actual_mean_change": mean_actual_change, "identity_residual": residual,
    ...
    return {"mode": retained["mode"], "epsilon": _number(retained["epsilon"]), "cycles": cycles}
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_rounding_cells.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-held-flow-rounding-cells", git_sha=sha,
        source_dirty=dirty, dirty_source_hash=digest, versions={
        ...
        telemetry=("exact addition cells", "scalar/NumPy endpoint agreement",
                   "signed pressure and rounding mean contributions", "derived local error bounds"),
        controls=("retained null/k1/k3", "conditional EPI stasis is not a full-state fixed point"),
