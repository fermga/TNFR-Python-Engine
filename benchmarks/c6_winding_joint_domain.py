...
from benchmarks.c6_winding_phase_response import (  # noqa: E402
    AMPLITUDE, _phase_readout, _winding_state, prepare_c6_phase_response,
)
...
from tnfr.operators.preconditions import validate_coupling, validate_silence  # noqa: E402
...
def _admission_band(graph):
    ...
    lower = max(values[name] for name in (
    ...
    values.update(
        positive_epi_lower=lower, epi_upper=values["configured_epi_max"],
        scope=(
...
def _joint_readout(capture, band, domain):
    ...
    return {
        "phase": phase, "phase_pi_represented": scaled,
        "represented_pi_scale": pi_repr,
        "phase_oscillation_pi": diameter,
        "inside_represented_prepared_phase_box": max(map(abs, scaled)) <= Fraction(1, 24),
        "lower_reserve": lower_reserve, "upper_reserve": upper_reserve,
        "within_represented_reserve_band": (
            lower_reserve >= band["positive_epi_lower"] and upper_reserve <= band["epi_upper"]
        ),
        "epi_min": min(epi), "epi_max": max(epi),
        ...
        "strict_il_capacity_margins": tuple(value - band["il_min_capacity"] for value in capacity),
        "um_capacity_margins": tuple(value - band["um_min_capacity"] for value in capacity),
...
def _repeat_refusal_control(graph):
    ...
    return {
        "word": word, "live_candidates_after_one_um": _plain(candidates),
...
def _conditional_transition(reference, before, after, following, flow, band):
    left, right = _joint_readout(before, band, reference), _joint_readout(after, band, reference)
    result = {
    ...
    try:
        observation = observe_c6_winding_joint_domain(
            reference, phase_before_pi=left["phase_pi_represented"],
            phase_after_pi=right["phase_pi_represented"], epi=before.snapshot.epi,
        )
    ...
    pressure_effect = tuple(a - b for a, b in zip(budget["expected_epi"], ideal, strict=True))
    total = tuple(a - b for a, b in zip(following.snapshot.epi, ideal, strict=True))
    residual = tuple(a - b - c for a, b, c in zip(
        total, pressure_effect, budget["state_defect"], strict=True,
    ))
    if any(residual):
        raise RuntimeError("the conditional-pressure/integrator defect decomposition failed")
    result.update(
        status="observed_conditional_transition", observation=observed,
        conditional_pressure_realization_effect=pressure_effect,
        actual_integrator_endpoint_defect=budget["state_defect"],
        runtime_minus_conditional_epi=total, defect_identity_residual=residual,
    )
    return result
...
def run_c6_joint_case(mode, epsilon):
    if type(epsilon) is not float or (mode, epsilon) not in CASES:
        raise ValueError("mode and epsilon must be one of the three inherited joint controls")
    ...
    domain = derive_c6_winding_joint_domain(
        ...
        capacity=1, epi_weight=weights["epi"], phase_weight=weights["phase"],
        timestep=STEP, epi_lower=band["positive_epi_lower"], epi_upper=band["epi_upper"],
    )
    ops = (Coupling(), Coherence(), Coupling(), Coherence(), Silence())
    word, validation = _word(ops, initialized=min(current.snapshot.epi) > 0)
    if word is None:
        raise RuntimeError(f"the declared two-cycle full word was refused: {validation}")
    record = {
        "mode": mode, "epsilon": Fraction(epsilon), "cycle_count": CYCLE_COUNT,
        "word": validation, "admission_band": band, "joint_domain_reference": asdict(domain),
        "initial": _captured_state(graph), "initial_capture": asdict(current),
        "initial_joint_readout": _joint_readout(current, band, domain),
        "initial_winding": _winding_state(graph), "initial_diagnostics": _diagnostics(graph),
    }
    cycles = []
    for cycle in range(CYCLE_COUNT):
        ...
        current, um = _observed_stage(
            graph, ops[2 * cycle], word.step(2 * cycle), current, phase_readout=_phase_readout,
        )
        um_winding = _winding_state(graph)
        if cycle == 0:
            record["repeated_um_control"] = _repeat_refusal_control(graph)
        current, il = _observed_stage(
            graph, ops[2 * cycle + 1], word.step(2 * cycle + 1), current, phase_readout=_phase_readout,
        )
        post_il = current
        il_winding = _winding_state(graph)
        current, flow = _advance_forced_support_interval(
            graph, original, _reference(post_il), post_il, post_il, duration=STEP,
        )
        cycles.append({
            "ordinal": cycle + 1, "independent_strict_um_readiness": strict_um,
            "before_capture": asdict(before), "um": um, "il": il, "flow": flow,
            "um_winding": um_winding, "il_winding": il_winding,
            "post_il_capture": asdict(post_il), "after_capture": asdict(current),
            ...
            "conditional_transition": _conditional_transition(
                domain, before, post_il, current, flow, band,
            ),
        })
    record.update(
        cycles=cycles, final_before_closure=_captured_state(graph),
        final_capture=asdict(current), final_joint_readout=_joint_readout(current, band, domain),
        final_diagnostics=_diagnostics(graph), final_winding=_winding_state(graph),
    )
    targets = tuple(graph)
    strict_sha = []
    for node in targets:
    ...
    closure = execute_pointwise_stage(
        graph, ops[-1], targets, sequence_context=word.step(4), collect_metrics=True,
    )
    ...
    record["closure_after_measurement"] = {
        "admissions": admissions, "independent_strict_sha_readiness": strict_sha,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_joint_domain.json")
    args = parser.parse_args()
    provenance = current_git_source_provenance(ROOT, SOURCE_SCOPE)
    sha, dirty, digest = provenance
    cases = [run_c6_joint_case(*case) for case in CASES]
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-two-cycle-joint-admission", git_sha=sha,
        ...
        telemetry=("strict UM/IL/SHA and complete-word admission", "joint-domain reserve", "phase and Euler defects", "winding/support"),
        controls=("null", "+k1", "+k3", "read-only direct-UM-repeat validator contrast"),
    ...
    report = {"manifest": manifest.to_dict(), "source_scope": SOURCE_SCOPE, "cases": cases}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(_artifact_payload(report), indent=2, allow_nan=False) + "\n", encoding="utf-8")
