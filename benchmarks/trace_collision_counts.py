...
def main() -> int:
    ...
    for p, f in CASES:
        ...
        for k in (2, 3, 4):
            ...
            if f == 1:
                ...
                ext_collision = True
            print(
                f"  F_{p}^{f}(q={field.q:<2}) {k:>2} {c.subset_size:>4} "
    ...
    print(
        f"  collision count / char formula  : {ClaimStatus.DERIVED.value} "
        "+ MEASURED (Fourier inversion)"
    )
    print(
        f"  type detector                   : {ClaimStatus.CONJECTURAL.value}"
        " / not claimed (NT-P05c)"
    )
