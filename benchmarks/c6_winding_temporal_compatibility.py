...
from tnfr.physics.c6_carried_cell_graph import derive_c6_carried_cell_graph  # noqa: E402
...
def _check_lineage(producers, hashes=None):
    if len(producers) != 8 or hashes is not None and len(hashes) != 8:
        raise ValueError("the temporal audit requires B40 and its seven historical inputs")
    ...
    if len(evidence) != len(ancestors):
        raise ValueError("the retained B40 report must identify seven original producers")
    for index, (entry, producer) in enumerate(zip(evidence, ancestors, strict=True), 1):
        if (_canonical(entry["producer_manifest"]) != _canonical(producer["manifest"])
                or tuple(entry["producer_source_scope"]) != tuple(producer["source_scope"])):
            raise ValueError("the temporal lineage metadata differs from its historical producer")
        if hashes is not None and entry["sha256"] != hashes[index]:
            raise ValueError("the temporal lineage hashes differ from the historical input bytes")
...
def _verified_source(producers):
    ...
    if manifest.claim_id != "O3.a-C6-coupled-pressure-passage" or tuple(parent["source_scope"]) != SOURCE_SCOPE:
    ...
    if any(key not in parent or _canonical(parent[key]) != _canonical(value) for key, value in rebuilt.items()):
        raise ValueError("the retained B40 report differs from its complete shared-owner replay")
    ...
    lattice = derive_c6_pressure_lattice(
        phase=tuple(source["phase"]), epi_weight=_represented_scalar(source["epi_weight"]),
        phase_weight=_represented_scalar(source["phase_weight"]),
        epi_lower=_represented_scalar(source["epi_lower"]), epi_upper=_represented_scalar(source["epi_upper"]),
    )
    state = _state(parent["B40_continuation"]["endpoint"])
    points = tuple(tuple(point["state"]["epi"]) for point in rebuilt["B38_static_pressure_balance"]["points"])
    return derive_c6_carried_profile(lattice), state, points


def _runtime_bridge():
    from benchmarks.c6_winding_phase_tail_runtime import run_c6_phase_tail_runtime_bridge
...
def analyze_c6_winding_temporal_compatibility(*producers, include_runtime=False):
    ...
    graph = derive_c6_carried_cell_graph(profile.lattice, epi_states=points, timestep=STEP)
    ...
    obstruction_record.update(
        ...
        correlated_carry_region_excluded=False, saved_trajectory_escape_certified=False, future_runtime_certified=False,
    )
    witness_record = asdict(witness)
    witness_record.pop("obstruction")
    witness_record.update(family_invariance_excluded=True, saved_trajectory_escape_certified=False)
    ...
    graph_record.update(
        ...
        whole_band_exit_certified=False, future_runtime_certified=False, reachable_from_saved_state_certified=False,
    )
    runtime = _runtime_bridge() if include_runtime else None
    return {
        "source": {"source_report_replayed": True, "saved_B40_state": asdict(current),
                   "phase": profile.lattice.source.phase, "B38_visible_rows": points},
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_temporal_compatibility.json")
    parser.add_argument("--skip-runtime", action="store_true", help="Produce only the static B41-B42 certificates")
    ...
    report = analyze_c6_winding_temporal_compatibility(*producers, include_runtime=not args.skip_runtime)
    sha, dirty, digest = provenance
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-temporal-cell-compatibility", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        telemetry=("complete-carry-cell obstruction", "all pairwise feasible edges", "finite family escape deadline", "optional live phase trace"),
        controls=("no continuation or reset of saved B40 carry", "existential edges not assumed composable",
                  "family escape distinct from full-band escape", "hypothetical and graph-owned states remain separate"),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or tuple(path.read_bytes() for path in paths) != raw:
        raise RuntimeError("analysis source or historical input changed during the temporal audit")
