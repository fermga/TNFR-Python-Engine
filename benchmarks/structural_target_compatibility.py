...
SOURCE_SCOPE = (
    ...
    "benchmarks/thol_pressure_feedback.py", "benchmarks/structural_target_compatibility.py",
)
...
def _snapshot(data):
    ...
    for name, value in data.items():
        if name == "nodes":
            ...
            values[name] = (tuple(map(Fraction, value)) if isinstance(value, list)
                            else Fraction(value))
...
def _reference(data):
    return derive_forced_support_balance(
        _snapshot(data["source"]), epi_weight=Fraction(data["epi_weight"]),
...
def analyze_target_case(case):
    ...
    target, current = _reference(case["original_target"]), _reference(case["postevent_reference"])
    if case["events"]:
        event = case["events"][-1]
    ...
    if (tuple(actual["nodes"]) != current.source.nodes or any(
        ...
        for name, field in (("epi", "epi"), ("capacity", "capacity"),
                            ("pressure", "stored_pressure"))
    )):
        raise ValueError("reference source does not match the recorded actual endpoint")
    if any(getattr(capture.snapshot, name) != getattr(current.source, name)
    ...
    if (capture.phase != phase or capture.forcing != current.forcing
    ...
    observation = observe_forced_support_target(
        target, current, current.source,
        forcing_components=decompose_non_epi_forcing(capture),
    )
    recorded_limit = Fraction(case["conditional_fixed_model_limit"]["pattern"]["error_variance"])
    ...
    return observation, {
        "case": case["case"], "observation": asdict(observation),
        "state_time": actual["time"],
        "channel_capture_time": channel_time, "channel_capture": asdict(capture),
        "scope": (
...
def compare_target_channels(before, after):
    ...
    contributions = tuple((name, dot(metric, tuple(
        mid * (b - a) for mid, a, b in zip(midpoint, old[name], new[name], strict=True)
    ))) for name in old)
    ...
    return {
        "channel_contributions": contributions, "compatibility_energy_change": change,
...
def analyze_target_report(parent):
    ...
    return {
        "cases": cases,
        "perturbation_channel_change": compare_target_channels(observations[0], observations[1]),
        "feedback_channel_change": compare_target_channels(observations[1], observations[2]),
...
def main():
    ...
    manifest = CoreExperimentManifest(
        ...
        telemetry=("projected target rate", "signed channel Gram terms",
                   "old-target energy derivative", "fixed-model profile mismatch"),
        controls=("independent P2/P3 rational tests", "distinct input and analysis provenance"),
    ...
    report.update(
        ...
        input_evidence={
            "path": str(args.input), "sha256": hashlib.sha256(source_bytes).hexdigest(),
    ...
    for case in report["cases"]:
        ...
        print(case["case"], "target compatible:", result["target_compatible"],
              "residual energy:", float(result["compatibility_energy"]),
              "current Rdot:", float(result["model_energy_rate"]))
    print("Feedback channel allocation:", {
        name: float(value) for name, value in report["feedback_channel_change"]["channel_contributions"]
    })
