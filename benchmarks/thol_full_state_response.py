...
from benchmarks.thol_distributed_target import (  # noqa: E402
    _common_source, _digest, _flow_steps, _held_capture_checks, _reference,
    _snapshot, _target,
)
from benchmarks.thol_distributed_transport import (  # noqa: E402
    STEPS, _forcing_readout, _physical_flow, _require_stage,
...
from tnfr.config.thresholds import (  # noqa: E402
    EPI_LATENT_MAX, MIN_NETWORK_DEGREE_COUPLING, VF_BASAL_THRESHOLD,
)
...
COMMON_FIELDS = (
    "prefix", "original_reference", "original_reference_sha256", "realization",
    "baseline_flow", "baseline_steps", "baseline_capture_checks", "baseline_target",
    "common_source", "before_event", "lineage",
)
PROTOCOL = {
    "source": "Actual all-node IL/OZ preparation, distributed THOL birth and first parent UM",
    "event_time": 1.0, "branches": BRANCHES,
    "intervention": "One public simultaneous default Emission on every actual born child",
    "steps": STEPS, "fallback_operator": None, "cohort_search": False,
    "horizon_search": False, "coefficient_tuning": False, "target_rewrite": False,
    ...
    return {
        **_common_source(graph),
        "emission_configuration": _literal({
            ...
            "VF_BASAL_THRESHOLD": graph.graph.get("VF_BASAL_THRESHOLD", VF_BASAL_THRESHOLD),
            ...
            "bounds": {key: value for key, value in graph.graph.items()
...
def _birth_families(graph, prefix):
    ...
    if (len(pairs) != 8 or len(set(parents + children)) != 16
            or tuple(graph) != parents + children
            or parents != tuple(prefix["birth"]["before"]["nodes"])):
        raise ValueError("the declared experiment requires the actual eight birth families")
    for parent, child in pairs:
        if (graph.nodes[child].get("parent_node") != parent
                or tuple(graph.nodes[parent].get("sub_nodes", ())) != (child,)
                or tuple(graph.graph["hierarchy"].get(parent, ())) != (child,)):
            raise ValueError("live parentage differs from the native THOL birth receipt")
    return {"parent_children": pairs, "parents": parents, "children": children,
            "nodes": tuple(graph), "selection": "All actual children, once; no label inference"}
...
def _emission_admission(graph, children):
    ...
    for node in children:
        ...
        try:
            ...
            reason = str(exc)
        rows.append({"node": node, "grammar": asdict(grammar),
    ...
    return {"targets": children, "rows": tuple(rows),
...
def _emission_event(graph, children):
    ...
    stage = execute_network_operator_stage(
        graph, operator, children, include_epi_jump_certificate=True,
    )
    ...
    native_nodes, native_targets = tuple(certificate.nodes), tuple(certificate.target_nodes)
    if (len(native_nodes) != len(before["nodes"]) or len(set(native_nodes)) != len(native_nodes)
            or set(native_nodes) != set(before["nodes"])
            or len(native_targets) != len(children) or len(set(native_targets)) != len(native_targets)
            or set(native_targets) != set(children)):
        raise RuntimeError("native Emission receipt has different node or target identities")
    ...
    if (tuple(Fraction(float(value)) for value in certificate.state_before) != exact_before
            or tuple(Fraction(float(value)) for value in certificate.runtime_proposed_state_after) != exact_after):
        raise RuntimeError("native Emission receipt does not match the actual committed EPI")
    conditions = dict(certificate.runtime_epi_realization_conditions)
    required = (
        "two_phase_jacobi_declared", "proposal_builder_replayed_from_declared_inputs",
        "scalar_epi_snapshot", "supported_hard_or_inactive_clip_policy",
        "edge_adaptation_inactive", "operator_gate_bound",
    )
    if (not all(conditions.get(key) is True for key in required)
            or not certificate.fixed_support_declared
            or not certificate.proposal_builder_replayed_from_declared_inputs):
        raise RuntimeError("native Emission partial event lacks replayed admission evidence")
    matrix, offset = certificate.exact_represented_linear_map, certificate.exact_represented_offset
    size = len(native_nodes)
    if (matrix != tuple(tuple(Fraction(i == j) for j in range(size)) for i in range(size))
            or len(offset) != size
            or any(offset[index] for index, node in enumerate(native_nodes) if node not in children)):
    ...
    if (residual != certificate.exact_runtime_minus_represented_affine
            or certificate.runtime_matches_represented_affine_exactly != (not any(residual))
            or conditions.get("runtime_matches_represented_affine_exactly") != (not any(residual))
            or conditions.get("all_graph_nodes_targeted_once") != (set(native_targets) == set(native_nodes))
            or certificate.runtime_epi_realization_certified != all(conditions.values())):
        raise RuntimeError("native Emission affine residual or global-scope flags disagree")
    ...
    native_fields = (
        "operator_name", "glyph", "nodes", "target_nodes", "stage_schedule",
        "fixed_support_declared", "exact_represented_linear_map", "exact_represented_offset",
        "exact_runtime_minus_represented_affine", "runtime_matches_represented_affine_exactly",
        "clip_mode", "clip_intervention_nodes", "edge_adaptation_nodes",
        "proposal_builder_replayed_from_declared_inputs", "runtime_epi_realization_conditions",
        "runtime_epi_realization_certified", "scope",
    )
    return {
        "before": before, "after": after, "before_capture": before_capture,
        "after_capture": after_capture, "actual_epi_jump": jump,
        "any_epi_jump": any(jump),
        "stage_result": {"operator": stage.operator, "glyph": stage.glyph,
                         "schedule": stage.schedule, "nodes_processed": stage.nodes_processed,
                         "epi_jump_certificate_abstention_reason":
                         stage.epi_jump_certificate_abstention_reason},
        ...
        "partial_event_evidence": {
            ...
            "native_targets_to_declared_indices": tuple(children.index(node) for node in native_targets),
            "native_order_before_epi": exact_before, "native_order_after_epi": exact_after,
            "represented_affine_after": affine_after, "exact_runtime_affine_residual": residual,
            "proposal_and_gate_replayed": True, "affine_residual_identity_verified": True,
            "global_certificate_failed_conditions": tuple(key for key, passed in conditions.items() if not passed),
            "scope": "Actual partial AL event and signed runtime residual; no full-network affine-gain certificate",
        },
        "after_node_attributes": tuple((node, _literal(dict(data))) for node, data in graph.nodes(data=True)),
        "scope": (
...
def _activity(capture):
    ...
    fresh_rate = tuple(nu * pressure for nu, pressure in zip(snapshot.capacity, fresh, strict=True))
    return {
        "positive_capacity_at_every_node": all(nu > 0 for nu in snapshot.capacity),
        "minimum_capacity": min(snapshot.capacity), "stored_nodal_rate": snapshot.rate,
        "fresh_kernel_nodal_rate": fresh_rate,
        "nonzero_fresh_rate_nodes": tuple(node for node, rate in zip(snapshot.nodes, fresh_rate) if rate),
...
def _forecast_error_accounting(prediction, steps):
    ...
    if tuple(steps[0]["before"]["snapshot"]["epi"]) != tuple(prediction.frames[0].epi):
        raise ValueError("native continuation did not begin at the frozen forecast state")
    for index, (record, h, matrix, frame) in enumerate(zip(
            steps, prediction.steps, prediction.fine_euler_matrices,
            prediction.frames[1:], strict=True)):
        if record["dt"] != h:
        ...
        local_pressure = tuple(h * nu * defect for nu, defect in zip(
        ...
        if actual != expected or reduced_error != _mv(c, expected) or _mv(t, reduced_error) != actual:
            raise RuntimeError("full-state forecast defect propagation identity failed")
        rows.append({
            "ordinal": index + 1, "elapsed_time": frame.time,
            "local_pressure_defect_impulse": local_pressure,
            "local_integrator_state_defect": local_step,
            "propagated_pressure_error": pressure_error, "propagated_integrator_error": step_error,
            "actual_epi": tuple(right["epi"]), "ideal_epi": tuple(frame.epi),
            "actual_minus_ideal_epi": actual, "actual_reduced_state": actual_reduced,
            "ideal_reduced_state": frame.reduced_state, "actual_minus_ideal_reduced": reduced_error,
            "identity_residual": (Fraction(0),) * n,
            "maximum_absolute_epi_error": max(map(abs, actual)),
        })
    return {"frames": tuple(rows), "all_exact_identities_pass": True,
...
def replay_response_branch(branch="control"):
    ...
    realization = observe_forced_support_realization(
        original, lineage["parent_children"], epi=_snapshot(baseline_capture).epi,
    )
    if realization.dimension != 16 or realization.full_state_dimension != 16:
        raise RuntimeError("the predeclared full sixteen-EPI realization was not recovered")
    common = {
        "prefix": prefix, "original_reference": asdict(original),
        "original_reference_sha256": original_hash, "lineage": lineage,
        "realization": asdict(realization), "baseline_flow": baseline_flow,
        "baseline_steps": _flow_steps(original, baseline_flow), "baseline_capture_checks": held,
        "baseline_target": asdict(baseline_target), "common_source": _source(graph),
        "before_event": before,
    }
    ...
    if branch == "child_emission":
        ...
        if not admission["allowed"]:
            return graph, {**common, "branch": branch, "status": "refused", "admission": admission,
                           "event": None, "frozen_prediction": None, "continuation_flow": None,
    ...
    event_budget = observe_forced_support_event(
        original, original, _snapshot(baseline_capture), _snapshot(post_capture),
    )
    prediction = predict_forced_support_realization_euler(
        original, lineage["parent_children"], tuple(Fraction(h) for h in STEPS),
        epi=_snapshot(post_capture).epi,
    )
    for field in ("observation", "right_inverse", "reduced_generator", "output_map", "reduced_source"):
        if getattr(prediction.realization, field) != getattr(realization, field):
            raise RuntimeError("forecast replaced the fixed full-state coordinates or dynamics")
    frozen_hash = _digest(prediction)
    frozen = {"payload": asdict(prediction), "sha256": frozen_hash,
              "frozen_before_continuation": True, "physical_time_at_freeze": graph.graph["_t"]}
    ...
    if _digest(prediction) != frozen_hash or graph.graph["_t"] != 1.5:
        raise RuntimeError("forecast or declared horizon changed during native continuation")
    final_capture = continuation["after_forcing"]
    final_checks = _held_capture_checks(post_capture, final_capture)
    if _reference(final_capture, original) is not original or _digest(original) != original_hash:
    ...
    return graph, {
        **common, "branch": branch, "status": "executed", "admission": admission,
        "event": event, "post_event_capture": post_capture,
        "post_event_target": asdict(post_target), "event_target_budget": asdict(event_budget),
        "post_event_model_checks": model_checks, "frozen_prediction": frozen,
        "continuation_flow": continuation, "continuation_steps": steps,
        "continuation_capture_checks": final_checks, "forecast_errors": forecast_errors,
        "endpoint_target": asdict(endpoint), "endpoint": deepcopy(_state(graph)),
        "activity": {"before_event": _activity(baseline_capture),
                     "after_event": _activity(post_capture), "endpoint": _activity(final_capture)},
        "original_target_response": {
            "event_variance_change": damage, "target_damage_present": damage > 0,
            "continuation_variance_change": change,
            "partial_target_recovery_observed": damage > 0 and change < 0,
            "returned_to_pre_event_target_score": damage > 0 and (
                endpoint.pattern.error_variance <= baseline_target.pattern.error_variance),
...
def _paired_delta(control, perturbed, metric):
    ...
    return {
        "epi_difference": delta, "weighted_mean_offset": mean,
        "centered_epi_difference": centered,
        "centered_H_energy": dot(metric, tuple(value * value for value in centered)) / 2,
        "full_H_energy": dot(metric, tuple(value * value for value in delta)) / 2,
        "full_epi_equal": not any(delta), "shape_equal_modulo_uniform_offset": not any(centered),
    }
...
def run_study():
    ...
    if perturbed["status"] == "executed":
        ...
        start = _paired_delta(
            ...
            perturbed["post_event_target"]["state"]["snapshot"]["epi"], h,
        )
        end = _paired_delta(
            control["endpoint_target"]["state"]["snapshot"]["epi"],
            perturbed["endpoint_target"]["state"]["snapshot"]["epi"], h,
        )
        paired = {
            "after_event": start, "endpoint": end,
            "centered_response_energy_change": end["centered_H_energy"] - start["centered_H_energy"],
            "mean_offset_change": end["weighted_mean_offset"] - start["weighted_mean_offset"],
            "nonzero_intervention": not start["full_epi_equal"],
            "full_epi_return_observed": not start["full_epi_equal"] and end["full_epi_equal"],
            "centered_response_decreased": end["centered_H_energy"] < start["centered_H_energy"],
            "scope": (
    ...
    return {
        "protocol": deepcopy(PROTOCOL), "branches": branches,
        "complete_common_source_equal": True, "common_fields_compared": COMMON_FIELDS,
        "paired_response": paired, "new_trajectories_executed": True,
        "coefficient_tuning": False, "target_rewrite": False,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=(
        ROOT / "artifacts/research/thol_full_state_response.json"))
    args = parser.parse_args()
    scope = ("src/tnfr", "benchmarks/thol_full_state_response.py",
    ...
    manifest = CoreExperimentManifest(
        claim_id="O1.b-generated-full-EPI-state-response", git_sha=sha,
        ...
        timestep=STEPS[0], seed=17, result_status=ClaimStatus.MEASURED,
        operator_sequence=("coherence", "dissonance", "self_organization", "coupling", "emission"),
        telemetry=("complete causal source", "native AL jump", "forecast-before-flow",
                   "pressure/integrator defects", "paired shape and mean response"),
        controls=("independent no-event branch", "fixed original profile and full EPI realization"),
