...
from tnfr.operators.event_timing import build_operator_event_schedule, build_physical_flow_partition  # noqa: E402
...
TAIL_ENTRY_CYCLE = 89
SEGMENT_DURATION = .0625
INTERVAL_DURATION = .25
EPI_LOWER, EPI_UPPER = .375, .625
...
def _declared_schedule(cycles):
    ...
    durations = tuple(INTERVAL_DURATION if 0 < i <= 2 * cycles and i % 2 == 0 else 0.
                      for i in range(len(names) + 1))
    schedule = build_operator_event_schedule(names, start_time=0., flow_durations=durations)
    partitions = tuple(build_physical_flow_partition(interval, (SEGMENT_DURATION,) * 4)
...
def _event_record(event):
    return {
        "operator_name": event.event.operator_name, "time": event.event.event_time,
        "phase_before": event.phase_before, "phase_after": event.phase_after,
        "capacity_before": event.before_snapshot.nu_f, "capacity_after": event.after_snapshot.nu_f,
        ...
        "stage_schedule": event.stage.schedule, "target_count": event.stage.nodes_processed,
    }
...
def run_c6_phase_tail_runtime_bridge(*, cycles=TAIL_ENTRY_CYCLE):
    ...
    graph = prepare_c6_phase_response("null", 0.)
    ...
    result = execute_nodal_remainder_event_schedule(
        graph, schedule, physical_flow_partitions=partitions, epi_lower=EPI_LOWER, epi_upper=EPI_UPPER,
    )
    if (not result.runtime_provenance_certified or len(result.events) != 2 * cycles + 1
            or len(result.flows) != 4 * cycles or result.initial_binding.state.epi != (.5,) * 6
            or any(result.initial_binding.state.remainder) or initial_capacity != (1.,) * 6
            or tuple(graph.edges) != initial_support or any(result.exact_nodal_balance_residual)):
        raise RuntimeError("the live preparation bridge lost its executor-owned finite contract")
    rows = []
    current_phase = initial_phase
    for index in range(cycles):
        um, il = result.events[2 * index:2 * index + 2]
        expected = observe_c6_coupling_coherence_phase_step(phase=current_phase)
        if (um.event.operator_name != "coupling" or il.event.operator_name != "coherence"
                ...
                or um.before_binding is not um.after_binding or il.before_binding is not il.after_binding
                or any(item.stage.schedule != "two_phase_jacobi" or item.stage.nodes_processed != 6
                       for item in (um, il))
                or any(item.before_snapshot.nu_f != (1.,) * 6 or item.after_snapshot.nu_f != (1.,) * 6
                       for item in (um, il))):
            raise RuntimeError("a live UM/IL phase or carry observation differs from the shared phase map")
        flows = result.flows[4 * index:4 * index + 4]
        weights = dict(flows[0].normalized_weights)
        lattice = derive_c6_pressure_lattice(
            phase=il.phase_after, epi_weight=float(weights["epi"]), phase_weight=float(weights["phase"]),
            epi_lower=EPI_LOWER, epi_upper=EPI_UPPER,
        )
        records = []
        for flow in flows:
            pressure = _observe_rebuilt_c6_pressure_lattice(lattice, flow.step.before.epi)
            if (flow.step.capacity != (1.,) * 6 or flow.step.timestep != SEGMENT_DURATION
                    or _signature(flow.phase_before) != _signature(il.phase_after)
                    or _signature(flow.phase_after) != _signature(il.phase_after)
                    or dict(flow.normalized_weights) != weights or flow.step.pressure != pressure.pressure
                    or not all(value for _name, value in flow.pressure_refresh_checks)):
                raise RuntimeError("the live flow differs from its actual phase-bound canonical pressure")
            records.append({"step": asdict(flow.step), "phase_before": flow.phase_before,
                            "phase_after": flow.phase_after, "start_time": flow.segment.start_time,
                            "end_time": flow.segment.end_time, "fresh_pressure_matches": True})
        rows.append({"ordinal": index + 1, "coupling": _event_record(um), "coherence": _event_record(il),
                     "phase_projection": asdict(expected), "flows": tuple(records)})
        current_phase = il.phase_after
    terminal = result.events[-1]
    actual_phase = tuple(graph.nodes[node][ALIAS_THETA[0]] for node in graph)
    if (terminal.event.operator_name != "silence" or terminal.before_binding is not terminal.after_binding
            ...
            or terminal.before_snapshot.nu_f != (1.,) * 6
            or not all(0. <= value < 1. for value in terminal.after_snapshot.nu_f)):
        raise RuntimeError("the terminal SHA boundary lost its phase/carry preservation or capacity change")
    following_phase_step = observe_c6_coupling_coherence_phase_step(phase=current_phase)
    fixed = _signature(following_phase_step.phase_after_coherence) == _signature(current_phase)
    if cycles == TAIL_ENTRY_CYCLE and not fixed:
        raise RuntimeError("the declared 89-cycle live phase endpoint did not enter its fixed projection")
    ...
    closure = derive_c6_carried_closure(profile, state=tail_entry, timestep=SEGMENT_DURATION)
    closure_record = asdict(closure)
    closure_record.pop("base_tube")
    return {
        "source": {"preparation": "canonical null C6 winding", "initial_phase": initial_phase,
                   "initial_state": asdict(result.initial_binding.state), "initial_capacity": initial_capacity,
                   "ordered_nodes": result.nodes, "support": initial_support,
                   "normalized_weights": result.flows[0].normalized_weights, "random_seed": 17,
                   "declared_carried_band": (EPI_LOWER, EPI_UPPER),
                   "carry_imported_or_reset": False},
        "cycle_count": cycles, "event_count": len(result.events), "flow_count": len(result.flows),
        "schedule": asdict(schedule), "cycles": tuple(rows), "terminal_silence": _event_record(terminal),
        ...
        "tail_entry_after_last_IL": asdict(tail_entry), "endpoint_before_SHA": asdict(endpoint),
        "tail_entry_time": result.events[-2].after_binding.time,
        "endpoint_time": terminal.before_binding.time, "endpoint_phase": current_phase,
        "tail_entry_closure": closure_record,
        "tail_entry_profile": asdict(profile.forced_balance),
        "exact_nodal_area": result.exact_nodal_area, "exact_reconstructed_change": result.exact_reconstructed_change,
        ...
        "scope": ("One admitted finite live word from the original null preparation, with a newly declared carried "
