...
from benchmarks.structural_target_compatibility import compare_target_channels  # noqa: E402
from benchmarks.thol_distributed_transport import (  # noqa: E402
    STEPS, _couple_parents, _physical_flow, prepare_distributed_transport_support,
)
...
from tnfr.operators.preconditions import (  # noqa: E402
    OperatorPreconditionError, validate_coupling, validate_phase_gate_u3,
)
from tnfr.physics.forced_support import (  # noqa: E402
    derive_forced_support_balance, observe_forced_support_event,
    observe_forced_support_step, observe_forced_support_target,
)
...
def _reference(readout, previous=None):
    ...
    if (previous is not None and previous.epi_weight == observation["epi_weight"]
            and previous.forcing == observation["forcing"]
            and all(getattr(previous.source, field) == getattr(snapshot, field)
                    for field in ("nodes", "conductance", "support_neighbors", "capacity"))):
        return previous
    return derive_forced_support_balance(
        snapshot, epi_weight=observation["epi_weight"], forcing=observation["forcing"],
    )


def _digest(reference):
    encoded = json.dumps(_payload(asdict(reference)), sort_keys=True,
...
def _target(original, current, readout):
    return observe_forced_support_target(
        original, current, _snapshot(readout), forcing_components=readout["components"],
    )
...
def _flow_steps(reference, flow):
    ...
    for segment in flow["segments"]:
        ...
        observation = observe_forced_support_step(
            reference, before, after, segment["interval"]["duration"],
        )
...
def _held_capture_checks(before, after):
    ...
    checks = {
        ...
        "normalized_weights_preserved": first["normalized_weights"] == last["normalized_weights"],
    ...
    if not all(checks.values()):
        raise RuntimeError("event-free flow changed captured phase, coefficients or support")
    return {**checks, "scope": (
...
def _common_source(graph):
    ...
    return {
        ...
        "node_attributes": tuple((node, _literal(dict(data)))
        ...
        "configuration": deepcopy({
            ...
            if key.startswith("UM_") or key in (
                "GLYPH_FACTORS", "DNFR_WEIGHTS", "_dnfr_weights",
                "VALIDATE_OPERATOR_PRECONDITIONS", "DELTA_PHI_MAX",
                "RANDOM_SEED", "SORT_NODES",
            )
        }),
    }
...
def _same_state_rate_change(before, after):
    ...
    if before.target_reference != after.target_reference or before.pattern != after.pattern:
        raise ValueError("rate comparison requires one unchanged EPI pattern and original target")
    ...
    realization = (after.stored_pressure_energy_rate_defect
...
def _lineage_targets(graph, prefix, branch):
    ...
    if branch not in LINEAGE_BRANCHES:
        raise ValueError("a lineage branch must select original parents or actual children")
    ...
    if (not parents or parents != tuple(prefix["birth"]["before"]["nodes"])
            or len(set(parents)) != len(parents) or len(set(children)) != len(children)
            or set(parents) & set(children) or tuple(graph) != parents + children):
        raise ValueError("birth cohorts must be unique, disjoint and exhaust actual ordered support")
    for parent, child in pairs:
        if (graph.nodes[child].get("parent_node") != parent
            ...
            raise ValueError("retained birth receipt disagrees with actual graph lineage")
    selected = parents if branch == "original_parents" else children
    return selected, {
        "parent_children": pairs, "parents": parents, "children": children,
        "current_nodes": tuple(graph), "selected_targets": selected,
        "disjoint_and_exhaustive": True, "live_parentage_verified": True,
...
def _cohort_admission(graph, targets):
    ...
    for node in targets:
        ...
        rows.append({
            "node": node, "grammar": asdict(grammar),
            "hard_u3_allowed": hard_reason is None, "hard_u3_refusal": hard_reason,
            "optional_gate_enabled": optional,
            "optional_gate_allowed": None if not optional else optional_reason is None,
            "optional_gate_refusal": optional_reason,
            "allowed": grammar.allowed and hard_reason is None and optional_reason is None,
        })
    if before != _common_source(graph):
        raise RuntimeError("read-only cohort admission changed its projected source")
    return {"targets": targets, "candidates": tuple(rows),
...
def run_distributed_target_branch(branch="no_event"):
    ...
    if branch in LINEAGE_BRANCHES:
        ...
        if not admission["allowed"]:
            return {
                "branch": branch, "status": "refused", "lineage": lineage,
                "admission": admission, "prefix": prefix,
                ...
                "baseline_flow": baseline_flow, "baseline_steps": baseline_steps,
                "baseline_capture_checks": baseline_capture_checks,
                "baseline_target": asdict(baseline_target), "common_source": common_source,
                "before_optional_event": preevent, "event": None,
                "after_optional_event": deepcopy(_state(graph)),
                "post_event_target": asdict(baseline_target),
                "continuation_flow": None, "continuation_steps": (),
                "continuation_capture_checks": None, "endpoint_target": None,
    ...
    if branch != "no_event":
        ...
        _, coupling = _couple_parents(
            graph, targets, case="attached",
            target_policy=("Every current node once, after birth and baseline transport"
                           if branch == "all_node_um" else
                           f"Actual {branch} cohort once, after the common baseline"),
        )
        ...
        event_budget = observe_forced_support_event(
            baseline_reference, current, _snapshot(baseline_readout),
        ...
        if post_event_target.pattern != baseline_target.pattern:
            raise RuntimeError("UM changed the fixed-target EPI pattern without an EPI jump")
        event = {
            "coupling": coupling, "exact_forced_event": asdict(event_budget),
    ...
    continuation_flow = _physical_flow(graph, _snapshot(
    ...
    if _reference(endpoint_readout, current) is not current:
        raise RuntimeError("continuation event-free flow changed its captured held model")
    ...
    return {
        ...
        **({"status": "executed", "lineage": lineage, "admission": admission}
        ...
        "before_optional_event": preevent, "event": event,
        "after_optional_event": postevent,
        "post_event_target": asdict(post_event_target),
        "continuation_flow": continuation_flow, "continuation_steps": continuation_steps,
        "continuation_capture_checks": continuation_capture_checks,
        "endpoint_target": asdict(endpoint_target), "endpoint": deepcopy(_state(graph)),
        "comparison": {
            "baseline_fixed_target_variance_change": (
                baseline_target.pattern.error_variance - initial_target.pattern.error_variance),
            "continuation_fixed_target_variance_change": (
                endpoint_target.pattern.error_variance - post_event_target.pattern.error_variance),
            "original_metric_mean_change": endpoint_target.pattern.mean - initial_target.pattern.mean,
            "mean_scope": (
            ...
            "initial_target_error_is_zero": not any(initial_target.pattern.relative_error),
...
def run_study():
    ...
    left, right = branches
    if (left["original_reference"] != right["original_reference"]
        ...
        raise RuntimeError("independent causal branches did not reproduce the common baseline")
    return {
        "branches": branches, "common_causal_baseline_reproduced": True,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=(
        ROOT / "artifacts/research/thol_distributed_target.json"))
    args = parser.parse_args()
    scope = (
        "src/tnfr", "benchmarks/thol_distributed_target.py",
    ...
    manifest = CoreExperimentManifest(
        ...
        operator_sequence=("coherence", "dissonance", "self_organization", "coupling", "coupling"),
        telemetry=("original relative profile and initial discrepancy", "current-model compatibility",
                   "event reset and old-target error", "exact mean drift and finite step budgets"),
