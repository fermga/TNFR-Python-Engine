...
from benchmarks.c6_winding_mean_cylinder import INPUT_NAME as HISTORICAL_NAME  # noqa: E402
from benchmarks.c6_winding_pressure_sign import _canonical  # noqa: E402
from benchmarks.c6_winding_relay_budget import replay_c6_relay_budget_evidence  # noqa: E402
...
def _boundary_interaction(anchor, state):
    ...
    rows = tuple(tuple(state.epi[4] if node == 4 and mask & 4 else value
                       for node, value in enumerate(anchor.visible_rows[mask & 3]))
                 for mask in range(8))
    reference = anchor.closure.base_tube.contraction.profile.lattice
    pressures = tuple(_observe_rebuilt_c6_pressure_lattice(reference, row).pressure for row in rows)
    h = F(anchor.closure.base_tube.contraction.timestep)
    increments = tuple(tuple(h * F(p) for p in row) for row in pressures)
    mixed = tuple(increments[6][i] - increments[2][i] - increments[4][i] + increments[0][i]
                  for i in range(6))
    node0_mixed = tuple(
        tuple(increments[1 | bit][i] - increments[1][i] - increments[bit][i] + increments[0][i]
              for i in range(6)) for bit in (2, 4)
    )
    triple = tuple(
        sum(((-1 if (3 - mask.bit_count()) % 2 else 1) * increments[mask][i]
             for mask in range(8)), F(0)) for i in range(6)
    )
    return {"visible_rows": rows, "pressure_rows": pressures, "exact_increments": increments,
            "node3_node4_mixed_increment": mixed, "node0_pair_mixed_increments": node0_mixed,
            "triple_mixed_increment": triple, "three_independent_relays_refuted": any(mixed),
...
def analyze_c6_winding_relay_handoff(parent, *, historical_bytes, step_budget=1024):
    ...
    anchor, state = replay_c6_relay_budget_evidence(parent, historical_bytes=historical_bytes)
    budget = derive_c6_carried_local_relay_budget(anchor, state=state, relay_node=0, budget_node=1)
    ...
    passage.pop("budget")
    old_area = tuple(F(x) for x in parent["B46_conditional_first_exit"]["total_nodal_area"])
    area = tuple(a + b for a, b in zip(old_area, observed.total_nodal_area, strict=True))
    historical = json.loads(historical_bytes)
    original_area = tuple(F(x) for x in historical["B43_original_phase_tail_runtime"]["exact_nodal_area"])
    ...
    return {
        "contract": {
            ...
            "computational_step_budget": step_budget, "budget_is_physical_parameter": False,
            "pressure_projected_or_carry_reset": False,
        },
        "source": {
            "retained_B46_endpoint": asdict(state), "historical_nodal_steps_replayed": 356,
            "previous_conditional_steps_replayed": 7, "serialized_numerical_chain_verified": True,
            "live_execution_seal_recreated": False, "phase": anchor.closure.base_tube.contraction.profile.lattice.source.phase,
            "capacity": (1.,) * 6, "timestep": anchor.closure.base_tube.contraction.timestep,
        ...
        "conditional_elapsed_time": F(budget.closure.base_tube.contraction.timestep) * observed.exit_step,
        "total_conditional_steps_after_B43": 7 + observed.exit_step,
        "conditional_lineage_from_B46_numerically_verified": True,
        "new_live_graph_steps": 0, "whole_band_exit_certified": False,
        "indefinite_trapping_certified": False, "future_runtime_certified": False,
    }
...
def replay_c6_relay_handoff_evidence(parent, *, relay_bytes, historical_bytes):
    ...
    if (manifest.claim_id != "O3.a-C6-carried-local-relay-handoff"
            or tuple(parent["source_scope"]) != SOURCE_SCOPE):
        raise ValueError("the invariant-region study requires the retained B47 handoff report")
    if type(relay_bytes) is not bytes or type(historical_bytes) is not bytes:
        raise TypeError("relay_bytes and historical_bytes must be exact retained input bytes")
    evidence = parent["input_evidence"]
    if (hashlib.sha256(relay_bytes).hexdigest() != evidence["sha256"]
            or hashlib.sha256(historical_bytes).hexdigest() != evidence["historical_sha256"]):
        raise ValueError("the retained B46/B43 input hashes differ from B47's declared sources")
    relay = json.loads(relay_bytes)
    if (_canonical(evidence["producer_manifest"]) != _canonical(relay["manifest"])
            or _canonical(evidence["producer_source_scope"]) != _canonical(relay["source_scope"])):
        raise ValueError("the retained B46 producer metadata differs from B47's source")
    expected = analyze_c6_winding_relay_handoff(
        relay, historical_bytes=historical_bytes,
        step_budget=parent["contract"]["computational_step_budget"],
    )
    payload = {key: value for key, value in parent.items()
               if key not in ("manifest", "source_scope", "input_evidence")}
    if _canonical(payload) != _canonical(expected):
        raise ValueError("the retained B47 result differs from its complete numerical replay")
    ...
    historical = json.loads(historical_bytes)
    weights = dict(historical["B43_original_phase_tail_runtime"]["source"]["normalized_weights"])
    reference = derive_c6_pressure_lattice(
        phase=tuple(expected["source"]["phase"]),
        epi_weight=float(F(weights["epi"])), phase_weight=float(F(weights["phase"])),
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_relay_handoff.json")
    ...
    if args.output.resolve() in (args.input.resolve(), args.historical_input.resolve()):
        raise ValueError("the derived report must not overwrite either historical input")
    ...
    report = analyze_c6_winding_relay_handoff(parent, historical_bytes=historical_raw, step_budget=args.step_budget)
    sha, dirty, digest = provenance
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-local-relay-handoff", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        telemetry=("pressure dependency support", "nonzero coupled-switch interaction", "signed corrected budget",
                   "independent band horizon", "first held-node exit", "complete nodal area and mean"),
        controls=("all incoming carries preserved", "arbitrary free-node values allowed by locality",
                  "source reports fully replayed", "conditional state lineage differs from a live execution seal"),
    ...
    if (current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or args.input.read_bytes() != raw
        ...
        raise RuntimeError("source or historical input changed during the local-relay audit")
    report.update(manifest=manifest.to_dict(), source_scope=SOURCE_SCOPE, input_evidence={
        ...
        "historical_path": str(args.historical_input), "historical_sha256": hashlib.sha256(historical_raw).hexdigest(),
    })
