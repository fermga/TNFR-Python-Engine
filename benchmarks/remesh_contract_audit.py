...
def _row(label, a):
    return (
        f"  {label:<28} {str(a.epi_recursion_verified):>5} "
...
def main() -> int:
    ...
    print(
        f"  {'candidate':<28} {'echo':>5} {'net':>5} {'id':>5} {'u5':>5} "
        f"{'REMESH':>8}"
    )
    ...
    _ = ExperimentManifest(
        ...
        controls=(
            "temporal_echo_discriminates",
            "static_lift_no_echo",
            "recurrence_all_four",
            "and_gate_requires_all",
        ),
        artifacts=(),
    )
    print()
    print(
        f"  tower realizes REMESH   : {camp.tower_realizes_remesh} "
        "(honest close: lift is a morphism, not REMESH)"
    )
    print(
        f"  audit discriminates     : {camp.audit_discriminates} "
        "(rejects static, accepts genuine REMESH)"
    )
    print(
        f"  REMESH for the tower    : {ClaimStatus.NEGATIVE.value} "
        "(NT-P04b; missing temporal echo)"
    )
    print(
        f"  four-condition gate     : {ClaimStatus.MEASURED.value} "
