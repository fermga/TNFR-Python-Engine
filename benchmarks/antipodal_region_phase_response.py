...
from benchmarks.compatible_capacity_regions import (  # noqa: E402
    _admissions, _captured_state, prepare_capacity_regions,
)
...
from tnfr.physics.coupling_support import (  # noqa: E402
    derive_antipodal_region_phase_balance, observe_antipodal_region_phase_response,
...
def prepare_phase_response(epsilon):
    if type(epsilon) is not float or epsilon not in EPSILONS:
        raise ValueError(f"epsilon must be one of the predeclared float controls {EPSILONS}")
    ...
    for node, base in zip(graph, BASE_PHASE, strict=True):
        graph.nodes[node][ALIAS_THETA[0]] = (base + (epsilon if node < 3 else -epsilon)) % math.tau
...
def _phase_readout(capture):
    ...
    return {
        "represented_lift": lift, "energy": energy, "interior": a, "bridge": b,
        "antisymmetric_projection_residual": tuple(v - p for v, p in zip(lift, projected)),
        "bridge_separation": Fraction(abs(angle_diff(float(capture.phase[2]), float(capture.phase[3])))),
...
def _observed_stage(graph, operator, step, before, *, phase_readout=_phase_readout):
    ...
    if operator.glyph is Glyph.IL:
        ...
        for node in targets:
            ...
            strict_readiness.append({
                "node": node, "passed": True,
                "warnings": coherence_precondition_warnings(graph, node),
            })
    factors = resolve_runtime_operator_factors(graph.graph["GLYPH_FACTORS"], operator.glyph, graph.graph)
    ...
    if operator.glyph is Glyph.UM:
        ...
        predicted_pressure = tuple(update.dnfr_after for update in predicted.node_updates)
        ...
        predicted = tuple(propose_coherence_stage(graph, n, factors["IL_dnfr_factor"]) for n in targets)
    ...
    stage = executor(graph, operator, targets, sequence_context=step, collect_metrics=True)
    if (stage.schedule != TWO_PHASE_JACOBI or stage.nodes_processed != len(targets)
            or any(graph.nodes[n]["glyph_history"][-1] != operator.glyph.value for n in targets)):
    ...
    matches = {
        ...
        "pressure": raw.snapshot.stored_pressure == tuple(Fraction(v) for v in predicted_pressure),
        "capacity": raw.snapshot.capacity == tuple(Fraction(v) for v in predicted_capacity),
    ...
    if not all(matches.values()):
        raise RuntimeError(f"prepared stage predictions or support boundary failed: {matches}")
    default_compute_delta_nfr(graph)
    following = capture_non_epi_forcing(graph)
    budget = asdict(observe_forced_support_event(
        _reference(before), _reference(following), before.snapshot, following.snapshot,
    ))
    ...
    return following, {
        ...
        "before": state_before, "before_capture": asdict(before), "raw_state": raw_state,
        "raw_capture": asdict(raw), "after_refresh": _captured_state(graph),
        "after_capture": asdict(following), "resolved_factors": factors,
        "independent_prediction": prediction, "prediction_matches_actual": matches,
        ...
        "coherence_before": coherence_before, "coherence_raw": coherence_raw,
        "coherence_refreshed": float(compute_coherence(graph)), "event_budget": budget,
        "phase_before": phase_readout(before), "phase_after": phase_readout(following),
...
def run_antipodal_phase_case(epsilon):
    ...
    linear = observe_antipodal_region_phase_response(model, interior=epsilon, bridge=epsilon)
    ...
    record = {
        "epsilon": Fraction(epsilon), "initial": initial_state,
        "initial_capture": asdict(initial), "initial_diagnostics": _diagnostics(graph),
        "word": admission, "phase_model": asdict(model), "linear_response": asdict(linear),
    ...
    initial_lift, final_lift = before_phase["represented_lift"], after_phase["represented_lift"]
    held = _reference(after_il)
    following, flow = _advance_forced_support_interval(
        graph, target, held, after_il, after_il, duration=STEP,
    )
    record.update(
        um=um, il=il, post_il_capture=asdict(after_il), final_capture=asdict(following),
        phase_before=before_phase, phase_after=after_phase,
        preparation_lift_residual=tuple(a - b for a, b in zip(initial_lift, initial_ideal)),
        observed_minus_linear=tuple(a - b for a, b in zip(final_lift, final_ideal)),
        observed_phase_energy_gain=(after_phase["energy"] / before_phase["energy"]
                                    if before_phase["energy"] else None),
        ...
        held_reference=asdict(held), flow=flow,
        final_before_closure=_captured_state(graph), final_diagnostics=_diagnostics(graph),
        final_nominal_pattern=asdict(observe_forced_support_pattern(
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=ROOT / "artifacts/research/antipodal_region_phase_response.json",
    )
    args = parser.parse_args()
    provenance = current_git_source_provenance(ROOT, SOURCE_SCOPE)
    sha, dirty, digest = provenance
    manifest = CoreExperimentManifest(
        claim_id="O3.a-antipodal-UM-IL-phase-response", git_sha=sha,
        ...
        telemetry=("exact local Jacobians", "represented phase lifts and linear defects",
                   "raw/refreshed coherence and pressure", "nodal endpoint and forcing budgets"),
