...
from benchmarks.thol_family_closure import _capture, _equal, _reference, _snapshot  # noqa: E402
...
from tnfr.physics.forcing_realization import NonEpiForcingObservation, decompose_non_epi_forcing  # noqa: E402
from tnfr.physics.support_transport import _from_data, observe_regional_support_euler  # noqa: E402
...
def _generation(raw):
    ...
    names = ("phase", "forcing", "phase_gradient", "full_kernel_pressure",
             "kernel_pressure_defect", "stored_pressure_residual")
    observation = NonEpiForcingObservation(
        snapshot=snap, epi_weight=F(raw["epi_weight"]),
        normalized_weights=tuple((k, F(v)) for k, v in raw["normalized_weights"]),
        **{key: reset._v(raw[key]) for key in names})
    return _capture({"observation": raw, "components": _payload(decompose_non_epi_forcing(observation))})
...
def _admit_reset(saved, original):
    ...
    if (saved["glyph"] != "EN" or F(saved["start_time"]) != START or F(saved["end_time"]) != END
            or tuple(saved["nodes"]) != nodes or nodes != original.source.nodes):
    ...
    for i, row in enumerate(rows):
        x, y, coefficient = (reset._v(row[key]) for key in ("before_epi", "after_epi", "row"))
        if (len(x) != n or len(y) != n or len(coefficient) != n or x != expected
    ...
    for key, value in (("S", matrix), ("c", offset), ("propagated_local_defects", propagated),
                       ("total_reset_defect", total), ("identity_residual", (F(0),)*n)):
    ...
    runtime = {
        "ideal_endpoint": _add(_add(reset.mv(matrix, snap.epi), offset), tuple(DT*v for v in ideal)),
    ...
    return {"snapshot": snap, "observation": observation, "components": components,
            "vectors": vectors, "S": matrix, "c": offset, "A": a, "b": b,
            "rows": rows, "runtime": runtime, "reset_defect": total, "saved": saved}
...
def audit_pair(left_reset, right_reset, original_reference, children):
    ...
    left, right = (_admit_reset(row, original_reference) for row in (left_reset, right_reset))
    for key in ("A", "b", "S", "c"):
    ...
    for lrow, rrow in zip(left["rows"], right["rows"], strict=True):
        ...
        for key in ("EN_mix", "neighbor_indices"):
            _equal(lrow["control"][key], rrow["control"][key], f"paired EN control {key}")
    snap, n = left["snapshot"], len(left["snapshot"].nodes)
    children = tuple(children)
    if not children or len(set(children)) != len(children) or not set(children) < set(snap.nodes):
    ...
    if any(force):
        raise ValueError("common positive-capacity affine source requires equal forcing")
    e = left["observation"].epi_weight
    delta = {key: _subtract(right["vectors"][key], left["vectors"][key]) for key in left["vectors"]}

    def snapshot(epi):
        return _from_data(snap.nodes, snap.conductance, snap.support_neighbors, epi, snap.capacity, pressure)

    def budget(before, after, dt):
        return observe_regional_support_euler(snapshot(before), snapshot(after), children,
                                              dt=dt, epi_weight=e, forcing=force)

    def shape(epi):
        return _paired_delta((F(0),)*len(indices), tuple(epi[i] for i in indices), metric)

    stages = []
    for ordinal, (lrow, rrow) in enumerate(zip(left["rows"], right["rows"], strict=True)):
        ...
        stages.append({"ordinal": ordinal, "node": lrow["node"], "delta_before": before,
                       "delta_after": after, "paired_increment": _subtract(after, before),
                       "control_increment": _subtract(reset._v(lrow["after_epi"]), reset._v(lrow["before_epi"])),
                       "perturbed_increment": _subtract(reset._v(rrow["after_epi"]), reset._v(rrow["before_epi"])),
    ...
    lag = tuple(e*(a-b) for a, b in zip(
        generation_snapshot.epi_gradient, integration.before.epi_gradient, strict=True))
    pressure_parts = {
        "held_generation_lag": lag,
        "generation_kernel_defect": _subtract(
            right["observation"].kernel_pressure_defect, left["observation"].kernel_pressure_defect),
        "generation_stored_minus_fresh": _subtract(
            right["observation"].stored_pressure_residual, left["observation"].stored_pressure_residual),
    }
    if reset._sum(pressure_parts.values(), n) != integration.balance.stored_pressure_defect:
        raise RuntimeError("held-pressure lag and generation defect decomposition failed")
    work = {}
    for name, values in pressure_parts.items():
        item = _component_budget(integration.balance, values)
        work[name] = {**item, "weighted_total_work": DT*item["weighted_total_rate"],
                      "variance_work": DT*item["variance_rate"]}
    mass = sum((F(row["budget"]["mass_change"]) for row in stages), F(0))+integration.mass_change+post.mass_change
    variance = sum((F(row["budget"]["variance_change"]) for row in stages), F(0))+integration.variance_change+post.variance_change
    endpoint = budget(delta["x0"], delta["xf"], F(0))
    if (mass != endpoint.mass_change or variance != endpoint.variance_change
            or sum((F(row["budget"]["variance_change"]) for row in stages), F(0)) != reset_total.variance_change):
        raise RuntimeError("paired regional stage telescope failed")
    residuals = {"reset": _subtract(right["reset_defect"], left["reset_defect"])}
    residuals.update({key: _subtract(right["runtime"][key], left["runtime"][key]) for key in (
        "pressure_term", "integration_remainder", "postintegration_change")})
    modes = _mode_diagnostic(left, delta, children, original_reference.metric_weights, residuals)
    return {"nodes": snap.nodes, "children": children, "region_indices": indices, "metric_weights": metric,
            "start_time": START, "end_time": END, "dt": DT, "delta": delta,
            "states": {key: shape(value) for key, value in delta.items()},
            "local_EN_writes": stages, "reset_budget": asdict(reset_total),
            "integration_budget": asdict(integration), "postintegration_budget": asdict(post),
            "pressure_difference": pressure, "forcing_difference": force, "pressure_work": work,
            "telescope": {"mass_change": mass, "variance_change": variance,
                          "mass_residual": mass-endpoint.mass_change, "variance_residual": variance-endpoint.variance_change},
...
def _mode_diagnostic(left, delta, children, metric, residuals):
    ...
    def mean(value, indices):
        return sum((metric[i]*value[i] for i in indices), F(0))/sum(metric[i] for i in indices)
    ...
    modes = {
        ...
        "child_centered": tuple(delta["x0"][i]-mb if i in bi else F(0) for i in range(n)),
        "parent_centered": tuple(delta["x0"][i]-mp if i in pi else F(0) for i in range(n)),
    }
    if reset._sum(modes.values(), n) != delta["x0"]:
        raise RuntimeError("fixed cohort decomposition failed")
    t = tuple(tuple(s-DT*a for s, a in zip(sr, ar, strict=True))
              for sr, ar in zip(left["S"], left["A"], strict=True))
    outputs = {key: center(reset.mv(t, value)) for key, value in modes.items()}
    outputs.update({f"residual_{key}": center(value) for key, value in residuals.items()})
    ...
    for i, name in enumerate(labels):
        for j in range(i, len(labels)):
            other = labels[j]
            inner = sum((metric[k]*a*b for k, a, b in zip(bi, outputs[name], outputs[other], strict=True)), F(0))
            gram.append({"left": name, "right": other, "inner_product": inner,
                         "energy_contribution": inner/2 if i == j else inner})
    energy = sum((row["energy_contribution"] for row in gram), F(0))
    direct = sum((metric[k]*v*v/2 for k, v in zip(bi, reconstructed, strict=True)), F(0))
    if energy != direct:
    ...
    return {"parent_mean": mp, "child_mean": mb, "T": t, "initial_modes": modes,
            "centered_child_contributions": outputs, "centered_endpoint": reconstructed,
            "gram_terms": gram, "energy": energy, "energy_residual": energy-direct,
...
def audit_saved(reset_report, recovery_report):
    ...
    for name in ("window", "native"):
        a, b = (report["historical_inputs"][name] for report in (reset_report, recovery_report))
    ...
    _equal(original.metric_weights, recovery_report["full_metric_weights"], "regional original full metric")
    ...
    if (len(nodes) != 16 or len(pairs) != 8 or any(len(pair) != 2 for pair in pairs)
    ...
    regions = [row for row in recovery_report["regions"] if row["label"] == "actual_children"]
    if len(regions) != 1:
    ...
    _equal(tuple(original.metric_weights[i] for i in indices), region["metric_weights"], "child metric")
    ...
    for time, key in ((START, "x0"), (END, "xf")):
        ...
        row = rows[0]
        for branch, field in zip(selected, ("control_epi", "perturbed_epi"), strict=True):
            _equal(tuple(F(branch["vectors"][key][i]) for i in indices), row[field], "regional endpoint EPI")
        _equal(result["states"][key], row["paired"], "regional paired endpoint readout")
        endpoints.append(row)
    return {"original_reference": reset_report["original_reference"], "actual_lineage": recovery_report["actual_lineage"],
            "common_historical_inputs": reset_report["historical_inputs"], "regional_endpoints": endpoints,
            "audit": result, "native_calls": 0, "kernel_calls": 0, "forcing_capture_calls": 0,
            "new_trajectories": 0, "autonomous_maintenance_certified": False}


def run_study(reset_path=RESET_PATH, recovery_path=RECOVERY_PATH, *,
              expected_reset_sha256=RESET_SHA256, expected_recovery_sha256=RECOVERY_SHA256):
    first, rb = reset._load(reset_path, expected_reset_sha256, "O3.b-retained-EN-AL-reset-accounting")
    second, cb = reset._load(recovery_path, expected_recovery_sha256, "O3.a-retained-regional-paired-response")
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/thol_child_distortion_audit_2026_09_18.json")
    args = parser.parse_args()
    if args.output.resolve() in (args.reset_input.resolve(), args.recovery_input.resolve()):
        raise ValueError("output must not overwrite retained inputs")
    scope = ("src/tnfr", "benchmarks/thol_child_distortion_audit.py", "benchmarks/thol_retained_reset_audit.py",
             "benchmarks/thol_family_closure.py", "benchmarks/thol_full_state_response.py",
             "benchmarks/thol_regional_balance_audit.py", "benchmarks/thol_pressure_feedback.py")
    provenance = current_git_source_provenance(ROOT, scope)
    result = run_study(args.reset_input, args.recovery_input, expected_reset_sha256=args.expected_reset_sha256,
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-retained-child-distortion", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        versions={"python": platform.python_version()}, graph_construction="No graph; two authenticated saved derivations",
        capacity_specification="Original full-support positive capacity and fixed child metric",
        solver="Exact retained paired stage ledger; no kernels or solver execution", timestep=None, seed=None,
        result_status=ClaimStatus.DERIVED, operator_sequence=("reception",),
        telemetry=("per-write paired child distortion", "held-pressure lag", "finite regional Euler budget", "complete mode Gram terms"),
        controls=("one fixed EN interval", "actual child lineage", "no new trajectories"), artifacts=(str(args.output),))
