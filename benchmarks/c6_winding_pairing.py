...
from benchmarks.c6_winding_phase_response import BASE_PHASE, _phase_readout  # noqa: E402
from benchmarks.c6_winding_rounding_cells import analyze_c6_rounding_report  # noqa: E402
...
def _stage(data):
    ...
    sigma = tuple(actual + laplacian for actual, laplacian in zip(
        capture.phase_gradient, laplacian_action(phase), strict=True,
    ))
    weights = dict(capture.normalized_weights)
    if any(capture.snapshot.capacity_gradient) or any(capture.snapshot.topology_gradient):
        raise ValueError("retained C6 pairing requires zero capacity and topology gradients")
    terms = {
        "epi_pressure": tuple(-Fraction(3, 2) * capture.epi_weight * value for value in h_epi),
        "ideal_phase_pressure": tuple(-Fraction(3, 2) * weights["phase"] * value for value in h_phase),
        "phase_realization": tuple(weights["phase"] * value for value in c6_pair_sums(sigma)),
    ...
    residual = tuple(actual - sum((term[i] for term in terms.values()), Fraction(0))
                     for i, actual in enumerate(pressure))
    epi_laplacian_residual = tuple(actual + Fraction(3, 2) * centered for actual, centered in zip(
        c6_pair_sums(capture.snapshot.epi_gradient), h_epi, strict=True,
    ))
    if any(residual) or any(epi_laplacian_residual):
        raise RuntimeError("exact C6 pressure-pair decomposition lost its identity")
    return {
        "phase_lift_radians": lifted, "phase_pi_represented": phase,
        "phase_mean_pi": sum(phase, Fraction(0)) / 6,
        "phase_pair_sums_pi": c6_pair_sums(phase), "phase_centered_pairs_pi": h_phase,
        "epi": epi, "epi_mean": sum(epi, Fraction(0)) / 6,
        "epi_pair_sums": c6_pair_sums(epi), "epi_centered_pairs": h_epi,
        "phase_gradient_pairs": c6_pair_sums(capture.phase_gradient),
        "phase_realization_error": sigma, "pressure_pairs": pressure,
        "pressure_pair_terms": terms, "pressure_pair_identity_residual": residual,
...
def _difference(after, before, factor):
    return tuple(right - factor * left for left, right in zip(before, after, strict=True))
...
def _case(retained, rounding):
    ...
    for cycle, checked_flow in zip(retained["cycles"], rounding["cycles"], strict=True):
        ...
        quotient_residual = tuple(total - il_factor * first - second
                                  for total, first, second in zip(full, ru, ri, strict=True))
        if any(quotient_residual):
            raise RuntimeError("observed phase-pair residuals lost their exact composition")
        epi_start, epi_end = stages["il_refresh"]["epi"], stages["flow_refresh"]["epi"]
        cycles.append({
            "ordinal": cycle["ordinal"], "stages": stages,
            "phase_pair_quotient": {
                "before": h0, "after_um": hu, "after_il": hi,
                "um_residual": ru, "il_residual": ri, "combined_residual": full,
            ...
            "flow_binding": {
                ...
                "starts_straddling_half_binade": min(epi_start) < Fraction(1, 2) < max(epi_start),
                "ends_straddling_half_binade": min(epi_end) < Fraction(1, 2) < max(epi_end),
            },
    ...
    return {
        "mode": retained["mode"], "epsilon": _number(retained["epsilon"]),
        "phase_pair_factors": {"um": um_factor, "il": il_factor, "combined": combined},
        "initial": initial, "cycles": cycles,
    }
...
def analyze_c6_pairing_report(parent):
    ...
    return {
        "cases": [_case(retained, rounding) for retained, rounding in zip(
            parent["cases"], validated["cases"], strict=True,
        )],
        "pair_indices": PAIR_INDICES,
        "phase_convention": {
            "represented_pi": REPRESENTED_PI, "stored_base": base,
            "scope": (
        ...
        "stored_base_reflection": {
            "pairs": reflected_pairs, "affine_targets": targets,
            "residuals": tuple(base[i] + base[j] - target for (i, j), target in zip(
                reflected_pairs, targets, strict=True,
            )),
            "scope": "Exact arithmetic on the stored base and represented pi, not a physical conjugation certificate",
        },
        "runtime_executed": False, "live_provenance_certified": False,
        "production_pairing_invariance_certified": False, "future_mean_bound_verified": False,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_pairing.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-retained-centered-pairing", git_sha=sha,
        ...
        telemetry=("centered phase/EPI pairs", "pressure-pair decomposition", "operator/refresh distinctions",
                   "conditional quotient residuals", "inherited mean and binade evidence"),
        controls=("retained null/k1/k3", "raw and centered pairing distinguished", "represented pi explicitly scoped"),
