...
def prepare_capacity_regions(case):
    ...
    for node in graph:
        graph.nodes[node].update(
            {
                ALIAS_EPI[0]: 0.5,
                ALIAS_VF[0]: 1.0 if node < 3 else 2.0,
...
def run_compatible_capacity_case(case):
    ...
    predicted_fields = all(
        (
            update.theta_after is None
            or graph.nodes[update.node][ALIAS_THETA[0]] == update.theta_after
        )
        and (
            update.vf_after is None
            or graph.nodes[update.node][ALIAS_VF[0]] == update.vf_after
        )
        and (
            update.dnfr_after is None
            or graph.nodes[update.node][ALIAS_DNFR[0]] == update.dnfr_after
        )
    ...
    predicted_edges = {
        (min(e.left, e.right), max(e.left, e.right), e.weight) for e in prediction.edges
    }
    actual_edges = {(min(u, v), max(u, v), data["weight"]) for u, v, data in new_edges}
    prediction_matches = {
        "all_proposed_fields": predicted_fields,
        "new_edges": actual_edges == predicted_edges,
        "epi_preserved": initial.snapshot.epi == raw.snapshot.epi,
        "all_requested_histories": all(
            graph.nodes[n]["glyph_history"][-1] == "UM" for n in targets
        ),
    }
    if not all(prediction_matches.values()):
        raise RuntimeError(
            "independent proposal prediction differs from actual UM writes"
        )
    ...
    event = {
        ...
        "prediction_matches_actual": prediction_matches,
        "new_edges": new_edges,
        "actual_operator_metrics": operator_metrics,
        "raw_state": raw_state,
        "raw_capture": asdict(raw),
        "after_refresh": post_um_state,
        "budget": event_budget,
        "capacity_exact_model_defect": tuple(
            a - b
            for a, b in zip(
                current.snapshot.capacity, before_support.balance.capacity_after
            )
        ),
    }
    ...
    record = {
        ...
        "post_um_capture": asdict(current),
        "final_capture": asdict(following),
        "before_support": asdict(before_support),
        "after_support": asdict(after_support),
        ...
        "initial_fixed_target_error": asdict(
            observe_forced_support_pattern(
                original,
                nodes=initial.snapshot.nodes,
                epi=initial.snapshot.epi,
            )
        ),
        "final_fixed_target_error": asdict(
            observe_forced_support_pattern(
                original,
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "artifacts/research/compatible_capacity_regions.json",
    )
    args = parser.parse_args()
    provenance = current_git_source_provenance(ROOT, SOURCE_SCOPE)
    sha, dirty, digest = provenance
    manifest = CoreExperimentManifest(
        claim_id="O3.a-U3-compatible-capacity-regions",
        git_sha=sha,
        source_dirty=dirty,
        ...
        operator_sequence=(
            "all-target UM",
            "one held Euler interval",
            "all-target SHA",
        ),
        telemetry=(
            "U3 and pressure supports",
            "exact capacity balance",
            "actual phase/capacity/links",
            "fixed profile and event/flow defects",
            "tetrad/coherence",
        ),
        controls=(
            "prepared split phases",
            "aligned phases with default functional links",
        ),
