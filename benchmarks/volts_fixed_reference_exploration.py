...
def _prefix(run):
    return NodalMeasurementRun(
        run_id=run.run_id, acquisition_id=run.acquisition_id,
        channel_ids=run.channel_ids, timestamps=run.timestamps[:25],
        samples=(run.samples[0][:25],), value_unit=run.value_unit,
...
def build_continuation(prefix, timestamps):
    ...
    if (len(prefix.timestamps) != 25 or prefix.channel_ids != ("Voltage",)
    ...
    rate = bound_fixed_reference_capacity(
        (start, start), (end, end), reference=(0, 0),
        elapsed_time=(duration, duration)).capacity
    ...
    tube = bound_fixed_reference_transport(
        (end, end), reference=(0, 0), capacity=rate,
    ...
    return {
        ...
        "run_id": prefix.run_id, "acquisition_id": prefix.acquisition_id,
        "timestamps": times, "nominal_reference": 0,
        ...
        "affine_ar1": baseline, "affine_ar1_status": reason,
        "sensor_uncertainty": None, "clock_uncertainty": None,
        "physical_acceptance_threshold": None,
        "split_scope": "within_single_acquisition",
        "physical_status": "not_admitted", "measurement_verdict": "not_assessed",
...
def score_continuation(run, forecast, *, expected_forecast_hash):
    ...
    def errors(values):
        ...
        with np.errstate(over="raise", invalid="raise"):
            ...
            return {"rmse_volts": float(np.sqrt(np.mean(residual**2))),
                    "max_absolute_error_volts": float(np.max(np.abs(residual))),
                    "signed_error_volts": tuple(float(x) for x in residual)}

    return {
        "forecast_hash": expected_forecast_hash, "observation_hash": run.content_hash,
...
def execute_study(data_path, *, spec_path, protocol_path,
                  expected_spec_hash, output):
    ...
    for source, name, expected in (
        ...
        safe_write(destination / name, lambda stream, p=source:
                   stream.write(Path(p).read_bytes()), mode="wb", encoding=None)
    ...
    try:
        ...
        result = {"status": "specified_model_unavailable", "reason": str(exc),
    ...
    _write_json(destination / "forecast.json", {
    ...
    names = ("Volts.rda", "spec.json", "protocol.md", "ingestion.json",
             "forecast.json", "result.json")
    manifest = CoreExperimentManifest(
        ...
        controls=("persistence", "prefix-only affine AR-1"), artifacts=names,
    )
    sidecar = EvidenceSidecar(
        ...
        provenance={"uses_future_samples": False,
                    "uses_outcome_derived_wiring": False,
                    "fits_on_evaluation_data": False,
                    "uses_evaluation_labels": False, "uses_postselection": False},
        claim_statement="One predeclared within-trace continuation was computed and compared descriptively",
        claim_status="measured", scope="exploratory; physical admission missing",
        assumptions=("unverified zero reference and pure-EPI observation map",
        ...
        graph_context={"nodes": ["Voltage", "reference"], "edges": [[0, 1]],
                       "support_status": "hypothetical; source wiring unverified",
                       "active_pressure_channel": "EPI only"},
        state_context={"initial_voltage": run.samples[0][24],
                       "reference": 0, "reference_capacity": 0,
                       "prefix_hash": forecast["prefix_hash"],
                       "nominal_capacity_display": forecast["capacity_display"]},
        observation_context={"split_scope": spec["split_scope"],
        ...
        cost_context={"record_rows": 50, "calibration_rows": 25,
                      "continuation_rows": 25, "automatic_refits": 0},
        artifact_hashes={
...
def main():
    ...
    result = execute_study(args.data, spec_path=args.spec,
