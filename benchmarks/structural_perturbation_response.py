...
def _word(operators, *, initialized):
    ...
    record = {
        "names": names,
        "context": context,
        "string_validator_passed": validation.passed,
    }
    try:
...
def _event(graph, child, operator, sequence_step, target, before_capture, *, refresh):
    ...
    record = {
        "target": child,
        "requested_name": operator.name,
        "requested_glyph": operator.glyph.value,
        "sequence_index": sequence_step.index,
        "admission": _plain(asdict(admission)),
        "before": before,
        "resolved_factors": _plain(
            resolve_runtime_operator_factors(
    ...
    try:
        ...
        if isinstance(error, ValueError) and not str(error).startswith(
            "IL precondition failed:"
        ):
            raise
        record.update(
            ...
            after_failure=_materialized(graph),
        )
        return before_capture, record
    ...
    original_before, original_after = _pattern(target, before_capture), _pattern(
        target, following
    )
    record.update(
        status="executed",
        raw_after_event=raw,
        after_refresh=_materialized(graph),
        actual_operator_metrics=_plain(
            graph.graph.get("operator_metrics", ())[metrics_start:]
        ),
        before_forcing_capture=asdict(before_capture),
        raw_forcing_capture=asdict(raw_capture),
        after_forcing_capture=asdict(following),
        before_reference=asdict(before_reference),
        after_reference=asdict(after_reference),
        before_compatibility_channels=_compatibility_channels(
            before_reference, before_capture
        ),
        after_compatibility_channels=_compatibility_channels(
            after_reference, following
        ),
        full_event_budget=observation,
        original_pattern_before=asdict(original_before),
        original_pattern_after=asdict(original_after),
        original_pattern_change=original_after.error_variance
        - original_before.error_variance,
    )
...
def _checkpoint(record):
    return {
        ...
        "case": record["case"],
        "retained_endpoint": record["final_before_closure"],
...
def run_structural_response_case(case):
    ...
    words = {
        "child": {
            "target": child,
            "retained_prefix": initial["state"]["glyph_history"][child],
            **child_word_record,
        },
        "parent": {
            "target": 0,
            "retained_prefix": initial["state"]["glyph_history"][0],
            **parent_word_record,
        },
    }
    record = {
        "case": case,
        "checkpoint": _checkpoint(prefix),
        "initial": initial,
        "original_target": asdict(target),
        "initial_reference": asdict(initial_reference),
        "initial_original_pattern": asdict(initial_pattern),
        "words": words,
    ...
    if child_word is None or parent_word is None:
        record.update(
            status="controlled_obstruction", reason="full-word validation refused"
        )
        return record
    if words["child"]["retained_prefix"] != ("UM",) or words["parent"][
        "retained_prefix"
    ] != (
        "IL",
        "OZ",
        ...
        raise RuntimeError(
            "the retained target histories differ from the declared word prefixes"
        )
    ...
    pattern_event_change = sum(
        (item["original_pattern_change"] for item in record["events"]), Fraction(0)
    )
    pattern_flow_change = sum(
        (
            item["original_pattern_after"]["error_variance"]
            - item["original_pattern_before"]["error_variance"]
    ...
    regime_flow_change = sum(
        (
            ...
            - item["regime_step_budget"]["before"]["error_variance"]
            for item in segments
        ),
        Fraction(0),
    )
    budget = {
        ...
        "original_pattern_total_change": final_pattern.error_variance
        - initial_pattern.error_variance,
        "original_pattern_identity_residual": (
            final_pattern.error_variance
            - initial_pattern.error_variance
            - pattern_event_change
            - pattern_flow_change
        ),
        "current_regime_event_change": regime_event_change,
        "current_regime_flow_change": regime_flow_change,
        "current_regime_total_change": final_regime.error_variance
        - initial_regime.error_variance,
        "current_regime_identity_residual": (
            final_regime.error_variance
            - initial_regime.error_variance
            - regime_event_change
            - regime_flow_change
        ),
    ...
    if (
        budget["original_pattern_identity_residual"]
        or budget["current_regime_identity_residual"]
    ):
    ...
    record.update(
        ...
        finite_telescope=budget,
        physical_elapsed_time=final["state"]["time"] - initial["state"]["time"],
    )
    closures = []
    for node, word, ops in (
        (child, child_word, child_ops),
        (0, parent_word, parent_ops),
    ):
...
def compare_structural_response(cases):
    ...
    if any(item["status"] != "measured" for item in cases):
        return {
            "status": "controlled_obstruction",
            "scope": "No recovery classification is available",
        }
    u, p, f = cases
    matches = {
        "common_initial_materialized_state": u["initial"]
        == p["initial"]
        == f["initial"],
        "common_original_target": u["original_target"]
        == p["original_target"]
        == f["original_target"],
        "matched_perturbed_pre_feedback_state": p["pre_feedback_materialized"]
        == f["pre_feedback_materialized"],
        "matched_VAL_IL_events": p["events"] == f["events"][:2],
        "matched_elapsed_time": all(
            item["physical_elapsed_time"] == STEP * SEGMENT_COUNT for item in cases
        ),
    }
...
def main():
    ...
    scope = (
        ...
        "benchmarks/canonical_winding_persistence.py",
        ...
        "benchmarks/structural_perturbation_response.py",
    )
    ...
    manifest = CoreExperimentManifest(
        ...
        operator_sequence=(
            "parent IL OZ THOL UM",
            "child UM [VAL IL [UM]]",
            "separate final SHA",
        ),
        telemetry=(
            "full event/reference budgets",
            "fixed target damage/benefit/gap",
            "matched state and RNG provenance",
            "tetrad and executor endpoints",
        ),
