...
def _bind_source_edges(state, snapshot):
    ...
    for left, right, attributes in state["edges"]:
        if left not in positions or right not in positions or left == right:
            raise ValueError("retained source must have simple nonloop edges on its nodes")
        i, j = positions[left], positions[right]
        _, weight = finite_represented_real(attributes.get("weight", 1.0), "retained source conductance")
        ...
        support[i].add(j)
        support[j].add(i)
    _equal(tuple((i, j, w) for (i, j), w in sorted(edges.items())), snapshot.conductance, "live full conductance")
    _equal(tuple(tuple(sorted(row)) for row in support), snapshot.support_neighbors, "live full support")
...
def _ancestry(prior, state, nodes):
    ...
    if (len(set(parents+children)) != 16 or parents+children != nodes
    ...
    for key, expected in (("parent_children", pairs), ("parents", parents), ("children", children), ("nodes", nodes)):
    ...
    for row in (birth["after"], state, prior["common_source"]["state"]):
        if tuple(row["nodes"]) != nodes or row["hierarchy"] != hierarchy or row["children"] != child_map:
            raise ValueError("retained state hierarchy differs from the actual birth pairs")
    ...
    for (parent, child), receipt in zip(pairs, receipts, strict=True):
        if (attributes[parent].get("sub_nodes") != [child]
...
def _component_budget(balance, values):
    ...
    variance = sum((balance.strengths[i]*z*values[i]
                    for i, z in zip(indices, balance.centered_epi, strict=True)), F(0))
...
def _region_report(snapshot, observation, components, region, label):
    ...
    if (sum((row["weighted_total_rate"] for row in channels.values()), F(0)) != balance.mass_forcing_rate
            or sum((row["variance_rate"] for row in channels.values()), F(0)) != balance.variance_forcing_rate
            or kernel["weighted_total_rate"]+stored["weighted_total_rate"] != balance.mass_defect_rate
            or kernel["variance_rate"]+stored["variance_rate"] != balance.variance_defect_rate):
        raise RuntimeError("regional channel or pressure-defect decomposition failed")
    return {"label": label, "balance": asdict(balance), "forcing_channels": channels,
            "fresh_kernel_defect": kernel, "stored_minus_fresh_residual": stored,
            "fresh_kernel_weighted_total_rate": balance.model_mass_rate+kernel["weighted_total_rate"],
            "fresh_kernel_variance_rate": balance.model_variance_rate+kernel["variance_rate"],
...
def _held_parent_response(snapshot, observation, parents, children):
    ...
    if any(j in child_indices for i in child_indices for j in snapshot.support_neighbors[i]):
        raise ValueError("conditional independent-child response requires no child-child support edges")
    rows = []
    for child in children:
        i = lookup[child]
        edges = tuple((j, weight) for source, j, weight in snapshot.conductance if source == i)
        degree = sum((weight for _, weight in edges), F(0))
        if degree <= 0 or any(j not in parent_indices for j, _ in edges):
            raise ValueError("every child must receive positive conductance only from actual parents")
        weights = tuple((snapshot.nodes[j], weight/degree) for j, weight in edges)
        neighbor_mean = sum((weight*snapshot.epi[j] for j, weight in edges), F(0))/degree
        ...
        pressure_residual = observation.epi_weight*(neighbor_mean-equilibrium)+observation.forcing[i]
        represented_model_rate = snapshot.capacity[i]*(observation.epi_weight*snapshot.epi_gradient[i]+observation.forcing[i])
        affine_identity = represented_model_rate+rate*(snapshot.epi[i]-equilibrium)
        if pressure_residual or affine_identity or rate <= 0 or sum((w for _, w in weights), F(0)) != 1:
            raise RuntimeError("conditional held-parent child response identity failed")
        rows.append({"child": child, "parent_weights": weights, "full_strength": degree,
                     "parent_neighbor_mean": neighbor_mean, "forcing_offset": force_offset,
                     "conditional_equilibrium_epi": equilibrium, "relaxation_rate": rate,
                     "current_epi": snapshot.epi[i], "model_nodal_rate": represented_model_rate,
                     "equilibrium_pressure_residual": pressure_residual, "affine_rate_identity_residual": affine_identity})
    return {"no_child_child_support_edges": True, "rows": rows,
...
def audit_prior(prior):
    ...
    if F(state["time"]) != F(1, 2):
        raise ValueError("regional source must be the first post-attachment t=0.5 snapshot")
    ...
    _equal(asdict(reference.source), asdict(snapshot), "original reference source snapshot")
    if (reference.epi_weight != observation.epi_weight or reference.forcing != observation.forcing
            or prior["original_reference_sha256"] != _digest(reference)):
        raise ValueError("original reference digest or forcing differs from the actual source")
    ...
    regions = [_region_report(snapshot, observation, components, pair, f"ancestry_pair_{i}")
               for i, pair in enumerate(pairs)]
    regions.append(_region_report(snapshot, observation, components, children, "actual_children"))
    response = _held_parent_response(snapshot, observation, parents, children)
    return {"source_time": F(1, 2), "source_state": state, "source_capture": raw_capture,
            "original_reference": asdict(reference), "actual_lineage": {"pairs": pairs, "parents": parents, "children": children},
            "regions": regions, "child_cohort_response": response, "snapshot_count": 1, "regional_observation_count": 9,
            "native_calls": 0, "new_trajectories": 0, "partition_search": False,
...
def load_evidence(path=INPUT_PATH, *, expected_sha256=INPUT_SHA256):
    ...
    if (tuple(row["branch"] for row in report["replayed_prior_reports"]) != ("control", "child_emission")
            or tuple(row["branch"] for row in report["branches"]) != ("control", "child_emission")):
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/thol_regional_balance_audit_2026_09_18.json")
    args = parser.parse_args()
    if args.output.resolve() == args.input.resolve():
        raise ValueError("output must not overwrite the retained input")
    scope = ("src/tnfr", "benchmarks/thol_regional_balance_audit.py", "benchmarks/thol_retained_reset_audit.py",
             "benchmarks/thol_family_closure.py", "benchmarks/thol_distributed_target.py", "benchmarks/thol_pressure_feedback.py")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O1.b-retained-THOL-regional-balance", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        versions={"python": platform.python_version()}, graph_construction="One retained full post-attachment graph snapshot",
        capacity_specification="Actual positive capacities; full graph degrees and boundary couplings",
        solver="Nine exact instantaneous regional observations; no integration", timestep=None, seed=None,
        result_status=ClaimStatus.DERIVED, operator_sequence=("self_organization", "coupling"),
        telemetry=("regional weighted-total and variance budgets", "source channels", "conditional child response"),
        controls=("pinned input", "actual ancestry", "one snapshot", "no induced normalization"), artifacts=(str(args.output),))
