...
from benchmarks.c6_winding_relay_handoff import (  # noqa: E402
    HISTORICAL_NAME,
    INPUT_NAME as RELAY_NAME,
    replay_c6_relay_handoff_evidence,
)
...
def _profile_cube(profile, state):
    ...
    rows = tuple(
        tuple(pair[int(bool(mask & (1 << i)))] for i, pair in enumerate(pairs))
...
def _quadratic_control(reference, rows, pairs, timestep):
    ...
    areas = tuple(
        tuple(
            F(timestep) * F(p)
            for p in _observe_rebuilt_c6_pressure_lattice(reference, row).pressure
        )
        for row in rows
    )
    ...
    base = vectors[0]
    matrix = tuple(
        tuple(base[i] - vectors[1 << j][i] for j in range(6)) for i in range(6)
    )
    ...
    if any(value <= 0 for value in metric) or not all(
        metric[i] * matrix[i][j] == metric[j] * matrix[j][i]
        for i in range(6)
        for j in range(6)
    ):
        raise ValueError(
            "this exact affine control requires its derived diagonal symmetrizer"
        )
    ...
    errors = tuple(
        tuple(
            ...
            - base[i]
            + sum(
                (matrix[i][j] for j in range(6) if mask >> j & 1),
                F(0),
            )
            for i in range(6)
        )
        for mask, row in enumerate(vectors)
    )
    ...
    directions = tuple(
        tuple(F(int(i == j) - int(i == 5)) for i in range(6)) for j in range(5)
    )
    matrices, trace = [], []
    for i in range(5):
        for j in range(i, 5):
    ...
    for direction in directions:
        matrices.append(
            tuple(
                tuple(direction[a] + direction[b] for b in range(6)) for a in range(6)
            )
        )
        trace.append(F(0))
    trace.append(F(0))
    ...
    for index in support:
        ...
        constraints.append(
            tuple(
                sign
                * sum((value * p for value, p in zip(mat[coordinate], row)), F(0))
                / scale
                for mat in matrices
            )
            + (sign * sum(row, F(0)),)
    ...
    system = tuple(
        tuple(row[column] for row in constraints) + (-trace[column],)
    ...
    coefficients = tuple(
        sum((weight * row[column] for weight, row in zip(weights, constraints)), F(0))
        for column in range(21)
    )
    if (
        not all(value > 0 for value in weights)
        or multiplier <= 0
        or sum(weights) != 1
        or coefficients != tuple(multiplier * value for value in trace)
    ):
        raise ValueError(
            "the proposed common-quadratic dual certificate does not verify"
        )
    return {
        "exact_nodal_areas": areas,
        "increment_unit": unit,
        "affine_matrix": matrix,
        "affine_equilibrium_switches": alpha,
        "symmetrizing_diagonal": tuple(metric),
        "negative_direction": tuple(min(widths) / value for value in widths),
        "negative_direction_image": image,
        "negative_quadratic_value": quadratic,
        "mixed_remainders": errors,
        "mixed_bounds": tuple(
            (min(row[i] for row in errors), max(row[i] for row in errors))
            for i in range(6)
        ),
        "positive_dual_support": tuple(divmod(index, 6) for index in support),
        "positive_dual_weights": weights,
        "positive_trace_multiplier": multiplier,
        "dual_identity_residual": tuple(
            a - multiplier * b for a, b in zip(coefficients, trace)
        ),
        "all_orthant_inward_common_quadratic_criterion_excluded": True,
        "all_quadratic_invariants_excluded": False,
        "piecewise_potentials_excluded": False,
...
def analyze_c6_winding_invariant_region(
    ...
    *,
    ...
    return {
        ...
        "new_conditional_trajectory_steps": 0,
    ...
    *,
    ...
    return {
        "contract": {
            ...
            "resource_limit_is_physical_parameter": False,
            "pressure_projected_or_carry_reset": False,
        },
        "source": {
            "retained_B47_endpoint": asdict(state),
        ...
        "new_conditional_trajectory_steps": len(result.entry_steps),
        "new_live_graph_steps": 0,
        "indefinite_trapping_certified": result.conditional_boundedness_certified,
        "whole_band_exit_certified": False,
        "future_runtime_certified": False,
...
def _rebuild_c6_forward_domain(
    parent, *, relay_bytes, historical_bytes, envelope_bytes
):
    ...
    if (
        retained.get("source_scope") != list(SOURCE_SCOPE)
        or retained.get("manifest", {}).get("claim_id")
        != "O3.a-C6-carried-forward-envelope"
    ):
        raise ValueError(
            "the predecessor audit requires the retained B49 forward-envelope report"
        )
    ...
    if any(retained.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B49 envelope differs from its complete canonical reconstruction"
        )
    profile, state = replay_c6_relay_handoff_evidence(
        parent,
        relay_bytes=relay_bytes,
    ...
    zones = tuple(
        C6CarriedForwardZone(item["epi"], item["bounds"])
        for item in envelope["retained_zones"]
    )
    source_by_row = {zone.epi: zone.bounds for zone in zones}
    protected_by_row = {
        item["epi"]: item["bounds"] for item in envelope["initial_zones"]
    }
    ...
    for row, pressure in zip(rows, envelope["pressures"], strict=True):
        ...
        translated = tuple(
            tuple(zone[i][j] + int(added[i] - added[j]) for j in range(7))
            for i in range(7)
        )
        for target_row, cell in protected_by_row.items():
            if not _dbm_subset(
                _dbm_intersection(translated, cell), source_by_row[target_row]
            ):
                raise RuntimeError(
                    "the retained envelope lost its exact clipped forward inclusion"
                )
...
def analyze_c6_winding_temporal_predecessors(
    ...
    *,
    ...
    rows, grid, origin = (
        envelope["epi_states"],
        envelope["grid_quantum"],
        state.exact_epi,
    )
    selected = tuple(
        item
        for item in validation["outgoing_slab_sum_projections"]
        if item.get("hypothetical_original_mean_affine_coset_witness") is not None
    )
    if not selected:
        raise ValueError(
            "the predecessor audit requires nonempty exact target witnesses"
        )
    records = []
    identities = set()
    for item in selected:
        ...
        if (
            ...
            or not 0 <= mask < len(rows)
            or type(node) is not int
            or not 0 <= node < 6
            or direction not in ("lower", "upper")
            or type(indices) is not list
            or len(indices) != 6
            or any(type(n) is not int for n in indices)
        ):
            raise ValueError(
                "each witness requires a valid row, facet and six exact integer indices"
            )
        ...
        if sum(exact) != sum(origin) or tuple(map(float, exact)) != rows[mask]:
            raise ValueError(
                "the selected point must preserve the exact original mean and its RN row"
            )
        target = NodalRemainderState(
            rows[mask],
            tuple(x - F(visible) for x, visible in zip(exact, rows[mask], strict=True)),
            state.epi_lower,
            state.epi_upper,
        )
        outgoing = advance_nodal_remainder(
            ...
            pressure=_observe_rebuilt_c6_pressure_lattice(
                profile.lattice, target.epi
            ).pressure,
        )
        edge = (min if direction == "lower" else max)(row[node] for row in rows)
        leaves_facet = (
            outgoing.after.epi[node] < edge
            if direction == "lower"
            else outgoing.after.epi[node] > edge
        )
        if not leaves_facet:
            raise ValueError(
                "the selected point does not leave its declared cube facet"
            )
        domains = {}
        for label, domain in (("retained_R259", zones), ("original_cube", None)):
            proof = derive_c6_carried_predecessors(
                ...
                domain_zones=domain,
                max_depth=max_depth,
                max_row_checks=max_row_checks,
            )
            payload = asdict(proof)
            payload.pop("reference")
            payload.update(
                completed_depth=proof.completed_depth,
                frontier_counts=proof.frontier_counts,
        ...
        records.append(
            dict(
                ...
                node=node,
                direction=direction,
                grid_indices=tuple(indices),
                target=asdict(target),
                outgoing_step=asdict(outgoing),
                domains=domains,
    ...
    return {
        "contract": {
            ...
            "computational_max_depth": max_depth,
            "computational_max_row_checks": max_row_checks,
            "resource_limits_are_physical_parameters": False,
        },
        "source": fresh["source"],
        "B50_B49_reconstruction": dict(
            complete_envelope_rebuilt=True,
            completed_layers=259,
            outgoing_facets=56,
...
def _rebuild_c6_outgoing_regions(
    parent, *, relay_bytes, historical_bytes, envelope_bytes
):
    ...
    for mask, (row, area) in enumerate(zip(rows, areas, strict=True)):
        ...
        for node in range(6):
            for direction in ("lower", "upper"):
                slab = _c6_unsafe_slab(
                    by_row[row], node, direction, lower, upper, shift
                )
                if slab is not None:
    ...
    geometry = dict(
        ...
        complete_RN_cells=tuple(asdict(cell) for cell in rn_cells),
    )
...
def analyze_c6_winding_region_exclusions(
    ...
    *,
    ...
    fresh, profile, state, zones, geometry, facets, targets = (
        _rebuild_c6_outgoing_regions(
            parent,
            relay_bytes=relay_bytes,
            historical_bytes=historical_bytes,
            envelope_bytes=envelope_bytes,
    ...
    for payload, query in zip(proof["queries"], result.queries, strict=True):
        payload.update(
            ...
            past_exclusion_depth=query.past_exclusion_depth,
    ...
    for facet, query in zip(facets, result.queries, strict=True):
        ...
        group = groups.setdefault(
            key, dict(node=key[0], direction=key[1], total=0, excluded=0)
        )
        group["total"] += 1
        group["excluded"] += int(query.origin_path_within_domain_excluded)
    complete = bool(facets) and all(
        query.origin_path_within_domain_excluded for query in result.queries
    )
    return {
        "contract": {
...
def _c6_poisson_contrast_weights():
    ...
    laplacian = tuple(
        tuple(
            F(int(i == j)) - F(int(j in ((i - 1) % 6, (i + 1) % 6)), 2)
            for j in range(6)
        )
        for i in range(6)
    )
    ...
    inverse = exact_matrix_inverse(matrix)
    rational = tuple(
        sum(a * b for a, b in zip(row, rhs, strict=True)) for row in inverse
    )
    ...
    image = tuple(
        sum(a * b for a, b in zip(row, weights, strict=True)) for row in laplacian
    )
    if sum(weights) or image != tuple(
        F(3, 2) * (int(i == 2) - int(i == 0)) for i in range(6)
    ):
...
def _analyze_c6_excursion_context(
    ...
    *,
    ...
    if (
        len(prior_inputs) != 4
        or tuple(item["sha256"] for item in prior_inputs[1:]) != expected_hashes
    ):
        raise ValueError("the regional evidence has different historical inputs")
    if (
        prior.get("source") != _payload(fresh["source"])
        or prior.get("B51_cube_geometry") != _payload(geometry)
        or prior.get("B51_outgoing_facets") != _payload(facets)
    ):
        raise ValueError(
            "the regional evidence does not identify the rebuilt origin and whole exit cover"
        )
    ...
    rechecked = derive_c6_carried_region_exclusions(
        ...
        epi_states=envelope["epi_states"],
        timestep=envelope["timestep"],
        target_regions=tuple(targets[index] for index in selected),
        domain_zones=zones,
    )
    for index, query in zip(selected, rechecked.queries, strict=True):
        ...
        if (
            ...
            or _payload(public) != old_queries[index]
        ):
            raise ValueError(
                "a claimed prior regional exclusion failed exact reconstruction"
            )
    rows = envelope["epi_states"]
    ...
    result = derive_c6_carried_excursion_exclusion(
        ...
        epi_states=rows,
        timestep=envelope["timestep"],
        active_epi_states=active,
        weights=_c6_poisson_contrast_weights(),
    ...
    proof.pop("reference")
    proof.update(
        origin_path_within_domain_excluded=result.origin_path_within_domain_excluded,
    ...
    return {
        ...
        "B52_proof_coordinate_derivation": dict(
            weights=_c6_poisson_contrast_weights(),
            zero_mean=True,
            primitive_integer_solution=True,
            laplacian="unit-C6 random-walk Laplacian",
...
def _cut_c6_proven_facets(zones, rows, shifts, geometry, facets):
    ...
    for facet in facets:
        ...
        if closed is None:
            ...
            domain[row] = closed
    return tuple(
        C6CarriedForwardZone(row, domain[row]) for row in rows if row in domain
    )
...
def _c6_public_exact_proof(proof):
    ...
    for key in (
        "domain_confined_paths_covered",
        "clipped_forward_inclusion_certified",
        "origin_path_within_domain_excluded",
        "actual_target_reached",
        "observed_prefix_steps",
        "common_grid_relaxes_coordinate_cosets",
...
def _c6_mode_candidates(raw, facets, rows):
    ...
    if (
        ...
        or data.get("schema") != "c6-mode-excursion-candidates-v1"
    ):
    ...
    for item in candidates:
        ...
        if (
            ...
            or set(target) != {"mask", "node", "direction"}
            or type(target["mask"]) is not int
            or type(target["node"]) is not int
            or target not in facets
        ):
            raise ValueError(
                "each mode candidate must identify a complete original unsafe slab"
            )
        ...
        if (
            ...
            or not active
            or any(
                type(mask) is not int or not 0 <= mask < len(rows) for mask in active
            )
            or len(set(active)) != len(active)
            or target["mask"] not in active
        ):
            raise ValueError(
                "mode candidates require distinct targets and valid active masks"
            )
        seen.add(index)
        ...
        if (
            ...
            or len(offsets) != len(active)
            or any(type(value) is not str for value in weights + offsets)
        ):
            raise ValueError(
                "mode weights and offsets must be complete exact rational string arrays"
            )
        try:
            values = tuple(F(value) for value in weights + offsets)
        except (ValueError, ZeroDivisionError) as exc:
            raise ValueError(
                "mode proof coordinates must be exact finite rationals"
            ) from exc
        ...
        output.append(
            (index, tuple(active), integers[:6], integers[6:], F(scale, divisor))
        )
    return tuple(output)
...
def analyze_c6_winding_mode_excursions(
    ...
    *,
    ...
    if parent != json.loads(parent_bytes):
        raise ValueError(
            "the mode audit requires the unchanged retained B47 input bytes"
        )
    blobs = (
        parent_bytes,
        relay_bytes,
    ...
    for position in (3, 4, 5):
        ...
        if tuple(item.get("sha256") for item in evidence) != expected:
            raise ValueError(
                "the mode audit lineage does not bind all retained historical input bytes"
            )
    prior = json.loads(excursion_bytes)
    if prior.get("manifest", {}).get(
        "claim_id"
    ) != "O3.a-C6-carried-excursion-exclusion" or prior.get("source_scope") != list(
        SOURCE_SCOPE
    ):
    ...
    if any(prior.get(key) != value for key, value in _payload(old).items()):
        raise ValueError(
            "the retained B52 excursion differs from its complete canonical reconstruction"
        )
    envelope = fresh["B49_relational_forward_envelope"]
    rows, grid, h = (
        envelope["epi_states"],
    ...
    shifts = tuple(
        tuple(F(h) * F(p) / grid for p in pressure)
    ...
    cut_domain = _cut_c6_proven_facets(
        ...
        shifts,
        geometry,
        tuple(facets[index] for index in sorted(prior_proved)),
    )
    ...
    for index, active_masks, weights, offsets, scale in candidates:
        pieces = tuple(
            C6CarriedForwardZone(zone.epi, intersection)
            for zone in targets[index]
            if zone.epi in by_row
            and (intersection := _dbm_intersection(zone.bounds, by_row[zone.epi]))
            is not None
        )
        record = dict(
            target=facets[index],
            normalization_scale=scale,
            target_regions=tuple(asdict(z) for z in pieces),
        )
        if not pieces:
            record.update(
                status="empty_reachable_target",
                origin_path_within_domain_excluded=True,
                proof=None,
            )
            proved.add(index)
        else:
            ...
            offset_pairs = tuple(
                (rows[mask], offset)
                for mask, offset in zip(active_masks, offsets, strict=True)
            ...
            proof = derive_c6_carried_mode_excursion_exclusion(
                ...
                active_epi_states=active,
                weights=weights,
                cell_offsets=offset_pairs,
                target_regions=pieces,
                domain_zones=reachable.retained_zones,
                max_prefix_steps=max_prefix_steps,
            )
            record.update(
                status=proof.status,
    ...
    post_domain = _cut_c6_proven_facets(
        reachable.retained_zones,
        rows,
        shifts,
        geometry,
        tuple(facets[index] for index in sorted(additional)),
    )
    post = derive_c6_carried_reachable_envelope(
        ...
        epi_states=rows,
        timestep=h,
        domain_zones=post_domain,
        max_intersections=max_intersections,
    )
    final = {zone.epi: zone.bounds for zone in post.retained_zones}
    absent = {
        index
        for index, regions in enumerate(targets)
        if all(
            zone.epi not in final
            or _dbm_intersection(zone.bounds, final[zone.epi]) is None
            for zone in regions
        )
    }
    ...
    return {
        "contract": {
            ...
            "max_intersections_per_closure": max_intersections,
            "max_prefix_steps_per_candidate": max_prefix_steps,
            "resource_limits_are_physical_parameters": False,
            "pressure_projected_or_carry_reset": False,
        },
        "source": fresh["source"],
        "B53_reconstruction": dict(
            complete_envelope_rebuilt=True,
            prior_B52_completely_rebuilt=True,
            ...
            prior_excursion_prefix_steps_replayed=old[
                "new_conditional_trajectory_steps"
            ],
            prior_regional_verification=old["B52_reconstruction"],
        ),
        "B53_cube_geometry": geometry,
        "B53_outgoing_facets": facets,
        "B53_prior_proven_facets": tuple(
            facets[index] for index in sorted(prior_proved)
        ),
        ...
        "B53_geometrically_absent_facets": tuple(
            facets[index] for index in sorted(absent)
        ),
        "B53_exit_groups": tuple(groups.values()),
        "B53_remaining_first_exit_facets": tuple(
            facet for index, facet in enumerate(facets) if index not in proved
        ),
        "excluded_first_exit_slabs": len(proved),
        "additional_first_exit_slabs_excluded": len(proved - prior_proved),
        "all_first_exit_slabs_excluded": len(proved) == len(facets),
        "primary_proof_origin_remains_B47": True,
        "new_conditional_trajectory_steps": new_steps,
        "new_live_graph_steps": 0,
        "indefinite_trapping_certified": False,
        "whole_band_exit_certified": False,
        "future_runtime_certified": False,
...
def analyze_c6_winding_return_regions(
    ...
    *,
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-mode-excursions"
        ...
        raise ValueError(
            "the return audit requires the complete retained B53 input lineage"
        )
    fresh = analyze_c6_winding_mode_excursions(
        ...
        envelope_bytes=envelope_bytes,
        region_bytes=region_bytes,
        excursion_bytes=excursion_bytes,
    ...
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B53 result differs from its complete canonical reconstruction"
        )
    ...
    channels = dict(
        json.loads(historical_bytes)["B43_original_phase_tail_runtime"]["source"][
            "normalized_weights"
        ]
    )
    reference = derive_c6_pressure_lattice(
        phase=tuple(fresh["source"]["phase"]),
        epi_weight=float(F(channels["epi"])),
        phase_weight=float(F(channels["phase"])),
        epi_lower=state.epi_lower,
        epi_upper=state.epi_upper,
    )
    domain = tuple(
        C6CarriedForwardZone(tuple(z["epi"]), tuple(map(tuple, z["bounds"])))
        for z in previous["retained_zones"]
    )
    by_row = {z.epi: z.bounds for z in domain}
    geometry = fresh["B53_cube_geometry"]
    shifts = tuple(
        tuple(F(previous["timestep"]) * F(p) / previous["grid_quantum"] for p in row)
    ...
    result = derive_c6_carried_return_region_exclusions(
        ...
        domain_zones=domain,
        target_region_groups=tuple(targets),
        max_intersections=max_intersections,
    ...
    for group in groups:
        group["excluded"] += sum(
            item["node"] == group["node"] and item["direction"] == group["direction"]
            for item in additional
        )
    envelope = _c6_public_exact_proof(result.return_envelope)
    for key in (
        "domain_confined_origin_paths_covered",
        "clipped_return_inclusion_certified",
        "intersections",
    ):
        envelope[key] = getattr(result.return_envelope, key)
    return dict(
        contract=dict(
            question="Can exact short-return predecessors exclude complete pending first-exit regions?",
            transient_selection="Upper represented node-2 value, with no internal transition",
            max_intersections=max_intersections,
            query_max_intersections=query_max_intersections,
            intermediate_exit_gates_preserved=True,
            physical_dynamics_changed=False,
        ),
        source=fresh["source"],
        B54_reconstruction=dict(
            prior_B53_completely_rebuilt=True,
            prior_mode_sha256=hashlib.sha256(mode_bytes).hexdigest(),
            prior_first_exit_slabs_reverified=fresh["excluded_first_exit_slabs"],
            prior_excursion_prefix_steps_replayed=fresh["B53_reconstruction"][
                "prior_excursion_prefix_steps_replayed"
            ],
        ),
        B54_cube_geometry=geometry,
        B54_outgoing_facets=fresh["B53_outgoing_facets"],
        B54_return_envelope=envelope,
        B54_region_queries=tuple(records),
        B54_query_intersections=result.intersections,
        B54_exit_groups=tuple(groups),
        B54_additional_first_exit_facets=tuple(additional),
        B54_remaining_first_exit_facets=remaining,
        excluded_first_exit_slabs=fresh["excluded_first_exit_slabs"] + len(additional),
        additional_first_exit_slabs_excluded=len(additional),
        all_first_exit_slabs_excluded=not remaining,
        primary_proof_origin_remains_B47=True,
        new_conditional_trajectory_steps=0,
        new_live_graph_steps=0,
        indefinite_trapping_certified=False,
...
def analyze_c6_winding_return_unions(
    ...
    *,
    ...
    max_intersections=250_000,
    query_max_intersections=500_000,
    query_max_subsumptions=1_000_000,
    query_max_zones=5000,
):
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-return-regions"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
        != tuple(hashlib.sha256(raw).hexdigest() for raw in inputs)
    ):
        raise ValueError(
            "the union audit requires the complete retained B54 input lineage"
        )
    ...
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B54 result differs from its complete canonical reconstruction"
        )
    ...
    targets = tuple(
        zones(item["query"]["target_regions"])
        for item in fresh["B54_region_queries"]
        if item["target"] in labels
    )
    result = derive_c6_carried_return_union_exclusions(
        reference,
        state=state,
        epi_states=tuple(map(tuple, previous["epi_states"])),
        timestep=previous["timestep"],
        transient_epi_states=tuple(map(tuple, previous["transient_epi_states"])),
        domain_zones=zones(previous["domain_zones"]),
        target_region_groups=targets,
        max_intersections=max_intersections,
        query_max_intersections=query_max_intersections,
        query_max_subsumptions=query_max_subsumptions,
        query_max_zones=query_max_zones,
    )
    records, additional = [], []
    for label, query in zip(labels, result.queries, strict=True):
        public = asdict(query)
    ...
    return dict(
        contract=dict(
            question="Can unjoined return predecessors exclude complete pending first-exit regions?",
            max_intersections=max_intersections,
            query_max_intersections=query_max_intersections,
            query_max_subsumptions=query_max_subsumptions,
            query_max_zones=query_max_zones,
            no_convex_hull_approximation_in_queries=True,
            physical_dynamics_changed=False,
        ),
        source=fresh["source"],
        B55_reconstruction=dict(
            prior_B54_completely_rebuilt=True,
            prior_return_sha256=hashlib.sha256(return_bytes).hexdigest(),
            prior_first_exit_slabs_reverified=fresh["excluded_first_exit_slabs"],
            prior_excursion_prefix_steps_replayed=fresh["B54_reconstruction"][
                "prior_excursion_prefix_steps_replayed"
            ],
        ),
        B55_cube_geometry=fresh["B54_cube_geometry"],
        B55_outgoing_facets=fresh["B54_outgoing_facets"],
        B55_return_envelope=envelope,
        B55_union_queries=tuple(records),
        B55_query_intersections=result.intersections,
        B55_query_subsumptions=result.subsumptions,
        B55_exit_groups=tuple(groups),
        B55_additional_first_exit_facets=tuple(additional),
...
def analyze_c6_winding_return_histories(
    ...
    *,
    ...
    return_bytes,
    union_bytes,
    history_candidates_bytes,
    max_intersections=250_000,
    max_word_work_items=10_000,
):
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-return-unions"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
        != tuple(hashlib.sha256(raw).hexdigest() for raw in inputs)
    ):
        raise ValueError(
            "the history audit requires the complete retained B55 input lineage"
        )
    ...
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B55 result differs from its complete canonical reconstruction"
        )
    previous = fresh["B55_return_envelope"]
    candidates = json.loads(history_candidates_bytes)
    relation_hash = hashlib.sha256(
        json.dumps(
            _payload(previous["return_relation"]),
            sort_keys=True,
            separators=(",", ":"),
        ).encode()
    ).hexdigest()
    if (
        type(candidates.get("schema_version")) is not int
        or candidates["schema_version"] != 1
        or candidates.get("return_relation_sha256") != relation_hash
    ):
        raise ValueError(
            "history candidates must bind the complete canonical return relation"
        )
    ...
    for sparse in candidates["coordinate_generators"]:
        ...
        for pair in sparse:
            if (
                ...
                or not 0 <= pair[0] < size
                or pair[0] in seen
            ):
                raise ValueError(
                    "each sparse coordinate generator needs distinct valid integer edge entries"
                )
            seen.add(pair[0])
            dense[pair[0]] = pair[1]
    ...
    common = dict(
        ...
        domain_zones=tuple(
            C6CarriedForwardZone(tuple(z["epi"]), tuple(map(tuple, z["bounds"])))
            for z in previous["domain_zones"]
        ),
        max_intersections=max_intersections,
    )
    counts = derive_c6_carried_return_count_relaxation(
        reference,
        **common,
        positive_circulation=tuple(candidates["positive_circulation"]),
    ...
    if [p["target"] for p in proposed] != list(labels):
        raise ValueError(
            "count endpoint proposals must cover every original remaining slab exactly once"
        )
    target_queries = [
        item for item in fresh["B55_union_queries"] if item["target"] in labels
    ]
    witnesses = []
    for proposal, item in zip(proposed, target_queries, strict=True):
        ...
        if not contains(original, point) or not contains(retained[target], point):
            raise ValueError(
                "a count endpoint must belong to its complete original unsafe slab and retained cell"
            )
        ...
        if index is not None:
            if type(index) is not int or not 0 <= index < len(
                counts.return_envelope.intermediate_transitions
            ):
            ...
            if edge.target_epi != target or not contains(edge.target_guard, point):
                raise ValueError(
                    "a count endpoint must satisfy its selected terminal transient guard"
                )
        ...
        if base not in retained or not contains(retained[base], displacement):
            raise ValueError(
                "the terminal base endpoint must lie in its retained source region"
            )
        if index is not None and not contains(edge.source_guard, displacement):
            raise ValueError(
                "the terminal base endpoint misses its transient source guard"
            )
        ...
        public.update(
            edge_counts_sha256=hashlib.sha256(
                json.dumps(edge_counts).encode()
            ).hexdigest(),
            minimum_edge_count=min(edge_counts),
            total_edge_count=sum(edge_counts),
            joint_guard_satisfaction_certified=False,
            actual_origin_reachability_certified=False,
        )
        witnesses.append(
            dict(
                target=item["target"],
                endpoint_grid_coordinates=point,
                terminal_transient_edge_index=index,
                terminal_transient_shift=shift,
                count_witness=public,
            )
        )
    words = []
    ...
    if type(proposals) is not list or not proposals:
        raise ValueError(
            "the history campaign requires nonempty closed return-word proposals"
        )
    for proposal in proposals:
        proof = derive_c6_carried_return_word_budget(
            reference,
            **common,
            word_edge_indices=tuple(proposal["edge_indices"]),
            max_work_items=max_word_work_items,
        )
        public = asdict(proof)
        public.pop("return_envelope")
        for key in (
            "word_budget_certified",
            "conditional_word_identity_certified",
            "common_grid_relaxes_coordinate_cosets",
            "actual_origin_reachability_certified",
            "conditional_boundedness_certified",
            "future_runtime_certified",
    ...
    for key in (
        "exact_cycle_displacement_lattice_certified",
        "nonnegative_counts_cover_coordinate_cosets",
        "abstract_mode_walk_exists_for_each_coordinate_coset_point",
        "joint_guard_satisfaction_certified",
        "actual_origin_reachability_certified",
        "conditional_boundedness_certified",
    ):
    ...
    return dict(
        contract=dict(
            question="Which joint temporal constraints are lost by global counts and recovered by exact word guards?",
            max_intersections=max_intersections,
            max_word_work_items=max_word_work_items,
            physical_dynamics_changed=False,
        ),
        source=fresh["source"],
        B56_reconstruction=dict(
            prior_B55_completely_rebuilt=True,
            prior_union_sha256=hashlib.sha256(union_bytes).hexdigest(),
            prior_first_exit_slabs_reverified=fresh["excluded_first_exit_slabs"],
        ),
        B56_return_envelope=envelope,
        B56_relation_sha256=relation_hash,
        B56_count_relaxation=count_public,
        B56_target_count_witnesses=tuple(witnesses),
        B56_word_budgets=tuple(words),
        B56_outgoing_facets=fresh["B55_outgoing_facets"],
        B56_exit_groups=fresh["B55_exit_groups"],
        B56_remaining_first_exit_facets=labels,
        excluded_first_exit_slabs=fresh["excluded_first_exit_slabs"],
        additional_first_exit_slabs_excluded=0,
        all_first_exit_slabs_excluded=not labels,
        primary_proof_origin_remains_B47=True,
        new_conditional_trajectory_steps=0,
        new_live_graph_steps=0,
        indefinite_trapping_certified=False,
...
def _analyze_c6_memory_domain(
    ...
    *,
    ...
    reference, arguments = _c6_memory_configuration(
        fresh, previous_prefix, historical_bytes, domain_zones
    )
    ...
    result = derive_c6_carried_return_memory_region_exclusions(
        ...
        **arguments,
        max_intersections=max_intersections,
        max_memory_work=max_memory_work,
        max_memory_arcs=max_memory_arcs,
        target_region_groups=targets,
        query_max_intersections=query_max_intersections,
    )
    ...
    for item in queries:
        label = item["target"]
        target = tuple(
            tuple(value for value in line)
            for line in item["query"]["target_regions"][0]["bounds"]
        )
        target_row = rows[label["mask"]]
        direct, terminal = [], []
        origin_hit = (
            target_row == state.epi
            and _dbm_intersection(origin_zone, target) is not None
        )
        if proof.initialization_complete:
            memory_states = [(-1, state.epi, origin_zone)] + [
                (i, edge.target_epi, zone)
                for i, (edge, zone) in enumerate(
                    zip(
                        envelope.return_relation,
                        proof.retained_endpoint_zones,
                        strict=True,
                    )
                )
                if zone is not None
            ...
            for index, row, zone in memory_states:
                if (
                    index != -1
                    and row == target_row
                    and _dbm_intersection(zone, target) is not None
                ):
                    direct.append(index)
                for j, edge in enumerate(envelope.intermediate_transitions):
                    ...
                    if (
                        piece is not None
                        and _dbm_intersection(_move(piece, edge.shift), target)
                        is not None
                    ):
                        terminal.append((index, j))
        excluded = (
            proof.initialization_complete
            and not origin_hit
            and not direct
            and not terminal
        )
        ...
        target_observations.append(
            dict(
                target=label,
                original_target_bounds=target,
                origin_in_target=origin_hit,
                direct_memory_indices=tuple(direct),
                terminal_transient_memory_edges=tuple(terminal),
    ...
    for group in groups:
        group["excluded"] += sum(
            label["node"] == group["node"] and label["direction"] == group["direction"]
    ...
    for key in (
        "memory_work",
        "domain_confined_origin_histories_covered",
        "conditional_invariance_certified",
        "conditional_boundedness_certified",
        "future_runtime_certified",
        "asymptotic_convergence_certified",
        ...
        public[key] = getattr(proof, key)
    return dict(
        return_envelope=_c6_public_exact_proof(envelope),
        memory_envelope=public,
        memory_queries=tuple(memory_queries),
        query_relation_complete=result.query_relation_complete,
        query_relation_intersections=result.query_relation_intersections,
        target_observations=tuple(target_observations),
        exit_groups=tuple(groups),
        additional_first_exit_facets=tuple(additional),
        remaining_first_exit_facets=remaining,
    )
...
def analyze_c6_winding_return_memory(
    ...
    *,
    ...
    history_bytes,
    max_intersections=250_000,
    max_memory_work=500_000,
    max_memory_arcs=100_000,
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-return-histories"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
        != tuple(hashlib.sha256(raw).hexdigest() for raw in inputs)
    ):
        raise ValueError(
            "the memory audit requires the complete retained B56 input lineage"
        )
    fresh = analyze_c6_winding_return_histories(
        ...
        history_candidates_bytes=history_candidates_bytes,
        **{
            key: prior["contract"][key]
            for key in ("max_intersections", "max_word_work_items")
        },
    )
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B56 result differs from its complete canonical reconstruction"
        )
    labels = fresh["B56_remaining_first_exit_facets"]
    queries = [
        item
        for item in json.loads(union_bytes)["B55_union_queries"]
        if item["target"] in labels
    ]
    domain = tuple(
        C6CarriedForwardZone(tuple(z["epi"]), tuple(map(tuple, z["bounds"])))
    ...
    additional, remaining = (
        analyzed["additional_first_exit_facets"],
        analyzed["remaining_first_exit_facets"],
    )
    return dict(
        contract=dict(
            question="Does joint carried-state memory of the last return exclude further whole first-exit slabs?",
            max_intersections=max_intersections,
            max_memory_work=max_memory_work,
            max_memory_arcs=max_memory_arcs,
            query_max_intersections=query_max_intersections,
        ...
        source=fresh["source"],
        B57_reconstruction=dict(
            prior_B56_completely_rebuilt=True,
...
def _compare_c6_memory_query_geometry(before, after, groups, maximum):
    ...
    from tnfr.physics.c6_carried_return import (
        _return_memory_query_graph,
        _return_memory_query_initialization,
    )
    ...
    def identities(envelope):
        return tuple(
            (e.source_epi, e.intermediate_epi, e.target_epi, e.shift)
            for e in envelope.return_relation
        )

    records_equal = identities(first) == identities(second)
    graph_before, complete_before, work_before = _return_memory_query_graph(
        before, maximum
    )
    ...
    for targets in groups:
        left, left_complete, left_work = _return_memory_query_initialization(
            before, targets, maximum
        )
        right, right_complete, right_work = _return_memory_query_initialization(
            after, targets, maximum
        )
        initial.append(
            dict(
                complete=left_complete and right_complete,
                equal=left_complete and right_complete and left == right,
                before_sha256=digest(left),
                after_sha256=digest(right),
                before_intersections=left_work,
                after_intersections=right_work,
            )
        )
    ...
    return dict(
        canonical_source_equal=source_equal,
        return_record_identities_equal=records_equal,
        retained_endpoint_arrays_equal=endpoints_equal,
        origin_images_equal=roots_equal,
        complete_query_graph_equal=graph_equal,
        before_graph_complete=complete_before,
        after_graph_complete=complete_after,
        before_graph_intersections=work_before,
        after_graph_intersections=work_after,
        before_arc_count=len(graph_before),
        after_arc_count=len(graph_after),
        before_graph_sha256=digest(graph_before),
        after_graph_sha256=digest(graph_after),
        ...
        actual_origin_reachability_certified=False,
        conditional_boundedness_certified=False,
    )
...
def analyze_c6_winding_memory_cuts(
    ...
    *,
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-return-memory"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
        != tuple(hashlib.sha256(raw).hexdigest() for raw in inputs)
    ):
        raise ValueError(
            "the cut audit requires the complete retained B57 input lineage"
        )
    ...
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B57 result differs from its complete canonical reconstruction"
        )
    previous = fresh["B57_return_envelope"]
    domain = _c6_read_zones(previous["domain_zones"])
    reference, arguments = _c6_memory_configuration(
        fresh, "B57", historical_bytes, domain
    )
    original = derive_c6_carried_return_memory_envelope(
        ...
        **{
            key: prior["contract"][key]
            for key in ("max_intersections", "max_memory_work", "max_memory_arcs")
        },
    )
    b54, b55 = json.loads(return_bytes), json.loads(union_bytes)
    facets = (
        *b54["B54_additional_first_exit_facets"],
        *b55["B55_additional_first_exit_facets"],
        *fresh["B57_additional_first_exit_facets"],
    )
    envelope = original.return_envelope
    shifts = tuple(
        tuple(F(envelope.timestep) * F(p) / envelope.grid_quantum for p in pressure)
        for pressure in envelope.pressures
    )
    if any(v.denominator != 1 for row in shifts for v in row):
    ...
    cut_domain = _cut_c6_proven_facets(
        domain, envelope.epi_states, shifts, b55["B55_cube_geometry"], facets
    )
    changed = derive_c6_carried_return_memory_envelope(
        reference,
        **dict(arguments, domain_zones=cut_domain),
        max_intersections=max_intersections,
        max_memory_work=max_memory_work,
        max_memory_arcs=max_memory_arcs,
    )
    ...
    comparison = _compare_c6_memory_query_geometry(
        original, changed, groups, query_max_intersections
    )
    ...
    return dict(
        contract=dict(
            question="Do certified slab complements change any remaining last-memory query?",
            max_intersections=max_intersections,
            max_memory_work=max_memory_work,
            max_memory_arcs=max_memory_arcs,
            query_max_intersections=query_max_intersections,
        ...
        source=fresh["source"],
        B58_reconstruction=dict(
            prior_B57_completely_rebuilt=True,
            prior_memory_sha256=hashlib.sha256(memory_bytes).hexdigest(),
            prior_first_exit_slabs_reverified=fresh["excluded_first_exit_slabs"],
        ),
        B58_proved_cuts=facets,
        B58_original_cube_geometry=b55["B55_cube_geometry"],
        B58_original_target_groups=tuple(
            tuple(asdict(zone) for zone in group) for group in groups
        ),
        B58_cut_domain=tuple(asdict(zone) for zone in cut_domain),
        B58_return_envelope=_c6_public_exact_proof(changed.return_envelope),
        B58_memory_envelope=public,
        B58_query_equivalence=comparison,
        B58_outgoing_facets=fresh["B57_outgoing_facets"],
        B58_exit_groups=fresh["B57_exit_groups"],
        B58_remaining_first_exit_facets=labels,
        same_query_geometry_repetition_avoided=comparison[
            "all_target_operators_and_initial_conditions_identical"
        ],
        excluded_first_exit_slabs=fresh["excluded_first_exit_slabs"],
        additional_first_exit_slabs_excluded=0,
...
def _c6_priority_hints(raw, target, inputs, maximum):
    ...
    if (
        ...
        or len(target) != 3
        or type(target[0]) is not int
        or type(target[1]) is not int
        or not 0 <= target[0] < 64
        or not 0 <= target[1] < 6
        or target[2] not in ("lower", "upper")
    ):
        raise ValueError(
            "priority_target must be an exact (mask, node, direction) tuple"
        )
    prior = json.loads(raw)
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-safe-partition"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
    ...
    selected = [
        item for item in prior["B59_partition_queries"] if item["target"] == label
    ]
    if len(selected) != 1:
    ...
    if not vertices or len(vertices) > maximum or len(zones) != len(vertices) + 1:
        raise ValueError(
            "priority hints require bounded complete indexed histories and a sentinel"
        )
    if any(
        type(vertex) is not list
        or len(vertex) != 2
        or any(type(value) is not int or value < 0 for value in vertex)
        for vertex in vertices
    ):
        raise ValueError(
            "priority vertices require exact nonnegative memory and piece indices"
        )
    hints = []
    for vertex, zone in zip(vertices, zones[:-1], strict=True):
        if zone is not None:
            if (
                type(zone) is not list
                or len(zone) != 7
                or any(type(row) is not list or len(row) != 7 for row in zone)
            ):
                raise ValueError(
                    "priority regions must be complete seven-dimensional DBMs"
                )
...
def analyze_c6_winding_safe_partition(
    ...
    *,
    ...
    max_memory_work=500_000,
    max_memory_arcs=100_000,
    exclusion_query_max_intersections=250_000,
    max_partition_pieces=5000,
    max_partition_construction_work=100_000,
    max_partition_arcs=100_000,
    max_partition_work=500_000,
    query_max_intersections=250_000,
    excluded_predecessor_depth=0,
    priority_bytes=None,
    priority_target=None,
    max_priority_regions=5000,
    max_cover_work=100_000,
):
    """Keep certified nonconvex safe pieces separate while querying full exit slabs."""
    from tnfr.physics.c6_carried_return import (
        _return_predecessor_depth,
    ...
    if (
        prior.get("manifest", {}).get("claim_id") != "O3.a-C6-carried-memory-cuts"
        or prior.get("source_scope") != list(SOURCE_SCOPE)
        or tuple(item.get("sha256") for item in prior.get("input_evidence", ()))
        != tuple(hashlib.sha256(raw).hexdigest() for raw in inputs)
    ):
        raise ValueError(
            "the safe partition requires the complete retained B58 input lineage"
        )
    ...
    if any(prior.get(key) != value for key, value in _payload(fresh).items()):
        raise ValueError(
            "the retained B58 result differs from its complete canonical reconstruction"
        )
    ...
    domain = _c6_read_zones(previous["domain_zones"])
    reference, arguments = _c6_memory_configuration(
        baseline, "B57", historical_bytes, domain
    )
    labels = fresh["B58_remaining_first_exit_facets"]
    selected = [
        item for item in baseline["B57_memory_queries"] if item["target"] in labels
    ]
    excluded_labels = baseline["B57_additional_first_exit_facets"]
    proposals = [
        item
        for item in baseline["B57_memory_queries"]
        if item["target"] in excluded_labels
    ]
    if [item["target"] for item in selected] != list(labels) or [
        item["target"] for item in proposals
    ] != list(excluded_labels):
        raise ValueError(
            "the safe partition must retain every original target and declared exclusion proposal"
        )
    groups = tuple(_c6_read_zones(item["query"]["target_regions"]) for item in selected)
    excluded_groups = tuple(
        _c6_read_zones(item["query"]["target_regions"]) for item in proposals
    )
    ...
    if priority_bytes is not None:
        ...
        extra = dict(
            priority_memory_regions=hints,
            max_priority_regions=max_priority_regions,
            max_cover_work=max_cover_work,
        )
        if priority_label not in labels:
            raise ValueError(
                "the priority target must remain an original complete pending label"
            )
    result = derive(
        reference,
        **arguments,
        target_region_groups=groups,
        excluded_target_region_groups=excluded_groups,
        max_intersections=max_intersections,
        max_memory_work=max_memory_work,
        max_memory_arcs=max_memory_arcs,
        ...
        max_partition_arcs=max_partition_arcs,
        max_partition_work=max_partition_work,
        query_max_intersections=query_max_intersections,
    ...
    report = dict(
        contract=dict(
            question="Do fixed exact safe-memory pieces exclude further whole first-exit slabs?",
            max_intersections=max_intersections,
            max_memory_work=max_memory_work,
            max_memory_arcs=max_memory_arcs,
            exclusion_query_max_intersections=exclusion_query_max_intersections,
        ...
        source=fresh["source"],
        B59_reconstruction=dict(
            prior_B58_completely_rebuilt=True,
            prior_cuts_sha256=hashlib.sha256(cuts_bytes).hexdigest(),
            prior_first_exit_slabs_reverified=fresh["excluded_first_exit_slabs"],
        ),
        ...
        B59_memory_envelope=base_public,
        B59_partition=public,
        B59_partition_queries=tuple(queries),
        B59_query_relation_complete=result.query_relation_complete,
        B59_query_relation_intersections=result.query_relation_intersections,
        B59_outgoing_facets=fresh["B58_outgoing_facets"],
        B59_exit_groups=tuple(exit_groups),
        B59_additional_first_exit_facets=tuple(additional),
        B59_remaining_first_exit_facets=remaining,
        excluded_first_exit_slabs=fresh["excluded_first_exit_slabs"] + len(additional),
        additional_first_exit_slabs_excluded=len(additional),
        all_first_exit_slabs_excluded=not remaining,
        primary_proof_origin_remains_B47=True,
    ...
    if depth:
        report = {
            key.replace("B59_", "B60_", 1) if key.startswith("B59_") else key: value
    ...
    if priority_bytes is not None:
        report = {
            (
                key.replace("B60_" if depth else "B59_", "B63_", 1)
                if key.startswith("B60_" if depth else "B59_")
                else key
            ): value
            for key, value in report.items()
        }
        ...
        report["B63_priority_evidence"] = dict(
            sha256=hashlib.sha256(priority_bytes).hexdigest(),
            target=priority_label,
            advisory_only=True,
            authorizes_cuts=False,
        )
        report["contract"].update(
...
def main():
    ...
    parser.add_argument(
        "--relay-input", type=Path, default=ROOT / "artifacts/research" / RELAY_NAME
    )
    parser.add_argument(
        "--historical-input",
        type=Path,
    ...
    parser.add_argument(
        "--envelope-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_forward_envelope.json",
    )
    parser.add_argument(
        "--witness-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_forward_envelope.validation.json",
    )
    ...
    parser.add_argument(
        "--region-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_region_exclusions.json",
    )
    parser.add_argument("--max-prefix-steps", type=int, default=4096)
    parser.add_argument(
        "--excursion-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_excursion_exclusion.json",
    )
    parser.add_argument(
        "--mode-candidates-input",
        type=Path,
        default=ROOT / "benchmarks/c6_winding_mode_excursion_candidates.json",
    )
    parser.add_argument(
        "--mode-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_mode_excursions.json",
    )
    parser.add_argument("--max-return-query-intersections", type=int, default=500_000)
    parser.add_argument(
        "--return-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_return_regions.json",
    )
    ...
    parser.add_argument(
        "--union-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_return_unions.json",
    )
    parser.add_argument(
        "--history-candidates-input",
        type=Path,
        default=ROOT / "benchmarks/c6_winding_return_count_candidates.json",
    )
    parser.add_argument("--max-word-work-items", type=int, default=10_000)
    parser.add_argument(
        "--history-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_return_histories.json",
    )
    parser.add_argument(
        "--memory-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_return_memory.json",
    )
    parser.add_argument(
        "--cuts-input",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_memory_cuts.json",
    )
    parser.add_argument(
        "--max-exclusion-query-intersections", type=int, default=250_000
    )
    ...
    if depth and args.method != "safe-partition":
        raise ValueError(
            "excluded predecessor depth requires the safe-partition method"
        )
    ...
    if args.output is None:
        name = {
            "backward": "c6_winding_invariant_region.json",
            "forward": "c6_winding_forward_envelope.json",
            ...
            "modes": "c6_winding_mode_excursions.json",
            "returns": "c6_winding_return_regions.json",
            "unions": "c6_winding_return_unions.json",
            "histories": "c6_winding_return_histories.json",
            "memory": "c6_winding_return_memory.json",
            "memory-cuts": "c6_winding_memory_cuts.json",
            "safe-partition": "c6_winding_safe_partition.json",
        }[args.method]
        if depth:
    ...
    if args.method == "predecessors":
        ...
        inputs += (
            args.envelope_input,
            args.region_input,
            args.excursion_input,
            args.mode_candidates_input,
        )
        if args.method in (
            "returns",
            "unions",
    ...
    if args.method == "backward":
        report = analyze_c6_winding_invariant_region(
            parent,
            relay_bytes=relay_raw,
            historical_bytes=historical_raw,
        ...
        report = analyze_c6_winding_temporal_predecessors(
            ...
            relay_bytes=relay_raw,
            historical_bytes=historical_raw,
            envelope_bytes=input_bytes[3],
            witness_bytes=input_bytes[4],
            max_depth=args.max_depth,
            max_row_checks=args.max_row_checks,
        )
    elif args.method == "regions":
        ...
        report = analyze_c6_winding_mode_excursions(
            ...
            envelope_bytes=input_bytes[3],
            region_bytes=input_bytes[4],
            excursion_bytes=input_bytes[5],
            candidates_bytes=input_bytes[6],
            max_intersections=args.max_intersections,
            max_prefix_steps=args.max_prefix_steps,
        )
        ...
        report = analyze_c6_winding_return_unions(
            ...
            candidates_bytes=input_bytes[6],
            mode_bytes=input_bytes[7],
            return_bytes=input_bytes[8],
            max_intersections=args.max_intersections,
            query_max_intersections=args.max_return_query_intersections,
            query_max_subsumptions=args.max_return_query_subsumptions,
            query_max_zones=args.max_return_query_zones,
        )
        ...
        report = analyze_c6_winding_return_histories(
            ...
            mode_bytes=input_bytes[7],
            return_bytes=input_bytes[8],
            union_bytes=input_bytes[9],
            history_candidates_bytes=input_bytes[10],
            max_intersections=args.max_intersections,
            max_word_work_items=args.max_word_work_items,
        )
    else:
        analyzer = {
            "memory": analyze_c6_winding_return_memory,
            "memory-cuts": analyze_c6_winding_memory_cuts,
            "safe-partition": analyze_c6_winding_safe_partition,
        }[args.method]
        extra = (
            dict(memory_bytes=input_bytes[12])
            if args.method in ("memory-cuts", "safe-partition")
            else {}
        )
        if args.method == "safe-partition":
            extra.update(
                cuts_bytes=input_bytes[13],
                ...
                max_partition_arcs=args.max_partition_arcs,
                max_partition_work=args.max_partition_work,
            )
            if depth:
                extra["excluded_predecessor_depth"] = depth
            if args.priority_input is not None:
                extra.update(
                    priority_bytes=input_bytes[14],
                    priority_target=(
                        int(args.priority_target[0]),
                        int(args.priority_target[1]),
                        args.priority_target[2],
                    ),
                    max_priority_regions=args.max_priority_regions,
                    max_cover_work=args.max_cover_work,
                )
        report = analyzer(
            parent,
            parent_bytes=raw,
            ...
            union_bytes=input_bytes[9],
            history_candidates_bytes=input_bytes[10],
            history_bytes=input_bytes[11],
            max_intersections=args.max_intersections,
            max_memory_work=args.max_memory_work,
    ...
    telemetry = (
        "exact pressure table",
        "negative quadratic direction",
        "positive rational dual certificate",
        "complete descending sets",
        "origin membership",
        "resource outcome",
    )
    if args.method == "forward":
        telemetry = (
            "exact pressure table",
            "protected pair strips",
            "complete descending relational layers",
            "outgoing facets",
            "original point membership",
            "actual entry",
            "resource outcome",
        )
    if args.method == "predecessors":
        telemetry = (
            "complete B49 reconstruction",
            "exact selected outgoing points",
            "complete predecessor frontiers",
            "shared-kernel edge identities",
            "finite-past exclusion versus initial transient",
        )
    if args.method == "regions":
        telemetry = (
            "complete B49 reconstruction",
            "full RN cube geometry",
            "complete unsafe slab cover",
            "complete regional predecessor layers",
            "origin exclusion",
            "first-exit group coverage",
        )
    if args.method == "excursion":
        telemetry = (
            "reverified prior region exclusions",
            "exact drift",
            "dual and primal ingress bounds",
            "proof-derived initial prefix",
            "full nodal carry",
            "whole first-exit group coverage",
        )
    if args.method == "modes":
        telemetry = (
            "complete B52 reconstruction",
            "origin-injected reachable hulls",
            "exact mode-dependent drift",
            "per-target primal and dual bounds",
            "whole first-exit coverage",
            "remaining complete regions",
        )
    if args.method == "returns":
        telemetry = (
            "complete B53 reconstruction",
            "exact short-return guards",
            "intermediate exit preservation",
            "complete backward return layers",
            "unchanged origin exclusion",
            "whole first-exit coverage",
        )
    if args.method == "unions":
        telemetry = (
            "complete B54 reconstruction",
            "unjoined transient target preimages",
            "exact region unions",
            "complete backward layers",
            "unchanged origin exclusion",
            "bounded subsumption work",
        )
    if args.method == "histories":
        telemetry = (
            "complete B55 reconstruction",
            "exact cycle displacement generators",
            "positive balanced circulation",
            "count witnesses without RN chronology",
            "joint word guards",
            "sharp consecutive-repeat budgets",
        )
    if args.method == "memory":
        telemetry = (
            "complete B56 reconstruction",
            "exact completed-return endpoint guards",
            "complete memory relation",
            "atomic descending worklist updates",
            "every direct and terminal transient target",
            "resource scope",
        )
    if args.method == "memory-cuts":
        telemetry = (
            "complete B57 reconstruction",
            "proved whole-slab complements",
            "complete clipped query graph",
            "all original target initializations",
            "exact operator equality versus resource abstention",
        )
    if args.method == "safe-partition":
        telemetry = (
            "complete B58 reconstruction",
            "reverified whole-target exclusions",
            "unjoined exact bad pieces",
            "disjoint safe seeds",
            "persistent seed clipping",
            "complete original target queries",
        )
        if depth:
            telemetry += (
                "exact guarded predecessor words",
                "complete predecessor layers",
                "bounded unjoined cuts",
            )
        if args.priority_input is not None:
            telemetry += (
                "advisory-only priority with charged classification",
                "complete retained-cover image check",
                "canonical premise reconstruction",
                "separate validation and query costs",
            )
    claim = {
        "backward": "O3.a-C6-carried-invariant-region",
        "forward": "O3.a-C6-carried-forward-envelope",
        ...
        "modes": "O3.a-C6-carried-mode-excursions",
        "returns": "O3.a-C6-carried-return-regions",
        "unions": "O3.a-C6-carried-return-unions",
        "histories": "O3.a-C6-carried-return-histories",
        "memory": "O3.a-C6-carried-return-memory",
        "memory-cuts": "O3.a-C6-carried-memory-cuts",
        "safe-partition": "O3.a-C6-carried-safe-partition",
    }[args.method]
    solver = {
        "backward": "Canonical refreshed pressure with exact carried-map preimages and complete set inclusion",
        ...
        "safe-partition": "Canonical reverified bad preimages, disjoint safe-memory seeds and bounded whole-target queries",
    }[args.method]
    if depth:
    ...
    manifest = CoreExperimentManifest(
        ...
        telemetry=telemetry,
        controls=(
            "full incoming carry preserved",
            "no sampled recurrence; proof-derived entry only when required",
            "unfinished descents never certify invariance",
            "numerical proof differs from live graph provenance",
        ),
    ...
    if (
        ...
        raise RuntimeError(
            "source or retained input changed during the invariant-region audit"
        )
    report.update(
    ...
    if args.method == "predecessors":
        ...
        key = {
            "regions": "B51_outgoing_facets",
            "excursion": "B52_outgoing_facets",
            "modes": "B53_outgoing_facets",
            "returns": "B54_outgoing_facets",
            "unions": "B55_outgoing_facets",
            "histories": "B56_outgoing_facets",
            "memory": "B57_outgoing_facets",
            "memory-cuts": "B58_outgoing_facets",
            "safe-partition": "B59_outgoing_facets",
        }[args.method]
        if depth:
        ...
        print(
            "Excluded first-exit slabs:",
            report["excluded_first_exit_slabs"],
            "/",
            len(report[key]),
        )
    else:
        key = (
            "B48_exact_correlated_viability"
            if args.method == "backward"
            else "B49_relational_forward_envelope"
        )
