...
from benchmarks.c6_winding_pressure_repayment import _net_prefixes, analyze_c6_winding_pressure_repayment  # noqa: E402
from benchmarks.c6_winding_pressure_sign import (  # noqa: E402
    CAPACITY, MAX_BOUNDARIES, MAX_STEPS, STEP, TARGET_NODE, _budget, _canonical, _gradient_balance,
)
...
from tnfr.physics.nodal_remainder_pressure import (  # noqa: E402
    derive_finite_level_nodal_return, observe_finite_nodal_pressure_drift,
)
...
def _verified_source(parent, previous, ancestor, source_parent):
    ...
    if manifest.claim_id != "O3.a-C6-carried-pressure-repayment" or tuple(parent["source_scope"]) != SOURCE_SCOPE:
        raise ValueError("input must be the retained B29 carried-pressure-repayment report")
    for key, producer in (("input_evidence", previous), ("parent_input_evidence", ancestor),
        ...
        if (_canonical(evidence["producer_manifest"]) != _canonical(producer["manifest"])
            ...
            raise ValueError("the supplied B28/B27/B26 sources differ from the B29 producer lineage")
    for retained, key in (("producer_input_evidence", "input_evidence"),
        ...
        if _canonical(parent["input_evidence"][retained]) != _canonical(previous[key]):
            raise ValueError("the B29 lineage differs from the supplied B28 input chain")
    ...
    if any(key not in parent or _canonical(parent[key]) != _canonical(value) for key, value in rebuilt.items()):
        raise ValueError("the retained B29 report differs from its complete shared-owner replay")
    ...
    if (rebuilt["continuation"]["net_area_reference"] != "B27.continuation.endpoint"
        ...
        raise ValueError("incoming area differs from the complete reconstructed change since the B27 endpoint")
    ...
    levels = tuple(rebuilt["two_level_return"][key] for key in ("negative_pressure", "positive_pressure"))
    observation = rebuilt["continuation"]["refreshed_pressure"]
    if observation["pressure"][TARGET_NODE] not in levels:
        raise ValueError("the inherited B29 endpoint is already outside the declared two-level source")
...
def _return_record(levels):
    ...
    return {**asdict(result), "pressure_provenance_certified": result.pressure_provenance_certified,
...
def _finite_class(observations):
    ...
    for observation in observations:
        ...
        if key in unique and unique[key] != values:
            raise RuntimeError("one visible state generated two different pressures under the fixed source")
        unique[key] = values
    states, pressures = tuple(unique), tuple(unique.values())
    observed = observe_finite_nodal_pressure_drift(
        epi_states=states, pressure_vectors=pressures, functional=(Fraction(0),) * 4 + (Fraction(1), Fraction(0)),
        timestep=STEP, epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
    )
    return {
        "unique_state_count": len(states), "observation": asdict(observed),
...
def analyze_c6_winding_pressure_levels(parent, previous, ancestor, source_parent, *, boundary_budget=MAX_BOUNDARIES,
    ...
    reference, initial, baseline, incoming, levels, observation = _verified_source(parent, previous, ancestor, source_parent)
    ...
    while stop is None:
        ...
        if exit_record.horizon != horizon:
            raise RuntimeError("the preflight horizon differs from the shared cell-exit replay")
    ...
    if steps:
        ...
        if sequence.steps != tuple(steps) or sequence.endpoint != state:
            raise RuntimeError("the complete prefix differs from its cell-boundary replays")
        ...
        if not itinerary.feasible or not all(membership):
            raise RuntimeError("inverse itinerary constraints lost the actual inherited carry")
    net = tuple(a + b for a, b in zip(incoming, local, strict=True))
    novel = current["pressure"][TARGET_NODE] if current["pressure"][TARGET_NODE] not in levels else None
    level_counts = tuple(sum(step.pressure[TARGET_NODE] == value for step in steps) for value in levels)
    if sum(level_counts) != len(steps):
        raise RuntimeError("a novel pressure level was integrated after the stopping condition")
    ...
    return {
        "source": {
            "location": "B29.continuation.endpoint", "inherited_state": asdict(initial),
            "retained_B29_report_replayed": True, "B27_area_reference_state": asdict(baseline),
        ...
        "pressure_levels": {
            "inherited_levels": levels, "executed_step_counts": level_counts,
            "captured_novel_level": novel, "novel_level_steps_integrated": 0,
            "inherited_return_arithmetic": _return_record(levels),
            "captured_return_arithmetic": _return_record(levels + (novel,)) if novel is not None else None,
            "scope": (
        ...
        "continuation": {
            "local_area_reference": "B29.continuation.endpoint", "net_area_reference": "B27.continuation.endpoint",
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_repayment.json")
    parser.add_argument("--parent-input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_sign.json")
    parser.add_argument("--previous-input", type=Path, default=ROOT / "artifacts/research/c6_winding_carry_itinerary.json")
    parser.add_argument("--source-input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_lattice.json")
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_levels.json")
    ...
    parent, previous, ancestor, source_parent = producers = tuple(json.loads(value) for value in raw)
    hashes = tuple(hashlib.sha256(value).hexdigest() for value in raw)
    edges = ((parent, "input_evidence", 1), (parent, "parent_input_evidence", 2), (parent, "source_input_evidence", 3),
             (previous, "input_evidence", 2), (previous, "source_input_evidence", 3), (ancestor, "input_evidence", 3))
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-finite-pressure-levels", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        telemetry=("captured versus executed pressure levels", "finite-level return modulus", "signed nodal area overshoot",
                   "deduplicated finite-class pressure drift", "exact inverse itinerary"),
        controls=("at most eight new boundaries and 256 steps", "actual carry retained", "no novel-level step"),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or tuple(path.read_bytes() for path in paths) != raw:
        raise RuntimeError("analysis source or historical input changed during the finite-level audit")
    ...
    report["input_evidence"]["producer_parent_input_evidence"] = parent["parent_input_evidence"]
