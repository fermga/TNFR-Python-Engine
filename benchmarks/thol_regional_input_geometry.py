...
INPUT_PATH = ROOT / "artifacts/research/thol_regional_response_criterion_2026_09_18.json"
...
def analyze_retained(retained):
    ...
    size = len(nodes)
    if (not 2 <= size <= 16 or len(set(nodes)) != size
            or not children or len(set(children)) != len(children)
            or any(child not in nodes for child in children)):
        raise ValueError("a bounded distinct node space and original cohort are required")
    ...
    _equal(metric, _vector(reference["metric_weights"], "original metric"), "original metric")
    if len(metric) != size:
    ...
    expected_t = tuple(tuple(s-dt*a for s, a in zip(sr, ar, strict=True))
    ...
    for stage, key in STAGES:
        ...
        columns = tuple(tuple(row[j] for row in geometry.input_map)
        ...
        for name in WITNESSES:
            ...
            stored = stages[stage]
            _equal(_matrix(stored["transition"], size, size, "transition"),
                   geometry.transition, f"{name}/{stage} transition")
            _equal(_vector(stored["metric_weights"], "stage metric"),
                   geometry.metric_weights, f"{name}/{stage} metric")
            ...
            _equal(_matrix(stored["centering"], len(region), size, "centering"),
                   geometry.centering, f"{name}/{stage} centering")
            stored_images = tuple((label, _vector(values, "nullspace image"))
        ...
        summaries[stage] = {
            ...
            "rank": geometry.rank,
            "parent_only_rank": exact_rank(tuple(row[1:] for row in geometry.input_map)),
            "protected_dimension": geometry.protected_dimension,
        }
    reception, held = (geometries[label] for label, _ in STAGES)
    difference = tuple(tuple(t-s for t, s in zip(tr, sr, strict=True))
                       for tr, sr in zip(held.input_map, reception.input_map, strict=True))
    pressure_columns = tuple(
        tuple(-dt*dot(row, tuple(dot(ar, vector) for ar in matrices["A"]))
              for row in reception.centering)
        for vector in reception.input_basis
    )
    pressure = tuple(tuple(column[i] for column in pressure_columns)
                     for i in range(len(region)))
    _equal(difference, pressure, "input-map difference -h C A N")
    return {
        "nodes": nodes, "children": children, "region_indices": region,
        "metric_weights": metric, "common_coefficients": common,
        "verified_witnesses": WITNESSES, "geometries": geometries,
        "geometry_summaries": summaries,
        "comparison": {"input_map_difference": difference,
                       "expected_pressure_contribution": pressure,
                       "rank_change": held.rank-reception.rank,
                       "parent_only_rank_change": summaries["held_pressure_interval"]["parent_only_rank"]-summaries["reception"]["parent_only_rank"],
                       "protected_dimension_change": held.protected_dimension-reception.protected_dimension},
        "native_calls": 0, "kernel_calls": 0, "forcing_capture_calls": 0,
        "new_trajectories": 0, "geometry_evaluations": 2,
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "artifacts/research/thol_regional_input_geometry_2026_09_18.json")
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-regional-environmental-input-geometry", git_sha=sha,
        ...
        telemetry=("environmental input rank", "protected read-outs", "weighted projections", "held-pressure attribution"),
        controls=("authenticated prior claim", "all retained witness geometries checked", "original cohort and coefficients"),
