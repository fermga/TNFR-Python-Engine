...
from benchmarks.c6_winding_pressure_levels import analyze_c6_winding_pressure_levels  # noqa: E402
...
from benchmarks.c6_winding_pressure_sign import (  # noqa: E402
    CAPACITY, MAX_BOUNDARIES, MAX_STEPS, STEP, _budget, _canonical, _gradient_balance,
)
...
from tnfr.physics.c6_pressure_lattice import (  # noqa: E402
    derive_c6_pressure_lattice, observe_c6_frozen_pressure_stencil, observe_c6_pressure_lattice,
)
...
TARGET_NODE = 4
STENCIL = (3, 4, 5)
EVIDENCE_KEYS = ("input_evidence", "parent_input_evidence", "previous_input_evidence", "source_input_evidence")


def _verified_source(parent, previous, ancestor, earlier, source_parent):
    manifest = CoreExperimentManifest(**parent["manifest"])
    manifest.validate_for_admission()
    if manifest.claim_id != "O3.a-C6-carried-finite-pressure-levels" or tuple(parent["source_scope"]) != SOURCE_SCOPE:
    ...
    for key, producer in zip(EVIDENCE_KEYS, producers, strict=True):
        ...
        if (_canonical(evidence["producer_manifest"]) != _canonical(producer["manifest"])
            ...
            raise ValueError("supplied B29/B28/B27/B26 sources differ from the B30 producer lineage")
    for retained, key in (("producer_input_evidence", "input_evidence"),
        ...
        if _canonical(parent["input_evidence"][retained]) != _canonical(previous[key]):
            raise ValueError("the B30 lineage differs from the supplied B29 historical input chain")
    ...
    if any(key not in parent or _canonical(parent[key]) != _canonical(value) for key, value in rebuilt.items()):
        raise ValueError("the retained B30 report differs from its complete bounded shared-owner replay")
    ...
    if (rebuilt["continuation"]["net_area_reference"] != "B27.continuation.endpoint"
            or incoming != tuple(b - a for a, b in zip(baseline.exact_epi, initial.exact_epi, strict=True))):
        raise ValueError("incoming area differs from the actual reconstructed change since the B27 endpoint")
    ...
    if observation["pressure"][TARGET_NODE] <= 0:
        raise ValueError("the inherited B30 endpoint must begin with positive node-4 pressure")
    return reference, initial, baseline, incoming, observation


def _stencil_record(reference, state, observation):
    result = observe_c6_frozen_pressure_stencil(reference, state=state, node=TARGET_NODE, timestep=STEP)
    if _canonical(_record(result.initial_observation)) != _canonical(observation):
        raise RuntimeError("the frozen-stencil owner disagrees with the freshly generated endpoint pressure")
    data = asdict(result)
    data.pop("reference")
...
def _nonpositive_cut(reference, observation):
    ...
    return {
        "node": TARGET_NODE, "current_gradient_index": current, "nonpositive_max_index": cut,
...
def analyze_c6_winding_pressure_stencil(parent, previous, ancestor, earlier, source_parent, *,
                                        boundary_budget=MAX_BOUNDARIES, step_budget=MAX_STEPS):
    ...
    reference, initial, baseline, incoming, observation = _verified_source(parent, previous, ancestor, earlier, source_parent)
    ...
    while stop is None:
        ...
        horizon = derive_nodal_remainder_cell_horizon(state=state, timestep=STEP, capacity=CAPACITY, pressure=pressure)
        ...
        observed = observe_nodal_remainder_cell_exit(
            state=state, timestep=STEP, capacity=CAPACITY, pressure=pressure, step_budget=remaining,
        )
        if observed.horizon != horizon:
            raise RuntimeError("the preflight and shared carried cell-exit horizons differ")
        sequence = observed.sequence
        refreshed = _record(observe_c6_pressure_lattice(reference, epi=sequence.endpoint.epi))
        ...
        if first_stencil_change is None and changed:
            first_stencil_change = {
                "step": len(steps) + count, "boundary": len(boundaries) + 1, "changed_nodes": changed,
                "before": asdict(state), "after": asdict(sequence.endpoint),
                "pressure_before": current["pressure"][TARGET_NODE], "pressure_after": refreshed["pressure"][TARGET_NODE],
            }
        boundaries.append({
            "ordinal": len(boundaries) + 1, "step_count": count, "cumulative_steps": len(steps) + count,
            "initial": asdict(state), "horizon": asdict(horizon), "source_pressure": current,
            "endpoint": asdict(sequence.endpoint), "refreshed_pressure": refreshed,
            "stencil_before": tuple(state.epi[i] for i in STENCIL),
            "stencil_after": tuple(sequence.endpoint.epi[i] for i in STENCIL), "changed_stencil_nodes": changed,
            ...
            "gradient_balance": _gradient_balance(state, sequence.endpoint, sequence.prefixes[-1].cumulative_nodal_area,
    ...
    if steps:
        ...
        sequence = observe_nodal_remainder_sequence(
            initial=initial, timesteps=(STEP,) * count, capacities=(CAPACITY,) * count, pressures=tuple(pressures),
        )
        if sequence.steps != tuple(steps) or sequence.endpoint != state:
            raise RuntimeError("the full carried prefix differs from its cell-boundary replays")
        ...
        itinerary = derive_nodal_remainder_itinerary(
            epi_states=(initial.epi,) + tuple(step.after.epi for step in steps), timesteps=(STEP,) * count,
            capacities=(CAPACITY,) * count, pressures=tuple(pressures), epi_lower=LOCAL_LOWER, epi_upper=LOCAL_UPPER,
        )
    ...
    return {
        "source": {
            "location": "B30.continuation.endpoint", "inherited_state": asdict(initial),
            "retained_B30_report_replayed": True, "B27_area_reference_state": asdict(baseline),
            "incoming_B27_nodal_area": incoming, "phase": reference.source.phase,
        ...
        "stop_reason": stop, "censor": censor, "boundaries": tuple(boundaries),
        "target_node": TARGET_NODE, "stencil_nodes": STENCIL, "first_stencil_change": first_stencil_change,
        "initial_frozen_stencil": initial_stencil, "endpoint_frozen_stencil": endpoint_stencil,
        ...
        "nonpositive_node4_pressure_steps_integrated": sum(step.pressure[TARGET_NODE] <= 0 for step in steps),
        "continuation": {
            "local_area_reference": "B30.continuation.endpoint", "net_area_reference": "B27.continuation.endpoint",
            "step_count": len(steps), "boundary_count": len(boundaries), "steps": tuple(map(asdict, steps)),
            "prefix_balances": _prefix_summary(sequence) if sequence is not None else (), "net_prefixes": net_prefixes,
            "endpoint": asdict(state), "refreshed_pressure": current, "local_nodal_area": local, "B27_nodal_area": net,
            "local_mean_nodal_area": sum(local, Fraction(0)) / 6, "B27_mean_nodal_area": sum(net, Fraction(0)) / 6,
            "gradient_balance": _gradient_balance(initial, state, local, reference.epi_quantum),
        ...
        "original_tail_reachability_certified": False, "future_pressure_sign_exit_certified": False,
        "future_compensation_cycle_certified": False, "positive_band_exit_certified": False,
        "scope": (
...
def main():
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_levels.json")
    parser.add_argument("--parent-input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_repayment.json")
    parser.add_argument("--previous-input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_sign.json")
    parser.add_argument("--earlier-input", type=Path, default=ROOT / "artifacts/research/c6_winding_carry_itinerary.json")
    parser.add_argument("--source-input", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_lattice.json")
    parser.add_argument("--output", type=Path, default=ROOT / "artifacts/research/c6_winding_pressure_stencil.json")
    args = parser.parse_args()
    paths = (args.input, args.parent_input, args.previous_input, args.earlier_input, args.source_input)
    ...
    links = (
        (("input_evidence", 1), ("parent_input_evidence", 2), ("previous_input_evidence", 3), ("source_input_evidence", 4)),
        (("input_evidence", 2), ("parent_input_evidence", 3), ("source_input_evidence", 4)),
        (("input_evidence", 3), ("source_input_evidence", 4)), (("input_evidence", 4),),
    )
    if any(producer[key]["sha256"] != hashes[index]
           for producer, edges in zip(producers[:-1], links, strict=True) for key, index in edges):
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-pressure-stencil", git_sha=sha, source_dirty=dirty, dirty_source_hash=digest,
        ...
        telemetry=("first actual stencil change", "frozen-stencil coordinate and uniform horizons",
                   "necessary integer pressure-sign cut", "local and inherited six-node budgets"),
        controls=("at most eight new boundaries and 256 steps", "no carry reset", "no post-sign-hit steps"),
        artifacts=(str(args.output),),
    )
    manifest.validate_for_admission()
    if current_git_source_provenance(ROOT, SOURCE_SCOPE) != provenance or tuple(path.read_bytes() for path in paths) != raw:
        raise RuntimeError("analysis source or historical input changed during the bounded stencil audit")
    ...
    for key, path, digest, producer in zip(
        ...
        paths, hashes, producers, strict=True,
    ):
