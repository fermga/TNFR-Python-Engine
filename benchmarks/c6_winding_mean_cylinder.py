...
def _same(observed, expected, label):
    if _canonical(observed) != _canonical(expected):
        raise ValueError(
            f"retained B43 {label} differs from its declared arithmetic replay"
        )


def _event(record, name, time, before, after, state, capacity_after=(1.0,) * 6):
    _same(
        record,
        {
            "operator_name": name,
...
def replay_c6_phase_tail_evidence(parent):
    ...
    if (
        manifest.claim_id != "O3.a-C6-temporal-cell-compatibility"
    ...
    for key, value in {
        ...
        "declared_phase_tail_reached": True,
        "phase_projection_fixed_at_endpoint": True,
    ...
    state = NodalRemainderState((0.5,) * 6, (F(0),) * 6, EPI_LOWER, EPI_UPPER)
    initial = state
    ...
    _same(
        runtime["source"],
        {
            ...
            "initial_capacity": (1.0,) * 6,
            "ordered_nodes": tuple(range(6)),
            "support": ((0, 1), (0, 5), (1, 2), (2, 3), (3, 4), (4, 5)),
            "normalized_weights": tuple(
                (key, F(value)) for key, value in weights.items()
            ),
            "random_seed": 17,
    ...
    _same(
        runtime["schedule"], asdict(_declared_schedule(TAIL_ENTRY_CYCLE)[0]), "schedule"
    )
    ...
    for index, cycle in enumerate(runtime["cycles"]):
        ...
        if len(cycle["flows"]) != 4:
            raise ValueError(
                "each retained B43 cycle must contain four refreshed segments"
            )
        tail = state
        for segment, flow in enumerate(cycle["flows"]):
            ...
            step = advance_nodal_remainder(
                state, timestep=SEGMENT_DURATION, capacity=(1.0,) * 6, pressure=pressure
            )
            _same(
                flow,
                {
                    ...
                    "end_time": time + (segment + 1) * SEGMENT_DURATION,
    ...
    if len(capacity_after) != 6 or any(
        type(value) is not float or not 0 <= value < 1 for value in capacity_after
    ):
        raise ValueError("the retained SHA capacity boundary must be explicit")
    _event(
        runtime["terminal_silence"],
        "silence",
        22.25,
        phase,
        phase,
    ...
    _same(runtime["tail_entry_time"], 22.0, "tail-entry time")
    _same(runtime["endpoint_time"], 22.25, "endpoint time")
    _same(runtime["endpoint_phase"], phase, "endpoint phase")
    _same(
        observe_c6_coupling_coherence_phase_step(phase=phase).phase_after_coherence,
        phase,
        "fixed phase",
    )
    change = tuple(
        b - a for a, b in zip(initial.exact_epi, state.exact_epi, strict=True)
    )
    _same(runtime["exact_nodal_area"], area, "nodal area")
    _same(runtime["exact_reconstructed_change"], change, "reconstructed change")
    _same(
        runtime["exact_nodal_balance_residual"],
        tuple(a - b for a, b in zip(change, area)),
        "nodal residual",
    )
    _same(runtime["mean_area"], sum(area, F(0)) / 6, "mean area")
    profile = derive_c6_carried_profile(lattice)
    tail_closure = asdict(
        derive_c6_carried_closure(profile, state=tail, timestep=SEGMENT_DURATION)
    )
...
def analyze_c6_winding_mean_cylinder(parent):
    ...
    obstruction = derive_c6_carried_mean_cylinder_obstruction(
        closure,
        positive_state=template(3),
        negative_state=template(0),
    )
    ...
    for name, (lower, upper) in {
        ...
        record = asdict(
            observe_c6_carried_affine_mean_escape(
                affine, mean_lower=lower, mean_upper=upper
            )
        )
        record.pop("obstruction")
    ...
    return {
        ...
        "B44_mean_cylinder_obstruction": bound,
        "B44_hypothetical_boundary_witnesses": cases,
        "B45_affine_mean_obstruction": affine_bound,
        "B45_hypothetical_boundary_witnesses": affine_cases,
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "artifacts/research/c6_winding_mean_cylinder.json",
    )
    ...
    manifest = CoreExperimentManifest(
        claim_id="O3.a-C6-carried-mean-cylinder-obstruction",
        git_sha=sha,
        source_dirty=dirty,
        ...
        telemetry=(
            "centered energy",
            "exact reconstructed mean",
            "coordinate increment gcds",
            "outward boundary witnesses",
        ),
        controls=(
            "actual B43 carry preserved",
            "no historical live seal recreated",
            "bounded local mean window",
    ...
    if (
        ...
        raise RuntimeError(
            "source or historical input changed during the mean-cylinder audit"
        )
    report.update(
