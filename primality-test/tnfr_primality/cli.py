...
TNFR Theory:
  ...
  This represents arithmetic pressure in structural coherence systems.
    ...
    parser.add_argument(
        "--optimized",
        action="store_true",
        help="Use optimized implementation with caching",
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Batch processing mode for multiple numbers",
    )
    parser.add_argument(
        "--benchmark", type=int, metavar="N", help="Run performance benchmark up to N"
    )
    parser.add_argument(
        "--validate",
        type=int,
        ...
        if args.batch:
            ...
            for n, is_prime, delta_nfr in results:
                if args.timing:
                    ...
                    print(
                        f"{n:12d}  {str(is_prime):>8}  {delta_nfr:14.6f}  {individual_time:15.2f}"
                    )
                else:
            ...
            for n in args.numbers:
                ...
                if args.timing:
                    ...
                    print(
                        f"{n:12d}  {str(is_prime):>8}  {delta_nfr:14.6f}  {elapsed_us:10.2f}"
                    )
                else:
                    print(f"{n:12d}  {str(is_prime):>8}  {delta_nfr:14.6f}")
