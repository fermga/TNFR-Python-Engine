...
from tnfr_primality import OptimizedTNFRPrimality, tnfr_is_prime
...
def comprehensive_benchmark() -> Dict[str, Any]:
    ...
    for scenario_name, numbers in scenarios.items():
        ...
        scenario_results = {
            ...
            "speedup_cached": (
                basic_time / opt_time_cached if opt_time_cached > 0 else 0
            ),
            "cache_effectiveness": (
                opt_time_1st / opt_time_cached if opt_time_cached > 0 else 0
            ),
            "results_consistent": consistent,
            "avg_time_per_number_us": (basic_time / len(numbers)) * 1_000_000,
            "processing_rate_per_sec": (
                len(numbers) / basic_time if basic_time > 0 else 0
            ),
        }
        ...
        print(
            f"  Optimized (cached): {scenario_results['optimized_time_cached_ms']:.2f} ms"
        )
...
def stress_test(duration_seconds: int = 30) -> Dict[str, Any]:
    ...
    results = {
        ...
        "errors": errors,
        "tests_per_second": (
            tests_performed / actual_duration if actual_duration > 0 else 0
        ),
...
def main():
    ...
    for scenario, results in benchmark_results.items():
        print(
            f"  {scenario}: {results['speedup_cached']:.1f}x speedup, "
