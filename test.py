...
def firma_estructural(palabra):
    ...
    vf = int(h[2], 16) / 100 + 0.5       # νf entre 0.5 y 1.5
    theta = int(h[24], 16) / 100         # θ entre 0.0 y 1.5
    si = int(h[30], 16) / 100            # Si entre 0.0 y 1.5
    return vf, theta, si
...
print(f"Estímulo estructural νf={vf_est:.2f}, θ={theta_est:.2f}, Si={si_est:.2f}\n")

# Inicializar red nodal
G = nx.Graph()
for i in range(1, 21):
    G.add_node(f"N{i}",
...
for paso in range(15):
    ...
    for n in G.nodes:
        ...
        if delta_vf < 0.3 and delta_theta < 0.3 and delta_si < 0.3:
            glifo = "RA"  # resonancia con el estímulo
        else:
            glifo = glifo_por_estructura(nodo, G) or "AL"

        aplicar_glifo(G, nodo, n, glifo, historial_glifos_por_nodo, paso)

        print(f"{n}  EPI={nodo['EPI']:.2f}  νf={nodo['νf']:.2f}  ΔNFR={nodo['ΔNFR']:.2f}  Si={nodo['Si']:.2f}  Glifo={glifo}")
