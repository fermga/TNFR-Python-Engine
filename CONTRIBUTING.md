...
TNFR Grammar 2.0 enables community-driven evolution through domain extensions. Community members can contribute specialized patterns, health analyzers, and domain-specific tools without modifying core TNFR.

### What are Domain Extensions?

Domain extensions package validated operator sequences and domain expertise for specific application areas:

- **Medical/Healthcare**: Therapeutic journeys, crisis intervention, trauma-informed care
- **Business**: Sales cycles, organizational change, team formation
- **Education**: Learning progressions, curriculum design, assessment patterns
- **Scientific**: Research methodologies, experimental design, collaboration patterns
- **Creative**: Artistic processes, creative cycles, innovation sprints

Each extension provides:
- Validated operator sequences (health score ≥ 0.75)
- Real-world use cases and applications
- Structural insights explaining mechanisms
- Domain-specific documentation

### Creating a New Extension

#### Step 1: Use the Extension Template

Start with the comprehensive template:

```bash
# Review the extension template
cat .github/EXTENSION_TEMPLATE.md

# Generate pattern templates for your domain
python tools/community/pattern_generator.py template your_domain pattern_name
```

#### Step 2: Design and Validate Patterns

Create operator sequences that solve real problems in your domain:
...
Use the pattern generator for validation and suggestions:
...
python tools/community/pattern_generator.py suggest "stabilize after crisis"
```

#### Step 3: Implement Extension Class

Create your extension following the base class pattern:
...
class YourDomainExtension(TNFRExtension):
    ...
    def get_domain_name(self) -> str:
        return "your_domain_name"  # lowercase, alphanumeric with underscores
    
    def get_pattern_definitions(self) -> Dict[str, PatternDefinition]:
        return {
            "pattern_id": PatternDefinition(
                name="Pattern Name",
                description="What this pattern achieves",
                examples=[
                    ["emission", "reception", "coherence"],  # Validated sequence
                ],
                min_health_score=0.75,
                use_cases=["Use case 1", "Use case 2", "Use case 3"],
                structural_insights=["Why X before Y", "What resonates"],
            ),
        }
```
...
Run the extension validator to ensure quality:
...
- ✓ Example sequence validity
- ✓ Structural correctness

**Acceptance Criteria:**
- Overall validation score ≥ 0.75
- All patterns meet minimum health requirements
- Zero critical issues
- Minimum 3 patterns with 3+ examples each
...
class TestYourDomainExtension:
    def test_pattern_health_scores(self):
        """Verify all patterns meet health requirements."""
...
```bash
pytest tests/extensions/test_your_domain.py -v
```

### Extension Quality Standards

All extensions must meet these standards:

#### Pattern Quality
- Minimum 3 patterns per extension
- Each pattern has ≥ 3 example sequences
- All sequences achieve health score ≥ 0.75
- Real-world use cases documented (minimum 3 per pattern)
- Structural insights explain operator choices

#### Code Quality
- English naming and docstrings (NumPy style)
- Type hints for all public APIs
- Comprehensive test coverage
- No breaking changes to core TNFR
- Follows TNFR architectural conventions
...
- [ ] Extension template completed
- [ ] Extension validator passes (score ≥ 0.75)
- [ ] All patterns validated (health ≥ 0.75)
- [ ] Tests written and passing
- [ ] Documentation complete
- [ ] Example code provided
- [ ] Issue created using "New Domain Extension" template

#### Pull Request Process

1. **Fork and Branch**
   ```bash
   git checkout -b extension-your-domain
   ```

2. **Commit Following Conventions**
   Use the AGENT_COMMIT_TEMPLATE format:
   ```
   Intent: Add [domain] extension with [N] validated patterns
   Operators involved: [extension system]
   Affected invariants: [#10 - domain neutrality maintained]
   Key changes:
   - New extension class for [domain]
   - [N] patterns with health ≥ 0.75
   - Comprehensive tests and documentation
   Expected risks/dissonances: None - extension system isolated from core
   Metrics: All patterns achieve C(t) ≥ 0.75
   ```

3. **Open Pull Request**
   Use the PR template and include:
   - Extension validator output
   - Pattern health scores
   - Testing results
   - Domain expert endorsements (if applicable)

4. **Address Review Feedback**
   Maintainers will review:
   - TNFR paradigm alignment
   - Pattern health and validity
...
#### Pattern Generator
Helps create and validate patterns:

```bash
# Generate pattern template
python tools/community/pattern_generator.py template medical crisis_care

# Validate sequence
python tools/community/pattern_generator.py validate \
    "emission,reception,coherence,coupling"

# Get suggestions
python tools/community/pattern_generator.py suggest "stabilize network"
```

### Extension Examples

Study the reference extensions:

- **Medical Extension**: `src/tnfr/extensions/medical/`
  - Crisis intervention patterns
  - Therapeutic journey sequences
  - Trauma-informed care approaches

- **Business Extension**: `src/tnfr/extensions/business/`
  - Sales cycle patterns
  - Organizational change management
  - Team formation sequences

### Getting Help

- **Template**: See `.github/EXTENSION_TEMPLATE.md` for complete guide
- **Examples**: Study `src/tnfr/extensions/medical/` and `business/`
- **Issues**: Use "New Domain Extension" issue template
- **Discussions**: Ask questions in GitHub Discussions
- **Validation**: Run `extension_validator.py` for quality checks

### Extension Recognition

Community contributors receive:
- Credit in extension metadata
- Mention in release notes
- Recognition in contributor hall of fame
- Opportunity to co-author academic papers
- Speaking opportunities at conferences

---

**Remember**: Extensions enable domain experts to contribute without deep TNFR implementation knowledge. The validation tools ensure quality while lowering barriers to entry.
