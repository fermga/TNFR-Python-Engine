...
- **Share caches, locks, and telemetry** through the provided helpers instead
  of ad-hoc globals. Reuse `tnfr.utils` exports (the replacement for `tnfr.helpers`), `tnfr.cache.CacheManager`,
...
- **Shared services** — `tnfr.utils` (formerly `tnfr.helpers`), `tnfr.cache`, `tnfr.locking`,
