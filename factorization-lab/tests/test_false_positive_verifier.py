...
class FalsePositiveTestCase:
    ...
    def __init__(
        self, n: int, non_factor: int, reason: str, expected_periodic: bool = False
    ):
        self.n = n
...
class FalsePositiveVerifierTestSuite(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_results = {}
        cls._run_full_suite = os.getenv("TNFR_RUN_LONG_TESTS", "").lower() in {
            "1",
            "true",
            "yes",
            "on",
                    ...
                    if candidate > 1 and n % candidate != 0:
                        test_cases.append(
                            FalsePositiveTestCase(
                                n,
                                candidate,
                                f"factor±{delta}: {factor}{delta:+d} for {n}",
                            )
                        )
                            ...
                            if submultiple > 1 and n % submultiple != 0:
                                test_cases.append(
                                    FalsePositiveTestCase(
                                        n,
                                        submultiple,
                                        f"harmonic submultiple {factor}÷{div}",
                                    )
                                )
    ...
    def test_false_positive_resistance_comprehensive(self):
        ...
        for i, test_case in enumerate(test_cases):
            if i % 20 == 0:
                print(
                    f"Progress: {i+1}/{len(test_cases)} ({100*(i+1)//len(test_cases)}%)"
                )
            ...
            try:
                ...
                certified_factors = (
                    getattr(result, "tnfr_certified_factors", None) or []
                )
                ...
                # Track by category
                category = (
                    test_case.reason.split(":")[0]
                    if ":" in test_case.reason
                    else test_case.reason
                )
                if category not in test_summary["categories"]:
                    test_summary["categories"][category] = {
                        "total": 0,
                        "false_positives": 0,
                    }
        ...
        self._generate_false_positive_report(
            test_summary, false_positives, verification_failures
        )

        # Assertions for test success
        false_positive_rate = (
            test_summary["false_positives"] / test_summary["total_cases"]
        )
        ...
        failure_rate = (
            test_summary["verification_failures"] / test_summary["total_cases"]
        )
        self.assertLessEqual(
            failure_rate,
            0.05,
        ...
        print(
            f"  - False positives: {test_summary['false_positives']} ({false_positive_rate:.1%})"
        )
        print(
            f"  - Verification failures: {test_summary['verification_failures']} ({failure_rate:.1%})"
        )
        print(
            f"  - Correct rejections: {test_summary['correct_rejections']} ({test_summary['correct_rejections']/test_summary['total_cases']:.1%})"
        )
    ...
    def test_verification_criteria_strictness(self):
        ...
        self.assertGreaterEqual(
            criteria["dnfr_gain_min"], 0.1, "Should require significant ΔNFR reduction"
        )
    ...
    def test_known_good_factors_still_pass(self):
        ...
        for n, expected_factors in known_factorizations:
            try:
                ...
                discovered_factors = set(
                    getattr(result, "candidate_factors", None) or []
                )
                ...
                for expected_factor in expected_factors:
                    ...
                    if expected_factor in discovered_factors:
                        ...
                        print(
                            f"WARNING: Expected factor {expected_factor} of {n} not surfaced in candidate_factors"
                        )
        ...
        print(
            f"✓ Known good factors recovery: {passed_recovery}/{total_factors} ({verification_rate:.1%})"
        )
    ...
    def test_periodic_pattern_detection_accuracy(self):
        ...
        print(
            f"✓ Periodic pattern detection accuracy: {periodicity_accuracy}/{total_tests} ({accuracy_rate:.1%})"
        )

    def _generate_false_positive_report(
        self, summary: Dict, false_positives: List, failures: List
    ):
        ...
        if summary["categories"]:
            ...
            for category, stats in summary["categories"].items():
                ...
                print(
                    f"  {category}: {stats['false_positives']}/{stats['total']} ({fp_rate:.1%})"
                )
