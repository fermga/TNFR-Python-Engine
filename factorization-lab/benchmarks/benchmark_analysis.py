...
def analyze_runtime_performance(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for suite_name, suite_data in results.items():
        ...
        analysis["suite_performance"][suite_name] = {
            ...
            "speedup_ratio": classical_stats["mean"] / tnfr_stats["mean"] if tnfr_stats["mean"] > 0 else 0,
        ...
        for detail in suite_data["detailed_results"]:
            ...
            classical_times = [r["runtime_ms"] for r in detail["classical_results"] if r["success"]]
            if classical_times:
    ...
    analysis["overall_stats"] = {
        ...
        "classical_runtime": {
            ...
            "median": statistics.median(all_classical_times) if all_classical_times else 0,
            "std": statistics.stdev(all_classical_times) if len(all_classical_times) > 1 else 0
        },
...
def create_performance_visualizations(results: Dict[str, Any], output_dir: Path = None) -> List[str]:
    ...
    ax1.bar([x - 0.2 for x in x_pos], tnfr_means, 0.4, label='TNFR', alpha=0.7, color='blue')
    ax1.bar([x + 0.2 for x in x_pos], classical_means, 0.4, label='Classical', alpha=0.7, color='red')
    ...
    # 2. Win rate by suite
    win_rates = [results[s]["advantage_stats"]["wins"] / results[s]["test_count"] for s in suites]
    ...
    for suite_data in results.values():
        all_advantages.extend([d["tnfr_advantage"] for d in suite_data["detailed_results"]])
    ...
    for i, (suite_name, suite_data) in enumerate(results.items()):
        ...
        for detail in suite_data["detailed_results"]:
            ...
            classical_best = [r["runtime_ms"] for r in detail["classical_results"] if r["success"]]
            if classical_best:
        ...
        valid_pairs = [(t, c) for t, c in zip(tnfr_times, classical_times) if c is not None]
        if valid_pairs:
            t_vals, c_vals = zip(*valid_pairs)
            plt.scatter(t_vals, c_vals, alpha=0.7, color=colors[i % len(colors)], 
                       label=suite_name.replace('_', ' ').title(), s=60)
    
    # Parity line
    max_time = max(max(tnfr_times), max([c for c in classical_times if c is not None] or [0]))
...
"""
        ...
        for detail in suite_data["detailed_results"]:
            ...
            elif max([detail["theoretical_factors"].count(p) for p in set(detail["theoretical_factors"])]) > 1:
                number_type = "power"
            else:
...
def main():
    ...
    try:
        ...
        print(f"TNFR wins: {analysis['overall_stats']['total_wins']} ({analysis['overall_stats']['overall_win_rate']:.1%})")
        print(f"Mean speedup: {analysis['overall_stats']['classical_runtime']['mean'] / analysis['overall_stats']['tnfr_runtime']['mean']:.2f}x")
