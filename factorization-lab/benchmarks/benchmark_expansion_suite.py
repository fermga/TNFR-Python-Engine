...
def pollard_rho_simple(
    n: int, max_iterations: int = 10000
...
def run_tnfr_benchmark(n: int, pure_mode: bool = True) -> TNFRBenchmarkResult:
    ...
    try:
        ...
        return TNFRBenchmarkResult(
            ...
            runtime_ms=runtime,
            partition_count=(
                result.partition_summary.get("partition_count", 0)
                if result.partition_summary
                else 0
            ),
            phi_s=result.phi_s,
...
def run_comparative_benchmark(
    n: int, number_type: str, pure_mode: bool = True
    ...
    best_classical_time = (
        min(classical_runtimes) if classical_runtimes else float("inf")
    )

    tnfr_advantage = (
        best_classical_time / tnfr_result.runtime_ms
        if tnfr_result.runtime_ms > 0
        else 0
    )

    accuracy_comparison = {}
    for result in classical_results:
        accuracy_comparison[result.method] = (
            sorted(result.factors) == theoretical_factors
        )
    accuracy_comparison["tnfr"] = (
        sorted(tnfr_result.tnfr_certified_factors) == theoretical_factors
    )
...
def run_benchmark_suite(
    suite_name: str, pure_mode: bool = True, verbose: bool = False
) -> Dict[str, Any]:
    ...
    for i, n in enumerate(numbers, 1):
        ...
        classical_runtimes = [
            r.runtime_ms for r in benchmark.classical_results if r.success
        ]
        ...
        if any(
            benchmark.accuracy_comparison.get(r.method, False)
            for r in benchmark.classical_results
        ):
            accuracy_stats["classical"] += 1

        if verbose:
            print(
                f"  TNFR: {benchmark.tnfr_result.runtime_ms:.2f}ms, "
                f"factors: {benchmark.tnfr_result.tnfr_certified_factors}"
            )
            print(
                f"  Classical best: {min(classical_runtimes) if classical_runtimes else 'N/A'}ms"
            )
    ...
    print(
        f"Classical Runtime (mean): {summary['classical_runtime_stats']['mean']:.2f}ms"
    )
...
def generate_benchmark_report(
    all_results: Dict[str, Any], output_file: str = None
) -> str:
...
"""
    ...
    total_tnfr_accuracy = statistics.mean(
        [r["tnfr_accuracy"] for r in all_results.values()]
    )
    total_classical_accuracy = statistics.mean(
        [r["classical_accuracy"] for r in all_results.values()]
    )
...
"""
        ...
        for result in results["detailed_results"]:
            classical_times = [
                r["runtime_ms"] for r in result["classical_results"] if r["success"]
            ]
            classical_best = min(classical_times) if classical_times else "N/A"

            tnfr_correct = (
                "✓" if result["accuracy_comparison"].get("tnfr", False) else "✗"
            )
...
def main():
    ...
    suites_to_run = (
        list(BENCHMARK_SUITES.keys()) if "all" in args.suites else args.suites
    )
    ...
    for suite_name in suites_to_run:
        try:
            results = run_benchmark_suite(
                suite_name, pure_mode=pure_mode, verbose=args.verbose
            )
