...
**B1 sub-route status (§13vicies-novies, May 2026)**: Branch B1 has been progressively constrained at the edge-channel level on G_P14. R∞-1a-operator (REMESH iterated in isolation, §13vicies-novies.8) and R∞-1a-composed (REMESH ∘ IL, §13vicies-novies.9) are both structurally and empirically refuted. The **Euler-Orthogonality Lemma** (§13vicies-novies.11) generalises the refutation to any catalog-wide edge-channel composition on the fixed graph G_P14: such compositions commute with the S_n prime-relabelling automorphism and cannot encode Riemann level statistics. Two B1 sub-routes remain structurally permitted: **R∞-1b** (composition on the P14 internal Hilbert space, basis-permutation-non-commuting by construction) and **R∞-1c** (canonically-modified graph with inter-prime edges derived from the nodal equation). R∞-1c is **pre-registered in §13vicies-novies.12** (canonical Kuramoto-U3 augmentation, δ_coh = (γ/π)·k_max·log p_max, F7-A KS-vs-GUE statistic, five locked controls) with a benchmark skeleton at [benchmarks/remesh_infinity_riemann_modified_graph.py](benchmarks/remesh_infinity_riemann_modified_graph.py); not yet executed.
