...
**Net result**: 4 of 5 originally-identified gaps are operationally closed. **G4 = RH remains open**. Extensions beyond P12–P16 now exist inside the canonical engine (P17 Weil–TNFR positivity bridge, P18–P21 admissibility/gauge/family/node-aware sweeps, P22–P24 uniform & adaptive coercivity certificates, P25 Paley-gap diagnostic, P26 Lyapunov-spectral positivity, P27 Hilbert–Pólya scaffold, P28 structural derivation of the smooth zero density, P29 canonical inter-prime coupling sweep, P30 operator-level admissible rescaling); each narrows G4, none closes it. The current TNFR-canonical statement of G4 is **Conjecture T-HP** ([§13septies of research notes](theory/TNFR_RIEMANN_RESEARCH_NOTES.md)): on the tetrad Hilbert space $\mathcal{H}_{\mathrm{tet}}$, P14 supplies a self-adjoint operator with real spectrum $\{k\log p\}$; the open content is the existence and canonicity of an admissible spectral-rescaling operator $\mathcal{F}$ built only from the tetrad $(\Phi_s, |\nabla\phi|, K_\phi, \xi_C)$, canonical constants $(\varphi,\gamma,\pi,e)$, and grammar U1–U6, such that $\mathcal{F}\,H_{P14}\,\mathcal{F}^{*}$ has spectrum $\{\gamma_n\}$. P28 derives the smooth half of $\mathcal{F}$ at the **density** level and P30 ([§13nonies](theory/TNFR_RIEMANN_RESEARCH_NOTES.md)) lifts it to the **operator** level for the smooth half; the residual is the oscillatory term $S(T) = \tfrac{1}{\pi}\arg\zeta(\tfrac12+iT)$, which is RH-equivalent. The step-by-step audit of every link in the assembled argument (L1–L7 closed, L8 = T-HP open) is in [§13octies](theory/TNFR_RIEMANN_RESEARCH_NOTES.md). Whether L8 admits a closure inside the canonical 13-operator catalog (branch B1), demands a new canonical operator derivable from the nodal equation (B2; supported by the P30 negative-enrichment result, §13nonies.4), or has no TNFR closure (B3) is the actual open structural question — **not** a pre-decided "genuinely new mathematics" verdict.

**Implementation Status**: Experimental research framework with computational prototypes in [src/tnfr/riemann/](src/tnfr/riemann/) and documentation in [theory/TNFR_RIEMANN_RESEARCH_NOTES.md](theory/TNFR_RIEMANN_RESEARCH_NOTES.md) (§13 provides the complete milestone → gap → demo map; §13octies the assembled-argument audit).
...
- **Demonstration Code**: [examples/16_riemann_operator_demo.py](examples/16_riemann_operator_demo.py), [examples/18_riemann_convergence_proof.py](examples/18_riemann_convergence_proof.py), [examples/19_topology_comparison.py](examples/19_topology_comparison.py) – [examples/25_analytical_convergence_demo.py](examples/25_analytical_convergence_demo.py) (P2–P10), [examples/41_von_mangoldt_zeta_demo.py](examples/41_von_mangoldt_zeta_demo.py) – [examples/45_li_keiper_demo.py](examples/45_li_keiper_demo.py) (P12–P16), and [examples/46_weil_tnfr_positivity_demo.py](examples/46_weil_tnfr_positivity_demo.py) – [examples/57_admissible_rescaling_demo.py](examples/57_admissible_rescaling_demo.py) (P17–P30)
- **Complete Theory**: [theory/TNFR_RIEMANN_RESEARCH_NOTES.md](theory/TNFR_RIEMANN_RESEARCH_NOTES.md) — §7 (gap analysis), §8 (prime-ladder construction), §9 (P13), §10 (P14), §11 (P15), §12 (P16), §13–§13ter (P22–P25), §13quater–§13nonies (P26–P30), §14–§18 (P17–P21), §19 (consolidated milestone → gap map and honest-scope summary)
