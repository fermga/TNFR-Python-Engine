...
**Net result**: 4 of 5 originally-identified gaps are operationally closed. **G4 = RH remains open**. Extensions beyond P12–P16 now exist inside the canonical engine (P22, P24, P26 Lyapunov-spectral positivity, P27 Hilbert–Pólya scaffold, P28 structural derivation of the smooth zero density, P29 canonical inter-prime coupling sweep); each narrows G4, none closes it. The current TNFR-canonical statement of G4 is **Conjecture T-HP** ([§13septies of research notes](theory/TNFR_RIEMANN_RESEARCH_NOTES.md)): on the tetrad Hilbert space $\mathcal{H}_{\mathrm{tet}}$, P14 supplies a self-adjoint operator with real spectrum $\{k\log p\}$; the open content is the existence and canonicity of an admissible spectral-rescaling operator $\mathcal{F}$ built only from the tetrad $(\Phi_s, |\nabla\phi|, K_\phi, \xi_C)$, canonical constants $(\varphi,\gamma,\pi,e)$, and grammar U1–U6, such that $\mathcal{F}\,H_{P14}\,\mathcal{F}^{*}$ has spectrum $\{\gamma_n\}$. P28 derives the smooth half of $\mathcal{F}$; the residual is the oscillatory term $S(T) = \tfrac{1}{\pi}\arg\zeta(\tfrac12+iT)$, which is RH-equivalent. The step-by-step audit of every link in the assembled argument (L1–L7 closed, L8 = T-HP open) is in [§13octies](theory/TNFR_RIEMANN_RESEARCH_NOTES.md). Whether L8 admits a closure inside the canonical 13-operator catalog (branch B1), demands a new canonical operator derivable from the nodal equation (B2), or has no TNFR closure (B3) is the actual open structural question — **not** a pre-decided "genuinely new mathematics" verdict.
...
- **Demonstration Code**: [examples/16_riemann_operator_demo.py](examples/16_riemann_operator_demo.py), [examples/41_von_mangoldt_zeta_demo.py](examples/41_von_mangoldt_zeta_demo.py), [examples/42_riemann_zeros_as_resonances.py](examples/42_riemann_zeros_as_resonances.py), [examples/43_prime_ladder_hamiltonian_demo.py](examples/43_prime_ladder_hamiltonian_demo.py), [examples/44_weil_explicit_formula_demo.py](examples/44_weil_explicit_formula_demo.py), [examples/45_li_keiper_demo.py](examples/45_li_keiper_demo.py)
- **Complete Theory**: [theory/TNFR_RIEMANN_RESEARCH_NOTES.md](theory/TNFR_RIEMANN_RESEARCH_NOTES.md) — §7 (gap analysis), §8 (prime-ladder construction), §9 (analytic continuation, P13), §10 (self-adjoint prime-ladder Hamiltonian, P14), §11 (Weil–Guinand explicit formula, P15), §12 (Li–Keiper positivity criterion, P16)
