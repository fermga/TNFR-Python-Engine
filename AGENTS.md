...
**B1 sub-route status (§13vicies-novies, May 2026)**: Branch B1 has been progressively constrained at the edge-channel level on G_P14. R∞-1a-operator (REMESH iterated in isolation, §13vicies-novies.8) and R∞-1a-composed (REMESH ∘ IL, §13vicies-novies.9) are both structurally and empirically refuted. The **Euler-Orthogonality Lemma** (§13vicies-novies.11) generalises the refutation to any catalog-wide edge-channel composition on the fixed graph G_P14: such compositions commute with the S_n prime-relabelling automorphism and cannot encode Riemann level statistics. **R∞-1c** (canonically-modified graph with inter-prime edges, pre-registered §13vicies-novies.12 and executed §13vicies-novies.13) returns `INDETERMINATE_DEGENERATE_CONSTRUCTION` with |D_canonical − D_shuffled| = 3.13×10⁻¹³ (machine-precision zero), confirming that the S_n symmetry obstruction extends to the augmented graph whenever the inter-prime weight law depends only on S_n-invariant combinations of (ν_f, p) — i.e. the modified-graph specialisation of the Euler-Orthogonality Lemma. The unique remaining structurally permitted B1 sub-route is **R∞-1b** (composition on the P14 internal Hilbert space, basis-permutation-non-commuting by construction, NOT subject to the Euler-Orthogonality argument, NOT yet pre-registered or tested).
