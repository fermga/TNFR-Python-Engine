...
- **Per-Node Qubit Geometry**: [examples/106_per_node_qubit_geometry.py](examples/106_per_node_qubit_geometry.py) (the substrate's hidden U(2) isospin / Hopf map S³→S²: each node = a Bloch vector of radius=energy (exact); isospin conserved under the diagonal flow; UM collapses it by phase-sync, ΔNFR-lever ops tilt it, AL/EN/RA/SHA/VAL/REMESH preserve it; HONEST: classical spin texture / product state, NO entanglement — geometry not quantum)
