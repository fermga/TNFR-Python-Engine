...
- **νf-Embedded Prime Visibility**: [examples/07_number_theory/116_nuf_emergent_prime_visibility.py](examples/07_number_theory/116_nuf_emergent_prime_visibility.py) - arithmetic enters ONLY through νf on an arithmetic-neutral graph (no imposed divisibility edges); the structural-diffusion channel sees the νf-prime imprint (r_pb≈0.25–0.34, shuffle-controlled, 3 seeds) while ΔNFR-derived substrate fields stay blind; νf-as-mobility echo, not emergence
...
- **νf-Embedded Prime Visibility**: [examples/07_number_theory/116_nuf_emergent_prime_visibility.py](examples/07_number_theory/116_nuf_emergent_prime_visibility.py) (the doctrine-faithful pivot: arithmetic enters ONLY through νf on an arithmetic-NEUTRAL Watts–Strogatz graph — no imposed divisibility/GCD edges. MEASURED, 3 seeds: the structural-DIFFUSION channel (current divergence/L1) acquires a clean prime signature r_pb≈0.25–0.34 / Cohen d≈0.6–0.9 in the prime scenario, collapsing to ≈0 under the prime-shuffled control (same νf histogram, primality alignment broken); the ΔNFR-derived substrate fields Φ_s/J_ΔNFR stay blind. MECHANISM = νf-as-mobility echo: νf is the per-node diffusivity in EPI+=dt·νf·ΔNFR, so prime nodes develop a distinctive EPI transient where νf acts; the geometry re-expresses what you inject through νf, it does NOT independently discover primes — mirrors example 103 and the REMESH-∞ "universality is structural/operational, not spectral". Surfaced + fixed a real cache bug: Φ_s/ξ_C/J_ΔNFR cache key was blind to ΔNFR (alias-key mismatch in _compute_dependency_hash), contaminating the first run with stale-identical substrate fields; characterization, not a closure)
