...
| **TNFR-Navier–Stokes** | `K_φ` cascade and a BKM-analogue U2 blow-up criterion; NS advection+viscosity maps faithfully to a VAL-vs-IL U2 enstrophy budget read in structural time `τ_str = ν_f·t` (fixed-Re compliant; peak debt grows with Re — relocates Clay, closes nothing); 2D asymmetry closed at the discrete level; continuum closure NS-G1 open. | [TNFR_NAVIER_STOKES_RESEARCH_NOTES.md](theory/TNFR_NAVIER_STOKES_RESEARCH_NOTES.md) |
