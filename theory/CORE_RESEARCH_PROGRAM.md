...
| S16 | Observable, stable and scale-persistent NFR structure | Endpoint and sampled-path certificates compose restricted pure-EPI hypotheses, stable forward-Euler updates and a common switching metric. All thirteen glyphs have atomic all-target Jacobi stages. Executed schedules and individually atomic event/REMESH cycles feed a sealed finite three-mesh comparison. The uniform exact companion theorem closes augmented-history monotonicity and temporal convergence on fixed support/metric without clipping; an adjacent-cycle layer gives a finite represented history telescope. A new outer executor binds that telescope to one graph and one causal finite invocation with graph-owned atomicity. A block observer extracts exact normalized lower margins from its nonempty contiguous boundary blocks without promoting them to class-wide results. A distinct conditional exact policy theorem proves a uniform `1-q` spatial-disagreement margin, prefix gain upper bound one and repeated geometric decay for common-gain schedule families on the fixed companion. The general reversible exact mode is also bound offline to finite executor-owned pressure-refreshed partitions with complete-matrix defect propagation. Promotion of that policy theorem to a forward-invariant binary64 executor class, generic runtime convergence, full graph/grammar-history continuity and changing node support remain open |
...
It does not inspect schedule maps or identify the binary64 executor. The next
runtime target is to prove a forward-invariant fixed-support/metric class with
uniform schedule gain and centered relative bounds on rounding and clipping
defects strong enough to preserve a positive net block margin.
...
| --- | --- | --- |
| S1-S2 | Promote the exact common-`q` REMESH/schedule theorem to a declared forward-invariant runtime class using uniform centered relative defect bounds | The runtime class is not forward invariant, its effective gain or relative defects approach the zero-margin boundary, exact history/metric compatibility fails, or only absolute defect bounds are available |
...
| S16 | Bind a runtime class to the exact common-`q` policy theorem without losing its positive net spatial-disagreement margin | Support, metric or policy changes; binary64/clipping defects lack a uniform relative bound; or the resulting block factor reaches one |
...
- promote the exact common-`q` theorem to a declared forward-invariant runtime
  class by deriving centered relative rounding/clipping defect bounds and a
  strictly positive net block margin;
