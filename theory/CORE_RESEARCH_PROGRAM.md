...
over a declared forward-invariant class, an intrablock prefix-amplification
...
This closes measurement for one finite causal block, not a repeated theorem.
Equilibrium has zero drop, and amplitude scaling drives every quadratic
absolute drop to zero, so a uniform positive absolute margin is unavailable.
The meaningful next target is a uniform positive normalized block margin over
a declared forward-invariant class together with a finite bound on all
intrablock prefix amplification.
...
| --- | --- | --- |
| S1-S2 | Prove or refute a uniform positive normalized block margin over a declared forward-invariant runtime class, together with a finite intrablock prefix-amplification bound | The class is not forward invariant, normalized margins approach zero, one prefix has unbounded relative amplification, exact history/metric compatibility fails, or the proof multiplies fixed-history REMESH factors |
...
| S16 | Extend the finite causal block observer to a declared repeatable class with a uniform positive normalized block margin and finite intrablock prefix bound | The proposed class is not forward invariant, includes the `alpha=1` zero-margin orbit, admits normalized margins tending to zero, or has unbounded prefix amplification |
...
finite values do not define a repeatable class. Absolute uniform decrease is
already blocked by equilibrium and amplitude scaling. The next stability
question is whether a declared forward-invariant class has a uniform positive
normalized block margin and bounded intrablock prefix amplification.

Other open branches remain:

- derive a uniform positive normalized block margin on a declared
  forward-invariant class plus a finite intrablock prefix-amplification bound,
  while retaining rounding, soft-clipping and pure-delay counterexamples;
