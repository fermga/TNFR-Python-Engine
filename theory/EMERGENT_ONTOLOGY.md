...
signal→scoped-TNFR-read-out confrontation pipeline,
...
1. **The vacuum.** `EPI = 0`; the nodal equation `∂EPI/∂t = νf·ΔNFR` remains mathematically
 defined for finite capacity and pressure, while grammar **U1a** requires a generator `{AL, NAV, REMESH}`
   ...
   spontaneous event.
2. **Emission — the first form.** `AL` sources `EPI` from the vacuum while
   retaining the pre-existing basal `νf`; it does not activate or otherwise
   write structural capacity. Measured `EPI` rises monotonically from `0`
   (`0 → 0.09 → … → 0.50`) — form where there was none.
