...
__all__ = [
    "AdmittedSpecification", "CoefficientBlocks", "Obligation",
    "parse_rational", "specification_digest", "admit_specification",
    "parse_coefficients", "affine_form", "point_constraint", "root_constraint",
    "exact_affine_minimum", "exact_minimum", "verify_candidate",
]
...
def parse_rational(value, *, max_bits=4096):
    ...
    if type(value) is not str:
        raise ValueError("Exact rational inputs must be integers, strings or Fractions; floats/bools are forbidden")
    ...
    if ratio:
        ...
        if exponent is not None and (len(exponent.lstrip('+-')) > 6 or abs(int(exponent)) > digits):
...
def _integer(value, label, max_bits=4096, nonnegative=False):
    if type(value) is not int or abs(value).bit_length() > max_bits or (nonnegative and value < 0):
        raise ValueError(f"{label} must be an exact {'nonnegative ' if nonnegative else ''}bounded integer")
    return value
    ...
    while pending:
        ...
        if type(value) is dict:
            if len(value) > 1_000_000 or any(type(key) is not str for key in value):
                raise ValueError("Specification JSON needs bounded string-keyed objects")
...
def _canonical_bytes(raw):
    ...
    _json_preflight(raw)
    return json.dumps(raw, sort_keys=True, separators=(',', ':'), allow_nan=False).encode('utf8')
...
def _bounds(raw, max_bits):
    ...
    bounds = tuple(tuple(_integer(value, "DBM bound", max_bits) for value in _sequence(row, 7, "DBM row")) for row in rows)
    return bounds
...
def _closed(bounds):
    ...
    if any(bounds[i][j] > bounds[i][k] + bounds[k][j] for i in range(7) for j in range(7) for k in range(7)):
...
def _fraction_pair(value):
    if type(value) is not Fraction or type(value.numerator) is not int or type(value.denominator) is not int:
...
def _specification(spec):
    ...
    if any(getattr(spec, name) is not value for name, value in zip(_SPEC_FIELDS, fields)):
...
def _coefficient_blocks(spec, blocks):
    _specification(spec)
    if type(blocks) is not CoefficientBlocks or blocks not in _COEFFICIENTS or blocks._spec is not spec:
        raise ValueError("Coefficients must be parsed for this admitted specification")
    fields, _values = _COEFFICIENTS[blocks]
    if any(getattr(blocks, name) is not value for name, value in zip(_BLOCK_FIELDS, fields)):
...
def _obligation(spec, obligation):
    _specification(spec)
    if (type(obligation) is not Obligation or type(obligation.ordinal) is not int
    ...
    if any(getattr(obligation, name) is not value for name, value in zip(_OBLIGATION_FIELDS, recorded)):
        raise ValueError("Admitted obligation fields were mutated")


def admit_specification(raw, expected_digest, *, max_vertices=10000, max_obligations=100000,
                        max_guards=100000, max_integer_bits=4096):
    ...
    """
    for value, label in ((max_vertices, "max_vertices"), (max_obligations, "max_obligations"), (max_guards, "max_guards")):
        _limit(value, label)
    ...
    required = {'normalization', 'coefficient_vertices', 'guards', 'root', 'drift_obligations', 'target_obligations'}
    ...
    normalization = _integer(raw['normalization'], "Normalization", max_integer_bits, True)
    if normalization == 0:
        raise ValueError("Normalization must be positive")
    guard_records = raw['guards']
    if type(guard_records) not in (list, tuple) or not 0 < len(guard_records) <= max_guards:
    ...
    for ordinal, record in enumerate(guard_records):
        ...
        if _integer(record['guard_id'], "Guard identity", max_integer_bits, True) != ordinal:
    ...
    root_point = tuple(_integer(x, "Root coordinate", max_integer_bits) for x in _sequence(root['point'], 6, "Root point"))
    ...
    for kind, key in (('drift', 'drift_obligations'), ('target', 'target_obligations')):
        records = raw[key]
        if type(records) not in (list, tuple) or len(records) + len(obligations) > max_obligations:
        ...
        for record in records:
            ...
            keys = (identity, 'source_vertex', 'target_vertex', 'guard_id', 'shift') if kind == 'drift' else (identity, 'vertex', 'guard_id')
            _mapping(record, keys, kind + ' obligation')
            index = _integer(record[identity], "Obligation identity", max_integer_bits, True)
            if index in seen:
                raise ValueError("Duplicate obligation identity")
            seen.add(index)
            guard_id = _integer(record['guard_id'], "Guard identity", max_integer_bits, True)
            if guard_id >= len(guards):
                raise ValueError("Unknown guard identity")
            if kind == 'drift':
                source = _integer(record['source_vertex'], "Source vertex", max_integer_bits, True)
                target = _integer(record['target_vertex'], "Target vertex", max_integer_bits, True)
                shift = tuple(_integer(x, "Shift coordinate", max_integer_bits) for x in _sequence(record['shift'], 6, "Shift"))
            else:
                source = target = _integer(record['vertex'], "Target vertex", max_integer_bits, True)
                shift = (0,) * 6
            if source not in columns or target not in columns:
                raise ValueError("Obligation lies outside coefficient domain")
            item = Obligation(kind, index, len(obligations), guard_id, source, target, shift)
            family.append(item)
            obligations.append(item)
        families.append(tuple(family))
    spec = AdmittedSpecification(expected_digest, vertices, normalization, tuple(guards), root_vertex,
                                 root_point, root_minimum, families[0], families[1], tuple(obligations), MappingProxyType(columns))
    _SPECIFICATIONS[spec] = (tuple(getattr(spec, name) for name in _SPEC_FIELDS), _fraction_pair(root_minimum),
                             tuple(tuple(getattr(item, name) for name in _OBLIGATION_FIELDS) for item in obligations))
    return spec
...
def parse_coefficients(spec, records, *, max_bits=256):
    ...
    for vertex, record in zip(spec.vertices, records):
        ...
        if _integer(record['vertex'], "Coefficient vertex", _MAX_BITS, nonnegative=True) != vertex:
            raise ValueError("Missing, reordered or mismatched coefficient block")
        gradient = tuple(parse_rational(x, max_bits=max_bits) for x in _sequence(record['gradient'], 6, "Gradient"))
    ...
    _COEFFICIENTS[blocks] = (tuple(getattr(blocks, name) for name in _BLOCK_FIELDS), tuple(map(_fraction_pair, flat)))
    return blocks
...
def _point(raw, max_bits=_MAX_BITS):
    return tuple(_integer(x, "Point coordinate", max_bits) for x in _sequence(raw, 6, "Point"))
...
def _point_row(spec, terms, point):
    ...
    for start, sign, shift in terms:
        for i in range(6):
            row[start + i] = row.get(start + i, Fraction(0)) + Fraction(sign * (point[i] + shift[i]), spec.normalization)
...
def root_constraint(spec):
    ...
    _specification(spec)
    return _point_row(spec, ((spec._columns[spec.root_vertex], 1, (0,) * 6),), spec.root_point), spec.root_minimum
...
def _multipliers(raw, max_bits):
    ...
    for item in raw:
        ...
        if type(i) is not int or type(j) is not int or not 0 <= i < 7 or not 0 <= j < 7 or i == j:
...
def _exact_minimum(bounds, linear, constant, max_bits):
    if not __debug__:
        raise RuntimeError("The exact extrema owner requires enabled assertions; optimized Python is unsupported")
    from ..physics.c6_carried_excursion import _linear_extremum

    linear = tuple(parse_rational(x, max_bits=max_bits) for x in _sequence(linear, 6, "Linear form"))
    ...
    if factor <= 0 or any(value != factor * weight for value, weight in zip(linear, weights)):
    ...
    if any(type(amount) is not int or amount <= 0 for _i, _j, amount in owner.dual_flow):
        raise RuntimeError("Exact owner returned a nonpositive or noninteger transport amount")
    multipliers = _multipliers([(i, j, factor * amount) for i, j, amount in owner.dual_flow], max_bits)
    minimum = constant + factor * owner.value
    if constant + sum(value * coordinate for value, coordinate in zip(linear, point)) != minimum:
    ...
    sources, sinks = [i for i, x in enumerate(signed) if x > 0], [i for i, x in enumerate(signed) if x < 0]
    possible = len(sources) * len(sinks)
    if possible > 12 or len(multipliers) > possible or any(not (signed[i] > 0 > signed[j] and value > 0) for i, j, value in multipliers):
        raise RuntimeError("Exact owner lost the signed transport support")
    return dict(coefficients=list(map(str, linear)), constant=str(constant), primitive_integer_weights=weights,
                positive_rational_weight_scale=str(factor), owner_integer_minimum=owner.value, minimum=str(minimum),
                attaining_point=point, multipliers=[(i, j, str(value)) for i, j, value in multipliers],
                nonnegative_certified=minimum >= 0, negative_attaining_witness=minimum < 0,
                exact_primal_dual_equality_checked=True, local_lower_bound_checker_passed=True,
                transport_positive_indices=sources, transport_negative_indices=sinks, possible_transport_edges=possible,
                used_transport_edges=len(multipliers), existing_integer_extremum_owner_calls=1,
                existing_min_cost_flow_calls=int(any(weights)), max_rational_bits=max_bits,
...
def verify_candidate(spec, candidate, *, max_bits=4096, collect_failures=False):
    ...
    _mapping(candidate, ('schema_version', 'specification_sha256', 'coefficients', 'drift_proofs', 'target_proofs'), "candidate")
    ...
    if type(candidate_digest) is not str or not _DIGEST.fullmatch(candidate_digest) or candidate_digest != spec.digest:
    ...
    parsed_proofs, terms, largest = [], 0, max(blocks.largest_bits, _bits(spec.root_minimum))
    for family, key in ((spec.drift_obligations, 'drift_proofs'), (spec.target_obligations, 'target_proofs')):
        ...
        for obligation, proof in zip(family, proofs):
            ...
            if _integer(proof[identity], "Proof identity", _MAX_BITS, nonnegative=True) != obligation.index:
    ...
    def failure(kind, index, reason):
        ...
        failures.append(dict(kind=kind, **({'vertex': index} if kind == 'root' else {'index': index}), reason=reason))

    root_row, minimum = root_constraint(spec)
    root_value = sum(value * _coefficient_value(blocks, column) for column, value in root_row)
    ...
    for obligation, multipliers in zip(spec.obligations, parsed_proofs):
        linear, constant = affine_form(spec, blocks, obligation)
        try:
            lower = _dual_lower(spec.guards[obligation.guard_id], linear, constant, multipliers)
        except ValueError as error:
            failure(obligation.kind, obligation.index, str(error))
        else:
            if lower < 0:
                failure(obligation.kind, obligation.index, 'The supplied affine lower bound is negative')
    return dict(accepted=not failures, specification_sha256=spec.digest, root_value=str(root_value),
                required_root_minimum=str(minimum), coefficient_blocks=len(spec.vertices), coefficient_count=spec.coefficient_count,
                distinct_guard_count=len(spec.guards), checked_root_obligations=1,
                checked_drift_obligations=len(spec.drift_obligations), checked_target_obligations=len(spec.target_obligations),
                checked_sparse_dual_terms=terms, parsed_rational_inputs=spec.coefficient_count + 1 + terms,
                largest_rational_input_bits=largest, max_rational_bits=max_bits, failures=failures,
                original_target_coverage_verified=False, whole_C6_first_exit_label_excluded=False,
