...
class TorchBackend(TNFRBackend):
    ...
    def compute_delta_nfr(
        ...
        *,
        ...
        phase = np.array([graph.nodes[node].get('phase', 0.0) for node in node_list], dtype=np.float32)
        ...
        delta_nfr_t = self._compute_delta_nfr_torch(
            phase_t, epi_t, vf_t, edge_src_t, edge_dst_t,
            w_phase, w_epi, w_vf, w_topo, graph.is_directed()
        )
