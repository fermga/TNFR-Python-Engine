...
3. has **trace-class resolvent** :math:`(\hat H + c\hat I)^{-1}`
   for any :math:`c > 0`,
4. has a unitary flow :math:`U(t) = e^{-it\hat H}` that preserves the
   unit-norm sphere and spectral energy :math:`\|H\psi\|^2`.
...
* Unitary spectral-energy conservation.  Under the unitary flow
  ...
  numerically on a battery of random initial states. This invariant is
  distinct from the five-field structural snapshot-energy candidate and
  supplies no dissipative monotonicity theorem.
