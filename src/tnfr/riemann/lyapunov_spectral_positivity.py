...
3. has **trace-class resolvent** :math:`(\hat H + c\hat I)^{-1}` for
   any :math:`c > 0`,
4. has a unitary flow :math:`U(t) = e^{-i t \hat H}` that preserves
   the unit-norm sphere and spectral energy :math:`\|H\psi\|^2`.
...
* Unitary spectral-energy conservation.  Under the unitary flow
  ...
  initial states. This Hamiltonian invariant and the five-field
  structural snapshot-energy candidate in :mod:`tnfr.physics.conservation`
  are different functionals. No dissipative restriction or inherited
  monotonicity is established here.
