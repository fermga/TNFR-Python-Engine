...
class HierarchicalTNFRNetwork:
    ...
    def compute_multiscale_dnfr(
        self, node_id: NodeId, target_scale: str
    ) -> DeltaNFR:
        ...
        for other_scale in self.networks_by_scale:
            ...
            coupling = self.cross_scale_couplings.get(
                (target_scale, other_scale), 0.0
            )
            if coupling > 0:
                # Aggregate ΔNFR from other scale
                other_G = self.networks_by_scale[other_scale]
                other_dnfr_values = [
                    other_G.nodes[n].get("delta_nfr", 0.0)
                    for n in other_G.nodes()
                ]
    ...
    def compute_total_coherence(self) -> float:
        ...
        for scale_name, G in self.networks_by_scale.items():
            ...
            dnfr_values = [
                abs(G.nodes[n].get("delta_nfr", 0.0))
                for n in G.nodes()
            ]
    ...
    def evolve_multiscale(
        self, dt: float = 0.1, steps: int = 10, operators: Optional[Sequence[str]] = None
    ) -> EvolutionResult:
