...
class _EPIValidators:
    @classmethod
    def validate_domain(cls, f_continuous: Sequence[complex] | np.ndarray, a_discrete: Sequence[complex] | np.ndarray, x_grid: Sequence[float] | np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]: ...
...
class BEPIElement(_EPIValidators):
    ...
    def compose(self, transform: Callable[[np.ndarray], np.ndarray], *, spectral_transform: Callable[[np.ndarray], np.ndarray] | None = None) -> BEPIElement: ...
...
def evaluate_coherence_transform(element: BEPIElement, transform: Callable[[BEPIElement], BEPIElement], *, kappa: float = 1.0, tolerance: float = 1e-09, space: BanachSpaceEPI | None = None, norm_kwargs: Mapping[str, float] | None = None) -> CoherenceEvaluation: ...
