...
from .._exact_time import (
    exact_or_represented_real as _rational,
...
__all__ = [
    "P2MeasurementBounds", "P2IntervalCalibration", "P2IntervalForecast",
    "P2IntervalComparison", "calibrate_p2_transport", "forecast_p2_transport",
    "score_p2_transport", "write_p2_transport_forecast",
]
...
class P2MeasurementBounds:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "structural_time_per_unit", _positive_interval(
...
def _coordinates(pair, measurement):
    ...
    return tuple(
        ((value - offset) / scale - error,
         (value - offset) / scale + error)
        for value, offset, scale, error in zip(
            values, measurement.offsets, measurement.scales,
            measurement.epi_error, strict=True)
    )
...
def _elapsed(timestamps, measurement):
    ...
    for time in timestamps[1:]:
        ...
        result.append((lower * measurement.structural_time_per_unit[0],
...
class P2IntervalCalibration:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "capacity", _positive_interval(
            self.capacity, "capacity"))
        for name in ("calibration_run_ids", "calibration_acquisition_ids",
                     "calibration_hashes"):
            object.__setattr__(self, name, _identity_sequence(
                getattr(self, name), name))
        n = len(self.calibration_run_ids)
        if (len(set(self.calibration_run_ids)) != n
...
def calibrate_p2_transport(runs, *, graph, measurement, support_provenance):
    ...
    if not isinstance(measurement, P2MeasurementBounds):
        raise TypeError("measurement must be P2MeasurementBounds")
    first = runs[0]
    if (len(first.channel_ids) != 2 or tuple(graph) != first.channel_ids
            or graph.is_directed() or graph.is_multigraph()
    ...
    for run in runs:
        ...
        enclosures.append(bound_p2_capacity(
            ...
            elapsed_time=_elapsed(
                (run.timestamps[0], run.timestamps[-1]), measurement)[-1]).capacity)
    ...
    return P2IntervalCalibration(
        first.channel_ids, measurement, (Fraction(lo), Fraction(hi)),
        tuple(run.run_id for run in runs),
        tuple(run.acquisition_id for run in runs),
        tuple(run.content_hash for run in runs), first.value_unit,
        first.time_unit, support_provenance,
    )
...
class P2IntervalForecast:
    ...
    def __post_init__(self):
        for name in ("calibration_hash", "evaluation_run_id",
        ...
        if (not isinstance(self.tube, P2TransportTube)
                or len(self.tube.samples) != len(self.timestamps)):
...
def forecast_p2_transport(calibration, *, evaluation_run_id,
                          evaluation_acquisition_id, initial_measurement,
                          timestamps):
    ...
    if (evaluation_run_id in calibration.calibration_run_ids
            or evaluation_acquisition_id
            in calibration.calibration_acquisition_ids):
    ...
    return P2IntervalForecast(calibration.content_hash, evaluation_run_id,
                              evaluation_acquisition_id, initial, times, tube)
...
def score_p2_transport(forecast, calibration, observation, *,
    ...
    for k, sample in enumerate(forecast.tube.samples[1:], 1):
        pair = _coordinates(tuple(row[k] for row in observation.samples),
                            calibration.measurement)
        for node in range(2):
            if disjoint(pair[node], sample.epi[node]):
                nodes.append((k, node))
        mean = ((pair[0][0] + pair[1][0]) / 2,
                (pair[0][1] + pair[1][1]) / 2)
        ...
        if disjoint(contrast, sample.contrast):
            contrasts.append(k)
    status = ("incompatible_with_declared_bounds" if nodes or means or contrasts
              else "not_falsified_by_enclosures")
    return P2IntervalComparison(forecast.content_hash, observation.content_hash,
                                status, tuple(nodes), tuple(means), tuple(contrasts))
...
def write_p2_transport_forecast(forecast, path):
    ...
    payload = {"forecast": _payload(asdict(forecast)),
