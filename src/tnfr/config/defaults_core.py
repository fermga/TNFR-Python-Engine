...
from ..constants.canonical import (  # Configuration constants (magic number replacements); Tetrad field thresholds (single canonical source)
    AL_BOOST_CANONICAL,
    DOWN_CANONICAL,
    DT_CANONICAL,
    DT_MIN_CANONICAL,
...
STRUCTURAL_ESCAPE_THRESHOLD = (
    U6_STRUCTURAL_POTENTIAL_LIMIT  # φ ≈ 1.618 (canonical golden escape threshold)
)
...
class CoreDefaults:
    ...
    DNFR_WEIGHTS: dict[str, float] = field(
        default_factory=lambda: {
            "phase": round(
                PHI_GAMMA_NORMALIZED, 3
            ),  # φ/(φ+γ) ≈ 0.737 (dominant golden-euler)
            "epi": round(GAMMA_PI_RATIO, 3),  # γ/(π+γ) ≈ 0.155 (Euler-pi stabilizer)
            "vf": round(
                PI_MINUS_E_OVER_PI * (2 / 3), 3
            ),  # ((π-e)/π) * (2/3) ≈ 0.090 (moderate transcendental)
    ...
    SI_WEIGHTS: dict[str, float] = field(
        default_factory=lambda: {
            "alpha": round(
                PHI_GAMMA_NORMALIZED, 3
            ),  # φ/(φ+γ) ≈ 0.737 (golden coherence)
            "beta": round(GAMMA_PI_RATIO, 3),  # γ/(π+γ) ≈ 0.155 (Euler stability)
            "gamma": round(GAMMA / (PHI * PI), 3),  # γ/(φ×π) ≈ 0.114 (notational)
        }
    )
    PHASE_K_GLOBAL: float = round(
        GAMMA_OVER_PI_PLUS_E / 2, 4
    )  # γ/(π+e) / 2 ≈ 0.0493 (canonical global phase coupling)
    PHASE_K_LOCAL: float = round(
        GAMMA_PI_RATIO, 3
    )  # γ/(π+γ) ≈ 0.155 (canonical local phase coupling)
    ...
    UM_COMPAT_THRESHOLD: float = (
        UM_COMPAT_THRESHOLD  # φ/(φ+γ) ≈ 0.7371 (canonical golden-Euler compatibility)
    )
    ...
    GLYPH_SELECTOR_MARGIN: float = (
        GLYPH_SELECTOR_MARGIN_CANONICAL  # γ/(π×e×φ) ≈ 0.0418 (boundary precision)
    )
    VF_ADAPT_TAU: int = 5
    VF_ADAPT_MU: float = (
        VF_ADAPT_MU_CANONICAL  # γ/(π+e) ≈ 0.0985 (transcendental adaptation)
    )
    ...
    GLYPH_FACTORS: dict[str, float] = field(
        default_factory=lambda: {
            ...
            "IL_dnfr_factor": round(
                PHI_GAMMA_NORMALIZED, 3
            ),  # φ/(φ+γ) ≈ 0.737 (golden coherence)
            "OZ_dnfr_factor": round(
                NUL_DENSIFICATION_FACTOR, 3
            ),  # φ/γ ≈ 2.803 (golden dissonance)
    ...
    GLYPH_THRESHOLDS: dict[str, float] = field(
        default_factory=lambda: {
            "hi": round(2.0 / (PI + GAMMA), 3),
            "lo": round(1.0 / (PI + GAMMA), 3),
            "dnfr": 1e-3,
        }  # notational hi/lo thresholds
    )
    ...
    GRAMMAR: dict[str, Any] = field(
        default_factory=lambda: {
            ...
            "force_dnfr": round(
                PI / (E + PHI + GAMMA), 3
            ),  # π/(e+φ+γ) ≈ 0.639 (notational force threshold)
            "force_accel": round(
                PI / (E + PHI + GAMMA), 3
            ),  # π/(e+φ+γ) ≈ 0.639 (notational acceleration threshold)
            "fallbacks": {"ZHIR": "NAV", "OZ": "ZHIR", "THOL": "NAV"},
        }
    )
    SELECTOR_WEIGHTS: dict[str, float] = field(
        default_factory=lambda: {
            "w_si": round(PI / (PI + E), 3),
            "w_dnfr": round(1.0 / (PI + 1.0), 3),
            "w_accel": round(GAMMA / (PI + 1.0), 3),
        }  # notational selector weights: π/(π+e), 1/(π+1), γ/(π+1)
    )
    ...
    VAL_MIN_EPI: float = (
        VAL_MIN_EPI  # γ/(π+γ) ≈ 0.1552 (canonical minimum structural base)
    )
...
class RemeshDefaults:
    ...
    FRACTION_STABLE_REMESH: float = round(
        4.0 / (E + PHI), 3
    )  # 4/(e+φ) ≈ 0.922 (notational stable fraction)
    ...
    REMESH_MIN_PHASE_SYNC: float = round(
        (E * PHI) / (PI + E), 3
    )  # (e×φ)/(π+e) ≈ 0.751 (notational phase sync threshold)
    REMESH_MAX_GLYPH_DISR: float = round(
        1.0 / (PI + GAMMA), 3
    )  # 1/(π+γ) ≈ 0.269 (notational maximum glyph disruption)
    REMESH_MIN_SIGMA_MAG: float = round(
        PI / (PI + E), 3
    )  # π/(π+e) ≈ 0.536 (notational sigma magnitude threshold)
    REMESH_MIN_KURAMOTO_R: float = round(
        4.0 / (E + PHI), 3
    )  # 4/(e+φ) ≈ 0.922 (notational Kuramoto-R threshold)
    REMESH_MIN_SI_HI_FRAC: float = round(
        PI / (PI + E), 3
    )  # π/(π+e) ≈ 0.536 (notational SI high fraction)
...
MIN_BUSINESS_COHERENCE = (
    MIN_BUSINESS_COHERENCE_CANONICAL  # (e×φ)/(π+e) ≈ 0.7506 (notational)
)
MIN_BUSINESS_SENSE_INDEX = round(
    1 / PHI + 0.082, 3
)  # ≈ 0.700 (CALIBRATED target; the "1/φ + 0.082" form is cosmetic — an empirical value, not derived)
...
CRITICAL_INFORMATION_DENSITY = (
    E * PHI / PI
)  # (e×φ)/π ≈ 1.400 (notational critical density)
