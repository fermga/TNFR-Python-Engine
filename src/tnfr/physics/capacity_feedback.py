...
__all__ = [
    "P2CapacityFeedbackReference", "P2CapacityFeedbackCycle", "P2CapacityFeedbackBound",
    "derive_p2_capacity_feedback", "observe_p2_capacity_feedback_cycle",
    "bound_p2_capacity_feedback",
    "P2Binary64CouplingReference", "P2Binary64CouplingObservation",
...
def derive_p2_capacity_feedback(
    *, base_capacity, epi_weight, vf_weight, coupling_factor, timestep,
    max_capacity_gap, epi_lower, epi_upper,
...
"""
    inputs = (
        ("base_capacity", base_capacity), ("epi_weight", epi_weight),
        ("vf_weight", vf_weight), ("coupling_factor", coupling_factor),
        ("timestep", timestep), ("max_capacity_gap", max_capacity_gap),
    ...
    if c <= 0 or e <= 0 or w < 0:
        raise ValueError("base capacity and EPI weight must be positive; vf weight nonnegative")
    if not 0 < gamma < 1:
    ...
    if ratio * limit > upper - lower:
        raise ValueError("capacity shift must fit inside the declared invariant EPI box")
    if step * (c + retention * limit) > 1:
        raise ValueError("post-Coupling Euler coefficients must be convex on the whole box")
    ...
    return P2CapacityFeedbackReference(
        c, e, w, gamma, h, limit, lower, upper,
        ratio, retention, step, beta, beta < 1,
    )
...
def _inputs(reference, capacity_gap, epi):
    ...
    if len(x) != 2:
        raise ValueError("P2 feedback requires exactly two ordered scalar EPI coordinates")
...
def _inside(ref, gap, epi):
    ...
    return (ref.epi_lower <= epi[0] <= ref.epi_upper - shift
...
"""
    ...
    metric_reset = c * ref.coupling_factor * gap * difference / (
        (2 * c + gap) * (2 * c + next_gap)
    )
    before_domain = _inside(ref, gap, x)
    event_domain, after_domain = _inside(ref, next_gap, x), _inside(ref, next_gap, after)
    identities = (
        difference_identity, q_identity, mean_change - mean_flow,
        metric_change - metric_reset, metric_after - metric_event,
    )
    if any(identities):
    ...
    return P2CapacityFeedbackCycle(
        ref, gap, x, next_gap, capacity, after,
        (x[0] + k * (c + gap), x[1] + k * c),
        (x[0] + k * (c + next_gap), x[1] + k * c),
        (after[0] + k * (c + next_gap), after[1] + k * c),
        pressure, rate, (s * (c + next_gap), s * c), multiplier,
        difference, after_difference, q, event_q, after_q,
        difference_identity, q_identity,
        mean_before, mean_after, mean_change, mean_flow, mean_change - mean_flow,
        metric_before, metric_event, metric_after, metric_change, metric_reset,
        metric_change - metric_reset, metric_after - metric_event,
        before_domain, event_domain, after_domain,
...
def bound_p2_capacity_feedback(
    reference, *, capacity_gap, epi, cycles,
...
"""
    ...
    initial_tail = (
        ref.step_coefficient * (ref.epi_upper - ref.epi_lower)
        * gap * rho / (2 * ref.coupling_factor)
    )
    ...
    return P2CapacityFeedbackBound(
        ref, gap, x, cycles, next_gap, convolution, q_bound, d_bound,
        mean_tail, mean_interval, d_bound / 2 + mean_tail,
...
def _binary64_platform():
    ...
    if not uses_ieee_binary64_rounding():
        raise ValueError("coupling lattice requires IEEE binary64 round-to-nearest-even")
...
def derive_p2_binary64_coupling_lattice(*, coupling_factor) -> P2Binary64CouplingReference:
...
"""
    ...
    if min(fixed_margin, descent_margin, upper_margin, lower_margin) <= 0:
        raise ValueError("coupling factor does not belong to the proved six-fixed-gap class")
    ...
    return P2Binary64CouplingReference(
        coupling_factor, gain, spacing, maximum, 6, unit,
        fixed_margin, descent_margin, upper_margin, lower_gain, lower_margin,
        horizon, horizon_verified,
    )
...
def observe_p2_binary64_coupling_lattice(
    reference, *, capacity_before, capacity_after,
...
"""
    ...
    initial_index, following_index = (before - 1) / ref.lattice_step, (after - 1) / ref.lattice_step
    ...
    if capacity_after != expected:
        raise ValueError("observed capacity endpoint differs from the shared production kernel")
    j, following = int(initial_index), int(following_index)
    if ((j <= ref.fixed_max_index and following != j)
            or (j > ref.fixed_max_index and not ref.fixed_max_index <= following < j)
            or (j >= 8 and following < 7)):
        raise RuntimeError("production capacity transition violated the proved lattice class")
    ...
    return P2Binary64CouplingObservation(
        ref, capacity_before, capacity_after, j, following, j - following,
        j == following, terminal, float(1 + terminal_gap), terminal_gap,
        0 if j <= ref.fixed_max_index else ref.uniform_horizon,
        ideal_gap, after - 1 - ideal_gap,
    )
