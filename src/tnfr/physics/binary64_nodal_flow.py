...
def _rounding_cell(value, exact_input):
    ...
    previous, following = Fraction.from_float(predecessor), Fraction.from_float(successor)
    ...
    return Binary64AdditionCell(
        value, predecessor, successor, lower, upper, even, exact_input,
        contains, lower_tie, upper_tie,
    )
...
def observe_binary64_unit_quarter_flow(
    *, epi: tuple[float, ...], pressure: tuple[float, ...],
    epi_lower: float = 0.05, epi_upper: float = 1.0,
    ...
    if not uses_ieee_binary64_rounding():
        raise RuntimeError("this observation requires the declared IEEE binary64 rounding behavior")
    ...
    scaling = tuple(actual - expected for actual, expected
                    in zip(represented_increment, exact_increment, strict=True))
    scaling_bound, addition_bound = Fraction(1, 2**1075), Fraction(1, 2**53)
    if any(abs(value) > scaling_bound for value in scaling):
        raise RuntimeError("binary64 power-of-two scaling exceeds its exact rounding bound")
    steps = []
    current = values
    for ordinal in range(1, 5):
        following = tuple(_float(euler_update(value, 0.0625, slope), "Euler endpoint")
                          for value, slope in zip(current, rate, strict=True))
        if any(not lower <= value <= upper for value in following):
            raise ValueError("unclipped Euler output leaves the declared positive EPI band")
        exact_sum = tuple(Fraction.from_float(value) + change
                          for value, change in zip(current, represented_increment, strict=True))
        source_cells = tuple(_rounding_cell(value, total)
                             for value, total in zip(current, exact_sum, strict=True))
        result_cells = tuple(_rounding_cell(value, total)
                             for value, total in zip(following, exact_sum, strict=True))
        errors = tuple(Fraction.from_float(value) - total
                       for value, total in zip(following, exact_sum, strict=True))
        stasis = tuple(before == after for before, after in zip(current, following, strict=True))
        if (not all(cell.contains_exact_input for cell in result_cells)
                or stasis != tuple(cell.contains_exact_input for cell in source_cells)
                or any(abs(error) > addition_bound for error in errors)):
            raise RuntimeError("production Euler disagrees with an exact nearest-even addition cell")
        steps.append(Binary64QuarterSubstep(
            ordinal, current, following, exact_sum, errors, source_cells, result_cells, stasis,
        ))
        current = following
    endpoint = tuple(Fraction.from_float(value) for value in current)
    ideal = tuple(value + force / 4 for value, force in zip(initial, exact_pressure, strict=True))
    defect = tuple(actual - expected for actual, expected in zip(endpoint, ideal, strict=True))
    scaling_defect = tuple(4 * value for value in scaling)
    addition_defect = tuple(sum((step.addition_error[i] for step in steps), Fraction(0))
                            for i in range(len(values)))
    residual = tuple(total - scaled - added for total, scaled, added
                     in zip(defect, scaling_defect, addition_defect, strict=True))
    ...
    mean_before, mean_after, mean_pressure = mean(initial), mean(endpoint), mean(exact_pressure)
    ...
    half_membership = tuple(half_band[0] <= value <= half_band[1] for value in exact_pressure)
    stasis = tuple(before == after for before, after in zip(values, current, strict=True))
    if mean_residual or any(value == 0.5 and frozen != member
                            for value, frozen, member in zip(values, stasis, half_membership, strict=True)):
        raise RuntimeError("binary64 held interval lost its mean or half-EPI stasis identity")
    return Binary64UnitQuarterFlow(
        values, held, lower, upper, rate, increment, initial, exact_pressure,
        exact_increment, scaling, tuple(steps), current, endpoint, ideal, defect,
        scaling_defect, addition_defect, residual, bound, addition_bound, scaling_bound,
        mean_before, mean_after, mean_pressure, mean(scaling_defect), mean(addition_defect),
        mean_defect, mean_residual, stasis, half_band, half_membership,
    )
...
def observe_binary64_paired_c6_diffusion(
    *, epi: tuple[float, ...], epi_weight: float,
    ...
    if len(values) != 6:
        raise ValueError("paired C6 diffusion requires exactly six ordered EPI coordinates")
    if not 0 <= weight <= 1:
    ...
    if any(not lower < value < upper for value in exact):
        raise ValueError("paired C6 EPI must lie strictly inside one common normal binade")
    ...
    if (center / spacing).denominator != 1:
        raise ValueError("the exact pair center must be on the common lattice, not a half-grid point")

    neighbors = tuple(((i - 1) % 6, (i + 1) % 6) for i in range(6))
    pressure = tuple(_float(mean_neighbor_difference(
        values[i], tuple(values[j] for j in row), coefficient=weight,
    ), "scalar EPI pressure") for i, row in enumerate(neighbors))
    source = tuple(i for i in range(6) for _ in neighbors[i])
    target = tuple(j for row in neighbors for j in row)
    vector_pressure = tuple(float(value) for value in edge_mean_differences(
        values, source, target, coefficient=weight,
    ))
    rational_weight = Fraction.from_float(weight)
    ideal_pressure = tuple(-rational_weight * value for value in laplacian_action(exact))
    ...
    if pressure != expected_pressure or vector_pressure != expected_pressure:
        raise RuntimeError("shared pure-EPI reducers disagree with the exact rounded C6 pressure")
    represented_pressure = tuple(Fraction.from_float(value) for value in pressure)
    binding = tuple(value - Fraction.from_float(other)
                    for value, other in zip(represented_pressure, vector_pressure, strict=True))
    pressure_defect = tuple(value - ideal for value, ideal
                            in zip(represented_pressure, ideal_pressure, strict=True))
    pressure_pairs = c6_pair_sums(represented_pressure)
    if any(pressure_pairs):
        raise RuntimeError("shared C6 EPI pressure lost exact opposite-node antisymmetry")

    source_interval = min(exact), max(exact)
    flow = observe_binary64_unit_quarter_flow(
        epi=values, pressure=pressure,
        epi_lower=float(source_interval[0]), epi_upper=float(source_interval[1]),
    )
    substep_pairs = tuple(c6_pair_sums(tuple(Fraction.from_float(value) for value in step.after))
    ...
    interval_preserved = all(source_interval[0] <= Fraction.from_float(value) <= source_interval[1]
                             for step in flow.substeps for value in step.after)
    mean_drift = flow.mean_after - center
    if (not interval_preserved or mean_drift
            or any(pair != source_pairs for pair in substep_pairs)):
        raise RuntimeError("pure-EPI C6 update lost its exact interval or reflected mean invariant")
    return Binary64PairedC6Diffusion(
        values, weight, lower, upper, spacing, center, source_interval, endpoint_interval,
        source_pairs, pressure, vector_pressure, ideal_pressure, pressure_defect, binding,
        pressure_pairs, substep_pairs, endpoint_pairs, mean_drift, flow,
        interval_preserved, True, True,
        source_interval[0] < source_interval[1] and all(flow.stasis), False,
    )
...
def _represented_interval_extrema(lower, upper, lower_closed, upper_closed):
    ...
    if first > last or not math.isfinite(first) or not math.isfinite(last):
        raise RuntimeError("a verified held trace produced an empty represented interval")
...
class Binary64QuarterPressureBox:
    ...
    def contains(self, pressures: tuple[float, ...]) -> bool:
        ...
        return all(cell.contains(value) for cell, value in zip(self.coordinates, values, strict=True))
...
def derive_binary64_quarter_pressure_box(
    *, epi: tuple[float, ...], pressure: tuple[float, ...],
    ...
    """
    flow = observe_binary64_unit_quarter_flow(
        epi=epi, pressure=pressure, epi_lower=epi_lower, epi_upper=epi_upper,
    )
    coordinates = []
    for index, recorded in enumerate(flow.exact_pressure):
        intervals = tuple(
            (step.result_cells[index].lower - Fraction.from_float(step.before[index]),
             step.result_cells[index].upper - Fraction.from_float(step.before[index]),
        ...
        first, last = _represented_interval_extrema(lower, upper, lower_closed, upper_closed)
        if not -2 < first <= last < 2:
            raise RuntimeError("a positive unit-band EPI trace lost its bounded increment range")
        ...
        pressure_lower_closed, pressure_upper_closed = first_cell.even_significand, last_cell.even_significand
        first_pressure, last_pressure = _represented_interval_extrema(
            pressure_lower, pressure_upper, pressure_lower_closed, pressure_upper_closed,
        )
        margin = min(recorded - pressure_lower, pressure_upper - recorded)
        cell = Binary64PressureTraceCell(
            lower, upper, lower_closed, upper_closed, first, last,
            pressure_lower, pressure_upper, pressure_lower_closed, pressure_upper_closed,
            first_pressure, last_pressure, margin,
        )
