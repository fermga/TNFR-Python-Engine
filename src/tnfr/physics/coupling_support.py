...
from .support_transport import (
    SupportTransportSnapshot, _energy, _laplacian, observe_support_transport,
)

__all__ = [
    "CompatibleCapacityBalance", "CouplingSupportObservation",
    "derive_compatible_capacity_balance", "observe_coupling_support",
...
def _node_order(nodes):
    ...
    if not result:
        raise ValueError("compatible-support observation requires a nonempty node order")
...
def _neighbor_rows(neighbors, size, *, allow_empty):
    rows = tuple(_ordered(row, "neighbor row") for row in _ordered(neighbors, "neighbors"))
    if len(rows) != size:
    ...
    for i, row in enumerate(rows):
        ...
        if (any(type(j) is not int or not 0 <= j < size or j == i for j in row)
                or len(set(row)) != len(row)):
            raise ValueError("neighbor rows must contain distinct loop-free valid indices")
...
def derive_compatible_capacity_balance(
    *, nodes, neighbors, capacity, coupling_factor,
...
"""
    ...
    gradient = tuple(-value / degree for value, degree in zip(bnu, degrees, strict=True))
    after = tuple(value + gamma * delta for value, delta in zip(nu, gradient, strict=True))
    ...
    mean_residuals = tuple(right - left for left, right in zip(means_before, means_after))
    fixed = after == nu
    constant = all(all(nu[i] == nu[component[0]] for i in component) for component in components)
    before_energy, after_energy = _energy(conductance, nu), _energy(conductance, after)
    norm_squared = sum((value**2 / degree for value, degree in zip(bnu, degrees)), Fraction(0))
    ...
    if (fixed != constant or identity or any(mean_residuals) or change > upper
            or (not fixed and change >= 0) or any(value < 0 for value in after)):
        raise RuntimeError("exact compatible-capacity balance lost a component or energy identity")
    return CompatibleCapacityBalance(
        ordered, rows, nu, gamma, degrees, components,
        means_before, means_after, mean_residuals, gradient, after,
        fixed, constant, before_energy, after_energy, drift, quadratic,
        change, identity, norm_squared, upper,
    )
...
"""
    if G.is_directed() or G.is_multigraph():
        raise ValueError("coupling-support observation requires a simple undirected graph")
    ...
    if not bool(G.graph.get("UM_SYNC_VF", True)):
        raise ValueError("coupling-support capacity analysis requires enabled UM_SYNC_VF")
    ...
    factors = resolve_runtime_operator_factors(G.graph.get("GLYPH_FACTORS"), Glyph.UM, G.graph)
    ...
    phases = tuple(finite_represented_real(
        get_attr(G.nodes[node], ALIAS_THETA, None, conv=lambda value: value, strict=True),
        "phase",
    ) for node in nodes)
    raw_rows = tuple(tuple(G.neighbors(node)) for node in nodes)
    selections = tuple(resolve_u3_phase_neighbors(
        G.graph, phases[i][0], row,
        phase_getter=lambda neighbor: phases[index[neighbor]][0],
        operator_code="UM", require_compatible=False,
    ) for i, row in enumerate(raw_rows))
    limits = tuple(finite_represented_real(item.effective_limit, "effective phase limit")[1]
    ...
    excluded = tuple(sorted((i, j) for i, row in enumerate(ordered_support)
                            for j in row if i < j and j not in rows[i]))
    blocked = tuple(nodes[i] for i, row in enumerate(rows) if not row)
    balance = None if blocked else derive_compatible_capacity_balance(
        nodes=nodes, neighbors=rows, capacity=snapshot.capacity, coupling_factor=gamma,
    )
    return CouplingSupportObservation(
        snapshot, tuple(value[1] for value in phases), ordered_support,
        rows, excluded, _components(rows), blocked, limits[0], gamma, balance,
    )
...
def derive_antipodal_region_phase_balance(
    *, coupling_phase_factor, coherence_phase_factor,
    ...
    coherence_full = derive_phase_response(
        cosine_gram=gram, mean_neighbors=full,
    ...
    def restrict(matrix):
        ...
        for column in columns:
            ...
            if column != (a, a, b, -b, -a, -a):
                raise RuntimeError("shared phase response left the antipodal tangent subspace")
    ...
    identity_det = (1 - product[0][0]) * (1 - product[1][1]) - product[0][1] * product[1][0]
    expected = -alpha**2 * (1 - t) - 2 * alpha * t / 3
    strict = identity_det < 0
    if identity_det != expected or identity_det != 1 - trace + determinant or strict != (alpha > 0):
        raise RuntimeError("exact antipodal phase response lost its characteristic identity")
    return AntipodalRegionPhaseBalance(
        t, alpha, coupling, coherence, product, trace, determinant, identity_det, strict,
    )
...
def observe_antipodal_region_phase_response(
    ...
    return AntipodalRegionPhaseResponse(
        reference, before, coupled, after, embedded_before, embedded_after,
