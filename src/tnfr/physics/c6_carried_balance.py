...
from .c6_pressure_lattice import (
    C6PressureLatticeObservation, _observe_rebuilt_c6_pressure_lattice,
)
...
def _point(closure, state):
    ...
    if (state.epi_lower, state.epi_upper) != (tube.state.epi_lower, tube.state.epi_upper):
        raise ValueError("the static state's declared band must equal the rebuilt closure band")
    state = NodalRemainderState(state.epi, state.remainder, state.epi_lower, state.epi_upper)
    ...
    pattern = observe_forced_support_pattern(
        profile.forced_balance, nodes=tuple(range(6)), epi=exact,
    )
    energy = dot(pattern.relative_error, pattern.relative_error)
    if energy > closure.energy_bound:
        raise ValueError("the static point lies outside the rebuilt centered energy envelope")
    return C6CarriedPressurePoint(state, observation, pattern.relative_error, energy)


def observe_c6_carried_pressure_point(closure, *, state: NodalRemainderState) -> C6CarriedPressurePoint:
...
def derive_c6_carried_pressure_balance(
    closure, *, states: tuple[NodalRemainderState, ...],
    ...
    if type(states) is not tuple or len(states) != 7:
        raise ValueError("the affine pressure witness requires exactly seven ordered states")
    points = tuple(_point(bound, state) for state in states)
    pressures = tuple(tuple(F(value) for value in point.observation.pressure) for point in points)
    matrix = tuple(tuple(row[i] for row in pressures) for i in range(6)) + ((F(1),) * 7,)
    inverse = exact_matrix_inverse(matrix)
    weights = tuple(row[-1] for row in inverse)
    if any(value <= 0 for value in weights):
        raise ValueError("the seven canonical pressures must admit strictly positive exact weights")
    total = sum(weights, F(0))
    balance = tuple(sum((weight * row[i] for weight, row in zip(weights, pressures, strict=True)), F(0))
                    for i in range(6))
    if total != 1 or any(balance):
        raise RuntimeError("the exact affine inverse lost its normalized pressure balance")
    means = tuple(sum(_validate_nodal_remainder_state(point.state), F(0)) / 6 for point in points)
    ...
    return C6CarriedPressureBalance(bound, points, weights, total, balance, means, common, matches)
