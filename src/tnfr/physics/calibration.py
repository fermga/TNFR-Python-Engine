...
def create_topology_calibration_profiles() -> dict[str, CalibrationProfile]:
    ...
    profiles["WS"] = CalibrationProfile(
        ...
        expected_correlation=PHYSICS_EXPECTED_CORRELATION_WS_CANONICAL,  # = 0.1 (WS correlation)
        correlation_std=0.133,
        parameter_dependencies={
            "n_nodes": PHYSICS_N_NODES_DEPENDENCY_CANONICAL,  # ≈ 0.0418 (weak dependency on network size)
            "k_degree": PHYSICS_K_DEGREE_DEPENDENCY_CANONICAL,  # = -0.16 (stronger dependency on connectivity)
            "p_rewire": PHYSICS_P_REWIRE_DEPENDENCY_CANONICAL,  # ≈ 0.0897 (moderate dependency on rewiring probability)
        },
        sample_size=32,
...
def calibrate_tc_xi_correlation(
    G: nx.Graph, topology_type: str, network_params: dict[str, Any]
) -> dict[str, float]:
    """
    ...
    margin_of_error = (
        t_multiplier * profile.correlation_std / np.sqrt(profile.sample_size)
    )
...
def validate_calibration_accuracy(
    ...
    return {
        ...
        "calibration_quality": (
            "EXCELLENT"
            if within_tolerance and within_bounds
            else "GOOD" if within_bounds else "NEEDS_REFINEMENT"
        ),
    }
...
SUPPORTED_TOPOLOGIES = {
    ...
    "BA": {
        ...
        "optional_params": [],
        "param_ranges": {"n_nodes": (10, 200), "m_attach": (1, 10)},
    },
    "Grid": {
        "name": "Grid_2D",
        "required_params": ["n_side"],
        "optional_params": [],
        "param_ranges": {"n_side": (3, 20)},
    },
}
