...
detectar cambios significativos.
    ...
    # Asegurar valores previos usando conversiones seguras
    if "EPI_prev" not in nodo:
        epi_actual = _as_float(nodo.get("EPI", 1.0), 1.0)
        nodo["EPI_prev"] = epi_actual if _safe_isfinite(epi_actual) else 1.0
    if "θ_prev" not in nodo:
        nodo["θ_prev"] = _as_float(nodo.get("θ", 0.0), 0.0)
...
def detectar_macronodos(
    ...
    for idx, grupo in enumerate(grupos):
        ...
        # EPI media categorizada usando conversiones seguras
        ...
        # Conexiones: acople por fase/EPI/νf/Si (al menos 3 condiciones) usando conversiones seguras
...
def algo_se_mueve(G: Any, historial_glifos_por_nodo: Dict[Any, List[Tuple[int, str]]], paso: int, umbral: float = 0.01) -> bool:
    ...
    for nodo in G.nodes:
        ...
        # Cambio reciente de EPI usando conversiones seguras
...
def evaluar_si_nodal(nodo: Dict[str, Any], paso: int | None = None) -> float:
    ...
    """
    # Factores base usando conversiones seguras
    vf = _as_float(nodo.get("νf", 1.0), 1.0)
    dNFR = _as_float(nodo.get("ΔNFR", 0.0), 0.0)
