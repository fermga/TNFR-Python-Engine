...
def _extract_graph_from_args(args: tuple, kwargs: dict) -> Optional[Any]:
...
def _compute_dependency_hash(graph: Any, dependencies: set[str]) -> str:
    ...
    node attributes like phase and delta_nfr.
    """
    if not dependencies or graph is None:
        return ""
    
    # Only hash if we have specific node property dependencies
    relevant_deps = {d for d in dependencies if d.startswith('node_')}
    ...
    sorted_deps = sorted(relevant_deps)
