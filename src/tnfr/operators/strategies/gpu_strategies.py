    ...
    def apply(self, prepared: PreparedBlock) -> OperationResult:
        ...
        try:
            ...
            for node in graph.nodes():
                ...
                set_attr(graph.nodes[node], ALIAS_VF, min(5.0, current_vf * 1.1))
    ...
    def apply(self, prepared: PreparedBlock) -> OperationResult:
        ...
        try:
            ...
            for node in graph.nodes():
                ...
                node_phase = get_attr(graph.nodes[node], ALIAS_THETA, 0.0)
                neighbor_phases = [
                    get_attr(graph.nodes[n], ALIAS_THETA, 0.0) for n in neighbors
                ]
                ...
                set_attr(graph.nodes[node], ALIAS_VF, current_vf * amplification)
...
def get_gpu_strategy_recommendations(graph_size: int) -> dict[str, Any]:
    ...
    try:
        ...
        recommendations["memory_estimate_mb"] = (
            graph_size * graph_size * 8 / (1024 * 1024)
        )
