...
This module consolidates the previously separate grammar systems (C1-C3 and RC1-RC4)
into a single, unified set of canonical constraints that emerge inevitably from
TNFR physics.
...
class UnifiedGrammarValidator:
    """Validates sequences using unified canonical constraints.
    ...
    def validate_transformer_context(sequence: List[Operator]) -> tuple[bool, str]:
        ...
        if violations:
            return (
                False,
                f"U4b violated: {'; '.join(violations)}"
            )
        
        return (
            True,
            f"U4b satisfied: transformers have proper context"
        )
...
def validate_unified(
    ...
    """Validate sequence using unified canonical constraints.
    ...
    -------
    bool
        True if sequence satisfies all unified constraints
    ...
    """
    is_valid, _ = UnifiedGrammarValidator.validate(sequence, epi_initial)
    return is_valid
