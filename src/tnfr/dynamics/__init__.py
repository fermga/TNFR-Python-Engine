...
def adapt_vf_by_coherence(G) -> None:
    ...
    tau = int(get_param(G, "VF_ADAPT_TAU"))
    mu = float(get_param(G, "VF_ADAPT_MU"))
...
def parametric_glyph_selector(G, n) -> str:
    ...
    nd = G.nodes[n]
    thr = _selector_thresholds(G)
    margin = float(get_param(G, "GLYPH_SELECTOR_MARGIN"))
...
def _apply_glyphs(G, selector, hist) -> None:
    ...
    use_canon = bool(get_param(G, "GRAMMAR_CANON").get("enabled", False))
    al_max = int(get_param(G, "AL_MAX_LAG"))
    en_max = int(get_param(G, "EN_MAX_LAG"))
...
def _update_nodes(
    ...
    *,
    ...
    _dt = float(get_param(G, "DT")) if dt is None else float(dt)
