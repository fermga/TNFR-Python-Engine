...
class TNFROptimizationOrchestrator:
    ...
    def _execute_nodal_optimization(
        ...
        evolution_results = self.nodal_optimizer.compute_vectorized_nodal_evolution(
            G, dt
        )
    ...
    def _execute_structural_optimization(
        ...
        return OptimizationResult(
            ...
            execution_time=execution_time,
            speedup_factor=(
                OPT_ORCH_CACHE_SPEEDUP_CANONICAL if stats["hits"] > 0 else 1.0
            ),  # π ≈ 3.1416 → canonical
    ...
    def _execute_hybrid_optimization(
        ...
        if self.structural_cache:
            struct_result = self._execute_structural_optimization(
                G, operation, **kwargs
            )
        ...
        if results:
            ...
            best_result.details["combined_strategies"] = [
    ...
    def _execute_auto_optimization(
        self, G: Any, operation: str, **kwargs
    ) -> OptimizationResult:
