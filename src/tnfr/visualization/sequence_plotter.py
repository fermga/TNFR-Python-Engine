...
class SequenceVisualizer:
    ...
    def plot_sequence_flow(
        ...
        # Add legend
        legend_elements = [
            mpatches.Patch(color=COMPATIBILITY_COLORS[CompatibilityLevel.EXCELLENT],
                          label="Excellent transition"),
            mpatches.Patch(color=COMPATIBILITY_COLORS[CompatibilityLevel.GOOD],
                          label="Good transition"),
            mpatches.Patch(color=COMPATIBILITY_COLORS[CompatibilityLevel.CAUTION],
                          label="Caution transition"),
            mpatches.Patch(color=COMPATIBILITY_COLORS[CompatibilityLevel.AVOID],
    ...
    def plot_health_dashboard(
        ...
        bars2 = ax_bars.bar(
            x_pos + width/2, benchmarks, width, label="Benchmark", color="#95a5a6", alpha=0.6
        )
        ...
        ax_bars.set_xticklabels([label[:4] for label in metrics_labels], rotation=45, ha="right", fontsize=8)
        ...
        ax_gauge.text(
            0.5, 0.7, f"Overall Health: {overall:.3f}",
    ...
    def plot_pattern_analysis(
        ...
        for i, op in enumerate(normalized):
            ...
            display_name = operator_display_name(op) or op
            ax.text(
                x_positions[i], y_base, display_name,
            ...
            ax.text(
                x_positions[i], y_base - 0.15, category,
        ...
        ax.text(
            0.5, 0.85, f"Detected Pattern: {pattern.upper()}",
            ha="center", va="center", fontsize=14, weight="bold",
