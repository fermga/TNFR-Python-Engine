...
def benchmark_basic(max_n: int = 10000, sample_size: int = 100) -> dict:
    ...
    if max_n > 100:
        test_numbers.extend(
            random.sample(range(100, min(max_n, 10000)), min(sample_size, max_n - 100))
        )
...
Examples:
  ...
  tnfr-is-prime --timing 17 97           # With timing
    ...
    parser.add_argument(
        "--cached", action="store_true", help="Use cached implementation"
    )
    ...
    if args.benchmark:
        ...
        if args.stats:
            ...
            for func_name, cache_info in results["cache_info"].items():
                ...
                print(
                    f"  Hit rate: {cache_info['hits']/(cache_info['hits']+cache_info['misses']):.1%}"
                )
    ...
    for n in args.numbers:
        ...
        if args.timing:
            ...
            print(
                f"{n:12d}  {str(is_prime):>11}  {delta_nfr:14.6f}  {elapsed_us:10.2f}"
            )
        else:
