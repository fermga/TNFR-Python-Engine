...
def _asegurar_defaults_nodo(nodo: Dict[str, Any]):
    ...
    for k, v in defaults.items():
        if k not in nodo or not np.isfinite(nodo.get(k, v)):
            nodo[k] = v
...
def actualizar_EPI_por_ecuacion_nodal(G: nx.Graph, dt: float = 1.0) -> None:
    ...
    for n in G.nodes:
        ...
        dEPI_dt = float(nodo.get("νf", 1.0)) * float(nodo.get("ΔNFR", 0.0))
        prev_dEPI_dt = float(nodo.get("dEPI_dt", 0.0))

        # Historial EPI
        nodo["EPI_prev3"] = nodo.get("EPI_prev2", nodo.get("EPI", 1.0))
        nodo["EPI_prev2"] = nodo.get("EPI_prev", nodo.get("EPI", 1.0))
        nodo["EPI_prev"] = nodo.get("EPI", 1.0)

        # Integración explícita (Euler) con saneamiento numérico
        EPI_new = nodo["EPI"] + dt * dEPI_dt
        if not np.isfinite(EPI_new):
            EPI_new = nodo["EPI"]  # no avanzar ante NaN/inf
        nodo["EPI"] = EPI_new
...
# =========================================================================================
# CLAMPS CANÓNICOS (sanity pass post-tick)
# =========================================================================================
...
def aplicar_clamps_canonicos(G: nx.Graph, umbrales: Optional[Dict[str, float]] = None) -> None:
    ...
    """
    EPI_max = (
        float(umbrales.get("EPI_max_dinamico", EPI_MAX_GLOBAL)) if umbrales else EPI_MAX_GLOBAL
    )
    max_abs_delta = (
        float(umbrales.get("bifurcacion_gradiente", DELTA_NFR_MAX_ABS)) if umbrales else DELTA_NFR_MAX_ABS
    )
    ...
    for n in G.nodes:
        ...
        # Sanitizar y acotar
        epi = nodo.get("EPI", 1.0)
        si = nodo.get("Si", 0.5)
        vf = nodo.get("νf", 1.0)
        dln = nodo.get("ΔNFR", 0.0)
...
class TemporalCoordinatorTNFR:
    ...
    def sincronizar_grupo_resonante(self, G: nx.Graph, grupo: Dict[str, Any]) -> int:
        ...
        for nodo_id in nodos:
            ...
            factor_correccion = 0.1 * G.nodes[nodo_id].get("Si", 0.5)  # más Si = más atraído
    ...
    def generar_pulso_reorganizacion_global(self, G: nx.Graph, paso_global: int) -> bool:
        ...
        for nodo_id in G.nodes:
            ...
            fase_actual = self.fases_sincronizacion.get(nodo_id, 0.0)
            vf = G.nodes[nodo_id].get("νf", 1.0)
            tiempo_normalizado = self.tiempo_topologico % (4 * np.pi)
            fase_objetivo = (vf * tiempo_normalizado) % (2 * np.pi) if np.isfinite(vf) else 0.0
...
class BifurcationManagerTNFR:
        ...
        try:
            ...
            if glifo == "VAL":
                ...
                trayectoria.parametros_iniciales["EPI"] = (
                    trayectoria.parametros_iniciales.get("EPI", 1.0) * 0.8 + epi_objetivo * 0.2
                )
            elif glifo == "THOL":
                for param in ["EPI", "νf", "Si"]:
                    if param in trayectoria.parametros_iniciales:
                        v = trayectoria.parametros_iniciales[param]
    ...
    def _evaluar_viabilidad_trayectoria(self, trayectoria: TrayectoriaBifurcacion) -> float:
        ...
        try:
            ...
            ratio_equilibrio = min(epi / vf, vf / epi) if vf > 0 else 0.0
        ...
        try:
            ...
            for param, valor in trayectoria.parametros_iniciales.items():
                if param in ["EPI", "νf", "Si", "ΔNFR"] and np.isfinite(valor):
                    nodo_data[param] = max(0.1, min(3.0, float(valor)))
            ...
            return {"exitosa": True, "configuracion_final": trayectoria.parametros_iniciales.copy(), "nodos_generados": [nodo_origen_id]}
        except Exception:
            return {"exitosa": False}
    ...
    def _fusionar_trayectorias_compatibles(self, trayectorias: List[TrayectoriaBifurcacion], G: nx.Graph, nodo_origen_id: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            for param in ["EPI", "νf", "Si", "ΔNFR"]:
                suma_ponderada = sum(t.parametros_iniciales.get(param, 1.0) * t.viabilidad for t in trayectorias)
                valor_fusionado = suma_ponderada / total_viabilidad
                if np.isfinite(valor_fusionado):
                    configuracion_fusionada[param] = max(0.1, min(3.0, float(valor_fusionado)))
...
def calcular_umbrales_dinamicos(C_t: float, densidad_nodal: float, fase_simulacion: str = "emergencia") -> Dict[str, float]:
    ...
    return {
        ...
        "EPI_max_dinamico": max(2.5, C_t * 2.8),
        "EPI_min_coherencia": max(0.4, C_t * 0.3),
...
def evaluar_condiciones_emergencia_dinamica(nodo: Dict[str, Any], umbrales: Dict[str, float], campo_coherencia: float) -> tuple[bool, str]:
    ...
    if nodo.get("νf", 0) < 0.3:
        return False, f"Frecuencia demasiado baja: {nodo.get('νf', 0):.3f}"

    # **Usar θ (fase) canónica**
    theta_nodo = nodo.get("θ", 0.5)
    if abs(theta_nodo - 0.0) > 0.7 and campo_coherencia > 1.2:
        return False, f"Disonancia con campo: θ={theta_nodo:.3f}, C(t)={campo_coherencia:.3f}"

    dNFR_abs = abs(nodo.get("ΔNFR", 0))
    if dNFR_abs > umbrales["bifurcacion_gradiente"]:
        return False, f"Gradiente excesivo: {dNFR_abs:.3f} > {umbrales['bifurcacion_gradiente']:.3f}"
...
def detectar_fase_simulacion(G: nx.Graph, paso_actual: int, historial_C_t: List[tuple[int, float]], ventana: int = 50) -> str:
    ...
    variabilidad = float(np.std(valores_recientes))
    tendencia = float(np.mean(valores_recientes[-10:]) - np.mean(valores_recientes[:10]))
...
def evaluar_activacion_glifica_dinamica(nodo: Dict[str, Any], umbrales: Dict[str, float], vecinos_data: Optional[List[Dict[str, Any]]] = None) -> Optional[str]:
    ...
    theta_actual = nodo.get("θ", 0.0)
    theta_prev = nodo.get("θ_prev", theta_actual)
    if abs(theta_actual - theta_prev) > umbrales["θ_mutacion"]:
        return "ZHIR"

    # SH'A — Colapso por baja coherencia + gradiente alto (usa umbral de GRADIENTE)
    if (nodo.get("EPI", 0.0) < umbrales.get("EPI_min_coherencia", 0.4)
            and abs(nodo.get("ΔNFR", 0.0)) > umbrales.get("bifurcacion_gradiente", 0.8)):
        return "SHA"

    # T'HOL — Autoorganización por aceleración estructural
    aceleracion = abs(nodo.get("d2EPI_dt2", 0.0))
    if aceleracion > umbrales.get("bifurcacion_aceleracion", 0.15):
        return "THOL"

    # R'A — Resonancia con vecinos (si están muy sincronizados en θ)
    if vecinos_data:
        thetas = [v.get("θ", 0.0) for v in vecinos_data]
        if thetas:
            resonancia_promedio = sum(abs(theta_actual - tv) for tv in thetas) / len(thetas)
            if resonancia_promedio < umbrales["θ_conexion"] * 0.5:
                return "RA"
...
def gestionar_conexiones_canonico(G: nx.Graph, paso: int, historia_Ct: List[tuple[int, float]]):
    ...
    C_t = sum(G.nodes[n]["EPI"] for n in G.nodes) / len(G) if len(G.nodes) else 0.0
